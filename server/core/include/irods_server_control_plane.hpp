...
namespace irods
{
    ...
    private:
        ...
        error process_operation(const zmq::message_t&, // incoming msg
        ...
        error process_host_list(const std::string&,                           // command
                                const std::string&,                           // wait option
                                const size_t&,                                // wait time in seconds
                                const host_list_t&,                           // hostnames
                                std::string&);                                // outgoing text
        error validate_host_list(const host_list_t&,                          // irods hostnames
                                 const host_list_t&,                          // incoming hostnames
                                 host_list_t&);                               // valid hostnames
        error get_resource_host_names(host_list_t&);                          // fetched hostnames
        error notify_icat_and_local_servers_preop(const std::string&,         // command
                                                  const std::string&,         // command option
                                                  const std::string&,         // wait option
                                                  const size_t&,              // wait seconds
                                                  const host_list_t&,         // irods hostnames
                                                  std::string&);              // output
        error notify_icat_and_local_servers_postop(const std::string&,        // command
                                                   const std::string&,        // command option
                                                   const std::string&,        // wait option
                                                   const size_t&,             // wait seconds
                                                   const host_list_t&,        // irods hostnames
                                                   std::string&);             // output
        error forward_command(const std::string&,                             // command name
        ...
        bool compare_host_names(const std::string&,  // host name 1
                                const std::string&); // host name 2

        bool is_host_in_list(const std::string&,  // host name in question
