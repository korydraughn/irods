                   ...
                   int ignoreCondInput)
{
    ...
    sqlResult_t *dataId, *collId, *replNum, *version, *dataType, *dataSize, *hierString, *filePath, *dataOwnerName,
        ...
        *dataModify, *dataMode, *dataName, *collName;
    ...
    *dataObjInfoHead = NULL;
    ...
    qcondCnt = initDataObjInfoQuery(dataObjInp, &genQueryInp, ignoreCondInput);
    ...
    if (status < 0) {
        if (status != CAT_NO_ROWS_FOUND) {
            rodsLog(LOG_NOTICE, "getDataObjInfo: rsGenQuery error, status = %d", status);
        }
    ...
    if (genQueryOut == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: NULL genQueryOut");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }

    if ((dataOwnerName = getSqlResultByInx(genQueryOut, COL_D_OWNER_NAME)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_D_OWNER_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataName = getSqlResultByInx(genQueryOut, COL_DATA_NAME)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_DATA_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((collName = getSqlResultByInx(genQueryOut, COL_COLL_NAME)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_COLL_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataId = getSqlResultByInx(genQueryOut, COL_D_DATA_ID)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_D_DATA_ID failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((collId = getSqlResultByInx(genQueryOut, COL_D_COLL_ID)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_D_COLL_ID failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((replNum = getSqlResultByInx(genQueryOut, COL_DATA_REPL_NUM)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_DATA_REPL_NUM failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((version = getSqlResultByInx(genQueryOut, COL_DATA_VERSION)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_DATA_VERSION failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataType = getSqlResultByInx(genQueryOut, COL_DATA_TYPE_NAME)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_DATA_TYPE_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataSize = getSqlResultByInx(genQueryOut, COL_DATA_SIZE)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_DATA_SIZE failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((hierString = getSqlResultByInx(genQueryOut, COL_D_RESC_HIER)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_D_RESC_HIER failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((filePath = getSqlResultByInx(genQueryOut, COL_D_DATA_PATH)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_D_DATA_PATH failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataOwnerZone = getSqlResultByInx(genQueryOut, COL_D_OWNER_ZONE)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_D_OWNER_ZONE failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((replStatus = getSqlResultByInx(genQueryOut, COL_D_REPL_STATUS)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_D_REPL_STATUS failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((statusString = getSqlResultByInx(genQueryOut, COL_D_DATA_STATUS)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_D_DATA_STATUS failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((chksum = getSqlResultByInx(genQueryOut, COL_D_DATA_CHECKSUM)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_D_DATA_CHECKSUM failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataExpiry = getSqlResultByInx(genQueryOut, COL_D_EXPIRY)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_D_EXPIRY failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataMapId = getSqlResultByInx(genQueryOut, COL_D_MAP_ID)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_D_MAP_ID failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataComments = getSqlResultByInx(genQueryOut, COL_D_COMMENTS)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_D_COMMENTS failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataCreate = getSqlResultByInx(genQueryOut, COL_D_CREATE_TIME)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_D_CREATE_TIME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataModify = getSqlResultByInx(genQueryOut, COL_D_MODIFY_TIME)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_D_MODIFY_TIME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataMode = getSqlResultByInx(genQueryOut, COL_DATA_MODE)) == NULL) {
        rodsLog(LOG_NOTICE, "getDataObjInfo: getSqlResultByInx for COL_DATA_MODE failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    ...
    *downCurrentInfo = *downOldInfo = NULL;
    ...
    while (tmpDataObjInfo != NULL) {
        ...
        if (!strlen(tmpDataObjInfo->rescName)) {
            ...
            irods::get_resource_property<rodsServerHost_t*>(
                tmpDataObjInfo->rescId, irods::RESOURCE_HOST, rodsServerHost);
        ...
        irods::error prop_err =
            irods::get_resource_property<std::string>(tmpDataObjInfo->rescId, irods::RESOURCE_CLASS, class_type);
...
int sortObjInfoForOpen(dataObjInfo_t** dataObjInfoHead, keyValPair_t* condInput, int writeFlag)
{
    ...
    if (!resc_hier) {
        ...
        for (dataObjInfo_t* dataObjInfo = *dataObjInfoHead; result >= 0 && found_info == NULL && dataObjInfo != NULL;
...
int sortDataObjInfoRandom(dataObjInfo_t** dataObjInfoHead)
{
    ...
    if (numInfo > 50) {
        rodsLog(LOG_NOTICE, "sortDataObjInfoRandom: numInfo %d > 50, setting it to 50", numInfo);
        numInfo = 50;
    }
                             ...
                             const char* destRescHier /* from optional condInput */)
{
    ...
    while (tmpDataObjInfo != NULL) {
        // No longer good enough to check if the resource names are the same. We have to verify that the resource
        // hierarchies match as well. - hcj
...
int sortObjInfoForRepl(dataObjInfo_t** dataObjInfoHead,    // RESC_HIER
                       ...
                       const char* dst_resc_hier)
{
    ...
    *oldDataObjInfoHead = NULL;
    if (resc_hier && dst_resc_hier && strcmp(dst_resc_hier, resc_hier) != 0) {
        ...
        while (tmp_info) {
            ...
            if (0 == strcmp(dst_resc_hier, tmp_info->rescHier)) {
                ...
                // queue-ing it up
                if (irods::is_hier_live(dst_resc_hier)
                        .ok() /* INT_RESC_STATUS_DOWN != tmp_info->rescInfo->rescStatus */) {
                    if (prev_info == NULL) {
                    ...
                    rodsLog(LOG_ERROR, "sortObjInfoForRepl - destination resource is down [%s]", dst_resc_hier);
                    return -1;
                }
            }
            else {
    ...
    dataObjInfo_t *currentArchInfo = 0, *currentCacheInfo = 0, *oldArchInfo = 0, *oldCacheInfo = 0,
                  *downCurrentInfo = 0, *downOldInfo = 0;

    sortObjInfo(dataObjInfoHead,
                &currentArchInfo,
                &currentCacheInfo,
                &oldArchInfo,
                &oldCacheInfo,
...
int initDataObjInfoQuery(dataObjInp_t* dataObjInp, genQueryInp_t* genQueryInp, int ignoreCondInput)
{
    ...
    if ((tmpStr = getValByKey(&dataObjInp->condInput, QUERY_BY_DATA_ID_KW)) == NULL) {
        memset(myColl, 0, MAX_NAME_LEN);
        memset(myData, 0, MAX_NAME_LEN);

        if ((status = splitPathByKey(dataObjInp->objPath, myColl, MAX_NAME_LEN, myData, MAX_NAME_LEN, '/')) < 0) {
            rodsLog(LOG_NOTICE,
    ...
    if (ignoreCondInput == 0 && (tmpStr = getValByKey(&dataObjInp->condInput, REPL_NUM_KW)) != NULL) {
...
int chkOrphanFile(rsComm_t* rsComm, char* filePath, const char* rescName, dataObjInfo_t* dataObjInfo)
{
    ...
    if (status < 0) {
        ...
        if ((collName = getSqlResultByInx(genQueryOut, COL_COLL_NAME)) == NULL) {
            rodsLog(LOG_NOTICE, "chkOrphanFile: getSqlResultByInx for COL_COLL_NAME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }

        if ((dataName = getSqlResultByInx(genQueryOut, COL_DATA_NAME)) == NULL) {
            rodsLog(LOG_NOTICE, "chkOrphanFile: getSqlResultByInx for COL_DATA_NAME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }

        if ((dataId = getSqlResultByInx(genQueryOut, COL_D_DATA_ID)) == NULL) {
            rodsLog(LOG_NOTICE, "chkOrphanFile: getSqlResultByInx for COL_D_DATA_ID failed");
            return UNMATCHED_KEY_OR_INDEX;
        }

        if ((replNum = getSqlResultByInx(genQueryOut, COL_DATA_REPL_NUM)) == NULL) {
            rodsLog(LOG_NOTICE, "chkOrphanFile: getSqlResultByInx for COL_DATA_REPL_NUM failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        ...
        if (dataObjInfo != NULL) {
            ...
            snprintf(dataObjInfo->objPath, MAX_NAME_LEN, "%s/%s", collName->value, dataName->value);
...
int chkOrphanDir(rsComm_t* rsComm, char* dirPath, const char* rescName)
{
    ...
    if (!exists(srcDirPath) || !is_directory(srcDirPath)) {
        rodsLog(LOG_ERROR, "chkOrphanDir: opendir error for %s, errno = %d", dirPath, errno);
        return UNIX_FILE_OPENDIR_ERR - errno;
    }
    directory_iterator end_itr; // default construction yields past-the-end
    for (directory_iterator itr(srcDirPath); itr != end_itr; ++itr) {
        path p = itr->path();
        snprintf(subfilePath, MAX_NAME_LEN, "%s", p.c_str());
        if (!exists(p)) {
            rodsLog(LOG_ERROR, "chkOrphanDir: stat error for %s, errno = %d", subfilePath, errno);
                          ...
                          keyValPair_t* condInput)
{
    ...
    status = matchDataObjInfoByCondInput(
        dataObjInfoHead, oldDataObjInfoHead, condInput, &matchedDataObjInfo, &matchedOldDataObjInfo);
    if (status < 0) {
        return status;
    }
    ...
    if ((*destDataObjInfo = chkCopyInResc(*dataObjInfoHead, _resc_name, destRescHier)) != NULL) {
    ...
    if (getValByKey(condInput, ALL_KW) != NULL) {
        ...
        matchAndTrimRescGrp(
            oldDataObjInfoHead, _resc_name, TRIM_MATCHED_RESC_INFO | TRIM_UNMATCHED_OBJ_INFO, &trimmedDataObjInfo);
        ...
        *destDataObjInfo = chkCopyInResc(*oldDataObjInfoHead, _resc_name, destRescHier);
        if (*destDataObjInfo != NULL) {
            ...
            matchAndTrimRescGrp(oldDataObjInfoHead, _resc_name, TRIM_MATCHED_RESC_INFO, NULL);
                        ...
                        int multiCopyFlag)
{
    ...
    if (multiCopyFlag) {
        matchAndTrimRescGrp(dataObjInfoHead, _resc_name, REQUE_MATCHED_RESC_INFO, NULL);
        matchAndTrimRescGrp(oldDataObjInfoHead, _resc_name, REQUE_MATCHED_RESC_INFO, NULL);
    }
    else {
    ...
    if (_resc_name.empty()) {
        if (*dataObjInfoHead == NULL) {
            ...
            rodsLog(LOG_ERROR, "resolveInfoForPhymv: %s already have copy in the resc", (*dataObjInfoHead)->objPath);
...

{
    ...
    if (dataObjInfoHead == NULL || *dataObjInfoHead == NULL || oldDataObjInfoHead == NULL ||
        matchedDataObjInfo == NULL || matchedOldDataObjInfo == NULL) {
        rodsLog(LOG_ERROR, "requeDataObjInfoByCondInput: NULL dataObjInfo input");
    ...
    *matchedOldDataObjInfo = NULL;
    ...
    while (tmpDataObjInfo != NULL) {
        ...
        else if (destHierCond && (strcmp(rescHier, tmpDataObjInfo->rescHier) == 0 ||
        ...
        else if (rescCond == 1 && (strstr(tmpDataObjInfo->rescHier, rescName) != 0)) {
            if (prevDataObjInfo != NULL) {
                prevDataObjInfo->next = tmpDataObjInfo->next;
            }
            else {
...
int resolveInfoForTrim(dataObjInfo_t** dataObjInfoHead, keyValPair_t* condInput)
{
    ...
    dataObjInfo_t *tmpDataObjInfo, *prevDataObjInfo;
    int matchedInfoCnt, unmatchedInfoCnt, matchedOldInfoCnt, unmatchedOldInfoCnt;
    int minCnt;
    char* tmpStr;
    int condFlag;
    int toTrim;
...
int requeDataObjInfoByDestResc(dataObjInfo_t** dataObjInfoHead, keyValPair_t* condInput, int writeFlag, int topFlag)
{
    ...
    if ((rescName = getValByKey(condInput, DEST_RESC_NAME_KW)) != NULL ||
        ...
        status = requeDataObjInfoByResc(dataObjInfoHead, rescName, writeFlag, topFlag);
    }
    return status;
}
int getDataObjInfoIncSpecColl(rsComm_t* rsComm, dataObjInp_t* dataObjInp, dataObjInfo_t** dataObjInfo)
{
    ...
    if (dataObjInp->specColl != NULL && dataObjInp->specColl->collClass != NO_SPEC_COLL &&
        dataObjInp->specColl->collClass != LINKED_COLL) {
        /* if it is linked, it already has been resolved */
        status = resolvePathInSpecColl(rsComm, dataObjInp->objPath, specCollPerm, 0, dataObjInfo);
        if (status == SYS_SPEC_COLL_OBJ_NOT_EXIST && dataObjInfo != NULL) {
            freeDataObjInfo(*dataObjInfo);
            dataObjInfo = NULL;
        }
    }
    else if ((status = resolvePathInSpecColl(rsComm, dataObjInp->objPath, specCollPerm, 1, dataObjInfo)) >= 0) {
        /* check if specColl in cache. May be able to save one query */
    }
    else if (getValByKey(&dataObjInp->condInput, ADMIN_RMTRASH_KW) != NULL &&
             rsComm->proxyUser.authInfo.authFlag == LOCAL_PRIV_USER_AUTH) {
        status = getDataObjInfo(rsComm, dataObjInp, dataObjInfo, NULL, 0);
    }
    else if (writeFlag > 0 && dataObjInp->oprType != REPLICATE_OPR) {
        status = getDataObjInfo(rsComm, dataObjInp, dataObjInfo, ACCESS_MODIFY_OBJECT, 0);
    }
    else {
        status = getDataObjInfo(rsComm, dataObjInp, dataObjInfo, ACCESS_READ_OBJECT, 0);
    }

    if (status < 0 && dataObjInp->specColl == NULL) {
        int status2;
        status2 = resolvePathInSpecColl(rsComm, dataObjInp->objPath, specCollPerm, 0, dataObjInfo);
        if (status2 < 0) {
    ...
    if (status >= 0 && NULL != dataObjInfo && NULL != (*dataObjInfo)->specColl) {
...
int regNewObjSize(rsComm_t* rsComm, char* objPath, int replNum, rodsLong_t newSize)
{
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "regNewObjSize: rsModDataObjMeta error for %s, status = %d", objPath, status);
    }
...
irods::error resolve_hierarchy_for_resc_from_cond_input(rsComm_t* _comm, const std::string& _resc, std::string& _hier)
{
    ...
    if (has_parent && has_child) {
        return ERROR(HIERARCHY_ERROR, "resource is mid-tier");
    }
    ...
    // this is a leaf node situation
        ...
        ret = resc_mgr.get_hier_to_root_for_resc(last_resc, _hier);
        if (!ret.ok()) {
            return PASS(ret);
        }
    }
...
bool contains_replica(const dataObjInfo_t* _objects, const std::string& _resc_name)
{
    return contains_replica_if(_objects, [&_resc_name](const auto& _node) {
        ...
        return std::any_of(b, e, [&_resc_name](const auto& _resource_name) { return _resource_name == _resc_name; });
    });
}

bool contains_replica(const dataObjInfo_t* _objects, int _replica_number)
{
    return contains_replica_if(_objects,
                               [_replica_number](const auto& _node) { return _node.replNum == _replica_number; });
}
