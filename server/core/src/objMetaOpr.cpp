...
int isData(rsComm_t* rsComm, char* objName, rodsLong_t* dataId)
{
    ...
    if (status < 0) {
        const auto err{ERROR(status, (boost::format("splitPathByKey failed for [%s]") % objName).str().c_str())};
        irods::log(err);
        return err.code();
    }
...
getPhyPath(rsComm_t* _comm, char* _obj_name, char* _resource, char* _phy_path, char* _resc_hier)
{
    // =-=-=-=-=-=-=-
    ...
    if (0 == strlen(_resc_hier)) {
        ...
        irods::error ret = irods::resolve_resource_hierarchy(irods::OPEN_OPERATION, _comm, &data_inp, resc_hier);
        if (!ret.ok()) {
            irods::log(PASS(ret));
    ...
    splitPathByKey(_obj_name, logical_parent_dir_name, MAX_NAME_LEN, logical_end_name, MAX_NAME_LEN, '/');
    ...
    if (status >= 0) {
        ...
        if (!phy_path_res) {
            rodsLog(LOG_ERROR, "getPhyPath: getSqlResultByInx for COL_D_DATA_PATH failed");
...
int isColl(rsComm_t* rsComm, char* objName, rodsLong_t* collId)
{
    ...
    if (status >= 0) {
        ...
        if ((collIdRes = getSqlResultByInx(genQueryOut, COL_COLL_ID)) == NULL) {
            rodsLog(LOG_ERROR, "isColl: getSqlResultByInx for COL_D_DATA_ID failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
...
int addAVUMetadataFromKVPairs(rsComm_t* rsComm, char* objName, char* inObjType, keyValPair_t* kVP)
{
    ...
    if (strcmp(inObjType, "-1")) {
        if (strlen(inObjType) >= sizeof(objType)) {
            rodsLog(
                LOG_ERROR, "inObjType: [%s] must be fewer than %ju characters", inObjType, (uintmax_t) sizeof(objType));
            return SYS_INVALID_INPUT_PARAM;
        }
...
rodsLong_t getTokenId(rsComm_t* rsComm, char* tokenNameSpace, char* tokenName)
{
    ...
    if (status >= 0) {
        ...
        if ((tokenIdRes = getSqlResultByInx(genQueryOut, COL_TOKEN_ID)) == NULL) {
            rodsLog(LOG_ERROR, "getTokenId: getSqlResultByInx for COL_TOKEN_ID failed");
            freeGenQueryOut(&genQueryOut);
            return UNMATCHED_KEY_OR_INDEX;
        }
...
rodsLong_t getUserId(rsComm_t* rsComm, char* userName, char* zoneName)
{
    ...
    if (status >= 0) {
        ...
        if ((userIdRes = getSqlResultByInx(genQueryOut, COL_USER_ID)) == NULL) {
            rodsLog(LOG_ERROR, "getUserId: getSqlResultByInx for COL_USER_ID failed");
            freeGenQueryOut(&genQueryOut);
            return UNMATCHED_KEY_OR_INDEX;
        }
...
int checkDupReplica(rsComm_t* rsComm, rodsLong_t dataId, char* rescName, char* filePath)
{
    ...
    if (status >= 0) {
        ...
        if ((replNum = getSqlResultByInx(genQueryOut, COL_DATA_REPL_NUM)) == NULL) {
            freeGenQueryOut(&genQueryOut);
            rodsLog(LOG_ERROR, "checkDupReplica: getSqlResultByInx COL_DATA_REPL_NUM failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
