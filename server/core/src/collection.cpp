                              ...
                              int singleFlag)
{
    ...
    if (genQueryInp == NULL || collection == NULL || genQueryOut == NULL) {
    ...
    if (accessPerm != NULL) {
        ...
        genQueryInp->maxRows = MAX_SQL_ROWS;
        status = trySpecificQueryDataObjInCollReCur(rsComm, collection, genQueryOut);
...
int isCollEmpty(rsComm_t* rsComm, char* collection)
{
    ...
    if (rsComm == NULL || collection == NULL) {
        rodsLog(LOG_ERROR, "isCollEmpty: Input rsComm or collection is NULL");
        return True;
    }
    ...
    if (handleInx < 0) {
        rodsLog(LOG_ERROR, "isCollEmpty: rsOpenCollection of %s error. status = %d", openCollInp.collName, handleInx);
        return True;
    }
    ...
    if (status >= 0) {
        *rodsObjStatOut = (rodsObjStat_t*) malloc(sizeof(rodsObjStat_t));
        ...
        if ((dataId = getSqlResultByInx(genQueryOut, COL_COLL_ID)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat: getSqlResultByInx for COL_COLL_ID failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else if ((ownerName = getSqlResultByInx(genQueryOut, COL_COLL_OWNER_NAME)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat:getSqlResultByInx for COL_COLL_OWNER_NAME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else if ((ownerZone = getSqlResultByInx(genQueryOut, COL_COLL_OWNER_ZONE)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat:getSqlResultByInx for COL_COLL_OWNER_ZONE failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else if ((createTime = getSqlResultByInx(genQueryOut, COL_COLL_CREATE_TIME)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat:getSqlResultByInx for COL_COLL_CREATE_TIME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else if ((modifyTime = getSqlResultByInx(genQueryOut, COL_COLL_MODIFY_TIME)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat:getSqlResultByInx for COL_COLL_MODIFY_TIME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else if ((collType = getSqlResultByInx(genQueryOut, COL_COLL_TYPE)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat:getSqlResultByInx for COL_COLL_TYPE failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else if ((collInfo1 = getSqlResultByInx(genQueryOut, COL_COLL_INFO1)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat:getSqlResultByInx for COL_COLL_INFO1 failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else if ((collInfo2 = getSqlResultByInx(genQueryOut, COL_COLL_INFO2)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat:getSqlResultByInx for COL_COLL_INFO2 failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else {
            ...
            if (strlen(collType->value) > 0) {
                ...
                if ((specCollCache = matchSpecCollCache(dataObjInp->objPath)) != NULL) {
                    ...
                    status = queueSpecCollCache(rsComm,
                                                genQueryOut, // JMC - backport 4680?
                                                dataObjInp->objPath);
                    if (status < 0) {
                        return status;
                    }
                    replSpecColl(&SpecCollCacheHead->specColl, &(*rodsObjStatOut)->specColl);
                }
            }
        }
    }
...
int rsMkCollR(rsComm_t* rsComm, const char* startColl, const char* destColl)
{
    ...
    while (tmpLen < pathLen) {
        ...
        if (status == CAT_NAME_EXISTS_AS_DATAOBJ && isTrashPath(tmpPath)) {
            ...
            rstrcpy(dataObjRenameInp.srcDataObjInp.objPath, tmpPath, MAX_NAME_LEN);
            rstrcpy(dataObjRenameInp.destDataObjInp.objPath, tmpPath, MAX_NAME_LEN);
        ...
        if (status < 0) {
            ...
            if (status == CATALOG_ALREADY_HAS_ITEM_BY_THAT_NAME) {
                rodsLog(LOG_DEBUG, "rsMkCollR: rsCollCreate - coll %s already exist.stat = %d", tmpPath, status);
                status = 0;
            }
            else {
                rodsLog(LOG_ERROR, "rsMkCollR: rsCollCreate failed for %s, status =%d", tmpPath, status);
            }
            // =-=-=-=-=-=-=-
            return status;
        }
