...
int allocFileDesc()
{
    ...
    rodsLog(LOG_NOTICE, "allocFileDesc: out of FileDesc");
...
int freeFileDesc(int fileInx)
{
    if (fileInx < 3 || fileInx >= NUM_FILE_DESC) {
        rodsLog(LOG_NOTICE, "freeFileDesc: fileInx %d out of range", fileInx);
...
int getServerHostByFileInx(int fileInx, rodsServerHost_t** rodsServerHost)
{
    int remoteFlag;
    if (fileInx < 3 || fileInx >= NUM_FILE_DESC) {
        rodsLog(LOG_DEBUG, "getServerHostByFileInx: Bad fileInx value %d", fileInx);
        return SYS_BAD_FILE_DESCRIPTOR;
    }

    if (FileDesc[fileInx].inuseFlag == 0) {
        rodsLog(LOG_DEBUG, "getServerHostByFileInx: fileInx %d not active", fileInx);
        return SYS_BAD_FILE_DESCRIPTOR;
    }
                     ...
                     int mode)
{
    ...
    if ((status = splitPathByKey(filePath.c_str(), myDir, MAX_NAME_LEN, myFile, MAX_NAME_LEN, '/')) < 0) {
        rodsLog(LOG_NOTICE, "mkDirForFilePath: splitPathByKey for %s error, status = %d", filePath.c_str(), status);
        return status;
    }
...
int mkFileDirR(rsComm_t* rsComm, size_t startDirLen, const std::string& destDir, const std::string& hier, int mode)
{
    ...
    if (destDir.size() < startDirLen) {
        rodsLog(LOG_ERROR,
                "mkFileDirR called with a destDir: [%s]"
                "shorter than its startDirLen: [%ju]",
                destDir.c_str(),
                (uintmax_t) startDirLen);
        return SYS_INVALID_INPUT_PARAM;
    }
    if (!rsComm) {
    ...
    while (physical_directory.size() > startDirLen) {
        irods::collection_object_ptr tmp_coll_obj(new irods::collection_object(physical_directory, hier, 0, 0));
        struct stat statbuf;
        ...
        if (stat_err.code() >= 0) {
            if (statbuf.st_mode & S_IFDIR) {
                ...
                rodsLog(
                    LOG_NOTICE, "mkFileDirR: A local non-directory %s already exists \n", physical_directory.c_str());
    ...
    while (!directories_to_create.empty()) {
        ...
        irods::collection_object_ptr tmp_coll_obj(new irods::collection_object(physical_directory, hier, mode, 0));
...
int chkEmptyDir(rsComm_t* rsComm, const std::string& cacheDir, const std::string& hier)
{
    ...
    irods::collection_object_ptr cacheDir_obj(new irods::collection_object(cacheDir, hier, 0, 0));
    ...
    while (readdir_err.ok() && 0 == readdir_err.code()) {
        ...
        irods::collection_object_ptr tmp_coll_obj(new irods::collection_object(childPath, hier, 0, 0));
        ...
        // handle hard error
        if (stat_err.code() < 0) {
            rodsLog(LOG_ERROR, "chkEmptyDir: fileStat error for %s, status = %d", childPath, stat_err.code());
            break;
        }
        ...
        if (myFileStat.st_mode & S_IFREG) {
            ...
            rodsLog(LOG_ERROR, "chkEmptyDir: file %s exists", childPath, status);
            break;
        }
    ...
    if (status != SYS_DIR_IN_VAULT_NOT_EMPTY) {
        irods::collection_object_ptr coll_obj(new irods::collection_object(cacheDir, hier, 0, 0));
...
{ // JMC - backport 4774
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "chkFilePathPerm: NULL rodsServerHost");
...
{
    if (path.find("/../") != std::string::npos || path.compare(path.size() - 3, path.size(), "/..") == 0) {
        /* "/../" or end with "/.."  */
...
int matchCliVaultPath(rsComm_t* _comm, const std::string& _path, rodsServerHost_t* _svr_host)
{
    ...
    if (home_pos != pos) {
        rodsLog(LOG_NOTICE,
                "matchCliVaultPath :: home/ is not found in the proper location for path [%s]",
                user_path.c_str());
        return -1;
    }
    ...
    if (user_pos != pos) {
        rodsLog(LOG_NOTICE,
                "matchCliVaultPath :: [%s] is not found in the proper location for path [%s]",
...
int bindStreamToIRods(rodsServerHost_t* rodsServerHost, int fd)
{
    ...
    fileInx = allocAndFillFileDesc(rodsServerHost, "", STREAM_FILE_NAME, "", fd, DEFAULT_FILE_MODE);
