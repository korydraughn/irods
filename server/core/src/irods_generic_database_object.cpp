...
namespace irods
{
    // =-=-=-=-=-=-=-
    // public - ctor
    generic_database_object::generic_database_object(const std::string& _type)
        : type_(_type)
    {
    } // ctor
    ...
    // =-=-=-=-=-=-=-
    // public - cctor
    generic_database_object::generic_database_object(const generic_database_object& _rhs)
    ...
    generic_database_object& generic_database_object::operator=(const generic_database_object&)
    {
        return *this;
    ...
    bool generic_database_object::operator==(const generic_database_object&) const
    {
        return false;
    ...
    error generic_database_object::resolve(const std::string& _interface, plugin_ptr& _ptr)
    {
        // =-=-=-=-=-=-=-
        ...
        if (!ret.ok()) {
            ...
            ret = db_mgr.init_from_type(type_, type_, type_, empty_context, db_ptr);
            if (!ret.ok()) {
                return PASS(ret);
            }
            else {
    ...
    error generic_database_object::get_re_vars(rule_engine_vars_t&)
    {
        return SUCCESS();
