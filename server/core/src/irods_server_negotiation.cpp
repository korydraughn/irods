...
namespace irods
{
    ...
    error check_sent_sid(const std::string _in_sid)
    {
        // =-=-=-=-=-=-=-
        // check incoming params
        if (_in_sid.empty()) {
            return ERROR(SYS_INVALID_INPUT_PARAM, "incoming SID is empty");
        }
        ...
        try {
            ...
            irods::error err = sign_server_sid(svr_sid, encr_key, signed_sid);
            if (!err.ok()) {
                return PASS(err);
            }
        ...
        irods::lookup_table<std::pair<std::string, std::string>>::iterator itr = remote_SID_key_map.begin();
        for (; itr != remote_SID_key_map.end(); ++itr) {
            ...
            // sign SID
            std::string signed_sid;
            irods::error err = sign_server_sid(entry.first, entry.second, signed_sid);
            if (!err.ok()) {
                return PASS(err);
            }
        ...
        return ERROR(SYS_SIGNED_SID_NOT_MATCHED, "signed SID was not matched");

    } // check_sent_sid

    /// =-=-=-=-=-=-=-
    /// @brief function which manages the TLS and Auth negotiations with the client
    error client_server_negotiation_for_server(irods::network_object_ptr _ptr, std::string& _result)
    {
        // =-=-=-=-=-=-=-
        ...
        int status = applyRuleWithInOutVars("acPreConnect", params, &rei);
        if (0 != status) {
            ...
            if (err.ok()) {
                ...
                if (kvp.find(CS_NEG_SID_KW) != kvp.end()) {
                    ...
                    if (!svr_sid.empty()) {
                        ...
                        if (!err.ok()) {
                            rodsLog(LOG_DEBUG, "CS_NEG\n%s", PASS(err).result().c_str());
                        }
                        else {
                            // =-=-=-=-=-=-=-
                        ...
                        rodsLog(LOG_WARNING, "CS_NEG :: %s - sent SID is empty", __FUNCTION__);
                    }
                }
                ...
                if (_result.empty()) {
                    return ERROR(UNMATCHED_KEY_OR_INDEX, "SSL result string missing from response");
                }
            }
            else {
                // =-=-=-=-=-=-=-
        ...
        if (CS_NEG_REQUIRE == rule_result && CS_NEG_USE_TCP == _result) {
            return ERROR(SERVER_NEGOTIATION_ERROR, "request to use TCP refused");
        }
        ...
        std::stringstream msg;
        msg << "failure detected from client for result [" << read_cs_neg->result_ << "]";
        return ERROR(SERVER_NEGOTIATION_ERROR, msg.str());
