...
namespace irods
{
    ...
    // =-=-=-=-=-=-=-
    // public - cctor
    mysql_object::mysql_object(const mysql_object& _rhs)
    ...
    mysql_object& mysql_object::operator=(const mysql_object&)
    {
        return *this;
    ...
    bool mysql_object::operator==(const mysql_object&) const
    {
        return false;
    ...
    error mysql_object::resolve(const std::string& _interface, plugin_ptr& _ptr)
    {
        // =-=-=-=-=-=-=-
        ...
        if (!ret.ok()) {
            ...
            ret = db_mgr.init_from_type(
                MYSQL_DATABASE_PLUGIN, MYSQL_DATABASE_PLUGIN, MYSQL_DATABASE_PLUGIN, empty_context, db_ptr);
            if (!ret.ok()) {
                return PASS(ret);
            }
            else {
    ...
    error mysql_object::get_re_vars(rule_engine_vars_t&)
    {
        return SUCCESS();
