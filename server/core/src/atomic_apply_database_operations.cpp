...
namespace
{
	  ...
	  -----------+------------------------+-----------             
        ...
        */
        ...
        try {
            ...
            if (!column) {
                log::api::error("Invalid arguments: missing non-ticket_id column for table [{}].", _op.table);
                     ...
                     const dml::update_op& _op) -> int
    {
        try {
            ...
            log::api::debug("{} :: Generated SQL = [{}]", __func__, sql);
                     ...
                     const dml::insert_op& _op) -> int
    {
        ...
        log::api::error("{} :: No insert statement executed! This message should not be reachable!", __func__);
                                   ...
                                   const std::string_view _table_name) -> bool
    {
        ...
        if (std::none_of(std::begin(supported_operations), std::end(supported_operations), pred)) {
            log::api::error("{} :: Database operation [{}] not supported on table [{}].", __func__, _db_op, _table_name);
            return false;
        }
    ...
    auto database_type_not_supported(const std::string_view _db_instance_name,
                                     const std::string_view _function_name) noexcept -> int
    {
        log::api::error("{} :: Database type not supported [{}].", _function_name, _db_instance_name);
...
namespace irods::experimental
{
    auto atomic_apply_database_operations(const std::vector<dml::operation_type>& _ops) -> int
    {
        try {
            ...
            log::api::error("{} :: Caught exception: {}", __func__, e.what());
            return e.code();
        }
        catch (const std::exception& e) {
            log::api::error("{} :: Caught exception: {}", __func__, e.what());
