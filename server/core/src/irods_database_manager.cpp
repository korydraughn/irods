...
namespace irods
{
    ...
    error database_manager::resolve(std::string _key, database_ptr& _value)
    {
        if (_key.empty()) {
        ...
        if (plugins_.has_entry(_key)) {
            ...
            msg << "no database plugin found for name [" << _key << "]";
            return ERROR(SYS_INVALID_INPUT_PARAM, msg.str());
        }

    } // resolve

    error database_manager::load_database_plugin(database_ptr& _plugin,
                                                 ...
                                                 const std::string& _context)
    {
        ...
        database* db = 0;
        error ret = load_plugin<database>(db, _plugin_name, PLUGIN_TYPE_DATABASE, _inst_name, _context);
        if (ret.ok() && db) {
            _plugin.reset(db);
            return SUCCESS();
        }
    ...
    error database_manager::init_from_type(const std::string& _type,
