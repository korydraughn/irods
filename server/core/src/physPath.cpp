...
int _dataObjChksum(rsComm_t* rsComm, dataObjInfo_t* dataObjInfo, char** chksumStr)
{
    ...
    if (ec == DIRECT_ARCHIVE_ACCESS) {
        const auto msg = fmt::format(R"_(Data object: "{}" is located an an archive resource. Ignoring its checksum.)_",
                                     dataObjInfo->filePath); // TODO Should this be the logical path instead?
