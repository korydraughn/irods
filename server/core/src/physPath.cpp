...
int getFileMode(dataObjInp_t* dataObjInp)
{
    ...
    if (dataObjInp != NULL && (dataObjInp->createMode & 0110) != 0) {
...
int getFilePathName(rsComm_t* rsComm, dataObjInfo_t* dataObjInfo, dataObjInp_t* dataObjInp)
{
    ...
    if (dataObjInp != NULL && (filePath = getValByKey(&dataObjInp->condInput, FILE_PATH_KW)) != NULL &&
        strlen(filePath) > 0) {
    ...
    if (dataObjInfo->rescName[0] == '\0') {
        rodsLog(LOG_ERROR, "getFilePathName: rescName for %s not resolved", dataObjInp->objPath);
        return SYS_INVALID_RESC_INPUT;
    }

    int chk_path = 0;
    irods::error err =
        irods::get_resource_property<int>(dataObjInfo->rescId, irods::RESOURCE_CHECK_PATH_PERM, chk_path);
    if (!err.ok()) {
        irods::log(PASS(err));
    }
    ...
    if (vaultPathPolicy.scheme == GRAFT_PATH_S) {
        status = setPathForGraftPathScheme(dataObjInp->objPath,
                                           vault_path.c_str(),
                                           vaultPathPolicy.addUserName,
                                           rsComm->clientUser.userName,
                                           vaultPathPolicy.trimDirCnt,
                                           dataObjInfo->filePath);
    }
    else {
        status = setPathForRandomScheme(
            dataObjInp->objPath, vault_path.c_str(), rsComm->clientUser.userName, dataObjInfo->filePath);
    }
...
int getVaultPathPolicy(rsComm_t* rsComm, dataObjInfo_t* dataObjInfo, vaultPathPolicy_t* outVaultPathPolicy)
{
    ...
    if (outVaultPathPolicy == NULL || dataObjInfo == NULL || rsComm == NULL) {
        rodsLog(LOG_ERROR, "getVaultPathPolicy: NULL input");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "getVaultPathPolicy: rule acSetVaultPathPolicy error, status = %d", status);
        return status;
    }
...
int setPathForRandomScheme(char* objPath, const char* vaultPath, char* userName, char* outPath)
{
    ...
    status = splitPathByKey(objPath, logicalCollName, MAX_NAME_LEN, logicalFileName, MAX_NAME_LEN, '/');

    if (status < 0) {
        rodsLog(LOG_ERROR, "setPathForRandomScheme: splitPathByKey error for %s, status = %d", outPath, status);
        return status;
    }
                              ...
                              char* outPath)
{
    ...
    for (i = 0; i < trimDirCnt; i++) {
        ...
        if (tmpPtr == NULL) {
            rodsLog(LOG_ERROR, "setPathForGraftPathScheme: objPath %s too short", objPath);
    ...
    if (addUserName > 0 && userName != NULL) {
        len = snprintf(outPath, MAX_NAME_LEN, "%s/%s/%s", vaultPath, userName, objPathPtr);
    }
    else {
        len = snprintf(outPath, MAX_NAME_LEN, "%s/%s", vaultPath, objPathPtr);
    }

    if (len >= MAX_NAME_LEN) {
        rodsLog(LOG_ERROR, "setPathForGraftPathScheme: filePath %s too long", objPath);
...
int resolveDupFilePath(rsComm_t* rsComm, dataObjInfo_t* dataObjInfo, dataObjInp_t* dataObjInp)
{
    ...
    if (chkAndHandleOrphanFile(rsComm,
                               dataObjInfo->objPath,
                               dataObjInfo->rescHier,
                               dataObjInfo->filePath,
                               dataObjInfo->rescName,
                               dataObjInfo->replStatus) >= 0) {
...
int getchkPathPerm(rsComm_t* rsComm, dataObjInp_t* dataObjInp, dataObjInfo_t* dataObjInfo)
{
    ...
    if ((filePath = getValByKey(&dataObjInp->condInput, FILE_PATH_KW)) != NULL && strlen(filePath) > 0) {
...
rodsLong_t getSizeInVault(rsComm_t* rsComm, dataObjInfo_t* dataObjInfo)
{
    ...
    if (status < 0 || NULL == myStat) { // JMC cppcheck - nullptr
        rodsLog(LOG_DEBUG, "getSizeInVault: l3Stat error for %s. status = %d", dataObjInfo->filePath, status);
        return status;
    }
    else {
...
{ // JMC - backport 4527
    ...
    switch (category) {
        case FILE_CAT:
            ...
            break;
        default:
            rodsLog(LOG_NOTICE, "_dataObjChksum: rescCat type %d is not recognized", category);
...
int dataObjChksumAndReg(rsComm_t* rsComm, dataObjInfo_t* dataObjInfo, char** chksumStr)
{
    ...
    if (status < 0) {
        rodsLog(
            LOG_NOTICE, "dataObjChksumAndReg: _dataObjChksum error for %s, status = %d", dataObjInfo->objPath, status);
        return status;
    }
                           ...
                           int replStatus)
{
    ...
    if ((status = chkOrphanFile(rsComm, filePath, _resc_name, &myDataObjInfo)) == 0) {
        ...
        if (replStatus > OLD_COPY || isTrashPath(myDataObjInfo.objPath)) {
            ...
            status = renameFilePathToNewDir(rsComm, REPL_DIR, &fileRenameInp, 1, new_fn);
            if (status < 0) {
                char* sys_error = NULL;
                const char* rods_error = rodsErrorName(status, &sys_error);
                rodsLog(LOG_ERROR,
                        "%s:%d renameFilePathToNewDir failed for file: %s - status = %d %s %s",
                        __FUNCTION__,
                        __LINE__,
                        filePath,
                        status,
            ...
            if (status < 0) {
                ...
                rstrcpy(fileRenameInp.oldFileName, fileRenameInp.newFileName, MAX_NAME_LEN);
            ...
            rstrcpy(fileRenameInp.oldFileName, filePath, MAX_NAME_LEN);
            char new_fn[MAX_NAME_LEN];
            status = renameFilePathToNewDir(rsComm, REPL_DIR, &fileRenameInp, 0, new_fn);
            if (status >= 0) {
        ...
        /* error */
        rodsLog(LOG_ERROR, "%s:%d chkOrphanFile failed for file: %s", __FUNCTION__, __LINE__, filePath);
        return status;
    }
}
...
int renameFilePathToNewDir(rsComm_t* rsComm, char* newDir, fileRenameInp_t* fileRenameInp, int renameFlag, char* new_fn)
{
    ...
    snprintf(newPtr, MAX_NAME_LEN - len, "/%s%s.%-u", newDir, oldPtr, irods::getRandom<unsigned int>());
...
int syncDataObjPhyPathS(rsComm_t* rsComm, dataObjInp_t* dataObjInp, dataObjInfo_t* dataObjInfo, char* acLCollection)
{
    ...
    int create_path = 0;
    err = irods::get_resource_property<int>(dataObjInfo->rescId, irods::RESOURCE_CREATE_PATH, create_path);
    if (!err.ok()) {
        irods::log(PASS(err));
    }
    ...
    if (getSizeInVault(rsComm, dataObjInfo) >= 0) {
        if ((status = chkAndHandleOrphanFile(rsComm,
                                             dataObjInfo->objPath,
                                             dataObjInfo->rescHier,
                                             dataObjInfo->filePath,
                                             dataObjInfo->rescName,
    ...
    /* rename it */
    rstrcpy(fileRenameInp.addr.hostAddr, location.c_str(), NAME_LEN);
    rstrcpy(fileRenameInp.newFileName, dataObjInfo->filePath, MAX_NAME_LEN);
...
int syncCollPhyPath(rsComm_t* rsComm, char* collection)
{
    ...
    status = rsQueryDataObjInCollReCur(rsComm, collection, &genQueryInp, &genQueryOut, NULL, 0);
    ...
    while (status >= 0) {
        sqlResult_t *dataIdRes, *subCollRes, *dataNameRes, *replNumRes, *rescNameRes, *filePathRes, *rescIdRes;
        char *tmpDataId, *tmpDataName, *tmpSubColl, *tmpReplNum, *tmpRescName, *tmpFilePath, *tmpRescId;
        dataObjInfo_t dataObjInfo;

        memset(&dataObjInfo, 0, sizeof(dataObjInfo));

        if ((dataIdRes = getSqlResultByInx(genQueryOut, COL_D_DATA_ID)) == NULL) {
            rodsLog(LOG_ERROR, "syncCollPhyPath: getSqlResultByInx for COL_D_DATA_ID failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        if ((subCollRes = getSqlResultByInx(genQueryOut, COL_COLL_NAME)) == NULL) {
            rodsLog(LOG_ERROR, "syncCollPhyPath: getSqlResultByInx for COL_COLL_NAME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        if ((dataNameRes = getSqlResultByInx(genQueryOut, COL_DATA_NAME)) == NULL) {
            rodsLog(LOG_ERROR, "syncCollPhyPath: getSqlResultByInx for COL_DATA_NAME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        if ((replNumRes = getSqlResultByInx(genQueryOut, COL_DATA_REPL_NUM)) == NULL) {
            rodsLog(LOG_ERROR, "syncCollPhyPath:getSqlResultByIn for COL_DATA_REPL_NUM failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        if ((rescNameRes = getSqlResultByInx(genQueryOut, COL_D_RESC_NAME)) == NULL) {
            rodsLog(LOG_ERROR, "syncCollPhyPath: getSqlResultByInx for COL_D_RESC_NAME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        if ((filePathRes = getSqlResultByInx(genQueryOut, COL_D_DATA_PATH)) == NULL) {
            rodsLog(LOG_ERROR, "syncCollPhyPath: getSqlResultByInx for COL_D_DATA_PATH failed");
            return UNMATCHED_KEY_OR_INDEX;
        }

        if ((rescIdRes = getSqlResultByInx(genQueryOut, COL_D_RESC_ID)) == NULL) {
            rodsLog(LOG_ERROR, "syncCollPhyPath: getSqlResultByInx for COL_D_RESC_ID failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        ...
        for (i = 0; i < genQueryOut->rowCnt; i++) {
            ...
            snprintf(dataObjInfo.objPath, MAX_NAME_LEN, "%s/%s", tmpSubColl, tmpDataName);
            ...
            status = syncDataObjPhyPathS(rsComm, NULL, &dataObjInfo, collection);
            if (status < 0) {
                rodsLog(LOG_ERROR,
...
int isInVault(dataObjInfo_t* dataObjInfo)
{
    ...
    if (strncmp(vault_path.c_str(), dataObjInfo->filePath, len) == 0) {
        return 1;
    }
    else {
        return 0;
...
int getLogPathFromPhyPath(char* phyPath, const char* rescVaultPath, char* outLogPath)
{
    ...
    if (strncmp(tmpZoneInfo->zoneName, tmpPtr, len) == 0 && *(tmpPtr + len) == '/') {
        /* start with zoneName */
        tmpPtr += (len + 1);
    }

    snprintf(outLogPath, MAX_NAME_LEN, "/%s/%s", tmpZoneInfo->zoneName, tmpPtr);
...
int rsMkOrphanPath(rsComm_t* rsComm, char* objPath, char* orphanPath)
{
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "rsMkOrphanPath: splitPathByKey error for %s, status = %d", objPath, status);
        return status;
    }
    ...
    *orphanPathPtr = '/';
    ...
    len = strlen(rsComm->clientUser.userName) + strlen(rsComm->clientUser.rodsZone);
    snprintf(orphanPathPtr, len + 20, "trash/orphan/%s#%s", rsComm->clientUser.userName, rsComm->clientUser.rodsZone);
    ...
    if (status < 0 && status != CAT_NAME_EXISTS_AS_COLLECTION) {
        rodsLogError(LOG_ERROR, status, "rsMkOrphanPath: rsCollCreate error for %s", orphanPath);
    }
    orphanPathPtr = orphanPath + strlen(orphanPath);

    snprintf(orphanPathPtr, strlen(childName) + 20, "/%s.%-u", childName, irods::getRandom<unsigned int>());
...
int getDataObjLockPath(char* objPath, char** outLockPath)
{
    ...
    irods::error ret = irods::get_full_path_for_unmoved_configs(LOCK_FILE_DIR, lock_path);
    if (!ret.ok()) {
        irods::log(PASS(ret));
        return ret.code();
    }

    len = lock_path.size() + strlen(tmpPath) + strlen(LOCK_FILE_TRAILER) + 10;
    *outLockPath = (char*) malloc(len);

    snprintf(*outLockPath, len, "%-s/%-s.%-s", lock_path.c_str(), tmpPath, LOCK_FILE_TRAILER);
...
int fsDataObjLock(char* objPath, int cmd, int type)
{
    ...
    if (status < 0 || NULL == path) {
        rodsLogError(LOG_ERROR, status, "fsDataObjLock: getDataObjLockPath error for %s", objPath);
        free(path);
        return status;
    }
    ...
    if (fd < 0) {
        ...
        rodsLogError(LOG_ERROR, status, "fsDataObjLock: open error for %s", objPath);
        return status;
    }
    struct flock lock;
    status = executeFilesystemLockCommand(cmd, type, fd, &lock);
    if (status < 0) {
        rodsLogError(LOG_DEBUG,
                     status,
                     "fsDataObjLock: fcntl error for %s, cmd = %d, type = %d",
                     objPath,
                     cmd,
                     type); // JMC - backport 4604
        return status;
    }
    if (cmd == F_GETLK) {
...
int fsDataObjUnlock(int cmd, int type, int fd)
{
    ...
    if (status < 0) {
        rodsLogError(LOG_DEBUG,
                     status,
                     "fsDataObjLock: fcntl error on unlock, status = %d",
...

{
    ...
    if (status < 0) {
        rodsLog(LOG_DEBUG, "getFileMetaFromVault: l3Stat error for %s. status = %d", dataObjInfo->filePath, status);
        return status;
    }
