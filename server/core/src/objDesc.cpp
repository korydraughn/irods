...
int allocL1desc()
{
    ...
    rodsLog(LOG_NOTICE, "allocL1desc: out of L1desc");
...
int allocSpecCollDesc()
{
    ...
    rodsLog(LOG_NOTICE, "allocSpecCollDesc: out of SpecCollDesc");
...
int freeSpecCollDesc(int specCollInx)
{
    if (specCollInx < 1 || specCollInx >= NUM_SPEC_COLL_DESC) {
        rodsLog(LOG_NOTICE, "freeSpecCollDesc: specCollInx %d out of range", specCollInx);
...
int closeAllL1desc(rsComm_t* rsComm)
{
    ...
    for (i = 3; i < NUM_L1_DESC; i++) {
        if (L1desc[i].inuseFlag == FD_INUSE && L1desc[i].l3descInx > 2) {
            l3Close(rsComm, i);
        }
    }
    return 0;
...
int freeL1desc(int l1descInx)
{
    ...
    if (L1desc[l1descInx].dataObjInpReplFlag == 1 && L1desc[l1descInx].dataObjInp != NULL) {
...
int fillL1desc(int l1descInx, dataObjInp_t* dataObjInp, dataObjInfo_t* dataObjInfo, int replStatus, rodsLong_t dataSize)
{
    ...
    if (condInput != NULL && condInput->len > 0) {
        ...
        else if ((tmpPtr = getValByKey(condInput, VERIFY_CHKSUM_KW)) != NULL) {
                  ...
                  int oprType)
{
    ...
    if (destRescHier && strlen(destRescHier)) {
        ...
        if (err.ok()) {
            ...
            if (status < 0) {
                rodsLog(LOG_ERROR, "getNumThreads: acSetNumThreads error, status = %d", status);
            }
            else {
                ...
                else if (numDestThr == 1 && srcRescHier == NULL && isLocalHost(location.c_str())) {
    ...
    free(rei.condInputData);
    if (status < 0) {
        rodsLog(LOG_ERROR, "getNumThreads: acGetNumThreads error, status = %d", status);
        return 0;
    }
    else {
        if (rei.status > 0) {
...
int initDataOprInp(dataOprInp_t* dataOprInp, int l1descInx, int oprType)
{
    ...
    if ((tmpStr = getValByKey(&dataObjInp->condInput, RBUDP_SEND_RATE_KW)) != NULL) {
        addKeyVal(&dataOprInp->condInput, RBUDP_SEND_RATE_KW, tmpStr);
    }

    if ((tmpStr = getValByKey(&dataObjInp->condInput, RBUDP_PACK_SIZE_KW)) != NULL) {
...
{
    if (l3descInx <= 2 || FileDesc[l3descInx].inuseFlag == 0 || FileDesc[l3descInx].rodsServerHost == NULL) {
        return l3descInx;
    }
...
int freeCollHandle(int handleInx)
{
    if (handleInx < 0 || static_cast<std::size_t>(handleInx) >= CollHandle.size()) {
        rodsLog(LOG_NOTICE, "freeCollHandle: handleInx %d out of range", handleInx);
        return SYS_FILE_DESC_OUT_OF_RANGE;
    }
                                ...
                                openStat_t* openStat)
{
    ...
    replDataObjInp(dataObjInp, L1desc[l1descInx].dataObjInp);
    L1desc[l1descInx].dataObjInpReplFlag = 1;
    dataObjInfo = L1desc[l1descInx].dataObjInfo = (dataObjInfo_t*) malloc(sizeof(dataObjInfo_t));
