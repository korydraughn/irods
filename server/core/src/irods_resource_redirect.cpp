...
namespace irods
{
    ...
    static error request_vote_for_file_object(rsComm_t* _comm,
                                              ...
                                              float& _out_vote)
    {
        ...
        if (p_err.ok()) {
            return ERROR(DIRECT_CHILD_ACCESS, "attempt to directly address a child resource");
        }
    ...
    static error resolve_hier_for_open_or_write_without_keyword(rsComm_t* _comm,
                                                                ...
                                                                std::string& _out_hier)
    {
        ...
        for (; itr != root_map.end(); ++itr) {
            ...
            irods::error ret =
                request_vote_for_file_object(_comm, _oper.c_str(), itr->first, _file_obj, voted_hier, vote);
            if (ret.ok()) {
                // =-=-=-=-=-=-=-
        ...
        if (diff <= 0.0) {
            return ERROR(HIERARCHY_ERROR, "no valid resource found for data object");
        }
    ...
    ///        resource keyword
    static error resolve_hier_for_open_or_write(rsComm_t* _comm,
                                                irods::file_object_ptr _file_obj,
                                                ...
                                                std::string& _out_hier)
    {
        ...
        if (_key_word) {
            ...
            if (kw_match_found) {
                float vote = 0.0;
                error ret = request_vote_for_file_object(_comm, _oper.c_str(), _key_word, _file_obj, _out_hier, vote);
                if (0.0 == vote) {
                    if (ret.code() == 0) {
                        ret.code(-1);
                    }
        ...
        return resolve_hier_for_open_or_write_without_keyword(_comm, _file_obj, _oper, _out_hier);
    ...
    /// @brief function to handle resolving the hier given the fco and
    ///        resource keyword
    static error resolve_hier_for_create(rsComm_t* _comm,
                                         irods::file_object_ptr _file_obj,
                                         const char* _key_word,
                                         ...
                                         std::string& _out_hier)
    {
        ...
        float vote = 0.0;
        error ret = request_vote_for_file_object(_comm, CREATE_OPERATION, resc_name, _file_obj, _out_hier, vote);
        if (0.0 == vote) {
            if (ret.code() == 0) {
    ...
    static error resolve_hier_for_create_or_open(rsComm_t* _comm,
                                                 irods::file_object_ptr _file_obj,
                                                 const char* _key_word,
                                                 dataObjInp_t* _data_obj_inp,
                                                 std::string& _out_hier)
    {
        ...
        if (_key_word) {
            ...
            if (kw_match_found) {
                float vote = 0.0;
                error ret = request_vote_for_file_object(_comm, WRITE_OPERATION, _key_word, _file_obj, _out_hier, vote);
                if (0.0 == vote) {
                    if (ret.code() == 0) {
                        ret.code(-1);
                    }
        ...
        // =-=-=-=-=-=-=-
        // either no kw match or no kw, so pick one...
        return resolve_hier_for_create(_comm, _file_obj, _key_word, _data_obj_inp, _out_hier);
    ...
    error determine_force_write_to_new_resource(const std::string& _oper,
                                                ...
                                                dataObjInp_t* _data_obj_inp)
    {
        ...
        if (PUT_OPR != _data_obj_inp->oprType || repls.empty() || !dst_resc_kw || !force_flag_kw ||
            strlen(dst_resc_kw) == 0 || !(OPEN_OPERATION == _oper || WRITE_OPERATION == _oper)) {
            return SUCCESS();
        }
        ...
        if (!hier_match_flg) {
            std::stringstream msg;
            msg << "cannot force put [" << _data_obj_inp->objPath << "] to a different resource [" << dst_resc_kw
                << "]";
    ...
    bool is_hier_in_obj_info_list(const std::string& _hier, dataObjInfo_t* _data_obj_info_head)
    {
    ...
    error resolve_resource_hierarchy(const std::string& _oper,
                                     rsComm_t* _comm,
                                     dataObjInp_t* _data_obj_inp,
                                     std::string& _out_hier,
                                     dataObjInfo_t** _data_obj_info)
    {
        ...
        if (!_comm) {
            ...
            return ERROR(SYS_INVALID_INPUT_PARAM, "null data obj inp pointer");
        }
        ...
        error ret = determine_force_write_to_new_resource(oper, file_obj, _data_obj_inp);
        if (!ret.ok()) {
            return PASS(ret);
        }
        ...
        if (OPEN_OPERATION == oper || WRITE_OPERATION == oper || UNLINK_OPERATION == oper) {
            // =-=-=-=-=-=-=-
            ...
            _out_hier = "";
            error ret = resolve_hier_for_open_or_write(_comm, file_obj, key_word, oper, _out_hier);
            if (!ret.ok()) {
            ...
            if (_data_obj_info && !is_hier_in_obj_info_list(_out_hier, *_data_obj_info)) {
                ...
                *_data_obj_info = NULL;
                ...
                // get updated list
                int status = getDataObjInfoIncSpecColl(_comm, _data_obj_inp, _data_obj_info);
                ...
                if (0 == *_data_obj_info || status < 0) {
                    ...
                    std::stringstream msg;
                    msg << "Failed to retrieve data object info list for [" << _data_obj_inp->objPath << "]";
                    return ERROR(HIERARCHY_ERROR, msg.str());
                }
                ...
                if (!is_hier_in_obj_info_list(_out_hier, *_data_obj_info)) {
                    std::stringstream msg;
                    msg << "Failed to find resource hierarchy [" << _out_hier << "for [" << _data_obj_inp->objPath
                        << "]";
                    return ERROR(HIERARCHY_ERROR, msg.str());
                }
            }
            ...
            int status = apply_policy_for_create_operation(_comm, _data_obj_inp, create_resc_name);
            if (status < 0) {
                return ERROR(status, "apply_policy_for_create_operation failed");
            }
            ...
            if (fac_err.ok()) {
                ret = resolve_hier_for_create_or_open(
                    _comm, file_obj, create_resc_name.c_str(), _data_obj_inp, _out_hier);
            }
            else {
                // =-=-=-=-=-=-=-
                // attempt to resolve for a create
                ret = resolve_hier_for_create(_comm, file_obj, create_resc_name.c_str(), _data_obj_inp, _out_hier);
            }
            if (!ret.ok()) {
                return PASSMSG("Failed to resolve resource hierarchy", ret);
            }
        ...
        // should not get here
        std::stringstream msg;
        msg << "operation not supported [" << oper << "]";
                            ...
                            dataObjInfo_t** _data_obj_info)
    {
        ...
        error res_err = resolve_resource_hierarchy(_oper, _comm, _data_obj_inp, resc_hier, _data_obj_info);
        if (!res_err.ok()) {
            std::stringstream msg;
        ...
        error err = get_resource_property<rodsServerHost_t*>(resc_id, RESOURCE_HOST, last_resc_host);
