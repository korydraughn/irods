...
int receiveDataFromServer(int conn_tmp_socket)
{
    ...
    while (!data_complete) {
        ...
        while (tokenized_strings != NULL) {
            ...
            if (tmpStr == "end_of_vars") {
                ...
                if (num_bytes < 0) {
                    rodsLog(LOG_ERROR,
                            "Error sending acknowledgment to rodsServer, errno = [%d][%s]",
                            errno,
                            strerror(errno));
                    return SYS_SOCK_READ_ERR;
                }
            ...
            if (status < 0) {
                irods::log(ERROR(SYS_INTERNAL_ERR,
                                 (boost::format("setenv([%s],[%s],1) failed with errno = [%d][%s]") % lhs.c_str() %
                                  rhs.c_str() % errno % strerror(errno))
                                     .str()
                                     .c_str()));
            }
    ...
    if (status < 0) {
        irods::log(ERROR(SYS_INTERNAL_ERR,
                         (boost::format("setenv([%s],[%s],1) failed with errno = [%d][%s]") % SP_NEW_SOCK % socket_buf %
...
int runIrodsAgent(sockaddr_un agent_addr)
{
    ...
    if (listen(listen_socket, 5) < 0) {
        rodsLog(LOG_ERROR,
                "Unable to set up socket for listening in runIrodsAgent, errno [%d]: %s",
                errno,
                strerror(errno));
    ...
    if (LogFd < 0) {
        rodsLog(LOG_NOTICE, "runIrodsAgent: Unable to open %s. errno = %d", logFile, errno);
        free(logFile);
        return -1;
    }
    (void) dup2(LogFd, 0);
        ...
        if (ready == -1 && errno == EINTR) {
            ...
            if (bytes_received == -1) {
                ...
                if (-1 == listen(tmp_socket, 5)) {
                    const auto err{
                        ERROR(SYS_SOCK_LISTEN_ERR, "Failed to set up socket for listening in receiveDataFromServer")};
                    irods::log(err);
                    return err.code();
                }
                ...
                if (-1 == conn_tmp_socket) {
                    const auto err{
                        ERROR(SYS_SOCK_ACCEPT_ERR, "Failed to accept client socket in receiveDataFromServer")};
                    irods::log(err);
                    return err.code();
                }
            }
...
int agentMain(rsComm_t* rsComm)
{
    ...
    while (result.ok() && status >= 0) {
        ...
        if ((result = ASSERT_PASS(ret, "Failed to factory an auth object for scheme: \"%s\".", rsComm->auth_scheme))
                .ok()) {
            irods::plugin_ptr ptr;
            ret = auth_obj->resolve(irods::AUTH_INTERFACE, ptr);
            if ((result =
                     ASSERT_PASS(ret, "Failed to resolve the auth plugin for scheme: \"%s\".", rsComm->auth_scheme))
                    .ok()) {
                ...
                result =
                    ASSERT_PASS(ret, "Failed during auth plugin agent start for scheme: \"%s\".", rsComm->auth_scheme);
            }
