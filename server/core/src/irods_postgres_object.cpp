...
namespace irods
{
    ...
    // =-=-=-=-=-=-=-
    // public - cctor
    postgres_object::postgres_object(const postgres_object& _rhs)
    ...
    postgres_object& postgres_object::operator=(const postgres_object&)
    {
        return *this;
    ...
    bool postgres_object::operator==(const postgres_object&) const
    {
        return false;
    ...
    error postgres_object::resolve(const std::string& _interface, plugin_ptr& _ptr)
    {
        // =-=-=-=-=-=-=-
        ...
        if (!ret.ok()) {
            ...
            ret = db_mgr.init_from_type(
                POSTGRES_DATABASE_PLUGIN, POSTGRES_DATABASE_PLUGIN, POSTGRES_DATABASE_PLUGIN, empty_context, db_ptr);
            if (!ret.ok()) {
                return PASS(ret);
            }
            else {
    ...
    error postgres_object::get_re_vars(rule_engine_vars_t&)
    {
        return SUCCESS();
