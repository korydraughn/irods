...
int svrToSvrConnect(rsComm_t* rsComm, rodsServerHost_t* rodsServerHost)
{
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "svrToSvrConnect: clientLogin to %s failed", rodsServerHost->hostName->name);
        return status;
    }
    else {
        return rodsServerHost->localFlag;
...
int setupSrvPortalForParaOpr(rsComm_t* rsComm, dataOprInp_t* dataOprInp, int oprType, portalOprOut_t** portalOprOut)
{
    ...
    *portalOprOut = myDataObjPutOut;
    ...
    if (getValByKey(&dataOprInp->condInput, STREAMING_KW) != NULL || proto == SOCK_DGRAM) {
        ...
        myDataObjPutOut->numThreads = getNumThreads(rsComm,
                                                    ...
                                                    getValByKey(&dataOprInp->condInput, RESC_HIER_STR_KW),
                                                    NULL,
                                                    oprType);
    }
    ...
    if (myDataObjPutOut->numThreads == 0) {
        ...
        try {
            const auto svr_port_range_start =
                irods::get_server_property<const int>(irods::CFG_SERVER_PORT_RANGE_START_KW);
        ...
        for (int i = 0; i < port_range_count; ++i) {
            portalSock = createSrvPortal(rsComm, &myDataObjPutOut->portList, proto);
            if (0 <= portalSock) {
                break;
            }
        }
        if (portalSock < 0) {
            rodsLog(LOG_NOTICE, "setupSrvPortalForParaOpr: createSrvPortal error, status = %d", portalSock);
...
int createSrvPortal(rsComm_t* rsComm, portList_t* thisPortList, int proto)
{
    ...
    if ((lsock = svrSockOpenForInConn(rsComm, &lport, &laddr, SOCK_STREAM)) < 0) {
        rodsLog(LOG_ERROR, "createSrvPortal: svrSockOpenForInConn failed: status=%d", lsock);
        return lsock;
    }
    ...
    if (ProcessType == CLIENT_PT) {
        ...
        if (LocalServerHost != NULL && strcmp(LocalServerHost->hostName->name, "localhost") != 0 &&
    ...
    if (listen(lsock, SOMAXCONN) < 0) {
        rodsLog(LOG_NOTICE, "setupSrvPortal: listen failed, errno: %d", errno);
        return SYS_SOCK_LISTEN_ERR;
    }

    if (proto == SOCK_DGRAM) {
        if ((udpsock = svrSockOpenForInConn(rsComm, &udpport, &udpaddr, SOCK_DGRAM)) < 0) {
            rodsLog(LOG_ERROR, "setupSrvPortal- sockOpenForInConn of SOCK_DGRAM failed: stat=%d", udpsock);
            CLOSE_SOCK(lsock);
            return udpsock;
        }
        else {
...
int acceptSrvPortal(rsComm_t* rsComm, portList_t* thisPortList)
{
    ...
    if (myFd < 0) {
        rodsLog(LOG_NOTICE, "acceptSrvPortal() -- accept() failed: errno=%d", errno);
...
int applyRuleForSvrPortal(int sockFd, int oprType, int preOrPost, int load, rsComm_t* rsComm)
{
    ...
    rei.rsComm = rsComm;
    int ret = applyRuleArg(
        (char*) (preOrPost == 0 ? "acPreProcForServerPortal" : "acPostProcForServerPortal"), args, 6, &rei, 0);
    free(lAddr);
    free(pAddr);
    return ret;
}
...
int svrPortalPutGet(rsComm_t* rsComm)
{
    ...
    if (numThreads <= 0 || numThreads > MAX_NUM_CONFIG_TRAN_THR) {
        rodsLog(LOG_NOTICE, "svrPortalPut: numThreads %d out of range");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (portalFd < 0) {
        rodsLog(LOG_NOTICE, "svrPortalPut: acceptSrvPortal error. errno = %d", errno);
    ...
    if (oprType == PUT_OPR) {
        fillPortalTransferInp(&myInput[0],
                              rsComm,
                              portalFd,
    ...
    if (numThreads == 1) {
        ...
        for (i = 1; i < numThreads; i++) {
            ...
            if (oprType == PUT_OPR) {
                ...
                fillPortalTransferInp(&myInput[i],
                                      rsComm,
                                      portalFd,
                                      l3descInx,
                                      0,
...
int fillPortalTransferInp(portalTransferInp_t* myInput,
                          rsComm_t* rsComm,
                          int srcFd,
                          int destFd,
                          int srcRescTypeInx,
                          ...
                          int flags)
{
    ...
    // portal input
    memcpy(myInput->shared_secret, rsComm->shared_secret, NAME_LEN);
    ...
    snprintf(myInput->encryption_algorithm, sizeof(myInput->encryption_algorithm), "%s", rsComm->encryption_algorithm);
    return 0;
}
...
void partialDataPut(portalTransferInp_t* myInput)
{
    ...
    bool use_encryption_flg = (myInput->rsComm->negotiation_results == irods::CS_NEG_USE_SSL);
    ...
    if (myInput->offset != 0) {
        myOffset = _l3Lseek(myInput->rsComm, destL3descInx, myInput->offset, SEEK_SET);
        if (myOffset < 0) {
            myInput->status = myOffset;
            rodsLog(LOG_NOTICE, "_partialDataPut: _objSeek error, status = %d ", myInput->status);
    ...
    irods::buffer_crypt crypt(
        myInput->key_size, myInput->salt_size, myInput->num_hash_rounds, myInput->encryption_algorithm);
    ...
    if (use_encryption_flg) {
        ...
        shared_secret.assign(&myInput->shared_secret[0], &myInput->shared_secret[iv_size]);
    }
    ...
    try {
        trans_buff_size =
            irods::get_advanced_setting<const int>(irods::CFG_TRANS_BUFFER_SIZE_FOR_PARA_TRANS) * 1024 * 1024;
    }
    catch (const irods::exception& e) {
        irods::log(e);
        return;
    ...
    while (bytesToGet > 0) {
        ...
        myInput->status = sendTranHeader(srcFd, PUT_OPR, myInput->flags, myOffset, toread0);

        if (myInput->status < 0) {
            rodsLog(LOG_NOTICE, "partialDataPut: sendTranHeader error. status = %d", myInput->status);
            if (myInput->threadNum > 0) {
                _l3Close(myInput->rsComm, destL3descInx);
            }
            CLOSE_SOCK(srcFd);
        ...
        while (toread0 > 0) {
            ...
            if (bytesRead == new_size) {
                ...
                if (use_encryption_flg) {
                    this_iv.assign(&buf[0], &buf[iv_size]);
                    cipher.assign(&buf[iv_size], &buf[new_size]);
                    irods::error ret = crypt.decrypt(shared_secret, this_iv, cipher, plain);
                    if (!ret.ok()) {
                        irods::log(PASS(ret));
                    ...
                    std::copy(plain.begin(), plain.end(), &buf[0]);
                    plain_size = plain.size();
                }

                if ((bytesWritten = _l3Write(myInput->rsComm, destL3descInx, buf, plain_size)) != (plain_size)) {
                    rodsLog(
                        LOG_NOTICE, "_partialDataPut:Bytes written %d don't match read %d", bytesWritten, bytesRead);
...
void partialDataGet(portalTransferInp_t* myInput)
{
    ...
    if (myInput->offset != 0) {
        myOffset = _l3Lseek(myInput->rsComm, srcL3descInx, myInput->offset, SEEK_SET);
        if (myOffset < 0) {
            myInput->status = myOffset;
            rodsLog(LOG_NOTICE, "_partialDataGet: _objSeek error, status = %d ", myInput->status);
            if (myInput->threadNum > 0) {
    ...
    while (bytesToGet > 0) {
        ...
        myInput->status = sendTranHeader(destFd, GET_OPR, myInput->flags, myOffset, toread0);

        if (myInput->status < 0) {
            rodsLog(LOG_NOTICE, "partialDataGet: sendTranHeader error. status = %d", myInput->status);
            if (myInput->threadNum > 0) {
                _l3Close(myInput->rsComm, srcL3descInx);
            }
            CLOSE_SOCK(destFd);
        ...
        while (toread0 > 0) {
            ...
            if (bytesRead == toread1) {
                ...
                if (use_encryption_flg) {
                    ...
                    // =-=-=-=-=-=-=-
                    // encrypt
                    in_buf.assign(&buf[0], &buf[bytesRead]);
                    ...
                    std::copy(iv.begin(), iv.end(), &buf[0]);
                    std::copy(cipher.begin(), cipher.end(), &buf[iv_size]);
                    ...
                    bytesWritten = myWrite(destFd, &new_size, sizeof(int), &bytesWritten);
                }

                // =-=-=-=-=-=-=-
                // then write the actual buffer
                bytesWritten = myWrite(destFd, buf, new_size, &bytesWritten);

                if (bytesWritten != new_size) {
                    rodsLog(
                        LOG_NOTICE, "_partialDataGet:Bytes written %d don't match read %d", bytesWritten, bytesRead);
                ...
                rodsLog(LOG_NOTICE, "_partialDataGet: toread %d bytes, %d bytes read", toread1, bytesRead);
                myInput->status = SYS_COPY_LEN_ERR;
                break;
            }
        } /* while loop toread0 */
...
void remToLocPartialCopy(portalTransferInp_t* myInput)
{
    ...
    if (myInput == NULL) {
        rodsLog(LOG_NOTICE, "remToLocPartialCopy: NULL input");
        return;
    }
    ...
    while (myInput->status >= 0) {
        ...
        if (myHeader.offset != curOffset) {
            ...
            myOffset = _l3Lseek(myInput->rsComm, destL3descInx, myHeader.offset, SEEK_SET);
            if (myOffset < 0) {
                myInput->status = myOffset;
                rodsLog(LOG_NOTICE, "remToLocPartialCopy: _objSeek error, status = %d ", myInput->status);
                break;
            }
        }
        ...
        while (toGet > 0) {
            ...
            if (bytesRead != new_size) {
                if (bytesRead < 0) {
                    myInput->status = bytesRead;
                    rodsLogError(LOG_ERROR, bytesRead, "remToLocPartialCopy: copy error for %lld", bytesRead);
                }
                else if ((myInput->flags & NO_CHK_COPY_LEN_FLAG) == 0) {
                    myInput->status = SYS_COPY_LEN_ERR - errno;
                    rodsLog(LOG_ERROR, "remToLocPartialCopy: toGet %lld, bytesRead %d", toGet, bytesRead);
                }
                break;
            }
            ...
            bytesWritten = _l3Write(myInput->rsComm, destL3descInx, buf, plain_size);

            if (bytesWritten != plain_size) {
                rodsLog(LOG_NOTICE, "_partialDataPut:Bytes written %d don't match read %d", bytesWritten, bytesRead);
...
int rbudpRemLocCopy(dataCopyInp_t* dataCopyInp)
{
    ...
    if (dataCopyInp == NULL) {
        rodsLog(LOG_NOTICE, "rbudpRemLocCopy: NULL dataCopyInp input");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if ((tmpStr = getValByKey(&dataOprInp->condInput, RBUDP_PACK_SIZE_KW)) != NULL) {
    ...
    if (oprType == COPY_TO_LOCAL_OPR) {
        ...
        status = getFileToPortalRbudp(portalOprOut, NULL, FileDesc[destL3descInx].fd, veryVerbose, packetSize);
    }
    else {
        int srcL3descInx = dataOprInp->srcL3descInx;

        if ((tmpStr = getValByKey(&dataOprInp->condInput, RBUDP_SEND_RATE_KW)) != NULL) {
            sendRate = atoi(tmpStr);
        }
        else {
            sendRate = DEF_UDP_SEND_RATE;
        }
        status = putFileToPortalRbudp(portalOprOut, NULL, FileDesc[srcL3descInx].fd, veryVerbose, sendRate, packetSize);
    }
    return status;
}
...
int remLocCopy(rsComm_t* rsComm, dataCopyInp_t* dataCopyInp)
{
    ...
    if (dataCopyInp == NULL) {
        rodsLog(LOG_NOTICE, "remLocCopy: NULL dataCopyInp input");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (numThreads > MAX_NUM_CONFIG_TRAN_THR || numThreads <= 0) {
        rodsLog(LOG_NOTICE, "remLocCopy: numThreads %d out of range", numThreads);
    ...
    sock = connectToRhostPortal(myPortList->hostAddr, myPortList->portNum, myPortList->cookie, rsComm->windowSize);
    if (sock < 0) {
        return sock;
    }
    ...
    if (oprType == COPY_TO_LOCAL_OPR) {
        fillPortalTransferInp(
            &myInput[0], rsComm, sock, dataOprInp->destL3descInx, 0, dataOprInp->destRescTypeInx, 0, 0, 0, 0);
    }
    else {
        fillPortalTransferInp(
            &myInput[0], rsComm, dataOprInp->srcL3descInx, sock, dataOprInp->srcRescTypeInx, 0, 0, 0, 0, 0);
    }

    if (numThreads == 1) {
        if (getValByKey(&dataOprInp->condInput, NO_CHK_COPY_LEN_KW) != NULL) {
        ...
        for (i = 1; i < numThreads; i++) {
            sock =
                connectToRhostPortal(myPortList->hostAddr, myPortList->portNum, myPortList->cookie, rsComm->windowSize);
            if (sock < 0) {
                return sock;
            }
            if (oprType == COPY_TO_LOCAL_OPR) {
                ...
                if (myFd < 0) { /* error */
                    retVal = myFd;
                    rodsLog(LOG_NOTICE, "remLocCopy: cannot open file, status = %d", myFd);
                    CLOSE_SOCK(sock);
                    continue;
                }

                fillPortalTransferInp(&myInput[i], rsComm, sock, myFd, 0, dataOprInp->destRescTypeInx, i, 0, 0, 0);
                ...
                fillPortalTransferInp(&myInput[i], rsComm, myFd, sock, dataOprInp->destRescTypeInx, 0, i, 0, 0, 0);
        ...
        if (retVal < 0) {
            ...
            if (dataSize <= 0 || totalWritten == dataSize) {
                ...
                rodsLog(LOG_NOTICE, "remLocCopy: totalWritten %lld dataSize %lld mismatch", totalWritten, dataSize);
                return SYS_COPY_LEN_ERR;
            }
        }
    }
...
int sameHostCopy(rsComm_t* rsComm, dataCopyInp_t* dataCopyInp)
{
    ...
    if (dataCopyInp == NULL) {
        rodsLog(LOG_NOTICE, "sameHostCopy: NULL dataCopyInp input");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (numThreads == 0) {
        ...
        rodsLog(LOG_NOTICE, "sameHostCopy: numThreads %d out of range", numThreads);
        return SYS_INVALID_PORTAL_OPR;
    }
    ...
    if (numThreads == 1) {
        ...
        for (i = 1; i < numThreads; i++) {
            ...
            if (out_fd < 0) { /* error */
                retVal = out_fd;
                rodsLog(LOG_NOTICE, "sameHostCopy: cannot open dest file, status = %d", out_fd);
                continue;
            }
            ...
            if (in_fd < 0) { /* error */
                retVal = in_fd;
                rodsLog(LOG_NOTICE, "sameHostCopy: cannot open src file, status = %d", in_fd);
                continue;
            }
            fillPortalTransferInp(&myInput[i],
                                  rsComm,
        ...
        if (retVal < 0) {
            ...
            if (dataSize <= 0 || totalWritten == dataSize) {
                ...
                rodsLog(LOG_NOTICE, "sameHostCopy: totalWritten %lld dataSize %lld mismatch", totalWritten, dataSize);
                return SYS_COPY_LEN_ERR;
            }
        }
    }
...
void sameHostPartialCopy(portalTransferInp_t* myInput)
{
    ...
    if (myInput == NULL) {
        rodsLog(LOG_NOTICE, "onsameHostPartialCopy: NULL input");
        return;
    }
    ...
    if (myInput->offset != 0) {
        ...
        if (myOffset < 0) {
            myInput->status = myOffset;
            rodsLog(LOG_NOTICE, "sameHostPartialCopy: _objSeek error, status = %d ", myInput->status);
            if (myInput->threadNum > 0) {
                _l3Close(myInput->rsComm, destL3descInx);
                _l3Close(myInput->rsComm, srcL3descInx);
            }
    ...
    while (toCopy > 0) {
        ...
        if (bytesRead <= 0) {
            if (bytesRead < 0) {
                myInput->status = bytesRead;
                rodsLogError(LOG_ERROR, bytesRead, "sameHostPartialCopy: copy error for %lld", bytesRead);
            }
            else if ((myInput->flags & NO_CHK_COPY_LEN_FLAG) == 0) {
                myInput->status = SYS_COPY_LEN_ERR - errno;
                rodsLog(LOG_ERROR, "sameHostPartialCopy: toCopy %lld, bytesRead %d", toCopy, bytesRead);
            }
            break;
        }

        bytesWritten = _l3Write(myInput->rsComm, destL3descInx, buf, bytesRead);

        if (bytesWritten != bytesRead) {
            rodsLog(LOG_NOTICE, "sameHostPartialCopy:Bytes written %d don't match read %d", bytesWritten, bytesRead);
...
void locToRemPartialCopy(portalTransferInp_t* myInput)
{
    ...
    if (myInput == NULL) {
        rodsLog(LOG_NOTICE, "locToRemPartialCopy: NULL input");
        return;
    }
    ...
    while (myInput->status >= 0) {
        ...
        if (myHeader.offset != curOffset) {
            curOffset = myHeader.offset;
            myOffset = _l3Lseek(myInput->rsComm, srcL3descInx, myHeader.offset, SEEK_SET);
            if (myOffset < 0) {
                myInput->status = myOffset;
                rodsLog(LOG_NOTICE, "locToRemPartialCopy: _objSeek error, status = %d ", myInput->status);
                break;
            }
        }
        ...
        while (toGet > 0) {
            ...
            if (bytesRead != toRead) {
                if (bytesRead < 0) {
                    myInput->status = bytesRead;
                    rodsLogError(LOG_ERROR, bytesRead, "locToRemPartialCopy: copy error for %lld", bytesRead);
                }
                else if ((myInput->flags & NO_CHK_COPY_LEN_FLAG) == 0) {
                    myInput->status = SYS_COPY_LEN_ERR - errno;
                    rodsLog(LOG_ERROR, "locToRemPartialCopy: toGet %lld, bytesRead %d", toGet, bytesRead);
                }
                break;
            }
            ...
            bytesWritten = myWrite(destFd, buf, new_size, NULL);
...
int svrPortalPutGetRbudp(rsComm_t* rsComm)
{
    ...
    if (tcpSock < 0) {
        rodsLog(LOG_NOTICE, "svrPortalPutGetRbudp: acceptSrvPortal error. errno = %d", errno);
        CLOSE_SOCK(lsock);
        return tcpSock;
    }
    else {
    ...
    if (status != sizeof(udpPortBuf)) {
        rodsLog(LOG_ERROR, "svrPortalPutGetRbudp: readn error. toread %d, bytes read %d ", sizeof(udpPortBuf), status);
    ...
    if (getValByKey(&myPortalOpr->dataOprInp.condInput, VERY_VERBOSE_KW) != NULL) {
        verbose = 2;
    }
    else {
        verbose = 0;
    ...
    if (myPortalOpr->oprType == PUT_OPR) {
        ...
        if (getpeername(tcpSock, (struct sockaddr*) &rbudpReceiver.rbudpBase.udpServerAddr, &laddrlen) < 0) {
            rodsLog(LOG_NOTICE, "svrPortalPutGetRbudp() - getpeername() failed: errno=%d", errno);
        ...
        if (status < 0) {
            rodsLog(LOG_ERROR, "svrPortalPutGetRbudp: getfileByFd error for %s", FileDesc[destL3descInx].fileName);
        ...
        /* use the addr of tcp sock */
        if (getpeername(tcpSock, (struct sockaddr*) &rbudpSender.rbudpBase.udpServerAddr, &laddrlen) < 0) {
            rodsLog(LOG_NOTICE, "svrPortalPutGetRbudp() - getpeername() failed: errno=%d", errno);
            sendClose(&rbudpSender);
            return USER_RODS_HOSTNAME_ERR;
        }
        rbudpSender.rbudpBase.udpServerAddr.sin_port = htons(rbudpSender.rbudpBase.udpRemotePort);
        ...
        if (status < 0) {
            rodsLog(LOG_ERROR, "svrPortalPutGetRbudp: sendfile error for %s", FileDesc[srcL3descInx].fileName);
            status += SYS_UDP_TRANSFER_ERR;
        }
        sendClose(&rbudpSender);
    }
...
void reconnManager(rsComm_t* rsComm)
{
    ...
    if (listen(rsComm->reconnSock, 1) < 0) {
        rodsLog(LOG_ERROR, "reconnManager: listen failed, errno: %d", errno);
        return;
    }
    ...
    while (1) {
        int nSelected;
        while ((nSelected = select(nSockets, &basemask, (fd_set*) NULL, (fd_set*) NULL, NULL)) < 0) {
            if (errno == EINTR) {
        ...
        if (newSock < 0) {
            acceptFailCnt++;
            rodsLog(LOG_ERROR, "reconnManager: accept for sock %d failed, errno = %d", rsComm->reconnSock, errno);
            if (acceptFailCnt > MAX_RECON_ERROR_CNT) {
                rodsLog(LOG_ERROR, "reconnManager: accept failed cnt > 10, reconnManager exit");
                close(rsComm->reconnSock);
                rsComm->reconnSock = -1;
                rsComm->reconnPort = 0;
                return;
        ...
        if (!ret.ok()) {
            ...
            rodsLog(LOG_ERROR, "reconnManager: cookie mismatch, got = %d vs %d", reconnMsg->cookie, rsComm->cookie);
            close(newSock);
            free(reconnMsg);
            continue;
        }
...
int svrChkReconnAtSendStart(rsComm_t* rsComm)
{
    if (rsComm->reconnSock > 0) {
        ...
        if (rsComm->reconnThrState == CONN_WAIT_STATE) {
            /* should not be here */
            rodsLog(
                LOG_NOTICE, "svrChkReconnAtSendStart: ThrState = CONN_WAIT_STATE, agentState=%d", rsComm->agentState);
...
int svrChkReconnAtReadStart(rsComm_t* rsComm)
{
    if (rsComm->reconnSock > 0) {
        ...
        if (rsComm->reconnThrState == CONN_WAIT_STATE) {
            /* should not be here */
            rodsLog(
                LOG_NOTICE, "svrChkReconnAtReadStart: ThrState = CONN_WAIT_STATE, agentState=%d", rsComm->agentState);
            rsComm->agentState = PROCESSING_STATE;
            rsComm->thread_ctx->cond->wait(boost_lock);
        }
        svrSwitchConnect(rsComm);
...
int svrSockOpenForInConn(rsComm_t* rsComm, int* portNum, char** addr, int proto)
{
    ...
    if (addr != NULL && *addr != NULL &&
        (isLoopbackAddress(*addr) || strcmp(*addr, "0.0.0.0") == 0 || strcmp(*addr, "localhost") == 0)) {
        /* localhost */
        char* myaddr;
        ...
        if (myaddr != NULL) {
            ...
            *addr = strdup(myaddr);
            ...
            rodsLog(LOG_NOTICE, "svrSockOpenForInConn: problem resolving local host addr %s", *addr);
        }
    }
    return status;
}
...
char* _getSvrAddr(rodsServerHost_t* rodsServerHost)
{
    ...
    while (tmpHostName != NULL) {
        if (strcmp(tmpHostName->name, "localhost") != 0 && !isLoopbackAddress(tmpHostName->name) &&
            strcmp(tmpHostName->name, "0.0.0.0") != 0 && strchr(tmpHostName->name, '.') != NULL) {
...
int forkAndExec(char* av[])
{
    ...
    if (childPid == 0) {
        ...
        rodsLog(LOG_ERROR, "exectar: RODS_FORK failed. errno = %d", errno);
    ...
    if (status >= 0 && childStatus != 0) {
        rodsLog(LOG_ERROR, "forkAndExec: waitpid status = %d, childStatus = %d", status, childStatus);
...
int singleRemToLocCopy(rsComm_t* rsComm, dataCopyInp_t* dataCopyInp)
{
    ...
    if (dataCopyInp == NULL) {
        rodsLog(LOG_NOTICE, "singleRemToLocCopy: NULL dataCopyInp input");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    while ((bytesRead = rsDataObjRead(rsComm, &dataObjReadInp, &dataObjReadInpBBuf)) > 0) {
        bytesWritten = _l3Write(rsComm, destL3descInx, dataObjReadInpBBuf.buf, bytesRead);

        if (bytesWritten != bytesRead) {
            rodsLog(LOG_ERROR, "singleRemToLocCopy: Read %d bytes, Wrote %d bytes.\n ", bytesRead, bytesWritten);
    ...
    free(dataObjReadInpBBuf.buf);
    if (dataSize <= 0 || totalWritten == dataSize || getValByKey(&dataOprInp->condInput, NO_CHK_COPY_LEN_KW) != NULL) {
        return 0;
    }
    else {
        rodsLog(LOG_ERROR, "singleRemToLocCopy: totalWritten %lld dataSize %lld mismatch", totalWritten, dataSize);
        return SYS_COPY_LEN_ERR;
    }
}
...
int singleLocToRemCopy(rsComm_t* rsComm, dataCopyInp_t* dataCopyInp)
{
    ...
    while ((bytesRead = _l3Read(rsComm, srcL3descInx, dataObjWriteInpBBuf.buf, trans_buff_size)) > 0) {
        dataObjWriteInp.len = dataObjWriteInpBBuf.len = bytesRead;
        bytesWritten = rsDataObjWrite(rsComm, &dataObjWriteInp, &dataObjWriteInpBBuf);
        if (bytesWritten != bytesRead) {
            rodsLog(LOG_ERROR, "singleLocToRemCopy: Read %d bytes, Wrote %d bytes.\n ", bytesRead, bytesWritten);
    ...
    if (dataSize <= 0 || totalWritten == dataSize || getValByKey(&dataOprInp->condInput, NO_CHK_COPY_LEN_KW) != NULL) {
        ...
        rodsLog(LOG_ERROR, "singleLocToRemCopy: totalWritten %lld dataSize %lld mismatch", totalWritten, dataSize);
        return SYS_COPY_LEN_ERR;
    }
}
...
int singleL1Copy(rsComm_t* rsComm, dataCopyInp_t* dataCopyInp)
{
    ...
    if (dataCopyInp == NULL) {
        rodsLog(LOG_NOTICE, "singleL1Copy: NULL dataCopyInp input");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    while ((bytesRead = rsDataObjRead(rsComm, &dataObjReadInp, &dataObjReadInpBBuf)) > 0) {
        ...
        if (bytesWritten != bytesRead) {
            rodsLog(LOG_ERROR, "singleL1Copy: Read %d bytes, Wrote %d bytes.\n ", bytesRead, bytesWritten);
            free(dataObjReadInpBBuf.buf);
            return SYS_COPY_LEN_ERR;
        }
        else {
    ...
    if (dataSize <= 0 || totalWritten == dataSize || getValByKey(&dataOprInp->condInput, NO_CHK_COPY_LEN_KW) != NULL) {
        ...
        rodsLog(LOG_ERROR, "singleL1Copy: totalWritten %lld dataSize %lld mismatch", totalWritten, dataSize);
        return SYS_COPY_LEN_ERR;
    }
}
...
irods::error readStartupPack(irods::network_object_ptr _ptr, startupPack_t** startupPack, struct timeval* tv)
{
    ...
    if (myHeader.msgLen > (int) sizeof(startupPack_t) * 2 || myHeader.msgLen <= 0) {
    ...
    ret = readMsgBody(_ptr, &myHeader, &inputStructBBuf, &bsBBuf, &errorBBuf, XML_PROT, tv);
    if (!ret.ok()) {
        return PASS(ret);
    }
    ...
    if (myHeader.bsLen != 0) {
        ...
        rodsLog(LOG_NOTICE, "readStartupPack: myHeader.bsLen = %d is not 0", myHeader.bsLen);
    }
    ...
    if (myHeader.errorLen != 0) {
        ...
        rodsLog(LOG_NOTICE, "readStartupPack: myHeader.errorLen = %d is not 0", myHeader.errorLen);
    }

    /* always use XML_PROT for the startup pack */
    int status = unpackStruct(inputStructBBuf.buf, (void**) startupPack, "StartupPack_PI", RodsPackTable, XML_PROT);
...
int initServiceUser()
{
    ...
    if (errno) {
        rodsLogError(
            LOG_ERROR, SYS_USER_RETRIEVE_ERR, "setServiceUser: error in getpwnam %s, errno = %d", serviceUser, errno);
        return SYS_USER_RETRIEVE_ERR - errno;
    }
    else {
        rodsLogError(LOG_ERROR, SYS_USER_RETRIEVE_ERR, "setServiceUser: user %s doesn't exist", serviceUser);
...
int changeToRootUser()
{
    ...
    if (seteuid(0) == -1) {
        my_errno = errno;
        errno = prev_errno;
        rodsLogError(LOG_ERROR, SYS_USER_NO_PERMISSION - my_errno, "changeToRootUser: can't change to root user id");
...
int changeToServiceUser()
{
    ...
    if (seteuid(ServiceUid) == -1) {
        ...
        errno = prev_errno;
        rodsLogError(
            LOG_ERROR, SYS_USER_NO_PERMISSION - my_errno, "changeToServiceUser: can't change to service user id");
        return SYS_USER_NO_PERMISSION - my_errno;
    }
...
int changeToUser(uid_t uid)
{
    ...
    if (seteuid(uid) == -1) {
        my_errno = errno;
        errno = prev_errno;
        rodsLogError(LOG_ERROR, SYS_USER_NO_PERMISSION - my_errno, "changeToUser: can't change to user id %d", uid);
        return SYS_USER_NO_PERMISSION - my_errno;
    }
    errno = prev_errno;
...
int dropRootPrivilege()
{
    ...
    if (setuid(new_real_uid) == -1) {
        ...
        errno = prev_errno;
        rodsLogError(
            LOG_ERROR, SYS_USER_NO_PERMISSION - my_errno, "dropRootPrivilege: can't setuid() to uid %d", new_real_uid);
        return SYS_USER_NO_PERMISSION - my_errno;
    }
                                           ...
                                           std::string& output)
{
    ...
    try {
        exec << script_language << " " << irods::get_irods_home_directory().string() << "/scripts/" << script_name;
    }
    catch (const irods::exception& e) {
        irods::log(e);
    ...
    const int pclose_ret = pclose(fp);
    if (pclose_ret == -1) {
        return ERROR(SYS_FORK_ERROR, "pclose() failed.");
    }
...
irods::error add_global_re_params_to_kvp_for_dynpep(keyValPair_t& _kvp)
{
    ...
    try {
        addKeyVal(&_kvp,
                  ...
                  irods::get_server_property<const std::string>(irods::CLIENT_USER_NAME_KW).c_str());
    }
    catch (const irods::exception& e) {
    ...
    try {
        addKeyVal(&_kvp,
                  ...
                  irods::get_server_property<const std::string>(irods::CLIENT_USER_ZONE_KW).c_str());
    }
    catch (const irods::exception& e) {
    ...
    try {
        addKeyVal(&_kvp,
                  ...
                  boost::lexical_cast<std::string>(irods::get_server_property<const int>(irods::CLIENT_USER_PRIV_KW))
                      .c_str());
    }
    ...
    try {
        addKeyVal(&_kvp,
                  ...
                  irods::get_server_property<const std::string>(irods::PROXY_USER_NAME_KW).c_str());
    }
    catch (const irods::exception& e) {
    ...
    try {
        addKeyVal(&_kvp,
                  ...
                  irods::get_server_property<const std::string&>(irods::PROXY_USER_ZONE_KW).c_str());
    }
    catch (const irods::exception& e) {
...
{
    try {
        _instance_name = boost::any_cast<const std::string&>(
            boost::any_cast<const std::unordered_map<std::string, boost::any>&>(
                irods::get_server_property<const std::vector<boost::any>>(
                    std::vector<std::string>{irods::CFG_PLUGIN_CONFIGURATION_KW, irods::PLUGIN_TYPE_RULE_ENGINE})[0])
                .at(irods::CFG_INSTANCE_NAME_KW));
    }
    catch (const irods::exception& e) {
        return irods::error(e);
    }
...
irods::error list_rule_plugin_instances(std::vector<std::string>& _instance_names)
{
    try {
        ...
        for (const auto& el : rule_engines) {
            _instance_names.push_back(boost::any_cast<const std::string&>(
                boost::any_cast<const std::unordered_map<std::string, boost::any>&>(el).at(
...
void applyACLFromKVP(rsComm_t* rsComm, dataObjInp_t* dataObjInp)
{
    ...
    if (const char* serialized_acl = getValByKey(&dataObjInp->condInput, ACL_INCLUDED_KW)) {
        ...
        for (std::vector<std::vector<std::string>>::const_iterator iter = deserialized_acl.begin();
             iter != deserialized_acl.end();
             ++iter) {
            ...
            const int status_parseUserName =
