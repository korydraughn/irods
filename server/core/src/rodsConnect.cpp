...
int getAndConnRcatHost(rsComm_t* rsComm, int rcatType, const char* rcatZoneHint, rodsServerHost_t** rodsServerHost)
{
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "getAndConnRcatHost:getRcatHost() failed. erro=%d", status);
        return status;
    }
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "getAndConnRcatHost: svrToSvrConnect to %s failed", (*rodsServerHost)->hostName->name);
...
int getAndConnRcatHostNoLogin(rsComm_t* rsComm, int rcatType, char* rcatZoneHint, rodsServerHost_t** rodsServerHost)
{
    ...
    if (status < 0) {
        rodsLog(
            LOG_NOTICE, "getAndConnRcatHost: svrToSvrConnectNoLogin to %s failed", (*rodsServerHost)->hostName->name);
        if ((*rodsServerHost)->rcatEnabled == REMOTE_ICAT) {
            status = convZoneSockError(status);
        }
    }
...
int getRcatHost(int rcatType, const char* rcatZoneHint, rodsServerHost_t** rodsServerHost)
{
    ...
    if (rcatType == MASTER_RCAT || myZoneInfo->slaveServerHost == NULL) {
...
int queueAddr(rodsServerHost_t* rodsServerHost, char* myHostName)
{
    ...
    if (irods::EMPTY_RESC_HOST != myHostName) {
        ...
        if (afterTime - beforeTime >= 2) {
            rodsLog(LOG_NOTICE,
                    "WARNING WARNING: get_canonical_name of %s is taking %d sec. This could severely affect "
                    "interactivity of your Rods system",
                    myHostName,
...
int queueHostName(rodsServerHost_t* rodsServerHost, const char* myName, int topFlag)
{
...
int queueRodsServerHost(rodsServerHost_t** rodsServerHostHead, rodsServerHost_t* myRodsServerHost)
{
...
int queueZone(const char* zoneName, int portNum, rodsServerHost_t* masterServerHost, rodsServerHost_t* slaveServerHost)
{
    ...
    if (portNum <= 0) {
        if (ZoneInfoHead != NULL) {
            ...
            rodsLog(LOG_ERROR, "queueZone:  Bad input portNum %d for %s", portNum, zoneName);
            free(myZoneInfo);
    ...
    if (masterServerHost == NULL) {
        rodsLog(LOG_DEBUG, "queueZone:  masterServerHost for %s is NULL", zoneName);
        return SYS_INVALID_SERVER_HOST;
    }
    else {
        return 0;
...
int matchHostConfig(rodsServerHost_t* myRodsServerHost)
{
    ...
    if (myRodsServerHost->localFlag == LOCAL_HOST) {
        ...
        while (tmpRodsServerHost != NULL) {
            ...
            if (tmpRodsServerHost->localFlag == LOCAL_HOST && myRodsServerHost->localFlag != UNKNOWN_HOST_LOC) {
                tmpRodsServerHost = tmpRodsServerHost->next;
                continue;
            }
...
int queueConfigName(rodsServerHost_t* configServerHost, rodsServerHost_t* myRodsServerHost)
{
...
int resolveHost(rodsHostAddr_t* addr, rodsServerHost_t** rodsServerHost)
{
    ...
    if (tmpRodsServerHost == NULL) {
        rodsLog(LOG_ERROR, "resolveHost: mkServerHost error");
        return SYS_INVALID_SERVER_HOST;
    }
...
int resoAndConnHostByDataObjInfo(rsComm_t* rsComm, dataObjInfo_t* dataObjInfo, rodsServerHost_t** rodsServerHost)
{
    ...
    int remoteFlag;
    if (dataObjInfo == NULL) {
        rodsLog(LOG_NOTICE, "resoAndConnHostByDataObjInfo: NULL dataObjInfo");
...
int printZoneInfo()
{
    ...
    while (tmpZoneInfo != NULL) {
        ...
        if (tmpRodsServerHost != NULL) {
            ...
            rodsLog(
                LOG_NOTICE, " HostAddr: %s   PortNum: %d\n\n", tmpRodsServerHost->hostName->name, tmpZoneInfo->portNum);
#else  /* SYSLOG */
            fprintf(stderr, "    ZoneName: %s   ", tmpZoneInfo->zoneName);
            fprintf(stderr, "Type: LOCAL_SLAVE_ICAT   ");
            fprintf(stderr, " HostAddr: %s   PortNum: %d\nn", tmpRodsServerHost->hostName->name, tmpZoneInfo->portNum);
#endif /* SYSLOG */
        }
...
int getZoneInfo(const char* rcatZoneHint, zoneInfo_t** myZoneInfo)
{
    ...
    *myZoneInfo = NULL;
    ...
    if (zoneInput == 0) {
        rodsLog(LOG_ERROR, "getRcatHost: No local Rcat");
        return SYS_INVALID_ZONE_NAME;
    }
    else {
        rodsLog(LOG_DEBUG, "getZoneInfo: Invalid zone name from hint %s", rcatZoneHint);
...
int getLocalZoneInfo(zoneInfo_t** outZoneInfo)
{
    ...
    rodsLog(LOG_ERROR, "getLocalZoneInfo: Local Zone does not exist");
                         ...
                         char* remoteZoneOpr)
{
    ...
    status = getRemoteZoneHost(rsComm, dataObjInp, rodsServerHost, remoteZoneOpr);
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "getAndConnRemoteZone: svrToSvrConnect to %s failed", (*rodsServerHost)->hostName->name);
    }
    if (status >= 0) {
        return REMOTE_HOST;
    }
...
int getAndConnRemoteZoneForCopy(rsComm_t* rsComm, dataObjCopyInp_t* dataObjCopyInp, rodsServerHost_t** rodsServerHost)
{
    ...
    status = getRcatHost(MASTER_RCAT, srcDataObjInp->objPath, &srcIcatServerHost);

    if (status < 0 || NULL == srcIcatServerHost) { // JMC cppcheck - nullptr
        rodsLog(LOG_ERROR, "getAndConnRemoteZoneForCopy: getRcatHost error for %s", srcDataObjInp->objPath);
        return status;
    }
    ...
    status = getRcatHost(MASTER_RCAT, destDataObjInp->objPath, &destIcatServerHost);

    if (status < 0 || NULL == destIcatServerHost) { // JMC cppcheck - nullptr
        rodsLog(LOG_ERROR, "getAndConnRemoteZoneForCopy: getRcatHost error for %s", destDataObjInp->objPath);
        return status;
    }
...
int getAndConnReHost(rsComm_t* rsComm, rodsServerHost_t** rodsServerHost)
{
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "getAndConnReHost:getReHost() failed. erro=%d", status);
        return status;
    }
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "getAndConnReHost: svrToSvrConnect to %s failed", (*rodsServerHost)->hostName->name);
    }
    if (status >= 0) {
        return REMOTE_HOST;
    }
