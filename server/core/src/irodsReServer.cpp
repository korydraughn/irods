...
void reServerMain(char* logDir)
{
    ...
    try {
        while (true) {
            ...
            rcComm_t* rc_comm =
                rcConnect(env.rodsHost, env.rodsPort, env.rodsUserName, env.rodsZone, NO_RECONN, &rcConnect_error_msg);
            if (!rc_comm) {
                rodsLog(LOG_ERROR,
                        "rcConnect failed %ji, %s",
                        static_cast<intmax_t>(rcConnect_error_msg.status),
                        rcConnect_error_msg.msg);
                reSvrSleep();
                continue;
            }
            ...
            /* run the failed job */
            runCnt = runQueuedRuleExec(rc_comm, &reExec, &genQueryOut, endTime, RE_FAILED_STATUS);
            closeQueryOut(rc_comm, genQueryOut);
            freeGenQueryOut(&genQueryOut);
            if (runCnt > 0 || (genQueryOut != NULL && genQueryOut->continueInx > 0)) {
                rcDisconnect(rc_comm);
