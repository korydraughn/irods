...
namespace irods
{
    ...
    error resource_manager::resolve(std::string _key, resource_ptr& _value)
    {
        ...
        if (resource_name_map_.has_entry(_key)) {
            ...
            msg << "no resource found for name [" << _key << "]";
    ...
    // =-=-=-=-=-=-=-
    // public - retrieve a resource given its key
    error resource_manager::resolve(rodsLong_t _resc_id, resource_ptr& _value)
    {
        if (resource_id_map_.has_entry(_resc_id)) {
            ...
            std::stringstream msg;
            msg << "no resource found for name [" << _resc_id << "]";
            return ERROR(SYS_RESC_DOES_NOT_EXIST, msg.str());
        }
    ...
    {
        resource* resc = 0;
        error ret = load_plugin<resource>(resc, _plugin_name, PLUGIN_TYPE_RESOURCE, _inst_name, _context);
        if (ret.ok() && resc) {
            _plugin.reset(resc);
        }
        else {
    ...
    error resource_manager::validate_vault_path(std::string _physical_path,
        ...
        while (continueInx > 0) {
            ...
            if (status < 0) {
                ...
                if (status != CAT_NO_ROWS_FOUND) {
                    // actually an error
                    rodsLog(LOG_NOTICE, "initResc: rsGenQuery error, status = %d", status);
    ...
    error resource_manager::get_root_resources(std::vector<std::string>& _list)
    {
        // =-=-=-=-=-=-=-
        // iterate over all resources in the table
        lookup_table<boost::shared_ptr<resource>>::iterator itr;
    ...
    error resource_manager::get_hier_to_root_for_resc(const std::string& _resc_name, std::string& _hierarchy)
    {
        ...
        while (!parent_name.empty()) {
            error ret = resolve(parent_name, resc);
            if (!ret.ok()) {
                return PASS(ret);
            }

            ret = get_parent_name(resc, parent_name);
            if (!ret.ok()) {
            ...
            if (!parent_name.empty()) {
                _hierarchy = parent_name + irods::hierarchy_parser::delimiter() + _hierarchy;
            }
        } // while
    ...
    {
        resource_ptr resc;
        error ret = resolve(_resc_name, resc);
        if (!ret.ok()) {
            return PASS(ret);
        }
        ...
        if (resc->num_children() > 0) {
            ...
            rodsLong_t resc_id;
            ret = resc->get_property<rodsLong_t>(RESOURCE_ID, resc_id);
            if (!ret.ok()) {
                return PASS(ret);
            }
    ...
    std::vector<resource_manager::leaf_bundle_t> resource_manager::gather_leaf_bundles_for_resc(
    ...
    error resource_manager::init_child_map(void)
    {
        for (auto itr : resource_name_map_) {
            ...
            std::string parent_id_str;
            error ret = child_resc->get_property<std::string>(RESOURCE_PARENT, parent_id_str);
            ...
            if (!resource_id_map_.has_entry(parent_id)) {
                rodsLog(LOG_ERROR, "invalid parent resource id %ld", parent_id);
                continue;
            }
            ...
            ret = child_resc->get_property<std::string>(RESOURCE_PARENT_CONTEXT, parent_child_context);
            if (!ret.ok()) {
            }

            parent_resc->add_child(child_name, parent_child_context, child_resc);
    ...
    error resource_manager::gather_operations()
    {
        ...
        for (resc_itr = resource_name_map_.begin(); resc_itr != resource_name_map_.end(); ++resc_itr) {
            ...
            if (get_err.ok()) {
                ...
                itr =
                    std::find<std::vector<std::string>::iterator, std::string>(proc_vec.begin(), proc_vec.end(), name);
                if (proc_vec.end() != itr) {
                    continue;
                }
            }
    ...
    int resource_manager::call_maintenance_operations(rcComm_t* _comm)
    {
        ...
        for (vec_itr = maintenance_operations_.begin(); vec_itr != maintenance_operations_.end(); ++vec_itr) {
            // =-=-=-=-=-=-=-
            // iterate through ops
            std::vector<pdmo_type>::iterator op_itr;
            for (op_itr = vec_itr->begin(); op_itr != vec_itr->end(); ++op_itr) {
                // =-=-=-=-=-=-=-
                // call the op
                try {
    ...
    std::vector<std::string> resource_manager::get_all_resc_hierarchies(void)
    {
        ...
        for (const auto& entry : resource_name_map_) {
            ...
            error err = resc->get_property<rodsLong_t>(RESOURCE_ID, leaf_id);
            if (!err.ok()) {
                THROW(err.code(), err.result());
            }
