...
int xmsgServerMain()
{
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "xmsgServerMain: startXmsgThreads error. status = %d", status);
        return status;
    }
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "xmsgServerMain: initRsComm error. status = %d", status);
        return status;
    }
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "xmsgServerMain: initAgent error. status = %d", status);
        return status;
    }
    ...
    try {
        ...
        svrComm.sock = sockOpenForInConn(&svrComm, &xmsgPort, NULL, SOCK_STREAM);
    }
    ...
    if (svrComm.sock < 0) {
        rodsLog(LOG_NOTICE, "xmsgServerMain: sockOpenForInConn error. status = %d", svrComm.sock);
        exit(1);
    }
    ...
    while (1) { /* infinite loop */
        ...
        while ((numSock = select(svrComm.sock + 1, &sockMask, (fd_set*) NULL, (fd_set*) NULL, (struct timeval*) NULL)) <
               0) {
            if (errno == EINTR) {
        ...
        if (newSock < 0) {
            rodsLog(LOG_NOTICE, "xmsgServerMain: acceptConn () error, errno = %d", errno);
            continue;
        }
