...
namespace irods
{
    ...
    // =-=-=-=-=-=-=-
    // public - cctor
    oracle_object::oracle_object(const oracle_object& _rhs)
    ...
    oracle_object& oracle_object::operator=(const oracle_object&)
    {
        return *this;
    ...
    bool oracle_object::operator==(const oracle_object&) const
    {
        return false;
    ...
    error oracle_object::resolve(const std::string& _interface, plugin_ptr& _ptr)
    {
        // =-=-=-=-=-=-=-
        ...
        if (!ret.ok()) {
            ...
            ret = db_mgr.init_from_type(
                ORACLE_DATABASE_PLUGIN, ORACLE_DATABASE_PLUGIN, ORACLE_DATABASE_PLUGIN, empty_context, db_ptr);
            if (!ret.ok()) {
                return PASS(ret);
            }
            else {
    ...
    error oracle_object::get_re_vars(rule_engine_vars_t&)
    {
