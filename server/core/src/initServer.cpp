...
initServerInfo(int processType, rsComm_t* rsComm)
{
    ...
    try {
        ...
        status = queueZone(zone_name.c_str(), zone_port, NULL, NULL);
        if (status < 0) {
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "initServerInfo: initHostConfigByFile error, status = %d", status);
        return status;
    }

    status = initLocalServerHost();
    if (status < 0) {
        rodsLog(LOG_NOTICE, "initServerInfo: initLocalServerHost error, status = %d", status);
        return status;
    }

    status = initRcatServerHostByFile();
    if (status < 0) {
        rodsLog(LOG_SYS_FATAL, "initServerInfo: initRcatServerHostByFile error, status = %d", status);
        return status;
    }
    ...
    if (irods::CFG_SERVICE_ROLE_PROVIDER == svc_role) {
        status = connectRcat();
        if (status < 0) {
            rodsLog(LOG_SYS_FATAL, "initServerInfo: connectRcat failed, status = %d", status);
    ...
    if (status < 0) {
        rodsLog(LOG_SYS_FATAL, "initServerInfo: initZone error, status = %d", status);
        return status;
    }
...
int initLocalServerHost()
{
    ...
    if (status < 0) {
        ...
        rodsLog(LOG_NOTICE, "initLocalServerHost: gethostname error, status = %d", status);
        return status;
    }
    ...
    if (status < 0) {
        ...
        rodsLog(LOG_NOTICE, "initLocalServerHost: queueAddr error, status = %d", status);
        status = 0;
    }
...
int initRcatServerHostByFile()
{
    std::string prop_str;
    try {
        snprintf(KerberosName,
                 sizeof(KerberosName),
                 "%s",
                 irods::get_server_property<const std::string>(irods::CFG_KERBEROS_NAME_KW).c_str());
    }
    catch (const irods::exception& e) {
    }
    ...
    try {
        ...
        snprintf(addr.hostAddr,
                 sizeof(addr.hostAddr),
                 "%s",
                 boost::any_cast<const std::string&>(
                     irods::get_server_property<const std::vector<boost::any>>(irods::CFG_CATALOG_PROVIDER_HOSTS_KW)[0])
                     .c_str());
        int rem_flg = resolveHost(&addr, &tmp_host);
        if (rem_flg < 0) {
    ...
    try {
        ...
        snprintf(addr.hostAddr,
                 sizeof(addr.hostAddr),
                 "%s",
                 irods::get_server_property<const std::string>(irods::CFG_IRODS_XMSG_HOST_KW).c_str());
        int rem_flg = resolveHost(&addr, &tmp_host);
        if (rem_flg < 0) {
            rodsLog(LOG_SYS_FATAL,
                    "initRcatServerHostByFile: resolveHost error for %s, status = %d",
    ...
    try {
        snprintf(localSID,
                 sizeof(localSID),
                 "%s",
                 irods::get_server_property<const std::string>(irods::CFG_ZONE_KEY_KW).c_str());
    }
    catch (const irods::exception& e) {
        try {
            snprintf(localSID,
                     sizeof(localSID),
                     "%s",
                     irods::get_server_property<const std::string>(LOCAL_ZONE_SID_KW).c_str());
        }
        catch (const irods::exception& e) {
            irods::log(irods::error(e));
            return e.code();
        ...
        for (const auto& el : irods::get_server_property<const std::vector<boost::any>>(irods::CFG_FEDERATION_KW)) {
            try {
                ...
                try {
                    ...
                    const auto& fed_zone_negotiation_key =
                        boost::any_cast<std::string>(federation.at(irods::CFG_NEGOTIATION_KEY_KW));
        ...
        try {
            for (const auto& rem_sid : irods::get_server_property<std::vector<std::string>>(REMOTE_ZONE_SID_KW)) {
                ...
                if (pos == std::string::npos) {
                    ...
                    try {
                        const auto& neg_key =
                            irods::get_server_property<const std::string>(irods::CFG_NEGOTIATION_KEY_KW);
...
{
    if (ZoneInfoHead && (strlen(ZoneInfoHead->zoneName) > 0) && ZoneInfoHead->masterServerHost) {
        return 0;
    }
    ...
    if (status < 0) {
        ...
        rodsLog(LOG_NOTICE, "initZone: rsGenQuery error, status = %d", status);
        return status;
    }

    if (genQueryOut == NULL) {
        rodsLog(LOG_NOTICE, "initZone: NULL genQueryOut");
        return CAT_NO_ROWS_FOUND;
    }

    if ((zoneName = getSqlResultByInx(genQueryOut, COL_ZONE_NAME)) == NULL) {
        rodsLog(LOG_NOTICE, "initZone: getSqlResultByInx for COL_ZONE_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((zoneType = getSqlResultByInx(genQueryOut, COL_ZONE_TYPE)) == NULL) {
        rodsLog(LOG_NOTICE, "initZone: getSqlResultByInx for COL_ZONE_TYPE failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((zoneConn = getSqlResultByInx(genQueryOut, COL_ZONE_CONNECTION)) == NULL) {
        rodsLog(LOG_NOTICE, "initZone: getSqlResultByInx for COL_ZONE_CONNECTION failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((zoneComment = getSqlResultByInx(genQueryOut, COL_ZONE_COMMENT)) == NULL) {
        rodsLog(LOG_NOTICE, "initZone: getSqlResultByInx for COL_ZONE_COMMENT failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    ...
    for (i = 0; i < genQueryOut->rowCnt; i++) {
        ...
        if (strcmp(tmpZoneType, "local") == 0) {
            ...
            rodsLog(LOG_ERROR, "initZone: connection info for zone %s not configured", tmpZoneName);
            continue;
        }
        ...
        if (tmpRodsServerHost->rcatEnabled == LOCAL_ICAT) {
            rodsLog(LOG_ERROR, "initZone: address %s for remote zone %s already in use", tmpZoneConn, tmpZoneName);
            continue;
        }
...
int initAgent(int processType, rsComm_t* rsComm)
{
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "initAgent: initServerInfo error, status = %d", status);
        return status;
    }
    ...
    status = initFileDesc();
    if (status < 0) {
        rodsLog(LOG_ERROR, "initAgent: initFileDesc error, status = %d", status);
        return status;
    }
    ...
    if (ProcessType == AGENT_PT) {
        status = applyRule("acChkHostAccessControl", NULL, &rei, NO_SAVE_REI);

        if (status < 0) {
            rodsLog(LOG_ERROR, "initAgent: acChkHostAccessControl error, status = %d", status);
            return status;
        }
    }
    ...
    if (rsComm->reconnFlag == RECONN_TIMEOUT) {
        rsComm->reconnSock = svrSockOpenForInConn(rsComm, &rsComm->reconnPort, &rsComm->reconnAddr, SOCK_STREAM);
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "initAgent: acSetPublicUserPolicy error, status = %d", status);
        return status;
    }
...
void rsPipeSignalHandler(int)
{
    ...
    if (ThisComm == NULL || ThisComm->reconnSock <= 0) {
        rodsLog(LOG_NOTICE, "caught a broken pipe signal and exiting");
        cleanupAndExit(SYS_CAUGHT_SIGNAL);
    }
    else {
        curTime = time(0);
        ...
        rodsLog(LOG_NOTICE, "caught a broken pipe signal. Attempt to reconnect");
...
int initHostConfigByFile()
{
    ...
    if (!ret.ok()) {
        rodsLog(LOG_NOTICE, "config file [%s] not found", HOST_CONFIG_FILE);
        return 0;
    }
        ...
        for (const auto& el : cfg.get<const std::vector<boost::any>>("host_entries")) {
            try {
                ...
                if ("remote" == address_type) {
                    ...
                    free(svr_host);
                    rodsLog(LOG_ERROR, "unsupported address type [%s]", address_type.c_str());
                    continue;
                }
...
int initRsComm(rsComm_t* rsComm)
{
    ...
    try {
        ...
        rstrcpy(rsComm->proxyUser.authInfo.authScheme, zone_auth_scheme.c_str(), NAME_LEN);
        rstrcpy(rsComm->clientUser.userName, zone_user.c_str(), NAME_LEN);
        rstrcpy(rsComm->clientUser.rodsZone, zone_name.c_str(), NAME_LEN);
        rstrcpy(rsComm->clientUser.authInfo.authScheme, zone_auth_scheme.c_str(), NAME_LEN);
        /* assume LOCAL_PRIV_USER_AUTH */
        rsComm->clientUser.authInfo.authFlag = rsComm->proxyUser.authInfo.authFlag = LOCAL_PRIV_USER_AUTH;
    }
    catch (const irods::exception& e) {
        irods::log(irods::error(e));
        return e.code();
...
int initRsCommWithStartupPack(rsComm_t* rsComm, startupPack_t* startupPack)
{
    ...
    if (startupPack != NULL) {
        ...
        if (tmpStr == NULL) {
            rodsLog(LOG_NOTICE, "initRsCommWithStartupPack: env %s does not exist", SP_NEW_SOCK);
        ...
        if (tmpStr == NULL) {
            rodsLog(LOG_NOTICE, "initRsCommWithStartupPack: env %s does not exist", SP_CONNECT_CNT);
            return SYS_GETSTARTUP_PACK_ERR;
        }
        ...
        if (tmpStr == NULL) {
            rodsLog(LOG_NOTICE, "initRsCommWithStartupPack: env %s does not exist", SP_PROTOCOL);
            return SYS_GETSTARTUP_PACK_ERR;
        }
        ...
        if (tmpStr == NULL) {
            rodsLog(LOG_NOTICE, "initRsCommWithStartupPack: env %s does not exist", SP_RECONN_FLAG);
            return SYS_GETSTARTUP_PACK_ERR;
        }
        ...
        if (tmpStr == NULL) {
            rodsLog(LOG_NOTICE, "initRsCommWithStartupPack: env %s does not exist", SP_PROXY_USER);
            return SYS_GETSTARTUP_PACK_ERR;
        }
        ...
        if (tmpStr == NULL) {
            rodsLog(LOG_NOTICE, "initRsCommWithStartupPack: env %s does not exist", SP_PROXY_RODS_ZONE);
            return SYS_GETSTARTUP_PACK_ERR;
        }
        ...
        if (tmpStr == NULL) {
            rodsLog(LOG_NOTICE, "initRsCommWithStartupPack: env %s does not exist", SP_CLIENT_USER);
            return SYS_GETSTARTUP_PACK_ERR;
        }
        ...
        if (tmpStr == NULL) {
            rodsLog(LOG_NOTICE, "initRsCommWithStartupPack: env %s does not exist", SP_CLIENT_RODS_ZONE);
            return SYS_GETSTARTUP_PACK_ERR;
        }
        ...
        if (tmpStr == NULL) {
            rodsLog(LOG_NOTICE, "getstartupPackFromEnv: env %s does not exist", SP_REL_VERSION);
            return SYS_GETSTARTUP_PACK_ERR;
        }
        ...
        if (tmpStr == NULL) {
            rodsLog(LOG_NOTICE, "initRsCommWithStartupPack: env %s does not exist", SP_API_VERSION);
            return SYS_GETSTARTUP_PACK_ERR;
        }
        ...
        if (tmpStr == NULL) {
            rodsLog(LOG_NOTICE, "initRsCommWithStartupPack: env %s does not exist", SP_OPTION);
        }
        else {
...
std::set<std::string> construct_controlled_user_set(
    const std::unordered_map<std::string, boost::any>& controlled_user_connection_list)
{
    std::set<std::string> user_set;
    try {
        for (const auto& user :
             boost::any_cast<const std::vector<boost::any>&>(controlled_user_connection_list.at("users"))) {
            user_set.insert(boost::any_cast<const std::string&>(user));
        }
    }
    catch (const boost::bad_any_cast& e) {
        THROW(INVALID_ANY_CAST,
...
int chkAllowedUser(const char* userName, const char* rodsZone)
{
    ...
    try {
        controlled_user_connection_list.reset(
            irods::get_server_property<const std::unordered_map<std::string, boost::any>>(
    ...
    try {
        const auto& control_type =
            boost::any_cast<const std::string&>(controlled_user_connection_list->at("control_type"));
...
int purgeLockFileDir(int chkLockFlag)
{
    ...
    if (dirPtr == NULL) {
        rodsLog(LOG_ERROR, "purgeLockFileDir: opendir error for %s, errno = %d", lock_dir.c_str(), errno);
    ...
    while ((myDirent = readdir(dirPtr)) != NULL) {
        ...
        if (chkLockFlag) {
            ...
            if (myFd < 0) {
                ...
                rodsLogError(LOG_ERROR, savedStatus, "purgeLockFileDir: open error for %s", lockFilePath);
                continue;
            }
        ...
        if (status != 0) {
            rodsLog(LOG_ERROR, "purgeLockFileDir: stat error for %s, errno = %d", lockFilePath, errno);
