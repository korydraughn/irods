...
namespace irods
{
    ...
    // =-=-=-=-=-=-=-
    // public - ctor
    structured_object::structured_object(subStructFileFdOprInp_t& _fd_inp)
        : file_object()
        , sub_file_path_("")
        , offset_(0)
        , spec_coll_(0)
    ...
    error structured_object::resolve(const std::string& _interface, plugin_ptr& _ptr)
    {
        // =-=-=-=-=-=-=-
        ...
        if (TAR_STRUCT_FILE_T == spec_coll_type_) {
            ...
            std::stringstream msg;
            msg << "unsupported structured type: " << spec_coll_type_;
            return ERROR(SYS_INVALID_INPUT_PARAM, msg.str());
        }
        ...
        irods::error err = resc_mgr.resolve(plugin_type, resc_ptr);
        if (err.ok()) {
            ...
            error init_err = resc_mgr.init_from_type(plugin_type,        // type
                                                     plugin_type,        // key
    ...
    error structured_object::get_re_vars(rule_engine_vars_t& _kvp)
    {
