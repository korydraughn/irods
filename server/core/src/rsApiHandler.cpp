...
int rsApiHandler(rsComm_t* rsComm, int apiNumber, bytesBuf_t* inputStructBBuf, bytesBuf_t* bsBBuf)
{
    ...
    if (apiInx < 0) {
        ...
        sendRodsMsg(net_obj, RODS_API_REPLY_T, NULL, NULL, NULL, apiInx, rsComm->irodsProt);
        return apiInx;
    }
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "rsApiHandler: User has no permission for apiNumber %d", apiNumber);
        sendApiReply(rsComm, apiInx, status, myOutStruct, &myOutBsBBuf);
        return status;
    }
    ...
    if (inputStructBBuf->len > 0 && RsApiTable[apiInx]->inPackInstruct == NULL) {
        rodsLog(LOG_NOTICE, "rsApiHandler: input struct error 1 for apiNumber %d", apiNumber);
        sendApiReply(rsComm, apiInx, SYS_API_INPUT_ERR, myOutStruct, &myOutBsBBuf);
        return SYS_API_INPUT_ERR;
    }

    if (inputStructBBuf->len <= 0 && RsApiTable[apiInx]->inPackInstruct != NULL) {
        rodsLog(LOG_NOTICE, "rsApiHandler: input struct error 2 for apiNumber %d", apiNumber);
        sendApiReply(rsComm, apiInx, SYS_API_INPUT_ERR, myOutStruct, &myOutBsBBuf);
        return SYS_API_INPUT_ERR;
    }

    if (bsBBuf->len > 0 && RsApiTable[apiInx]->inBsFlag <= 0) {
        rodsLog(LOG_NOTICE, "rsApiHandler: input byte stream error for apiNumber %d", apiNumber);
        sendApiReply(rsComm, apiInx, SYS_API_INPUT_ERR, myOutStruct, &myOutBsBBuf);
        return SYS_API_INPUT_ERR;
    }

    if (inputStructBBuf->len > 0) {
        status = unpackStruct(inputStructBBuf->buf,
                              (void**) (static_cast<void*>(&myInStruct)),
                              (char*) RsApiTable[apiInx]->inPackInstruct,
                              RodsPackTable,
                              rsComm->irodsProt);
        if (status < 0) {
            rodsLog(LOG_NOTICE, "rsApiHandler: unpackStruct error for apiNumber %d, status = %d", apiNumber, status);
            sendApiReply(rsComm, apiInx, status, myOutStruct, &myOutBsBBuf);
            return status;
        }
    }
    ...
    if (numArg == 0) {
        retVal = api_entry->call_wrapper(api_entry.get(), rsComm);
    }
    else if (numArg == 1) {
        retVal = api_entry->call_wrapper(api_entry.get(), rsComm, myArgv[0]);
    }
    else if (numArg == 2) {
        retVal = api_entry->call_wrapper(api_entry.get(), rsComm, myArgv[0], myArgv[1]);
    }
    else if (numArg == 3) {
        retVal = api_entry->call_wrapper(api_entry.get(), rsComm, myArgv[0], myArgv[1], myArgv[2]);
    }
    else if (numArg == 4) {
        retVal = api_entry->call_wrapper(api_entry.get(), rsComm, myArgv[0], myArgv[1], myArgv[2], myArgv[3]);
    }
...
int sendApiReply(rsComm_t* rsComm, int apiInx, int retVal, void* myOutStruct, bytesBuf_t* myOutBsBBuf)
{
    ...
    if (RsApiTable[apiInx]->outPackInstruct != NULL && myOutStruct != NULL) {
        status = packStruct((char*) myOutStruct,
                            &outStructBBuf,
        ...
        if (status < 0) {
            ...
            sendRodsMsg(net_obj, RODS_API_REPLY_T, NULL, NULL, NULL, status, rsComm->irodsProt);
    ...
    if (rsComm->rError.len > 0) {
        status = packStruct((char*) &rsComm->rError, &rErrorBBuf, "RError_PI", RodsPackTable, 0, rsComm->irodsProt);
...
int chkApiPermission(rsComm_t* rsComm, int apiInx)
{
    ...
    if (ProcessType == XMSG_SERVER_PT) {
        if ((xmsgSvrOnly + xmsgSvrAlso) == 0) {
            rodsLog(
                LOG_ERROR, "chkApiPermission: xmsgServer not allowed to handle api %d", RsApiTable[apiInx]->apiNumber);
            return SYS_NO_API_PRIV;
        }
    }
    else if (xmsgSvrOnly != 0) {
        rodsLog(
            LOG_ERROR, "chkApiPermission: non xmsgServer not allowed to handle api %d", RsApiTable[apiInx]->apiNumber);
        return SYS_NO_API_PRIV;
    }
...
static int apply_acPostProcForParallelTransferReceived(rsComm_t* rsComm)
{
    ...
    rei.rsComm = rsComm;
    int ret =
        applyRuleArg("acPostProcForParallelTransferReceived", args, sizeof(args) / sizeof(args[0]), &rei, NO_SAVE_REI);
    return ret;
}
...
int handlePortalOpr(rsComm_t* rsComm)
{
    ...
    switch (oprType) {
        ...
        case GET_OPR:
            ...
            break;
        default:
            rodsLog(LOG_NOTICE, "handlePortalOpr: Invalid portal oprType: %d", oprType);
...
int readAndProcClientMsg(rsComm_t* rsComm, int flags)
{
        ...
        while (1) {
            ...
            if (!ret.ok()) {
                if (isL1descInuse() && retryCnt < MAX_READ_HEADER_RETRY) {
                    rodsLogError(
                        LOG_ERROR, status, "readAndProcClientMsg:readMsgHeader error. status = %d", ret.code());
                    retryCnt++;
                    continue;
                }
                if (ret.code() == USER_SOCK_CONNECT_TIMEDOUT) {
                    rodsLog(LOG_ERROR, "readAndProcClientMsg: readMsgHeader by pid %d timedout", getpid());
                    return ret.code();
                }
            }
            break;
    ...
    ret = readMsgBody(net_obj, &myHeader, &inputStructBBuf, &bsBBuf, &errorBBuf, rsComm->irodsProt, NULL);
    if (!ret.ok()) {
        irods::log(PASS(ret));
        svrChkReconnAtReadEnd(rsComm);
        return ret.code();
    ...
    if (strcmp(myHeader.type, RODS_API_REQ_T) == 0) {
        ...
        rodsLog(LOG_NOTICE, "agentMain: msg type %s not support by server", myHeader.type);
...
int sendAndRecvBranchMsg(rsComm_t* rsComm, int apiInx, int status, void* myOutStruct, bytesBuf_t* myOutBsBBuf)
{
    ...
    retval = sendAndProcApiReply(rsComm, apiInx, status, myOutStruct, myOutBsBBuf);
    if (retval < 0) {
        rodsLog(LOG_ERROR, "sendAndRecvBranchMsg: sendAndProcApiReply error. status = %d", retval);
...
int svrSendCollOprStat(rsComm_t* rsComm, collOprStat_t* collOprStat)
{
    ...
    if (status != SYS_CLI_TO_SVR_COLL_STAT_REPLY) {
        rodsLog(LOG_ERROR, "svrSendCollOprStat: client reply %d != %d.", status, SYS_CLI_TO_SVR_COLL_STAT_REPLY);
...
int _svrSendCollOprStat(rsComm_t* rsComm, collOprStat_t* collOprStat)
{
    ...
    status = sendAndProcApiReply(rsComm, rsComm->apiInx, SYS_SVR_TO_CLI_COLL_STAT, collOprStat, NULL);
    if (status < 0) {
        rodsLogError(LOG_ERROR, status, "svrSendCollOprStat: sendAndProcApiReply failed. status = %d", status);
        return status;
    }
    ...
    if (status < 0) {
        rodsLogError(LOG_ERROR,
                     status,
                     "_svrSendCollOprStat: read handshake failed. [%s] status = %d",
...
void readTimeoutHandler(int)
{
    alarm(0);
    if (isL1descInuse()) {
        rodsLog(LOG_ERROR, "readTimeoutHandler: read header by %d timed out. L1desc is busy.", getpid());
        longjmp(Jenv, L1DESC_INUSE);
    }
    else {
        rodsLog(LOG_ERROR, "readTimeoutHandler: read header by %d has timed out.", getpid());
