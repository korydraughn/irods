...
namespace irods
{
                               ...
                               std::string& _resc_name)
    {
        ...
        if ("null" != _resc_list && "forced" == _option && _comm->proxyUser.authInfo.authFlag < LOCAL_PRIV_USER_AUTH) {
            _cond_input = NULL;
        }
        else if (_cond_input) {
            char* name = NULL;
    ...
    error resolve_resource_name(const std::string& _resc_name, keyValPair_t* _cond_input, std::string& _out)
    {
        if (_resc_name.empty()) {
            char* name = 0;
...
// for resource group given a resource name
    ...
    error get_host_for_hier_string(const std::string& _hier_str, // hier string
    ...
    error get_vault_path_for_hier_string(const std::string& _hier, std::string& _rtn_vault_path)
    {
        rodsLong_t resc_id = 0;
        irods::error ret = resc_mgr.hier_to_leaf_id(_hier, resc_id);
        if (!ret.ok()) {
    ...
    error get_resc_type_for_hier_string(const std::string& _hier, std::string& _resc_type)
    {
        rodsLong_t resc_id = 0;
        irods::error ret = resc_mgr.hier_to_leaf_id(_hier, resc_id);
        if (!ret.ok()) {
