...
int initProcLog()
{
    snprintf(ProcLogDir, MAX_NAME_LEN, "%s/%s", getLogDir(), PROC_LOG_DIR_NAME);
    return 0;
}
...
int logAgentProc(rsComm_t* rsComm)
{
    ...
    if (*rsComm->clientUser.userName == '\0' || *rsComm->proxyUser.userName == '\0') {
        return 0;
    }
    ...
    if (fptr == NULL) {
        rodsLog(LOG_ERROR, "logAgentProc: Cannot open input file %s. errno = %d", procPath, errno);
...
int readProcLog(int pid, procLog_t* procLog)
{
    ...
    if (procTokens.size() != 7) {
        rodsLog(LOG_ERROR, "readProcLog: error fscanf file %s. Number of param read = %d", procPath, procTokens.size());
