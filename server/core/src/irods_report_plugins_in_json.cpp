...
namespace irods
{
    error add_plugin_type_to_json_array(const std::string _plugin_type, const char* _type_name, json_t*& _json_array)
    {
        ...
        for (plugin_name_generator::plugin_list_t::iterator itr = plugin_list.begin(); itr != plugin_list.end();
             ++itr) {
    ...
    error get_plugin_array(json_t*& _plugins)
    {
        _plugins = json_array();
        if (!_plugins) {
    ...
    error serialize_resource_plugin_to_json(const resource_ptr& _resc, json_t* _entry)
    {
        if (!_entry) {
