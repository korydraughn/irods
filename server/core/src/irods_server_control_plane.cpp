...
namespace irods
{
    ...
    static error forward_server_control_command(const std::string& _name,
                                                ...
                                                std::string& _output)
    {
        ...
        try {
            ...
            encryption_algorithm.reset(
                get_server_property<const std::string>(CFG_SERVER_CONTROL_PLANE_ENCRYPTION_ALGORITHM_KW));
        ...
        irods::error ret = crypt.encrypt(shared_secret, iv, data_to_encrypt, data_to_send);
        if (!ret.ok()) {
            return PASS(ret);
        }
        ...
        memcpy(rep.data(), data_to_send.data(), data_to_send.size());
        zmq_skt.send(rep);
    ...
    static error server_operation_shutdown(const std::string& _wait_option,
    ...
    static error rule_engine_operation_shutdown(const std::string&, // _wait_option,
                                                const size_t,       //       _wait_seconds,
                                                std::string& _output)
    {
        rodsEnv my_env;
    ...
    static error operation_pause(const std::string&, // _wait_option,
                                 const size_t,       //       _wait_seconds,
                                 std::string& _output)
    {
        rodsEnv my_env;
    ...
    static error operation_resume(const std::string&, // _wait_option,
                                  const size_t,       //       _wait_seconds,
                                  std::string& _output)
    {
        rodsEnv my_env;
    ...
    static error operation_status(const std::string&, // _wait_option,
                                  const size_t,       //       _wait_seconds,
                                  std::string& _output)
    {
        rodsEnv my_env;
        ...
        if (!obj) {
            return ERROR(SYS_MALLOC_ERR, "allocation of json object failed");
        }
        ...
        if (!arr) {
            return ERROR(SYS_MALLOC_ERR, "allocation of json array failed");
        }
    ...
    static error operation_ping(const std::string&, // _wait_option,
                                const size_t,       //       _wait_seconds,
                                std::string& _output)
    {
        _output += "{\n    \"status\": \"alive\"\n},\n";
    ...
    bool server_control_executor::compare_host_names(const std::string& _hn1, const std::string& _hn2)
    {
    ...
    bool server_control_executor::is_host_in_list(const std::string& _hn, const host_list_t& _hosts)
    {
        for (size_t i = 0; i < _hosts.size(); ++i) {
            if (compare_host_names(_hn, _hosts[i])) {
                return true;
            }
        ...
        : port_prop_(_prop)
    {
        ...
        icat_host_name_ = boost::any_cast<const std::string&>(
        ...
        if ("localhost" == icat_host_name_) {
            ...
            rodsLog(LOG_ERROR, "server_control_executor - icat_host_name is localhost, not a fqdn");
            // TODO :: throw fancy exception here when we disallow localhost
        }

    } // ctor

    error server_control_executor::forward_command(const std::string& _name,
                                                   const std::string& _host,
                                                   const std::string& _port_keyword,
                                                   ...
                                                   std::string& _output)
    {
        bool we_are_the_host = compare_host_names(_host, my_host_name_);
        ...
        if (we_are_the_host) {
            ...
            ret = process_host_list(_name, _wait_option, _wait_seconds, hosts, _output);
        }
        else {
            ret = forward_server_control_command(_name, _host, _port_keyword, _output);
        }
    ...
    error server_control_executor::get_resource_host_names(host_list_t& _host_names)
    {
        ...
        _reloadRodsEnv(my_env);
        rcComm_t* comm =
            rcConnect(my_env.rodsHost, my_env.rodsPort, my_env.rodsUserName, my_env.rodsZone, NO_RECONN, 0);
        if (!comm) {
    ...
    void server_control_executor::operator()()
    {
        ...
        if (shared_secret.empty() || encryption_algorithm->empty() || 0 == port || 0 == num_hash_rounds) {
        ...
        while (true) {
            try {
                ...
                while (server_state::STOPPED != s() && server_state::EXITED != s()) {
                    ...
                    ret = crypt.encrypt(shared_secret, iv, data_to_encrypt, data_to_send);
                    if (!ret.ok()) {
                        irods::log(PASS(ret));
                    }
                ...
                rodsLog(LOG_ERROR,
                        "ZMQ encountered an error in the control plane loop: [%s] Restarting control thread...",
                        _e.what());
                continue;
            }
        }
    ...
    error server_control_executor::notify_icat_and_local_servers_preop(const std::string& _cmd_name,
                                                                       ...
                                                                       std::string& _output)
    {
        ...
        const bool found_my_host = is_host_in_list(my_host_name_, _cmd_hosts);
        const bool found_icat_host = is_host_in_list(icat_host_name_, _cmd_hosts);
        ...
        if (found_icat_host || is_all_opt) {
            ret = forward_command(
                _cmd_name, icat_host_name_, CFG_SERVER_CONTROL_PLANE_PORT, _wait_option, _wait_seconds, _output);
            // takes sec, microsec
        ...
        if (!is_icat_host && (found_my_host || is_all_opt)) {
            ret = forward_command(
                _cmd_name, my_host_name_, CFG_SERVER_CONTROL_PLANE_PORT, _wait_option, _wait_seconds, _output);
        }
    ...
    error server_control_executor::notify_icat_and_local_servers_postop(const std::string& _cmd_name,
                                                                        const std::string& _cmd_option,
                                                                        const std::string& _wait_option,
                                                                        const size_t& _wait_seconds,
                                                                        const host_list_t& _cmd_hosts,
    ...
    error server_control_executor::validate_host_list(const host_list_t& _irods_hosts,
                                                      const host_list_t& _cmd_hosts,
                                                      host_list_t& _valid_hosts)
    {
        host_list_t::const_iterator itr;
        for (itr = _cmd_hosts.begin(); itr != _cmd_hosts.end(); ++itr) {
            // check host value against list from the icat
            if (!is_host_in_list(*itr, _irods_hosts) && *itr != icat_host_name_) {
    ...
    error server_control_executor::extract_command_parameters(const control_plane_command& _cmd,
                                                              std::string& _name,
                                                              std::string& _option,
                                                              ...
                                                              host_list_t& _hosts)
    {
        ...
        std::map<std::string, std::string>::const_iterator itr = _cmd.options.find(SERVER_CONTROL_OPTION_KW);
        ...
        for (itr = _cmd.options.begin(); itr != _cmd.options.end(); ++itr) {
            ...
            else if (itr->first.find(SERVER_CONTROL_HOST_KW) != std::string::npos) {
    ...
    error server_control_executor::process_host_list(const std::string& _cmd_name,
                                                     const std::string& _wait_option,
                                                     const size_t& _wait_seconds,
                                                     ...
                                                     std::string& _output)
    {
        ...
        for (itr = _hosts.begin(); itr != _hosts.end(); ++itr) {
            ...
            std::string output;
            if (compare_host_names(*itr, my_host_name_)) {
                ...
                error ret = forward_command(_cmd_name, *itr, port_prop_, _wait_option, _wait_seconds, output);
                if (!ret.ok()) {
                    _output += output;
                    log(PASS(ret));
                    fwd_err = PASS(ret);
    ...
    error server_control_executor::process_operation(const zmq::message_t& _msg, std::string& _output)
    {
        ...
        std::auto_ptr<avro::InputStream> in =
            avro::memoryInputStream(static_cast<const uint8_t*>(data_to_process.data()), data_to_process.size());
        ...
        ret = extract_command_parameters(cmd, cmd_name, cmd_option, wait_option, wait_seconds, cmd_hosts);
        if (!ret.ok()) {
            irods::log(PASS(ret));
            return PASS(ret);
        }
        ...
        ret = notify_icat_and_local_servers_preop(cmd_name, cmd_option, wait_option, wait_seconds, cmd_hosts, _output);
        if (!ret.ok()) {
            final_ret = PASS(ret);
            irods::log(final_ret);
        }
        ...
        ret = process_host_list(cmd_name, wait_option, wait_seconds, valid_hosts, _output);
        if (!ret.ok()) {
            final_ret = PASS(ret);
            irods::log(final_ret);
        }
        ...
        ret = notify_icat_and_local_servers_postop(cmd_name,
                                                   cmd_option,
                                                   wait_option,
                                                   wait_seconds,
