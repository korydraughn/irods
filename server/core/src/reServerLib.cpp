...
int getNextQueuedRuleExec(genQueryOut_t** inGenQueryOut,
                          ...
                          int jobType)
{
    sqlResult_t *ruleExecId, *ruleName, *reiFilePath, *userName, *exeAddress, *exeTime, *exeFrequency, *priority,
        *lastExecTime, *exeStatus, *estimateExeTime, *notificationAddr;
    int i, status;
    genQueryOut_t* genQueryOut;

    if (inGenQueryOut == NULL || *inGenQueryOut == NULL || queuedRuleExec == NULL ||
        queuedRuleExec->packedReiAndArgBBuf == NULL || queuedRuleExec->packedReiAndArgBBuf->buf == NULL) {
        rodsLog(LOG_ERROR, "getNextQueuedRuleExec: NULL input");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }

    genQueryOut = *inGenQueryOut;
    if ((ruleExecId = getSqlResultByInx(genQueryOut, COL_RULE_EXEC_ID)) == NULL) {
        rodsLog(LOG_NOTICE, "getNextQueuedRuleExec: getSqlResultByInx for EXEC_ID failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    if ((ruleName = getSqlResultByInx(genQueryOut, COL_RULE_EXEC_NAME)) == NULL) {
        rodsLog(LOG_NOTICE, "getNextQueuedRuleExec: getSqlResultByInx for EXEC_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    if ((reiFilePath = getSqlResultByInx(genQueryOut, COL_RULE_EXEC_REI_FILE_PATH)) == NULL) {
        rodsLog(LOG_NOTICE, "getNextQueuedRuleExec: getSqlResultByInx for REI_FILE_PATH failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    if ((userName = getSqlResultByInx(genQueryOut, COL_RULE_EXEC_USER_NAME)) == NULL) {
        rodsLog(LOG_NOTICE, "getNextQueuedRuleExec: getSqlResultByInx for USER_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    if ((exeAddress = getSqlResultByInx(genQueryOut, COL_RULE_EXEC_ADDRESS)) == NULL) {
        rodsLog(LOG_NOTICE, "getNextQueuedRuleExec: getSqlResultByInx for EXEC_ADDRESS failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    if ((exeTime = getSqlResultByInx(genQueryOut, COL_RULE_EXEC_TIME)) == NULL) {
        rodsLog(LOG_NOTICE, "getNextQueuedRuleExec: getSqlResultByInx for EXEC_TIME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    if ((exeFrequency = getSqlResultByInx(genQueryOut, COL_RULE_EXEC_FREQUENCY)) == NULL) {
        rodsLog(LOG_NOTICE, "getNextQueuedRuleExec:getResultByInx for RULE_EXEC_FREQUENCY failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    if ((priority = getSqlResultByInx(genQueryOut, COL_RULE_EXEC_PRIORITY)) == NULL) {
        rodsLog(LOG_NOTICE, "getNextQueuedRuleExec: getSqlResultByInx for PRIORITY failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    if ((lastExecTime = getSqlResultByInx(genQueryOut, COL_RULE_EXEC_LAST_EXE_TIME)) == NULL) {
        rodsLog(LOG_NOTICE, "getNextQueuedRuleExec: getSqlResultByInx for LAST_EXE_TIME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    if ((exeStatus = getSqlResultByInx(genQueryOut, COL_RULE_EXEC_STATUS)) == NULL) {
        rodsLog(LOG_NOTICE, "getNextQueuedRuleExec: getSqlResultByInx for EXEC_STATUS failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    if ((estimateExeTime = getSqlResultByInx(genQueryOut, COL_RULE_EXEC_ESTIMATED_EXE_TIME)) == NULL) {
        rodsLog(LOG_NOTICE, "getNextQueuedRuleExec: getResultByInx for ESTIMATED_EXE_TIME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    if ((notificationAddr = getSqlResultByInx(genQueryOut, COL_RULE_EXEC_NOTIFICATION_ADDR)) == NULL) {
        rodsLog(LOG_NOTICE, "getNextQueuedRuleExec:getResultByInx for NOTIFICATION_ADDR failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    ...
    for (i = startInx; i < genQueryOut->rowCnt; i++) {
        ...
        if (0 != (jobType & RE_FAILED_STATUS) && 0 != strcmp(exeStatusStr, RE_FAILED)) {
            // Only processing failed execution requests
            continue;
        }
        else if (0 == (jobType & RE_FAILED_STATUS) && 0 == strcmp(exeStatusStr, RE_FAILED)) {
            ...
            if (reExec->doFork == 1 && /* multiProc */
                ...
                rodsLog(LOG_NOTICE, "getNextQueuedRuleExec: reId %s in RUNNING state. Run again", ruleExecIdStr);
            }
        }
        status = fillExecSubmitInp(queuedRuleExec,
                                   exeStatusStr,
                                   exeTimeStr,
                                   ruleExecIdStr,
...
int modExeInfoForRepeat(rcComm_t* rcComm, char* ruleExecId, char* pastTime, char* delay, int opStatus)
{
    ...
    ***/
    ...
    if (status < 0) {
        ...
        if (status1 == 3 || (status1 != 2 && opStatus != 0))
            rodsLog(
                LOG_NOTICE, "Rule id %s set to run again at %s (frequency %s seconds)", ruleExecId, myTimeNext, delay);
    }
...
int regExeStatus(rcComm_t* rcComm, char* ruleExecId, char* exeStatus)
{
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "regExeStatus: rcRuleExecMod of id %s failed, status = %d", ruleExecId, status);
    }
...
int runQueuedRuleExec(rcComm_t* rcComm, reExec_t* reExec, genQueryOut_t** genQueryOut, time_t endTime, int jobType)
{
    int inx, status;
    ...
    while (time(NULL) <= endTime) {
        ...
        if ((inx = getNextQueuedRuleExec(genQueryOut, inx + 1, myRuleExecInp, reExec, jobType)) < 0) {
            /* no job to run */
        ...
        /* mark running */
        status = regExeStatus(rcComm, myRuleExecInp->ruleExecId, RE_RUNNING);
        if (status < 0) {
            rodsLog(LOG_ERROR,
                    "runQueuedRuleExec: regExeStatus of id %s failed,stat = %d",
                    myRuleExecInp->ruleExecId,
                    status);
            freeReThr(reExec, thrInx);
            continue;
        }
...
int postForkExecProc(reExecProc_t* reExecProc)
{
    ...
    while (!rc_comm) {
        rc_comm = rcConnect(env.rodsHost, env.rodsPort, env.rodsUserName, env.rodsZone, NO_RECONN, NULL);
        if (!rc_comm) {
            rodsLog(LOG_ERROR, "rcConnect failedd");
        }
    }
...
{
    std::vector<std::string> string_vec{RE_EXE,
                                        "-j",
...
int initReExec(reExec_t* reExec)
{
    ...
    for (i = 0; i < reExec->maxRunCnt; i++) {
        ...
        reExec->reExecProc[i].ruleExecSubmitInp.packedReiAndArgBBuf = (bytesBuf_t*) malloc(sizeof(bytesBuf_t));
        reExec->reExecProc[i].ruleExecSubmitInp.packedReiAndArgBBuf->buf = malloc(REI_BUF_LEN);
        reExec->reExecProc[i].ruleExecSubmitInp.packedReiAndArgBBuf->len = REI_BUF_LEN;
    }
    return 0;
}
...
int waitAndFreeReThr(rcComm_t* rcComm, reExec_t* reExec)
{
    ...
    if (childPid < 0) {
        if (reExec->runCnt > 0) {
            int i;
            rodsLog(LOG_NOTICE, "waitAndFreeReThr: no outstanding child. but runCnt=%d", reExec->runCnt);
            for (i = 0; i < reExec->maxRunCnt; i++) {
                if (reExec->reExecProc[i].procExecState != RE_PROC_IDLE) {
                    freeReThr(reExec, i);
                }
        ...
        if (thrInx >= 0) {
            ...
            if (status1 >= 0) {
                ...
                if ((exeFrequency = getSqlResultByInx(genQueryOut, COL_RULE_EXEC_FREQUENCY)) == NULL) {
                    rodsLog(LOG_NOTICE, "waitAndFreeReThr:getResultByInx for RULE_EXEC_FREQUENCY failed");
                }
                if ((exeStatus = getSqlResultByInx(genQueryOut, COL_RULE_EXEC_STATUS)) == NULL) {
                    rodsLog(LOG_NOTICE, "waitAndFreeReThr:getResultByInx for RULE_EXEC_STATUS failed");
                }

                if (exeFrequency == NULL || exeStatus == NULL || strlen(exeFrequency->value) == 0 ||
                    strcmp(exeStatus->value, RE_RUNNING) == 0) {
                    // r5676
                    int i;
                    int overlap = 0;
                    for (i = 0; i < reExec->maxRunCnt; i++) {
                        if (i != thrInx &&
                            strcmp(reExec->reExecProc[i].ruleExecSubmitInp.ruleExecId, ruleExecId) == 0) {
                            overlap++;
                        }
                    }
                    ...
                    if (overlap == 0) { // r5676
                        ...
                        if ((reExecProc->jobType & RE_FAILED_STATUS) == 0) {
                            ...
                            ruleExecDelInp_t ruleExecDelInp;
                            rodsLog(
                                LOG_ERROR, "waitAndFreeReThr: %s executed but still in iCat. Job deleted", ruleExecId);
                            rstrcpy(ruleExecDelInp.ruleExecId, ruleExecId, NAME_LEN);
                            status = rcRuleExecDel(rcComm, &ruleExecDelInp);
                        }
                    } // r5676
...
int matchPidInReExec(reExec_t* reExec, pid_t pid)
{
    ...
    rodsLog(LOG_ERROR, "matchPidInReExec: no match for pid %d", pid);
...
int freeReThr(reExec_t* reExec, int thrInx)
{
    ...
    packedReiAndArgBBuf = reExec->reExecProc[thrInx].ruleExecSubmitInp.packedReiAndArgBBuf;

    bzero(packedReiAndArgBBuf->buf, REI_BUF_LEN);
    bzero(&reExec->reExecProc[thrInx].ruleExecSubmitInp, sizeof(ruleExecSubmitInp_t));
    reExec->reExecProc[thrInx].ruleExecSubmitInp.packedReiAndArgBBuf = packedReiAndArgBBuf;

    return 0;
}

int runRuleExec(rcComm_t* _comm, reExecProc_t* _re_exec_proc)
{
    if (!_comm || !_re_exec_proc) {
        rodsLog(LOG_ERROR, "%s :: NULL input parameter", __FUNCTION__);
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }

    // unpack the rei to get the user information
    ruleExecInfoAndArg_t* rei_and_arg = NULL;
    int status = unpackStruct(_re_exec_proc->ruleExecSubmitInp.packedReiAndArgBBuf->buf,
                              (void**) &rei_and_arg,
                              "ReiAndArg_PI",
                              RodsPackTable,
                              NATIVE_PROT);
    ...
    rstrcpy((char*) exec_rule.rule_text_.buf, _re_exec_proc->ruleExecSubmitInp.ruleName, rule_len + 1);
...
int postProcRunRuleExec(rcComm_t* rcComm, reExecProc_t* reExecProc)
{
    ...
    if (strlen(myRuleExecInp->exeFrequency) > 0) {
        rodsLog(LOG_NOTICE, "postProcRunRuleExec: exec of freq: %s", myRuleExecInp->exeFrequency);

        savedStatus = modExeInfoForRepeat(
            rcComm, myRuleExecInp->ruleExecId, myRuleExecInp->exeTime, myRuleExecInp->exeFrequency, reExecProc->status);
    }
        ...
        if ((reExecProc->jobType & RE_FAILED_STATUS) == 0) {
            ...
            rodsLog(LOG_ERROR,
                    ...
                    myRuleExecInp->ruleExecId,
                    myRuleExecInp->ruleName);
            rstrcpy(ruleExecDelInp.ruleExecId, myRuleExecInp->ruleExecId, NAME_LEN);
            status = rcRuleExecDel(rcComm, &ruleExecDelInp);
            if (status < 0) {
                rodsLog(LOG_ERROR,
        ...
        rstrcpy(ruleExecDelInp.ruleExecId, myRuleExecInp->ruleExecId, NAME_LEN);
        rodsLog(LOG_NOTICE, "postProcRunRuleExec: exec of %s done", myRuleExecInp->ruleExecId);
        status = rcRuleExecDel(rcComm, &ruleExecDelInp);
        if (status < 0) {
            rodsLog(LOG_ERROR,
...
int matchRuleExecId(reExec_t* reExec, char* ruleExecIdStr, procExecState_t execState)
{
    int i;

    if (reExec == NULL || ruleExecIdStr == NULL || execState == RE_PROC_IDLE) {
        return 0;
    }

    for (i = 0; i < reExec->maxRunCnt; i++) {
        if (reExec->reExecProc[i].procExecState == execState &&
            strcmp(reExec->reExecProc[i].ruleExecSubmitInp.ruleExecId, ruleExecIdStr) == 0) {
            return 1;
        }
    }
    return 0;
                      ...
                      char* notificationAddr)
{
    ...
    if (st_size > ruleExecSubmitInp->packedReiAndArgBBuf->len) {
        ...
        ruleExecSubmitInp->packedReiAndArgBBuf->buf = malloc((int) st_size);
    ...
    bzero(ruleExecSubmitInp->packedReiAndArgBBuf->buf, ruleExecSubmitInp->packedReiAndArgBBuf->len);
    status = read(fd, ruleExecSubmitInp->packedReiAndArgBBuf->buf, ruleExecSubmitInp->packedReiAndArgBBuf->len);
    close(fd);
...
int reServerSingleExec(char* ruleExecId, int jobType)
{
    ...
    while (!rc_comm) {
        rc_comm = rcConnect(env.rodsHost, env.rodsPort, env.rodsUserName, env.rodsZone, NO_RECONN, NULL);
        if (!rc_comm) {
            rodsLog(LOG_ERROR, "rcConnect failed %d");
        }
    }
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "reServerSingleExec: getReInfoById error for %s, status = %d", ruleExecId, status);
        rcDisconnect(rc_comm);
        return status;
    }
    ...
    if (estimateExeTime == nullptr) {
        rodsLog(LOG_NOTICE, "reServerSingleExec: getResultByInx for ESTIMATED_EXE_TIME failed");
        rcDisconnect(rc_comm);
        return UNMATCHED_KEY_OR_INDEX;
    }
    ...
    if (notificationAddr == nullptr) {
        rodsLog(LOG_NOTICE, "reServerSingleExec:getResultByInx for NOTIFICATION_ADDR failed");
        rcDisconnect(rc_comm);
        return UNMATCHED_KEY_OR_INDEX;
    }
    ...
    status = fillExecSubmitInp(&reExecProc.ruleExecSubmitInp,
                               ...
                               exeFrequency->value,
                               priority->value,
                               estimateExeTime->value,
                               notificationAddr->value);
