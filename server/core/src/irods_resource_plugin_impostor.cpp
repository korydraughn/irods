...
irods::error impostor_file_read(irods::plugin_context& _ctx, void*, const int)
{
    return irods::impostor_resource::report_error(_ctx);
} // impostor_file_read

irods::error impostor_file_write(irods::plugin_context& _ctx, const void*, const int)
{
    return irods::impostor_resource::report_error(_ctx);
} // impostor_file_write
...
irods::error impostor_file_lseek(irods::plugin_context& _ctx, const long long, const int)
{
    return irods::impostor_resource::report_error(_ctx);
} // impostor_file_lseek
...
namespace irods
{
    impostor_resource::impostor_resource(const std::string& _inst_name, const std::string& _context)
        : resource(_inst_name, _context)
    {
        using namespace irods;
        using namespace std;
        add_operation(RESOURCE_OP_CREATE, function<error(plugin_context&)>(impostor_file_create));

        add_operation(irods::RESOURCE_OP_OPEN, function<error(plugin_context&)>(impostor_file_open));

        add_operation<void*, const int>(
            irods::RESOURCE_OP_READ,
            std::function<error(irods::plugin_context&, void*, const int)>(impostor_file_read));

        add_operation<const void*, const int>(
            irods::RESOURCE_OP_WRITE, function<error(plugin_context&, const void*, const int)>(impostor_file_write));

        add_operation(RESOURCE_OP_CLOSE, function<error(plugin_context&)>(impostor_file_close));

        add_operation(irods::RESOURCE_OP_UNLINK, function<error(plugin_context&)>(impostor_file_unlink));

        add_operation<struct stat*>(irods::RESOURCE_OP_STAT,
                                    function<error(plugin_context&, struct stat*)>(impostor_file_stat));

        add_operation(irods::RESOURCE_OP_MKDIR, function<error(plugin_context&)>(impostor_file_mkdir));

        add_operation(irods::RESOURCE_OP_OPENDIR, function<error(plugin_context&)>(impostor_file_opendir));

        add_operation<struct rodsDirent**>(
            irods::RESOURCE_OP_READDIR, function<error(plugin_context&, struct rodsDirent**)>(impostor_file_readdir));

        add_operation<const char*>(irods::RESOURCE_OP_RENAME,
                                   function<error(plugin_context&, const char*)>(impostor_file_rename));

        add_operation(irods::RESOURCE_OP_FREESPACE, function<error(plugin_context&)>(impostor_file_getfs_freespace));

        add_operation<const long long, const int>(
            irods::RESOURCE_OP_LSEEK, function<error(plugin_context&, long long, int)>(impostor_file_lseek));

        add_operation(irods::RESOURCE_OP_RMDIR, function<error(plugin_context&)>(impostor_file_rmdir));

        add_operation(irods::RESOURCE_OP_CLOSEDIR, function<error(plugin_context&)>(impostor_file_closedir));

        add_operation<const char*>(irods::RESOURCE_OP_STAGETOCACHE,
                                   function<error(plugin_context&, const char*)>(impostor_file_stage_to_cache));

        add_operation<const char*>(irods::RESOURCE_OP_SYNCTOARCH,
                                   function<error(plugin_context&, const char*)>(impostor_file_sync_to_arch));

        add_operation(irods::RESOURCE_OP_REGISTERED, function<error(plugin_context&)>(impostor_file_registered));

        add_operation(irods::RESOURCE_OP_UNREGISTERED, function<error(plugin_context&)>(impostor_file_unregistered));

        add_operation(irods::RESOURCE_OP_MODIFIED, function<error(plugin_context&)>(impostor_file_modified));

        add_operation<const std::string*>(irods::RESOURCE_OP_NOTIFY,
                                          function<error(plugin_context&, const std::string*)>(impostor_file_notify));

        add_operation(irods::RESOURCE_OP_TRUNCATE, function<error(plugin_context&)>(impostor_file_truncate));
        ...
        add_operation(irods::RESOURCE_OP_REBALANCE, function<error(plugin_context&)>(impostor_file_rebalance));
    ...
    error impostor_resource::report_error(plugin_context& _ctx)
    {
        std::string resc_name;
        ...
        return ERROR(INVALID_ACCESS_TO_IMPOSTOR_RESOURCE, msg);
    }
