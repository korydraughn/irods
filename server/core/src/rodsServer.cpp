...
namespace
{
    ...
    irods::error createAndSetRECacheSalt()
    {
        ...
        try {
            ...
            cache_salt << "pid" << static_cast<intmax_t>(getpid()) << "_" << cache_salt_random;
...
int main(int argc, char** argv)
{
    ...
    if (mkdtemp_result == NULL) {
        rodsLog(LOG_ERROR,
                "Error creating tmp directory for iRODS sockets, mkdtemp errno [%d]: [%s]",
                errno,
                strerror(errno));
        free(logDir);
    ...
    if (agent_spawning_pid == 0) {
        ...
        while (true) {
            ...
            if ((time(0) - sock_connect_start_time) > 5) {
                rodsLog(LOG_ERROR,
                        "Error connecting to agent factory socket, errno = [%d]: %s",
                        saved_errno,
...
static bool instantiate_shared_memory_for_plugin(const std::unordered_map<std::string, boost::any>& _plugin_object)
{
    try {
        const auto& mem_name =
            boost::any_cast<const std::string&>(_plugin_object.at(irods::CFG_SHARED_MEMORY_INSTANCE_KW));
...
{
    try {
        for (const auto& item : irods::get_server_property<const std::unordered_map<std::string, boost::any>&>(
            ...
            if (item.first == irods::PLUGIN_TYPE_RULE_ENGINE) {
                ...
                for (const auto& plugin :
                     boost::any_cast<const std::unordered_map<std::string, boost::any>&>(item.second)) {
                    instantiate_shared_memory_for_plugin(
                        boost::any_cast<const std::unordered_map<std::string, boost::any>&>(plugin.second));
                }
            }
        }
    }
...
int serverMain(char* logDir)
{
    ...
    try {
        ...
        while (true) {
            ...
            while ((numSock = select(svrComm.sock + 1, &sockMask, (fd_set*) NULL, (fd_set*) NULL, &time_out)) < 0) {
                if (errno == EINTR) {
...
int sendEnvironmentVarIntToSocket(const char* var, int val, int socket)
{
    ...
    if (status < 0) {
        ...
        rodsLog(LOG_DEBUG,
                "Failed to send entire message in sendEnvironmentVarIntToSocket - msg [%s] is [%d] bytes long, sent "
                "[%d] bytes",
                msg.str().c_str(),
                msg.str().length(),
                status);
...
int execAgent(int newSock, startupPack_t* startupPack)
{
    ...
    if (status < 0) {
        ...
        rodsLog(LOG_DEBUG,
                "Failed to send entire message - msg [%s] is [%d] bytes long, sent [%d] bytes",
                tmp_socket_file,
                strlen(tmp_socket_file),
                status);
    }
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR,
                "Error in recv acknowledgement from agent factory process, errno = [%d]: %s",
                errno,
                strerror(errno));
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR,
                "Unable to connect to socket in agent factory process, errno = [%d]: %s",
                errno,
                strerror(errno));
        return SYS_SOCK_CONNECT_ERR;
    }

    status = sendEnvironmentVarStrToSocket(
        SP_RE_CACHE_SALT,
    ...
    if (status < 0) {
        rodsLog(
            LOG_ERROR, "Error in recv child_pid from agent factory process, errno = [%d]: %s", errno, strerror(errno));
        return SYS_SOCK_READ_ERR;
    }
...
int queConnectedAgentProc(int childPid, agentProc_t* connReq, agentProc_t** agentProcHead)
{
    if (connReq == NULL) {
        return USER__NULL_INPUT_ERR;
    }
...
int recordServerProcess(rsComm_t* svrComm)
{
    ...
    if (cp != NULL) {
        ...
        if (fd != NULL) {
            ...
            if (err_code != 0) {
                rodsLog(
                    LOG_ERROR, "chmod failed in recordServerProcess on [%s] with error code %d", filePath, err_code);
            }
        }
    }
    return 0;
...
agentProc_t* getConnReqFromQue()
{
    ...
    irods::server_state& server_state = irods::server_state::instance();
    while (irods::server_state::STOPPED != server_state() && irods::server_state::EXITED != server_state() &&
           myConnReq == NULL) {
        boost::unique_lock<boost::mutex> read_req_lock(ReadReqCondMutex);
        if (ConnReqHead != NULL) {
        ...
        if (!ret.ok()) {
            ...
            while (bytes_to_send) {
                ...
                if (bytes_sent > 0) {
                    ...
                    rodsLog(
                        LOG_ERROR, "Socket error encountered during heartbeat; socket returned %s", strerror(errsav));
                    break;
                }
            }
            mySockClose(newSock);
            ...
            if (startupPack->clientUser[0] == '\0') {
                int status = chkAllowedUser(startupPack->clientUser, startupPack->clientRodsZone);
                if (status < 0) {
...
int procSingleConnReq(agentProc_t* connReq)
{
    ...
    if (!ret.ok()) {
        rodsLog(LOG_NOTICE,
                "readStartupPack error from %s, status = %d",
