...
namespace irods
{
    ...
    // =-=-=-=-=-=-=-
    // public - ctor
    file_object::file_object(rsComm_t* _c,
    ...
    // from dataObjInfo
    file_object::file_object(rsComm_t* _rsComm, const dataObjInfo_t* _dataObjInfo)
    {
    ...
    bool file_object::operator==(const file_object& _rhs) const
    {
        bool result = true;
        if (this->repl_requested() != _rhs.repl_requested() || this->logical_path() != _rhs.logical_path()) {
            result = false;
        }
        return result;
    }
    ...
    error file_object::resolve(const std::string& _interface, plugin_ptr& _ptr)
    {
        // =-=-=-=-=-=-=-
    ...
    error file_object::get_re_vars(rule_engine_vars_t& _kvp)
    {
