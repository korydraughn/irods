...
namespace
{
    ...
    namespace handler
    {
        auto open(UDTSOCKET _socket, request_context& _ctx, const nlohmann::json& _req) -> void
        {
            if (_ctx.file.is_open()) {
                send_error_response(_socket, error_code::ok, "Data object already open");
                return;
            }
            ...
            _ctx.data_id = _req["data_id"].get<int>();
            _ctx.resource = _req["resource"].get<std::string>();
            _ctx.resource_hierarchy = _req["resource_hierarchy"].get<std::string>();
            _ctx.replica_number = _req["replica_number"].get<int>();
            _ctx.logical_path = _req["logical_path"].get<std::string>();
            _ctx.physical_path = _req["physical_path"].get<std::string>();

            // Open file.
            _ctx.file.open(_ctx.physical_path, common::to_openmode(_req["open_mode"].get<int>())); 

            if (!_ctx.file) {
                send_error_response(_socket, error_code::data_object_not_open, "Could not open file");
                return;
            }
        ...
        auto update_catalog(request_context& _ctx) -> void;

        auto close(UDTSOCKET _socket, request_context& _ctx, const nlohmann::json& _req) -> void
        {
            if (!_ctx.file.is_open()) {
                send_error_response(_socket, error_code::data_object_not_open, "Data object not open");
                return;
            }
            ...
            if (_ctx.update_catalog) {
                update_catalog(_ctx);
            }

            send_error_response(_socket, error_code::ok);

            _ctx.close_connection = true;
        }

        auto read(UDTSOCKET _socket, request_context& _ctx, const nlohmann::json& _req) -> void
        {
            if (!_ctx.file.is_open()) {
                send_error_response(_socket, error_code::data_object_not_open, "Data object not open");
                return;
            ...
            const auto bytes_read = _ctx.file.rdbuf()->sgetn(&buf[0], buf.size());
        ...
        auto write(UDTSOCKET _socket, request_context& _ctx, const nlohmann::json& _req) -> void
        {
            if (!_ctx.file.is_open()) {
                send_error_response(_socket, error_code::data_object_not_open, "Data object not open");
                return;
            ...
            if (received > 0) {
                if ((bytes_written = _ctx.file.rdbuf()->sputn(buf.data(), received)) > 0) {
                    _ctx.update_catalog = true;
                }
            }
        ...
        auto seek(UDTSOCKET _socket, request_context& _ctx, const nlohmann::json& _req) -> void
        {
            if (!_ctx.file.is_open()) {
                send_error_response(_socket, error_code::data_object_not_open, "Data object not open");
                return;
            ...
            _ctx.file.seekp(offset, seek_dir);

            common::send_message(_socket, {
                {"error_code", static_cast<int>(error_code::ok)},
                {"position", static_cast<std::fstream::off_type>(_ctx.file.tellp())}
            });
        }
        ...
        auto update_catalog(request_context& _ctx) -> void
        {
            ...
            info.dataId = _ctx.data_id;
            std::strncpy(info.objPath, _ctx.logical_path.c_str(), _ctx.logical_path.size());
            std::strncpy(info.rescHier, _ctx.resource_hierarchy.c_str(), _ctx.resource_hierarchy.size());
            info.replNum = _ctx.replica_number;

            namespace fs = boost::filesystem;

            const auto file_size = std::to_string(fs::file_size(_ctx.physical_path));
            ...
            if (const auto ec = getRodsEnv(&env); ec != 0) {
                log::server::error({{"log_message", "Could not get iRODS environment for data object size update."},
                                    {"logical_path", _ctx.logical_path},
    ...
    using op_handler = std::function<void(UDTSOCKET, request_context&, const nlohmann::json&)>;
