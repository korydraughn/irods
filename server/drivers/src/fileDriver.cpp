...
irods::error fileRead(rsComm_t* _comm, irods::first_class_object_ptr _object, void* _buf, const int _len)
{
    // =-=-=-=-=-=-=-
    // retrieve the resource name given the object
    irods::plugin_ptr ptr;
...
irods::error fileWrite(rsComm_t* _comm, irods::first_class_object_ptr _object, const void* _buf, const int _len)
{
    // =-=-=-=-=-=-=-
    // retrieve the resource name given the object
    irods::plugin_ptr ptr;
...
irods::error fileLseek(rsComm_t* _comm,
...
irods::error fileStageToCache(rsComm_t* _comm,
                              irods::first_class_object_ptr _object,
...
irods::error fileNotify(rsComm_t* _comm, irods::first_class_object_ptr _object, const std::string& _operation)
{
    ...
    if (!resc_hier.empty()) {
        ...
        if (!ret.ok()) {
            ...
            // make the call to the "open" interface
            resc = boost::dynamic_pointer_cast<irods::resource>(ptr);
            ret = resc->call<const std::string*>(_comm, irods::RESOURCE_OP_NOTIFY, _object, &_operation);
            if (!ret.ok()) {
                std::stringstream msg;
