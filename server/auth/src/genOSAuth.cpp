...
int main(int argc, char* argv[])
{
    ...
    if (strcmp(username, username2)) {
        printf("Username %s does not match OS user %s", username, username2);
        return 1;
    }
    ...
    if (sizeof(challenge_len) != read(0, (void*) &challenge_len, sizeof(challenge_len))) {
        printf("Couldn't read challenge length from stdin: %s", strerror(errno));
        return 1;
    }
    ...
    if (CHALLENGE_LEN != read(0, &challenge[0], CHALLENGE_LEN)) {
        printf("Couldn't read challenge from stdin: %s", strerror(errno));
        return 1;
    }
    ...
    if (write(1, authenticator, 16) == -1) {
        int errsv = errno;
        printf("Error %s writing the authenticator to stdout.", strerror(errsv));
        return 1;
    }
