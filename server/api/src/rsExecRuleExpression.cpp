...
int rsExecRuleExpression(rsComm_t* _comm, exec_rule_expression_t* _exec_rule)
{
    ...
    int status =
        unpackStruct(_exec_rule->packed_rei_.buf, (void**) &rei_and_arg, "ReiAndArg_PI", RodsPackTable, NATIVE_PROT);
    if (status < 0) {
    ...
    irods::error err = re_ctx_mgr.exec_rule_expression(instance_name_string, my_rule_text, _exec_rule->params_);
    if (!err.ok()) {
