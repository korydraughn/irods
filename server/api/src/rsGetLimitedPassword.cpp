...
int rsGetLimitedPassword(rsComm_t* rsComm,
                         ...
                         getLimitedPasswordOut_t** getLimitedPasswordOut)
{
    ...
    if (rodsServerHost->localFlag == LOCAL_HOST) {
        ...
        if (irods::CFG_SERVICE_ROLE_PROVIDER == svc_role) {
            status = _rsGetLimitedPassword(rsComm, getLimitedPasswordInp, getLimitedPasswordOut);
        }
        ...
        status = rcGetLimitedPassword(rodsServerHost->conn, getLimitedPasswordInp, getLimitedPasswordOut);
    }

    if (status < 0) {
        rodsLog(LOG_NOTICE, "rsGetLimitedPassword: rcGetLimitedPassword failed, status = %d", status);
    }
    return status;
}

int _rsGetLimitedPassword(rsComm_t* rsComm,
                          getLimitedPasswordInp_t* getLimitedPasswordInp,
                          getLimitedPasswordOut_t** getLimitedPasswordOut)
{
    int status;
    ...
    status = chlMakeLimitedPw(rsComm, getLimitedPasswordInp->ttl, myGetLimitedPasswordOut->stringToHashWith);
    if (status < 0) {
