...
int rsDataObjRepl(rsComm_t* rsComm, dataObjInp_t* dataObjInp, transferStat_t** transStat)
{
    ...
    status = resolvePathInSpecColl(rsComm, dataObjInp->objPath, READ_COLL_PERM, 0, &dataObjInfo);
    ...
    *transStat = (transferStat_t*) malloc(sizeof(transferStat_t));
    ...
    if (lockType != NULL) {
        lockFd = irods::server_api_call(DATA_OBJ_LOCK_AN, rsComm, dataObjInp, NULL, (void**) NULL, NULL);
        if (lockFd >= 0) {
            ...
            rodsLogError(
                LOG_ERROR, lockFd, "rsDataObjRepl: lock error for %s. lockType = %s", dataObjInp->objPath, lockType);
            return lockFd;
        }
    }
    // =-=-=-=-=-=-=-
    ...
    if (lockFd > 0) {
        ...
        irods::server_api_call(DATA_OBJ_UNLOCK_AN, rsComm, dataObjInp, NULL, (void**) NULL, NULL);
    }
...
int _rsDataObjRepl(rsComm_t* rsComm, dataObjInp_t* dataObjInp, transferStat_t* transStat, dataObjInfo_t* outDataObjInfo)
{
    ...
    if (multiCopyFlag) {
        status = getDataObjInfo(rsComm, dataObjInp, &dataObjInfoHead, accessPerm, 0);
    }
    else {
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "%s: getDataObjInfo for [%s] failed", __FUNCTION__, dataObjInp->objPath);
        return status;
    }
    ...
    if (multiCopyFlag == 0) { // JMC - backport 4594
        ...
        status = resolveSingleReplCopy(
            &dataObjInfoHead, &oldDataObjInfoHead, root_resc_name, &destDataObjInfo, &dataObjInp->condInput);
    ...
    if (destDataObjInfo != NULL) {
        status = _rsDataObjReplUpdate(rsComm, dataObjInp, dataObjInfoHead, destDataObjInfo, transStat);
        if (status >= 0) {
    ...
    if (!root_resc_name.empty()) {
        ...
        status = _rsDataObjReplNewCopy(
            rsComm, dataObjInp, dataObjInfoHead, root_resc_name.c_str(), transStat, outDataObjInfo);
        if (status < 0) {
            savedStatus = status;
        }
    }
 ...
 *          update it.
 */
int _rsDataObjReplS(rsComm_t* rsComm,
                    dataObjInp_t* dataObjInp,
                    dataObjInfo_t* srcDataObjInfo,
                    const char* _root_resc_name,
                    ...
                    int updateFlag)
{
    ...
    l1descInx = dataObjOpenForRepl(rsComm, dataObjInp, srcDataObjInfo, _root_resc_name, destDataObjInfo, updateFlag);
    if (l1descInx < 0) {
        return l1descInx;
    }
    ...
    *srcDataObjInfo = *inpSrcDataObjInfo;
    ...
    if (updateFlag > 0) {
        ...
        if (inpDestDataObjInfo == NULL || inpDestDataObjInfo->dataId <= 0) {
            rodsLog(
                LOG_ERROR, "dataObjOpenForRepl: dataId of %s copy to be updated not defined", srcDataObjInfo->objPath);
            freeDataObjInfo(myDestDataObjInfo);
            freeDataObjInfo(srcDataObjInfo);
    ...
    l1DataObjInp->numThreads = dataObjInp->numThreads = getNumThreads(rsComm,
                                                                      l1DataObjInp->dataSize,
                                                                      l1DataObjInp->numThreads,
                                                                      &dataObjInp->condInput,
                                                                      dst_hier_str,
                                                                      srcDataObjInfo->rescHier,
                                                                      dataObjInp->oprType);
    ...
    if ((l1DataObjInp->numThreads > 0 || l1DataObjInp->dataSize > single_buff_sz) &&
    ...
    irods::file_object_ptr file_obj(new irods::file_object(rsComm, myDestDataObjInfo));
...
int dataObjCopy(rsComm_t* rsComm, int l1descInx)
{
    ...
    if (srcRemoteFlag == REMOTE_ZONE_HOST && destRemoteFlag == REMOTE_ZONE_HOST) {
    ...
    else if (srcRemoteFlag != REMOTE_ZONE_HOST && destRemoteFlag != REMOTE_ZONE_HOST &&
    ...
    else if ((srcRemoteFlag == LOCAL_HOST && destRemoteFlag != LOCAL_HOST) || destRemoteFlag == REMOTE_ZONE_HOST) {
        initDataOprInp(&dataCopyInp.dataOprInp, l1descInx, COPY_TO_REM_OPR);
        ...
        if (L1desc[l1descInx].dataObjInp->numThreads > 0) {
            ...
            if (status < 0 || NULL == portalOprOut) { // JMC cppcheck - nullptr
                rodsLog(
                    LOG_NOTICE, "dataObjCopy: preProcParaPut error for %s", L1desc[srcL1descInx].dataObjInfo->objPath);
                free(portalOprOut);
                return status;
            }
    ...
    else if ((srcRemoteFlag != LOCAL_HOST && destRemoteFlag == LOCAL_HOST) || srcRemoteFlag == REMOTE_ZONE_HOST) {
        ...
        if (L1desc[l1descInx].dataObjInp->numThreads > 0) {
            ...
            if (status < 0 || NULL == portalOprOut) { // JMC cppcheck - null ptr
                rodsLog(
                    LOG_NOTICE, "dataObjCopy: preProcParaGet error for %s", L1desc[srcL1descInx].dataObjInfo->objPath);
                free(portalOprOut);
                return status;
            }
            dataCopyInp.portalOprOut = *portalOprOut;
    ...
    if (getValByKey(&L1desc[l1descInx].dataObjInp->condInput, NO_CHK_COPY_LEN_KW) != NULL) {
        /* don't check the transfer len */
        addKeyVal(&dataOprInp->condInput, NO_CHK_COPY_LEN_KW, "");
        if (L1desc[l1descInx].dataObjInp->numThreads > 1) {
            L1desc[l1descInx].dataObjInp->numThreads = L1desc[srcL1descInx].dataObjInp->numThreads =
                dataCopyInp.portalOprOut.numThreads = 1;
        }
    }
    int status = rsDataCopy(rsComm, &dataCopyInp);

    if (status >= 0 && portalOprOut != NULL && L1desc[l1descInx].dataObjInp != NULL) {
...
int l3DataCopySingleBuf(rsComm_t* rsComm, int l1descInx)
{
    ...
    else if (getValByKey(&L1desc[l1descInx].dataObjInp->condInput, NO_CHK_COPY_LEN_KW) != NULL) {
        /* need to update size */
        L1desc[l1descInx].dataSize = L1desc[l1descInx].dataObjInp->dataSize = bytesRead;
    }
    ...
    if (bytesWritten != bytesRead) {
        if (bytesWritten >= 0) {
            rodsLog(
                LOG_NOTICE, "l3DataCopySingleBuf: l3FilePut error, towrite %d, written %d", bytesRead, bytesWritten);
            return SYS_COPY_LEN_ERR;
        }
        else {
            return bytesWritten;
...
int l3DataStageSync(rsComm_t* rsComm, int l1descInx)
{
    ...
    srcL1descInx = L1desc[l1descInx].srcL1descInx;
    if (L1desc[srcL1descInx].dataSize < 0 && L1desc[srcL1descInx].dataSize != UNKNOWN_FILE_SZ) {
        rodsLog(LOG_ERROR,
    ...
    else if (L1desc[srcL1descInx].dataSize >= 0 || L1desc[srcL1descInx].dataSize == UNKNOWN_FILE_SZ) {
    ...
    if (status < 0) {
        ...
        L1desc[l1descInx].bytesWritten = L1desc[l1descInx].dataSize = L1desc[srcL1descInx].dataSize;
    }
...
int l3FileSync(rsComm_t* rsComm, int srcL1descInx, int destL1descInx)
{
    ...
    irods::error err =
        irods::get_resource_property<int>(destDataObjInfo->rescId, irods::RESOURCE_CREATE_PATH, dst_create_path);
    if (!err.ok()) {
        irods::log(PASS(err));
    }
    ...
    if (status >= 0 && CREATE_PATH == dst_create_path && NULL != sync_out) {
...
int _l3FileStage(rsComm_t* rsComm,
                 dataObjInfo_t* srcDataObjInfo, // JMC - backport 4527
                 dataObjInfo_t* destDataObjInfo,
                 int mode)
{
    // int rescTypeInx, cacheRescTypeInx;
...
int rsReplAndRequeDataObjInfo(rsComm_t* rsComm, dataObjInfo_t** srcDataObjInfoHead, char* destRescName, char* flagStr)
{
    ...
    status = _rsDataObjRepl(rsComm, &dataObjInp, &transStat, myDataObjInfo);
...
int stageBundledData(rsComm_t* rsComm, dataObjInfo_t** subfileObjInfoHead)
{
    ...
    status = unbunAndStageBunfileObj(rsComm, dataObjInfoHead->filePath, &cacheRescName);
...
int unbunAndStageBunfileObj(rsComm_t* rsComm, char* bunfileObjPath, char** outCacheRescName)
{
    ...
    status = _unbunAndStageBunfileObj(rsComm, &bunfileObjInfoHead, &dataObjInp.condInput, outCacheRescName, 0);
                             ...
                             int rmBunCopyFlag)
{
    ...
    addKeyVal(&dataObjInp.condInput,
              BUN_FILE_PATH_KW, // JMC - backport 4768
