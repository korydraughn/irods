...
int rsSubStructFileRmdir(rsComm_t* rsComm, subFile_t* subFile)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsSubStructFileRmdir: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteSubStructFileRmdir(rsComm_t* rsComm, subFile_t* subFile, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileRmdir: Invalid rodsServerHost");
...
int _rsSubStructFileRmdir(rsComm_t* _comm, subFile_t* _sub_file)
{
    // =-=-=-=-=-=-=-
