...
int createBunDirForBulkPut(rsComm_t* rsComm,
                           dataObjInp_t* dataObjInp,
                           const char*,
                           specColl_t* specColl,
                           char* phyBunDir);
int initDataObjInpFromBulkOpr(dataObjInp_t* dataObjInp, bulkOprInp_t* bulkOprInp);
int bulkRegUnbunSubfiles(rsComm_t* rsComm,
                         const char*,
...
int _bulkRegUnbunSubfiles(rsComm_t* rsComm,
                          const char*,
                          const std::string& rescHier,
                          char* collection,
                          char* phyBunDir,
                          ...
                          genQueryOut_t* attriArray);
int bulkProcAndRegSubfile(rsComm_t* rsComm,
                          const char*,
                          const std::string& rescHier,
                          char* subObjPath,
                          char* subfilePath,
                          ...
                          renamedPhyFiles_t* renamedPhyFiles,
                          genQueryOut_t* attriArray);
int bulkRegSubfile(rsComm_t* rsComm,
                   const char*,
                   const std::string& rescHier,
                   char* subObjPath,
                   char* subfilePath,
...
int rsBulkDataObjPut(rsComm_t* rsComm, bulkOprInp_t* bulkOprInp, bytesBuf_t* bulkOprInpBBuf)
{
    ...
    resolveLinkedPath(rsComm, bulkOprInp->objPath, &specCollCache, &bulkOprInp->condInput);
    ...
    if (remoteFlag < 0) {
        ...
        if (getValByKey(&dataObjInp.condInput, RESC_HIER_STR_KW) == NULL) {
            std::string hier;
            irods::error ret =
                irods::resource_redirect(irods::CREATE_OPERATION, rsComm, &dataObjInp, hier, host, local);
            if (!ret.ok()) {
        ...
        status = rcBulkDataObjPut(rodsServerHost->conn, bulkOprInp, bulkOprInpBBuf);
    }
    return status;
}
...
int _rsBulkDataObjPut(rsComm_t* rsComm, bulkOprInp_t* bulkOprInp, bytesBuf_t* bulkOprInpBBuf)
{
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "_rsBulkDataObjPut: unbunBulkBuf for dir %s. stat = %d", phyBunDir, status);
        return status;
    }
                           ...
                           char* phyBunDir)
{
    ...
    if (specColl != NULL) {
        status = getMountedSubPhyPath(specColl->collection, specColl->phyPath, dataObjInp->objPath, phyBunDir);
        if (status < 0) {
            return status;
        }
    ...
    if (status < 0) {
        rodsLog(
            LOG_ERROR, "createBunDirForBulkPut: getFilePathName err for %s. status = %d", dataObjInp->objPath, status);
        return status;
    }
    do {
        snprintf(phyBunDir,
                         ...
                         genQueryOut_t* attriArray)
{
    ...
    status = _bulkRegUnbunSubfiles(
        rsComm, _resc_name, rescHier, collection, phyBunDir, flags, &bulkDataObjRegInp, &renamedPhyFiles, attriArray);

    if (bulkDataObjRegInp.rowCnt > 0) {
        int status1;
        genQueryOut_t* bulkDataObjRegOut = NULL;
        status1 = rsBulkDataObjReg(rsComm, &bulkDataObjRegInp, &bulkDataObjRegOut);
        if (status1 < 0) {
            status = status1;
            rodsLog(LOG_ERROR, "%s: rsBulkDataObjReg error for %s. stat = %d", __FUNCTION__, collection, status1);
                          ...
                          genQueryOut_t* attriArray)
{
    ...
    if (!exists(srcDirPath) || !is_directory(srcDirPath)) {
        rodsLog(LOG_ERROR, "%s: opendir error for %s, errno = %d", __FUNCTION__, phyBunDir, errno);
    ...
    for (directory_iterator itr(srcDirPath); itr != end_itr; ++itr) {
        ...
        if (!exists(p)) {
            rodsLog(LOG_ERROR, "%s: stat error for %s, errno = %d", __FUNCTION__, subfilePath, errno);
        ...
        snprintf(subObjPath, MAX_NAME_LEN, "%s/%s", collection, childPath.c_str());
        if (is_directory(p)) {
            status = rsMkCollR(rsComm, "/", subObjPath);
            if (status < 0) {
                rodsLog(LOG_ERROR, "%s: rsMkCollR of %s error. status = %d", __FUNCTION__, subObjPath, status);
                savedStatus = status;
                continue;
            }
            status = _bulkRegUnbunSubfiles(rsComm,
                                           _resc_name,
                                           rescHier,
                                           subObjPath,
                                           subfilePath,
            ...
            if (status < 0) {
                rodsLog(
                    LOG_ERROR, "%s: _bulkRegUnbunSubfiles of %s error. status=%d", __FUNCTION__, subObjPath, status);
                savedStatus = status;
                continue;
            }
        }
            ...
            st_size = file_size(p);
            status = bulkProcAndRegSubfile(rsComm,
                                           _resc_name,
                                           rescHier,
                                           subObjPath,
                                           subfilePath,
            ...
            unlink(subfilePath);
            if (status < 0) {
                rodsLog(LOG_ERROR, "%s:bulkProcAndRegSubfile of %s err.stat=%d", __FUNCTION__, subObjPath, status);
                savedStatus = status;
                continue;
            }
        }
                          ...
                          genQueryOut_t* attriArray)
{
    ...
    status = getFilePathName(rsComm, &dataObjInfo, &dataObjInp);
    if (status < 0) {
        rodsLog(LOG_ERROR, "regSubFile: getFilePathName err for %s. status = %d", dataObjInp.objPath, status);
        return status;
    }
    ...
    if (exists(p)) {
        ...
        if (chkOrphanFile(rsComm, dataObjInfo.filePath, _resc_name, &dataObjInfo) <= 0) {
            /* not an orphan file */
        ...
        status = renameFilePathToNewDir(rsComm, ORPHAN_DIR, &fileRenameInp, 1, new_fn);
        if (status < 0) {
            rodsLog(LOG_ERROR,
        ...
        if (modFlag > 0) {
            status =
                addRenamedPhyFile(subObjPath, fileRenameInp.oldFileName, fileRenameInp.newFileName, renamedPhyFiles);
            if (status < 0) {
                return status;
            }
        }
        ...
        status = mkDirForFilePath(rsComm, 0, dataObjInfo.filePath, dataObjInfo.rescHier, getDefDirMode());
        if (status < 0) {
    ...
    if (attriArray != NULL) {
        ...
        status = getAttriInAttriArray(subObjPath, attriArray, &myDataMode, &myChksum);
        if (status < 0) {
            rodsLog(LOG_NOTICE, "bulkProcAndRegSubfile: matchObjPath error for %s, stat = %d", subObjPath, status);
        }
        else {
            if ((flags & VERIFY_CHKSUM_FLAG) != 0 && myChksum != NULL) {
                ...
                if (status < 0) {
                    rodsLog(LOG_ERROR, "bulkProcAndRegSubfile: chksumLocFile error for %s ", dataObjInfo.filePath);
                    return status;
                }
...
int fillBulkDataObjRegInp(const char* rescName,
                          const char* rescHier,
                          char* objPath,
                          char* filePath,
                          char* dataType,
                          ...
                          genQueryOut_t* bulkDataObjRegInp)
{
    ...
    if (bulkDataObjRegInp == NULL || rescName == NULL || objPath == NULL || filePath == NULL) {
        return USER__NULL_INPUT_ERR;
    }
    ...
    rstrcpy(&bulkDataObjRegInp->sqlResult[0].value[MAX_NAME_LEN * rowCnt], objPath, MAX_NAME_LEN);
    rstrcpy(&bulkDataObjRegInp->sqlResult[1].value[NAME_LEN * rowCnt], dataType, NAME_LEN);
    snprintf(&bulkDataObjRegInp->sqlResult[2].value[NAME_LEN * rowCnt], NAME_LEN, "%lld", dataSize);
    rstrcpy(&bulkDataObjRegInp->sqlResult[3].value[NAME_LEN * rowCnt], rescName, NAME_LEN);
    rstrcpy(&bulkDataObjRegInp->sqlResult[4].value[MAX_NAME_LEN * rowCnt], filePath, MAX_NAME_LEN);
    snprintf(&bulkDataObjRegInp->sqlResult[5].value[NAME_LEN * rowCnt], NAME_LEN, "%d", dataMode);
    if (modFlag == 1) {
        rstrcpy(&bulkDataObjRegInp->sqlResult[6].value[NAME_LEN * rowCnt], MODIFY_OPR, NAME_LEN);
    }
    else {
        rstrcpy(&bulkDataObjRegInp->sqlResult[6].value[NAME_LEN * rowCnt], REGISTER_OPR, NAME_LEN);
    }
    snprintf(&bulkDataObjRegInp->sqlResult[7].value[NAME_LEN * rowCnt], NAME_LEN, "%d", replNum);
    if (chksum != NULL && strlen(chksum) > 0) {
        rstrcpy(&bulkDataObjRegInp->sqlResult[8].value[NAME_LEN * rowCnt], chksum, NAME_LEN);
    }
    else {
    ...
    try {
        ...
        snprintf(
            &bulkDataObjRegInp->sqlResult[9].value[MAX_NAME_LEN * rowCnt], MAX_NAME_LEN, "%s", resc_id_str.c_str());
    }
    catch (boost::bad_lexical_cast&) {
        rodsLog(LOG_ERROR, "failed to cast [%Ld] to a string", resc_id);
                   ...
                   renamedPhyFiles_t* renamedPhyFiles)
{
    ...
    status = fillBulkDataObjRegInp(rescName,
                                   rescHier.c_str(),
                                   subObjPath,
                                   subfilePath,
                                   "generic",
                                   ...
                                   bulkDataObjRegInp);
    if (status < 0) {
        rodsLog(LOG_ERROR, "bulkRegSubfile: fillBulkDataObjRegInp error for %s. status = %d", subfilePath, status);
        return status;
    }

    if (bulkDataObjRegInp->rowCnt >= MAX_NUM_BULK_OPR_FILES) {
        genQueryOut_t* bulkDataObjRegOut = NULL;
        status = rsBulkDataObjReg(rsComm, bulkDataObjRegInp, &bulkDataObjRegOut);
        if (status < 0) {
            rodsLog(LOG_ERROR, "bulkRegSubfile: rsBulkDataObjReg error for %s. status = %d", subfilePath, status);
            cleanupBulkRegFiles(rsComm, bulkDataObjRegInp);
        }
        postProcRenamedPhyFiles(renamedPhyFiles, status);
        postProcBulkPut(rsComm, bulkDataObjRegInp, bulkDataObjRegOut);
...
int addRenamedPhyFile(char* subObjPath, char* oldFileName, char* newFileName, renamedPhyFiles_t* renamedPhyFiles)
{
    if (subObjPath == NULL || oldFileName == NULL || newFileName == NULL || renamedPhyFiles == NULL) {
        return USER__NULL_INPUT_ERR;
    }

    if (renamedPhyFiles->count >= MAX_NUM_BULK_OPR_FILES) {
        rodsLog(LOG_ERROR, "addRenamedPhyFile: count >= %d for %s", MAX_NUM_BULK_OPR_FILES, subObjPath);
        return SYS_RENAME_STRUCT_COUNT_EXCEEDED;
    }
    rstrcpy(&renamedPhyFiles->objPath[renamedPhyFiles->count][0], subObjPath, MAX_NAME_LEN);
    rstrcpy(&renamedPhyFiles->origFilePath[renamedPhyFiles->count][0], oldFileName, MAX_NAME_LEN);
    rstrcpy(&renamedPhyFiles->newFilePath[renamedPhyFiles->count][0], newFileName, MAX_NAME_LEN);
...
int postProcRenamedPhyFiles(renamedPhyFiles_t* renamedPhyFiles, int regStatus)
{
    ...
    if (regStatus >= 0) {
        ...
        for (i = 0; i < renamedPhyFiles->count; i++) {
            status = rename(&renamedPhyFiles->newFilePath[i][0], &renamedPhyFiles->origFilePath[i][0]);
            if (status < 0) {
...
int cleanupBulkRegFiles(rsComm_t* rsComm, genQueryOut_t* bulkDataObjRegInp)
{
    ...
    if ((filePath = getSqlResultByInx(bulkDataObjRegInp, COL_D_DATA_PATH)) == NULL) {
        rodsLog(LOG_NOTICE, "cleanupBulkRegFiles: getSqlResultByInx for COL_D_DATA_PATH failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    if ((rescName = getSqlResultByInx(bulkDataObjRegInp, COL_D_RESC_NAME)) == NULL) {
        rodsLog(LOG_NOTICE, "rsBulkDataObjReg: getSqlResultByInx for COL_D_RESC_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
...
{
    dataObjInfo_t dataObjInfo;
    sqlResult_t *objPath, *dataType, *dataSize, *rescName, *filePath, *dataMode, *oprType, *replNum, *chksum;
    char *tmpObjPath, *tmpDataType, *tmpDataSize, *tmpFilePath, *tmpDataMode, *tmpReplNum, *tmpChksum;
    sqlResult_t* objId;
    int status, i;
    dataObjInp_t dataObjInp;
    ruleExecInfo_t rei;
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "postProcBulkPut: acBulkPutPostProcPolicy error status = %d", status);
        return status;
    }
    ...
    if ((objPath = getSqlResultByInx(bulkDataObjRegInp, COL_DATA_NAME)) == NULL) {
        rodsLog(LOG_ERROR, "postProcBulkPut: getSqlResultByInx for COL_DATA_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataType = getSqlResultByInx(bulkDataObjRegInp, COL_DATA_TYPE_NAME)) == NULL) {
        rodsLog(LOG_ERROR, "postProcBulkPut: getSqlResultByInx for COL_DATA_TYPE_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    if ((dataSize = getSqlResultByInx(bulkDataObjRegInp, COL_DATA_SIZE)) == NULL) {
        rodsLog(LOG_ERROR, "postProcBulkPut: getSqlResultByInx for COL_DATA_SIZE failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    if ((rescName = getSqlResultByInx(bulkDataObjRegInp, COL_D_RESC_NAME)) == NULL) {
        rodsLog(LOG_ERROR, "postProcBulkPut: getSqlResultByInx for COL_D_RESC_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((filePath = getSqlResultByInx(bulkDataObjRegInp, COL_D_DATA_PATH)) == NULL) {
        rodsLog(LOG_ERROR, "postProcBulkPut: getSqlResultByInx for COL_D_DATA_PATH failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataMode = getSqlResultByInx(bulkDataObjRegInp, COL_DATA_MODE)) == NULL) {
        rodsLog(LOG_ERROR, "postProcBulkPut: getSqlResultByInx for COL_DATA_MODE failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((oprType = getSqlResultByInx(bulkDataObjRegInp, OPR_TYPE_INX)) == NULL) {
        rodsLog(LOG_ERROR, "postProcBulkPut: getSqlResultByInx for OPR_TYPE_INX failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((replNum = getSqlResultByInx(bulkDataObjRegInp, COL_DATA_REPL_NUM)) == NULL) {
        rodsLog(LOG_ERROR, "postProcBulkPut: getSqlResultByInx for COL_DATA_REPL_NUM failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    chksum = getSqlResultByInx(bulkDataObjRegInp, COL_D_DATA_CHECKSUM);

    /* the output */
    if ((objId = getSqlResultByInx(bulkDataObjRegOut, COL_D_DATA_ID)) == NULL) {
        rodsLog(LOG_ERROR, "postProcBulkPut: getSqlResultByInx for COL_D_DATA_ID failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
