...
int rsStructFileExtract(rsComm_t* rsComm, structFileOprInp_t* structFileOprInp)
{
    ...
    if (local == LOCAL_HOST) {
        ...
        if (local < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsStructFileExtract: resolveHost returned unrecognized value %d", local);
...
int remoteStructFileExtract(rsComm_t* rsComm, structFileOprInp_t* structFileOprInp, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteStructFileExtract: Invalid rodsServerHost");
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE,
                ...
                structFileOprInp->specColl->collection,
                status);
    }
...
int _rsStructFileExtract(rsComm_t* _comm, structFileOprInp_t* _struct_inp)
{
    // =-=-=-=-=-=-=-
    // pointer check
    if (_comm == NULL || _struct_inp == NULL || _struct_inp->specColl == NULL) {
