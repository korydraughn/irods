                           ...
                           rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "rsFileSyncToArchByHost: Input NULL rodsServerHost");
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsFileSyncToArchByHost: resolveHost returned value %d", remoteFlag);
                         ...
                         rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileSyncToArch: Invalid rodsServerHost");
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "remoteFileSyncToArch: rcFileSyncToArch failed for %s", fileSyncToArchInp->filename);
    }
...
int _rsFileSyncToArch(rsComm_t* _comm, fileStageSyncInp_t* _sync_inp, fileSyncOut_t** _sync_out)
{
    ...
    if (!sync_err.ok()) {
        if (getErrno(sync_err.code()) == ENOENT) {
            ...
            int status = mkDirForFilePath(_comm, 0, _sync_inp->filename, _sync_inp->rescHier, getDefDirMode());
            if (status < 0) {
            ...
            irods::collection_object_ptr coll_obj(
                new irods::collection_object(_sync_inp->filename, _sync_inp->rescHier, 0, 0));
