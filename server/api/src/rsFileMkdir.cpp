...
int rsFileMkdir(rsComm_t* rsComm, fileMkdirInp_t* fileMkdirInp)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsFileMkdir: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteFileMkdir(rsComm_t* rsComm, fileMkdirInp_t* fileMkdirInp, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileMkdir: Invalid rodsServerHost");
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "remoteFileOpen: rcFileMkdir failed for %s", fileMkdirInp->dirName);
    }
...
int _rsFileMkdir(rsComm_t* _comm, fileMkdirInp_t* _mkdir_inp)
{
    ...
    irods::collection_object_ptr coll_obj(
        new irods::collection_object(_mkdir_inp->dirName, _mkdir_inp->rescHier, _mkdir_inp->mode, 0));
