...
int rsSetRoundRobinContext(rsComm_t* _comm, setRoundRobinContextInp_t* _inp)
{
    ...
    if (!_comm || !_inp) {
        rodsLog(LOG_ERROR, "rsSetRoundRobinContex - invalid input param");
    ...
    irods::error ret = resc_mgr.resolve(_inp->resc_name_, resc);
    if (!ret.ok()) {
        irods::log(PASS(ret));
        return SYS_INVALID_INPUT_PARAM;
    }
    ...
    ret = resc->get_property<std::string>(irods::RESOURCE_TYPE, resc_type);
    if (resc_type != "roundrobin") {
        rodsLog(LOG_ERROR, "rsSetRoundRobinContex - resource is not of type roundrobin [%s]", resc_type.c_str());
        return SYS_INVALID_INPUT_PARAM;
    }
    ...
    if (!resc->has_child(_inp->context_)) {
        rodsLog(LOG_ERROR, "rsSetRoundRobinContex - invalid next child [%s]", _inp->context_);
        return SYS_INVALID_INPUT_PARAM;
    }
    ...
    int status = getAndConnRcatHost(_comm, MASTER_RCAT, (const char*) NULL, &server_host);
    if (status < 0) {
        rodsLog(LOG_DEBUG, "rsSetRoundRobinContex - getAndConnRcatHost failed.");
        return status;
    }
    ...
    if (server_host->localFlag == LOCAL_HOST) {
        ...
        status = chlModResc(_comm, _inp->resc_name_, "context", _inp->context_);
        ...
        status = procApiRequest(server_host->conn, SET_RR_CTX_AN, _inp, NULL, (void**) NULL, NULL);
        if (status < 0) {
            replErrorStack(server_host->conn->rError, &_comm->rError);
        }
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "rsSetRoundRobinContex: error %d", status);
    }
