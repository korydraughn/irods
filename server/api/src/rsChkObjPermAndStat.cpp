...
int rsChkObjPermAndStat(rsComm_t* rsComm, chkObjPermAndStat_t* chkObjPermAndStatInp)
{
    ...
    status = getAndConnRcatHost(rsComm, SLAVE_RCAT, (const char*) chkObjPermAndStatInp->objPath, &rodsServerHost);
    ...
    if (rodsServerHost->localFlag == LOCAL_HOST) {
        ...
        status = rcChkObjPermAndStat(rodsServerHost->conn, chkObjPermAndStatInp);
    }
...
int chkCollForBundleOpr(rsComm_t* rsComm, chkObjPermAndStat_t* chkObjPermAndStatInp)
{
    ...
    if (irods::CFG_SERVICE_ROLE_PROVIDER == svc_role) {
        ...
        if ((resource = getValByKey(&chkObjPermAndStatInp->condInput, RESC_NAME_KW)) == NULL) {
            rodsLog(LOG_ERROR, "chkCollForBundleOpr: RESC_NAME_KW not specified for %s", chkObjPermAndStatInp->objPath);
            return SYS_INVALID_RESC_INPUT;
        }

        if ((resc_hier = getValByKey(&chkObjPermAndStatInp->condInput, RESC_HIER_STR_KW)) == NULL) {
            rodsLog(
                LOG_ERROR, "chkCollForBundleOpr: RESC_HIER_STR_KW not specified for %s", chkObjPermAndStatInp->objPath);
            return SYS_INVALID_RESC_INPUT;
        }

        memset(&openCollInp, 0, sizeof(openCollInp));
        rstrcpy(openCollInp.collName, chkObjPermAndStatInp->objPath, MAX_NAME_LEN);
        openCollInp.flags = RECUR_QUERY_FG | LONG_METADATA_FG | NO_TRIM_REPL_FG;
        ...
        while ((status = rsReadCollection(rsComm, &handleInx, &collEnt)) >= 0) {
            if (collEnt->specColl.collClass != NO_SPEC_COLL) {
                ...
                /* check permission */
                myId = chlCheckAndGetObjectID(rsComm, "-c", collEnt->specColl.collection, ACCESS_READ_OBJECT);
                if (myId < 0) {
                    status = myId;
                    rodsLog(LOG_ERROR,
                            "chkCollForBundleOpr: no accPerm to specColl %s. status = %d",
                            collEnt->specColl.collection,
                            status);
                    rsCloseCollection(rsComm, &handleInx);
                    freeCollEntForChkColl(collEnt);
                    freeCollEntForChkColl(curCollEnt);
            ...
            if (collEnt->objType == DATA_OBJ_T) {
                if (curCollEnt == NULL) {
                    ...
                    if (collEnt->replStatus > 0 && strcmp(resource, collEnt->resource) == 0 &&
                    ...
                    if (strcmp(curCollEnt->dataName, collEnt->dataName) == 0 &&
                        ...
                        snprintf(myPath, MAX_NAME_LEN, "%s/%s", curCollEnt->collName, curCollEnt->dataName);
                        ...
                        myId = chlCheckAndGetObjectID(rsComm, "-d", myPath, ACCESS_READ_OBJECT);
                        if (myId < 0 && myId != CAT_UNKNOWN_FILE) {
                            /* could return CAT_UNKNOWN_FILE if mounted files */
                            status = myId;
                            rodsLog(LOG_ERROR, "chkCollForBundleOpr: no accPerm to %s. status = %d", myPath, status);
                            rsCloseCollection(rsComm, &handleInx);
                            freeCollEntForChkColl(collEnt);
                            return status;
                        }
                            ...
                            saveCollEntForChkColl(collEnt);
                            collEnt = NULL;
                            if (curCollEnt->replStatus > 0 && strcmp(resource, curCollEnt->resource) == 0 &&
