                        ...
                        bytesBuf_t* subStructFileReadOutBBuf)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        status = remoteSubStructFileRead(rsComm, subStructFileReadInp, subStructFileReadOutBBuf, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsSubStructFileRead: resolveHost returned unrecognized value %d", remoteFlag);
                            ...
                            rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileRead: Invalid rodsServerHost");
    ...
    status = rcSubStructFileRead(rodsServerHost->conn, subStructFileReadInp, subStructFileReadOutBBuf);

    if (status < 0) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileRead: rcFileRead failed for fd %d", subStructFileReadInp->fd);
    }
...
int _rsSubStructFileRead(rsComm_t* _comm, subStructFileFdOprInp_t* _read_inp, bytesBuf_t* _out_buf)
{
    ...
    irods::error read_err = fileRead(_comm, struct_obj, _out_buf->buf, _read_inp->len);
    if (!read_err.ok()) {
