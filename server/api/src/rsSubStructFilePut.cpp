...
int rsSubStructFilePut(rsComm_t* rsComm, subFile_t* subFile, bytesBuf_t* subFilePutOutBBuf)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        status = remoteSubStructFilePut(rsComm, subFile, subFilePutOutBBuf, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsSubStructFilePut: resolveHost returned unrecognized value %d", remoteFlag);
                           ...
                           rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteSubStructFilePut: Invalid rodsServerHost");
    ...
    status = rcSubStructFilePut(rodsServerHost->conn, subFile, subFilePutOutBBuf);
...
int _rsSubStructFilePut(rsComm_t* _comm, subFile_t* _sub_file, bytesBuf_t* _out_buf)
{
    ...
    irods::structured_object_ptr struct_obj(new irods::structured_object(*_sub_file));
    ...
    if (fd < 0) {
        if (getErrno(fd) == EEXIST) {
            rodsLog(LOG_DEBUG10, "_rsSubStructFilePut: filePut for %s, status = %d", _sub_file->subFilePath, fd);
        }
        else {
            rodsLog(
                LOG_NOTICE, "_rsSubStructFilePut: subStructFileOpen error for %s, stat=%d", _sub_file->subFilePath, fd);
        }
        return fd;
    }
    ...
    if (status != _out_buf->len) {
        if (status >= 0) {
            rodsLog(LOG_NOTICE,
                    ...
                    _sub_file->subFilePath,
                    _out_buf->len,
                    status);
            status = SYS_COPY_LEN_ERR;
        }
        else {
            rodsLog(LOG_NOTICE, "_rsSubStructFilePut: Write error for %s, status = %d", _sub_file->subFilePath, status);
        }
    }
