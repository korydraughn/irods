                              ...
                              getTempPasswordForOtherOut_t** getTempPasswordForOtherOut)
{
    ...
    rodsLog(LOG_DEBUG, "rsGetTempPasswordForOther get stat=%d", status);
    if (status < 0) {
        return status;
    }
    ...
    if (rodsServerHost->localFlag == LOCAL_HOST) {
        ...
        if (irods::CFG_SERVICE_ROLE_PROVIDER == svc_role) {
            status = _rsGetTempPasswordForOther(rsComm, getTempPasswordForOtherInp, getTempPasswordForOtherOut);
        }
        ...
        status =
            rcGetTempPasswordForOther(rodsServerHost->conn, getTempPasswordForOtherInp, getTempPasswordForOtherOut);
    }
    if (status < 0) {
        rodsLog(LOG_NOTICE, "rsGetTempPasswordForOther: rcGetTempPasswordForOther failed, status = %d", status);
    }
    return status;
}
                               ...
                               getTempPasswordForOtherOut_t** getTempPasswordForOtherOut)
{
    ...
    status =
        chlMakeTempPw(rsComm, myGetTempPasswordForOtherOut->stringToHashWith, getTempPasswordForOtherInp->otherUser);
    if (status < 0) {
        rodsLog(LOG_NOTICE, "_rsGetTempPasswordForOther: getTempPasswordForOther, status = %d", status);
    }
