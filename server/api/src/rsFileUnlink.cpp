...
int rsFileUnlink(rsComm_t* rsComm, fileUnlinkInp_t* fileUnlinkInp)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsFileUnlink: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteFileUnlink(rsComm_t* rsComm, fileUnlinkInp_t* fileUnlinkInp, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileUnlink: Invalid rodsServerHost");
    ...
    if (status < 0) {
        rodsLog(
            LOG_NOTICE, "remoteFileUnlink: rcFileUnlink failed for %s, status = %d", fileUnlinkInp->fileName, status);
    }
...
int _rsFileUnlink(rsComm_t* _comm, fileUnlinkInp_t* _unlink_inp)
{
    ...
    irods::file_object_ptr file_obj(
        new irods::file_object(_comm, _unlink_inp->objPath, _unlink_inp->fileName, _unlink_inp->rescHier, 0, 0, 0));
