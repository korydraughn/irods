...
int rsModAccessControl(rsComm_t* rsComm, modAccessControlInp_t* modAccessControlInp)
{
    ...
    status = getAndConnRcatHost(rsComm, MASTER_RCAT, (const char*) newModAccessControlInp.path, &rodsServerHost);
    if (status < 0) {
        return status;
    }
    ...
    if (rodsServerHost->localFlag == LOCAL_HOST) {
        ...
        status = rcModAccessControl(rodsServerHost->conn, &newModAccessControlInp);
    }

    if (status < 0) {
        rodsLog(LOG_NOTICE, "rsModAccessControl: rcModAccessControl failed");
    }
    return status;
}
...
int _rsModAccessControl(rsComm_t* rsComm, modAccessControlInp_t* modAccessControlInp)
{
    ...
    if (status == 0) {
        status2 = applyRuleArg("acPostProcForModifyAccessControl", args, argc, &rei2, NO_SAVE_REI);
        if (status2 < 0) {
            if (rei2.status < 0) {
                status2 = rei2.status;
            }
