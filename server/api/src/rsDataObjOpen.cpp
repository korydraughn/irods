...
int rsDataObjOpen(rsComm_t* rsComm, dataObjInp_t* dataObjInp)
{
    ...
    if (remoteFlag < 0) {
        ...
        status = rcDataObjOpenAndStat(rodsServerHost->conn, dataObjInp, &openStat);
        if (status < 0) {
            return status;
        }
        ...
        if (getValByKey(&dataObjInp->condInput, RESC_HIER_STR_KW) == NULL) {
            irods::error ret =
                irods::resolve_resource_hierarchy(irods::OPEN_OPERATION, rsComm, dataObjInp, hier, &dataObjInfoHead);
            if (ret.ok()) {
                // =-=-=-=-=-=-=-
...
int _rsDataObjOpen(rsComm_t* rsComm, dataObjInp_t* dataObjInp, dataObjInfo_t* dataObjInfoHead)
{
    ...
    else if (getValByKey(&dataObjInp->condInput, PHYOPEN_BY_SIZE_KW) != NULL) {
    ...
    if (lockType != NULL) {
        ...
        if (lockFd > 0) {
            ...
            rodsLogError(
                LOG_ERROR, lockFd, "_rsDataObjOpen: lock error for %s. lockType = %s", dataObjInp->objPath, lockType);
            return lockFd;
        }
    }
    ...
    irods::error prop_err = irods::get_resource_property<std::string>(dataObjInfoHead->rescId, "class", resc_class);
    if (prop_err.ok()) {
...
{ // JMC - backport 4537
    ...
    if (phyOpenFlag == DO_NOT_PHYOPEN) {
        ...
        try {
            single_buff_sz =
                irods::get_advanced_setting<const int>(irods::CFG_MAX_SIZE_FOR_SINGLE_BUFFER) * 1024 * 1024;
        }
        ...
        if (getValByKey(&dataObjInp->condInput, DATA_INCLUDED_KW) != NULL && dataObjInfo->dataSize <= single_buff_sz) {
            status = 0;
        }
        else if (dataObjInfo->dataSize != UNKNOWN_FILE_SZ && dataObjInfo->dataSize <= single_buff_sz) {
            status = 0;
        }
        else {
...
int dataOpen(rsComm_t* rsComm, int l1descInx)
{
    ...
    if (status <= 0) {
        rodsLog(LOG_NOTICE, "dataOpen: l3Open of %s failed, status = %d", myDataObjInfo->filePath, status);
        return status;
    }
    else {
...
int l3OpenByHost(rsComm_t* rsComm, int l3descInx, int flags)
{
    ...
    newL3descInx = rsFileOpenByHost(rsComm, &fileOpenInp, FileDesc[l3descInx].rodsServerHost);
...
int createEmptyRepl(rsComm_t* rsComm, dataObjInp_t* dataObjInp, dataObjInfo_t** dataObjInfoHead)
{
    ...
    if (getValByKey(condInput, DEST_RESC_NAME_KW) == NULL && getValByKey(condInput, BACKUP_RESC_NAME_KW) == NULL &&
...
// =-=-=-=-=-=-=-
// JMC - backport 4590
int procDataObjOpenForWrite(rsComm_t* rsComm,
