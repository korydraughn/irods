...
int rsFileOpendir(rsComm_t* rsComm, fileOpendirInp_t* fileOpendirInp)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsFileOpendir: resolveHost returned unrecognized value %d", remoteFlag);
    ...
    fileInx = allocAndFillFileDesc(
        rodsServerHost, fileOpendirInp->objPath, fileOpendirInp->dirName, fileOpendirInp->resc_hier_, status, 0);
    if (fileInx < 0) {
...
int remoteFileOpendir(rsComm_t* rsComm, fileOpendirInp_t* fileOpendirInp, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileOpendir: Invalid rodsServerHost");
    ...
    if (fileInx < 0) {
        rodsLog(LOG_NOTICE, "remoteFileOpendir: rcFileOpendir failed for %s", fileOpendirInp->dirName);
    }
...
int _rsFileOpendir(rsComm_t* _comm, fileOpendirInp_t* _opendir_inp, void** _dir_ptr)
{
    ...
    irods::collection_object_ptr coll_obj(
        new irods::collection_object(_opendir_inp->dirName, _opendir_inp->resc_hier_, 0, 0));
