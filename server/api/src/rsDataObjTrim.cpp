...
int rsDataObjTrim(rsComm_t* rsComm, dataObjInp_t* dataObjInp)
{
    ...
    if (hier_char == NULL) {
        ...
        irods::error ret = irods::resource_redirect(irods::UNLINK_OPERATION, rsComm, dataObjInp, hier, host, local);
        if (!ret.ok()) {
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "rsDataObjTrim: getDataObjInfo for %s", dataObjInp->objPath);
        return status;
    }
...
int trimDataObjInfo(rsComm_t* rsComm, dataObjInfo_t* dataObjInfo)
{
    ...
    if (status < 0) {
        rodsLogError(LOG_ERROR, status, "trimDataObjInfo: rsDataObjTrim of %s error", dataObjInfo->objPath);
    }
    return status;
}
