...
int rsChkNVPathPerm(rsComm_t* rsComm, fileOpenInp_t* chkNVPathPermInp)
{
    ...
    if (remoteFlag < 0) {
        ...
        status = rsChkNVPathPermByHost(rsComm, chkNVPathPermInp, rodsServerHost);
        return status;
    }
}
...
int rsChkNVPathPermByHost(rsComm_t* rsComm, fileOpenInp_t* chkNVPathPermInp, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "rsChkNVPathPermByHost: Input NULL rodsServerHost");
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsChkNVPathPerm: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteChkNVPathPerm(rsComm_t* rsComm, fileOpenInp_t* chkNVPathPermInp, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteChkNVPathPerm: Invalid rodsServerHost");
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "remoteChkNVPathPerm: rcChkNVPathPerm failed for %s", chkNVPathPermInp->fileName);
    }
...
int _rsChkNVPathPerm(rsComm_t* rsComm, fileOpenInp_t* chkNVPathPermInp)
{
    ...
    if (sysUid < 0) {
        ...
        sysUid = rsComm->clientUser.sysUid = getUnixUid(rsComm->clientUser.userName);
    ...
    while (1) {
        irods::file_object_ptr file_obj(
            new irods::file_object(rsComm, chkNVPathPermInp->objPath, tmpPath, chkNVPathPermInp->resc_hier_, 0, 0, 0));
    ...
    if (sysUid != (int) myFileStat.st_uid && (myFileStat.st_mode & S_IWOTH) == 0) {
        return SYS_NO_PATH_PERMISSION;
    }
    else {
        return 0;
