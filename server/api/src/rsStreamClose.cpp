...
int rsStreamClose(rsComm_t* rsComm, fileCloseInp_t* streamCloseInp)
{
    ...
    if (fileInx < 3 || fileInx >= NUM_FILE_DESC) {
        rodsLog(LOG_ERROR, "rsStreamClose: fileInx %d out of range", fileInx);
    ...
    if (strcmp(FileDesc[fileInx].fileName, STREAM_FILE_NAME) != 0) {
        rodsLog(LOG_ERROR, "rsStreamClose: fileName %s is invalid for stream", FileDesc[fileInx].fileName);
        return SYS_INVALID_FILE_PATH;
    }
