...
int rsFileClose(rsComm_t* rsComm, fileCloseInp_t* fileCloseInp)
{
    ...
    int remoteFlag;
    int retVal;
    remoteFlag = getServerHostByFileInx(fileCloseInp->fileInx, &rodsServerHost);
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsFileClose: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteFileClose(rsComm_t* rsComm, fileCloseInp_t* fileCloseInp, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileClose: Invalid rodsServerHost");
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "remoteFileClose: rcFileClose failed for %d, status = %d", remFileCloseInp.fileInx, status);
    }
...
int _rsFileClose(rsComm_t* _comm, fileCloseInp_t* _close_inp)
{
    ...
    // the FD table
    if (0 == strcmp(STREAM_FILE_NAME, FileDesc[_close_inp->fileInx].fileName)) {
