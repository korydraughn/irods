...
int regUnbunSubfiles(rsComm_t* rsComm,
...
int regSubfile(rsComm_t* rsComm,
               const dataObjInfo_t& _dataObjInfo,
               char* subObjPath,
               char* subfilePath,
...
int rsStructFileExtAndReg(rsComm_t* rsComm, structFileExtAndRegInp_t* structFileExtAndRegInp)
{
    ...
    resolveLinkedPath(rsComm, structFileExtAndRegInp->objPath, &specCollCache, &structFileExtAndRegInp->condInput);

    resolveLinkedPath(rsComm, structFileExtAndRegInp->collection, &specCollCache, NULL);

    if (!isSameZone(structFileExtAndRegInp->objPath, structFileExtAndRegInp->collection)) {
    ...
    if (remoteFlag < 0) {
        ...
        status = rcStructFileExtAndReg(rodsServerHost->conn, structFileExtAndRegInp);
        return status;
    }
    ...
    if (getValByKey(&dataObjInp.condInput, RESC_HIER_STR_KW) == NULL) {
        irods::error ret = irods::resource_redirect(irods::OPEN_OPERATION, rsComm, &dataObjInp, hier, host, local);
        if (!ret.ok()) {
            ...
            if (CAT_NO_ROWS_FOUND == pass_err.code()) {
                ...
                addRErrorMsg(&rsComm->rError, REPLICA_NOT_IN_RESC, msg.c_str());
                ...
                addRErrorMsg(&rsComm->rError, SYS_RESC_DOES_NOT_EXIST, msg.c_str());
            }
            else {
                addRErrorMsg(&rsComm->rError, pass_err.code(), pass_err.result().c_str());
            }
    ...
    if (l1descInx < 0) {
        rodsLog(
            LOG_ERROR, "rsStructFileExtAndReg: rsDataObjOpen of %s error. status = %d", dataObjInp.objPath, l1descInx);
        return l1descInx;
    }
    ...
    status = unbunPhyBunFile(rsComm,
                             dataObjInp.objPath,
                             dataObjInfo->rescName, // JMC - backport 4657
    ...
    if (status == SYS_DIR_IN_VAULT_NOT_EMPTY) {
        ...
        snprintf(phyBunDir,
                 ...
                 "%s.%-u",
                 tmp,
                 irods::getRandom<unsigned int>()); // JMC cppcheck - src & dst snprintf
        status = unbunPhyBunFile(rsComm,
                                 dataObjInp.objPath,
    ...
    if (getValByKey(&structFileExtAndRegInp->condInput, FORCE_FLAG_KW) != NULL) {
        flags = flags | FORCE_FLAG_FLAG;
    }
    if (getValByKey(&structFileExtAndRegInp->condInput, BULK_OPR_KW) != NULL) {
        status = bulkRegUnbunSubfiles(rsComm,
                                      dataObjInfo->rescName,
                                      dataObjInfo->rescHier,
        ...
        status = regUnbunSubfiles(rsComm, *dataObjInfo, structFileExtAndRegInp->collection, phyBunDir, flags, NULL);
    }
    ...
    if (status == CAT_NO_ROWS_FOUND) {
        ...
        rodsLog(LOG_ERROR, "_rsUnbunAndRegPhyBunfile: rsStructFileExtAndReg for dir %s.stat=%d", phyBunDir, status);
    }
    rsDataObjClose(rsComm, &dataObjCloseInp);
...
int chkCollForExtAndReg(rsComm_t* rsComm, char* collection, rodsObjStat_t** rodsObjStatOut)
{
    ...
    if (status < 0) {
        ...
        if (status < 0) {
            rodsLog(LOG_ERROR, "chkCollForExtAndReg: rsMkCollR of %s error. status = %d", collection, status);
            return status;
        }
        else {
            status = collStatAllKinds(rsComm, &dataObjInp, &myRodsObjStat);
    ...
    if (status < 0 || NULL == myRodsObjStat) { // JMC cppcheck - nullptr
        rodsLog(LOG_ERROR, "chkCollForExtAndReg: collStat of %s error. status = %d", dataObjInp.objPath, status);
        return status;
    }
    else if (myRodsObjStat->specColl != NULL && myRodsObjStat->specColl->collClass != MOUNTED_COLL) {
        /* only do mounted coll */
        freeRodsObjStat(myRodsObjStat);
        rodsLog(LOG_ERROR, "chkCollForExtAndReg: %s is a struct file collection", dataObjInp.objPath);
    ...
    if (myRodsObjStat->specColl == NULL) {
        ...
        status = checkCollAccessPerm(rsComm, myRodsObjStat->specColl->collection, ACCESS_DELETE_OBJECT);
    }

    if (status < 0) {
        rodsLog(LOG_ERROR, "chkCollForExtAndReg: no permission to write %s, status = %d", collection, status);
        freeRodsObjStat(myRodsObjStat);
    }
    else {
                     ...
                     genQueryOut_t* attriArray)
{
    ...
    if (!exists(srcDirPath) || !is_directory(srcDirPath)) {
        rodsLog(LOG_ERROR, "regUnbunphySubfiles: opendir error for %s, errno = %d", phyBunDir, errno);
    ...
    for (directory_iterator itr(srcDirPath); itr != end_itr; ++itr) {
        ...
        if (!exists(p)) {
            rodsLog(LOG_ERROR, "regUnbunphySubfiles: stat error for %s, errno = %d", subfilePath, errno);
        ...
        // JMC - backport 4833
        if (is_symlink(p)) {
            rodsLogError(LOG_ERROR, SYMLINKED_BUNFILE_NOT_ALLOWED, "regUnbunSubfiles: %s is a symlink", subfilePath);
        ...
        if (is_directory(p)) {
            ...
            if (status < 0) {
                rodsLog(LOG_ERROR, "regUnbunSubfiles: rsMkCollR of %s error. status = %d", subObjPath, status);
                savedStatus = status;
                continue;
            }
            status = regUnbunSubfiles(rsComm, _dataObjInfo, subObjPath, subfilePath, flags, attriArray);
            if (status < 0) {
                rodsLog(LOG_ERROR, "regUnbunSubfiles: regUnbunSubfiles of %s error. status=%d", subObjPath, status);
                savedStatus = status;
                continue;
            }
        }
            ...
            st_size = file_size(p);
            status = regSubfile(rsComm, _dataObjInfo, subObjPath, subfilePath, st_size, flags);
            unlink(subfilePath);
            if (status < 0) {
                rodsLog(LOG_ERROR, "regUnbunSubfiles: regSubfile of %s error. status=%d", subObjPath, status);
                savedStatus = status;
                continue;
            }
        }
               ...
               int flags)
{
    ...
    if (exists(p)) {
        ...
        if (chkOrphanFile(rsComm, dataObjInfo.filePath, _dataObjInfo.rescName, &dataObjInfo) > 0) {
            ...
            if ((flags & FORCE_FLAG_FLAG) != 0 && dataObjInfo.dataId > 0 &&
                ...
                rodsLog(
                    LOG_ERROR, "regSubFile: phypath %s is already in use. status = %d", dataObjInfo.filePath, status);
                return status;
            }
        }
    }
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "regSubFile: link error %s to %s. errno = %d", subfilePath, dataObjInfo.filePath, errno);
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "regSubFile: svrRegDataObj of %s. errno = %d", dataObjInfo.objPath, errno);
        unlink(dataObjInfo.filePath);
    }
    else {
        ruleExecInfo_t rei;
        ...
        rei.doi = &dataObjInfo;
        rei.status = applyRule("acPostProcForTarFileReg", NULL, &rei, NO_SAVE_REI);
