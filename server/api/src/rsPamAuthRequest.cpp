...
int rsPamAuthRequest(rsComm_t* rsComm, pamAuthRequestInp_t* pamAuthRequestInp, pamAuthRequestOut_t** pamAuthRequestOut)
{
    ...
    status = getAndConnRcatHost(rsComm, MASTER_RCAT, (const char*) rsComm->clientUser.rodsZone, &rodsServerHost);
    if (status < 0) {
        return status;
    }
    if (rodsServerHost->localFlag == LOCAL_HOST) {
        ...
        if (irods::CFG_SERVICE_ROLE_PROVIDER == svc_role) {
            status = _rsPamAuthRequest(rsComm, pamAuthRequestInp, pamAuthRequestOut);
        }
        ...
        if (status) {
            rodsLog(LOG_NOTICE, "rsPamAuthRequest: could not establish SSL connection, status %d", status);
            return status;
        }

        status = rcPamAuthRequest(rodsServerHost->conn, pamAuthRequestInp, pamAuthRequestOut);
        sslEnd(rodsServerHost->conn);
        rcDisconnect(rodsServerHost->conn);
        rodsServerHost->conn = NULL;
        if (status < 0) {
            rodsLog(LOG_NOTICE, "rsPamAuthRequest: rcPamAuthRequest to remote server failed, status %d", status);
        }
    }
    return status;
}
...
int runPamAuthCheck(char* username, char* password)
{
    ...
    if (pid) {
        ...
        */
        ...
        close(p2cp[1]);
        i = execl(PAM_AUTH_CHECK_PROG, PAM_AUTH_CHECK_PROG, username, (char*) NULL);
        perror("execl");
...
int _rsPamAuthRequest(rsComm_t* rsComm, pamAuthRequestInp_t* pamAuthRequestInp, pamAuthRequestOut_t** pamAuthRequestOut)
{
    ...
    *pamAuthRequestOut = (pamAuthRequestOut_t*) malloc(sizeof(pamAuthRequestOut_t));
    ...
    status = runPamAuthCheck(pamAuthRequestInp->pamUser, pamAuthRequestInp->pamPassword);
    if (status == 256) {
    ...
    status = chlUpdateIrodsPamPassword(
        rsComm, pamAuthRequestInp->pamUser, pamAuthRequestInp->timeToLive, NULL, &result->irodsPamPassword);
    return status;
}
