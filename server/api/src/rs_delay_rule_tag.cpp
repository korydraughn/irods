...
auto rs_delay_rule_tag(RsComm* _comm, DelayRuleTagInput* _input) -> int
{
    if (!_comm || !_input) {
    ...
    if (!is_non_empty_string(_input->rule_id, sizeof(DelayRuleTagInput::rule_id))) {
        log_api::error("{}: Rule ID must be a non-empty string.", __func__);
        return SYS_INVALID_INPUT_PARAM;
    }

    if (!is_non_empty_string(_input->tag, sizeof(DelayRuleTagInput::tag))) {
    ...
    if (host_info->localFlag != LOCAL_HOST) {
        ...
        return rc_delay_rule_tag(host_info->conn, _input);
    }
    ...
    try {
        const auto ec = chl_delay_rule_tag(*_comm, _input->rule_id, _input->tag);

        if (ec < 0) {
            log_api::error("{}: chl_delay_rule_tag failed with error code [{}].", __func__, ec);
        }
