...
int applyRuleForPostProcForRead(rsComm_t* rsComm, bytesBuf_t* dataObjReadOutBBuf, char* objPath)
{
    ...
    *myInOutStruct = dataObjReadOutBBuf->len;
    addMsParamToArray(&msParamArray, "*ReadBuf", BUF_LEN_MS_T, myInOutStruct, dataObjReadOutBBuf, 0);
    int status = applyRule("acPostProcForDataObjRead(*ReadBuf)", &msParamArray, &rei2, NO_SAVE_REI);
    free(rei2.doi);
    if (status < 0) {
        if (rei2.status < 0) {
            status = rei2.status;
        }
        rodsLog(LOG_ERROR, "rsDataObjRead: acPostProcForDataObjRead error=%d", status);
...
int rsDataObjRead(rsComm_t* rsComm, openedDataObjInp_t* dataObjReadInp, bytesBuf_t* dataObjReadOutBBuf)
{
    ...
    if (l1descInx < 2 || l1descInx >= NUM_L1_DESC) {
        rodsLog(LOG_NOTICE, "rsDataObjRead: l1descInx %d out of range", l1descInx);
...
int l3Read(rsComm_t* rsComm, int l1descInx, int len, bytesBuf_t* dataObjReadOutBBuf)
{
    ...
    if (getStructFileType(dataObjInfo->specColl) >= 0) {
        ...
        switch (category) {
            ...
            default:
                rodsLog(LOG_NOTICE, "l3Read: rescCat type %d is not recognized", category);
