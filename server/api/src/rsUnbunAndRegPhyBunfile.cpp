...
int rsUnbunAndRegPhyBunfile(rsComm_t* rsComm, dataObjInp_t* dataObjInp)
{
    ...
    if ((resc_name = getValByKey(&dataObjInp->condInput, DEST_RESC_NAME_KW)) == NULL) {
...
int _rsUnbunAndRegPhyBunfile(rsComm_t* rsComm, dataObjInp_t* dataObjInp, const char* _resc_name)
{
    ...
    if (remoteFlag == REMOTE_HOST) {
        ...
        status = remoteUnbunAndRegPhyBunfile(rsComm, dataObjInp, rodsServerHost);
        return status;
    }
    /* process this locally */
    if ((bunFilePath = getValByKey(&dataObjInp->condInput, BUN_FILE_PATH_KW)) // JMC - backport 4768
        == NULL) {
        rodsLog(LOG_ERROR, "_rsUnbunAndRegPhyBunfile: No filePath input for %s", dataObjInp->objPath);
    ...
    status = unbunPhyBunFile(rsComm,
                             ...
                             dataType,
                             0,
                             resc_hier); // JMC - backport 4632, 4657, 4664
    ...
    if (status == CAT_NO_ROWS_FOUND) {
        ...
        rodsLog(LOG_ERROR, "_rsUnbunAndRegPhyBunfile: regUnbunPhySubfiles for dir %s. stat = %d", phyBunDir, status);
    }
...
int regUnbunPhySubfiles(rsComm_t* rsComm, const char* _resc_name, char* phyBunDir, int rmBunCopyFlag)
{
    ...
    for (directory_iterator itr(srcDirPath); itr != end_itr; ++itr) {
        ...
        /* do the registration */
        addKeyVal(&dataObjInp.condInput, QUERY_BY_DATA_ID_KW, (char*) childPath.c_str());
        status = getDataObjInfo(rsComm, &dataObjInp, &dataObjInfoHead, NULL, 1);
        if (status < 0) {
            rodsLog(LOG_DEBUG, "regUnbunphySubfiles: getDataObjInfo error for %s, status = %d", subfilePath, status);
        ...
        if (strcmp(dataObjInfoHead->rescName, BUNDLE_RESC) != 0) {
            /* no match */
            rodsLog(LOG_DEBUG, "regUnbunphySubfiles: No copy in BUNDLE_RESC for %s", dataObjInfoHead->objPath);
        ...
        if (rmBunCopyFlag > 0) {
            rstrcpy(dataObjUnlinkInp.objPath, bunDataObjInfo->objPath, MAX_NAME_LEN);
...
int regPhySubFile(rsComm_t* rsComm, char* subfilePath, dataObjInfo_t* bunDataObjInfo, const char* _resc_name)
{
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "regPhySubFile: getFilePathName err for %s. status = %d", dataObjInp.objPath, status);
        return status;
    }
    ...
    status = mkDirForFilePath(rsComm, 0, stageDataObjInfo.filePath, stageDataObjInfo.rescHier, getDefDirMode());
    if (status < 0) {
    ...
    if (status < 0) {
        rodsLog(
            LOG_ERROR, "regPhySubFile: link error %s to %s. errno = %d", subfilePath, stageDataObjInfo.filePath, errno);
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "regPhySubFile: rsRegReplica error for %s. status = %d", bunDataObjInfo->objPath, status);
        return status;
    }
...
int unbunPhyBunFile(rsComm_t* rsComm,
                    char* objPath,
...
{ // JMC - backport 4632, 4657
    ...
    snprintf(structFileOprInp.specColl->collection, MAX_NAME_LEN, "%s.dir", objPath); // JMC - backport 4657
    rstrcpy(structFileOprInp.specColl->objPath, objPath, MAX_NAME_LEN);               // JMC - backport 4657
    ...
    if (status == SYS_DIR_IN_VAULT_NOT_EMPTY) {
        ...
        if (chkOrphanDir(rsComm, phyBunDir, _resc_name) > 0) {
            ...
            if (status >= 0) {
                rodsLog(LOG_NOTICE, "unbunPhyBunFile: %s has been moved to ORPHAN_DIR.stat=%d", phyBunDir, status);
                status = rsStructFileExtract(rsComm, &structFileOprInp);
            }
            else {
                rodsLog(LOG_ERROR, "unbunPhyBunFile: renameFilePathToNewDir err for %s.stat=%d", phyBunDir, status);
...
int remoteUnbunAndRegPhyBunfile(rsComm_t* rsComm, dataObjInp_t* dataObjInp, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteUnbunAndRegPhyBunfile: Invalid rodsServerHost");
    ...
    for (directory_iterator itr(srcDirPath); itr != end_itr; ++itr) {
        ...
        if (is_regular_file(p)) {
            if ((linkCnt = hard_link_count(p)) >= 2) {
                ...
                rodsLog(LOG_ERROR, "rmLinkedFilesInUnixDir: st_nlink of %s is only %d", subfilePath, linkCnt);
            }
        }
        else { /* a directory */
