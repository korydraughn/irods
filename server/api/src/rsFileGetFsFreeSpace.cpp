                         ...
                         fileGetFsFreeSpaceOut_t** fileGetFsFreeSpaceOut)
{
    ...
    *fileGetFsFreeSpaceOut = NULL;
    ...
    if (remoteFlag == LOCAL_HOST) {
        status = _rsFileGetFsFreeSpace(rsComm, fileGetFsFreeSpaceInp, fileGetFsFreeSpaceOut);
    }
    else if (remoteFlag == REMOTE_HOST) {
        status = remoteFileGetFsFreeSpace(rsComm, fileGetFsFreeSpaceInp, fileGetFsFreeSpaceOut, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsFileGetFsFreeSpace: resolveHost returned unrecognized value %d", remoteFlag);
                             ...
                             rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileGetFsFreeSpace: Invalid rodsServerHost");
    ...
    status = rcFileGetFsFreeSpace(rodsServerHost->conn, fileGetFsFreeSpaceInp, fileGetFsFreeSpaceOut);

    if (status < 0) {
        rodsLog(LOG_NOTICE,
                "remoteFileGetFsFreeSpace: rcFileGetFsFreeSpace failed for %s, status = %d",
                fileGetFsFreeSpaceInp->fileName,
                status);
    }
...
int _rsFileGetFsFreeSpace(rsComm_t* _comm,
