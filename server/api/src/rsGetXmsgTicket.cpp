...
int rsGetXmsgTicket(rsComm_t*, getXmsgTicketInp_t* getXmsgTicketInp, xmsgTicketInfo_t** outXmsgTicketInfo)
{
    int status;
    int hashSlotNum;
    time_t thisTime;
    ...
    *outXmsgTicketInfo = (xmsgTicketInfo_t*) calloc(1, sizeof(xmsgTicketInfo_t));
    ...
    while (1) {
        ...
        status = addTicketToHQue(*outXmsgTicketInfo, &XmsgHashQue[hashSlotNum]);
