...
int rsModAVUMetadata(rsComm_t* rsComm, modAVUMetadataInp_t* modAVUMetadataInp)
{
    ...
    if (rodsServerHost->localFlag == LOCAL_HOST) {
        ...
        status = rcModAVUMetadata(rodsServerHost->conn, modAVUMetadataInp);
    }

    if (status < 0) {
        rodsLog(LOG_NOTICE, "rsModAVUMetadata: rcModAVUMetadata failed");
    }
    return status;
}
...
#define ARG(arg)                                                                                                       \
    ...
    status2 = applyRuleArg("acPreProcForModifyAVUMetadata", args, argc, &rei2, NO_SAVE_REI);
    if (status2 < 0) {
        if (rei2.status < 0) {
            status2 = rei2.status;
        }
    ...
    if (strcmp(modAVUMetadataInp->arg0, "add") == 0) {
        ...
        status = chlDeleteAVUMetadata(rsComm,
                                      1,
                                      modAVUMetadataInp->arg1,
                                      modAVUMetadataInp->arg2,
                                      modAVUMetadataInp->arg3,
        ...
        status = chlCopyAVUMetadata(
            rsComm, modAVUMetadataInp->arg1, modAVUMetadataInp->arg2, modAVUMetadataInp->arg3, modAVUMetadataInp->arg4);
    }
    else if (strcmp(modAVUMetadataInp->arg0, "mod") == 0) {
