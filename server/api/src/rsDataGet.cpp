...
int rsDataGet(rsComm_t* rsComm, dataOprInp_t* dataOprInp, portalOprOut_t** portalOprOut)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        status = remoteDataGet(rsComm, dataOprInp, portalOprOut, rodsServerHost);
...
{
    return setupSrvPortalForParaOpr(rsComm, dataOprInp, GET_OPR, portalOprOut);
}

int remoteDataGet(rsComm_t* rsComm,
                  ...
                  rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteDataGet: Invalid rodsServerHost");
