...
int rsFileTruncate(rsComm_t* rsComm, fileOpenInp_t* fileTruncateInp)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsFileTruncate: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteFileTruncate(rsComm_t* rsComm, fileOpenInp_t* fileTruncateInp, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileTruncate: Invalid rodsServerHost");
...
int _rsFileTruncate(rsComm_t* _comm, fileOpenInp_t* _trunc_inp)
{
    ...
    irods::file_object_ptr file_obj(
        new irods::file_object(_comm, _trunc_inp->objPath, _trunc_inp->fileName, _trunc_inp->resc_hier_, 0, 0, 0));
