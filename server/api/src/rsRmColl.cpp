...
int rsRmColl(rsComm_t* rsComm, collInp_t* rmCollInp, collOprStat_t** collOprStat)
{
    ...
    resolveLinkedPath(rsComm, rmCollInp->collName, &specCollCache, &rmCollInp->condInput);
    status = getAndConnRcatHost(rsComm, MASTER_RCAT, (const char*) rmCollInp->collName, &rodsServerHost);
    ...
    if (status < 0) {
        ...
        rodsLog(LOG_ERROR, "rsRmColl:acPreprocForRmColl error for %s,stat=%d", rmCollInp->collName, status);
    ...
    if (getValByKey(&rmCollInp->condInput, RECURSIVE_OPR__KW) == NULL) {
        ...
        if (isTrashPath(rmCollInp->collName) == False && getValByKey(&rmCollInp->condInput, FORCE_FLAG_KW) != NULL) {
            rodsLog(LOG_NOTICE, "rsRmColl: Recursively removing %s.", rmCollInp->collName);
        }
        status = _rsRmCollRecur(rsComm, rmCollInp, collOprStat);
    }
    rei.status = status;
    rei.status = applyRule("acPostProcForRmColl", NULL, &rei, NO_SAVE_REI);

    clearKeyVal(rei.condInputData);
    free(rei.condInputData);

    if (rei.status < 0) {
        rodsLog(LOG_ERROR, "rsRmColl:acPostProcForRmColl error for %s,stat=%d", rmCollInp->collName, status);
    }
...
int _rsRmColl(rsComm_t* rsComm, collInp_t* rmCollInp, collOprStat_t** collOprStat)
{
    ...
    if (getValByKey(&rmCollInp->condInput, UNREG_COLL_KW) != NULL) {
        ...
        status = resolvePathInSpecColl(rsComm, rmCollInp->collName, WRITE_COLL_PERM, 0, &dataObjInfo);
        if (status < 0 && status != CAT_NO_ROWS_FOUND) {
            return status;
        }
        else if (status == COLL_OBJ_T && dataObjInfo->specColl != NULL) {
            if (dataObjInfo->specColl->collClass == LINKED_COLL) {
                rstrcpy(rmCollInp->collName, dataObjInfo->objPath, MAX_NAME_LEN);
                status = svrUnregColl(rsComm, rmCollInp);
            }
            else {
    ...
    if (status >= 0 && collOprStat != NULL) {
        *collOprStat = (collOprStat_t*) malloc(sizeof(collOprStat_t));
        ...
        rstrcpy((*collOprStat)->lastObjPath, rmCollInp->collName, MAX_NAME_LEN);
    }
    return status;
}
...
int _rsRmCollRecur(rsComm_t* rsComm, collInp_t* rmCollInp, collOprStat_t** collOprStat)
{
    ...
    if (status == COLL_OBJ_T && dataObjInfo->specColl != NULL && dataObjInfo->specColl->collClass == LINKED_COLL) {
        rstrcpy(rmCollInp->collName, dataObjInfo->objPath, MAX_NAME_LEN);
    ...
    if (status != COLL_OBJ_T || dataObjInfo->specColl == NULL) {
        /* a normal coll */
        if (rmCollInp->oprType != UNREG_OPR && getValByKey(&rmCollInp->condInput, FORCE_FLAG_KW) == NULL &&
...
int _rsPhyRmColl(rsComm_t* rsComm, collInp_t* rmCollInp, dataObjInfo_t* dataObjInfo, collOprStat_t** collOprStat)
{
    ...
    if (handleInx < 0) {
        rodsLog(LOG_ERROR, "_rsPhyRmColl: rsOpenCollection of %s error. status = %d", openCollInp.collName, handleInx);
        return handleInx;
    }
    ...
    if ((tmpValue = getValByKey(&rmCollInp->condInput, AGE_KW)) != NULL) {
        if (CollHandle[handleInx].rodsObjStat != NULL) {
            ...
            int modifyTime = atoi(CollHandle[handleInx].rodsObjStat->modifyTime);
    ...
    while ((status = rsReadCollection(rsComm, &handleInx, &collEnt)) >= 0) {
        ...
        if (collEnt->objType == DATA_OBJ_T) {
            ...
            if (status < 0) {
                rodsLog(LOG_ERROR, "_rsPhyRmColl:rsDataObjUnlink failed for %s. stat = %d", dataObjInp.objPath, status);
            ...
            if (status < 0) {
                ...
                rodsLog(LOG_ERROR, "_rsPhyRmColl:acPreprocForRmColl error for %s,stat=%d", tmpCollInp.collName, status);
                free(collEnt);
                return status;
            }
            ...
            rei.status = applyRule("acPostProcForRmColl", NULL, &rei, NO_SAVE_REI);
            if (rei.status < 0) {
                rodsLog(LOG_ERROR, "_rsRmColl:acPostProcForRmColl error for %s,stat=%d", tmpCollInp.collName, status);
            }
        }
        if (status < 0) {
            savedStatus = status;
...
int rsMkTrashPath(rsComm_t* rsComm, char* objPath, char* trashPath)
{
    ...
    *trashPathPtr = '/';
    ...
    if (*tmpStr == '\0') {
        rodsLog(LOG_ERROR, "rsMkTrashPath: input path %s too short", objPath);
    ...
    *trashPathPtr = '\0';
    ...
    if (rsComm->clientUser.authInfo.authFlag == REMOTE_USER_AUTH ||
        ...
        snprintf(trashPathPtr, MAX_NAME_LEN, "trash/home/%s/%s", rsComm->clientUser.userName, tmpStr);
    }

    if ((status = splitPathByKey(trashPath, destTrashColl, MAX_NAME_LEN, myFile, MAX_NAME_LEN, '/')) < 0) {
        rodsLog(LOG_ERROR, "rsMkTrashPath: splitPathByKey error for %s ", trashPath);
        return USER_INPUT_PATH_ERR;
    }
    ...
    if (status < 0) {
        rodsLog(
            LOG_ERROR, "rsMkTrashPath: rsMkCollR error for startPath %s, destPath %s ", startTrashPath, destTrashColl);
    }
...
int rsMvCollToTrash(rsComm_t* rsComm, collInp_t* rmCollInp)
{
    ...
    status =
        rsQueryDataObjInCollReCur(rsComm, rmCollInp->collName, &genQueryInp, &genQueryOut, ACCESS_DELETE_OBJECT, 0);
    ...
    while (status >= 0) {
        ...
        if ((subColl = getSqlResultByInx(genQueryOut, COL_COLL_NAME)) == NULL) {
            rodsLog(LOG_ERROR, "rsMvCollToTrash: getSqlResultByInx for COL_COLL_NAME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }

        if ((dataObj = getSqlResultByInx(genQueryOut, COL_DATA_NAME)) == NULL) {
            rodsLog(LOG_ERROR, "rsMvCollToTrash: getSqlResultByInx for COL_DATA_NAME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }

        if ((rescName = getSqlResultByInx(genQueryOut, COL_D_RESC_NAME)) == NULL) {
            rodsLog(LOG_ERROR, "rsMvCollToTrash: getSqlResultByInx for COL_D_RESC_NAME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }

        snprintf(dataObjInfo.objPath, MAX_NAME_LEN, "%s/%s", subColl->value, dataObj->value);
        ...
        if (status < 0 && status != NO_MORE_RULES_ERR && status != SYS_DELETE_DISALLOWED) {
            rodsLog(LOG_NOTICE,
    ...
    if (status < 0 && status != CAT_NO_ROWS_FOUND) {
        rodsLog(
            LOG_ERROR, "rsMvCollToTrash: rsQueryDataObjInCollReCur error for %s, stat=%d", rmCollInp->collName, status);
        freeGenQueryOut(&genQueryOut);
        return status;
    }
