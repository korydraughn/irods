...
int rsFileWrite(rsComm_t* rsComm, const fileWriteInp_t* fileWriteInp, const bytesBuf_t* fileWriteInpBBuf)
{
    ...
    remoteFlag = getServerHostByFileInx(fileWriteInp->fileInx, &rodsServerHost);

    if (remoteFlag == LOCAL_HOST) {
        retVal = _rsFileWrite(rsComm, fileWriteInp, fileWriteInpBBuf);
    }
    else if (remoteFlag == REMOTE_HOST) {
        retVal = remoteFileWrite(rsComm, fileWriteInp, fileWriteInpBBuf, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsFileWrite: resolveHost returned unrecognized value %d", remoteFlag);
                    ...
                    rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileWrite: Invalid rodsServerHost");
    ...
    const fileWriteInp_t remoteFileWriteInp{.fileInx = convL3descInx(fileWriteInp->fileInx), .len = fileWriteInp->len};
    retVal = rcFileWrite(rodsServerHost->conn, &remoteFileWriteInp, fileWriteInpBBuf);

    if (retVal < 0) {
        rodsLog(
            LOG_NOTICE, "remoteFileWrite: rcFileWrite failed for %s", FileDesc[remoteFileWriteInp.fileInx].fileName);
    }
...
int _rsFileWrite(rsComm_t* _comm, const fileWriteInp_t* _write_inp, const bytesBuf_t* _write_bbuf)
{
    // =-=-=-=-=-=-=-
    ...
    irods::file_object_ptr file_obj(new irods::file_object(_comm,
    ...
    irods::error write_err = fileWrite(_comm, file_obj, _write_bbuf->buf, _write_inp->len);
    // =-=-=-=-=-=-=-
