...
int rsDataObjCreate(rsComm_t* rsComm, dataObjInp_t* dataObjInp)
{
    ...
    if (lockType != NULL) {
        ...
        if (lockFd >= 0) {
            ...
            rodsLogError(
                LOG_ERROR, lockFd, "rsDataObjCreate: lock error for %s. lockType = %s", dataObjInp->objPath, lockType);
            return lockFd;
        }
    }
    ...
    if (rodsObjStatOut == NULL || (rodsObjStatOut->objType == UNKNOWN_OBJ_T && rodsObjStatOut->specColl == NULL)) {
    ...
    else if (rodsObjStatOut->specColl != NULL && rodsObjStatOut->objType == UNKNOWN_OBJ_T) {
        /* newly created. take out FORCE_FLAG since it could be used by put */
        /* rmKeyVal (&dataObjInp->condInput, FORCE_FLAG_KW); */
        ...
        if (getValByKey(&dataObjInp->condInput, FORCE_FLAG_KW) != NULL) {
            ...
            std::string hier;
            irods::error ret = irods::resolve_resource_hierarchy(irods::WRITE_OPERATION, rsComm, dataObjInp, hier);
            if (!ret.ok()) {
...
int _rsDataObjCreate(rsComm_t* rsComm, dataObjInp_t* dataObjInp)
{
    ...
    status = l1descInx = _rsDataObjCreateWithResc(rsComm, dataObjInp, "INVALID_RESOURCE_NAME");
    ...
    if (status >= 0) {
        ...
        rodsLog(LOG_NOTICE, "rsDataObjCreate: Internal error");
...
int specCollSubCreate(rsComm_t* rsComm, dataObjInp_t* dataObjInp)
{
    ...
    status = resolvePathInSpecColl(rsComm, dataObjInp->objPath, WRITE_COLL_PERM, 0, &dataObjInfo);
    if (dataObjInfo == NULL) { // JMC cppcheck
        rodsLog(LOG_ERROR, "specCollSubCreate :: dataObjInp is null");
        return status;
    }
    if (status >= 0) {
        rodsLog(LOG_ERROR, "specCollSubCreate: phyPath %s already exist", dataObjInfo->filePath);
    ...
    fillL1desc(l1descInx, dataObjInp, dataObjInfo, NEWLY_CREATED_COPY, dataObjInp->dataSize);
...
int _rsDataObjCreateWithResc(rsComm_t* rsComm, dataObjInp_t* dataObjInp, const std::string& _resc_name)
{
    ...
    char* kvp_str = getValByKey(&dataObjInp->condInput, KEY_VALUE_PASSTHROUGH_KW);
    if (kvp_str) {
...
int dataObjCreateAndReg(rsComm_t* rsComm, int l1descInx)
{
    ...
    if (status < 0) {
        ...
        rodsLog(
            LOG_NOTICE, "dataObjCreateAndReg: rsRegDataObj for %s failed, status = %d", myDataObjInfo->objPath, status);
        return status;
    }
    else {
...
int dataCreate(rsComm_t* rsComm, int l1descInx)
{
    ...
    if (status <= 0) {
        rodsLog(LOG_NOTICE, "dataCreate: l3Create of %s failed, status = %d", myDataObjInfo->filePath, status);
        return status;
    }
    else {
...
int l3Create(rsComm_t* rsComm, int l1descInx)
{
    ...
    if (getStructFileType(dataObjInfo->specColl) >= 0) {
        ...
        l3descInx = l3CreateByObjInfo(rsComm, L1desc[l1descInx].dataObjInp, L1desc[l1descInx].dataObjInfo);
    }
...
int l3CreateByObjInfo(rsComm_t* rsComm, dataObjInp_t* dataObjInp, dataObjInfo_t* dataObjInfo)
{
    ...
    do {
        ...
        rstrcpy(fileCreateInp.fileName, dataObjInfo->filePath, MAX_NAME_LEN);
        retryCnt++;
    } while (getErrno(l3descInx) == EEXIST && resolveDupFilePath(rsComm, dataObjInfo, dataObjInp) >= 0 &&
...
int getRescForCreate(rsComm_t* _comm, dataObjInp_t* _obj_inp, std::string& _resc_name)
{
    ...
    if (status < 0) {
        ...
        rodsLog(
            LOG_NOTICE, "getRescForCreate:acSetRescSchemeForCreate error for %s,status=%d", _obj_inp->objPath, status);
