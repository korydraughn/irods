...
int rsGeneralRowPurge(rsComm_t* rsComm, generalRowPurgeInp_t* generalRowPurgeInp)
{
    ...
    if (rodsServerHost->localFlag == LOCAL_HOST) {
        ...
        status = rcGeneralRowPurge(rodsServerHost->conn, generalRowPurgeInp);
    }

    if (status < 0) {
        rodsLog(LOG_NOTICE, "rsGeneralRowPurge: rcGeneralRowPurge failed");
    }
    return status;
}
...
int _rsGeneralRowPurge(rsComm_t* rsComm, generalRowPurgeInp_t* generalRowPurgeInp)
{
    ...
    rodsLog(LOG_DEBUG, "_rsGeneralRowPurge tableName=%s", generalRowPurgeInp->tableName);

    if (strcmp(generalRowPurgeInp->tableName, "serverload") == 0) {
        status = chlPurgeServerLoad(rsComm, generalRowPurgeInp->secondsAgo);
        return status;
    }
    if (strcmp(generalRowPurgeInp->tableName, "serverloaddigest") == 0) {
        status = chlPurgeServerLoadDigest(rsComm, generalRowPurgeInp->secondsAgo);
        return status;
    }
