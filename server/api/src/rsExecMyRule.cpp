...
{
    if (_exec_inp == NULL) {
        rodsLog(LOG_NOTICE, "rsExecMyRule error. NULL input");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }

    char* available_str = getValByKey(&_exec_inp->condInput, "available");
    ...
    if (remoteFlag == REMOTE_HOST) {
        return remoteExecMyRule(_comm, _exec_inp, _out_arr, rods_svr_host);
    }

    char* inst_name_str = getValByKey(&_exec_inp->condInput, irods::CFG_INSTANCE_NAME_KW.c_str());
    ...
    if (_exec_inp->inpParamArray == NULL) {
        _exec_inp->inpParamArray = (msParamArray_t*) malloc(sizeof(msParamArray_t));
    ...
    *_out_arr = rei.msParamArray;
    ...
    if (err.code() < 0) {
        rodsLog(LOG_ERROR, "rsExecMyRule : execMyRule error for %s, status = %d", _exec_inp->myRule, err.code());
        return err.code();
    }
                     ...
                     rodsServerHost_t* rods_svr_host)
{
    ...
    if (rods_svr_host == NULL) {
        rodsLog(LOG_ERROR, "remoteExecMyRule: Invalid rods_svr_host");
