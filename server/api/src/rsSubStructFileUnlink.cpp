...
int rsSubStructFileUnlink(rsComm_t* rsComm, subFile_t* subFile)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsSubStructFileUnlink: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteSubStructFileUnlink(rsComm_t* rsComm, subFile_t* subFile, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileUnlink: Invalid rodsServerHost");
...
int _rsSubStructFileUnlink(rsComm_t* _comm, subFile_t* _sub_file)
{
    // =-=-=-=-=-=-=-
