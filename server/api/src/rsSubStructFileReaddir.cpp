...
int rsSubStructFileReaddir(rsComm_t* rsComm,
                           ...
                           rodsDirent_t** rodsDirent)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        status = remoteSubStructFileReaddir(rsComm, subStructFileReaddirInp, rodsDirent, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsSubStructFileReaddir: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteSubStructFileReaddir(rsComm_t* rsComm,
                               subStructFileFdOprInp_t* subStructFileReaddirInp,
                               rodsDirent_t** rodsDirent,
                               rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileReaddir: Invalid rodsServerHost");
    ...
    status = rcSubStructFileReaddir(rodsServerHost->conn, subStructFileReaddirInp, rodsDirent);

    if (status < 0 && status != -1) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileReaddir: rcFileReaddir failed for fd %d", subStructFileReaddirInp->fd);
    }
...
int _rsSubStructFileReaddir(rsComm_t* _comm, subStructFileFdOprInp_t* _read_inp, rodsDirent_t** _dirent)
{
    ...
    irods::structured_object_ptr struct_obj(new irods::structured_object(*_read_inp));
