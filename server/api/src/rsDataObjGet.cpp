...
int _rsDataObjGet(rsComm_t* rsComm,
                  ...
                  dataObjInfo_t* dataObjInfoHead)
{
    ...
    if (L1desc[l1descInx].l3descInx <= 2) {
        ...
        status = l3DataGetSingleBuf(rsComm, l1descInx, dataObjOutBBuf, portalOprOut);
        if (status >= 0) {
            int status2;
    ...
    retval = sendAndRecvBranchMsg(rsComm, rsComm->apiInx, status, (void*) *portalOprOut, dataObjOutBBuf);
...
int preProcParaGet(rsComm_t* rsComm, int l1descInx, portalOprOut_t** portalOprOut)
{
    ...
    if (L1desc[l1descInx].remoteZoneHost != NULL) {
        status = remoteDataGet(rsComm, &dataOprInp, portalOprOut, L1desc[l1descInx].remoteZoneHost);
    }
    else {
...
int l3DataGetSingleBuf(rsComm_t* rsComm, int l1descInx, bytesBuf_t* dataObjOutBBuf, portalOprOut_t** portalOprOut)
{
    ...
    *portalOprOut = (portalOprOut_t*) malloc(sizeof(portalOprOut_t));
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "l3DataGetSingleBuf: rsDataObjClose of %d error, status = %d", l1descInx, status);
    }
...
int l3FileGetSingleBuf(rsComm_t* rsComm, int l1descInx, bytesBuf_t* dataObjOutBBuf)
{
    ...
    copyKeyVal(&dataObjInfo->condInput, &fileGetInp.condInput);
