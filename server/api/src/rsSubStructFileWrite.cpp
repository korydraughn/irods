                         ...
                         bytesBuf_t* subStructFileWriteOutBBuf)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        status = remoteSubStructFileWrite(rsComm, subStructFileWriteInp, subStructFileWriteOutBBuf, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsSubStructFileWrite: resolveHost returned unrecognized value %d", remoteFlag);
                             ...
                             rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileWrite: Invalid rodsServerHost");
    ...
    status = rcSubStructFileWrite(rodsServerHost->conn, subStructFileWriteInp, subStructFileWriteOutBBuf);

    if (status < 0) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileWrite: rcFileWrite failed for fd %d", subStructFileWriteInp->fd);
    }
...
int _rsSubStructFileWrite(rsComm_t* _comm, subStructFileFdOprInp_t* _write_inp, bytesBuf_t* _out_buf)
{
    ...
    irods::structured_object_ptr struct_obj(new irods::structured_object(*_write_inp));
