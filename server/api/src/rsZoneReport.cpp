...
int rsZoneReport(rsComm_t* _comm, bytesBuf_t** _bbuf)
{
    ...
    int status = getAndConnRcatHost(_comm, MASTER_RCAT, (const char*) NULL, &rods_host);
    if (status < 0) {
        return status;
    }
                                        ...
                                        json_t*& _resc_arr)
{
    ...
    msg << "failed in svrToSvrConnect for resource [" << _resc_name << "], at hostname [" << _host_name
        << "], error code " << _error;
    ...
    if (!obj) {
        rodsLog(LOG_ERROR, "%s:%d - failed to create json object", __FUNCTION__, __LINE__);
    }
...
static irods::error get_server_reports(rsComm_t* _comm, json_t*& _resc_arr)
{
    ...
    for (irods::resource_manager::iterator itr = resc_mgr.begin(); itr != resc_mgr.end(); ++itr) {
        ...
        std::map<rodsServerHost_t*, int>::iterator svr_itr = svr_reported.find(tmp_host);
        ...
        if (status < 0) {
            report_server_connect_error(status, tmp_host->hostName->name, resc_name, _resc_arr);
            continue;
        }
        ...
        if (status < 0) {
            freeBBuf(bbuf);
            bbuf = NULL;
            rodsLog(LOG_ERROR, "rcServerReport failed for [%s], status = %d", tmp_host->hostName->name, status);
        }
        ...
        json_error_t j_err;
        json_t* j_resc = json_loads(tmp_str.data(), JSON_REJECT_DUPLICATES, &j_err);
...
static irods::error get_coordinating_resources(rsComm_t* _comm, json_t*& _resources)
{
...
int _rsZoneReport(rsComm_t* _comm, bytesBuf_t** _bbuf)
{
    ...
    freeBBuf(bbuf);
    if (!cat_svr) {
        rodsLog(LOG_ERROR, "_rsZoneReport - json_loads failed [%s]", j_err.text);
    ...
    if (!zone_obj) {
        rodsLog(LOG_ERROR, "failed to allocate json_object");
    ...
    if (!zone_arr) {
        rodsLog(LOG_ERROR, "failed to allocate json_array");
        return SYS_MALLOC_ERR;
    }
