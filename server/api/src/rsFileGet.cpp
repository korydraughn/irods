...
int rsFileGet(rsComm_t* rsComm, fileOpenInp_t* fileGetInp, bytesBuf_t* fileGetOutBBuf)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        status = remoteFileGet(rsComm, fileGetInp, fileGetOutBBuf, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsFileGet: resolveHost returned unrecognized value %d", remoteFlag);
                  ...
                  rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileGet: Invalid rodsServerHost");
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "remoteFileGet: rcFileGet failed for %s", fileGetInp->fileName);
    }
...
int _rsFileGet(rsComm_t* _comm, fileOpenInp_t* _get_inp, bytesBuf_t* _get_buf)
{
    ...
    if (fd < 0) {
        rodsLog(LOG_NOTICE, "_rsFileGet: fileGet for %s, status = %d", _get_inp->fileName, fd);
        return fd;
    }
    ...
    irods::file_object_ptr file_obj(new irods::file_object(
        _comm, _get_inp->objPath, _get_inp->fileName, _get_inp->resc_hier_, fd, _get_inp->mode, _get_inp->flags));

    // =-=-=-=-=-=-=-
    // pass condInput
    file_obj->cond_input(_get_inp->condInput);

    irods::error read_err = fileRead(_comm, file_obj, _get_buf->buf, len);
