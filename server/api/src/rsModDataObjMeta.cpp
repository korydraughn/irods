...
int rsModDataObjMeta(rsComm_t* rsComm, modDataObjMeta_t* modDataObjMetaInp)
{
    ...
    status = getAndConnRcatHost(rsComm, MASTER_RCAT, (const char*) dataObjInfo->objPath, &rodsServerHost);
    ...
    if (irods::CFG_SERVICE_ROLE_PROVIDER == svc_role) {
        ...
        if (status >= 0) {
            ...
            if (i < 0) {
                ...
                rodsLog(LOG_ERROR, "_rsModDataObjMeta:acPostProcForModifyDataObjMeta error stat=%d", i);
                return i;
            }
        }
        else {
...
int _call_file_modified_for_modification(rsComm_t* rsComm, modDataObjMeta_t* modDataObjMetaInp)
{
    ...
    if (getValByKey(regParam, ALL_KW) != NULL) {
        ...
        while (tmpDataObjInfo != NULL) {
            ...
            irods::file_object_ptr file_obj(new irods::file_object(rsComm, tmpDataObjInfo));
