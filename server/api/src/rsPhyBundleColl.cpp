...
int rsPhyBundleColl(rsComm_t* rsComm, structFileExtAndRegInp_t* phyBundleCollInp)
{
    ...
    if (hier_kw == NULL) {
        irods::error ret = irods::resolve_resource_hierarchy(irods::CREATE_OPERATION, rsComm, &data_inp, hier);
        if (!ret.ok()) {
...
int _rsPhyBundleColl(rsComm_t* rsComm, structFileExtAndRegInp_t* phyBundleCollInp, const char* _resc_name)
{
    ...
    if (handleInx < 0) {
        rodsLog(LOG_ERROR, "_rsPhyBundleColl: rsOpenCollection of %s error. status = %d", collInp.collName, handleInx);
        return handleInx;
    }

    if (CollHandle[handleInx].rodsObjStat->specColl != NULL) {
        rodsLog(LOG_ERROR, "_rsPhyBundleColl: unable to bundle special collection %s", collInp.collName);
    ...
    int l1descInx = createPhyBundleDataObj(
        rsComm, phyBundleCollInp->collection, _resc_name, rescHier, &dataObjInp, dataType); // JMC - backport 4658
    ...
    createPhyBundleDir(
        rsComm, L1desc[l1descInx].dataObjInfo->filePath, phyBunDir, L1desc[l1descInx].dataObjInfo->rescHier);
    ...
    while ((status = rsReadCollection(rsComm, &handleInx, &collEnt)) >= 0) {
        if (collEnt->objType == DATA_OBJ_T) {
            if (curSubFileCond.collName[0] == '\0') {
                /* a new dataObj.  */
                rstrcpy(curSubFileCond.collName, collEnt->collName, MAX_NAME_LEN);
                rstrcpy(curSubFileCond.dataName, collEnt->dataName, MAX_NAME_LEN);
                ...
                if (bunReplCacheHeader.numSubFiles >= maxSubFileCnt || // JMC - backport 4771
                    ...
                    /* bundle is full */
                    status = bundleAndRegSubFiles(rsComm,
                                                  l1descInx,
                                                  phyBunDir,
                                                  phyBundleCollInp->collection,
                                                  &bunReplCacheHeader,
                                                  chksumFlag); // JMC - backport 4528
                    if (status < 0) {
                        rodsLog(LOG_ERROR,
                ...
                curSubFileCond.subPhyPath[0] = curSubFileCond.cachePhyPath[0] = '\0';
                rstrcpy(curSubFileCond.collName, collEnt->collName, MAX_NAME_LEN);
                rstrcpy(curSubFileCond.dataName, collEnt->dataName, MAX_NAME_LEN);
                curSubFileCond.dataId = strtoll(collEnt->dataId, 0, 0);
            } /* end of  name compare */
    ...
    if (status < 0) {
        savedStatus = status;
        rodsLog(
            LOG_ERROR, "_rsPhyBundleColl:replAndAddSubFileToDir err for %s,stat=%d", curSubFileCond.subPhyPath, status);
    }
    ...
    if (status < 0) {
        rodsLog(
            LOG_ERROR, "_rsPhyBundleColl:bunAndRegSubFiles err for %s,stat=%d", phyBundleCollInp->collection, status);
    }
...
int replAndAddSubFileToDir(rsComm_t* rsComm,
                           curSubFileCond_t* curSubFileCond,
                           const char* myRescName,
                           char* phyBunDir,
                           bunReplCacheHeader_t* bunReplCacheHeader)
{
    int status;
    ...
    if (curSubFileCond->subPhyPath[0] == '\0') {
        ...
        status = replDataObjForBundle(
            rsComm, curSubFileCond->collName, curSubFileCond->dataName, myRescName, 0, 0, 1, &dataObjInfo);
        if (status >= 0) {
            setSubPhyPath(phyBunDir, curSubFileCond->dataId, curSubFileCond->subPhyPath);
            rstrcpy(curSubFileCond->cachePhyPath, dataObjInfo.filePath, MAX_NAME_LEN);
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "_rsPhyBundleColl:addSubFileToDir error for %s,stst=%d", curSubFileCond->subPhyPath, status);
    }
    return status;
}

int bundleAndRegSubFiles(rsComm_t* rsComm,
                         int l1descInx,
                         char* phyBunDir,
                         char* collection,
                         bunReplCacheHeader_t* bunReplCacheHeader,
...
{ // JMC - backport 4528
    ...
    status =
        phyBundle(rsComm, L1desc[l1descInx].dataObjInfo, phyBunDir, collection, CREATE_TAR_OPR); // JMC - backport 4643
    if (status < 0) {
        rodsLog(LOG_ERROR,
    ...
    while (tmpBunReplCache != NULL) {
        ...
        snprintf(subPhyPath, MAX_NAME_LEN, "%s/%lld", phyBunDir, tmpBunReplCache->dataId);
        // =-=-=-=-=-=-=-
        // JMC - backport 4528
        if (chksumFlag != 0) {
            status = fileChksum(rsComm,
        ...
        if (chksumFlag != 0) {
            ...
            if (status < 0) {
                ...
                rodsLogError(LOG_ERROR,
                             status,
                             "bundleAndRegSubFiles: rsModDataObjMeta error for %s.",
                             tmpBunReplCache->objPath);
            }
        }
        // =-=-=-=-=-=-=-
...
{ // JMC - backport 4643
    ...
    if (dataType != NULL &&                                // JMC - backport 4633
        (strstr(dataType, GZIP_TAR_DT_STR) != NULL ||      // JMC - backport 4658
         strstr(dataType, BZIP2_TAR_DT_STR) != NULL || strstr(dataType, ZIP_DT_STR) != NULL)) {
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "phyBundle: rsStructFileSync of %s error. stat = %d", dataObjInfo->objPath, status);
    }
...
int addSubFileToDir(curSubFileCond_t* curSubFileCond, bunReplCacheHeader_t* bunReplCacheHeader)
{
    ...
    snprintf(bunReplCache->objPath, MAX_NAME_LEN, "%s/%s", curSubFileCond->collName, curSubFileCond->dataName);
...
int replDataObjForBundle(rsComm_t* rsComm,
                         char* collName,
                         char* dataName,
                         const char* rescName,
                         char* rescHier,
                         ...
                         dataObjInfo_t* outCacheObjInfo)
{
    ...
    status = _rsDataObjRepl(rsComm, &dataObjInp, &transStat, outCacheObjInfo);
...
int createPhyBundleDataObj(rsComm_t* rsComm,
                           char* collection,
                           const std::string& _resc_name,
                           const char* rescHier,
                           dataObjInp_t* dataObjInp, // should be able to only use rescHier
                           char* dataType)
{ // JMC - backport 4658
    int l1descInx;
    int status;
    ...
    std::string type;
    irods::error err = irods::get_resource_property<std::string>(resc_id, irods::RESOURCE_TYPE, type);
    if (!err.ok()) {
        irods::log(PASS(err));
    }
    ...
    do {
        ...
        while (1) {
            status = rsMkBundlePath(rsComm, collection, dataObjInp->objPath, irods::getRandom<unsigned int>());
            if (status < 0) {
                rodsLog(LOG_ERROR, "createPhyBundleFile: getPhyBundlePath err for %s.stat = %d", collection, status);
                return status;
            }
...
int rsMkBundlePath(rsComm_t* rsComm, char* collection, char* bundlePath, int myRanNum)
{
    ...
    *bundlePathPtr = '/';
    ...
    if (*tmpStr == '\0') {
        rodsLog(LOG_ERROR, "rsMkBundlePath: input path %s too short", collection);
        return USER_INPUT_PATH_ERR;
    }

    /* cannot bundle trash and bundle */
    if (strncmp(tmpStr, "trash/", 6) == 0 || strncmp(tmpStr, "bundle/", 7) == 0) {
        rodsLog(LOG_ERROR, "rsMkBundlePath: cannot bundle trash or bundle path %s", collection);
        return USER_INPUT_PATH_ERR;
    }
    ...
    *bundlePathPtr = '\0';
    ...
    if ((status = splitPathByKey(bundlePath, destBundleColl, MAX_NAME_LEN, myFile, MAX_NAME_LEN, '/')) < 0) {
        rodsLog(LOG_ERROR, "rsMkBundlePath: splitPathByKey error for %s ", bundlePath);
        return USER_INPUT_PATH_ERR;
    }
...
int remotePhyBundleColl(rsComm_t* rsComm, structFileExtAndRegInp_t* phyBundleCollInp, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remotePhyBundleColl: Invalid rodsServerHost");
