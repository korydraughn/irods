...
int rsFilePut(rsComm_t* rsComm, fileOpenInp_t* filePutInp, bytesBuf_t* filePutInpBBuf, filePutOut_t** _put_out)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        status = _rsFilePut(rsComm, filePutInp, filePutInpBBuf, rodsServerHost, _put_out);
    }
    else if (remoteFlag == REMOTE_HOST) {
        status = remoteFilePut(rsComm, filePutInp, filePutInpBBuf, rodsServerHost, _put_out);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsFilePut: resolveHost returned unrecognized value %d", remoteFlag);
                  ...
                  filePutOut_t** _put_out)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFilePut: Invalid rodsServerHost");
    ...
    status = rcFilePut(rodsServerHost->conn, filePutInp, filePutInpBBuf, _put_out);

    if (status < 0 && status != DIRECT_ARCHIVE_ACCESS) {
        rodsLog(LOG_NOTICE, "remoteFilePut: rcFilePut failed for %s", filePutInp->fileName);
    }
               ...
               filePutOut_t** _put_out)
{
    ...
    if (fd < 0) {
        if (getErrno(fd) == EEXIST) {
            rodsLog(LOG_DEBUG10, "_rsFilePut: filePut for %s, status = %d", _put_inp->fileName, fd);
        }
        else if (fd != DIRECT_ARCHIVE_ACCESS) {
            rodsLog(LOG_DEBUG, "_rsFilePut: filePut for %s, status = %d", _put_inp->fileName, fd);
        }
        return fd;
    }
    ...
    irods::file_object_ptr file_obj(
        new irods::file_object(_comm, _put_inp->objPath, _put_inp->fileName, _put_inp->resc_hier_, fd, 0, 0));
    file_obj->in_pdmo(_put_inp->in_pdmo);
    file_obj->cond_input(_put_inp->condInput);

    irods::error write_err = fileWrite(_comm, file_obj, _put_bbuf->buf, _put_bbuf->len);
    ...
    snprintf((*_put_out)->file_name, sizeof((*_put_out)->file_name), "%s", file_obj->physical_path().c_str());
