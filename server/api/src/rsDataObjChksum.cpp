...
namespace
{
    // assumes zone redirection has already occurred, and that the function is being called on a server in the zone the
    // object belongs to
    int verifyVaultSizeEqualsDatabaseSize(rsComm_t* rsComm, dataObjInfo_t* dataObjInfo)
    {
        ...
        if (status_rsFileStat < 0) {
            rodsLog(LOG_ERROR,
                    "verifyVaultSizeEqualsDatabaseSize: rsFileStat of objPath [%s] rescHier [%s] fileName [%s] failed "
                    "with [%d]",
                    fileStatInp.objPath,
        ...
        addInxVal(
            &genQueryInp.sqlCondInp, COL_D_DATA_PATH, (boost::format(" = '%s'") % dataObjInfo->filePath).str().c_str());
        ...
        if (status_rsGenQuery < 0) {
            ...
            rodsLog(LOG_ERROR,
                    "_rsFileChksum: rsGenQuery of [%s] [%s] [%s] failed with [%d]",
        ...
        try {
            ...
            freeGenQueryOut(&genQueryOut);
            rodsLog(LOG_ERROR,
                    "_rsFileChksum: lexical_cast of [%s] for [%s] [%s] [%s] failed",
                    genQueryOut->sqlResult[0].value,
                    dataObjInfo->filePath,
                    dataObjInfo->rescHier,
                    dataObjInfo->objPath);
        ...
        if (size_in_database != size_in_vault) {
            rodsLog(LOG_ERROR,
                    "_rsFileChksum: file size mismatch. resource hierarchy [%s] vault path [%s] size [%ji] object path "
                    "[%s] size [%ji]",
                    dataObjInfo->rescHier,
                    dataObjInfo->filePath,
                    static_cast<intmax_t>(size_in_vault),
                    dataObjInfo->objPath,
                    static_cast<intmax_t>(size_in_database));
...
int rsDataObjChksum(rsComm_t* rsComm, dataObjInp_t* dataObjChksumInp, char** outChksum)
{
    ...
    if (remoteFlag < 0) {
        ...
        status = rcDataObjChksum(rodsServerHost->conn, dataObjChksumInp, outChksum);
        return status;
    }
    else {
        // =-=-=-=-=-=-=-
        ...
        if (getValByKey(&dataObjChksumInp->condInput, RESC_HIER_STR_KW) == NULL) {
            std::string hier;
            irods::error ret = irods::resolve_resource_hierarchy(irods::OPEN_OPERATION, rsComm, dataObjChksumInp, hier);
            if (!ret.ok()) {
...
int _rsDataObjChksum(rsComm_t* rsComm, dataObjInp_t* dataObjInp, char** outChksumStr, dataObjInfo_t** dataObjInfoHead)
{
    ...
    *outChksumStr = NULL;
    ...
    const bool shouldVerifyVaultSizeEqualsDatabaseSize =
        getValByKey(&dataObjInp->condInput, VERIFY_VAULT_SIZE_EQUALS_DATABASE_SIZE_KW) != NULL;
    ...
    if (!allFlag) {
        ...
        if (verifyFlag && strlen(tmpDataObjInfo->chksum) > 0) {
            status = verifyDatObjChksum(rsComm, tmpDataObjInfo, outChksumStr);
        ...
        do {
            ...
            irods::error err =
                irods::get_resource_property<std::string>(tmpDataObjInfo->rescId, irods::RESOURCE_CLASS, resc_class);
            if (!err.ok()) {
            ...
            if (strlen(tmpDataObjInfo->chksum) == 0) {
                ...
                status = dataObjChksumAndRegInfo(rsComm, tmpDataObjInfo, &tmpChksumStr);
            }
            else if (verifyFlag) {
...
int verifyDatObjChksum(rsComm_t* rsComm, dataObjInfo_t* dataObjInfo, char** outChksumStr)
{
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "verifyDatObjChksum:_dataObjChksum error for %s, stat=%d", dataObjInfo->objPath, status);
        return status;
    }
    if (*outChksumStr == NULL) {
    ...
    if (strcmp(*outChksumStr, dataObjInfo->chksum) != 0) {
        ...
        error_message << "verifyDatObjChksum: computed chksum [" << *outChksumStr << "] != icat value ["
                      << dataObjInfo->chksum << "] for [" << dataObjInfo->objPath << "] hierarchy ["
                      << dataObjInfo->rescHier << "] replNum [" << dataObjInfo->replNum << "]";
