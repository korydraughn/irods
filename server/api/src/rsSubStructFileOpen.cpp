...
int rsSubStructFileOpen(rsComm_t* rsComm, subFile_t* subFile)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsSubStructFileOpen: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteSubStructFileOpen(rsComm_t* rsComm, subFile_t* subFile, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileOpen: Invalid rodsServerHost");
...
int _rsSubStructFileOpen(rsComm_t* _comm, subFile_t* _sub_file)
{
    // =-=-=-=-=-=-=-
