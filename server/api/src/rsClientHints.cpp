...
int rsClientHints(rsComm_t* _comm, bytesBuf_t** _bbuf)
{
    ...
    int status = _rsClientHints(_comm, _bbuf);
    if (status < 0) {
        rodsLog(LOG_ERROR, "rsClientHints: rcClientHints failed, status = %d", status);
    }
...
int _rsClientHints(rsComm_t* _comm, bytesBuf_t** _bbuf)
{
    ...
    json_error_t j_err;
    json_t* client_hints = json_loads((char*) ies_buf->buf, JSON_REJECT_DUPLICATES, &j_err);
    freeBBuf(ies_buf);
    if (!client_hints) {
        rodsLog(LOG_ERROR, "_rsClientHints - json_loads failed [%s]", j_err.text);
    ...
    irods::rule_engine_context_manager<irods::unit, ruleExecInfo_t*, irods::DONT_AUDIT_RULE> re_ctx_mgr(
