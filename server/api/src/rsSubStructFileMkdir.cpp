...
int rsSubStructFileMkdir(rsComm_t* rsComm, subFile_t* subFile)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsSubStructFileMkdir: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteSubStructFileMkdir(rsComm_t* rsComm, subFile_t* subFile, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileMkdir: Invalid rodsServerHost");
...
int _rsSubStructFileMkdir(rsComm_t* _comm, subFile_t* _sub_file)
{
    // =-=-=-=-=-=-=-
