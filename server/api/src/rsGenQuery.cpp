...
namespace
{
    std::string get_column_name(int j)
    {
    ...
    option_element selectInpOptionsMap[] = {{ORDER_BY, "ORDER_BY", "order"},
                                            {ORDER_BY_DESC, "ORDER_BY_DESC", "order_desc"}};

    option_element selectInpFunctionMap[] = {{SELECT_MIN, "SELECT_MIN", "min"},
                                             {SELECT_MAX, "SELECT_MAX", "max"},
                                             {SELECT_SUM, "SELECT_SUM", "sum"},
                                             {SELECT_AVG, "SELECT_AVG", "avg"},
                                             {SELECT_COUNT, "SELECT_COUNT", "count"}};

    std::string format_selected_column(int columnIndex, int columnOption)
    {
    ...
    void insert_genquery_inp_into_stream(const genQueryInp_t* genQueryInp, OutStream& f)
    {
        f << "maxRows: " << genQueryInp->maxRows << " continueInx: " << genQueryInp->continueInx
          << " rowOffset: " << genQueryInp->rowOffset << '\n';
        ...
        for (int i = 0; i < genQueryInp->selectInp.len; ++i) {
            f << "    column: " << genQueryInp->selectInp.inx[i] << " "
              << get_column_name(genQueryInp->selectInp.inx[i]) << '\n';
        ...
        for (int i = 0; i < genQueryInp->sqlCondInp.len; ++i) {
            f << "    column: " << genQueryInp->sqlCondInp.inx[i] << " "
              << get_column_name(genQueryInp->sqlCondInp.inx[i]) << '\n';
...
static irods::error strip_new_query_terms(genQueryInp_t* _inp)
{
    ...
    for (int i = 0; i < tmp.len; ++i) {
        if (tmp.inx[i] == COL_R_RESC_CHILDREN || tmp.inx[i] == COL_R_RESC_CONTEXT || tmp.inx[i] == COL_R_RESC_PARENT ||
            tmp.inx[i] == COL_R_RESC_PARENT_CONTEXT || tmp.inx[i] == COL_D_RESC_HIER) {
            continue;
        }
        else {
...
static irods::error get_resc_id_cond_for_hier_cond(const char* _cond, std::string& _new_cond)
{
     ...
     */
    if (std::string::npos != in_cond.find("=") && std::string::npos != hier.find("%")) {
...
static irods::error strip_resc_hier_name_from_query_inp(genQueryInp_t* _inp, int& _pos)
{
    ...
    for (int i = 0; i < tmpV.len; ++i) {
        if (tmpV.inx[i] == COL_D_RESC_HIER) {
            ...
            irods::error ret = get_resc_id_cond_for_hier_cond(tmpV.value[i], new_cond);
            if (!ret.ok()) {
                irods::log(PASS(ret));
            }
...
static irods::error add_resc_hier_name_to_query_out(genQueryOut_t* _out, int& _pos)
{
    ...
    for (int i = 0; i < _out->rowCnt; ++i) {
        ...
        irods::error ret = irods::lexical_cast<rodsLong_t>(leaf_id_str, leaf_id);
        if (!ret.ok()) {
            irods::log(PASS(ret));
            continue;
        }
    ...
    for (std::size_t i = 0; i < resc_hiers.size(); ++i) {
        snprintf(&sqlResult->value[i * sqlResult->len], sqlResult->len, "%s", resc_hiers[i].c_str());
    }
...
static irods::error proc_query_terms_for_community_server(const std::string& _zone_hint, genQueryInp_t* _inp)
{
    ...
    while (tmp_zone) {
        if (zone_name == tmp_zone->zoneName && tmp_zone->masterServerHost->conn &&
...
int rsGenQuery(rsComm_t* rsComm, genQueryInp_t* genQueryInp, genQueryOut_t** genQueryOut)
{
    ...
    status = getAndConnRcatHost(rsComm, SLAVE_RCAT, zone_hint_str.c_str(), &rodsServerHost);
    ...
    if (rodsServerHost->localFlag == LOCAL_HOST) {
        ...
        if (irods::CFG_SERVICE_ROLE_PROVIDER == svc_role) {
            ...
            rodsLog(LOG_NOTICE, "rsGenQuery error. RCAT is not configured on this host");
    ...
    if (status < 0 && status != CAT_NO_ROWS_FOUND) {
        ...
        rodsLog(LOG_NOTICE, "rsGenQuery: %sGenQuery failed, status = %d", prefix.c_str(), status);
    }
    return status;
}
...
int _rsGenQuery(rsComm_t* rsComm, genQueryInp_t* genQueryInp, genQueryOut_t** genQueryOut)
{
    ...
    *genQueryOut = (genQueryOut_t*) malloc(sizeof(genQueryOut_t));
    ...
    if (PrePostProcForGenQueryFlag == 1) {
        ...
        if (i < 0) {
            rodsLog(LOG_ERROR, "rsGenQuery:acPreProcForGenQuery error,stat=%d", i);
    ...
    if (PrePostProcForGenQueryFlag == 1) {
        ...
        if (i < 0) {
            rodsLog(LOG_ERROR, "rsGenQuery:acPostProcForGenQuery error,stat=%d", i);
            if (i != NO_MICROSERVICE_FOUND_ERR) {
                return i;
            }
        }
    ...
    if (status < 0) {
        if (status != CAT_NO_ROWS_FOUND) {
            rodsLog(LOG_NOTICE, "_rsGenQuery: genQuery status = %d", status);
        }
        return status;
    }
    return status;
