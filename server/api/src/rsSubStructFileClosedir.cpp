...
int rsSubStructFileClosedir(rsComm_t* rsComm, subStructFileFdOprInp_t* subStructFileClosedirInp)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        status = remoteSubStructFileClosedir(rsComm, subStructFileClosedirInp, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsSubStructFileClosedir: resolveHost returned unrecognized value %d", remoteFlag);
                                ...
                                rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileClosedir: Invalid rodsServerHost");
    ...
    if (status < 0) {
        rodsLog(
            LOG_NOTICE, "remoteSubStructFileClosedir: rcFileClosedir failed for fd %d", subStructFileClosedirInp->fd);
    }

    return status;
}

int _rsSubStructFileClosedir(rsComm_t* _comm, subStructFileFdOprInp_t* _close_inp)
{
    // =-=-=-=-=-=-=-
    // create first class structured object
    irods::structured_object_ptr struct_obj(new irods::structured_object(*_close_inp));
