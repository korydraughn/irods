...
int rsGetRescQuota(rsComm_t* rsComm, getRescQuotaInp_t* getRescQuotaInp, rescQuota_t** rescQuota)
{
    ...
    status = getAndConnRcatHost(rsComm, SLAVE_RCAT, (const char*) getRescQuotaInp->zoneHint, &rodsServerHost);
    ...
    if (rodsServerHost->localFlag == LOCAL_HOST) {
        ...
        status = rcGetRescQuota(rodsServerHost->conn, getRescQuotaInp, rescQuota);
    }
...
int _rsGetRescQuota(rsComm_t* rsComm, getRescQuotaInp_t* getRescQuotaInp, rescQuota_t** rescQuota)
{
    ...
    *rescQuota = NULL;
    ...
    status = getQuotaByResc(rsComm, getRescQuotaInp->userName, getRescQuotaInp->rescName, &genQueryOut);
    if (status >= 0) {
...
int queRescQuota(rescQuota_t** rescQuotaHead, genQueryOut_t* genQueryOut)
{
    ...
    char *tmpQuotaLimit, *tmpQuotaOver, *tmpRescName, *tmpQuotaRescId, *tmpQuotaUserId;
    int i;
    rescQuota_t* tmpRescQuota;

    if ((quotaLimit = getSqlResultByInx(genQueryOut, COL_QUOTA_LIMIT)) == NULL) {
        rodsLog(LOG_ERROR, "queRescQuota: getSqlResultByInx for COL_QUOTA_LIMIT failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((quotaOver = getSqlResultByInx(genQueryOut, COL_QUOTA_OVER)) == NULL) {
        rodsLog(LOG_ERROR, "queRescQuota: getSqlResultByInx for COL_QUOTA_OVER failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((rescName = getSqlResultByInx(genQueryOut, COL_R_RESC_NAME)) == NULL) {
        rodsLog(LOG_ERROR, "queRescQuota: getSqlResultByInx for COL_R_RESC_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((quotaRescId = getSqlResultByInx(genQueryOut, COL_QUOTA_RESC_ID)) == NULL) {
        rodsLog(LOG_ERROR, "queRescQuota: getSqlResultByInx for COL_QUOTA_RESC_ID failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((quotaUserId = getSqlResultByInx(genQueryOut, COL_QUOTA_USER_ID)) == NULL) {
        rodsLog(LOG_ERROR, "queRescQuota: getSqlResultByInx for COL_QUOTA_USER_ID failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    ...
    for (i = 0; i < genQueryOut->rowCnt; i++) {
        ...
        fillRescQuotaStruct(tmpRescQuota, tmpQuotaLimit, tmpQuotaOver, tmpRescName, tmpQuotaRescId, tmpQuotaUserId);
...
int setRescQuota(rsComm_t* _comm, const char* _obj_path, const char* _resc_name, rodsLong_t _data_size)
{
    ...
    bzero(&get_resc_quota_inp, sizeof(get_resc_quota_inp));
    rstrcpy(get_resc_quota_inp.zoneHint, _obj_path, MAX_NAME_LEN);
    snprintf(get_resc_quota_inp.userName, NAME_LEN, "%s#%s", _comm->clientUser.userName, _comm->clientUser.rodsZone);

    rescQuota_t* total_quota = NULL;
    status = rsGetRescQuota(_comm, &get_resc_quota_inp, &total_quota);
    if (status >= 0 && total_quota) {
        ...
        rodsLog(LOG_DEBUG, "%s rsGetRescQuota for total quota - status %d", __FUNCTION__, status);
    }

    // if no global enforcement was successful try per resource
    rstrcpy(get_resc_quota_inp.rescName, _resc_name, NAME_LEN);

    rescQuota_t* resc_quota = NULL;
    status = rsGetRescQuota(_comm, &get_resc_quota_inp, &resc_quota);
...
int updatequotaOverrun(const char* _resc_hier, rodsLong_t _data_size, int _flags)
{
    ...
    if ((_flags & RESC_QUOTA) > 0) {
        ...
        ret = irods::get_resource_property<rodsLong_t>(resc_id, irods::RESOURCE_QUOTA_OVERRUN, over_run);
        // property may not be set yet so skip error
        over_run += _data_size;
        ret = irods::set_resource_property<rodsLong_t>(root, irods::RESOURCE_QUOTA_OVERRUN, over_run);
        if (!ret.ok()) {
            irods::log(PASS(ret));
            return ret.code();
        }
...
int chkRescQuotaPolicy(rsComm_t* rsComm)
{
    if (RescQuotaPolicy == RESC_QUOTA_UNINIT) {
        ...
        int status = applyRule("acRescQuotaPolicy", NULL, &rei, NO_SAVE_REI);
        clearKeyVal(rei.condInputData);
        free(rei.condInputData);
        if (status < 0) {
            rodsLog(LOG_ERROR, "queRescQuota: acRescQuotaPolicy error status = %d", status);
