...
int rsDataObjUnlink(rsComm_t* rsComm, dataObjInp_t* dataObjUnlinkInp)
{
    ...
    status = getAndConnRcatHost(rsComm, MASTER_RCAT, (const char*) dataObjUnlinkInp->objPath, &rodsServerHost);
    ...
    if (getValByKey(&dataObjUnlinkInp->condInput, RESC_HIER_STR_KW) == NULL) {
        ...
        irods::error ret = irods::resolve_resource_hierarchy(
            irods::UNLINK_OPERATION, rsComm, dataObjUnlinkInp, hier, &dataObjInfoHead);
        if (!ret.ok()) {
    ...
    if (dataObjUnlinkInp->oprType == UNREG_OPR || getValByKey(&dataObjUnlinkInp->condInput, FORCE_FLAG_KW) != NULL ||
        getValByKey(&dataObjUnlinkInp->condInput, REPL_NUM_KW) != NULL ||
        getValByKey(&dataObjUnlinkInp->condInput, EMPTY_BUNDLE_ONLY_KW) != NULL || dataObjInfoHead->specColl != NULL ||
        rmTrashFlag == 1) {
...
int resolveDataObjReplStatus(rsComm_t* rsComm, dataObjInp_t* dataObjUnlinkInp)
{
    ...
    status = getDataObjInfo(rsComm, dataObjUnlinkInp, &dataObjInfoHead, ACCESS_DELETE_OBJECT, 1);
    ...
    while (tmpDataObjInfo != NULL) {
        if (tmpDataObjInfo->replStatus == 0) {
            ...
            else if (atoi(tmpDataObjInfo->dataModify) > atoi(newestDataObjInfo->dataModify)) {
                newestDataObjInfo = tmpDataObjInfo;
            }
        }
        else {
...
int dataObjUnlinkS(rsComm_t* rsComm, dataObjInp_t* dataObjUnlinkInp, dataObjInfo_t* dataObjInfo)
{
    ...
    if (dataObjInfo->specColl == NULL) {
        if (dataObjUnlinkInp->oprType == UNREG_OPR && rsComm->clientUser.authInfo.authFlag != LOCAL_PRIV_USER_AUTH) {
            ruleExecInfo_t rei;
    ...
    if (dataObjUnlinkInp->oprType != UNREG_OPR) {
        ...
        if (status < 0) {
            ...
            rodsLog(
                LOG_NOTICE, "dataObjUnlinkS: l3Unlink error for %s. status = %d", dataObjUnlinkInp->objPath, status);
            /* allow ENOENT to go on and unregister */
            if (myError != ENOENT && myError != EACCES) {
                char orphanPath[MAX_NAME_LEN];
                int status1 = 0;
                rodsLog(LOG_NOTICE, "dataObjUnlinkS: orphan file %s", dataObjInfo->filePath);
                while (1) {
                    if (isOrphanPath(dataObjUnlinkInp->objPath) == NOT_ORPHAN_PATH) {
                        /* don't rename orphan path */
                        status1 = rsMkOrphanPath(rsComm, dataObjInfo->objPath, orphanPath);
                        if (status1 < 0) {
                            break;
                        }
                        /* reg the orphan path */
                    ...
                    if (status1 == CAT_NAME_EXISTS_AS_DATAOBJ || status1 == CATALOG_ALREADY_HAS_ITEM_BY_THAT_NAME) {
                        continue;
                    }
                    else if (status1 < 0) {
                        rodsLogError(LOG_ERROR,
...
int l3Unlink(rsComm_t* rsComm, dataObjInfo_t* dataObjInfo)
{
    ...
    irods::error prop_err =
        irods::get_resource_property<std::string>(dataObjInfo->rescId, irods::RESOURCE_CLASS, resc_class);
    if (prop_err.ok()) {
...
int rsMvDataObjToTrash(rsComm_t* rsComm, dataObjInp_t* dataObjInp, dataObjInfo_t** dataObjInfoHead)
{
    ...
    if (getValByKey(&dataObjInp->condInput, DATA_ACCESS_KW) == NULL) {
        addKeyVal(&dataObjInp->condInput, DATA_ACCESS_KW, ACCESS_DELETE_OBJECT);
    }

    status = getDataObjInfo(rsComm, dataObjInp, dataObjInfoHead, ACCESS_DELETE_OBJECT, 0);
    if (status < 0) {
        rodsLog(
            LOG_NOTICE, "rsMvDataObjToTrash: getDataObjInfo error for %s. status = %d", dataObjInp->objPath, status);
        return status;
    }
    ...
    while (status == CAT_NAME_EXISTS_AS_DATAOBJ || status == CAT_NAME_EXISTS_AS_COLLECTION ||
           status == SYS_PHY_PATH_INUSE || getErrno(status) == EISDIR) {
