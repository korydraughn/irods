...
int rsDataCopy(rsComm_t* rsComm, dataCopyInp_t* dataCopyInp)
{
    ...
    if (getValByKey(&dataOprInp->condInput, EXEC_LOCALLY_KW) != NULL || dataCopyInp->portalOprOut.numThreads == 0) {
...
int remoteDataCopy(rsComm_t* rsComm, dataCopyInp_t* dataCopyInp, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteDataCopy: Invalid rodsServerHost");
    ...
    dataCopyInp->dataOprInp.destL3descInx = convL3descInx(dataCopyInp->dataOprInp.destL3descInx);

    status = rcDataCopy(rodsServerHost->conn, dataCopyInp);

    if (status < 0) {
        rodsLog(LOG_NOTICE, "remoteDataCopy: rcDataCopy failed");
    }
...
int _rsDataCopy(rsComm_t* rsComm, dataCopyInp_t* dataCopyInp)
{
    ...
    if (dataCopyInp == NULL) {
        rodsLog(LOG_NOTICE, "_rsDataCopy: NULL dataCopyInp input");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (dataOprInp->oprType == SAME_HOST_COPY_OPR) {
    ...
    else if (dataOprInp->oprType == COPY_TO_LOCAL_OPR || dataOprInp->oprType == COPY_TO_REM_OPR) {
        ...
        rodsLog(LOG_NOTICE, "_rsDataCopy: Invalid oprType %d", dataOprInp->oprType);
