...
{
    if (!_comm || !_inp || !_out) {
        addRErrorMsg(&_comm->rError, STDOUT_STATUS, "null input param(s)");
    ...
    std::string hier;
    irods::error ret = resc_mgr.leaf_id_to_hier(_inp->resc_id_, hier);
    if (!ret.ok()) {
        addRErrorMsg(&_comm->rError, STDOUT_STATUS, ret.result().c_str());
        return ret.code();
    }
