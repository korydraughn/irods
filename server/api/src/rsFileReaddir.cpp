...
int rsFileReaddir(rsComm_t* rsComm, fileReaddirInp_t* fileReaddirInp, rodsDirent_t** fileReaddirOut)
{
    ...
    *fileReaddirOut = NULL;
    ...
    remoteFlag = getServerHostByFileInx(fileReaddirInp->fileInx, &rodsServerHost);

    if (remoteFlag == LOCAL_HOST) {
        status = _rsFileReaddir(rsComm, fileReaddirInp, fileReaddirOut);
    }
    else if (remoteFlag == REMOTE_HOST) {
        status = remoteFileReaddir(rsComm, fileReaddirInp, fileReaddirOut, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsFileReaddir: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteFileReaddir(rsComm_t* rsComm,
                      ...
                      rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileReaddir: Invalid rodsServerHost");
...
int _rsFileReaddir(rsComm_t* _comm, fileReaddirInp_t* _file_readdir_inp, rodsDirent_t** _rods_dirent)
{
    ...
    irods::collection_object_ptr coll_obj(new irods::collection_object(
        FileDesc[_file_readdir_inp->fileInx].fileName, FileDesc[_file_readdir_inp->fileInx].rescHier, 0, 0));
    ...
    irods::error readdir_err = fileReaddir(_comm, coll_obj, _rods_dirent);
    if (!readdir_err.ok()) {
