...
int rsFileCreate(rsComm_t* rsComm, fileCreateInp_t* fileCreateInp, fileCreateOut_t** _out)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsFileCreate: resolveHost returned unrecognized value %d", remoteFlag);
    ...
    fileInx = allocAndFillFileDesc(rodsServerHost,
                                   fileCreateInp->objPath,
                                   fileCreateInp->fileName,
                                   fileCreateInp->resc_hier_,
                                   fd,
                                   fileCreateInp->mode);
                     ...
                     fileCreateOut_t** _out)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileCreate: Invalid rodsServerHost");
    ...
    if (fileInx < 0) {
        rodsLog(LOG_NOTICE, "remoteFileCreate: rcFileCreate failed for %s", fileCreateInp->fileName);
    }
...
int _rsFileCreate(rsComm_t* _comm, fileCreateInp_t* _create_inp, rodsServerHost_t* _server_host, fileCreateOut_t** _out)
{
    ...
    if ((_create_inp->otherFlags & NO_CHK_PERM_FLAG) == 0) { // JMC - backport 4758
        int status = chkFilePathPerm(_comm, _create_inp, _server_host,
    ...
    if (!create_err.ok()) {
        ...
        if (getErrno(create_err.code()) == ENOENT) {
            ...
            int status = mkDirForFilePath(
                _comm, 0, file_obj->physical_path().c_str(), file_obj->resc_hier().c_str(), getDefDirMode());
            if (status != 0) {
            ...
            irods::collection_object_ptr coll_obj(
                new irods::collection_object(_create_inp->fileName, _create_inp->resc_hier_, 0, 0));
    ...
    rstrcpy((*_out)->file_name, file_obj->physical_path().c_str(), MAX_NAME_LEN);

    rstrcpy(_create_inp->resc_hier_, file_obj->resc_hier().c_str(), MAX_NAME_LEN);
    rstrcpy(_create_inp->fileName, file_obj->physical_path().c_str(), MAX_NAME_LEN);
