...
int applyRuleForPostProcForWrite(rsComm_t* rsComm, bytesBuf_t* dataObjWriteInpBBuf, char* objPath)
{
    ...
    *myInOutStruct = dataObjWriteInpBBuf->len;
    addMsParamToArray(&msParamArray, "*WriteBuf", BUF_LEN_MS_T, myInOutStruct, dataObjWriteInpBBuf, 0);
    ...
    if (status < 0) {
        ...
        rodsLog(LOG_ERROR, "rsDataObjWrite: acPostProcForDataObjWrite error=%d", status);
...
int rsDataObjWrite(rsComm_t* rsComm, openedDataObjInp_t* dataObjWriteInp, bytesBuf_t* dataObjWriteInpBBuf)
{
    ...
    if (l1descInx < 2 || l1descInx >= NUM_L1_DESC) {
        rodsLog(LOG_NOTICE, "rsDataObjWrite: l1descInx %d out of range", l1descInx);
    ...
    if (L1desc[l1descInx].remoteZoneHost != NULL) {
        ...
        bytesWritten = rcDataObjWrite(L1desc[l1descInx].remoteZoneHost->conn, dataObjWriteInp, dataObjWriteInpBBuf);
        dataObjWriteInp->l1descInx = l1descInx;
    }
    else {
        int i = applyRuleForPostProcForWrite(rsComm, dataObjWriteInpBBuf, L1desc[l1descInx].dataObjInfo->objPath);
        if (i < 0) {
            return i;
        }
        ...
        irods::file_object_ptr file_obj(new irods::file_object(rsComm, L1desc[l1descInx].dataObjInfo));
