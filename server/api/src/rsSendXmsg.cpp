...
int rsSendXmsg(rsComm_t* rsComm, sendXmsgInp_t* sendXmsgInp)
{
    ...
    status = getTicketMsgStructByTicket(sendXmsgInp->ticket.rcvTicket, &ticketMsgStruct);
    ...
    if (ticketMsgStruct->ticket.sendTicket != sendXmsgInp->ticket.sendTicket) {
        ...
        rodsLog(LOG_ERROR,
                ...
                sendXmsgInp->ticket.sendTicket,
                ticketMsgStruct->ticket.sendTicket);
    ...
    if (miscInfo != NULL && strlen(miscInfo) > 0) {
        if (!strcmp(miscInfo, "CLEAR_STREAM")) {
            ...
            if (sendXmsgInp->ticket.rcvTicket > 5) {
                ...
                i = rmTicketMsgStructFromHQue(ticketMsgStruct, (ticketHashQue_t*) ticketMsgStruct->ticketHashQue);
                return i;
            }
        }
