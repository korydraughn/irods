...
json_t* make_federation_set(const std::vector<std::string>& _feds)
{
    if (_feds.empty()) {
        return nullptr;
    }
...
irods::error sanitize_server_config_keys(json_t* _svr_cfg)
{
    if (!_svr_cfg) {
    ...
    json_object_set_new(_svr_cfg, "zone_key", json_string("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"));

    json_object_set_new(_svr_cfg, "negotiation_key", json_string("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"));

    json_object_set_new(_svr_cfg, "server_control_plane_key", json_string("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"));

    // get the federation object
    json_t* fed_obj = json_object_get(_svr_cfg, "federation");
    if (!fed_obj) {
        return SUCCESS();
    }
    ...
    {
        json_object_set_new(obj, "negotiation_key", json_string("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"));
        json_object_set_new(obj, "zone_key", json_string("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"));
    }
...
irods::error convert_server_config(json_t*& _svr_cfg)
{
    // =-=-=-=-=-=-=-
    ...
    json_error_t error;
    _svr_cfg = json_load_file(svr_cfg.c_str(), 0, &error);
    if (!_svr_cfg) {
...
irods::error convert_host_access_control(json_t*& _host_ctrl)
{
    std::string cfg_file;
    ...
    json_error_t error;
    _host_ctrl = json_load_file(cfg_file.c_str(), 0, &error);
    if (!_host_ctrl) {
        std::string msg("failed to load file [");
        msg += cfg_file;
        msg += "] json error [";
...
irods::error convert_irods_host(json_t*& _irods_host)
{
    std::string cfg_file;
    ...
    json_error_t error;
    _irods_host = json_load_file(cfg_file.c_str(), 0, &error);
    if (!_irods_host) {
        std::string msg("failed to load file [");
        msg += cfg_file;
        msg += "] json error [";
...
irods::error convert_service_account(json_t*& _svc_acct)
{
    ...
    if (!_svc_acct) {
        ...
        // sanitize the keys
        json_object_set_new(
            _svc_acct, "irods_server_control_plane_key", json_string("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"));
        return SUCCESS();
    }
...
irods::error get_host_system_information(json_t*& _host_system_information)
{
    ...
    if (ret.ok()) {
        json_object_set_new(
            _host_system_information, "service_account_user_name", json_string(user_name_string.c_str()));
    }
    else {
        irods::log(ret);
    ...
    if (ret.ok()) {
        json_object_set_new(
            _host_system_information, "service_account_group_name", json_string(group_name_string.c_str()));
    }
    else {
        irods::log(ret);
    ...
    if (ret.ok()) {
        json_object_set_new(
            _host_system_information, "os_distribution_name", json_string(os_distribution_name.c_str()));
    }
    else {
        irods::log(PASS(ret));
    ...
    if (ret.ok()) {
        json_object_set_new(
            _host_system_information, "os_distribution_version", json_string(os_distribution_version.c_str()));
    }
    else {
        irods::log(PASS(ret));
...
irods::error get_resource_array(json_t*& _resources)
{
    _resources = json_array();
    if (!_resources) {
        return ERROR(SYS_MALLOC_ERR, "json_array() failed");
    }
    ...
    for (irods::resource_manager::iterator itr = resc_mgr.begin(); itr != resc_mgr.end(); ++itr) {
        ...
        if (host_name != irods::EMPTY_RESC_HOST && std::string::npos == host_name.find(local_host_name) &&
        ...
        if (!entry) {
            return ERROR(SYS_MALLOC_ERR, "failed to alloc entry");
        }
...
irods::error get_file_contents(const std::string _fn, std::string& _cont)
{
    ...
    typedef bitr::base64_from_binary< // convert binary values to base64 characters
    ...
    std::copy(base64_text(in_s.c_str()), base64_text(in_s.c_str() + in_s.size()), bitr::ostream_iterator<char>(o_str));
...
irods::error get_config_dir(json_t*& _cfg_dir)
{
    namespace fs = boost::filesystem;
    ...
    for (fs::directory_iterator itr(config_dir); itr != fs::directory_iterator(); ++itr) {
        if (fs::is_regular_file(itr->path())) {
            ...
            json_object_set_new(files, name.c_str(), json_string(contents.c_str()));
        }
...
irods::error load_version_file(json_t*& _version)
{
    // =-=-=-=-=-=-=-
    // if json file exists, simply load that
    ...
    if (fs::exists(version_file)) {
        json_error_t error;
        _version = json_load_file(version_file.string().c_str(), 0, &error);
        if (!_version) {
            std::string msg("failed to load file [");
...
int _rsServerReport(rsComm_t* _comm, bytesBuf_t** _bbuf)
{
    ...
    if (!resc_svr) {
        rodsLog(LOG_ERROR, "_rsServerReport: failed to allocate resc_svr");
        return SYS_MALLOC_ERR;
    }
