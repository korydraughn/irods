...
int rsObjStat(rsComm_t* rsComm, dataObjInp_t* dataObjInp, rodsObjStat_t** rodsObjStatOut)
{
    ...
    linkCnt = resolveLinkedPath(rsComm, dataObjInp->objPath, &specCollCache, NULL);

    *rodsObjStatOut = NULL;
    status = getAndConnRcatHost(rsComm, SLAVE_RCAT, (const char*) dataObjInp->objPath, &rodsServerHost);
...
int _rsObjStat(rsComm_t* rsComm, dataObjInp_t* dataObjInp, rodsObjStat_t** rodsObjStatOut)
{
    ...
    if ((tmpStr = getValByKey(&dataObjInp->condInput, SEL_OBJ_TYPE_KW)) == NULL || strcmp(tmpStr, "dataObj") == 0) {
    ...
    if (tmpStr == NULL || strcmp(tmpStr, "collection") == 0) {
        ...
        if (status >= 0) {
            if ((*rodsObjStatOut)->specColl == NULL) {
                if (getSpecCollCache(rsComm, dataObjInp->objPath, 0, &specCollCache) >= 0) {
                    replSpecColl(&specCollCache->specColl, &(*rodsObjStatOut)->specColl);
                }
            }
            return status;
...
int dataObjStat(rsComm_t* rsComm, dataObjInp_t* dataObjInp, rodsObjStat_t** rodsObjStatOut)
{
    ...
    if (status >= 0) {
        if ((dataSize = getSqlResultByInx(genQueryOut, COL_DATA_SIZE)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat: getSqlResultByInx for COL_DATA_SIZE failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else if ((dataMode = getSqlResultByInx(genQueryOut, COL_DATA_MODE)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat: getSqlResultByInx for COL_DATA_MODE failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else if ((replStatus = getSqlResultByInx(genQueryOut, COL_D_REPL_STATUS)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat: getSqlResultByInx for COL_D_REPL_STATUS failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else if ((dataId = getSqlResultByInx(genQueryOut, COL_D_DATA_ID)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat: getSqlResultByInx for COL_D_DATA_ID failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else if ((chksum = getSqlResultByInx(genQueryOut, COL_D_DATA_CHECKSUM)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat:getSqlResultByInx for COL_D_DATA_CHECKSUM failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else if ((ownerName = getSqlResultByInx(genQueryOut, COL_D_OWNER_NAME)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat:getSqlResultByInx for COL_D_OWNER_NAME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else if ((ownerZone = getSqlResultByInx(genQueryOut, COL_D_OWNER_ZONE)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat:getSqlResultByInx for COL_D_OWNER_ZONE failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else if ((createTime = getSqlResultByInx(genQueryOut, COL_D_CREATE_TIME)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat:getSqlResultByInx for COL_D_CREATE_TIME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else if ((modifyTime = getSqlResultByInx(genQueryOut, COL_D_MODIFY_TIME)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat:getSqlResultByInx for COL_D_MODIFY_TIME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else if ((rescHier = getSqlResultByInx(genQueryOut, COL_D_RESC_HIER)) == NULL) {
            rodsLog(LOG_ERROR, "_rsObjStat:getSqlResultByInx for COL_D_RESC_HIER failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else {
            int i;
            ...
            *rodsObjStatOut = (rodsObjStat_t*) malloc(sizeof(rodsObjStat_t));
            ...
            for (i = 0; i < genQueryOut->rowCnt; i++) {
                if (atoi(&replStatus->value[replStatus->len * i]) > 0) {
                    rstrcpy((*rodsObjStatOut)->dataId, &dataId->value[dataId->len * i], NAME_LEN);
                    (*rodsObjStatOut)->objSize = strtoll(&dataSize->value[dataSize->len * i], 0, 0);
                    (*rodsObjStatOut)->dataMode = atoi(&dataMode->value[dataMode->len * i]);
                    rstrcpy((*rodsObjStatOut)->chksum, &chksum->value[chksum->len * i], NAME_LEN);
                    rstrcpy((*rodsObjStatOut)->ownerName, &ownerName->value[ownerName->len * i], NAME_LEN);
                    rstrcpy((*rodsObjStatOut)->ownerZone, &ownerZone->value[ownerZone->len * i], NAME_LEN);
                    rstrcpy((*rodsObjStatOut)->createTime, &createTime->value[createTime->len * i], TIME_LEN);
                    rstrcpy((*rodsObjStatOut)->modifyTime, &modifyTime->value[modifyTime->len * i], TIME_LEN);
                    rstrcpy((*rodsObjStatOut)->rescHier, &rescHier->value[rescHier->len * i], MAX_NAME_LEN);
                    break;
                }
            }
            ...
            if (strlen((*rodsObjStatOut)->dataId) == 0) {
                ...
                rstrcpy((*rodsObjStatOut)->ownerName, ownerName->value, NAME_LEN);
                rstrcpy((*rodsObjStatOut)->ownerZone, ownerZone->value, NAME_LEN);
                rstrcpy((*rodsObjStatOut)->createTime, createTime->value, TIME_LEN);
                rstrcpy((*rodsObjStatOut)->modifyTime, modifyTime->value, TIME_LEN);
                rstrcpy((*rodsObjStatOut)->rescHier, rescHier->value, MAX_NAME_LEN);
            }
        }
    }
