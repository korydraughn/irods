...
int rsFileOpenByHost(rsComm_t* rsComm, fileOpenInp_t* fileOpenInp, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "rsFileOpenByHost: Input NULL rodsServerHost");
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsFileOpenByHost: resolveHost returned unrecognized value %d", remoteFlag);
    ...
    fileInx = allocAndFillFileDesc(
        rodsServerHost, fileOpenInp->objPath, fileOpenInp->fileName, fileOpenInp->resc_hier_, fd, fileOpenInp->mode);
...
int remoteFileOpen(rsComm_t* rsComm, fileOpenInp_t* fileOpenInp, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileOpen: Invalid rodsServerHost");
    ...
    if (fileInx < 0) {
        rodsLog(LOG_NOTICE, "remoteFileOpen: rcFileOpen failed for %s", fileOpenInp->fileName);
    }
...
int _rsFileOpen(rsComm_t* _comm, fileOpenInp_t* _open_inp)
{
    ...
    irods::file_object_ptr file_obj(new irods::file_object(
        _comm, _open_inp->objPath, _open_inp->fileName, _open_inp->resc_hier_, 0, _open_inp->mode, _open_inp->flags));
