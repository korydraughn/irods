    ...
    *fileStatOut = NULL;
    ...
    if (0 == fileStatInp->rescId) {
        if (0 == strlen(fileStatInp->rescHier)) {
            rodsLog(LOG_ERROR, "rsFileStat - rescId and rescHier are invalid");
            return SYS_INVALID_INPUT_PARAM;
        }
        irods::error ret = resc_mgr.hier_to_leaf_id(fileStatInp->rescHier, fileStatInp->rescId);
        if (!ret.ok()) {
            irods::log(PASS(ret));
            return ret.code();
        }
    ...
    ret = resc->get_property<rodsServerHost_t*>(irods::RESOURCE_HOST, rodsServerHost);
    if (!ret.ok()) {
        irods::log(PASS(ret));
        return ret.code();
    }
    ...
    if (rodsServerHost->localFlag < 0) {
        ...
        status = rsFileStatByHost(rsComm, fileStatInp, fileStatOut, rodsServerHost);
        return status;
    }
}
                     ...
                     rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "rsFileStatByHost: Input NULL rodsServerHost");
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        status = remoteFileStat(rsComm, fileStatInp, fileStatOut, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsFileStat: resolveHost returned unrecognized value %d", remoteFlag);
                   ...
                   rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileStat: Invalid rodsServerHost");
    ...
    if (status < 0) {
        rodsLog(LOG_DEBUG, "remoteFileStat: rcFileStat failed for %s", fileStatInp->fileName);
    }
...
int _rsFileStat(rsComm_t* _comm, fileStatInp_t* _stat_inp, rodsStat_t** _stat_out)
{
    ...
    irods::file_object_ptr file_obj(
        new irods::file_object(_comm, _stat_inp->objPath, _stat_inp->fileName, _stat_inp->rescId, 0, 0, 0));
