...
int rsFileStageToCache(rsComm_t* rsComm, fileStageSyncInp_t* fileStageToCacheInp)
{
    ...
    if (remoteFlag < 0) {
        ...
        status = rsFileStageToCacheByHost(rsComm, fileStageToCacheInp, rodsServerHost);
        return status;
    }
}

int rsFileStageToCacheByHost(rsComm_t* rsComm,
                             ...
                             rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "rsFileStageToCacheByHost: Input NULL rodsServerHost");
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        status = remoteFileStageToCache(rsComm, fileStageToCacheInp, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsFileStageToCacheByHost: resolveHost returned value %d", remoteFlag);
...
int remoteFileStageToCache(rsComm_t* rsComm, fileStageSyncInp_t* fileStageToCacheInp, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileStageToCache: Invalid rodsServerHost");
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "remoteFileStageToCache: rcFileStageToCache failed for %s", fileStageToCacheInp->filename);
    }
...
int _rsFileStageToCache(rsComm_t* _comm, fileStageSyncInp_t* _stage_inp)
{
    ...
    // parallel I/O
    int status = mkDirForFilePath(_comm, 0, _stage_inp->cacheFilename, _stage_inp->rescHier, getDefDirMode());
    if (status < 0) {
    ...
    irods::file_object_ptr file_obj(new irods::file_object(
        _comm, _stage_inp->objPath, _stage_inp->filename, "", 0, _stage_inp->mode, _stage_inp->flags));
    ...
    irods::error stage_err = fileStageToCache(_comm, file_obj, _stage_inp->cacheFilename);
    // =-=-=-=-=-=-=-
    ...
    if (!stage_err.ok()) {
        ...
        if (getErrno(stage_err.code()) == EEXIST) {
            ...
            irods::collection_object_ptr coll_obj(
                new irods::collection_object(_stage_inp->cacheFilename, _stage_inp->rescHier, 0, 0));
        ...
        if (stage_err.code() != SYS_OPERATION_IN_PROGRESS) {
            stage_err = fileStageToCache(_comm, file_obj, _stage_inp->cacheFilename);
        }
