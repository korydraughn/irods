...
int rsGetTempPassword(rsComm_t* rsComm, getTempPasswordOut_t** getTempPasswordOut)
{
    ...
    if (rodsServerHost->localFlag == LOCAL_HOST) {
        ...
        status = rcGetTempPassword(rodsServerHost->conn, getTempPasswordOut);
    }

    if (status < 0) {
        rodsLog(LOG_NOTICE, "rsGetTempPassword: rcGetTempPassword failed, status = %d", status);
    }
    return status;
}
...
int _rsGetTempPassword(rsComm_t* rsComm, getTempPasswordOut_t** getTempPasswordOut)
{
    ...
    status = chlMakeTempPw(rsComm, myGetTempPasswordOut->stringToHashWith, "");
    if (status < 0) {
        rodsLog(LOG_NOTICE, "_rsGetTempPassword: getTempPassword, status = %d", status);
    }
