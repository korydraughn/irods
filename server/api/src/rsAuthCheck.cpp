    ...
    if (irods::CFG_SERVICE_ROLE_PROVIDER == svc_role) {
        ...
        *authCheckOut = (authCheckOut_t*) malloc(sizeof(authCheckOut_t));
            ...
            if (kvp.end() != kvp.find(irods::AUTH_SCHEME_KEY)) {
                ...
                if (response_key_pos != std::string::npos) {
                    char* response_ptr =
                        ...
                        (response_key_pos + irods::AUTH_RESPONSE_KEY.length() + irods::KVP_DEF_ASSOCIATION.length());
        ...
        status = chlCheckAuth(rsComm,
                              ...
                              &clientPrivLevel);
        if (status < 0) {
            rodsLog(LOG_NOTICE, "rsAuthCheck: chlCheckAuth status = %d", status);
        }
        ...
        if (status == 0) {
            ...
            if (len <= 0) {
                rodsLog(LOG_DEBUG,
                        "rsAuthCheck: Warning, cannot authenticate this server to remote server, no LocalZoneSID "
        ...
        status = getAndConnRcatHostNoLogin(rsComm, MASTER_RCAT, rsComm->proxyUser.rodsZone, &rodsServerHost);

        if (status < 0) {
            rodsLog(LOG_NOTICE, "rsAuthCheck:getAndConnRcatHostNoLogin() failed. erro=%d", status);
            return status;
        }
