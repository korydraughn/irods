...
int rsCollRepl(rsComm_t* rsComm, collInp_t* collReplInp, collOprStat_t** collOprStat)
{
    ...
    if (remoteFlag < 0) {
        ...
        status = svrSendZoneCollOprStat(rsComm, rodsServerHost->conn, *collOprStat, retval);
        return status;
    }
    ...
    if (handleInx < 0) {
        rodsLog(LOG_ERROR, "rsCollRepl: rsOpenCollection of %s error. status = %d", collReplInp->collName, handleInx);
        return handleInx;
    }
    ...
    if (CollHandle[handleInx].rodsObjStat->specColl != NULL) {
        rodsLog(LOG_ERROR, "rsCollRepl: unable to replicate mounted collection %s", collReplInp->collName);
    ...
    while ((status = rsReadCollection(rsComm, &handleInx, &collEnt)) >= 0) {
        if (collEnt->objType == DATA_OBJ_T) {
            if (totalFileCnt == 0)
                totalFileCnt = CollHandle[handleInx].dataObjSqlResult.totalRowCount;
            ...
            status = _rsDataObjRepl(rsComm, &dataObjInp, &myTransStat, NULL);
            ...
            if (collOprStat != NULL && (*collOprStat)->filesCnt >= fileCntPerStatOut) {
