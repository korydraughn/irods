...
int rsDataObjLseek(rsComm_t* rsComm, openedDataObjInp_t* dataObjLseekInp, fileLseekOut_t** dataObjLseekOut)
{
    ...
    if (L1desc[l1descInx].remoteZoneHost != NULL) {
        ...
        status = rcDataObjLseek(L1desc[l1descInx].remoteZoneHost->conn, dataObjLseekInp, dataObjLseekOut);
    ...
    if (l3descInx <= 2) {
        rodsLog(LOG_NOTICE, "rsDataObjLseek: l3descInx %d out of range", l3descInx);
        return SYS_FILE_DESC_OUT_OF_RANGE;
    }
    ...
    if (getStructFileType(dataObjInfo->specColl) >= 0) {
        ...
        *dataObjLseekOut = (fileLseekOut_t*) malloc(sizeof(fileLseekOut_t));
        ...
        (*dataObjLseekOut)->offset = _l3Lseek(rsComm, l3descInx, dataObjLseekInp->offset, dataObjLseekInp->whence);
