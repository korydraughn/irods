...
int rsSyncMountedColl(rsComm_t* rsComm, dataObjInp_t* syncMountedCollInp)
{
    ...
    if (rodsObjStatOut->specColl == NULL) {
        freeRodsObjStat(rodsObjStatOut);
        rodsLog(LOG_ERROR, "rsSyncMountedColl: %s not a mounted collection", syncMountedCollInp->objPath);
        return SYS_COLL_NOT_MOUNTED_ERR;
    }

    bzero(&myDataObjInp, sizeof(myDataObjInp));
    rstrcpy(myDataObjInp.objPath, rodsObjStatOut->specColl->objPath, MAX_NAME_LEN);
    remoteFlag = getAndConnRemoteZone(rsComm, &myDataObjInp, &rodsServerHost, REMOTE_OPEN);
    ...
    if (remoteFlag < 0) {
        ...
        status = rcSyncMountedColl(rodsServerHost->conn, syncMountedCollInp);
    }
    else {
        status = _rsSyncMountedColl(rsComm, rodsObjStatOut->specColl, syncMountedCollInp->oprType);
    }
