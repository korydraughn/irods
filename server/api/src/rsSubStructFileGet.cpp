...
int rsSubStructFileGet(rsComm_t* rsComm, subFile_t* subFile, bytesBuf_t* subFileGetOutBBuf)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        status = remoteSubStructFileGet(rsComm, subFile, subFileGetOutBBuf, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsSubStructFileGet: resolveHost returned unrecognized value %d", remoteFlag);
                           ...
                           rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileGet: Invalid rodsServerHost");
    ...
    status = rcSubStructFileGet(rodsServerHost->conn, subFile, subFileGetOutBBuf);
