...
int rsDataObjRename(rsComm_t* rsComm, dataObjCopyInp_t* dataObjRenameInp)
{
    ...
    status = getAndConnRcatHost(rsComm, MASTER_RCAT, (const char*) srcDataObjInp->objPath, &rodsServerHost);
    ...
    srcType = resolvePathInSpecColl(rsComm, srcDataObjInp->objPath, WRITE_COLL_PERM, 0, &srcDataObjInfo);

    destType = resolvePathInSpecColl(rsComm, destDataObjInp->objPath, WRITE_COLL_PERM, 0, &destDataObjInfo);

    if (srcDataObjInfo != NULL && srcDataObjInfo->specColl != NULL &&
        strcmp(srcDataObjInfo->specColl->collection, srcDataObjInp->objPath) == 0) {
    ...
    if (destType >= 0) {
        rodsLog(LOG_ERROR, "rsDataObjRename: dest specColl objPath %s exists", destDataObjInp->objPath);
        freeDataObjInfo(srcDataObjInfo);
    ...
    status =
        getAndConnRcatHost(rsComm, MASTER_RCAT, (const char*) dataObjRenameInp->srcDataObjInp.objPath, &rodsServerHost);
    if (status < 0) {
        return status;
    }
...
int _rsDataObjRename(rsComm_t* rsComm, dataObjCopyInp_t* dataObjRenameInp)
{
    ...
    if (irods::CFG_SERVICE_ROLE_PROVIDER == svc_role) {
        ...
        if ((status = splitPathByKey(srcDataObjInp->objPath, srcColl, MAX_NAME_LEN, srcObj, MAX_NAME_LEN, '/')) < 0) {
            rodsLog(LOG_ERROR,
        ...
        if ((status = splitPathByKey(destDataObjInp->objPath, destColl, MAX_NAME_LEN, destObj, MAX_NAME_LEN, '/')) <
            0) {
            rodsLog(LOG_ERROR,
                    "_rsDataObjRename: splitPathByKey for %s error, status = %d",
                    destDataObjInp->objPath,
        ...
        if (srcDataObjInp->oprType == RENAME_DATA_OBJ) {
            if (strstr(dataObjInfoHead->dataType, BUNDLE_STR) != NULL) { // JMC - backport 4658
                rodsLog(LOG_ERROR, "_rsDataObjRename: cannot rename tar bundle type obj %s", srcDataObjInp->objPath);
        ...
        if (strcmp(srcColl, destColl) != 0) {
            ...
            if (status < 0) {
                rodsLog(LOG_ERROR, "_rsDataObjRename: dest coll %s does not exist, status = %d", destColl, status);
                return status;
            }
        ...
        if (status >= 0) {
            ...
            if (status >= 0) {
                ...
                args[1] = destDataObjInp->objPath;
                argc = 2;
                status = applyRuleArg("acPostProcForObjRename", args, argc, &rei2, NO_SAVE_REI);
                if (status < 0) {
                    if (rei2.status < 0) {
                        status = rei2.status;
                    }
...
int l3Rename(rsComm_t* rsComm, dataObjInfo_t* dataObjInfo, char* newFileName)
{
    ...
    if (getStructFileType(dataObjInfo->specColl) >= 0) {
        ...
        rstrcpy(subStructFileRenameInp.subFile.subFilePath, dataObjInfo->subPath, MAX_NAME_LEN);
...
int moveMountedCollObj(rsComm_t* rsComm, dataObjInfo_t* srcDataObjInfo, int srcType, dataObjInp_t* destDataObjInp)
{
    ...
    if (srcType == DATA_OBJ_T) {
        status = moveMountedCollDataObj(rsComm, srcDataObjInfo, destDataObjInp);
    }
    else if (srcType == COLL_OBJ_T) {
        status = moveMountedCollCollObj(rsComm, srcDataObjInfo, destDataObjInp);
    }
    else {
...
int moveMountedCollDataObj(rsComm_t* rsComm, dataObjInfo_t* srcDataObjInfo, dataObjInp_t* destDataObjInp)
{
    ...
    if (status == LOCAL_DIR_T) {
        ...
        rodsLog(
            LOG_ERROR, "moveMountedCollDataObj: targ path %s is a dir. status = %d", destDataObjInfo.filePath, status);
        return status;
    }
    else if (status == LOCAL_FILE_T) {
        dataObjInfo_t myDataObjInfo;
        status = chkOrphanFile(rsComm, destDataObjInfo.filePath, destDataObjInfo.rescName, &myDataObjInfo);
        if (status == 1) {
            /* orphan */
            ...
            if (error_code < 0) {
                rodsLog(LOG_ERROR,
                        "renameFilePathToNewDir failed in moveMountedCollDataObj with error code %d",
                        error_code);
            }
...
int moveMountedCollCollObj(rsComm_t* rsComm, dataObjInfo_t* srcDataObjInfo, dataObjInp_t* destDataObjInp)
{
    ...
    while (rsFileReaddir(rsComm, &fileReaddirInp, &rodsDirent) >= 0) {
        ...
        snprintf(subSrcDataObjInfo.objPath, MAX_NAME_LEN, "%s/%s", srcDataObjInfo->objPath, myRodsDirent.d_name);
        snprintf(subSrcDataObjInfo.subPath, MAX_NAME_LEN, "%s/%s", srcDataObjInfo->subPath, myRodsDirent.d_name);
        snprintf(subSrcDataObjInfo.filePath, MAX_NAME_LEN, "%s/%s", srcDataObjInfo->filePath, myRodsDirent.d_name);
        ...
        if (status < 0 || fileStatOut == NULL) { // JMC cppcheck
            rodsLog(LOG_ERROR,
                    ...
                    subSrcDataObjInfo.filePath,
                    status);
            return status;
        }
        snprintf(subSrcDataObjInfo.dataCreate, TIME_LEN, "%d", fileStatOut->st_ctim);
        snprintf(subSrcDataObjInfo.dataModify, TIME_LEN, "%d", fileStatOut->st_mtim);
        snprintf(subDestDataObjInp.objPath, MAX_NAME_LEN, "%s/%s", destDataObjInp->objPath, myRodsDirent.d_name);
        if ((fileStatOut->st_mode & S_IFREG) != 0) { /* a file */
            subSrcDataObjInfo.dataSize = fileStatOut->st_size;
            status = moveMountedCollDataObj(rsComm, &subSrcDataObjInfo, &subDestDataObjInp);
        }
        else {
            status = moveMountedCollCollObj(rsComm, &subSrcDataObjInfo, &subDestDataObjInp);
        }
        if (status < 0) {
            savedStatus = status;
            rodsLog(LOG_ERROR,
                    "moveMountedCollCollObj: moveMountedColl for %s error, stat = %d",
                    subSrcDataObjInfo.objPath,
                    status);
        }
        free(fileStatOut);
        fileStatOut = NULL;
