...
int rsGeneralAdmin(rsComm_t* rsComm, generalAdminInp_t* generalAdminInp)
{
    ...
    if (rodsServerHost->localFlag == LOCAL_HOST) {
        ...
        status = rcGeneralAdmin(rodsServerHost->conn, generalAdminInp);
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "rsGeneralAdmin: rcGeneralAdmin error %d", status);
    }
    return status;
}
...
int _addChildToResource(generalAdminInp_t* _generalAdminInp, rsComm_t* _rsComm)
{
    ...
    if (rescContext.find(';') != std::string::npos) {
        rodsLog(LOG_ERROR,
                "_addChildToResource: semicolon ';' not allowed in child context string [%s]",
                rescContext.c_str());
        return SYS_INVALID_INPUT_PARAM;
    }
    ...
    if (rescContext.find('{') != std::string::npos) {
        rodsLog(LOG_ERROR,
                "_addChildToResource: open curly bracket '{' not allowed in child context string [%s]",
                rescContext.c_str());
        return SYS_INVALID_INPUT_PARAM;
    }
    ...
    if (rescContext.find('}') != std::string::npos) {
        rodsLog(LOG_ERROR,
                "_addChildToResource: close curly bracket '}' not allowed in child context string [%s]",
                rescContext.c_str());
        return SYS_INVALID_INPUT_PARAM;
    }
    ...
    rodsLog(LOG_NOTICE,
            "rsGeneralAdmin add child \"%s\" to resource \"%s\"",
            rescChildren.c_str(),
...
int _removeChildFromResource(generalAdminInp_t* _generalAdminInp, rsComm_t* _rsComm)
{
    ...
    rodsLog(LOG_NOTICE,
            "rsGeneralAdmin remove child \"%s\" from resource \"%s\"",
...
int _rsGeneralAdmin(rsComm_t* rsComm, generalAdminInp_t* generalAdminInp)
{
    ...
    if (strcmp(generalAdminInp->arg0, "add") == 0) {
        ...
        if (strcmp(generalAdminInp->arg1, "zone") == 0) {
            status = chlRegZone(
                rsComm, generalAdminInp->arg2, generalAdminInp->arg3, generalAdminInp->arg4, generalAdminInp->arg5);
            if (status == 0) {
        ...
        if (strcmp(generalAdminInp->arg1, "specificQuery") == 0) {
            status = chlAddSpecificQuery(rsComm, generalAdminInp->arg2, generalAdminInp->arg3);
            return status;
        }
    ...
    if (strcmp(generalAdminInp->arg0, "modify") == 0) {
        if (strcmp(generalAdminInp->arg1, "user") == 0) {
            ...
            status = chlModUser(rsComm, generalAdminInp->arg2, generalAdminInp->arg3, generalAdminInp->arg4);
        ...
        if (strcmp(generalAdminInp->arg1, "zone") == 0) {
            status = chlModZone(rsComm, generalAdminInp->arg2, generalAdminInp->arg3, generalAdminInp->arg4);
            if (status != 0) {
                chlRollback(rsComm);
            }
            if (status == 0 && strcmp(generalAdminInp->arg3, "name") == 0) {
        ...
        if (strcmp(generalAdminInp->arg1, "zonecollacl") == 0) {
            status = chlModZoneCollAcl(rsComm, generalAdminInp->arg2, generalAdminInp->arg3, generalAdminInp->arg4);
            return status;
        }
        if (strcmp(generalAdminInp->arg1, "localzonename") == 0) {
            ...
            args[0] = generalAdminInp->arg2;
            args[1] = generalAdminInp->arg3;
            status = applyRuleArg("acRenameLocalZone", args, 2, &rei, NO_SAVE_REI);
            return status;
        }
        if (strcmp(generalAdminInp->arg1, "resourcedatapaths") == 0) {
            status = chlModRescDataPaths(
                rsComm, generalAdminInp->arg2, generalAdminInp->arg3, generalAdminInp->arg4, generalAdminInp->arg5);
    ...
    if (strcmp(generalAdminInp->arg0, "rm") == 0) {
        ...
        if (strcmp(generalAdminInp->arg1, "resource") == 0) {
            ...
            if (i < 0) {
                ...
                rodsLog(
                    LOG_ERROR, "rsGeneralAdmin:acPreProcForDeleteResource error for %s,stat=%d", resc_name.c_str(), i);
                return i;
            }
        ...
        if (strcmp(generalAdminInp->arg1, "token") == 0) {
            ...
            if (i < 0) {
                ...
                rodsLog(
                    LOG_ERROR, "rsGeneralAdmin:acPreProcForDeleteToken error for %s.%s,stat=%d", args[0], args[1], i);
                return i;
            }

            status = chlDelToken(rsComm, generalAdminInp->arg2, generalAdminInp->arg3);
    ...
    if (strcmp(generalAdminInp->arg0, "set-quota") == 0) {
        status = chlSetQuota(
            rsComm, generalAdminInp->arg1, generalAdminInp->arg2, generalAdminInp->arg3, generalAdminInp->arg4);
