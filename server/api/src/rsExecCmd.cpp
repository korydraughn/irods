...
int rsExecCmd(rsComm_t* rsComm, execCmd_t* execCmdInp, execCmdOut_t** execCmdOut)
{
    ...
    const char* args[4] = {execCmdInp->cmd, execCmdInp->cmdArgv, execCmdInp->execAddr, execCmdInp->hintPath};

    status = applyRuleArg("acPreProcForExecCmd", args, 4, &rei, NO_SAVE_REI);
    if (status < 0) {
        rodsLog(LOG_ERROR, "initAgent: acPreProcForExecCmd error, status = %d", status);
        return (status);
    }

    /* some sanity check on the cmd path */
    if (strchr(execCmdInp->cmd, '/') != NULL) {
        rodsLog(LOG_ERROR, "rsExecCmd: bad cmd path %s", execCmdInp->cmd);
    ...
    */
    ...
    if (*execCmdInp->hintPath != '\0') {
        ...
        if (resc_hier_ptr == NULL) {
            irods::error ret = irods::resolve_resource_hierarchy(irods::OPEN_OPERATION, rsComm, &dataObjInp, resc_hier);
            if (!ret.ok()) {
        ...
        if (status < 0) {
            rodsLog(LOG_ERROR, "rsExecCmd: getDataObjInfo error for hintPath %s", execCmdInp->hintPath);
            return status;
        }
        ...
        if (execCmdInp->addPathToArgv > 0) {
            ...
            snprintf(execCmdInp->cmdArgv, HUGE_NAME_LEN, "%s %s", dataObjInfoHead->filePath, tmpArgv);
        }
        ...
        *execCmdInp->hintPath = '\0'; /* no need for hint */
        rstrcpy(execCmdInp->execAddr, location.c_str(), LONG_NAME_LEN);
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        status = remoteExecCmd(rsComm, execCmdInp, execCmdOut, rodsServerHost);
    }
    else {
        rodsLog(LOG_NOTICE, "rsFileOpenByHost: resolveHost of %s error, status = %d", addr.hostAddr, remoteFlag);
...
int remoteExecCmd(rsComm_t* rsComm, execCmd_t* execCmdInp, execCmdOut_t** execCmdOut, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteExecCmd: Invalid rodsServerHost");
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "remoteExecCmd: rcExecCmd failed for %s. status = %d", execCmdInp->cmd, status);
    }
    else if (status > 0 && getValByKey(&execCmdInp->condInput, STREAM_STDOUT_KW) != NULL) {
        int fileInx = status;
        (*execCmdOut)->status = bindStreamToIRods(rodsServerHost, fileInx);
        if ((*execCmdOut)->status < 0) {
            fileCloseInp_t remFileCloseInp;
            rodsLog(LOG_ERROR, "remoteExecCmd: bindStreamToIRods failed. status = %d", (*execCmdOut)->status);
...
int _rsExecCmd(execCmd_t* execCmdInp, execCmdOut_t** execCmdOut)
{
...
#endif
    {
        rodsLog(LOG_ERROR, "_rsExecCmd: pipe create failed. errno = %d", errno);
        ExecCmdMutex.unlock();
    ...
    if (childPid == 0) {
        ...
        if ((tmpStr = getValByKey(&execCmdInp->condInput, EXEC_CMD_RULE_KW)) != NULL) {
    ...
    if (getValByKey(&execCmdInp->condInput, STREAM_STDOUT_KW) != NULL &&
        ...
        if (myExecCmdOut->status < 0) {
            rodsLog(LOG_ERROR, "_rsExecCmd: bindStreamToIRods failed. status = %d", myExecCmdOut->status);
            close(stdoutFd[0]);
        }
    }
    else {
        ...
        if (status >= 0 && myExecCmdOut->status >= 0 && childStatus != 0) {
            rodsLog(LOG_ERROR,
                    "_rsExecCmd: waitpid status = %d, myExecCmdOut->status = %d, childStatus = %d",
                    status,
                    myExecCmdOut->status,
                    childStatus);
...
int initCmdArg(char* av[], char* cmdArgv, char* cmdPath)
{
    ...
    if (strlen(cmdArgv) > 0) {
        ...
        quoteCnt = curLen = 0;
        while (*curPtr != '\0') {
            if ((*curPtr == ' ' && quoteCnt == 0 && curLen > 0) || quoteCnt == 2) {
                /* end of a argv */
            ...
            else if ((*curPtr == '\'' || *curPtr == '\"') && (*(curPtr - 1) == '\\')) {
                /* escaped quotes */
                curPtr++;
                curLen++;
            }
