...
int rsFileChksum(rsComm_t* rsComm, fileChksumInp_t* fileChksumInp, char** chksumStr)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        status = remoteFileChksum(rsComm, fileChksumInp, chksumStr, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
                     ...
                     rodsServerHost_t* rodsServerHost)
{
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "remoteFileChksum: rcFileChksum failed for %s", fileChksumInp->fileName);
    }
...
int _rsFileChksum(rsComm_t* rsComm, fileChksumInp_t* fileChksumInp, char** chksumStr)
{
    ...
    status = fileChksum(rsComm,
                        ...
                        *chksumStr);
    if (status < 0) {
        rodsLog(LOG_DEBUG, "_rsFileChksum: fileChksum for %s, status = %d", fileChksumInp->fileName, status);
        free(*chksumStr);
        *chksumStr = NULL;
    }
...
int fileChksum(rsComm_t* rsComm, char* objPath, char* fileName, char* rescHier, char* orig_chksum, char* chksumStr)
{
    ...
    if (orig_chksum) {
        irods::error ret = irods::get_hash_scheme_from_checksum(orig_chksum, chkstr_scheme);
        if (!ret.ok()) {
            // irods::log( PASS( ret ) );
        }
    }
    ...
    irods::file_object_ptr file_obj(new irods::file_object(
        rsComm, objPath, fileName, rescHier, -1, 0, O_RDONLY)); // FIXME :: hack until this is better abstracted - JMC
