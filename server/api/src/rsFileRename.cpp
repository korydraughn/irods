...
int rsFileRename(rsComm_t* rsComm, fileRenameInp_t* fileRenameInp, fileRenameOut_t** rename_out)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsFileRename: resolveHost returned unrecognized value %d", remoteFlag);
                     ...
                     rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileRename: Invalid rodsServerHost");
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "remoteFileRename: rcFileRename failed for %s", fileRenameInp->newFileName);
    }
...
int _rsFileRename(rsComm_t* _comm, fileRenameInp_t* _rename_inp, fileRenameOut_t** _rename_out)
{
    ...
    // mkDirForFilePath( _comm, "/", _rename_inp->newFileName, getDefDirMode () ); - The actual file path depends on the
    // resource

    // =-=-=-=-=-=-=-
    // make the call to rename via the resource plugin
    irods::file_object_ptr file_obj(
        new irods::file_object(_comm, _rename_inp->objPath, _rename_inp->oldFileName, _rename_inp->rescHier, 0, 0, 0));
    ...
    snprintf((*_rename_out)->file_name, sizeof((*_rename_out)->file_name), "%s", file_obj->physical_path().c_str());
