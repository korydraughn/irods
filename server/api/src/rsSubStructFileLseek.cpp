                         ...
                         fileLseekOut_t** subStructFileLseekOut)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        status = remoteSubStructFileLseek(rsComm, subStructFileLseekInp, subStructFileLseekOut, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsSubStructFileLseek: resolveHost returned unrecognized value %d", remoteFlag);
                             ...
                             rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileLseek: Invalid rodsServerHost");
    ...
    status = rcSubStructFileLseek(rodsServerHost->conn, subStructFileLseekInp, subStructFileLseekOut);

    if (status < 0) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileLseek: rcFileLseek failed for fd %d", subStructFileLseekInp->fd);
    }
...
int _rsSubStructFileLseek(rsComm_t* _comm, subStructFileLseekInp_t* _lseek_inp, fileLseekOut_t** _lseek_out)
{
    ...
    irods::structured_object_ptr struct_obj(new irods::structured_object());
    ...
    irods::error lseek_err = fileLseek(_comm, struct_obj, _lseek_inp->offset, _lseek_inp->whence);
    if (!lseek_err.ok()) {
