...
int rsFileLseek(rsComm_t* rsComm, fileLseekInp_t* fileLseekInp, fileLseekOut_t** fileLseekOut)
{
    ...
    *fileLseekOut = NULL;
    ...
    remoteFlag = getServerHostByFileInx(fileLseekInp->fileInx, &rodsServerHost);

    if (remoteFlag == LOCAL_HOST) {
        retVal = _rsFileLseek(rsComm, fileLseekInp, fileLseekOut);
    }
    else if (remoteFlag == REMOTE_HOST) {
        retVal = remoteFileLseek(rsComm, fileLseekInp, fileLseekOut, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsFileLseek: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteFileLseek(rsComm_t* rsComm,
                    ...
                    rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileLseek: Invalid rodsServerHost");
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "remoteFileLseek: rcFileLseek failed for %d, status = %d", fileLseekInp->fileInx, status);
    }
...
int _rsFileLseek(rsComm_t* _comm, fileLseekInp_t* _lseek_inp, fileLseekOut_t** _lseek_out)
{
    ...
    irods::error lseek_err = fileLseek(_comm, file_obj, _lseek_inp->offset, _lseek_inp->whence);
    // =-=-=-=-=-=-=-
