...
int rsFileRmdir(rsComm_t* rsComm, fileRmdirInp_t* fileRmdirInp)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsFileRmdir: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteFileRmdir(rsComm_t* rsComm, fileRmdirInp_t* fileRmdirInp, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileRmdir: Invalid rodsServerHost");
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "remoteFileOpen: rcFileRmdir failed for %s", fileRmdirInp->dirName);
    }
...
int _rsFileRmdir(rsComm_t* _comm, fileRmdirInp_t* _rmdir_inp)
{
    irods::collection_object_ptr coll_obj(
        new irods::collection_object(_rmdir_inp->dirName, _rmdir_inp->rescHier, 0, 0));
    ...
    if ((_rmdir_inp->flags & RMDIR_RECUR) != 0) {
        ...
        if (strstr(_rmdir_inp->dirName, CACHE_DIR_STR) == NULL) {
            rodsLog(LOG_ERROR, "_rsFileRmdir: recursive rm of non cachedir path %s", _rmdir_inp->dirName);
        ...
        while (readdir_err.ok() && 0 == readdir_err.code()) {
            ...
            irods::collection_object_ptr myPath_obj(new irods::collection_object(myPath, _rmdir_inp->rescHier, 0, 0));
