...
int irsPhyPathReg(rsComm_t* rsComm, dataObjInp_t* phyPathRegInp)
{
    ...
    if (NULL == tmp_hier) {
        ...
        if (dst_resc) {
            ...
            // this is a leaf node situation
                ...
                ret = resc_mgr.get_hier_to_root_for_resc(dst_resc, hier);
                if (!ret.ok()) {
                    irods::log(PASS(ret));
                    return ret.code();
                }
                ...
                addKeyVal(&phyPathRegInp->condInput, DEST_RESC_NAME_KW, root_resc.c_str());
            }
            // =-=-=-=-=-=-=-
            ...
            // root node and pathological situation
                ...
                irods::error ret =
                    irods::resolve_resource_hierarchy(irods::CREATE_OPERATION, rsComm, phyPathRegInp, hier);
                if (!ret.ok()) {
            ...
            irods::error ret = irods::resolve_resource_hierarchy(irods::CREATE_OPERATION, rsComm, phyPathRegInp, hier);
            if (!ret.ok()) {
                std::string msg("failed for [");
                msg += phyPathRegInp->objPath;
                msg += "]";
    ...
    if (coll_type && (strcmp(coll_type, HAAW_STRUCT_FILE_STR) == 0 || strcmp(coll_type, TAR_STRUCT_FILE_STR) == 0 ||
    ...
    std::string location;
    ret = irods::get_resource_property<std::string>(resc_id, irods::RESOURCE_LOCATION, location);
    if (!ret.ok()) {
    ...
    if (size_kw || remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_ERROR, "rsPhyPathReg: resolveHost returned unrecognized value %d", remoteFlag);
...
int remotePhyPathReg(rsComm_t* rsComm, dataObjInp_t* phyPathRegInp, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_ERROR, "remotePhyPathReg: Invalid rodsServerHost");
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "remotePhyPathReg: rcPhyPathReg failed for %s", phyPathRegInp->objPath);
    }

    return status;
}

int _rsPhyPathReg(rsComm_t* rsComm,
                  ...
                  rodsServerHost_t* rodsServerHost)
{
    ...
    if ((tmpFilePath = getValByKey(&phyPathRegInp->condInput, FILE_PATH_KW)) == NULL) {
        rodsLog(LOG_ERROR, "_rsPhyPathReg: No filePath input for %s", phyPathRegInp->objPath);
    ...
    if (getValByKey(&phyPathRegInp->condInput, NO_CHK_FILE_PERM_KW) == NULL &&
        ...
        if (status < 0) {
            rodsLog(LOG_ERROR, "_rsPhyPathReg: chkFilePathPerm error for %s", phyPathRegInp->objPath);
            return status;
        }
    }
    ...
    if (getValByKey(&phyPathRegInp->condInput, COLLECTION_KW) != NULL) {
        ...
        if (excludePatternFile != NULL) {
            ExcludePatterns = readPathnamePatternsFromFile(rsComm, excludePatternFile, resc_hier);
        }
    ...
    else if ((tmpStr = getValByKey(&phyPathRegInp->condInput, COLLECTION_TYPE_KW)) != NULL &&
             strcmp(tmpStr, MOUNT_POINT_STR) == 0) {
        rodsLong_t resc_id = 0;
...
int filePathRegRepl(rsComm_t* rsComm, dataObjInp_t* phyPathRegInp, char* filePath, const char* _resc_name)
{
    ...
    status = getDataObjInfo(rsComm, phyPathRegInp, &dataObjInfoHead, ACCESS_READ_OBJECT, 0);
    if (status < 0) {
        rodsLog(LOG_ERROR, "filePathRegRepl: getDataObjInfo for %s", phyPathRegInp->objPath);
        return status;
    }
    ...
    else if (getValByKey(&phyPathRegInp->condInput, ADMIN_KW) != NULL) {
        addKeyVal(&regReplicaInp.condInput, ADMIN_KW, "");
    }
...
int filePathReg(rsComm_t* rsComm, dataObjInp_t* phyPathRegInp, const char* _resc_name)
{
    ...
    if (nullptr == data_size_str && dataObjInfo.dataSize <= 0 &&
        ...
        rodsLog(
            LOG_ERROR, "filePathReg: getFileMetadataFromVault for %s failed, status = %d", dataObjInfo.objPath, status);
    ...
    if ((getValByKey(&phyPathRegInp->condInput, REG_CHKSUM_KW) != NULL) ||
        ...
        if (status < 0) {
            rodsLog(LOG_ERROR, "filePathReg: _dataObjChksum for %s failed, status = %d", dataObjInfo.objPath, status);
            clearKeyVal(&dataObjInfo.condInput);
            return status;
        }
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "filePathReg: svrRegDataObj for %s failed, status = %d", dataObjInfo.objPath, status);
    }
    else {
        ruleExecInfo_t rei;
...
int dirPathReg(rsComm_t* rsComm, dataObjInp_t* phyPathRegInp, char* filePath, const char* _resc_name)
{
    ...
    if (status < 0 || NULL == rodsObjStatOut) { // JMC cppcheck - nullptr
        ...
        freeRodsObjStat(rodsObjStatOut);
        rodsLog(LOG_ERROR, "mountFileDir: %s already mounted", phyPathRegInp->objPath);
    ...
    if (dirFd < 0) {
        rodsLog(LOG_ERROR, "dirPathReg: rsFileOpendir for %s error, status = %d", filePath, dirFd);
        return dirFd;
    }
    ...
    while ((status = rsFileReaddir(rsComm, &fileReaddirInp, &rodsDirent)) >= 0) {
        ...
        if (strcmp(rodsDirent->d_name, ".") == 0 || strcmp(rodsDirent->d_name, "..") == 0) {
            free(rodsDirent); // JMC - backport 4835
            continue;
        }

        subPhyPathRegInp = *phyPathRegInp;
        snprintf(subPhyPathRegInp.objPath, MAX_NAME_LEN, "%s/%s", phyPathRegInp->objPath, rodsDirent->d_name);
        ...
        if (status != 0) {
            rodsLog(LOG_ERROR, "dirPathReg: rsFileStat failed for %s, status = %d", fileStatInp.fileName, status);
            free(rodsDirent); // JMC - backport 4835
            return status;
        }
        ...
        if ((myStat->st_mode & S_IFREG) != 0) { /* a file */
            ...
            if (status != 0) {
                if (rsComm->rError.len < MAX_ERROR_MESSAGES) {
                    ...
                    snprintf(error_msg,
                             ERR_MSG_LEN,
                             "dirPathReg: %s failed for %s, status = %d",
                             reg_func_name.c_str(),
                             subPhyPathRegInp.objPath,
                             status);
...
int mountFileDir(rsComm_t* rsComm, dataObjInp_t* phyPathRegInp, char* filePath, const char* rescVaultPath)
{
    ...
    if (isCollEmpty(rsComm, phyPathRegInp->objPath) == False) {
        rodsLog(LOG_ERROR, "mountFileDir: collection %s not empty", phyPathRegInp->objPath);
    ...
    if (status < 0) {
        ...
        rodsLog(
            LOG_NOTICE, "mountFileDir: rsFileStat failed for %s, status = %d, create it", fileStatInp.fileName, status);
        ...
        rodsLog(LOG_ERROR, "mountFileDir: phyPath %s is not a directory", fileStatInp.fileName);
        free(myStat);
...
int unmountFileDir(rsComm_t* rsComm, dataObjInp_t* phyPathRegInp)
{
    ...
    if (status < 0 || NULL == rodsObjStatOut) { // JMC cppcheck - nullptr
        ...
        freeRodsObjStat(rodsObjStatOut);
        rodsLog(LOG_ERROR, "unmountFileDir: %s not mounted", phyPathRegInp->objPath);
        return SYS_COLL_NOT_MOUNTED_ERR;
    }

    if (getStructFileType(rodsObjStatOut->specColl) >= 0) {
        status = _rsSyncMountedColl(rsComm, rodsObjStatOut->specColl, PURGE_STRUCT_FILE_CACHE);
        if (status < 0) {
    ...
    addKeyVal(&modCollInp.condInput, COLLECTION_TYPE_KW, "NULL_SPECIAL_VALUE");
...
int structFileReg(rsComm_t* rsComm, dataObjInp_t* phyPathRegInp)
{
    ...
    if ((structFilePath = getValByKey(&phyPathRegInp->condInput, FILE_PATH_KW)) == NULL) {
        rodsLog(LOG_ERROR, "structFileReg: No structFilePath input for %s", phyPathRegInp->objPath);
        return SYS_INVALID_FILE_PATH;
    }

    collType = getValByKey(&phyPathRegInp->condInput, COLLECTION_TYPE_KW);
    if (collType == NULL) {
        rodsLog(LOG_ERROR, "structFileReg: Bad COLLECTION_TYPE_KW for structFilePath %s", dataObjInp.objPath);
    ...
    if (strncmp(structFilePath, phyPathRegInp->objPath, len) == 0 &&
        ...
        rodsLog(
            LOG_ERROR, "structFileReg: structFilePath %s inside collection %s", structFilePath, phyPathRegInp->objPath);
    ...
    if (getSpecCollCache(rsComm, structFilePath, 0, &specCollCache) >= 0) {
        rodsLog(LOG_ERROR, "structFileReg: structFilePath %s is in a mounted path", structFilePath);
        return SYS_STRUCT_FILE_INMOUNTED_COLL;
    }
    ...
    if (rodsObjStatOut->specColl != NULL) {
        freeRodsObjStat(rodsObjStatOut);
        rodsLog(LOG_ERROR, "structFileReg: %s already mounted", phyPathRegInp->objPath);
        return SYS_MOUNT_MOUNTED_COLL_ERR;
    }
    ...
    if (isCollEmpty(rsComm, phyPathRegInp->objPath) == False) {
        rodsLog(LOG_ERROR, "structFileReg: collection %s not empty", phyPathRegInp->objPath);
        return SYS_COLLECTION_NOT_EMPTY;
    }
...
int structFileSupport(rsComm_t* rsComm, char* collection, char* collType, char* resc_hier)
{
    ...
    if (rsComm == NULL || collection == NULL || collType == NULL || resc_hier == NULL) {
        return 0;
    }
...
int linkCollReg(rsComm_t* rsComm, dataObjInp_t* phyPathRegInp)
{
    ...
    if ((linkPath = getValByKey(&phyPathRegInp->condInput, FILE_PATH_KW)) == NULL) {
        rodsLog(LOG_ERROR, "linkCollReg: No linkPath input for %s", phyPathRegInp->objPath);
        return SYS_INVALID_FILE_PATH;
    }

    collType = getValByKey(&phyPathRegInp->condInput, COLLECTION_TYPE_KW);
    if (collType == NULL || strcmp(collType, LINK_POINT_STR) != 0) {
        rodsLog(LOG_ERROR, "linkCollReg: Bad COLLECTION_TYPE_KW for linkPath %s", phyPathRegInp->objPath);
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    len = strlen(phyPathRegInp->objPath);
    if (strncmp(linkPath, phyPathRegInp->objPath, len) == 0 && linkPath[len] == '/') {
        rodsLog(LOG_ERROR, "linkCollReg: linkPath %s inside collection %s", linkPath, phyPathRegInp->objPath);
        return SYS_COLL_LINK_PATH_ERR;
    }

    len = strlen(linkPath);
    if (strncmp(phyPathRegInp->objPath, linkPath, len) == 0 && phyPathRegInp->objPath[len] == '/') {
        rodsLog(LOG_ERROR, "linkCollReg: collection %s inside linkPath %s", linkPath, phyPathRegInp->objPath);
        return SYS_COLL_LINK_PATH_ERR;
    }

    if (getSpecCollCache(rsComm, linkPath, 0, &specCollCache) >= 0 &&
        specCollCache->specColl.collClass != LINKED_COLL) {
        rodsLog(LOG_ERROR, "linkCollReg: linkPath %s is in a spec coll path", linkPath);
        return SYS_COLL_LINK_PATH_ERR;
    }
    ...
    if (status < 0) {
        ...
        status = rsRegColl(rsComm, &collCreateInp);
        if (status < 0) {
            rodsLog(LOG_ERROR, "linkCollReg: rsRegColl error for  %s, status = %d", collCreateInp.collName, status);
            return status;
        }
        status = collStat(rsComm, phyPathRegInp, &rodsObjStatOut);
        if (status < 0) {
    ...
    if (rodsObjStatOut && // JMC cppcheck - nullptr
        rodsObjStatOut->specColl != NULL && rodsObjStatOut->specColl->collClass != LINKED_COLL) {
        freeRodsObjStat(rodsObjStatOut);
        rodsLog(LOG_ERROR, "linkCollReg: link collection %s in a spec coll path", phyPathRegInp->objPath);
        return SYS_COLL_LINK_PATH_ERR;
    }
    ...
    if (isCollEmpty(rsComm, phyPathRegInp->objPath) == False) {
        rodsLog(LOG_ERROR, "linkCollReg: collection %s not empty", phyPathRegInp->objPath);
        return SYS_COLLECTION_NOT_EMPTY;
    }
    ...
    /* have to use dataObjInp.objPath because structFile path was removed */
    addKeyVal(&collCreateInp.condInput, COLLECTION_INFO1_KW, linkPath);
...
pathnamePatterns_t* readPathnamePatternsFromFile(rsComm_t* rsComm, char* filename, char* resc_hier)
{
    int status;
    rodsStat_t* stbuf;
    fileStatInp_t fileStatInp;
    ...
    if (status != 0) {
        if (status != UNIX_FILE_STAT_ERR - ENOENT) {
            rodsLog(
                LOG_DEBUG, "readPathnamePatternsFromFile: can't stat %s. status = %d", fileStatInp.fileName, status);
        }
        return NULL;
    }
    ...
    if (status < 0) {
        rodsLog(
            LOG_NOTICE, "readPathnamePatternsFromFile: could not read %s. status = %d", fileOpenInp.fileName, status);
