...
int rsRegColl(rsComm_t* rsComm, collInp_t* regCollInp)
{
    ...
    if (status >= 0) {
        if (rodsObjStatOut != NULL) {
            if (rodsObjStatOut->specColl != NULL) { // JMC cppcheck null ptr ref
                rodsLog(LOG_ERROR, "rsRegColl: Reg path %s is in spec coll", dataObjInp.objPath);
    ...
    status = getAndConnRcatHost(rsComm, MASTER_RCAT, (const char*) regCollInp->collName, &rodsServerHost);
    if (status < 0) {
        return status;
    }
