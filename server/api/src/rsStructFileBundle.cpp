...
int rsStructFileBundle(rsComm_t* rsComm, structFileExtAndRegInp_t* structFileBundleInp)
{
    ...
    if (getValByKey(&structFileBundleInp->condInput, RESC_HIER_STR_KW) == NULL) {
        irods::error ret = irods::resource_redirect(irods::CREATE_OPERATION, rsComm, &data_inp, hier, host, local);
        if (!ret.ok()) {
...
int _rsStructFileBundle(rsComm_t* rsComm, structFileExtAndRegInp_t* structFileBundleInp)
{
    ...
    if (l1descInx < 0) {
        rodsLog(
            LOG_ERROR, "rsStructFileBundle: rsDataObjCreate of %s error. status = %d", dataObjInp.objPath, l1descInx);
        return l1descInx;
    }
    ...
    if (handleInx < 0) {
        rodsLog(
            LOG_ERROR, "rsStructFileBundle: rsOpenCollection of %s error. status = %d", collInp.collName, handleInx);
        rmdir(phyBunDir);
        return handleInx;
    }
    ...
    while ((status = rsReadCollection(rsComm, &handleInx, &collEnt)) >= 0) {
        ...
        if (collEnt->objType == DATA_OBJ_T) {
            ...
            if (collEnt->collName[collLen] == '\0') {
                ...
                snprintf(
                    tmpPath, MAX_NAME_LEN, "%s/%s/%s", phyBunDir, collEnt->collName + collLen + 1, collEnt->dataName);
                status = mkDirForFilePath(rsComm, strlen(phyBunDir), tmpPath, collEnt->resc_hier, getDefDirMode());
                if (status < 0) {
            ...
            if (status < 0) {
                rodsLog(
                    LOG_ERROR, "rsStructFileBundle: link error %s to %s. errno = %d", collEnt->phyPath, tmpPath, errno);
                ...
                rodsLog(
                    LOG_DEBUG, "_rsStructFileBundle - LINK  [%s] on resc [%s]", collEnt->phyPath, collEnt->resc_hier);
            }
        }
        else {
            // =-=-=-=-=-=-=-
    ...
    status = phyBundle(rsComm,
                       L1desc[l1descInx].dataObjInfo,
                       phyBunDir,
                       collInp.collName,
                       structFileBundleInp->oprType); // JMC - backport 4643
                           ...
                           rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteStructFileBundle: Invalid rodsServerHost");
