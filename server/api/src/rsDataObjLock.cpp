...
int rsDataObjLock(rsComm_t* rsComm, dataObjInp_t* dataObjInp)
{
    ...
    int remoteFlag = getAndConnRcatHost(rsComm, MASTER_RCAT, (const char*) dataObjInp->objPath, &rodsServerHost);
    if (remoteFlag < 0) {
        return remoteFlag;
    }
...
int getLockCmdAndType(keyValPair_t* condInput, int* cmd, int* type)
{
    ...
    if (strcmp(lockType, READ_LOCK_TYPE) == 0) {
        ...
        *type = F_WRLCK;
        ...
        rodsLogError(LOG_ERROR, status, "getLockCmdAndType: illegal lock type %s", lockType);
        return status;
    }
    ...
    if (strcmp(lockCmd, SET_LOCK_CMD) == 0) {
        ...
        *cmd = F_GETLK;
        ...
        rodsLogError(LOG_ERROR, status, "getLockCmdAndType: illegal lock cmd %s", lockCmd);
        return status;
    }
    return 0;
}
...
int _rsDataObjUnlock(dataObjInp_t* dataObjInp)
{
    ...
    if (fd_string == NULL) {
        ...
        rodsLogError(LOG_ERROR, status, "getLockCmdAndType: LOCK_FD_KW not defined for unlock operation");
        return status;
    }
