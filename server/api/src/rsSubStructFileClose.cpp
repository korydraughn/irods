...
int rsSubStructFileClose(rsComm_t* rsComm, subStructFileFdOprInp_t* subStructFileCloseInp)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        status = remoteSubStructFileClose(rsComm, subStructFileCloseInp, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsSubStructFileClose: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteSubStructFileClose(rsComm_t* rsComm,
                             ...
                             rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileClose: Invalid rodsServerHost");
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileClose: rcFileClose failed for fd %d", subStructFileCloseInp->fd);
    }
