...
int _rsRuleExecDel(rsComm_t* rsComm, ruleExecDelInp_t* ruleExecDelInp)
{
    ...
    status = getReInfoById(rsComm, ruleExecDelInp->ruleExecId, &genQueryOut);
    ...
    if (status < 0) {
        ...
        if (irods::CFG_SERVICE_ROLE_PROVIDER == svc_role) {
            ...
            const auto err{ERROR(SYS_SERVICE_ROLE_NOT_SUPPORTED,
                                 (boost::format("role not supported [%s]") % svc_role.c_str()).str().c_str())};
            irods::log(err);
            return err.code();
        }
    }
    ...
    /* some sanity check */
    snprintf(reiDir, MAX_NAME_LEN, "/%-s/%-s.", PACKED_REI_DIR, REI_FILE_NAME);
    ...
    if (strstr(reiFilePath->value, reiDir) == NULL) {
        if (irods::CFG_SERVICE_ROLE_PROVIDER == svc_role) {
            ...
            rodsLog(LOG_NOTICE, "_rsRuleExecDel: reiFilePath: %s is not a proper rei path", reiFilePath->value);
            ...
            snprintf(errMsg, sizeof errMsg, "Rule was removed but reiPath was invalid: %s", reiFilePath->value);
    ...
    if (status < 0) {
        ...
        rodsLog(LOG_ERROR, "_rsRuleExecDel: unlink of %s error, status = %d", reiFilePath->value, status);
        if (errno != ENOENT) {
            freeGenQueryOut(&genQueryOut);
            return status;
        }
    ...
    if (irods::CFG_SERVICE_ROLE_PROVIDER == svc_role) {
        ...
        if (unlinkStatus) {
            ...
            char errMsg[105];
            /* Add a message to the error stack for the client user */
            snprintf(errMsg, sizeof errMsg, "Rule was removed but unlink of rei file failed");
            i = addRErrorMsg(&rsComm->rError, 0, errMsg);
            if (i < 0) {
                irods::log(ERROR(i, "addRErrorMsg failed"));
            }
            snprintf(errMsg, sizeof errMsg, "rei file: %s", reiFilePath->value);
