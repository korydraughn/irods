...
int rsSpecificQuery(rsComm_t* rsComm, specificQueryInp_t* specificQueryInp, genQueryOut_t** genQueryOut)
{
    ...
    status = getAndConnRcatHost(rsComm, SLAVE_RCAT, (const char*) zoneHint, &rodsServerHost);
    if (status < 0) {
        return status;
    }
    ...
    if (rodsServerHost->localFlag == LOCAL_HOST) {
        ...
        if (irods::CFG_SERVICE_ROLE_PROVIDER == svc_role) {
            ...
            rodsLog(LOG_NOTICE, "rsSpecificQuery error. RCAT is not configured on this host");
        ...
        status = rcSpecificQuery(rodsServerHost->conn, specificQueryInp, genQueryOut);
    }
    if (status < 0 && status != CAT_NO_ROWS_FOUND) {
        rodsLog(LOG_NOTICE, "rsSpecificQuery: rcSpecificQuery failed, status = %d", status);
    }
    return status;
}
    ...
    if (status < 0) {
        ...
        *genQueryOut = NULL;
        if (status != CAT_NO_ROWS_FOUND) {
            rodsLog(LOG_NOTICE, "_rsSpecificQuery: specificQuery status = %d", status);
        }
        return status;
    }
    return status;
