...
int rsProcStat(rsComm_t* rsComm, procStatInp_t* procStatInp, genQueryOut_t** procStatOut)
{
    ...
    if (*procStatInp->rodsZone != '\0') {
        remoteFlag = getRcatHost(MASTER_RCAT, procStatInp->rodsZone, &rodsServerHost);
        if (remoteFlag < 0) {
            rodsLog(LOG_ERROR, "rsProcStat: getRcatHost() failed. erro=%d", remoteFlag);
            return remoteFlag;
        }
        if (rodsServerHost->localFlag == REMOTE_HOST) {
            status = remoteProcStat(rsComm, procStatInp, procStatOut, rodsServerHost);
        }
        else {
...
int _rsProcStat(rsComm_t* rsComm, procStatInp_t* procStatInp, genQueryOut_t** procStatOut)
{
    ...
    if (remoteFlag < 0) {
        rodsLog(LOG_ERROR, "_rsProcStat: getRcatHost() failed. erro=%d", remoteFlag);
        return remoteFlag;
    }
    else if (remoteFlag == REMOTE_HOST) {
        addKeyVal(&myProcStatInp.condInput, EXEC_LOCALLY_KW, "");
        status = remoteProcStat(rsComm, &myProcStatInp, procStatOut, rodsServerHost);
...
int localProcStat(procStatInp_t* procStatInp, genQueryOut_t** procStatOut)
{
    ...
    if (!exists(srcDirPath) || !is_directory(srcDirPath)) {
        ...
        rodsLogError(LOG_ERROR, status, "localProcStat: opendir local dir error for %s", ProcLogDir);
        return status;
    }
    ...
    for (directory_iterator itr(srcDirPath); itr != end_itr; ++itr) {
        ...
        if (!exists(p)) {
            rodsLogError(LOG_ERROR, status, "localProcStat: stat error for %s", childPath);
            continue;
        }
        if (is_regular_file(p)) {
            if (count >= numProc) {
                rodsLog(LOG_ERROR, "localProcStat: proc count %d exceeded", numProc);
                break;
            }
...
int remoteProcStat(rsComm_t* rsComm,
                   ...
                   rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_ERROR, "remoteProcStat: Invalid rodsServerHost");
...
int addProcToProcStatOut(procLog_t* procLog, genQueryOut_t* procStatOut)
{
    ...
    snprintf(&procStatOut->sqlResult[0].value[NAME_LEN * rowCnt], NAME_LEN, "%d", procLog->pid);
    snprintf(&procStatOut->sqlResult[1].value[NAME_LEN * rowCnt], NAME_LEN, "%u", procLog->startTime);
    rstrcpy(&procStatOut->sqlResult[2].value[NAME_LEN * rowCnt], procLog->clientName, NAME_LEN);
    rstrcpy(&procStatOut->sqlResult[3].value[NAME_LEN * rowCnt], procLog->clientZone, NAME_LEN);
    rstrcpy(&procStatOut->sqlResult[4].value[NAME_LEN * rowCnt], procLog->proxyName, NAME_LEN);
    rstrcpy(&procStatOut->sqlResult[5].value[NAME_LEN * rowCnt], procLog->proxyZone, NAME_LEN);
    rstrcpy(&procStatOut->sqlResult[6].value[NAME_LEN * rowCnt], procLog->remoteAddr, NAME_LEN);
    rstrcpy(&procStatOut->sqlResult[7].value[NAME_LEN * rowCnt], procLog->serverAddr, NAME_LEN);
    rstrcpy(&procStatOut->sqlResult[8].value[NAME_LEN * rowCnt], procLog->progName, NAME_LEN);
