...
int rsQuerySpecColl(rsComm_t* rsComm, dataObjInp_t* dataObjInp, genQueryOut_t** genQueryOut)
{
    ...
    remoteFlag = getAndConnRcatHost(rsComm, SLAVE_RCAT, (const char*) dataObjInp->objPath, &rodsServerHost);

    if (remoteFlag < 0) {
        return remoteFlag;
    }
    else if (remoteFlag == REMOTE_HOST) {
        status = rcQuerySpecColl(rodsServerHost->conn, dataObjInp, genQueryOut);
        return status;
    }
    ...
    status = _rsQuerySpecColl(rsComm, specCollInx, dataObjInp, *genQueryOut, continueFlag);
...
int openSpecColl(rsComm_t* rsComm, dataObjInp_t* dataObjInp, int parentInx)
{
    ...
    status = resolvePathInSpecColl(rsComm,
                                   dataObjInp->objPath,
    ...
    if (status < 0 || NULL == dataObjInfo) { // JMC cppcheck - nullptr
        rodsLog(LOG_NOTICE, "rsQuerySpecColl: resolveSpecColl error for %s, status = %d", dataObjInp->objPath, status);
        return status;
    }

    if (dataObjInfo->specColl->collClass == LINKED_COLL) {
        rodsLog(LOG_ERROR, "rsQuerySpecColl: %s is a linked collection", dataObjInp->objPath);
    ...
    if (l3descInx < 0) {
        rodsLog(LOG_NOTICE, "openSpecColl: specCollOpendir error for %s, status = %d", dataObjInp->objPath, l3descInx);
        return l3descInx;
    }
...
int initOutForQuerySpecColl(genQueryOut_t** genQueryOut)
{
    ...
    myGenQueryOut->sqlResult[0].value = (char*) malloc(MAX_NAME_LEN * MAX_SPEC_COLL_ROW);
    memset(myGenQueryOut->sqlResult[0].value, 0, MAX_NAME_LEN * MAX_SPEC_COLL_ROW);
    myGenQueryOut->sqlResult[1].attriInx = COL_DATA_NAME;
    myGenQueryOut->sqlResult[1].len = MAX_NAME_LEN;
    myGenQueryOut->sqlResult[1].value = (char*) malloc(MAX_NAME_LEN * MAX_SPEC_COLL_ROW);
    memset(myGenQueryOut->sqlResult[1].value, 0, MAX_NAME_LEN * MAX_SPEC_COLL_ROW);
    myGenQueryOut->sqlResult[2].attriInx = COL_D_CREATE_TIME;
    myGenQueryOut->sqlResult[2].len = NAME_LEN;
    myGenQueryOut->sqlResult[2].value = (char*) malloc(NAME_LEN * MAX_SPEC_COLL_ROW);
    memset(myGenQueryOut->sqlResult[2].value, 0, NAME_LEN * MAX_SPEC_COLL_ROW);
    myGenQueryOut->sqlResult[3].attriInx = COL_D_MODIFY_TIME;
    myGenQueryOut->sqlResult[3].len = NAME_LEN;
    myGenQueryOut->sqlResult[3].value = (char*) malloc(NAME_LEN * MAX_SPEC_COLL_ROW);
    memset(myGenQueryOut->sqlResult[3].value, 0, NAME_LEN * MAX_SPEC_COLL_ROW);
    myGenQueryOut->sqlResult[4].attriInx = COL_DATA_SIZE;
    myGenQueryOut->sqlResult[4].len = NAME_LEN;
    myGenQueryOut->sqlResult[4].value = (char*) malloc(NAME_LEN * MAX_SPEC_COLL_ROW);
    memset(myGenQueryOut->sqlResult[4].value, 0, NAME_LEN * MAX_SPEC_COLL_ROW);
...
int _rsQuerySpecColl(rsComm_t* rsComm,
                     int specCollInx,
                     ...
                     int continueFlag)
{
    ...
    if (SpecCollDesc[specCollInx].inuseFlag != FD_INUSE) {
        rodsLog(LOG_ERROR, "_rsQuerySpecColl: Input specCollInx %d not active", specCollInx);
        return BAD_INPUT_DESC_INDEX;
    }

    if ((tmpStr = getValByKey(&dataObjInp->condInput, SEL_OBJ_TYPE_KW)) != NULL) {
    ...
    while (genQueryOut->rowCnt < MAX_SPEC_COLL_ROW) {
        ...
        if (strcmp(myRodsDirent.d_name, ".") == 0 || strcmp(myRodsDirent.d_name, "..") == 0) {
            continue;
        }

        myDataObjInfo = *dataObjInfo;

        snprintf(myDataObjInfo.subPath, MAX_NAME_LEN, "%s/%s", dataObjInfo->subPath, myRodsDirent.d_name);
        snprintf(myDataObjInfo.filePath, MAX_NAME_LEN, "%s/%s", dataObjInfo->filePath, myRodsDirent.d_name);

        rodsStat_t* fileStatOut = NULL;
        status = l3Stat(rsComm, &myDataObjInfo, &fileStatOut);
        if (status < 0) {
            rodsLog(LOG_ERROR, "_rsQuerySpecColl: l3Stat for %s error, status = %d", myDataObjInfo.filePath, status);
            /* XXXXX need clean up */
            return status;
        }
        ...
        if ((fileStatOut->st_mode & S_IFREG) != 0) { /* a file */
            ...
            rstrcpy(&genQueryOut->sqlResult[0].value[MAX_NAME_LEN * rowCnt], dataObjInfo->subPath, MAX_NAME_LEN);
            rstrcpy(&genQueryOut->sqlResult[1].value[MAX_NAME_LEN * rowCnt], myRodsDirent.d_name, MAX_NAME_LEN);
            snprintf(&genQueryOut->sqlResult[2].value[NAME_LEN * rowCnt], NAME_LEN, "%d", fileStatOut->st_ctim);
            snprintf(&genQueryOut->sqlResult[3].value[NAME_LEN * rowCnt], NAME_LEN, "%d", fileStatOut->st_mtim);
            snprintf(&genQueryOut->sqlResult[4].value[NAME_LEN * rowCnt], NAME_LEN, "%lld", fileStatOut->st_size);
            ...
            if (selObjType != DATA_OBJ_T) {
                rowCnt = genQueryOut->rowCnt;
                rstrcpy(&genQueryOut->sqlResult[0].value[MAX_NAME_LEN * rowCnt], myDataObjInfo.subPath, MAX_NAME_LEN);
                snprintf(&genQueryOut->sqlResult[2].value[NAME_LEN * rowCnt], NAME_LEN, "%d", fileStatOut->st_ctim);
                snprintf(&genQueryOut->sqlResult[3].value[NAME_LEN * rowCnt], NAME_LEN, "%d", fileStatOut->st_mtim);
                snprintf(&genQueryOut->sqlResult[4].value[NAME_LEN * rowCnt], NAME_LEN, "%lld", fileStatOut->st_size);
                genQueryOut->rowCnt++;
            ...
            if (recurFlag > 0) {
                ...
                rstrcpy(newDataObjInp.objPath, dataObjInfo->subPath, MAX_NAME_LEN);
                newSpecCollInx = openSpecColl(rsComm, &newDataObjInp, specCollInx);
                if (newSpecCollInx < 0) {
                    rodsLog(LOG_ERROR,
                            "_rsQuerySpecColl: openSpecColl err for %s, stat = %d",
                            newDataObjInp.objPath,
                            newSpecCollInx);
                    status = newSpecCollInx;
                    break;
                }
                status = _rsQuerySpecColl(rsComm, newSpecCollInx, &newDataObjInp, genQueryOut, 0);
                if (status < 0) {
                    break;
                }
            }
    ...
    if (genQueryOut->rowCnt < MAX_SPEC_COLL_ROW) {
        ...
        if (status >= 0 && recurFlag && continueFlag && parentInx > 0) {
            newDataObjInp = *dataObjInp;
            rstrcpy(newDataObjInp.objPath, SpecCollDesc[parentInx].dataObjInfo->objPath, MAX_NAME_LEN);
            status = _rsQuerySpecColl(rsComm, parentInx, &newDataObjInp, genQueryOut, continueFlag);
        }
        else {
            /* no more */
...
int specCollReaddir(rsComm_t* rsComm, int specCollInx, rodsDirent_t** rodsDirent)
{
    ...
    if (getStructFileType(dataObjInfo->specColl) >= 0) {
        ...
        rstrcpy(subStructFileReaddirInp.addr.hostAddr, location.c_str(), NAME_LEN);
        rstrcpy(subStructFileReaddirInp.resc_hier, dataObjInfo->rescHier, MAX_NAME_LEN);
        status = rsSubStructFileReaddir(rsComm, &subStructFileReaddirInp, rodsDirent);
    }
        ...
        rodsLog(LOG_ERROR, "specCollReaddir: Unknown specColl collClass = %d", specColl->collClass);
...
int specCollClosedir(rsComm_t* rsComm, int specCollInx)
{
    ...
    if (getStructFileType(dataObjInfo->specColl) >= 0) {
        ...
        rstrcpy(subStructFileClosedirInp.addr.hostAddr, location.c_str(), NAME_LEN);
        rstrcpy(subStructFileClosedirInp.resc_hier, dataObjInfo->rescHier, MAX_NAME_LEN);
        ...
        rodsLog(LOG_ERROR, "specCollClosedir: Unknown specColl collClass = %d", specColl->collClass);
        status = SYS_UNKNOWN_SPEC_COLL_CLASS;
    }
...
int l3Opendir(rsComm_t* rsComm, dataObjInfo_t* dataObjInfo)
{
    ...
    if (getStructFileType(dataObjInfo->specColl) >= 0) {
        ...
        if (status < 0) {
            rodsLog(LOG_ERROR, "l3Opendir: rsFileOpendir for %s error, status = %d", dataObjInfo->filePath, status);
        }
    }
    return status;
}
