...
int rsFileClosedir(rsComm_t* rsComm, fileClosedirInp_t* fileClosedirInp)
{
    ...
    remoteFlag = getServerHostByFileInx(fileClosedirInp->fileInx, &rodsServerHost);
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsFileClosedir: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteFileClosedir(rsComm_t* rsComm, fileClosedirInp_t* fileClosedirInp, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileClosedir: Invalid rodsServerHost");
...
int _rsFileClosedir(rsComm_t* _comm, fileClosedirInp_t* _closedir_inp)
{
    ...
    irods::collection_object_ptr coll_obj(new irods::collection_object(
        FileDesc[_closedir_inp->fileInx].fileName, FileDesc[_closedir_inp->fileInx].rescHier, 0, 0));
