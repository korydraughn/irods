...
int rsModColl(rsComm_t* rsComm, collInp_t* modCollInp)
{
    ...
    status = getAndConnRcatHost(rsComm, MASTER_RCAT, (const char*) modCollInp->collName, &rodsServerHost);
...
int _rsModColl(rsComm_t* rsComm, collInp_t* modCollInp)
{
    ...
    if (irods::CFG_SERVICE_ROLE_PROVIDER == svc_role) {
        ...
        if ((tmpStr = getValByKey(&modCollInp->condInput, COLLECTION_TYPE_KW)) != NULL) {
            rstrcpy(collInfo.collType, tmpStr, NAME_LEN);
        }
        if ((tmpStr = getValByKey(&modCollInp->condInput, COLLECTION_INFO1_KW)) != NULL) {
            rstrcpy(collInfo.collInfo1, tmpStr, MAX_NAME_LEN);
        }
        if ((tmpStr = getValByKey(&modCollInp->condInput, COLLECTION_INFO2_KW)) != NULL) {
        ...
        if (i < 0) {
            ...
            rodsLog(
                LOG_ERROR, "rsGeneralAdmin:acPreProcForModifyCollMeta error for %s,stat=%d", modCollInp->collName, i);
            return i;
        }
        /**  June 1 2009 for pre-post processing rule hooks **/
