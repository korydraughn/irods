...
namespace irods
{
    namespace reg_repl
    {
        void get_object_and_collection_from_path(const std::string& _object_path,
        ...
        std::string compute_checksum_for_resc(rsComm_t* _comm,
                                              const std::string& _logical_path,
                                              const std::string& _physical_path,
                                              const std::string& _resc_hier)
        {
            fileChksumInp_t chk_inp{};
            rstrcpy(chk_inp.objPath, _logical_path.c_str(), MAX_NAME_LEN);
            rstrcpy(chk_inp.fileName, _physical_path.c_str(), MAX_NAME_LEN);
            rstrcpy(chk_inp.rescHier, _resc_hier.c_str(), MAX_NAME_LEN);
            ...
            if (chksum_err < 0) {
                THROW(chksum_err,
                      boost::format("rsDataObjChksum failed for [%s] on [%s]") % _logical_path % _resc_hier);
            }

            return chksum;
        } // compute_checksum_for_resc

        rodsLong_t get_file_size_from_filesystem(rsComm_t* _comm,
                                                 ...
                                                 const std::string& _file_path)
        {
            ...
            if (status_rsFileStat < 0) {
                THROW(status_rsFileStat,
                      boost::format("rsFileStat of objPath [%s] rescHier [%s] fileName [%s] failed with [%d]") %
                          stat_inp.objPath % stat_inp.rescHier % stat_inp.fileName % status_rsFileStat);
        ...
        void verify_and_update_replica(rsComm_t* _comm, regReplica_t* _reg_inp)
        {
            ...
            const auto dst_size =
                get_file_size_from_filesystem(_comm, dst_info->objPath, dst_info->rescHier, dst_info->filePath);
            ...
            if (strlen(src_info->chksum) > 0) {
                dst_checksum =
                    compute_checksum_for_resc(_comm, dst_info->objPath, dst_info->filePath, dst_info->rescHier);
                if (!dst_checksum.empty() && dst_checksum != src_info->chksum) {
                    rstrcpy(dst_info->chksum, dst_checksum.c_str(), sizeof(dst_info->chksum));
                    addKeyVal(&reg_param, CHKSUM_KW, dst_checksum.c_str());
                }
            } // checksum

            if (reg_param.len > 0) {
                addKeyVal(&reg_param, ALL_REPL_STATUS_KW, "TRUE");
                ...
                if (mod_err < 0) {
                    THROW(mod_err,
                          boost::format("rsModDataObjMeta failed for [%s] on resc [%s]") % dst_info->objPath %
                              dst_info->rescHier);
                }
            } // if reg_params
...
int rsRegReplica(rsComm_t* rsComm, regReplica_t* regReplicaInp)
{
    ...
    status = getAndConnRcatHost(rsComm, MASTER_RCAT, (const char*) srcDataObjInfo->objPath, &rodsServerHost);
...
int _rsRegReplica(rsComm_t* rsComm, regReplica_t* regReplicaInp)
{
    ...
    if (irods::CFG_SERVICE_ROLE_PROVIDER == svc_role) {
        ...
        if (status == CAT_SUCCESS_BUT_WITH_NO_INFO ||
            ...
            status2 =
                checkDupReplica(rsComm, srcDataObjInfo->dataId, destDataObjInfo->rescName, destDataObjInfo->filePath);
            if (status2 >= 0) {
...
int _call_file_modified_for_replica(rsComm_t* rsComm, regReplica_t* regReplicaInp)
{
    ...
    irods::file_object_ptr file_obj(new irods::file_object(rsComm, destDataObjInfo));
