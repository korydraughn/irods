...
int rsSubStructFileRename(rsComm_t* rsComm, subStructFileRenameInp_t* subStructFileRenameInp)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsSubStructFileRename: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteSubStructFileRename(rsComm_t* rsComm,
                              ...
                              rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileRename: Invalid rodsServerHost");
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE,
                ...
                subStructFileRenameInp->subFile.subFilePath,
                status);
    }
...
int _rsSubStructFileRename(rsComm_t* _comm, subStructFileRenameInp_t* _rename_inp)
{
    ...
    irods::structured_object_ptr struct_obj(new irods::structured_object(_rename_inp->subFile));
