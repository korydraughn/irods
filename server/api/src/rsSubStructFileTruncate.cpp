...
int rsSubStructFileTruncate(rsComm_t* rsComm, subFile_t* subFile)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsSubStructFileTruncate: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteSubStructFileTruncate(rsComm_t* rsComm, subFile_t* subFile, rodsServerHost_t* rodsServerHost)
{
    int status;

    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileTruncate: Invalid rodsServerHost");
...
int _rsSubStructFileTruncate(rsComm_t* _comm, subFile_t* _sub_file)
{
    // =-=-=-=-=-=-=-
