...
int rsSubStructFileStat(rsComm_t* rsComm, subFile_t* subFile, rodsStat_t** subStructFileStatOut)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        status = remoteSubStructFileStat(rsComm, subFile, subStructFileStatOut, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsSubStructFileStat: resolveHost returned unrecognized value %d", remoteFlag);
                            ...
                            rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileStat: Invalid rodsServerHost");
