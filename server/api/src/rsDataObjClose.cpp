...
int irsDataObjClose(rsComm_t* rsComm, openedDataObjInp_t* dataObjCloseInp, dataObjInfo_t** outDataObjInfo)
{
    ...
    if (l1descInx <= 2 || l1descInx >= NUM_L1_DESC) {
        rodsLog(LOG_NOTICE, "rsDataObjClose: l1descInx %d out of range", l1descInx);
        return SYS_FILE_DESC_OUT_OF_RANGE;
    }

    // ensure that l1 descriptor is in use before closing
    if (L1desc[l1descInx].inuseFlag != FD_INUSE) {
        rodsLog(LOG_ERROR, "rsDataObjClose: l1descInx %d out of range", l1descInx);
        return BAD_INPUT_DESC_INDEX;
    }

    // sanity check for in-flight l1 descriptor
    if (!L1desc[l1descInx].dataObjInp) {
        rodsLog(LOG_ERROR, "rsDataObjClose: invalid dataObjInp for index %d", l1descInx);
    ...
    if (L1desc[l1descInx].remoteZoneHost != NULL) {
        ...
        if (L1desc[l1descInx].lockFd > 0) {
            ...
            irods::server_api_call(DATA_OBJ_UNLOCK_AN, rsComm, L1desc[l1descInx].dataObjInp, NULL, (void**) NULL, NULL);
        ...
        if (status >= 0 && L1desc[l1descInx].oprStatus >= 0) {
            ...
            if (L1desc[l1descInx].openType == CREATE_TYPE) {
                ...
                rei.status = applyRule("acPostProcForCreate", NULL, &rei, NO_SAVE_REI);
                ...
                rei.status = applyRule("acPostProcForOpen", NULL, &rei, NO_SAVE_REI);
                /* doi might have changed */
                L1desc[l1descInx].dataObjInfo = rei.doi;
                // =-=-=-=-=-=-=-
                // JMC - bacport 4623
                ...
                rei.doi = L1desc[l1descInx].dataObjInfo;
                rei.status = status;
                rei.status = applyRule("acPostProcForRepl", NULL, &rei, NO_SAVE_REI);
                /* doi might have changed */
                L1desc[l1descInx].dataObjInfo = rei.doi;
                // =-=-=-=-=-=-=-
                clearKeyVal(rei.condInputData);
            ...
            if (L1desc[l1descInx].oprType == COPY_DEST) {
                ...
                rei.status = applyRule("acPostProcForCopy", NULL, &rei, NO_SAVE_REI);
                /* doi might have changed */
                L1desc[l1descInx].dataObjInfo = rei.doi;
                clearKeyVal(rei.condInputData);
                free(rei.condInputData);
            }
            else if (L1desc[l1descInx].oprType == PUT_OPR || L1desc[l1descInx].openType == CREATE_TYPE ||
                     (L1desc[l1descInx].openType == OPEN_FOR_WRITE_TYPE &&
                      (L1desc[l1descInx].bytesWritten > 0 || dataObjCloseInp->bytesWritten > 0))) {
                initReiWithDataObjInp(&rei, rsComm, L1desc[l1descInx].dataObjInp);
                rei.doi = L1desc[l1descInx].dataObjInfo;
                rei.status = status;
                ...
                rei.status = applyRule("acPostProcForPut", NULL, &rei, NO_SAVE_REI);
                /* doi might have changed */
                L1desc[l1descInx].dataObjInfo = rei.doi;
                clearKeyVal(rei.condInputData);
                free(rei.condInputData);
            }
            else if (L1desc[l1descInx].dataObjInp != NULL && L1desc[l1descInx].dataObjInp->oprType == PHYMV_OPR) {
                initReiWithDataObjInp(&rei, rsComm, L1desc[l1descInx].dataObjInp);
                rei.doi = L1desc[l1descInx].dataObjInfo;
                rei.status = status;
                rei.status = applyRule("acPostProcForPhymv", NULL, &rei, NO_SAVE_REI);
                /* doi might have changed */
                L1desc[l1descInx].dataObjInfo = rei.doi;
                clearKeyVal(rei.condInputData);
                free(rei.condInputData);
    ...
    if ((L1desc[l1descInx].oprType == REPLICATE_DEST || L1desc[l1descInx].oprType == COPY_DEST) && srcL1descInx > 2) {
...
int _modDataObjSize(rsComm_t* _comm, int _l1descInx, dataObjInfo_t* _info)
{
    ...
    if (getValByKey(&L1desc[_l1descInx].dataObjInp->condInput, ADMIN_KW) != NULL) {
    ...
    if (status < 0) {
        rodsLog(
            LOG_NOTICE, "_modDataObjSize: rsModDataObjMeta failed, dataSize [%d] status = %d", _info->dataSize, status);
    }
    return status;
}
...
int _rsDataObjClose(rsComm_t* rsComm, openedDataObjInp_t* dataObjCloseInp)
{
    ...
    if (l3descInx > 2) {
        ...
        if (status < 0) {
            rodsLog(LOG_NOTICE, "_rsDataObjClose: l3Close of %d failed, status = %d", l3descInx, status);
            return status;
        }
    }
    ...
    if (L1desc[l1descInx].oprStatus < 0) {
        ...
        if (L1desc[l1descInx].replStatus == NEWLY_CREATED_COPY) {
            if (L1desc[l1descInx].oprType == REPLICATE_OPR || L1desc[l1descInx].oprType == REPLICATE_DEST ||
        ...
        if (vault_size < 0) {
            rodsLog(LOG_ERROR, "_rsDataObjClose - getSizeInVault failed [%ld]", vault_size);
            return vault_size;
        }

        if (L1desc[l1descInx].dataObjInfo->dataSize != vault_size) {
            L1desc[l1descInx].dataObjInfo->dataSize = vault_size;
            int status = _modDataObjSize(rsComm, l1descInx, L1desc[l1descInx].dataObjInfo);
            if (status < 0) {
                rodsLog(LOG_ERROR, "_rsDataObjClose - _modDataObjSize failed [%d]", status);
                return status;
            }
        }
    ...
    if (dataObjCloseInp->bytesWritten > 0 && L1desc[l1descInx].bytesWritten <= 0) {
    ...
    if (L1desc[l1descInx].bytesWritten <= 0 && L1desc[l1descInx].oprType != REPLICATE_DEST &&
        L1desc[l1descInx].oprType != PHYMV_DEST && L1desc[l1descInx].oprType != COPY_DEST) {
        /* no write */
        // =-=-=-=-=-=-=-
        // JMC - backport 4537
        if (L1desc[l1descInx].purgeCacheFlag > 0) {
            int status1 = trimDataObjInfo(rsComm, L1desc[l1descInx].dataObjInfo);
            if (status1 < 0) {
    ...
    if ((OPEN_FOR_WRITE_TYPE == L1desc[l1descInx].openType || CREATE_TYPE == L1desc[l1descInx].openType) &&
    ...
    if (L1desc[l1descInx].oprType == PHYMV_DEST) {
        ...
        srcL1descInx = L1desc[l1descInx].srcL1descInx;
        if (srcL1descInx <= 2) {
            rodsLog(LOG_NOTICE, "_rsDataObjClose: srcL1descInx %d out of range", srcL1descInx);
            return SYS_FILE_DESC_OUT_OF_RANGE;
        }
        ...
        if (getValByKey(&L1desc[l1descInx].dataObjInp->condInput, ADMIN_KW) != NULL) {
            addKeyVal(&regParam, ADMIN_KW, "");
        }
        ...
        if (status >= 0) {
            ...
            updatequotaOverrun(destDataObjInfo->rescHier, destDataObjInfo->dataSize, RESC_QUOTA);
        }
        else {
            if (L1desc[srcL1descInx].l3descInx > 2) {
        ...
        if (L1desc[l1descInx].replStatus & OPEN_EXISTING_COPY) {
            ...
            if (getValByKey(&L1desc[l1descInx].dataObjInp->condInput, SU_CLIENT_USER_KW) != NULL) {
                addKeyVal(&regReplicaInp.condInput, SU_CLIENT_USER_KW, "");
                addKeyVal(&regReplicaInp.condInput, ADMIN_KW, "");
            }
            else if (getValByKey(&L1desc[l1descInx].dataObjInp->condInput, ADMIN_KW) != NULL) {
                addKeyVal(&regReplicaInp.condInput, ADMIN_KW, "");
            }
            ...
            if (srcDataObjInfo->dataSize != newSize) {
                ...
                if (status < 0) {
                    rodsLog(
                        LOG_NOTICE, "_rsDataObjClose: _modDataObjSize srcDataObjInfo failed, status = [%d]", status);
                    return status;
                }
            }
            if (destDataObjInfo->dataSize != newSize) {
                ...
                if (status < 0) {
                    rodsLog(
                        LOG_NOTICE, "_rsDataObjClose: _modDataObjSize destDataObjInfo failed, status = [%d]", status);
                    return status;
                }
            }
        ...
        /* put or copy */
        if (l3descInx < 2 && getValByKey(&L1desc[l1descInx].dataObjInp->condInput, CROSS_ZONE_CREATE_KW) != NULL &&
        ...
        if (L1desc[l1descInx].replStatus == NEWLY_CREATED_COPY) {
            /* update quota overrun */
            updatequotaOverrun(L1desc[l1descInx].dataObjInfo->rescHier, newSize, ALL_QUOTA);
        }
    }
    ...
    /* for post processing */
    L1desc[l1descInx].bytesWritten = L1desc[l1descInx].dataObjInfo->dataSize = newSize;
...
int procChksumForClose(rsComm_t* rsComm, int l1descInx, char** chksumStr)
{
    ...
    *chksumStr = NULL;
    if (oprType == REPLICATE_DEST || oprType == PHYMV_DEST) {
        srcL1descInx = L1desc[l1descInx].srcL1descInx;
        if (srcL1descInx <= 2) {
            rodsLog(LOG_NOTICE, "procChksumForClose: srcL1descInx %d out of range", srcL1descInx);
            return SYS_FILE_DESC_OUT_OF_RANGE;
        }
        ...
        if (strlen(srcDataObjInfo->chksum) > 0 && srcDataObjInfo->replStatus > 0) {
    ...
    if (L1desc[l1descInx].chksumFlag == 0) {
        ...
        if (strlen(L1desc[l1descInx].chksum) > 0) {
            ...
            if (srcL1descInx <= 2) {
                ...
                rodsLog(LOG_DEBUG, "procChksumForClose: invalid srcL1descInx %d for copy", srcL1descInx);
