...
int rsSubStructFileOpendir(rsComm_t* rsComm, subFile_t* subFile)
{
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        if (remoteFlag < 0) {
            ...
            rodsLog(LOG_NOTICE, "rsSubStructFileOpendir: resolveHost returned unrecognized value %d", remoteFlag);
...
int remoteSubStructFileOpendir(rsComm_t* rsComm, subFile_t* subFile, rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteSubStructFileOpendir: Invalid rodsServerHost");
...
int _rsSubStructFileOpendir(rsComm_t* _comm, subFile_t* _sub_file)
{
    // =-=-=-=-=-=-=-
