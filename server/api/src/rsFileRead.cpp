...
int rsFileRead(rsComm_t* rsComm, fileReadInp_t* fileReadInp, bytesBuf_t* fileReadOutBBuf)
{
    ...
    remoteFlag = getServerHostByFileInx(fileReadInp->fileInx, &rodsServerHost);
    ...
    if (remoteFlag == LOCAL_HOST) {
        ...
        retVal = remoteFileRead(rsComm, fileReadInp, fileReadOutBBuf, rodsServerHost);
    }
    else {
        if (remoteFlag < 0) {
            return remoteFlag;
            ...
            rodsLog(LOG_NOTICE, "rsFileRead: resolveHost returned unrecognized value %d", remoteFlag);
                   ...
                   rodsServerHost_t* rodsServerHost)
{
    ...
    if (rodsServerHost == NULL) {
        rodsLog(LOG_NOTICE, "remoteFileRead: Invalid rodsServerHost");
    ...
    retVal = rcFileRead(rodsServerHost->conn, fileReadInp, fileReadOutBBuf);

    if (retVal < 0) {
        rodsLog(LOG_NOTICE, "remoteFileRead: rcFileRead failed for %s", FileDesc[fileReadInp->fileInx].fileName);
    }
...
int _rsFileRead(rsComm_t* _comm, fileReadInp_t* _read_inp, bytesBuf_t* _read_bbuf)
{
    ...
    irods::error ret = fileRead(_comm, file_obj, _read_bbuf->buf, _read_inp->len);
    // =-=-=-=-=-=-=
