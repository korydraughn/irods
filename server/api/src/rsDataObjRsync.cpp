...
int rsDataObjRsync(rsComm_t* rsComm, dataObjInp_t* dataObjInp, msParamArray_t** outParamArray)
{
    ...
    *outParamArray = NULL;
    ...
    if (rsyncMode == NULL) {
        rodsLog(LOG_ERROR, "rsDataObjRsync: RSYNC_MODE_KW input is missing");
    ...
    if (strcmp(rsyncMode, IRODS_TO_IRODS) == 0) {
        if (isLocalZone(dataObjInp->objPath) == 0) {
            ...
            if (destObjPath == NULL) {
                rodsLog(LOG_ERROR, "rsDataObjRsync: RSYNC_DEST_PATH_KW input is missing for %s", dataObjInp->objPath);
                return USER_RSYNC_NO_MODE_INPUT_ERR;
            }
            myDataObjInp = *dataObjInp;
            remoteZoneOpr = REMOTE_CREATE;
            rstrcpy(myDataObjInp.objPath, destObjPath, MAX_NAME_LEN);
            remoteFlag = getAndConnRemoteZone(rsComm, &myDataObjInp, &rodsServerHost, remoteZoneOpr);
        }
        else {
            remoteFlag = getAndConnRemoteZone(rsComm, dataObjInp, &rodsServerHost, remoteZoneOpr);
        }
    }
    else {
        remoteFlag = getAndConnRemoteZone(rsComm, dataObjInp, &rodsServerHost, remoteZoneOpr);
    ...
    if (remoteFlag < 0) {
        ...
        status = _rcDataObjRsync(rodsServerHost->conn, dataObjInp, outParamArray);
        return status;
    }
    ...
    if (strcmp(rsyncMode, IRODS_TO_LOCAL) == 0) {
        ...
        rodsLog(LOG_ERROR, "rsDataObjRsync: rsyncMode %s  not supported");
        return USER_RSYNC_NO_MODE_INPUT_ERR;
    }
...
int rsRsyncDataToFile(rsComm_t* rsComm, dataObjInp_t* dataObjInp)
{
    ...
    if (fileChksumStr == NULL) {
        rodsLog(LOG_ERROR, "rsRsyncDataToFile: RSYNC_CHKSUM_KW input is missing for %s", dataObjInp->objPath);
        return CHKSUM_EMPTY_IN_STRUCT_ERR;
    }

    addKeyVal(&dataObjInp->condInput, ORIG_CHKSUM_KW, fileChksumStr);

    status = _rsDataObjChksum(rsComm, dataObjInp, &dataObjChksumStr, &dataObjInfoHead);
    ...
    if (status < 0 && status != CAT_NO_ACCESS_PERMISSION && status != CAT_NO_ROWS_FOUND) {
         ...
         */
        rodsLog(LOG_ERROR, "rsRsyncDataToFile: _rsDataObjChksum of %s error. status = %d", dataObjInp->objPath, status);
        return status;
    }

    freeAllDataObjInfo(dataObjInfoHead);
    status = dataObjChksumStr && strcmp(dataObjChksumStr, fileChksumStr) == 0 ? 0 : SYS_SVR_TO_CLI_GET_ACTION;
...
int rsRsyncFileToData(rsComm_t* rsComm, dataObjInp_t* dataObjInp)
{
    ...
    if (fileChksumStr == NULL) {
        rodsLog(LOG_ERROR, "rsRsyncFileToData: RSYNC_CHKSUM_KW input is missing");
        return CHKSUM_EMPTY_IN_STRUCT_ERR;
    }
    ...
    char* dataObjChksumStr = NULL;
    dataObjInfo_t* dataObjInfoHead = NULL;
    int status = _rsDataObjChksum(rsComm, dataObjInp, &dataObjChksumStr, &dataObjInfoHead);

    if (status < 0 && status != CAT_NO_ACCESS_PERMISSION && status != CAT_NO_ROWS_FOUND) {
        /* XXXXX CAT_NO_ACCESS_PERMISSION mean the chksum was calculated but
         * cannot be registered. But the chksum value is OK.
         */
        rodsLog(LOG_ERROR, "rsRsyncFileToData: _rsDataObjChksum of %s error. status = %d", dataObjInp->objPath, status);
    }

    freeAllDataObjInfo(dataObjInfoHead);
    if (dataObjChksumStr != NULL && strcmp(dataObjChksumStr, fileChksumStr) == 0) {
        free(dataObjChksumStr);
        return 0;
    }
    free(dataObjChksumStr);
...
int rsRsyncDataToData(rsComm_t* rsComm, dataObjInp_t* dataObjInp)
{
    ...
    destObjPath = getValByKey(&dataObjInp->condInput, RSYNC_DEST_PATH_KW);
    if (destObjPath == NULL) {
        rodsLog(LOG_ERROR, "rsRsyncDataToData: RSYNC_DEST_PATH_KW input is missing for %s", dataObjInp->objPath);
        return USER_RSYNC_NO_MODE_INPUT_ERR;
    }

    memset(&dataObjCopyInp, 0, sizeof(dataObjCopyInp));
    rstrcpy(dataObjCopyInp.srcDataObjInp.objPath, dataObjInp->objPath, MAX_NAME_LEN);
    dataObjCopyInp.srcDataObjInp.dataSize = dataObjInp->dataSize;
    replDataObjInp(dataObjInp, &dataObjCopyInp.destDataObjInp);
    rstrcpy(dataObjCopyInp.destDataObjInp.objPath, destObjPath, MAX_NAME_LEN);
    ...
    status = rsDataObjChksum(rsComm, &dataObjCopyInp.srcDataObjInp, &srcChksumStr);

    if (status < 0 && (status != CAT_NO_ACCESS_PERMISSION || srcChksumStr == NULL)) {
        /* XXXXX CAT_NO_ACCESS_PERMISSION mean the chksum was calculated but
         * cannot be registered. But the chksum value is OK.
         */
        rodsLog(LOG_ERROR,
                "rsRsyncDataToData: _rsDataObjChksum error for %s, status = %d",
                dataObjCopyInp.srcDataObjInp.objPath,
                status);
    ...
    addKeyVal(&dataObjCopyInp.destDataObjInp.condInput, REG_CHKSUM_KW, srcChksumStr);
