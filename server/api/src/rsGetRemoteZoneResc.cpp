...
int rsGetRemoteZoneResc(rsComm_t* rsComm, dataObjInp_t* dataObjInp, rodsHostAddr_t** rescAddr)
{
    ...
    char* remoteOprType = getValByKey(&dataObjInp->condInput, REMOTE_ZONE_OPR_KW);
    if (!remoteOprType) {
        rodsLog(LOG_ERROR, "rsGetRemoteZoneResc: REMOTE_ZONE_OPR_KW not defined for %s", dataObjInp->objPath);
    ...
    std::string hier;
    err = irods::resolve_resource_hierarchy(oper, rsComm, dataObjInp, hier);
    if (!err.ok()) {
    ...
    if (do_sort) {
        ...
        if (status < 0) {
            rodsLog(LOG_ERROR, "rsGetRemoteZoneResc - sortObjInfoForOpen failed for [%s]", dataObjInp->objPath);
            return status;
        }
