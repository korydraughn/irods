...
int rsUserAdmin(rsComm_t* rsComm, userAdminInp_t* userAdminInp)
{
    ...
    if (rodsServerHost->localFlag == LOCAL_HOST) {
        ...
        status = rcUserAdmin(rodsServerHost->conn, userAdminInp);
    }

    if (status < 0) {
        rodsLog(LOG_NOTICE, "rsUserAdmin: rcUserAdmin failed");
    }
    return status;
}
...
int _rsUserAdmin(rsComm_t* rsComm, userAdminInp_t* userAdminInp)
{
    ...
    rodsLog(LOG_DEBUG, "_rsUserAdmin arg0=%s", userAdminInp->arg0);
    ...
    if (strcmp(userAdminInp->arg0, "userpw") == 0) {
        ...
        argc = 3;
        status2 = applyRuleArg("acPreProcForModifyUser", args, argc, &rei2, NO_SAVE_REI);
        if (status2 < 0) {
            if (rei2.status < 0) {
                status2 = rei2.status;
            }
            rodsLog(LOG_ERROR,
                    "_rsUserAdmin: acPreProcForModifyUser failed [%s] [%s] [%d]",
                    userAdminInp->arg1,
                    userAdminInp->arg2,
                    status2);
            return status2;
        ...
        status2 = applyRuleArg("acPostProcForModifyUser", args, argc, &rei2, NO_SAVE_REI);
        if (status2 < 0) {
            if (rei2.status < 0) {
                status2 = rei2.status;
            }
    ...
    if (strcmp(userAdminInp->arg0, "modify") == 0) {
        if (strcmp(userAdminInp->arg1, "group") == 0) {
            ...
            argc = 4;
            status2 = applyRuleArg("acPreProcForModifyUserGroup", args, argc, &rei2, NO_SAVE_REI);
            if (status2 < 0) {
                if (rei2.status < 0) {
                    status2 = rei2.status;
                }
            ...
            status =
                chlModGroup(rsComm, userAdminInp->arg2, userAdminInp->arg3, userAdminInp->arg4, userAdminInp->arg5);
            if (status == 0) {
                status2 = applyRuleArg("acPostProcForModifyUserGroup", args, argc, &rei2, NO_SAVE_REI);
                if (status2 < 0) {
                    if (rei2.status < 0) {
                        status2 = rei2.status;
                    }
    ...
    if (strcmp(userAdminInp->arg0, "mkuser") == 0) {
        ...
        rei.rsComm = rsComm;
        snprintf(userInfo.userName, sizeof(userInfo.userName), "%s", userAdminInp->arg1);
        snprintf(userInfo.userType, sizeof(userInfo.userType), "%s", "rodsuser");
        rei.uoio = &userInfo;
        ...
        status = chlModUser(rsComm, userAdminInp->arg1, "password", userAdminInp->arg2);
        if (status != 0) {
            chlRollback(rsComm);
        }
        return status;
    ...
    if (strcmp(userAdminInp->arg0, "mkgroup") == 0) {
        ...
        rei.rsComm = rsComm;
        snprintf(userInfo.userName, sizeof(userInfo.userName), "%s", userAdminInp->arg1);
        snprintf(userInfo.userType, sizeof(userInfo.userType), "%s", userAdminInp->arg2);
        snprintf(userInfo.rodsZone, sizeof(userInfo.rodsZone), "%s", userAdminInp->arg3);
        snprintf(userInfo.authInfo.authStr, sizeof(userInfo.authInfo.authStr), "%s", userAdminInp->arg4);
        rei.uoio = &userInfo;
        rei.uoic = &rsComm->clientUser;
        rei.uoip = &rsComm->proxyUser;
        status = applyRuleArg("acCreateUser", NULL, 0, &rei, SAVE_REI);
...
int icatApplyRule(rsComm_t* rsComm, char* ruleName, char* arg1)
{
    ...
    rei.uoic = &rsComm->clientUser;
    rei.uoip = &rsComm->proxyUser;
    status = applyRuleArg(ruleName, args, 1, &rei, NO_SAVE_REI);
    return status;
}
