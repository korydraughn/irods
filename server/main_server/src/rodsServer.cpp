...
namespace
{
    ...
    void set_grid_configuration_option_value(RcComm& _comm,
                                             const std::string_view _namespace,
                                             const std::string_view _option_name,
                                             const std::string_view _option_value)
    {
        using log = irods::experimental::log::server;
        ...
        std::strcpy(input.name_space, _namespace.data());
        std::strcpy(input.option_name, _option_name.data());
        std::strcpy(input.option_value, _option_value.data());

        if (const auto ec = rc_set_grid_configuration_value(&_comm, &input); ec != 0) {
            log::error("Failed to set option value in r_grid_configuration "
                       "[error_code={}, namespace={}, option_name={}, option_value={}].",
                       ec, _namespace, _option_name, _option_value);
        }
    } // set_grid_configuration_option_value
    ...
    std::optional<bool> is_delay_server_running_on_remote_host(const std::string_view _hostname)
    {
        ...
        try {
            ...
            return grid_status.at("re_server_pid").get<int>() > 0;
        }
