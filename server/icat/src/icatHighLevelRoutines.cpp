...
auto chl_delay_rule_tag(RsComm& _comm, const char* _rule_id, const char* _tag) -> int
{
    ...
    const auto ret = db->call(&_comm, irods::DATABASE_OP_DELAY_RULE_TAG, ptr, _rule_id, _tag);
...
auto chl_delay_rule_tag_clear(RsComm& _comm, const char* _rule_id) -> int
{
    ...
    const auto ret = db->call(&_comm, irods::DATABASE_OP_DELAY_RULE_TAG_CLEAR, ptr, _rule_id);

    // NOLINTNEXTLINE(bugprone-narrowing-conversions,cppcoreguidelines-narrowing-conversions)
    return ret.code();
} // chl_delay_rule_tag_clear
