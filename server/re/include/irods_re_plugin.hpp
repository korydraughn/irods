...
namespace irods
{
     ...
     * Create rule_engine_context_manager
            rule_engine_context_manager<unit, ruleExecInfo_t*, AUDIT_RULE> re_ctx_mgr =
     rule_engine_context_manager<unit, ruleExecInfo_t*, AUDIT_RULE>(global_re_mgr, &rei);

     * to call any operation with dynamic pep
            dynamic_operation_execution_manager<T,C,DONT_AUDIT_RULE> rex_mgr
     (std::shared_ptr<rule_engine_context_manager<unit,ruleExecInfo_t*,DONT_AUDIT_RULE> > (new
     rule_engine_context_manager<unit,ruleExecInfo_t*,DONT_AUDIT_RULE >(global_re_mgr, &rei))); rex_mgr.call("instance",
     "op", op, args...);
    ...
    */
    ...
    // the unpack constructor tell pluggable rule engine interface that the intention is to unpack the packed parameter
    // list and call another rule when the rule engine interface tries to pack the parameters again, it will instead
    // pass them on to the plugin
    class unpack
    {
    public:
    ...
    // serialize wraps a function so that it can be called at runtime wihtout prior knowledge of original parameter
    // types only when the parameter type matches the oringal does the call succeed
    ...
    public:
        ...
        error operator()(Bs&&... _ps)
        {
            if (typeid(std::function<error(Bs...)>) == op_.type()) {
                ...
                return ERROR(RE_TYPE_ERROR,
                             std::string("operation ") + op_name_ + " called with wrong types of arguments");
            }
        }
    ...
    public:
        ...
        error start_operation(T& _in)
        {
            try {
                ...
                return ERROR(
                    INVALID_ANY_CAST,
                    boost::format("failed to extract start operation from instance [%s] exception message [%s]") %
                        instance_name_ % e.what());
            }
        }
        ...
        error stop_operation(T& _in)
        {
            try {
                ...
                return ERROR(
                    INVALID_ANY_CAST,
                    boost::format("failed to extract stop operation from instance [%s] exception message [%s]") %
                        instance_name_ % e.what());
            }
        }
        ...
        error rule_exists(const std::string& _rn, T& _re_ctx, bool& _out)
        {
            try {
                auto fcn =
                    boost::any_cast<std::function<error(T&, const std::string&, bool&)>>(operations_["rule_exists"]);
                return fcn(_re_ctx, _rn, _out);
            }
            catch (const boost::bad_any_cast& e) {
                return ERROR(
                    INVALID_ANY_CAST,
                    boost::format("failed to extract rule_exists operation from instance [%s] exception message [%s]") %
                        instance_name_ % e.what());
            }
        }
        ...
        error list_rules(T& _re_ctx, std::vector<std::string>& _rule_vec)
        {
            try {
                auto fcn =
                    boost::any_cast<std::function<error(T&, std::vector<std::string>&)>>(operations_["list_rules"]);
                return fcn(_re_ctx, _rule_vec);
            }
            catch (const boost::bad_any_cast& e) {
                return ERROR(
                    INVALID_ANY_CAST,
                    boost::format("failed to extract list_rules operation from instance [%s] exception message [%s]") %
                        instance_name_ % e.what());
            }
        }
        ...
        error exec_rule(const std::string& _rn, T& _re_ctx, As&&... _ps, callback _callback)
        {
            try {
                ...
                auto fcn =
                    boost::any_cast<std::function<error(T&, const std::string&, std::list<boost::any>&, callback)>>(
                        operations_["exec_rule"]);
                return fcn(_re_ctx, _rn, l, _callback);
            }
            catch (const boost::bad_any_cast& e) {
                return ERROR(
                    INVALID_ANY_CAST,
                    boost::format("failed to extract exec_rule operation from instance [%s] exception message [%s]") %
                        instance_name_ % e.what());
            }
        }

        error exec_rule_text(T& _re_ctx,
                             ...
                             callback _callback)
        {
            try {
                ...
                return ERROR(
                    INVALID_ANY_CAST,
                    boost::format(
                        "failed to extract exec_rule_text operation from instance [%s] exception message [%s]") %
                        instance_name_ % e.what());
            }
        }
        ...
        {
            try {
                auto fcn = boost::any_cast<std::function<error(T&, const std::string&, msParamArray_t*, callback)>>(
                ...
                return ERROR(
                    INVALID_ANY_CAST,
                    boost::format(
                        "failed to extract exec_rule_expression operation from instance [%s] exception message [%s]") %
                        instance_name_ % e.what());
            }
        }
    ...
    public:
        ...
        error exec_rule_text(const std::string& _instance_name,
        ...
        error exec_rule_expression(const std::string& _instance_name,
                                   const std::string& _rule_text,
                                   msParamArray_t* _ms_params)
        {
        ...
        error call(std::string _instance_name, std::string _operation_name, OP _operation, As&&... _ps)
        {
            ...
            rodsLog(LOG_DEBUG, "exec_op [%s]", _operation_name.c_str());

            bool execOnce = false;
            std::function<error()> op = [&execOnce, &_ps..., &_operation]() {
                if (execOnce) {
                    return ERROR(RE_TYPE_ERROR,
                                 "cannot execute operation more than once with client supplied arguments");
                }
                else {
                    execOnce = true;
    ...
    public:
        ...
        {
            for (auto itr = begin(re_plugin_map_); itr != end(re_plugin_map_); ++itr)
                delete itr->second;
        }
        ...
        error resolve(std::string& _plugin_name, const std::string& _inst_name, pluggable_rule_engine<T>*& _re_ptr)
        {
            ...
            if (itr == end(re_plugin_map_)) {
                error err = load_plugin<pluggable_rule_engine<T>>(
                    _re_ptr, _plugin_name, dir_, _inst_name, std::string("empty_context"));
                if (!err.ok()) {
                    irods::log(PASS(err));
                    return err;
                }
    ...
    {
    public:
        rule_engine_manager(rule_engine_plugin_manager<T>& _re_plugin_mgr,
                            std::vector<re_pack_inp<T>>& _re_packs,
                            microservice_manager<C>& _ms_mgr)
            : ms_mgr_(_ms_mgr)
            , re_plugin_mgr_(_re_plugin_mgr)
        {
        ...
        {
            std::for_each(
                begin(re_packs_), end(re_packs_), [](re_pack_inp<T>& _inp) { _inp.re_->stop_operation(_inp.re_ctx_); });
        }
            ...
            auto er = [this](re_pack_inp<T>& _itr, const std::string& _rn, decltype(_ps)... _ps) {
                std::function<error(const std::string&, re_pack_inp<T>&, decltype(_ps)...)> fun =
                    [this](const std::string& _rn, re_pack_inp<T>& _itr, decltype(_ps)... _ps) {
                        return _itr.re_->template exec_rule<As...>(
                            _rn, _itr.re_ctx_, std::forward<As>(_ps)..., callback(*this));
                    };
                return this->rex_mgr_.call(
                    _itr.instance_name_, std::string("exec_rule"), fun, _rn, _itr, std::forward<As>(_ps)...);
            };
    ...
    public:
        ...
        error list_rules(std::vector<std::string>& rule_vec)
        {
            for (auto itr = begin(this->re_mgr_.re_packs_); itr != end(this->re_mgr_.re_packs_); ++itr) {
...
#define UNIT unit()
    ...
    public:
        ...
        error exec_microservice(std::string _msn, default_ms_ctx _ctx, As&&... _ps)
        {
            // =-=-=-=-=-=-=-
            // debug message for creating dynPEP rules
            rodsLog(LOG_DEBUG, "microservice_manager::exec_microservice_no_audit [%s]", _msn.c_str());
    ...
    error callback::operator()(const std::string& _rn, As&&... _ps)
    {
        if (callback_hdlr_.type() == typeid(rule_engine_context_manager<default_re_ctx, default_ms_ctx, AUDIT_RULE>*)) {
            auto cb = boost::any_cast<rule_engine_context_manager<default_re_ctx, default_ms_ctx, AUDIT_RULE>*>(
                callback_hdlr_);
            return cb->exec_rule(_rn, std::forward<As>(_ps)...);
        }
        else if (callback_hdlr_.type() ==
                 typeid(rule_engine_context_manager<default_re_ctx, default_ms_ctx, DONT_AUDIT_RULE>*)) {
            auto cb = boost::any_cast<rule_engine_context_manager<default_re_ctx, default_ms_ctx, DONT_AUDIT_RULE>*>(
                callback_hdlr_);
            return cb->exec_rule(_rn, std::forward<As>(_ps)...);
        }
        else {
    ...
    T any_extract(boost::any _p, std::function<T()> f)
    {
        if (_p.type() == typeid(T*)) {
            ...
            // rodsLog(LOG_ERROR, (std::string("cannot convert type ") + _p.type().name() + " to " +
            // typeid(T).name()).c_str());
            return f();
        }
    }
    ...
    struct global_re_plugin_mgr
    {
        ...
        rule_engine_plugin_manager<default_re_ctx> global_re_plugin_mgr =
            rule_engine_plugin_manager<default_re_ctx>(PLUGIN_TYPE_RULE_ENGINE);
        rule_engine_manager<default_re_ctx, default_ms_ctx> global_re_mgr =
            rule_engine_manager<default_re_ctx, default_ms_ctx>(global_re_plugin_mgr, global_re_packs, global_ms_mgr);
    };
