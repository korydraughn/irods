...
/**
 * \fn msiGetTaggedValueFromString (msParam_t *inTagParam, msParam_t *inStrParam, msParam_t *outValueParam,
 *ruleExecInfo_t *)
 *
 * \brief   This microservice gets a tagged value from a string; given a tag-name gets the value from a file in
 *tagged-format (pseudo-XML).
 *
 * \module core
 *
 * \since pre-2.1
 *
 *
 * \note This performs some regular expression matching. Given a regular expression as a tag-value 't', it identifies
 *the corresponding string in the match string with a string that matches a sub-string value: '\<t\>.*\</t\>'. The
 *service is used for processing a tagged structure.
 *
 * \usage See clients/icommands/test/rules/
 *
...
/**
 * \fn msiExtractTemplateMDFromBuf(msParam_t* bufParam, msParam_t* tagParam, msParam_t *metadataParam, ruleExecInfo_t
 **rei)
 *
 ...
 **/
int msiAssociateKeyValuePairsToObj(msParam_t* metadataParam,
                                   msParam_t* objParam,
                                   msParam_t* typeParam,
                                   ruleExecInfo_t* rei)
{
    ...
    i = addAVUMetadataFromKVPairs(rei->rsComm, objName, objType, (keyValPair_t*) metadataParam->inOutStruct);
    return i;
}
...
int msiSetKeyValuePairsToObj(msParam_t* metadataParam, msParam_t* objParam, msParam_t* typeParam, ruleExecInfo_t* rei)
{
    ...
    objName = (char*) objParam->inOutStruct;
    objType = (char*) typeParam->inOutStruct;
    ret = setAVUMetadataFromKVPairs(rei->rsComm, objName, objType, (keyValPair_t*) metadataParam->inOutStruct);
    return ret;
}
// =-=-=-=-=-=-=-
                                  ...
                                  ruleExecInfo_t* rei)
{
    ...
    objName = (char*) objParam->inOutStruct;
    objType = (char*) typeParam->inOutStruct;
    i = removeAVUMetadataFromKVPairs(rei->rsComm, objName, objType, (keyValPair_t*) metadataParam->inOutStruct);
    return i;
}
