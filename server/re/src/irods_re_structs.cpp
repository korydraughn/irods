...
int applyRule(char* inAction, msParamArray_t* inMsParamArray, ruleExecInfo_t* rei, int reiSaveFlag)
{
    ...
    if (!err.ok()) {
        rodsLog(LOG_ERROR, "applyRule: %d, %s", err.code(), err.result().c_str());
    }
    return err.code();
}
                 ...
                 int reiSaveFlag)
{
    ...
    if (!err.ok()) {
        rodsLog(LOG_ERROR, "applyRuleArg: %d, %s", err.code(), err.result().c_str());
    }
    return err.code();
}
...
int applyRuleWithInOutVars(const char* _action, std::list<boost::any>& _params, ruleExecInfo_t* _rei)
{
    ...
    if (!err.ok()) {
        rodsLog(LOG_ERROR, "applyRuleWithInOutVars: %d, %s", err.code(), err.result().c_str());
    }
    return err.code();
}
...
int initReiWithCollInp(ruleExecInfo_t* rei, rsComm_t* rsComm, collInp_t* collCreateInp, collInfo_t* collInfo)
{
    ...
    if ((status = splitPathByKey(
             collCreateInp->collName, collInfo->collParentName, MAX_NAME_LEN, collInfo->collName, MAX_NAME_LEN, '/')) <
        0) {
        rodsLog(
            LOG_ERROR, "initReiWithCollInp: splitPathByKey for %s error, status = %d", collCreateInp->collName, status);
        return status;
    }
    else {
...
int unpackRei(rsComm_t* rsComm, ruleExecInfo_t** rei, bytesBuf_t* packedReiBBuf)
{
    ...
    status = unpackStruct(packedReiBBuf->buf, (void**) rei, "Rei_PI", RodsPackTable, NATIVE_PROT);
...
{
    if (!rei) {
        rodsLog(LOG_ERROR, "packReiAndArg: NULL rei input");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }

    if (!packedReiAndArgBBuf) {
        rodsLog(LOG_ERROR, "packReiAndArg: NULL packedReiAndArgBBuf input");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }

    if (myArgc > 0 && (myArgv == NULL || *myArgv == NULL)) {
        rodsLog(LOG_ERROR, "packReiAndArg: NULL myArgv input");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }

    ruleExecInfoAndArg_t reiAndArg{.rei = rei, .reArg = {.myArgc = myArgc, .myArgv = myArgv}};

    /* pack the reiAndArg */

    int status = packStruct((void*) &reiAndArg, packedReiAndArgBBuf, "ReiAndArg_PI", RodsPackTable, 0, NATIVE_PROT);

    if (status < 0) {
        rodsLog(LOG_ERROR, "packReiAndArg: packStruct error. status = %d", status);
        return status;
    }
...
int unpackReiAndArg(rsComm_t* rsComm, ruleExecInfoAndArg_t** reiAndArg, bytesBuf_t* packedReiAndArgBBuf)
{
    ...
    status = unpackStruct(packedReiAndArgBBuf->buf, (void**) reiAndArg, "ReiAndArg_PI", RodsPackTable, NATIVE_PROT);

    if (status < 0) {
        rodsLog(LOG_ERROR, "unpackReiAndArg: unpackStruct error. status = %d", status);
        return status;
    }
...
std::map<std::string, std::vector<std::string>> getTaggedValues(const char* str)
{
    ...
    while ((str = strchr(str, '<'))) {
        ...
        if (str[1] == '/') {
            ...
            if (last_tag.size() != static_cast<std::size_t>(tag_end - str - 2) ||
                strncmp(last_tag.c_str(), str + 2, last_tag.size()) != 0) {
                         ...
                         ruleExecInfo_t* rei)
{
    ...
    if (strlen(ruleSubmitInfo->userName) == 0) {
        if (strlen(rei->uoic->userName) != 0) {
            snprintf(ruleSubmitInfo->userName, sizeof(ruleSubmitInfo->userName), "%s", rei->uoic->userName);
        }
        else if (strlen(rei->rsComm->clientUser.userName) != 0) {
            snprintf(
                rei->rsComm->clientUser.userName, sizeof(rei->rsComm->clientUser.userName), "%s", rei->uoic->userName);
        }
    }
    return 0;
}
