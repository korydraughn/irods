...
namespace irods
{
    ...
    unpack::unpack(std::list<boost::any>& _l)
        : l_(_l){};

    std::vector<re_pack_inp<default_re_ctx>> init_global_re_packs()
    {
        std::vector<re_pack_inp<default_re_ctx>> ret;
        const auto& re_plugin_configs = irods::get_server_property<const std::vector<boost::any>>(
        ...
        for (const auto& el : re_plugin_configs) {
            ...
            ret.emplace_back(boost::any_cast<const std::string&>(map.at("instance_name")),
