...
int msiXmsgServerConnect(msParam_t* outConnParam, ruleExecInfo_t* rei)
{
    ...
    if (conn == NULL) {
        rodsLog(LOG_ERROR, "msiXmsgServerConnect: rcConnectXmsg failed:%i :%s\n", errMsg.status, errMsg.msg);
...
/**
 * \fn msiXmsgCreateStream(msParam_t* inConnParam, msParam_t* inGgetXmsgTicketInpParam, msParam_t*
 *outXmsgTicketInfoParam, ruleExecInfo_t *rei)
 *
 ...
 * \usage See clients/icommands/test/rules/
 *
 * \param[in] inConnParam - a msParam of type RcComm_MS_T which is a connection descriptor obtained by
 *msiXmsgServerConnect. \param[in] inGgetXmsgTicketInpParam - a msParam of type GetXmsgTicketInp_MS_T which is actually
 *an integer giving expiration time. \param[out] outXmsgTicketInfoParam - a msParam of type XmsgTicketInfo_MS_T which is
 *an information struct for the ticket generated for this stream. \param[in,out] rei - The RuleExecInfo structure that
 *is automatically handled by the rule engine. The user does not include rei as a parameter in the rule invocation.
 *
 * \DolVarDependence none
 * \DolVarModified none
 * \iCatAttrDependence none
                        ...
                        ruleExecInfo_t* rei)
{
    ...
    if (inConnParam->inOutStruct == NULL) {
        rodsLog(LOG_ERROR, "msiXmsgCreateStream: input inConnParam is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    conn = (rcComm_t*) inConnParam->inOutStruct;

    if (inGgetXmsgTicketInpParam->inOutStruct != NULL &&
        !(strcmp(inGgetXmsgTicketInpParam->type, STR_MS_T) == 0 &&
          strcmp((char*) inGgetXmsgTicketInpParam->inOutStruct, "") == 0)) {
...
/**
 * \fn msiCreateXmsgInp(msParam_t* inMsgNumber, msParam_t* inMsgType, msParam_t* inNumberOfReceivers, msParam_t* inMsg,
 *msParam_t* inNumberOfDeliverySites, msParam_t* inDeliveryAddressList, msParam_t* inDeliveryPortList, msParam_t*
 *inMiscInfo, msParam_t* inXmsgTicketInfoParam, msParam_t* outSendXmsgInpParam, ruleExecInfo_t *)
 *
 ...
 * \param[in] inMsgType - a msParam of type uint or STR_MS_T which is currently 0 (SINGLE_MSG_TICKET) or 1
 *(MULTI_MSG_TICKET). \param[in] inNumberOfReceivers - a msParam of type uint or STR_MS_T which is a number of receivers
 *of the message. \param[in] inMsg - a msParam of type  STR_MS_T which is a message body. \param[in]
 *inNumberOfDeliverySites - a msParam of type int or STR_MS_T which is a Number of Receiving Addresses. \param[in]
 *inDeliveryAddressList - a msParam of type STR_MS_T which is a List of Host Addresses (comma separated). \param[in]
 *inDeliveryPortList - a msParam of type STR_MS_T which is a List of Correspondng Ports (comma separated). \param[in]
 *inMiscInfo - a msParam of type STR_MS_T which is other Information. \param[in] inXmsgTicketInfoParam - a msParam of
 *type XmsgTicketInfo_MS_T which is outXmsgTicketInfoParam from msiXmsgCreateStream \param[out] outSendXmsgInpParam - a
 *msParam of type SendXmsgInp_MS_T which is a Xmsg packet. \param[in,out] - The RuleExecInfo structure that is
 *automatically handled by the rule engine. The user does not include rei as a parameter in the rule invocation.
 *
 * \DolVarDependence none
 * \DolVarModified none
 * \iCatAttrDependence none
                     ...
                     ruleExecInfo_t*)
{
    ...
    snprintf(sendXmsgInp->sendXmsgInfo.msgType,
             sizeof(sendXmsgInp->sendXmsgInfo.msgType),
             "%s",
             (char*) inMsgType->inOutStruct);
 ...
 *
 * \param[in] inConnParam - a msParam of type RcComm_MS_T which is a connection descriptor obtained by
 *msiXmsgServerConnect. \param[in] inSendXmsgInpParam - a msParam of type SendXmsgInp_MS_T outSendXmsgInpParam from
 *inSendXmsgInpParam. \param[in,out] rei - The RuleExecInfo structure that is automatically handled by the rule engine.
...
/**
 * \fn msiRcvXmsg(msParam_t* inConnParam, msParam_t* inTicketNumber, msParam_t* inMsgNumber, msParam_t* outMsgType,
 *msParam_t* outMsg, msParam_t* outSendUser, ruleExecInfo_t *rei)
 *
 ...
 *
 * \param[in] inConnParam - a msParam of type RcComm_MS_T which is a connection descriptor obtained by
 *msiXmsgServerConnect. \param[in] inTicketNumber - a msParam of type XmsgTicketInfo_MS_T or STR_MS_T or unit which is
 *outXmsgTicketInfoParam from msiXmsgCreateStream or outXmsgTicketInfoParam->rcvTicket (a string which the sender passes
 *to the receiver). \param[in] inMsgNumber - a msParam of type unit or STR_MS_T which is a message serial number to
 *fetch. \param[out] outMsgType - a msParam of type STR_MS_T which is a message type. \param[out] outMsg - a msParam of
 *type STR_MS_T which is a message body. \param[out] outSendUser - a msParam of type STR_MS_T which is a sender
 *information. \param[in,out] rei - The RuleExecInfo structure that is automatically handled by the rule engine. The
 *user does not include rei as a parameter in the rule invocation.
 *
 * \DolVarDependence none
 * \DolVarModified none
 * \iCatAttrDependence none
 ...
 *
 * \param[in] inConnParam - a msParam of type RcComm_MS_T which is a connection descriptor obtained by
 *msiXmsgServerConnect. \param[in,out] rei - The RuleExecInfo structure that is automatically handled by the rule
