 ...
 *below. REPEAT UNTIL SUCCESS OR UNTIL <time> REPEAT UNTIL SUCCESS OR nnnn TIMES DOUBLE FOR EVER DOUBLE UNTIL SUCCESS -
 *delay is doubled every time. DOUBLE nnnn TIMES DOUBLE UNTIL <time> DOUBLE UNTIL SUCCESS OR UNTIL <time> DOUBLE UNTIL
 *SUCCESS OR nnnn TIMES DOUBLE UNTIL SUCCESS UPTO <time> Example: <PLUSET>1m</PLUSET><EF>10m<//EF>  means start after 1
 *minute and repeat every 20 minutes \endcode \param[in] mPB - mPB is a msParam of type STR_MS_T which is a body of
 *microservices/rules within backticks (``) \param[in] mPC - mPC is a msParam of type STR_MS_T which is a recoveryBody
 *of microservices/rules within backticks (``) \param[in,out] rei - The RuleExecInfo structure that is automatically
...
void set_plugin_instance_name(const char* _xml, char* _plugin_name)
{
    ...
    if (it != taggedValues->end()) {
        rstrcpy(_plugin_name, it->second.front().c_str(), MAX_NAME_LEN);
...
int recover_delayExec(msParam_t*, msParam_t*, ruleExecInfo_t* rei)
{
    ...
    if (!GlobalDelayExecStack.empty()) {
        ...
        itr->copy(ruleExecDelInp.ruleExecId, itr->size());
        GlobalDelayExecStack.pop_back();
    }
    else {
        rodsLog(LOG_ERROR, "%s - GlobalDelayExecStack is empty", __FUNCTION__);
...
static int carryOverMsParam(msParamArray_t* sourceMsParamArray, msParamArray_t* targetMsParamArray)
{
    ...
    for (i = 0; i < sourceMsParamArray->len; i++) {
        ...
        if ((mP = getMsParamByLabel(targetMsParamArray, mPt->label)) != NULL) {
            ...
            if (status < 0) {
                rodsLogError(
                    LOG_ERROR, status, "%s encountered an error when calling replInOutStruct", __PRETTY_FUNCTION__);
            }
            mP->inpOutBuf = replBytesBuf(mPt->inpOutBuf);
        }
        else
            addMsParamToArray(targetMsParamArray, mPt->label, mPt->type, mPt->inOutStruct, mPt->inpOutBuf, 1);
    }
 ...
 * \param[in] mPB - a msParam of type STR_MS_T which is a body. A statement list - micro-services and ruleNames
 *separated by ## \param[in] mPC - a msParam of type STR_MS_T which is arecoverBody. A statement list - micro-services
 *and ruleNames separated by ## \param[in,out] rei - The RuleExecInfo structure that is automatically handled by the
...
int msiFreeBuffer(msParam_t* memoryParam, ruleExecInfo_t* rei)
{
    ...
    if (!strcmp(memoryParam->type, "STR_PI") &&
        (!strcmp((char*) memoryParam->inOutStruct, "stdout") || !strcmp((char*) memoryParam->inOutStruct, "stderr"))) {
...
/**
 * \fn msiGetDiffTime(msParam_t* inpParam1, msParam_t* inpParam2, msParam_t* inpParam3, msParam_t* outParam,
 *ruleExecInfo_t *rei)
 *
...
int msiGetFormattedSystemTime(msParam_t* outParam, msParam_t* inpParam, msParam_t* inpFormatParam, ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiGetFormattedSystemTime: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    format = (char*) inpParam->inOutStruct;
