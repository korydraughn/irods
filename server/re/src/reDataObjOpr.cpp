...
int msiDataObjCreate(msParam_t* inpParam1, msParam_t* msKeyValStr, msParam_t* outParam, ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiDataObjCreate: input rei or rsComm is NULL");
    ...
    if (rei->status < 0) {
        rodsLogAndErrorMsg(LOG_ERROR,
                           &rsComm->rError,
                           rei->status,
                           "msiDataObjCreate: input inpParam1 error. status = %d",
                           rei->status);
        return rei->status;
    }

    validKwFlags =
        DEST_RESC_NAME_FLAG | CREATE_MODE_FLAG | DATA_TYPE_FLAG | FORCE_FLAG_FLAG | DATA_SIZE_FLAGS | OBJ_PATH_FLAG;
    rei->status =
        parseMsKeyValStrForDataObjInp(msKeyValStr, myDataObjInp, DEST_RESC_NAME_KW, validKwFlags, &outBadKeyWd);
...
int msiDataObjOpen(msParam_t* inpParam, msParam_t* outParam, ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiDataObjOpen: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (strcmp(inpParam->type, STR_MS_T) == 0) {
        ...
        rei->status = parseMsKeyValStrForDataObjInp(inpParam, myDataObjInp, OBJ_PATH_KW, validKwFlags, &outBadKeyWd);
    }
    else {
...
int msiDataObjClose(msParam_t* inpParam, msParam_t* outParam, ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiDataObjClose: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (inpParam == NULL) {
        ...
        rodsLogAndErrorMsg(LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjClose: input inpParam is NULL");
        return rei->status;
    }
    ...
    if (strcmp(inpParam->type, DataObjCloseInp_MS_T) == 0) {
        ...
        if (myInt >= 0) {
            ...
            rei->status = myInt;
            rodsLogAndErrorMsg(
                LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjClose: parseMspForPosInt error for param1.");
            return rei->status;
        }
    }
...
/**
 * \fn msiDataObjLseek (msParam_t *inpParam1, msParam_t *inpParam2, msParam_t *inpParam3, msParam_t *outParam,
 *ruleExecInfo_t *rei)
 *
 ...
 * \usage See clients/icommands/test/rules/
 *
 * \param[in] inpParam1 - a msParam of type DataObjLseekInp_MS_T or INT_MS_T or a STR_MS_T which would be the
 *descriptor. \param[in] inpParam2 - Optional - a msParam of type INT_MS_T or DOUBLE_MS_T or a STR_MS_T which would be
 *the offset. \param[in] inpParam3 - Optional - a msParam of type INT_MS_T or a STR_MS_T which would be from where. Can
 *be SEEK_SET, SEEK_CUR, and SEEK_END. \param[out] outParam - a msParam of type Double_MS_T or DataObjLseekOut_MS_T
 *which is the return status. \param[in,out] rei - The RuleExecInfo structure that is automatically handled by the rule
 *engine. The user does not include rei as a parameter in the rule invocation.
 *
 * \DolVarDependence none
 * \DolVarModified none
 * \iCatAttrDependence none
 ...
 * \sa none
 **/
int msiDataObjLseek(msParam_t* inpParam1,
                    msParam_t* inpParam2,
                    msParam_t* inpParam3,
                    msParam_t* outParam,
                    ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiDataObjLseek: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (inpParam1 == NULL) {
        rei->status = SYS_INTERNAL_NULL_INPUT_ERR;
        rodsLogAndErrorMsg(LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjLseek: input inpParam1 is NULL");
        return rei->status;
    }
    ...
    if (inpParam2 != NULL) {
        if (strcmp(inpParam2->type, INT_MS_T) == 0) {
            ...
            if (strcmp((char*) inpParam2->inOutStruct, "null") != 0) {
                myDataObjLseekInp->offset = strtoll((char*) inpParam2->inOutStruct, 0, 0);
            }
        }
    ...
    if (inpParam3 != NULL) {
        ...
        if (strcmp(inpParam3->type, STR_MS_T) == 0) {
            ...
            else if (strcmp((char*) inpParam3->inOutStruct, "SEEK_CUR") == 0) {
                myDataObjLseekInp->whence = SEEK_CUR;
            }
            else if (strcmp((char*) inpParam3->inOutStruct, "SEEK_END") == 0) {
    ...
    if (myDataObjLseekInp->whence != SEEK_SET && myDataObjLseekInp->whence != SEEK_CUR &&
        myDataObjLseekInp->whence != SEEK_END) {
        rei->status = USER_PARAM_TYPE_ERR;
        rodsLogAndErrorMsg(LOG_ERROR,
                           &rsComm->rError,
...
int msiDataObjRead(msParam_t* inpParam1, msParam_t* inpParam2, msParam_t* outParam, ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiDataObjRead: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (inpParam1 == NULL) {
        rei->status = SYS_INTERNAL_NULL_INPUT_ERR;
        rodsLogAndErrorMsg(LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjRead: input inpParam1 is NULL");
        return rei->status;
    }
    ...
    if (strcmp(inpParam1->type, DataObjReadInp_MS_T) == 0) {
        ...
        if (myInt >= 0) {
            ...
            rei->status = myInt;
            rodsLogAndErrorMsg(
                LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjRead: parseMspForPosInt error for param1.");
            return rei->status;
        }
    }
    ...
    if (inpParam2 != NULL) {
        ...
        if (myInt < 0) {
            if (myInt != SYS_NULL_INPUT) {
                rei->status = myInt;
                rodsLogAndErrorMsg(
                    LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjRead: parseMspForPosInt error for param2.");
                return rei->status;
            }
        }
        else {
    ...
    if (rei->status >= 0) {
        ...
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjRead: rsDataObjRead failed, status = %d", rei->status);
    }
 ...
 * \usage See clients/icommands/test/rules/
 *
 * \param[in] inpParam1 - a msParam of type DataObjWriteInp_MS_T or INT_MS_T or a STR_MS_T which would be the
 *descriptor. \param[in] inpParam2 - Optional - a msParam of type BUF_LEN_MS_T or a STR_MS_T, the input is inpOutBuf and
 *the length of the buffer in the BBuf. \li "stderr", "stdout" can be passed as well \param[out] outParam - a msParam of
 *type INT_MS_T for the length written. \param[in,out] rei - The RuleExecInfo structure that is automatically handled by
 *the rule engine. The user does not include rei as a parameter in the rule invocation.
 *
 * \DolVarDependence none
 * \DolVarModified none
 * \iCatAttrDependence none
...
int msiDataObjWrite(msParam_t* inpParam1, msParam_t* inpParam2, msParam_t* outParam, ruleExecInfo_t* rei)
{
    ...
    if (inpParam1 == NULL || (inpParam1->inpOutBuf == NULL && inpParam1->inOutStruct == NULL)) {
        rei->status = SYS_INTERNAL_NULL_INPUT_ERR;
        rodsLogAndErrorMsg(LOG_ERROR,
                           &rsComm->rError,
                           rei->status,
    ...
    if (strcmp(inpParam1->type, DataObjWriteInp_MS_T) == 0) {
        ...
        if (myInt >= 0) {
            ...
            rei->status = myInt;
            rodsLogAndErrorMsg(
                LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjWrite: parseMspForPosInt error for param1.");
            return rei->status;
        }
    }
    ...
    if (inpParam2 != NULL) {
        ...
        if (strcmp(inpParam2->type, STR_MS_T) == 0) {
            tmpBBuf.len = myDataObjWriteInp->len = strlen((char*) inpParam2->inOutStruct);
...
int msiDataObjUnlink(msParam_t* inpParam, msParam_t* outParam, ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiDataObjUnlink: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (strcmp(inpParam->type, STR_MS_T) == 0) {
        bzero(&dataObjInp, sizeof(dataObjInp));
        myDataObjInp = &dataObjInp;
        validKwFlags = OBJ_PATH_FLAG | FORCE_FLAG_FLAG | REPL_NUM_FLAG | RMTRASH_FLAG | ADMIN_RMTRASH_FLAG | UNREG_FLAG;
        rei->status = parseMsKeyValStrForDataObjInp(inpParam, myDataObjInp, OBJ_PATH_KW, validKwFlags, &outBadKeyWd);
    }
    else {
        rei->status = parseMspForDataObjInp(inpParam, &dataObjInp, &myDataObjInp, 0);
...
int msiDataObjRepl(msParam_t* inpParam1, msParam_t* msKeyValStr, msParam_t* outParam, ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiDataObjRepl: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (rei->status < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjRepl: input inpParam1 error. status = %d", rei->status);
        return rei->status;
    }

    validKwFlags = OBJ_PATH_FLAG | DEST_RESC_NAME_FLAG | NUM_THREADS_FLAG | BACKUP_RESC_NAME_FLAG | RESC_NAME_FLAG |
                   UPDATE_REPL_FLAG | REPL_NUM_FLAG | ALL_FLAG | ADMIN_FLAG | VERIFY_CHKSUM_FLAG | RBUDP_TRANSFER_FLAG |
                   RBUDP_SEND_RATE_FLAG | RBUDP_PACK_SIZE_FLAG;
    rei->status =
        parseMsKeyValStrForDataObjInp(msKeyValStr, myDataObjInp, DEST_RESC_NAME_KW, validKwFlags, &outBadKeyWd);
 ...
 * \sa none
**/
int msiDataObjCopy(msParam_t* inpParam1,
                   msParam_t* inpParam2,
                   ...
                   ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiDataObjCopy: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (rei->status < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjCopy: input inpParam1 error. status = %d", rei->status);
        return rei->status;
    }
    ...
    if (rei->status < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjCopy: input inpParam2 error. status = %d", rei->status);
        return rei->status;
    }

    validKwFlags = OBJ_PATH_FLAG | DEST_RESC_NAME_FLAG | FILE_PATH_FLAG | DATA_TYPE_FLAG | VERIFY_CHKSUM_FLAG |
                   FORCE_FLAG_FLAG | NUM_THREADS_FLAG;
    rei->status = parseMsKeyValStrForDataObjInp(
        msKeyValStr, &myDataObjCopyInp->destDataObjInp, DEST_RESC_NAME_KW, validKwFlags, &outBadKeyWd);
 ...
 * \sa none
 **/
int msiDataObjPut(msParam_t* inpParam1,
                  msParam_t* inpParam2,
                  msParam_t* msKeyValStr,
                  msParam_t* outParam,
                  ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiDataObjPut: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    rei->status = parseMspForDataObjInp(inpParam1, dataObjInp, &myDataObjInp, 1);

    if (rei->status < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjPut: input inpParam1 error. status = %d", rei->status);
        clearDataObjInp(dataObjInp);
        free(dataObjInp);
        return rei->status;
    }

    rei->status = parseMspForCondInp(inpParam2, &dataObjInp->condInput, DEST_RESC_NAME_KW);

    if (rei->status < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjPut: input inpParam2 error. status = %d", rei->status);
        clearDataObjInp(dataObjInp);
        free(dataObjInp);
        return rei->status;
    }

    validKwFlags = LOCAL_PATH_FLAG | DEST_RESC_NAME_FLAG | FILE_PATH_FLAG | REPL_NUM_FLAG | DATA_TYPE_FLAG |
                   VERIFY_CHKSUM_FLAG | ALL_FLAG | FORCE_FLAG_FLAG | NUM_THREADS_FLAG | OBJ_PATH_FLAG;
    rei->status = parseMsKeyValStrForDataObjInp(msKeyValStr, dataObjInp, LOCAL_PATH_KW, validKwFlags, &outBadKeyWd);
    ...
    rei->status = addMsParam(myMsParamArray, CL_PUT_ACTION, DataObjInp_MS_T, (void*) dataObjInp, NULL);

    if (rei->status < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjPut: addMsParam error. status = %d", rei->status);
    ...
    rei->status =
        sendAndRecvBranchMsg(rsComm, rsComm->apiInx, SYS_SVR_TO_CLI_MSI_REQUEST, (void*) myMsParamArray, NULL);
...
int msiDataObjGet(msParam_t* inpParam1, msParam_t* msKeyValStr, msParam_t* outParam, ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiDataObjGet: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (rei->status < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjGet: input inpParam1 error. status = %d", rei->status);
        clearDataObjInp(dataObjInp);
        free(dataObjInp);
        return rei->status;
    }

    validKwFlags = LOCAL_PATH_FLAG | FORCE_FLAG_FLAG | NUM_THREADS_FLAG | RESC_NAME_FLAG | REPL_NUM_FLAG |
                   VERIFY_CHKSUM_FLAG | OBJ_PATH_FLAG;
    rei->status = parseMsKeyValStrForDataObjInp(msKeyValStr, dataObjInp, LOCAL_PATH_KW, validKwFlags, &outBadKeyWd);
    ...
    rei->status = addMsParam(myMsParamArray, CL_GET_ACTION, DataObjInp_MS_T, (void*) dataObjInp, NULL);

    if (rei->status < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjGet: addMsParam error. status = %d", rei->status);
        clearMsParamArray(myMsParamArray, 0);
        free(myMsParamArray);
        clearDataObjInp(dataObjInp);
        free(dataObjInp);
...
int msiDataObjChksum(msParam_t* inpParam1, msParam_t* msKeyValStr, msParam_t* outParam, ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiDataObjChksum: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (rei->status < 0) {
        rodsLogAndErrorMsg(LOG_ERROR,
                           &rsComm->rError,
                           rei->status,
                           "msiDataObjChksum: input inpParam1 error. status = %d",
                           rei->status);
        return rei->status;
    }

    validKwFlags = CHKSUM_ALL_FLAG | FORCE_CHKSUM_FLAG | REPL_NUM_FLAG | OBJ_PATH_FLAG | VERIFY_CHKSUM_FLAG;
    if ((rei->status =
             parseMsKeyValStrForDataObjInp(msKeyValStr, myDataObjInp, KEY_WORD_KW, validKwFlags, &outBadKeyWd)) < 0) {
        if (outBadKeyWd != NULL) {
            rodsLogAndErrorMsg(LOG_ERROR,
                               &rsComm->rError,
                               rei->status,
 ...
 * \sa none
 **/
int msiDataObjPhymv(msParam_t* inpParam1,
                    msParam_t* inpParam2,
                    msParam_t* inpParam3,
                    msParam_t* inpParam4,
                    msParam_t* inpParam5,
                    ...
                    ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiDataObjPhymv: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if ((rei->status = parseMspForDataObjInp(inpParam1, &dataObjInp, &myDataObjInp, 0)) < 0) {
        rodsLogAndErrorMsg(LOG_ERROR,
                           &rsComm->rError,
                           rei->status,
                           "msiDataObjPhymv: input inpParam1 error. status = %d",
                           rei->status);
        return rei->status;
    }

    if ((rei->status = parseMspForCondInp(inpParam2, &myDataObjInp->condInput, DEST_RESC_NAME_KW)) < 0) {
        rodsLogAndErrorMsg(LOG_ERROR,
                           &rsComm->rError,
                           rei->status,
                           "msiDataObjPhymv: input inpParam2 error. status = %d",
                           rei->status);
        return rei->status;
    }

    if ((rei->status = parseMspForCondInp(inpParam3, &myDataObjInp->condInput, RESC_NAME_KW)) < 0) {
        rodsLogAndErrorMsg(LOG_ERROR,
                           &rsComm->rError,
                           rei->status,
                           "msiDataObjPhymv: input inpParam3 error. status = %d",
                           rei->status);
        return rei->status;
    }

    if ((rei->status = parseMspForCondInp(inpParam4, &myDataObjInp->condInput, REPL_NUM_KW)) < 0) {
        rodsLogAndErrorMsg(LOG_ERROR,
                           &rsComm->rError,
                           rei->status,
                           "msiDataObjPhymv: input inpParam4 error. status = %d",
                           rei->status);
        return rei->status;
    }

    if ((rei->status = parseMspForCondInp(inpParam5, &myDataObjInp->condInput, ADMIN_KW)) < 0) {
        rodsLogAndErrorMsg(LOG_ERROR,
                           &rsComm->rError,
                           rei->status,
                           "msiDataObjPhymv: input inpParam5 error. status = %d",
                           rei->status);
        return rei->status;
    }
 ...
 * \sa none
 **/
int msiDataObjRename(msParam_t* inpParam1,
                     msParam_t* inpParam2,
                     msParam_t* inpParam3,
                     msParam_t* outParam,
                     ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiDataObjRename: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    /* parse inpParam1 */
    rei->status = parseMspForDataObjCopyInp(inpParam1, &dataObjRenameInp, &myDataObjRenameInp);

    if (rei->status < 0) {
        rodsLogAndErrorMsg(LOG_ERROR,
                           &rsComm->rError,
                           rei->status,
                           "msiDataObjRename: input inpParam1 error. status = %d",
                           rei->status);
        return rei->status;
    }

    /* parse inpParam2 */
    rei->status = parseMspForDataObjInp(inpParam2, &myDataObjRenameInp->destDataObjInp, &myDataObjInp, 1);

    if (rei->status < 0) {
        rodsLogAndErrorMsg(LOG_ERROR,
                           &rsComm->rError,
                           rei->status,
                           "msiDataObjRename: input inpParam2 error. status = %d",
                           rei->status);
        return rei->status;
    }
 ...
 * \sa none
 **/
int msiDataObjTrim(msParam_t* inpParam1,
                   msParam_t* inpParam2,
                   msParam_t* inpParam3,
                   msParam_t* inpParam4,
                   msParam_t* inpParam5,
                   ...
                   ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiDataObjTrim: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if ((rei->status = parseMspForDataObjInp(inpParam1, &dataObjInp, &myDataObjInp, 0)) < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjTrim: input inpParam1 error. status = %d", rei->status);
        return rei->status;
    }

    if ((rei->status = parseMspForCondInp(inpParam2, &myDataObjInp->condInput, RESC_NAME_KW)) < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjTrim: input inpParam2 error. status = %d", rei->status);
        return rei->status;
    }

    if ((rei->status = parseMspForCondInp(inpParam3, &myDataObjInp->condInput, REPL_NUM_KW)) < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjTrim: input inpParam3 error. status = %d", rei->status);
        return rei->status;
    }

    if ((rei->status = parseMspForCondInp(inpParam4, &myDataObjInp->condInput, COPIES_KW)) < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjTrim: input inpParam4 error. status = %d", rei->status);
        return rei->status;
    }

    if ((rei->status = parseMspForCondInp(inpParam5, &myDataObjInp->condInput, ADMIN_KW)) < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiDataObjTrim: input inpParam5 error. status = %d", rei->status);
        return rei->status;
    }
...
int msiCollCreate(msParam_t* inpParam1, msParam_t* inpParam2, msParam_t* outParam, ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiCollCreate: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    /* parse inpParam1 */
    rei->status = parseMspForCollInp(inpParam1, &collCreateInp, &myCollCreateInp, 1);

    if (rei->status < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiCollCreate: input inpParam1 error. status = %d", rei->status);
        return rei->status;
    }
...
int msiRmColl(msParam_t* inpParam1, msParam_t* msKeyValStr, msParam_t* outParam, ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiRmColl: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    /* parse inpParam1 */
    rei->status = parseMspForCollInp(inpParam1, &rmCollInp, &myRmCollInp, 1);

    if (rei->status < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiRmColl: input inpParam1 error. status = %d", rei->status);
        return rei->status;
    }

    validKwFlags = RMTRASH_FLAG | ADMIN_RMTRASH_FLAG | FORCE_FLAG_FLAG | COLL_NAME_FLAG;
    if ((rei->status = parseMsKeyValStrForCollInp(msKeyValStr, myRmCollInp, KEY_WORD_KW, validKwFlags, &outBadKeyWd)) <
        0) {
        if (outBadKeyWd != NULL) {
            rodsLogAndErrorMsg(LOG_ERROR,
                               &rsComm->rError,
 ...
 * \sa none
 **/
int msiPhyPathReg(msParam_t* inpParam1,
                  msParam_t* inpParam2,
                  msParam_t* inpParam3,
                  msParam_t* inpParam4,
                  msParam_t* outParam,
                  ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiPhyPathReg: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (rei->status < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiPhyPathReg: input inpParam1 error. status = %d", rei->status);
        return rei->status;
    }

    if ((rei->status = parseMspForCondInp(inpParam2, &myDataObjInp->condInput, DEST_RESC_NAME_KW)) < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiPhyPathReg: input inpParam2 error. status = %d", rei->status);
        return rei->status;
    }

    if ((rei->status = parseMspForCondInp(inpParam3, &myDataObjInp->condInput, FILE_PATH_KW)) < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiPhyPathReg: input inpParam3 error. status = %d", rei->status);
        return rei->status;
    }

    if ((rei->status = parseMspForPhyPathReg(inpParam4, &myDataObjInp->condInput)) < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiPhyPathReg: input inpParam4 error. status = %d", rei->status);
        return rei->status;
    }
...
int msiObjStat(msParam_t* inpParam1, msParam_t* outParam, ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiObjStat: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (rei->status < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiObjStat: input inpParam1 error. status = %d", rei->status);
        return rei->status;
    }
 ...
 * \sa none
 **/
int msiDataObjRsync(msParam_t* inpParam1,
                    msParam_t* inpParam2,
                    msParam_t* inpParam3,
                    msParam_t* inpParam4,
                    msParam_t* outParam,
                    ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiDataObjRsync: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (rei->status < 0) {
        rodsLogAndErrorMsg(LOG_ERROR,
                           &rsComm->rError,
                           rei->status,
                           "msiDataObjRsync: input inpParam1 error. status = %d",
                           rei->status);
        return rei->status;
    }

    if ((rei->status = parseMspForCondInp(inpParam2, &myDataObjInp->condInput, RSYNC_MODE_KW)) < 0) {
        rodsLogAndErrorMsg(LOG_ERROR,
                           &rsComm->rError,
                           rei->status,
                           "msiDataObjRsync: input inpParam2 error. status = %d",
                           rei->status);
        return rei->status;
    }

    if ((rei->status = parseMspForCondInp(inpParam3, &myDataObjInp->condInput, DEST_RESC_NAME_KW)) < 0) {
    }

    if ((rei->status = parseMspForCondInp(inpParam4, &myDataObjInp->condInput, RSYNC_DEST_PATH_KW)) < 0) {
        rodsLogAndErrorMsg(LOG_ERROR,
                           &rsComm->rError,
                           rei->status,
                           "msiDataObjRsync: input inpParam4 error. status = %d",
                           rei->status);
        return rei->status;
    }
    ...
    if (rsyncMode == NULL) {
        rodsLog(LOG_ERROR, "msiDataObjRsync: RSYNC_MODE_KW input is missing");
        rei->status = USER_RSYNC_NO_MODE_INPUT_ERR;
        return rei->status;
    }

    if (strcmp(rsyncMode, IRODS_TO_LOCAL) == 0 || strcmp(rsyncMode, LOCAL_TO_IRODS) == 0) {
        rodsLog(LOG_ERROR, "msiDataObjRsync: local/iRODS rsync not supported for %s", myDataObjInp->objPath);
        rei->status = NO_LOCAL_FILE_RSYNC_IN_MSI;
        return rei->status;
    }
    else if (strcmp(rsyncMode, IRODS_TO_COLLECTION) == 0) {
        targCollection = getValByKey(&myDataObjInp->condInput, RSYNC_DEST_PATH_KW);
        if (targCollection == NULL) {
            rodsLog(LOG_ERROR, "msiDataObjRsync:  RSYNC_DEST_PATH_KW input for %s is missing", myDataObjInp->objPath);
            rei->status = USER_INPUT_PATH_ERR;
            return rei->status;
        }
        tmpPtr = strchr(myDataObjInp->objPath + 1, '/');
        if (tmpPtr == NULL) {
            rodsLog(LOG_ERROR, "msiDataObjRsync:  problem parsing %s", myDataObjInp->objPath);
            rei->status = USER_INPUT_PATH_ERR;
            return rei->status;
        }
 ...
 * \sa none
 **/
int msiCollRsync(msParam_t* inpParam1,
                 msParam_t* inpParam2,
                 msParam_t* inpParam3,
                 msParam_t* inpParam4,
                 msParam_t* outParam,
                 ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiCollRsync: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (srcColl == NULL) {
        ...
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiCollRsync: input inpParam1 error. status = %d", rei->status);
        return rei->status;
    }
    ...
    if (destColl == NULL) {
        rei->status = SYS_INVALID_FILE_PATH;
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiCollRsync: input inpParam2 error. status = %d", rei->status);
        return rei->status;
    }

    if ((rei->status = parseMspForCondInp(inpParam3, &dataObjInp.condInput, DEST_RESC_NAME_KW)) < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiCollRsync: input inpParam3 error. status = %d", rei->status);
        return rei->status;
    }

    if ((rei->status = parseMspForCondInp(inpParam4, &dataObjInp.condInput, RSYNC_MODE_KW)) < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiCollRsync: input inpParam4 error. status = %d", rei->status);
        return rei->status;
    }
    ...
    if (rsyncMode == NULL) {
        rodsLog(LOG_ERROR, "msiCollRsync: RSYNC_MODE_KW input is missing");
        rei->status = USER_RSYNC_NO_MODE_INPUT_ERR;
        return rei->status;
    }

    if (strcmp(rsyncMode, IRODS_TO_LOCAL) == 0 || strcmp(rsyncMode, LOCAL_TO_IRODS) == 0) {
        rodsLog(LOG_ERROR, "msiCollRsync: local/iRODS rsync not supported for %s", srcColl);
        rei->status = NO_LOCAL_FILE_RSYNC_IN_MSI;
        return rei->status;
    }
...
int _rsCollRsync(rsComm_t* rsComm, dataObjInp_t* dataObjInp, char* srcColl, char* destColl)
{
    ...
    if (handleInx < 0) {
        rodsLog(LOG_ERROR, "_rsCollRsync: rsOpenCollection of %s error. status = %d", openCollInp.collName, handleInx);
        return handleInx;
    }
    ...
    while ((status1 = rsReadCollection(rsComm, &handleInx, &collEnt)) >= 0) {
        if (collEnt->objType == DATA_OBJ_T) {
            snprintf(dataObjInp->objPath, MAX_NAME_LEN, "%s/%s", srcColl, collEnt->dataName);
            snprintf(destChildPath, MAX_NAME_LEN, "%s/%s", destColl, collEnt->dataName);
            addKeyVal(&dataObjInp->condInput, RSYNC_DEST_PATH_KW, destChildPath);
            ...
            if ((status = splitPathByKey(collEnt->collName, parPath, MAX_NAME_LEN, childPath, MAX_NAME_LEN, '/')) < 0) {
            ...
            snprintf(destChildPath, MAX_NAME_LEN, "%s/%s", destColl, childPath);

            status = _rsCollRsync(rsComm, dataObjInp, collEnt->collName, destChildPath);
        }
 ...
 * \sa none
 **/
int msiExecCmd(msParam_t* inpParam1,
               msParam_t* inpParam2,
               msParam_t* inpParam3,
               msParam_t* inpParam4,
               msParam_t* inpParam5,
               ...
               ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiExecCmd: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    /* parse inpParam1 */
    rei->status = parseMspForExecCmdInp(inpParam1, &execCmdInp, &myExecCmdInp);

    if (rei->status < 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiExecCmd: input inpParam1 error. status = %d", rei->status);
        return rei->status;
    }
    ...
    if (strlen(rei->ruleName) > 0 && strcmp(rei->ruleName, EXEC_MY_RULE_KW) != 0) {
...
int msiCollRepl(msParam_t* collection, msParam_t* msKeyValStr, msParam_t* status, ruleExecInfo_t* rei)
{
    ...
    validKwFlags = COLL_NAME_FLAG | DEST_RESC_NAME_FLAG | BACKUP_RESC_NAME_FLAG | RESC_NAME_FLAG | UPDATE_REPL_FLAG |
                   REPL_NUM_FLAG | ALL_FLAG | ADMIN_FLAG | VERIFY_CHKSUM_FLAG;
    rei->status = parseMsKeyValStrForCollInp(msKeyValStr, collInp, DEST_RESC_NAME_KW, validKwFlags, &outBadKeyWd);
...
/**
 * \fn msiTarFileExtract (msParam_t *inpParam1, msParam_t *inpParam2, msParam_t *inpParam3,  msParam_t *outParam,
 *ruleExecInfo_t *rei)
 *
 ...
 * \sa N/A
 **/
int msiTarFileExtract(msParam_t* inpParam1,
                      msParam_t* inpParam2,
                      msParam_t* inpParam3,
                      msParam_t* outParam,
                      ruleExecInfo_t* rei)
{
    rsComm_t* rsComm;
    structFileExtAndRegInp_t structFileExtAndRegInp, *myStructFileExtAndRegInp;
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiTarFileExtract: input rei or rsComm is NULL");
        if (rei) {
            rei->status = SYS_INTERNAL_NULL_INPUT_ERR;
        }
        return SYS_INTERNAL_NULL_INPUT_ERR;
    ...
    if (inpParam1 == NULL || inpParam2 == NULL) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiTarFileExtract: input Param1 and/or Param2 are NULL");
        rei->status = SYS_INTERNAL_NULL_INPUT_ERR;
        return rei->status;
    }
    ...
    if (strcmp(inpParam1->type, STR_MS_T) == 0) {
        ...
        snprintf(myStructFileExtAndRegInp->objPath,
                 sizeof(myStructFileExtAndRegInp->objPath),
                 "%s",
                 (char*) inpParam1->inOutStruct);
    }
    else if (strcmp(inpParam1->type, StructFileExtAndRegInp_MS_T) == 0) {
        myStructFileExtAndRegInp = (structFileExtAndRegInp_t*) inpParam1->inOutStruct;
    }
    else {
    ...
    if (strcmp(inpParam2->type, STR_MS_T) == 0) {
        if (strcmp((char*) inpParam2->inOutStruct, "null") != 0) {
            snprintf(myStructFileExtAndRegInp->collection,
                     sizeof(myStructFileExtAndRegInp->collection),
                     "%s",
    ...
    if (inpParam3 != NULL && strcmp(inpParam3->type, STR_MS_T) == 0 &&
        ...
        addKeyVal(&myStructFileExtAndRegInp->condInput, DEST_RESC_NAME_KW, (char*) inpParam3->inOutStruct);
        /* RESC_NAME_KW is needed so Open will pick this resource */
        addKeyVal(&myStructFileExtAndRegInp->condInput, RESC_NAME_KW, (char*) inpParam3->inOutStruct);
    }
...
/**
 * \fn msiTarFileCreate (msParam_t *inpParam1, msParam_t *inpParam2, msParam_t *inpParam3,  msParam_t *inpParam4,
ruleExecInfo_t *rei)
 *
 ...
 * \sa N/A
**/
int msiTarFileCreate(msParam_t* inpParam1,
                     msParam_t* inpParam2,
                     msParam_t* inpParam3,
                     msParam_t* inpParam4,
                     ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiTarFileCreate: input rei or rsComm is NULL");
        if (rei) {
            rei->status = SYS_INTERNAL_NULL_INPUT_ERR;
        }
        return SYS_INTERNAL_NULL_INPUT_ERR;
    ...
    if (inpParam1 == NULL || inpParam2 == NULL) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiTarFileCreate: input Param1 and/or Param2 are NULL");
        rei->status = SYS_INTERNAL_NULL_INPUT_ERR;
        return rei->status;
    }
 ...
 * \usage See clients/icommands/test/rules/
 *
 * \param[in] inpParam1 - A StructFileExtAndRegInp_MS_T or a STR_MS_T which would be taken as the collection for the
 *phybun. \param[in] inpParam2 - optional - a STR_MS_T which specifies the target resource. If one wants to modify the
 *value of the maximum number of subfiles contained in the tar file (default=5120), the input should be like this:
 *                       "<target resource name>++++N=10000++++s=1". In this example, it will allow to merge up to 10000
 *files or up to 1 GB in volume, whatever occurs first, in a single tar file. Note that if these numbers are too high
 *(especially "N"), it can cause some significant overhead for operations like retrieving a single file within a tar
 *file (stage, untar and register in iRODS lots of files). If the syntax after "++++" is invalid, it will be ignored.
 * \param[out] outParam - An INT_MS_T containing the status.
 * \param[in,out] rei - The RuleExecInfo structure that is automatically
 *    handled by the rule engine. The user does not include rei as a
 *    parameter in the rule invocation.
...
int msiPhyBundleColl(msParam_t* inpParam1, msParam_t* inpParam2, msParam_t* outParam, ruleExecInfo_t* rei)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiPhyBundleColl: input rei or rsComm is NULL");
        if (rei) {
            rei->status = SYS_INTERNAL_NULL_INPUT_ERR;
        }
        return SYS_INTERNAL_NULL_INPUT_ERR;
    ...
    if (inpParam1 == NULL) {
        rodsLogAndErrorMsg(LOG_ERROR, &rsComm->rError, rei->status, "msiPhyBundleColl: input Param1 is NULL");
        rei->status = SYS_INTERNAL_NULL_INPUT_ERR;
        return rei->status;
    }
    ...
    if (rei->uoic->authInfo.authFlag < LOCAL_PRIV_USER_AUTH) {
        ...
        rodsLog(LOG_ERROR,
                "msiPhyBundleColl: User %s does not have sufficient privilege, status = %d",
                rei->uoic->userName,
                rei->status);
        return rei->status;
    }
