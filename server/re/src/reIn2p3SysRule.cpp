...
{
    char condstr[MAX_NAME_LEN], fname[MAX_NAME_LEN], msg[MAX_MESSAGE_SIZE], monStatus[MAX_NAME_LEN], suffix[MAX_VALUE];
    const char* delim1 = "#";
    const char* delim2 = ",";
    ...
    if (output_tokens.size() != 9) {
        rodsLog(LOG_ERROR,
                "rodsMonPerfLog: output_tokens is of incorrect size: size [%ju], output [%s]",
                (uintmax_t) output_tokens.size(),
                output);
        return -1;
    }
    ...
    if (resc_tokens.size() != disk_tokens.size() || resc_tokens.size() != value_tokens.size()) {
        rodsLog(LOG_ERROR,
                "rodsMonPerfLog: resc_tokens [%ju], disk_tokens [%ju], value_tokens [%ju]. output [%s]",
                (uintmax_t) resc_tokens.size(),
                (uintmax_t) disk_tokens.size(),
                (uintmax_t) value_tokens.size(),
                output);
        return -1;
    }
    ...
    for (std::vector<std::string>::size_type index = 0; index < resc_tokens.size(); ++index) {
        ...
        snprintf(msg,
                 ...
                 serverName,
                 resc_tokens[index].c_str(),
                 output_tokens[0].c_str(),
                 output_tokens[1].c_str(),
                 output_tokens[2].c_str(),
                 output_tokens[3].c_str(),
                 disk_tokens[index].c_str(),
                 output_tokens[5].c_str(),
                 output_tokens[6].c_str(),
                 value_tokens[index].c_str());
        ...
        if (rc3 >= 0) {
            ...
            rodsLog(LOG_ERROR,
                    "msiServerMonPerf: unable to retrieve the status metadata for the resource %s",
                    resc_tokens[index].c_str());
        }
#ifndef windows_platform
        ...
        if (rc2 != 0) {
            rodsLog(LOG_ERROR,
                    "msiServerMonPerf: unable to register the free space metadata for the resource %s",
                    resc_tokens[index].c_str());
        }
        if (rc4 != 0) {
            rodsLog(LOG_ERROR,
                    "msiServerMonPerf: unable to register the status metadata for the resource %s",
                    resc_tokens[index].c_str());
        }
    }
...
int getListOfResc(rsComm_t* rsComm,
                  ...
                  int nservers,
                  monInfo_t monList[MAX_NSERVERS],
                  int* nlist)
{
    /**********************************************************
        ...
        for (const auto& el : cfg.get<const std::vector<boost::any>>("access_entries")) {
            try {
                ...
                if (group_match || user_match) {
                    ...
                    if (((host_client.to_ulong() ^ address_entry.to_ulong()) & ~mask_entry.to_ulong()) == 0) {
                        return 0;
                    }
                }
            }
...
int msiServerMonPerf(msParam_t* verb, msParam_t* ptime, ruleExecInfo_t* rei)
{
    ...
    for (i = 0; i < nresc; i++) {
        ...
        if (check == 0) {
            const char* path =
                strcmp(rescList[thrCount].rescType, "unixfilesystem") == 0 ? rescList[i].vaultPath : "none";
            snprintf(thrInput[thrCount].cmdArgv,
                     sizeof(thrInput[thrCount].cmdArgv),
                     "%s -fs %s",
                     valinit_stream.str().c_str(),
                     path);
            ...
            for (i = 0; i < thrCount; i++) {
                if (!threadIsAlive[i]) {
                    ...
                    if (rc == 0) {
                        char noanswer[MAXSTR] = MON_OUTPUT_NO_ANSWER;
                        threadIsAlive[i] = 1;
                        rodsMonPerfLog(thrInput[i].execAddr, thrInput[i].rescName, noanswer, &(thrInput[i].rei));
                    }
#endif
                }
            }
...
int msiFlushMonStat(msParam_t* inpParam1, msParam_t* inpParam2, ruleExecInfo_t* rei)
{
    ...
    if (inpParam1 == NULL) {
        rodsLogAndErrorMsg(LOG_ERROR, &rsComm->rError, rei->status, "msiFlushMonStat: input Param1 is NULL");
        return rei->status;
    }
    ...
    if (inpParam2 == NULL) {
        rodsLogAndErrorMsg(LOG_ERROR, &rsComm->rError, rei->status, "msiFlushMonStat: input Param2 is NULL");
        return rei->status;
    }
    ...
    if (strcmp(tablename, "serverload") != 0 && strcmp(tablename, "serverloaddigest") != 0) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiFlushMonStat: table %s does not exist", tablename);
        return rei->status;
    }
...
int msiDigestMonStat(msParam_t* cpu_wght,
                     ...
                     ruleExecInfo_t* rei)
{
    char rescList[MAX_NSERVERS][MAX_NAME_LEN], *tResult, timeList[MAX_NSERVERS][MAX_NAME_LEN];
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        rodsLog(LOG_ERROR, "msiDigestMonStat: input rei or rsComm is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }

    rsComm = rei->rsComm;

    if (cpu_wght == NULL || mem_wght == NULL || swap_wght == NULL || runq_wght == NULL || disk_wght == NULL ||
        netin_wght == NULL || netout_wght == NULL) {
        rodsLogAndErrorMsg(
            LOG_ERROR, &rsComm->rError, rei->status, "msiDigestMonStat: at least one of the input param is NULL");
        return rei->status;
    }
