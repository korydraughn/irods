...
int msiCreateCollByAdmin(msParam_t* xparColl, msParam_t* xchildName, ruleExecInfo_t* rei)
{
    ...
    snprintf(collInfo.collName, sizeof(collInfo.collName), "%s/%s", parColl, childName);
    snprintf(collInfo.collOwnerName, sizeof(collInfo.collOwnerName), "%s", rei->uoio->userName);
    snprintf(collInfo.collOwnerZone, sizeof(collInfo.collOwnerZone), "%s", rei->uoio->rodsZone);
...
int msiAddUserToGroup(msParam_t* msParam, ruleExecInfo_t* rei)
{
    ...
    if (irods::CFG_SERVICE_ROLE_PROVIDER == svc_role) {
        ...
        i = chlModGroup(rei->rsComm, groupName, "add", rei->uoio->userName, rei->uoio->rodsZone);
    }
    else if (irods::CFG_SERVICE_ROLE_CONSUMER == svc_role) {
        i = SYS_NO_RCAT_SERVER_ERR;
    }
...
int msiSetQuota(msParam_t* type, msParam_t* name, msParam_t* resource, msParam_t* value, ruleExecInfo_t* rei)
{
    ...
    if (rei->uoic->authInfo.authFlag < LOCAL_PRIV_USER_AUTH) {
        ...
        rodsLog(LOG_ERROR, "msiSetQuota: User %s is not local admin. Status = %d", rei->uoic->userName, status);
        return status;
    }
