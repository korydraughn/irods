...
unsigned char* prepareServerSharedMemory(const std::string& _key)
{
    ...
    try {
        bi::shared_memory_object* tmp_s =
            new bi::shared_memory_object(bi::open_or_create, shared_memory_name.c_str(), bi::read_write, 0600);
        ...
        rodsLog(
            LOG_ERROR, "prepareServerSharedMemory: failed to prepare shared memory. Exception caught [%s]", e.what());
        return NULL;
    }
}
...
unsigned char* prepareNonServerSharedMemory(const std::string& _key)
{
    ...
    if (!ret.ok()) {
        rodsLog(LOG_ERROR,
                "prepareNonServerSharedMemory: failed to get shared memory name [%s]",
    ...
    try {
        bi::shared_memory_object* tmp_s =
            new bi::shared_memory_object(bi::open_only, shared_memory_name.c_str(), bi::read_only);
        shm_obj[_key] = std::unique_ptr<bi::shared_memory_object>(tmp_s);
        ...
        rodsLog(LOG_ERROR,
                "prepareNonServerSharedMemory: failed to get shared memory object [%s]. Exception caught [%s]",
                shared_memory_name.c_str(),
                e.what());
        return NULL;
    }
...
irods::error getSharedMemoryName(const std::string& _key, std::string& shared_memory_name)
{
    try {
        ...
        rodsLog(
            LOG_ERROR, "getSharedMemoryName: failed to retrieve re cache salt from server_properties\n%s", e.what());
