...
int msiRegisterData(ruleExecInfo_t* rei)
{
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "msiRegisterData: rsRegDataObj for %s failed, status = %d", myDataObjInfo->objPath, status);
        return status;
    }
    else {
...
int recover_msiRegisterData(ruleExecInfo_t* rei)
{
    msiRollback(rei); /* rolling back */
    return 0;
}
int print_bye(ruleExecInfo_t* rei)
{
    RE_TEST_MACRO("Bye\n");
    fprintf(stdout, "Bye\n");
    return 0;
...
int print_eol(ruleExecInfo_t* rei)
{
    RE_TEST_MACRO("\n");
    fprintf(stdout, "\n");
    return 0;
...
int recover_print_bye(ruleExecInfo_t* rei)
{
...
int recover_print_eol(ruleExecInfo_t* rei)
{
    RE_TEST_MACRO("*\b");
    fprintf(stdout, "*\b");
    return 0;
...
int recover_print_hello_arg(msParam_t* xs, ruleExecInfo_t*)
{
    unsigned int i;
    char* s;
...
int msitest1(msParam_t* A, msParam_t* B, msParam_t* C, ruleExecInfo_t* rei)
{
    ...
    snprintf(tmpStr,
             199,
             "msitest1: In A=%s, In B=%s, Out C=%s",
             (char*) mPA->inOutStruct,
             (char*) mPB->inOutStruct,
             (char*) mPC->inOutStruct);
    printf("%s\n", tmpStr);
...
int msitest2(msParam_t* A, msParam_t* B, msParam_t* C, ruleExecInfo_t* rei)
{
    ...
    snprintf(tmpStr,
             199,
             "msitest2: In A=%s, Out B=%s, Out C=%s",
             (char*) mPA->inOutStruct,
             (char*) mPB->inOutStruct,
             (char*) mPC->inOutStruct);
    printf("%s\n", tmpStr);
