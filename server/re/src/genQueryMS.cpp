 ...
 *
 *
 * \note Takes a SQL-like iRODS query (no FROM clause) and returns a table structure. Use #msiGetMoreRows to get all
 *rows.
 *
 * \usage See clients/icommands/test/rules/
 *
 ...
 *
 *
 * \note The resulting continueInx can be used to determine whether there are remaining rows to retrieve from the
 *generated query.
 *
 * \usage See clients/icommands/test/rules/
 *
...
/**
 * \fn msiGetMoreRows(msParam_t *genQueryInp_msp, msParam_t *genQueryOut_msp, msParam_t *continueInx, ruleExecInfo_t
 **rei)
 *
 ...
 *
 *
 * \note This microservice gets the next batch of rows for an open iCAT query. Likely to follow #msiMakeGenQuery and
 *#msiExecGenQuery.
 *
 * \usage None
 *
 * \param[in] genQueryInp_msp - Required - a GenQueryInp_MS_T containing the query parameters and conditions.
 * \param[in] genQueryOut_msp - Required - a GenQueryOut_MS_T to write results to. If its continuation index is 0 the
 *query will be closed. \param[out] continueInx - a INT_MS_T containing the new continuation index (after the query).
 * \param[in,out] rei - The RuleExecInfo structure that is automatically
 *    handled by the rule engine. The user does not include rei as a
 *    parameter in the rule invocation.
 *
...
int msiGetMoreRows(msParam_t* genQueryInp_msp, msParam_t* genQueryOut_msp, msParam_t* continueInx, ruleExecInfo_t* rei)
{
    ...
    if (strcmp(genQueryOut_msp->type, GenQueryOut_MS_T)) {
        rodsLog(
            LOG_ERROR, "msiGetMoreRows: genQueryOut_msp type is %s, should be GenQueryOut_MS_T", genQueryOut_msp->type);
        return USER_PARAM_TYPE_ERR;
    }
 ...
 * \param[in] genQueryInp_msp - Required - a GenQueryInp_MS_T containing the query parameters and conditions.
 * \param[in] genQueryOut_msp - Required - a GenQueryOut_MS_T to write results to. If its continuation index is 0 the
 *query will be closed. \param[in,out] rei - The RuleExecInfo structure that is automatically handled by the rule
...
int msiCloseGenQuery(msParam_t* genQueryInp_msp, msParam_t* genQueryOut_msp, ruleExecInfo_t* rei)
{
    ...
    if (strcmp(genQueryOut_msp->type, GenQueryOut_MS_T)) {
        rodsLog(LOG_ERROR,
                "msiCloseGenQuery: genQueryOut_msp type is %s, should be GenQueryOut_MS_T",
                genQueryOut_msp->type);
        return USER_PARAM_TYPE_ERR;
    }
...
/**
 * \fn msiAddConditionToGenQuery(msParam_t *attribute, msParam_t *opr, msParam_t *value, msParam_t *queryInput,
 *ruleExecInfo_t *rei)
 *
 ...
 * \usage See clients/icommands/test/rules/
 *
 * \param[in] attribute - Required - A STR_MS_T with the iCAT attribute name (see
 *wiki.irods.org/index.php/icatAttributes). \param[in] opr - Required - A STR_MS_T with the operator. \param[in] value -
 *Required - A STR_MS_T with the value. \param[in,out] queryInput - Required - A GenQueryInp_MS_T. \param[in,out] rei -
