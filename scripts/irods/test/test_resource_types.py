    ...
    def test_univmss_resource_plugin_supports_paths_containing_single_quotes__issue_8928(self):
        ...
        try:
            ...
            with self.subTest('iput fails when path contains single quotes and escape_single_quotes not enabled'):
                ...
                self.user0.assert_icommand(['ils', '-l', filename], 'STDOUT', [f' X {filename}\n'])
            ...
            with self.subTest('sync to archive - iput'):
                ...
                _, out, _ = self.user0.assert_icommand(['ils', '-l', filename], 'STDOUT', [
                    ' 0 demoResc;cacheResc ',
                    ' 1 demoResc;archiveResc ',
                    f' & {filename}\n'
                ])
                self.assertNotIn(f' X {filename}\n', out)
            ...
            with self.subTest('rename'):
                ...
                self.assertFalse(lib.replica_exists(self.user0, f'{self.user0.session_collection}/{filename}', 0))
                self.assertTrue(lib.replica_exists(self.user0, f'{self.user0.session_collection}/{new_filename}', 0))

                self.user0.assert_icommand(['imv', new_filename, filename])
                self.assertTrue(lib.replica_exists(self.user0, f'{self.user0.session_collection}/{filename}', 0))
                self.assertFalse(lib.replica_exists(self.user0, f'{self.user0.session_collection}/{new_filename}', 0))
