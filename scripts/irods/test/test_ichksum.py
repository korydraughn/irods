...
class Test_Ichksum(resource_suite.ResourceBase, unittest.TestCase):
    ...
    def test_ichksum_force(self):
        ...
        # Show that the new checksum does not match the original.
        self.assertNotEqual(file_chksum, new_chksum)

        # The checksum will remain the same because ichksum reads, at most, the number of bytes
        # recorded in the catalog.
        self.admin.assert_icommand(['ichksum', '-f', filename], 'STDOUT_SINGLELINE', file_chksum)
