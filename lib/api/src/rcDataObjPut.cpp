...
int rcDataObjPut(rcComm_t* conn, dataObjInp_t* dataObjInp, char* locFilePath)
{
    ...
    if (getValByKey(&dataObjInp->condInput, DATA_INCLUDED_KW) != NULL) {
        if (dataObjInp->dataSize > single_buff_sz) {
            ...
            if (status < 0) {
                rodsLog(LOG_NOTICE, "rcDataObjPut: fileBBufWithFile error for %s", locFilePath);
                return status;
            }
        }
    }
    ...
    if (portalOprOut->numThreads <= 0) {
        status = putFile(conn, portalOprOut->l1descInx, locFilePath, dataObjInp->objPath, dataObjInp->dataSize);
    }
        ...
        if (irods::CS_NEG_USE_SSL == conn->negotiation_results) {
            ...
            rodsLog(LOG_ERROR, "putFileToPortal: Encryption is not supported with RBUDP");
            return SYS_INVALID_PORTAL_OPR;
        }
        else {
            status = putFileToPortalRbudp(portalOprOut, locFilePath, -1, veryVerbose, 0, 0);
        }
    }
    else {
        if (getValByKey(&dataObjInp->condInput, VERY_VERBOSE_KW) != NULL) {
        ...
        conn->transStat.numThreads = portalOprOut->numThreads;
        status = putFileToPortal(conn, portalOprOut, locFilePath, dataObjInp->objPath, dataObjInp->dataSize);
    }
...
int _rcDataObjPut(rcComm_t* conn, dataObjInp_t* dataObjInp, bytesBuf_t* dataObjInpBBuf, portalOprOut_t** portalOprOut)
{
    ...
    status = procApiRequest(conn, DATA_OBJ_PUT_AN, dataObjInp, dataObjInpBBuf, (void**) portalOprOut, NULL);
