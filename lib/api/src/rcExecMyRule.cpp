...
{
    int status = procApiRequest(conn, EXEC_MY_RULE_AN, execMyRuleInp, NULL, (void**) outParamArray, NULL);
    ...
    while (status == SYS_SVR_TO_CLI_MSI_REQUEST) {
        ...
        if ((myParam = putParam = getMsParamByLabel(*outParamArray, CL_PUT_ACTION)) ||
            ...
            if ((locFilePath = getValByKey(&dataObjInp->condInput, LOCAL_PATH_KW)) ||
                ...
                status = putParam ? rcDataObjPut(conn, dataObjInp, locFilePath)
                                  : rcDataObjGet(conn, dataObjInp, locFilePath);
                rcOprComplete(conn, status);
            }
            else {
                rodsLogError(LOG_ERROR, status, "rcExecMyRule: splitPathByKey for %s error", dataObjInp->objPath);
        ...
        *outParamArray = NULL;
        ...
        status = branchReadAndProcApiReply(conn, EXEC_MY_RULE_AN, (void**) outParamArray, NULL);
        if (status < 0) {
            rodsLogError(LOG_DEBUG, status, "rcExecMyRule: readAndProcApiReply failed. status = %d", status);
        }
    }
