...
namespace irods
{
    ...
    **/
    ...
    public:
        ...
        error call(rsComm_t* _comm, const std::string& _operation_name, irods::first_class_object_ptr _fco)
        {
            using namespace std;
            try {
                ...
                std::function<error(plugin_context&, std::string*)> adapted_fcn =
                    [this, &_operation_name](plugin_context& _ctx, std::string* _out_param) {
                ...
                for (auto& ns : NamespacesHelper::Instance()->getNamespaces()) {
                    ...
                    if (RuleExistsHelper::Instance()->checkOperation(rule_name)) {
                        if (re_ctx_mgr.rule_exists(rule_name, ret).ok() && ret) {
                            ...
                            if (!op_err.ok()) {
                                rodsLog(LOG_DEBUG,
                                        "Pre-pep rule [%s] failed with error code [%d]",
                                        rule_name.c_str(),
                                        op_err.code());
                                saved_op_err = op_err;
                                pre_pep_failed = true;
                ...
                if (pre_pep_failed) {
                    rodsLog(LOG_DEBUG,
                            "Pre-pep for operation [%s] failed, rule and post-pep not executed",
                ...
                if (!to_return_op_err.ok()) {
                    rodsLog(LOG_DEBUG,
                            "Operation [%s] failed with error code [%d], post-pep not executed",
                            _operation_name.c_str(),
                            op_err.code());
                ...
                for (auto& ns : NamespacesHelper::Instance()->getNamespaces()) {
                    ...
                    if (RuleExistsHelper::Instance()->checkOperation(rule_name)) {
                        if (re_ctx_mgr.rule_exists(rule_name, ret).ok() && ret) {
                            ...
                            if (!op_err.ok()) {
                                rodsLog(LOG_DEBUG,
                                        "Post-pep rule [%s] failed with error code [%d]",
                                        rule_name.c_str(),
                                        op_err.code());
                            }
                        }
                   ...
                   types_t... _t)
        {
            ...
            try {
                ...
                std::function<error(plugin_context&, std::string*, types_t...)> adapted_fcn =
                    [this, &_operation_name](plugin_context& _ctx, std::string* _out_param, types_t... _t) {
                        _ctx.rule_results(*_out_param);
                ...
                for (auto& ns : NamespacesHelper::Instance()->getNamespaces()) {
                    ...
                    if (RuleExistsHelper::Instance()->checkOperation(rule_name)) {
                        if (re_ctx_mgr.rule_exists(rule_name, ret).ok() && ret) {
                            op_err = re_ctx_mgr.exec_rule(
                                rule_name, instance_name_, ctx, &out_param, forward<types_t>(_t)...);
                            if (!op_err.ok()) {
                                rodsLog(LOG_DEBUG,
                                        "Pre-pep rule [%s] failed with error code [%d]",
                                        rule_name.c_str(),
                ...
                if (!to_return_op_err.ok()) {
                    rodsLog(LOG_DEBUG,
                            "Rule [%s] failed with error code [%d], post-pep not executed",
                            _operation_name.c_str(),
                            op_err.code());
                    return to_return_op_err;
                }
