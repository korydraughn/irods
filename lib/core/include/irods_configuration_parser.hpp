...
namespace irods
{
            ...
            for (const auto& key : _keys) {
                try {
                    if (!cur_val) {
                        ...
                        try {
                            cur_val.reset(
                                boost::any_cast<std::unordered_map<std::string, boost::any>&>(*cur_val).at(key));
                        }
                        catch (const boost::bad_any_cast&) {
                            THROW(INVALID_ANY_CAST, "value was not a map");
                        }
    ...
    private:
        ...
        boost::any convert_json(json_t*); // jansson object

        error copy_and_swap(const std::unordered_map<std::string, boost::any>&); // object to swap in
