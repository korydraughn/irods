...
namespace irods
{
    static error resolve_plugin_path(const std::string& _type, std::string& _path)
    {
        ...
        try {
            ...
            rodsLog(LOG_DEBUG, "resolved plugin home [%s]", _path.c_str());
    ...
    /**
     * \fn PluginType* load_plugin( PluginType*& _plugin, const std::string& _plugin_name, const std::string&
     *_interface, const std::string& _instance_name, const Ts&... _args );
     *
                      ...
                      const Ts&... _args)
    {
        ...
        if (!handle) {
            std::stringstream msg;
            msg << "failed to open shared object file [" << so_name << "] :: dlerror: is [" << dlerror() << "]";
        ...
        rodsLog(LOG_DEBUG, "load_plugin - loaded [%s]", _plugin_name.c_str());
