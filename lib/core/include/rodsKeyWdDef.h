...
#define CROSS_ZONE_CREATE_KW                                                                                           \
    "replDataObjInp" /* use the same for backward                                                                      \
                      * compatibility */
...
#define STREAM_STDOUT_KW                                                                                               \
    "streamStdout"            /* the stream stdout for                                                                 \
                               * execCmd */
...
#define ACL_COLLECTION_KW                                                                                              \
    "aclCollection" /* the collection from which                                                                       \
                     * the ACL should be used */
#define NO_CHK_COPY_LEN_KW                                                                                             \
    "noChkCopyLen"         /* Don't check the len                                                                      \
                            * when transfering  */
#define TICKET_KW "ticket" /* for ticket-based-access */
#define PURGE_CACHE_KW                                                                                                 \
    "purgeCache" /* purge the cache copy right JMC - backport 4537                                                     \
                  * after the operation */
...
// JMC - backport 4599
#define LOCK_TYPE_KW                                                                                                   \
    "lockType" /* valid values are READ_LOCK_TYPE                                                                      \
                * WRITE_LOCK_TYPE and UNLOCK_TYPE */
#define LOCK_CMD_KW                                                                                                    \
    "lockCmd"                           /* valid values are SET_LOCK_WAIT_CMD,                                         \
