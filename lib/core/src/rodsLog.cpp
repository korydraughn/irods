...
void rodsLog(int level, const char* formatStr, ...)
{
    ...
    errOrOut = stdout;
#endif
    if (ProcessType == SERVER_PT || ProcessType == AGENT_PT || ProcessType == RE_SERVER_PT) {
        char timeBuf[100];
        time(&timeValue);
...
void rodsLogAndErrorMsg(int level, rError_t* myError, int status, const char* formatStr, ...)
{
    ...
    if (message_len > 0 && message[message_len - 1] == '\n') {
        ...
        if (myError != NULL) {
            snprintf(errMsg, ERR_MSG_LEN, "%s: %s", prefix, message);
        ...
        if (myError != NULL) {
            snprintf(errMsg, ERR_MSG_LEN, "%s: %s\n", prefix, message);
            addRErrorMsg(myError, status, errMsg);
        }
#else
        sprintf(nt_log_msg, "%s%s: %s\n", extraInfo, prefix, message);
...
{
    myPid = getpid();
    if (sqlVerbosityLevel)
        rodsLog(LOG_SQL, "pid: %d sql: %s", myPid, sql);
}
void rodsLogSqlResult(char* stat)
{
    myPid = getpid();
    if (sqlVerbosityLevel)
        rodsLog(LOG_SQL, "pid: %d result: %s", myPid, stat);
}
...
const char* rodsErrorName(int errorValue, char** subName)
{
    ...
    if (subName) {
        ...
        *subName = subCode && errorValue < 0 ? strdup(strerror(-subCode)) : strdup("");
    }
...
void rodsLogErrorOld(int level, int rodsErrorCode, char* textStr)
{
    ...
    if (textStr && strlen(textStr) > 0) {
        rodsLog(level, "%s Error: %d: %s, %s", textStr, rodsErrorCode, errName, errSubName);
    }
    else {
        rodsLog(level, "Error: %d: %s, %s", rodsErrorCode, errName, errSubName);
    }
    free(errSubName);
}
...
void rodsLogError(int level, int rodsErrorCode, char* formatStr, ...)
{
    ...
    if (strlen(errSubName) > 0) {
        rodsLog(level, "%s status = %d %s, %s", message, rodsErrorCode, errName, errSubName);
    }
    else {
        rodsLog(level, "%s status = %d %s", message, rodsErrorCode, errName);
    }
    free(message);
    free(errSubName);
}
