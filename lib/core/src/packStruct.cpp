               ...
               irodsProt_t irodsProt)
{
    ...
    int status = packChildStruct(inStruct, packedOutput, rootPackedItem, myPackTable, 1, packFlag, irodsProt, NULL);
...
int parsePackInstruct(const char* packInstruct, packItem_t& packItemHead)
{
    ...
    while (copyStrFromPiBuf(inptr, buf, 0) > 0) {
        ...
        if (strcmp(buf, ";") == 0) { /* delimiter */
            if (gotTypeCast == 0 && gotItemName == 0) {
                ...
                rodsLog(LOG_ERROR, "parsePackInstruct: No varName for %s", packInstruct);
            ...
            /* int dependent type */
            if (gotTypeCast > 0 || gotItemName > 0) {
                rodsLog(LOG_ERROR, "parsePackInstruct: % position error for %s", packInstruct);
                if (myPackItem != &packItemHead) {
                    free(myPackItem);
                }
                return SYS_PACK_INSTRUCT_FORMAT_ERR;
            ...
            if (myPackItem->typeInx < 0) {
                rodsLog(LOG_ERROR, "parsePackInstruct: packTypeLookup failed for %s", buf);
                if (myPackItem != &packItemHead) {
                    free(myPackItem);
                }
                return SYS_PACK_INSTRUCT_FORMAT_ERR;
            ...
            if (outLen <= 0) {
                rodsLog(LOG_ERROR, "parsePackInstruct: ? No variable following ? for %s", packInstruct);
                if (myPackItem != &packItemHead) {
                    free(myPackItem);
                }
                return SYS_PACK_INSTRUCT_FORMAT_ERR;
            ...
            myPackItem->typeInx = (packTypeInx_t) packTypeLookup(buf);
            if (myPackItem->typeInx < 0) {
                rodsLog(LOG_ERROR, "parsePackInstruct: packTypeLookup failed for %s in %s", buf, packInstruct);
                if (myPackItem != &packItemHead) {
                    free(myPackItem);
                }
                return SYS_PACK_INSTRUCT_FORMAT_ERR;
            ...
            rodsLog(LOG_ERROR, "parsePackInstruct: too many string around %s in %s", buf, packInstruct);
            if (myPackItem != &packItemHead) {
                free(myPackItem);
            }
            return SYS_PACK_INSTRUCT_FORMAT_ERR;
    ...
    if (myPackItem != NULL) {
        rodsLog(LOG_ERROR, "parsePackInstruct: Pack Instruction %s not properly terminated", packInstruct);
        if (myPackItem != &packItemHead) {
            free(myPackItem);
        }
        return SYS_PACK_INSTRUCT_FORMAT_ERR;
...
{
    return {.bBuf = {.buf = malloc(len), .len = 0}, .bufSize = len, .nopackBufArray = {}};
}

packedOutput_t initPackedOutputWithBuf(void* buf, const int len)
{
    return {.bBuf = {.buf = buf, .len = 0}, .bufSize = len, .nopackBufArray = {}};
}
...
int resolvePackedItem(packItem_t& myPackedItem, const void*& inPtr, packOpr_t packOpr)
{
    ...
    if (strlen(myPackedItem.name) == 0) {
        if (myPackedItem.pointerType == 0 || myPackedItem.pointer != NULL) {
            rodsLog(LOG_ERROR, "resolvePackedItem: Cannot resolve %s", myPackedItem.strValue);
            return SYS_PACK_INSTRUCT_FORMAT_ERR;
        }
...
int resolveIntDepItem(packItem_t& myPackedItem)
{
    ...
    while ((c = *tmpPtr) != '\0') {
        if (c == ' ' || c == '\t' || c == '\n' || c == ':') {
            ...
            rodsLog(LOG_ERROR, "resolveIntDepItem: end reached while parsing for key: %s", myPackedItem.name);
            return SYS_PACK_INSTRUCT_FORMAT_ERR;
        }
    ...
    if (keyVal == SYS_PACK_INSTRUCT_FORMAT_ERR) {
        rodsLog(LOG_ERROR, "resolveIntDepItem: resolveIntInItem error for %s", buf);
        return SYS_PACK_INSTRUCT_FORMAT_ERR;
    }
                ...
                if (c == ',') {
                    ...
                    while (1) {
                        ...
                        if (c == '=') {
                            ...
                            rodsLog(LOG_ERROR, "resolveIntDepItem: end reached with no PI%s", myPackedItem.name);
                            return SYS_PACK_INSTRUCT_FORMAT_ERR;
                        }
                        tmpPtr++;
                    }
    ...
    if (status < 0) {
        ...
        rodsLog(LOG_ERROR, "resolveIntDepItem: parsePackFormat error for =%s", myPI);
        return status;
    }
...
int resolveIntInItem(const char* name, const packItem_t& myPackedItem)
{
    ...
    while (tmpPackedItem != NULL) {
        if (strcmp(name, tmpPackedItem->name) == 0 && packTypeTable[tmpPackedItem->typeInx].number == PACK_INT_TYPE) {
...
int resolveStrInItem(packItem_t& myPackedItem)
{
    ...
    if (tmpPackedItem == NULL) {
        rodsLog(LOG_ERROR, "resolveStrInItem: Cannot resolve %s in %s", name, myPackedItem.name);
        return SYS_PACK_INSTRUCT_FORMAT_ERR;
    }
...
const char* matchPackInstruct(const char* name, const packInstruct_t* myPackTable)
{
    ...
    rodsLog(LOG_ERROR, "matchPackInstruct: Cannot resolve %s", name);
...
int resolveDepInArray(packItem_t& myPackedItem)
{
    ...
    for (size_t nameIndex = 0; '\0' != myPackedItem.name[nameIndex]; nameIndex++) {
        ...
        if ('[' == c || '(' == c) {
            if (openSymbol) {
                rodsLog(LOG_ERROR, "resolveDepInArray: got %c inside %c for %s", c, openSymbol, myPackedItem.name);
                return SYS_PACK_INSTRUCT_FORMAT_ERR;
            }
            else if (('[' == c && myPackedItem.dim >= MAX_PACK_DIM) ||
                     ('(' == c && myPackedItem.hintDim >= MAX_PACK_DIM)) {
                rodsLog(
                    LOG_ERROR, "resolveDepInArray: dimension of %s larger than %d", myPackedItem.name, MAX_PACK_DIM);
                return SYS_PACK_INSTRUCT_FORMAT_ERR;
            }
            openSymbol = c;
            ...
            if ((']' == c && '[' != openSymbol) || (')' == c && '(' != openSymbol)) {
                rodsLog(LOG_ERROR,
                        "resolveDepInArray: Got %c without %c for %s",
                        c,
                        (']' == c) ? '[' : '(',
                        myPackedItem.name);
                return SYS_PACK_INSTRUCT_FORMAT_ERR;
                ...
                rodsLog(LOG_ERROR, "resolveDepInArray: Empty %c%c in %s", (']' == c) ? '[' : '(', c, myPackedItem.name);
                return SYS_PACK_INSTRUCT_FORMAT_ERR;
            }
            openSymbol = '\0';

            int& dimSize = (']' == c) ? myPackedItem.dimSize[myPackedItem.dim++]
                       ...
                       irodsProt_t irodsProt)
{
    ...
    switch (myTypeNum) {
        ...
        case PACK_BIN_TYPE:
            ...
            status = packChar(
                inPtr, packedOutput, numElement * elementSz, myPackedItem.name, myPackedItem.typeInx, irodsProt);
            if (status < 0) {
                return status;
            }
            break;
        ...
        case PACK_PI_STR_TYPE: {
            ...
            for (i = 0; i < numStr; i++) {
                status = packString(inPtr, packedOutput, maxStrLen, myPackedItem.name, irodsProt);
                if (status < 0) {
                    rodsLog(LOG_ERROR,
                            "packNonpointerItem:strlen %d of %s > dim size %d,content:%s",
                            strlen((const char*) inPtr),
                            myPackedItem.name,
                            maxStrLen,
                            inPtr);
                    return status;
        ...
        case PACK_INT_TYPE:
            ...
            status = packInt(inPtr, packedOutput, numElement, myPackedItem.name, irodsProt);
            if (status < 0) {
                return status;
            }
        ...
        case PACK_INT16_TYPE:
            ...
            status = packInt16(inPtr, packedOutput, numElement, myPackedItem.name, irodsProt);
            if (status < 0) {
                return status;
            }
            myPackedItem.intValue = status;
        ...
        case PACK_DOUBLE_TYPE:
            ...
            status = packDouble(inPtr, packedOutput, numElement, myPackedItem.name, irodsProt);
            if (status < 0) {
                return status;
            }
            break;
        ...
        case PACK_STRUCT_TYPE:
            ...
            status =
                packChildStruct(inPtr, packedOutput, myPackedItem, myPackTable, numElement, packFlag, irodsProt, NULL);
                    ...
                    irodsProt_t irodsProt)
{
    ...
    const void* const* pointerArray =
        (myDim == 0) ? singletonArray : static_cast<const void* const*>(myPackedItem.pointer);
    if (myDim == 0) {
        singletonArray[0] = myPackedItem.pointer;
        if (myTypeNum == PACK_PI_STR_TYPE) {
            /* save the str */
            snprintf(myPackedItem.strValue,
                     sizeof(myPackedItem.strValue),
                     "%s",
                     static_cast<const char*>(myPackedItem.pointer));
        }
    }
    ...
    switch (myTypeNum) {
        ...
        case PACK_BIN_TYPE:
            for (i = 0; i < numPointer; i++) {
                ...
                if (myPackedItem.pointerType == NO_PACK_POINTER) {
                    status = packNopackPointer(
                        const_cast<void*>(pointer), packedOutput, numElement * elementSz, myPackedItem.name, irodsProt);
                }
                else {
                    status = packChar(pointer,
                                      packedOutput,
                                      numElement * elementSz,
                                      myPackedItem.name,
                                      myPackedItem.typeInx,
                                      irodsProt);
                }
                if ((packFlag & FREE_POINTER) && myPackedItem.pointerType == A_POINTER) {
            ...
            if ((packFlag & FREE_POINTER) && myPackedItem.pointerType == A_POINTER && numPointer > 0 && myDim > 0) {
                /* Array of pointers */
        ...
        case PACK_PI_STR_TYPE: {
            ...
            for (j = 0; j < numPointer; j++) {
                ...
                for (i = 0; i < numStr; i++) {
                    status = packString(pointer, packedOutput, maxStrLen, myPackedItem.name, irodsProt);
                    if (status < 0) {
                        rodsLog(LOG_ERROR,
        ...
        case PACK_INT_TYPE:
            for (i = 0; i < numPointer; i++) {
                ...
                status = packInt(pointer, packedOutput, numElement, myPackedItem.name, irodsProt);
                if ((packFlag & FREE_POINTER) && myPackedItem.pointerType == A_POINTER) {
                    free(const_cast<void*>(origPtr));
                }
                if (status < 0) {
        ...
        case PACK_INT16_TYPE:
            for (i = 0; i < numPointer; i++) {
                ...
                status = packInt16(pointer, packedOutput, numElement, myPackedItem.name, irodsProt);
                if ((packFlag & FREE_POINTER) && myPackedItem.pointerType == A_POINTER) {
                    free(const_cast<void*>(origPtr));
                }
                if (status < 0) {
        ...
        case PACK_DOUBLE_TYPE:
            for (i = 0; i < numPointer; i++) {
                ...
                status = packDouble(pointer, packedOutput, numElement, myPackedItem.name, irodsProt);
                if ((packFlag & FREE_POINTER) && myPackedItem.pointerType == A_POINTER) {
                    free(const_cast<void*>(origPtr));
                }
                if (status < 0) {
        ...
        case PACK_STRUCT_TYPE:
            ...
            for (i = 0; i < numPointer; i++) {
                origPtr = pointer = pointerArray[i];
                status = packChildStruct(
                    pointer, packedOutput, myPackedItem, myPackTable, numElement, packFlag, irodsProt, NULL);
                if ((packFlag & FREE_POINTER) && myPackedItem.pointerType == A_POINTER) {
                    free(const_cast<void*>(origPtr));
                }
                if (status < 0) {
             ...
             irodsProt_t irodsProt)
{
    ...
    if (myPackedItem.pointerType > 0) {
        /* a pointer type */
        status = packPointerItem(myPackedItem, packedOutput, myPackTable, packFlag, irodsProt);
    }
    else {
        status = packNonpointerItem(myPackedItem, inPtr, packedOutput, myPackTable, packFlag, irodsProt);
    }
             ...
             irodsProt_t irodsProt)
{
    ...
    if (irodsProt == XML_PROT && packTypeTable[typeInx].number == PACK_BIN_TYPE) {
               ...
               irodsProt_t irodsProt)
{
    ...
    if (irodsProt == XML_PROT) {
        ...
        origDoublePtr = tmpDoublePtr = (rodsLong_t*) malloc(sizeof(rodsLong_t) * numElement);
        ...
        extendPackedOutput(packedOutput, sizeof(rodsLong_t) * numElement, outPtr);
                    ...
                    const char* packInstructInp)
{
    ...
    for (int i = 0; i < numElement; i++) {
        ...
        while (tmpItem != NULL) {
            ...
            if (tmpItem->pointerType == 0 && packTypeTable[tmpItem->typeInx].number == PACK_DOUBLE_TYPE) {
                doubleInStruct = 1;
            }
#endif
            int status = packItem(*tmpItem, inPtr, packedOutput, myPackTable, packFlag, irodsProt);
            if (status < 0) {
                return status;
            }
               ...
               irodsProt_t irodsProt)
{
    ...
    if (myPackedItem.pointerType > 0) {
        /* a pointer type */
        status = unpackPointerItem(myPackedItem, inPtr, unpackedOutput, myPackTable, irodsProt);
    }
    else {
        status = unpackNonpointerItem(myPackedItem, inPtr, unpackedOutput, myPackTable, irodsProt);
    }
                         ...
                         irodsProt_t irodsProt)
{
    ...
    switch (myTypeNum) {
        ...
        case PACK_BIN_TYPE:
            status = unpackChar(
                inPtr, unpackedOutput, numElement * elementSz, myPackedItem.name, myPackedItem.typeInx, irodsProt);
            if (status < 0) {
                return status;
            }
            break;
        ...
        case PACK_PI_STR_TYPE: {
            ...
            for (i = 0; i < numStr; i++) {
                char* outStr = NULL;
                status = unpackString(inPtr, unpackedOutput, maxStrLen, myPackedItem.name, irodsProt, outStr);
                if (status < 0) {
                    rodsLog(LOG_ERROR,
        ...
        case PACK_INT_TYPE:
            status = unpackInt(inPtr, unpackedOutput, numElement, myPackedItem.name, irodsProt);
            if (status < 0) {
                return status;
            }
            myPackedItem.intValue = status;
            break;
        case PACK_INT16_TYPE:
            status = unpackInt16(inPtr, unpackedOutput, numElement, myPackedItem.name, irodsProt);
            if (status < 0) {
                return status;
            }
            myPackedItem.intValue = status;
            break;
        case PACK_DOUBLE_TYPE:
            status = unpackDouble(inPtr, unpackedOutput, numElement, myPackedItem.name, irodsProt);
            if (status < 0) {
                return status;
            }
            break;
        case PACK_STRUCT_TYPE:
            status = unpackChildStruct(inPtr, unpackedOutput, myPackedItem, myPackTable, numElement, irodsProt, NULL);
            ...
            break;
        default:
            rodsLog(LOG_ERROR, "unpackNonpointerItem: Unknow type %d - %s ", myTypeNum, myPackedItem.name);
            return SYS_PACK_INSTRUCT_FORMAT_ERR;
    }
    return status;
}
...
int unpackXmlCharToOutPtr(const void*& inPtr, void*& outPtr, int len, const char* name, const packTypeInx_t typeInx)
{
    ...
    if (packTypeTable[typeInx].number == PACK_BIN_TYPE) {
        ...
        if (int status = base64_decode(
                static_cast<const unsigned char*>(inPtr), inLen, static_cast<unsigned char*>(outPtr), &outLen)) {
            return status;
        }
        if (static_cast<int>(outLen) != len) {
            rodsLog(LOG_NOTICE, "unpackXmlCharToOutPtr: required len %d != %d from base64_decode", len, outLen);
        }
    }
    else {
        if (inLen != len) {
            rodsLog(LOG_NOTICE, "unpackXmlCharToOutPtr: required len %d != %d from input", len, inLen);
            if (inLen > len) {
                return USER_PACKSTRUCT_INPUT_ERR;
            }
        }
                 ...
                 char*& outStr)
{
    ...
    if (irodsProt == XML_PROT) {
        status = unpackXmlString(inPtr, unpackedOutput, maxStrLen, name, outStr);
    }
    else {
...
int unpackStringToOutPtr(const void*& inPtr, void*& outPtr, int maxStrLen, const char* name, irodsProt_t irodsProt)
{
    ...
    if (irodsProt == XML_PROT) {
        status = unpackXmlStringToOutPtr(inPtr, outPtr, maxStrLen, name);
    }
    else {
              ...
              irodsProt_t irodsProt)
{
    ...
    /* adjust len */
    unpackedOutput.bBuf.len =
        static_cast<int>(static_cast<char*>(outPtr) - static_cast<char*>(unpackedOutput.bBuf.buf)) +
    ...
    if (inPtr == NULL) {
        ...
        for (i = 0; i < numElement; i++) {
            ...
            if (myStrlen < 0) {
                ...
                rodsLog(LOG_ERROR, "unpackXmlIntToOutPtr: input %s with value %s too long", name, inPtr);
                return USER_PACKSTRUCT_INPUT_ERR;
            }
                 ...
                 irodsProt_t irodsProt)
{
    ...
    extendPackedOutput(unpackedOutput, sizeof(rodsLong_t) * (numElement + 1), outPtr);
...
int unpackDoubleToOutPtr(const void*& inPtr, void*& outPtr, int numElement, const char* name, irodsProt_t irodsProt)
{
    ...
    if (irodsProt == XML_PROT) {
        status = unpackXmlDoubleToOutPtr(inPtr, outPtr, numElement, name);
    }
    else {
    ...
    if (inPtr == NULL) {
        ...
        for (i = 0; i < numElement; i++) {
            ...
            if (myStrlen < 0) {
                ...
                rodsLog(LOG_ERROR, "unpackXmlDoubleToOutPtr: input %s with value %s too long", name, inPtr);
                return USER_PACKSTRUCT_INPUT_ERR;
            }
            strncpy(tmpStr, (const char*) inPtr, myStrlen);
            tmpStr[myStrlen] = '\0';
    ...
    if (myDim > 0) {
        if (numPointer > 0) {
            ...
            if (myTypeNum == PACK_DOUBLE_TYPE || myTypeNum == PACK_INT_TYPE || myTypeNum == PACK_INT16_TYPE) {
                ...
                pointerArray =
                    static_cast<void**>(addPointerToPackedOut(unpackedOutput, allocLen * sizeof(void*), NULL));
            }
        }
        else {
            return 0;
    ...
    switch (myTypeNum) {
        ...
        case PACK_BIN_TYPE:
            if (myDim == 0) {
                if (myPackedItem.pointerType == NO_PACK_POINTER) {
                    ...
                    status = unpackCharToOutPtr(
                        inPtr, outPtr, numElement * elementSz, myPackedItem.name, myPackedItem.typeInx, irodsProt);
                }
        ...
        case PACK_STRUCT_TYPE: {
            ...
            if (myDim == 0) {
                ...
                status =
                    unpackChildStruct(inPtr, subPackedOutput, myPackedItem, myPackTable, numElement, irodsProt, NULL);
                ...
                for (i = 0; i < numPointer; i++) {
                    ...
                    outPtr = malloc(numElement * SUB_STRUCT_ALLOC_SZ);
                    packedOutput_t subPackedOutput = initPackedOutputWithBuf(outPtr, numElement * SUB_STRUCT_ALLOC_SZ);
                    status = unpackChildStruct(
                        inPtr, subPackedOutput, myPackedItem, myPackTable, numElement, irodsProt, NULL);
...
void* addPointerToPackedOut(packedOutput_t& packedOutput, int len, void* pointer)
{
    ...
    packedOutput.bBuf.len =
        static_cast<int>(static_cast<char*>(outPtr) - static_cast<char*>(packedOutput.bBuf.buf)) + sizeof(void*);
...
int unpackNatStringToOutPtr(const void*& inPtr, void*& outPtr, int maxStrLen)
{
    ...
    if (inPtr == NULL) {
        rodsLog(LOG_ERROR, "unpackStringToOutPtr: NULL inPtr");
        return SYS_PACK_INSTRUCT_FORMAT_ERR;
    }
    myStrlen = strlen((const char*) inPtr);
...
int getAllocLenForStr(const packItem_t& myPackedItem, const void* inPtr, int numStr, int maxStrLen)
{
    ...
    if (numStr <= 1) {
        ...
        if (maxStrLen < 0) {
            rodsLog(LOG_ERROR, "unpackPointerItem: maxStrLen < 0 with numStr > 1 for %s", myPackedItem.name);
            return SYS_PACK_INSTRUCT_FORMAT_ERR;
        }
...
int parseXmlTag(const void* inPtr, const char* name, int flag, int& skipLen)
{
    ...
    if (flag & END_TAG_FL) {
        ...
        if ((tmpPtr = strstr(inStrPtr, endTag)) == NULL) {
            rodsLog(LOG_ERROR, "parseXmlTag: XML end tag error for %s, expect </%s>", inPtr, name);
            return SYS_PACK_INSTRUCT_FORMAT_ERR;
        }
        ...
        if (strncmp(inStrPtr, name, nameLen) != 0) {
            /* this can be normal */
            rodsLog(LOG_DEBUG10, "parseXmlValue: XML start tag error for %s, expect <%s>", inPtr, name);
            return SYS_PACK_INSTRUCT_FORMAT_ERR;
        }
        inStrPtr += nameLen;
        myLen += nameLen;
...
int alignPackedOutput64(packedOutput_t& packedOutput)
{
    ...
    packedOutput.bBuf.len =
        packedOutput.bBuf.len + 8 - static_cast<int>(static_cast<char*>(alignedOutPtr) - static_cast<char*>(outPtr));
