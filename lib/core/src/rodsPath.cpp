...
int parseRodsPath(rodsPath_t* rodsPath, rodsEnv* myRodsEnv)
{
    ...
    if (len == 0) {
    ...
    else if (strcmp(rodsPath->inPath, ".") == 0 || strcmp(rodsPath->inPath, "./") == 0) {
        /* '.' or './' */
        rstrcpy(rodsPath->outPath, myRodsEnv->rodsCwd, MAX_NAME_LEN);
        rodsPath->objType = COLL_OBJ_T;
        return 0;
             ...
             strcmp(rodsPath->inPath, "^") == 0 || strcmp(rodsPath->inPath, "^/") == 0) {
        /* ~ or ~/ */
        ...
        if (rodsPath->inPath[1] == '/') {
            snprintf(rodsPath->outPath, MAX_NAME_LEN, "%s/%s", myRodsEnv->rodsHome, rodsPath->inPath + 2);
        }
        else {
            /* treat it like a relative path */
            snprintf(rodsPath->outPath, MAX_NAME_LEN, "%s/%s", myRodsEnv->rodsCwd, rodsPath->inPath + 2);
        }
    }
        ...
        /* a relative path */
        snprintf(rodsPath->outPath, MAX_NAME_LEN, "%s/%s", myRodsEnv->rodsCwd, rodsPath->inPath);
    }
    ...
    while ((tmpPtr1 = strstr(rodsPath->outPath, "/../")) != NULL) {
        ...
        while (*tmpPtr2 != '/') {
            ...
            if (tmpPtr2 < rodsPath->outPath) {
                rodsLog(LOG_ERROR, "parseRodsPath: parsing error for %s", rodsPath->outPath);
...
int addSrcInPath(rodsPathInp_t* rodsPathInp, const char* inPath)
{
    ...
    if (rodsPathInp == NULL || inPath == NULL) {
        rodsLog(LOG_ERROR, "addSrcInPath: NULL input");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if ((rodsPathInp->numSrc % PTR_ARRAY_MALLOC_LEN) == 0) {
        ...
        if (rodsPathInp->numSrc > 0) {
            memcpy(newSrcPath, rodsPathInp->srcPath, rodsPathInp->numSrc * sizeof(rodsPath_t));
            memcpy(newTargPath, rodsPathInp->targPath, rodsPathInp->numSrc * sizeof(rodsPath_t));
                     ...
                     rodsPathInp_t* rodsPathInp)
{
    ...
    if (destFileType != NO_INPUT_T) {
        ...
        if (nInput > 1) {
            rstrcpy(rodsPathInp->destPath->inPath, argv[argc - 1], MAX_NAME_LEN);
        }
        else {
...
int getLastPathElement(char* inInPath, char* lastElement)
{
    ...
    if (len >= 1) {
        tmpPtr2 = tmpPtr1 - 1;
        if (*tmpPtr2 == '.' || *tmpPtr2 == '~' || *tmpPtr2 == '^' || *tmpPtr2 == '/') {
            *tmpPtr2 = '\0';
            return 0;
        }
    }
