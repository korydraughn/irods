...
int mkdirUtil(rcComm_t* conn, rodsArguments_t* myRodsArgs, rodsPathInp_t* rodsPathInp)
{
    ...
    for (i = 0; i < rodsPathInp->numSrc; i++) {
        rstrcpy(collCreateInp.collName, rodsPathInp->srcPath[i].outPath, MAX_NAME_LEN);
        status = rcCollCreate(conn, &collCreateInp);
        if (status < 0) {
            rodsLogError(LOG_ERROR, status, "mkdirUtil: mkColl of %s error.", rodsPathInp->srcPath[i].outPath);
            savedStatus = status;
        }
    }
