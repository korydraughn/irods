...
namespace irods
{
    // =-=-=-=-=-=-=-
    // public - ctor
    pam_auth_object::pam_auth_object(rError_t* _r_error)
    ...
    pam_auth_object::pam_auth_object(const pam_auth_object& _rhs)
        : auth_object(_rhs)
    {
    ...
    // =-=-=-=-=-=-=-
    // public - assignment operator
    pam_auth_object& pam_auth_object::operator=(const pam_auth_object& _rhs)
    {
    ...
    bool pam_auth_object::operator==(const pam_auth_object&) const
    {
        return false;
    }
    ...
    error pam_auth_object::resolve(const std::string& _interface, plugin_ptr& _ptr)
    {
        // =-=-=-=-=-=-=-
        ...
        auth_ptr a_ptr;
        error ret = auth_mgr.resolve(AUTH_PAM_SCHEME, a_ptr);
        if (!ret.ok()) {
            // =-=-=-=-=-=-=-
            ...
            ret = auth_mgr.init_from_type(
                ProcessType, AUTH_PAM_SCHEME, AUTH_PAM_SCHEME, AUTH_PAM_SCHEME, empty_context, a_ptr);
            if (!ret.ok()) {
                return PASS(ret);
            }
            else {
    ...
    error pam_auth_object::get_re_vars(rule_engine_vars_t& _kvp)
    {
        // =-=-=-=-=-=-=-
