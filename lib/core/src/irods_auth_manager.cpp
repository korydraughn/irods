...
namespace irods
{
    ...
    auth_manager::auth_manager(const auth_manager& _rhs)
    {
    ...
    error auth_manager::resolve(const std::string& _key, auth_ptr& _value)
    {
        error result = SUCCESS();

        if ((result = ASSERT_ERROR(!_key.empty(), SYS_INVALID_INPUT_PARAM, "Empty plugin name.")).ok()) {
            if ((result = ASSERT_ERROR(plugins_.has_entry(_key),
                                       SYS_INVALID_INPUT_PARAM,
                                       "No auth plugin found for name: \"%s\".",
                                       _key.c_str()))
                    .ok()) {
    ...
    static error load_auth_plugin(auth_ptr& _plugin,
                                  ...
                                  const std::string& _context)
    {
        ...
        auth* ath = 0;
        error ret = load_plugin<auth>(ath, _plugin_name, PLUGIN_TYPE_AUTHENTICATION, _inst_name, _context);
    ...
    error auth_manager::init_from_type(const int& _proc_type,
