...
namespace irods
{
    krb_auth_object::krb_auth_object(rError_t* _r_error)
    ...
    error krb_auth_object::resolve(const std::string& _interface, plugin_ptr& _ptr)
    {
        ...
        if ((result = ASSERT_ERROR(_interface == AUTH_INTERFACE,
            ...
            if (!(result = ASSERT_PASS(ret, "Failed to resolve the KRB auth plugin.")).ok()) {
                ...
                ret = auth_mgr.init_from_type(
                    ProcessType, AUTH_KRB_SCHEME, AUTH_KRB_SCHEME, AUTH_KRB_SCHEME, empty_context, ath);
    ...
    bool krb_auth_object::operator==(const krb_auth_object& _rhs) const
    {
    ...
    error krb_auth_object::get_re_vars(rule_engine_vars_t& _kvp)
    {
