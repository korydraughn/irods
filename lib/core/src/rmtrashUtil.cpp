    ...
    if (rodsPathInp->numSrc <= 0) {
        ...
        if (myRodsArgs->admin == True) {
            if (myRodsArgs->user == True) {
                snprintf(trashPath, MAX_NAME_LEN, "/%s/trash/home/%s", myZoneName, myRodsArgs->userString);
            }
            else if (myRodsArgs->orphan == True) {
                snprintf(trashPath, MAX_NAME_LEN, "/%s/trash/orphan", myZoneName);
            }
            else {
                snprintf(trashPath, MAX_NAME_LEN, "/%s/trash/home", myZoneName);
            }
        }
        else {
            int remoteFlag = 0;
            status = getZoneType(conn, myZoneName, conn->clientUser.rodsZone, myZoneType);
            if (status >= 0) {
            ...
            if (remoteFlag == 0) {
                snprintf(trashPath, MAX_NAME_LEN, "/%s/trash/home/%s", myZoneName, conn->clientUser.userName);
            }
            else {
                snprintf(trashPath,
                         MAX_NAME_LEN,
    ...
    for (i = 0; i < rodsPathInp->numSrc; i++) {
        if (rodsPathInp->srcPath[i].objType == UNKNOWN_OBJ_T) {
            ...
            if (rodsPathInp->srcPath[i].objState == NOT_EXIST_ST) {
                rodsLog(LOG_ERROR, "rmtrashUtil: srcPath %s does not exist", rodsPathInp->srcPath[i].outPath);
        ...
        if (rodsPathInp->srcPath[i].objType == DATA_OBJ_T) {
            status = rmtrashDataObjUtil(conn, rodsPathInp->srcPath[i].outPath, myRodsArgs, &dataObjInp);
        }
        else if (rodsPathInp->srcPath[i].objType == COLL_OBJ_T) {
            status = rmtrashCollUtil(conn, rodsPathInp->srcPath[i].outPath, myRodsArgs, &collInp);
        }
        else {
            /* should not be here */
            rodsLog(LOG_ERROR,
...
int rmtrashDataObjUtil(rcComm_t* conn, char* srcPath, rodsArguments_t* rodsArgs, dataObjInp_t* dataObjInp)
{
    ...
    if (srcPath == NULL) {
        rodsLog(LOG_ERROR, "rmtrashDataObjUtil: NULL srcPath input");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (status >= 0 && rodsArgs->verbose == True) {
        ...
        printTiming(conn, dataObjInp->objPath, -1, NULL, &startTime, &endTime);
    }
...
int initCondForRmtrash(rodsArguments_t* rodsArgs, dataObjInp_t* dataObjInp, collInp_t* collInp)
{
    ...
    if (dataObjInp == NULL) {
        rodsLog(LOG_ERROR, "initCondForRmtrash: NULL dataObjInp input");
        return USER__NULL_INPUT_ERR;
    }
...
int rmtrashCollUtil(rcComm_t* conn, char* srcColl, rodsArguments_t* rodsArgs, collInp_t* collInp)
{
    ...
    if (srcColl == NULL) {
        rodsLog(LOG_ERROR, "rmtrashCollUtil: NULL srcColl input");
        return USER__NULL_INPUT_ERR;
    }
