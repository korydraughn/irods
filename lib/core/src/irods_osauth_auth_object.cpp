...
namespace irods
{
    // =-=-=-=-=-=-=-
    // public - ctor
    osauth_auth_object::osauth_auth_object(rError_t* _r_error)
    ...
    // =-=-=-=-=-=-=-
    // public - cctor
    osauth_auth_object::osauth_auth_object(const osauth_auth_object& _rhs)
        : auth_object(_rhs)
    {
    ...
    osauth_auth_object& osauth_auth_object::operator=(const osauth_auth_object& _rhs)
    {
    ...
    bool osauth_auth_object::operator==(const osauth_auth_object&) const
    {
        return false;
    }
    ...
    error osauth_auth_object::resolve(const std::string& _interface, plugin_ptr& _ptr)
    {
        // =-=-=-=-=-=-=-
        ...
        auth_ptr a_ptr;
        error ret = auth_mgr.resolve(AUTH_OSAUTH_SCHEME, a_ptr);
        if (!ret.ok()) {
            // =-=-=-=-=-=-=-
            ...
            ret = auth_mgr.init_from_type(
                ProcessType, AUTH_OSAUTH_SCHEME, AUTH_OSAUTH_SCHEME, AUTH_OSAUTH_SCHEME, empty_context, a_ptr);
            if (!ret.ok()) {
                return PASS(ret);
            }
            else {
    ...
    error osauth_auth_object::get_re_vars(rule_engine_vars_t& _kvp)
    {
        // =-=-=-=-=-=-=-
