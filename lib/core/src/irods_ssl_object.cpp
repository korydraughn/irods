...
namespace irods
{
    ...
    // =-=-=-=-=-=-=-
    // public - ctor
    ssl_object::ssl_object(const rcComm_t& _comm)
    ...
    // =-=-=-=-=-=-=-
    // public - ctor
    ssl_object::ssl_object(const rsComm_t& _comm)
        : network_object(_comm)
        , ssl_ctx_(_comm.ssl_ctx)
        , ssl_(_comm.ssl)
        , host_("")
    ...
    // =-=-=-=-=-=-=-
    // public - cctor
    ssl_object::ssl_object(const ssl_object& _rhs)
        : network_object(_rhs)
    ...
    ssl_object& ssl_object::operator=(const ssl_object& _rhs)
    {
    ...
    bool ssl_object::operator==(const ssl_object& _rhs) const
    {
    ...
    error ssl_object::resolve(const std::string& _interface, plugin_ptr& _ptr)
    {
        // =-=-=-=-=-=-=-
        ...
        if (!ret.ok()) {
            ...
            ret = netwk_mgr.init_from_type(
                ProcessType, SSL_NETWORK_PLUGIN, SSL_NETWORK_PLUGIN, SSL_NETWORK_PLUGIN, empty_context, net_ptr);
            if (!ret.ok()) {
                return PASS(ret);
            }
            else {
    ...
    error ssl_object::get_re_vars(rule_engine_vars_t& _kvp)
    {
    ...
    error ssl_object::to_client(rcComm_t* _comm)
    {
        ...
        std::copy(shared_secret_.begin(), shared_secret_.end(), &_comm->shared_secret[0]);
        _comm->key_size = key_size_;
        _comm->salt_size = salt_size_;
        _comm->num_hash_rounds = num_hash_rounds_;
        snprintf(_comm->encryption_algorithm, sizeof(_comm->encryption_algorithm), "%s", encryption_algorithm_.c_str());
