...
int mkCollR(rcComm_t* conn, char* startColl, char* destColl)
{
    ...
    while (tmpLen < pathLen) {
        ...
        if (status < 0) {
            rodsLog(LOG_NOTICE, "mkCollR: mkdir failed for %s, status =%d", tmpPath, status);
            return status;
        }
...
#endif
        if (status < 0) {
            rodsLog(LOG_NOTICE, "mkdirR: mkdir failed for %s, errno =%d", tmpPath, errno);
...
int mkdirForFilePath(char* filePath)
{
    ...
    if ((status = splitPathByKey(filePath, parent, MAX_NAME_LEN, child, MAX_NAME_LEN, '/')) < 0) {
        rodsLogError(
            LOG_ERROR, status, "mkdirForFilePath:: splitPathByKey for %s error, status = %d", filePath, status);
        return status;
    }
...
int getRodsObjType(rcComm_t* conn, rodsPath_t* rodsPath)
{
    ...
    if (status < 0) {
        ...
        if (status == OBJ_PATH_DOES_NOT_EXIST || status == USER_FILE_DOES_NOT_EXIST) {
            return NOT_EXIST_ST;
        }
        else {
            rodsLogError(LOG_ERROR, status, "rcObjStat of %s failed", rodsPath->outPath);
            return status;
        }
    }
    else if (rodsPath->objType == COLL_OBJ_T && rodsObjStatOut->objType != COLL_OBJ_T) {
        rodsPath->objState = NOT_EXIST_ST;
    }
    else if (rodsPath->objType == DATA_OBJ_T && rodsObjStatOut->objType != DATA_OBJ_T) {
        rodsPath->objState = NOT_EXIST_ST;
    }
    else {
...
int genAllInCollQCond(char* collection, char* collQCond)
{
    ...
    if (strcmp(collection, "/") == 0) {
        ...
        snprintf(collQCond, MAX_NAME_LEN, " = '%s' || like '%s/%%' ", collection, collection);
    }
    return 0;
}
                    ...
                    genQueryOut_t** genQueryOut)
{
    ...
    status = (*queryHandle->genQuery)((rcComm_t*) queryHandle->conn, genQueryInp, genQueryOut);
                       ...
                       keyValPair_t* condInput)
{
    ...
    if ((flags & INCLUDE_CONDINPUT_IN_QUERY) != 0 && condInput != NULL &&
...
int setQueryInpForData(int flags, genQueryInp_t* genQueryInp)
{
    ...
    if ((flags & LONG_METADATA_FG) != 0 || (flags & VERY_LONG_METADATA_FG) != 0) {
                ...
                struct timeval* endTime)
{
    ...
    if ((status = splitPathByKey(objPath, myDir, MAX_NAME_LEN, myFile, MAX_NAME_LEN, '/')) < 0) {
        rodsLogError(LOG_NOTICE, status, "printTiming: splitPathByKey for %s error, status = %d", objPath, status);
        return status;
    }
    ...
    timeInSec = (float) diffTime.tv_sec + ((float) diffTime.tv_usec / 1000000.0);
...
int printTime(char* objPath, struct timeval* startTime, struct timeval* endTime)
{
    ...
    if ((status = splitPathByKey(objPath, myDir, MAX_NAME_LEN, myFile, MAX_NAME_LEN, '/')) < 0) {
        rodsLogError(LOG_NOTICE, status, "printTime: splitPathByKey for %s error, status = %d", objPath, status);
        return status;
    }
...
int printNoSync(char* objPath, rodsLong_t fileSize, char* reason)
{
    ...
    fprintf(stdout, "   %-25.25s  %10.3f MB --- %s no sync required \n", myFile, sizeInMb, reason);
...
int queryCollAclSpecific(rcComm_t* conn, char* collName, char* zoneHint, genQueryOut_t** genQueryOut)
{
    ...
    myGenQueryOut = *genQueryOut = (genQueryOut_t*) malloc(sizeof(genQueryOut_t));
...
{
    genQueryOut_t* myGenQueryOut;
    sqlResult_t *collName, *collType, *collInfo1, *collInfo2, *collOwner, *collCreateTime, *collModifyTime,
        *tmpSqlResult;

    if (genQueryOut == NULL || (myGenQueryOut = *genQueryOut) == NULL || collSqlResult == NULL) {
        return USER__NULL_INPUT_ERR;
    }
    ...
    if ((collName = getSqlResultByInx(myGenQueryOut, COL_COLL_NAME)) == NULL) {
        rodsLog(LOG_ERROR, "genQueryOutToCollRes: getSqlResultByInx for COL_COLL_NAME failed");
    ...
    if ((collType = getSqlResultByInx(myGenQueryOut, COL_COLL_TYPE)) == NULL) {
        ...
        setSqlResultValue(&collSqlResult->collType, COL_COLL_NAME, INHERIT_PAR_SPEC_COLL_STR, myGenQueryOut->rowCnt);
        setSqlResultValue(&collSqlResult->collInfo1, COL_COLL_INFO1, "", myGenQueryOut->rowCnt);
        setSqlResultValue(&collSqlResult->collInfo2, COL_COLL_INFO2, "", myGenQueryOut->rowCnt);
        setSqlResultValue(&collSqlResult->collOwner, COL_COLL_OWNER_NAME, "", myGenQueryOut->rowCnt);
        setSqlResultValue(&collSqlResult->collCreateTime, COL_COLL_CREATE_TIME, "", myGenQueryOut->rowCnt);
        setSqlResultValue(&collSqlResult->collModifyTime, COL_COLL_MODIFY_TIME, "", myGenQueryOut->rowCnt);
        /* myGenQueryOut could came from rcQuerySpecColl call */
        if ((tmpSqlResult = getSqlResultByInx(myGenQueryOut, COL_DATA_NAME)) != NULL) {
            if (tmpSqlResult->value != NULL) {
                free(tmpSqlResult->value);
            }
        }
        if ((tmpSqlResult = getSqlResultByInx(myGenQueryOut, COL_D_CREATE_TIME)) != NULL) {
            if (tmpSqlResult->value != NULL) {
                free(tmpSqlResult->value);
            }
        }
        if ((tmpSqlResult = getSqlResultByInx(myGenQueryOut, COL_D_MODIFY_TIME)) != NULL) {
            if (tmpSqlResult->value != NULL) {
                free(tmpSqlResult->value);
            }
        }
        if ((tmpSqlResult = getSqlResultByInx(myGenQueryOut, COL_DATA_SIZE)) != NULL) {
            if (tmpSqlResult->value != NULL) {
                free(tmpSqlResult->value);
            }
        }
        ...
        if ((collInfo1 = getSqlResultByInx(myGenQueryOut, COL_COLL_INFO1)) == NULL) {
            rodsLog(LOG_ERROR, "genQueryOutToCollRes: getSqlResultByInx COL_COLL_INFO1 failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else {
            collSqlResult->collInfo1 = *collInfo1;
        }
        if ((collInfo2 = getSqlResultByInx(myGenQueryOut, COL_COLL_INFO2)) == NULL) {
            rodsLog(LOG_ERROR, "genQueryOutToCollRes: getSqlResultByInx COL_COLL_INFO2 failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else {
            collSqlResult->collInfo2 = *collInfo2;
        }
        if ((collOwner = getSqlResultByInx(myGenQueryOut, COL_COLL_OWNER_NAME)) == NULL) {
            rodsLog(LOG_ERROR, "genQueryOutToCollRes: getSqlResultByInx COL_COLL_OWNER_NAME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else {
            collSqlResult->collOwner = *collOwner;
        }
        if ((collCreateTime = getSqlResultByInx(myGenQueryOut, COL_COLL_CREATE_TIME)) == NULL) {
            rodsLog(LOG_ERROR, "genQueryOutToCollRes: getSqlResultByInx COL_COLL_CREATE_TIME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else {
            collSqlResult->collCreateTime = *collCreateTime;
        }
        if ((collModifyTime = getSqlResultByInx(myGenQueryOut, COL_COLL_MODIFY_TIME)) == NULL) {
            rodsLog(LOG_ERROR, "genQueryOutToCollRes: getSqlResultByInx COL_COLL_MODIFY_TIME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else {
...
{
    genQueryOut_t* myGenQueryOut;
    sqlResult_t *collName, *dataName, *dataSize, *dataMode, *createTime, *modifyTime, *chksum, *replStatus, *dataId,
        *resource, *resc_hier, *phyPath, *ownerName, *replNum, *dataType;
    if (genQueryOut == NULL || (myGenQueryOut = *genQueryOut) == NULL || dataObjSqlResult == NULL) {
        return USER__NULL_INPUT_ERR;
    }
    ...
    if ((collName = getSqlResultByInx(myGenQueryOut, COL_COLL_NAME)) == NULL) {
        rodsLog(LOG_ERROR, "genQueryOutToDataObjRes: getSqlResultByInx for COL_COLL_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    else {
    ...
    if ((dataName = getSqlResultByInx(myGenQueryOut, COL_DATA_NAME)) == NULL) {
        rodsLog(LOG_ERROR, "genQueryOutToDataObjRes: getSqlResultByInx for COL_DATA_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    else {
    ...
    if ((dataMode = getSqlResultByInx(myGenQueryOut, COL_DATA_MODE)) == NULL) {
        setSqlResultValue(&dataObjSqlResult->dataMode, COL_DATA_MODE, "", myGenQueryOut->rowCnt);
    }
    else {
    ...
    if ((dataSize = getSqlResultByInx(myGenQueryOut, COL_DATA_SIZE)) == NULL) {
        setSqlResultValue(&dataObjSqlResult->dataSize, COL_DATA_SIZE, "-1", myGenQueryOut->rowCnt);
    }
    else {
        dataObjSqlResult->dataSize = *dataSize;
    }

    if ((createTime = getSqlResultByInx(myGenQueryOut, COL_D_CREATE_TIME)) == NULL) {
        setSqlResultValue(&dataObjSqlResult->createTime, COL_D_CREATE_TIME, "", myGenQueryOut->rowCnt);
    }
    else {
        dataObjSqlResult->createTime = *createTime;
    }

    if ((modifyTime = getSqlResultByInx(myGenQueryOut, COL_D_MODIFY_TIME)) == NULL) {
        setSqlResultValue(&dataObjSqlResult->modifyTime, COL_D_MODIFY_TIME, "", myGenQueryOut->rowCnt);
    }
    else {
        dataObjSqlResult->modifyTime = *modifyTime;
    }

    if ((dataId = getSqlResultByInx(myGenQueryOut, COL_D_DATA_ID)) == NULL) {
        setSqlResultValue(&dataObjSqlResult->dataId, COL_D_DATA_ID, "", myGenQueryOut->rowCnt);
    }
    else {
        dataObjSqlResult->dataId = *dataId;
    }

    if ((chksum = getSqlResultByInx(myGenQueryOut, COL_D_DATA_CHECKSUM)) == NULL) {
        setSqlResultValue(&dataObjSqlResult->chksum, COL_D_DATA_CHECKSUM, "", myGenQueryOut->rowCnt);
    }
    else {
        dataObjSqlResult->chksum = *chksum;
    }

    if ((replStatus = getSqlResultByInx(myGenQueryOut, COL_D_REPL_STATUS)) == NULL) {
        setSqlResultValue(&dataObjSqlResult->replStatus, COL_D_REPL_STATUS, "", myGenQueryOut->rowCnt);
    }
    else {
        dataObjSqlResult->replStatus = *replStatus;
    }

    if ((resource = getSqlResultByInx(myGenQueryOut, COL_D_RESC_NAME)) == NULL) {
        setSqlResultValue(&dataObjSqlResult->resource, COL_D_RESC_NAME, "", myGenQueryOut->rowCnt);
    }
    else {
        dataObjSqlResult->resource = *resource;
    }
    if ((resc_hier = getSqlResultByInx(myGenQueryOut, COL_D_RESC_HIER)) == NULL) {
        setSqlResultValue(&dataObjSqlResult->resc_hier, COL_D_RESC_HIER, "", myGenQueryOut->rowCnt);
    }
    else {
        dataObjSqlResult->resc_hier = *resc_hier;
    }

    if ((dataType = getSqlResultByInx(myGenQueryOut, COL_DATA_TYPE_NAME)) == NULL) {
        setSqlResultValue(&dataObjSqlResult->dataType, COL_DATA_TYPE_NAME, "", myGenQueryOut->rowCnt);
    }
    else {
        dataObjSqlResult->dataType = *dataType;
    }

    if ((phyPath = getSqlResultByInx(myGenQueryOut, COL_D_DATA_PATH)) == NULL) {
        setSqlResultValue(&dataObjSqlResult->phyPath, COL_D_DATA_PATH, "", myGenQueryOut->rowCnt);
    }
    else {
        dataObjSqlResult->phyPath = *phyPath;
    }

    if ((ownerName = getSqlResultByInx(myGenQueryOut, COL_D_OWNER_NAME)) == NULL) {
        setSqlResultValue(&dataObjSqlResult->ownerName, COL_D_OWNER_NAME, "", myGenQueryOut->rowCnt);
    }
    else {
        dataObjSqlResult->ownerName = *ownerName;
    }

    if ((replNum = getSqlResultByInx(myGenQueryOut, COL_DATA_REPL_NUM)) == NULL) {
        setSqlResultValue(&dataObjSqlResult->replNum, COL_DATA_REPL_NUM, "", myGenQueryOut->rowCnt);
    }
    else {
...
int rclOpenCollection(rcComm_t* conn, char* collection, int flags, collHandle_t* collHandle)
{
    ...
    if (conn == NULL || collection == NULL || collHandle == NULL) {
        rodsLog(LOG_ERROR, "rclOpenCollection: NULL conn, collection or collHandle input");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (rodsObjStatOut->specColl != NULL && rodsObjStatOut->specColl->collClass != STRUCT_FILE_COLL &&
...
int readCollection(collHandle_t* collHandle, collEnt_t* collEnt)
{
    ...
    if (queryHandle == NULL || collHandle == NULL || collEnt == NULL) {
        rodsLog(LOG_ERROR, "rclReadCollection: NULL queryHandle or collHandle input");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (collHandle->dataObjInp.specColl != NULL) {
        if (collHandle->dataObjInp.specColl->collClass == LINKED_COLL) {
            ...
            status = queryCollInColl(queryHandle,
                                     collHandle->linkedObjPath,
                                     collHandle->flags & (~RECUR_QUERY_FG),
                                     &collHandle->genQueryInp,
                                     &genQueryOut);
        }
        else {
            if (strlen(collHandle->linkedObjPath) > 0) {
                rstrcpy(collHandle->dataObjInp.objPath, collHandle->linkedObjPath, MAX_NAME_LEN);
            }
            addKeyVal(&collHandle->dataObjInp.condInput, SEL_OBJ_TYPE_KW, "collection");
            collHandle->dataObjInp.openFlags = 0; /* start over */
            status =
                (*queryHandle->querySpecColl)((rcComm_t*) queryHandle->conn, &collHandle->dataObjInp, &genQueryOut);
        }
    }
    else {
        memset(&collHandle->genQueryInp, 0, sizeof(genQueryInp_t));
        status = queryCollInColl(
            queryHandle, collHandle->dataObjInp.objPath, collHandle->flags, &collHandle->genQueryInp, &genQueryOut);
    }

    collHandle->rowInx = 0;
    collHandle->state = COLL_COLL_OBJ_QUERIED;
    if (status >= 0) {
        status = genQueryOutToCollRes(&genQueryOut, &collHandle->collSqlResult);
    }
...
int genDataResInColl(queryHandle_t* queryHandle, collHandle_t* collHandle)
{
    ...
    if (collHandle->dataObjInp.specColl != NULL) {
        if (collHandle->dataObjInp.specColl->collClass == LINKED_COLL) {
            ...
            addKeyVal(&collHandle->dataObjInp.condInput, SEL_OBJ_TYPE_KW, "dataObj");
            collHandle->dataObjInp.openFlags = 0; /* start over */
            status =
                (*queryHandle->querySpecColl)((rcComm_t*) queryHandle->conn, &collHandle->dataObjInp, &genQueryOut);
        }
    ...
    if (status >= 0) {
        status = genQueryOutToDataObjRes(&genQueryOut, &collHandle->dataObjSqlResult);
    }
    else if (status != CAT_NO_ROWS_FOUND) {
        rodsLog(LOG_ERROR,
...
int getNextCollMetaInfo(collHandle_t* collHandle, collEnt_t* outCollEnt)
{
    ...
    if (collHandle->rowInx >= collSqlResult->rowCnt) {
        ...
        if (continueInx > 0) {
            ...
            if (dataObjInp->specColl != NULL) {
                ...
                status = (*queryHandle->querySpecColl)((rcComm_t*) queryHandle->conn, dataObjInp, &genQueryOut);
            }
            else {
    ...
    if (*collType != '\0') {
        ...
        if (strcmp(collType, INHERIT_PAR_SPEC_COLL_STR) == 0) {
            if (dataObjInp->specColl == NULL) {
                rodsLog(LOG_ERROR, "getNextCollMetaInfo: parent specColl is NULL for %s", outCollEnt->collName);
            ...
            status = resolveSpecCollType(collType, outCollEnt->collName, collInfo1, collInfo2, &outCollEnt->specColl);
        }
    }
    else {
        outCollEnt->specColl.collClass = NO_SPEC_COLL;
...
{
    if (!_query_handle) {
        rodsLog(LOG_ERROR, "null query_handle ptr");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }

    if (!_resc_hier) {
        rodsLog(LOG_ERROR, "null resc_hier ptr");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }

    if (_resc_id <= 0) {
        rodsLog(LOG_ERROR, "resc_id is invalid");
    ...
    int status = _query_handle->getHierForId(_query_handle->conn, &inp, &out);
    if (status < 0) {
        return status;
    }
...
int getNextDataObjMetaInfo(collHandle_t* collHandle, collEnt_t* outCollEnt)
{
    ...
    if (strlen(dataId) > 0 && (collHandle->flags & NO_TRIM_REPL_FG) == 0) {
        ...
        for (i = collHandle->rowInx; i < dataObjSqlResult->rowCnt; i++) {
            if (selectedInx < 0) {
                ...
                if (gotCopy == 0 && atoi(&replStatus[replStatusLen * i]) > 0) {
                    /* pick a good copy */
                    selectedInx = i;
                    gotCopy = 1;
                }
    ...
    outCollEnt->replStatus = atoi(&replStatus[replStatusLen * selectedInx]);
    ...
    if (rodsObjStat->specColl != NULL && rodsObjStat->specColl->collClass != LINKED_COLL) {
...
char* getZoneHintForGenQuery(genQueryInp_t* genQueryInp)
{
    ...
    for (i = 0; i < genQueryInp->sqlCondInp.len; i++) {
        ...
        if (inx == COL_COLL_NAME || inx == COL_COLL_PARENT_NAME || inx == COL_ZONE_NAME) {
            char* tmpPtr;
...
int getZoneType(rcComm_t* conn, char* icatZone, char* inZoneName, char* outZoneType)
{
    ...
    if ((zoneType = getSqlResultByInx(genQueryOut, COL_ZONE_TYPE)) == NULL) {
        rodsLog(LOG_NOTICE, "getZoneInfo: getSqlResultByInx for COL_ZONE_TYPE failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
...
int getCollSizeForProgStat(rcComm_t* conn, char* srcColl, operProgress_t* operProgress)
{
    ...
    status = rclOpenCollection(conn, srcColl, RECUR_QUERY_FG, &collHandle);

    if (status < 0) {
        rodsLog(LOG_ERROR, "getCollSizeForProgStat: rclOpenCollection of %s error. status = %d", srcColl, status);
        return status;
    }
    ...
    while ((status = rclReadCollection(conn, &collHandle, &collEnt)) >= 0) {
        if (collEnt.objType == DATA_OBJ_T) {
            ...
            if (collEnt.specColl.collClass != NO_SPEC_COLL) {
                ...
                status = getCollSizeForProgStat(conn, collEnt.collName, operProgress);
                if (status < 0 && status != CAT_NO_ROWS_FOUND) {
                    return status;
                }
            }
...
int getDirSizeForProgStat(rodsArguments_t* rodsArgs, char* srcDir, operProgress_t* operProgress)
{
    ...
    if (!exists(srcDirPath) || !is_directory(srcDirPath)) {
        rodsLog(LOG_ERROR, "getDirSizeForProgStat: opendir local dir error for %s, errno = %d\n", srcDir, errno);
        return USER_INPUT_PATH_ERR;
    }
    ...
    for (directory_iterator itr(srcDirPath); itr != end_itr; ++itr) {
        ...
        if (!exists(p)) {
            rodsLog(LOG_ERROR, "getDirSizeForProgStat: stat error for %s, errno = %d\n", srcChildPath, errno);
            return USER_INPUT_PATH_ERR;
        }
            ...
            status = getDirSizeForProgStat(rodsArgs, srcChildPath, operProgress);
            if (status < 0) {
                return status;
            }
        }
...
guiProgressCallback iCommandProgStat(operProgress_t* operProgress)
{
    ...
    else if ((status = splitPathByKey(operProgress->curFileName, myDir, MAX_NAME_LEN, myFile, MAX_NAME_LEN, '/')) < 0) {
        rodsLogError(LOG_NOTICE,
                     status,
    ...
    if (operProgress->flag == 0) {
        printf("%-lld/%-lld - %5.2f%% of files done   ",
               operProgress->totalNumFilesDone,
               operProgress->totalNumFiles,
               VERIFY_DIV(operProgress->totalNumFilesDone, operProgress->totalNumFiles) * 100.0);
        printf("%-.3f/%-.3f MB - %5.2f%% of file sizes done\n",
               ...
               VERIFY_DIV(operProgress->totalFileSizeDone, operProgress->totalFileSize) * 100.0);
        printf("Processing %s - %-.3f MB   %s\n", myFile, (float) operProgress->curFileSize / 1048576.0, timeStr);
    }
        ...
        printf("%s - %-.3f/%-.3f MB - %5.2f%% done   %s\n",
               ...
               VERIFY_DIV(operProgress->curFileSizeDone, operProgress->curFileSize) * 100.0,
               timeStr);
...
int getOpenedCollLen(collHandle_t* collHandle)
{
    ...
    if (collHandle->rodsObjStat->specColl != NULL && (len = strlen(collHandle->linkedObjPath)) > 0) {
    }
    else {
...
int rmSubDir(char* mydir)
{
    ...
    if (!exists(srcDirPath) || !is_directory(srcDirPath)) {
        ...
        rodsLogError(LOG_ERROR, status, "rmSubDir: opendir local dir error for %s", mydir);
        return status;
    }
    directory_iterator end_itr; // default construction yields past-the-end
    for (directory_iterator itr(srcDirPath); itr != end_itr; ++itr) {
        path p = itr->path();
        snprintf(childPath, MAX_NAME_LEN, "%s", p.c_str());
        if (!exists(p)) {
            savedStatus = USER_INPUT_PATH_ERR - errno;
            rodsLogError(LOG_ERROR, savedStatus, "rmSubDir: stat error for %s", childPath);
            continue;
        }
        if (is_directory(p)) {
            ...
            if (status < 0) {
                savedStatus = USER_INPUT_PATH_ERR - errno;
                rodsLogError(LOG_ERROR, status, "rmSubDir: rmSubDir error for %s ", childPath);
            }
            if (rmdir(childPath) != 0) {
                savedStatus = USER_INPUT_PATH_ERR - errno;
                rodsLogError(LOG_ERROR, status, "rmSubDir: rmdir error for %s ", childPath);
            }
            continue;
        }
        else {
            savedStatus = USER_INPUT_PATH_ERR - errno;
            rodsLogError(LOG_ERROR, status, "rmSubDir: %s is not a dir", childPath);
            continue;
        }
    }
    return savedStatus;
...
int rmFilesInDir(char* mydir)
{
    ...
    if (!exists(srcDirPath) || !is_directory(srcDirPath)) {
        status = USER_INPUT_PATH_ERR - errno;
        rodsLogError(LOG_ERROR, status, "rmFilesInDir: opendir local dir error for %s", mydir);
        return status;
    }
    directory_iterator end_itr; // default construction yields past-the-end
    for (directory_iterator itr(srcDirPath); itr != end_itr; ++itr) {
        ...
        if (!exists(p)) {
            savedStatus = USER_INPUT_PATH_ERR - errno;
            rodsLogError(LOG_ERROR, savedStatus, "rmFilesInDir: stat error for %s", childPath);
            continue;
        }
...
int getNumFilesInDir(const char* mydir)
{
    ...
    if (!exists(srcDirPath) || !is_directory(srcDirPath)) {
        status = USER_INPUT_PATH_ERR - errno;
        rodsLogError(LOG_ERROR, status, "getNumFilesInDir: opendir local dir error for %s", mydir);
        return status;
    }
    directory_iterator end_itr; // default construction yields past-the-end
    for (directory_iterator itr(srcDirPath); itr != end_itr; ++itr) {
        ...
        if (!exists(p)) {
            savedStatus = USER_INPUT_PATH_ERR - errno;
            rodsLogError(LOG_ERROR, savedStatus, "getNumFilesInDir: stat error for %s", childPath);
            continue;
        }
        if (is_regular_file(p)) {
            count++;
    ...
    for (index = 0; index < pp->num_patterns; index++) {
        ...
        if (*pattern == '/') {
            ...
            if (fnmatch(pattern, pathname, FNM_PATHNAME) == 0) {
                rodsLog(LOG_DEBUG, "matchPathname: match name %s with pattern %s", pathname, pattern);
                /* we have a match */
                return 1;
            }
        }
            ...
            if (fnmatch(pattern, name, 0) == 0) {
                rodsLog(LOG_DEBUG, "matchPathname: match name %s with pattern %s", name, pattern);
                /* we have a match */
                return 1;
            }
        }
    ...
    for (srcInx = 0; srcInx < rodsPathInp->numSrc; srcInx++) {
        ...
        if (destPath->objType >= UNKNOWN_FILE_T && strcmp(destPath->outPath, STDOUT_FILE_NAME) == 0) {
            ...
            *targPath = *destPath;
            ...
            else if (destPath->objType == DATA_OBJ_T || destPath->objType == LOCAL_FILE_T || rodsPathInp->numSrc == 1) {
                *targPath = *destPath;
                if (destPath->objType <= COLL_OBJ_T) {
                    targPath->objType = DATA_OBJ_T;
                }
