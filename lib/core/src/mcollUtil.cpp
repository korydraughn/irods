...
int mcollUtil(rcComm_t* conn, rodsEnv* myRodsEnv, rodsArguments_t* myRodsArgs, rodsPathInp_t* rodsPathInp)
{
    ...
    for (int i = 0; i < rodsPathInp->numSrc; i++) {
        ...
        else if (myRodsArgs->syncCacheDir == True || myRodsArgs->purgeCacheDir == True) { /* sync or purge */
            srcPath = &rodsPathInp->srcPath[i];                                           /* file Path */
        ...
        if (status < 0 && status != CAT_NO_ROWS_FOUND) {
            rodsLogError(LOG_ERROR, status, "imcoll error");
            savedStatus = status;
        }
    }
    return savedStatus;
...
int initCondForMcoll(rodsEnv* myRodsEnv, rodsArguments_t* rodsArgs, dataObjInp_t* dataObjOprInp)
{
    if (dataObjOprInp == NULL) {
        rodsLog(LOG_ERROR, "initCondForReg: NULL dataObjOprInp input");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (rodsArgs->purgeCacheDir == True) {
        dataObjOprInp->oprType = dataObjOprInp->oprType | PURGE_STRUCT_FILE_CACHE;
    }

    if (rodsArgs->unmount == True) {
        addKeyVal(&dataObjOprInp->condInput, COLLECTION_TYPE_KW, UNMOUNT_STR);
    }
    else if (rodsArgs->mountCollection == True) {
        char* mountType = rodsArgs->mountType;
        if (strcmp(mountType, "h") == 0 || strcmp(mountType, "haaw") == 0) {
            addKeyVal(&dataObjOprInp->condInput, COLLECTION_TYPE_KW, HAAW_STRUCT_FILE_STR);
        }
        else if (strcmp(mountType, "m") == 0 || strcmp(mountType, "msso") == 0) {
            addKeyVal(&dataObjOprInp->condInput, COLLECTION_TYPE_KW, MSSO_STRUCT_FILE_STR);
            /* XXXXX need to add "msso structFile" token */
            addKeyVal(&dataObjOprInp->condInput, DATA_TYPE_KW, MSSO_DT_STR);
        }
        else if (strcmp(mountType, "t") == 0 || strcmp(mountType, "tar") == 0) {
            addKeyVal(&dataObjOprInp->condInput, COLLECTION_TYPE_KW, TAR_STRUCT_FILE_STR);
            /* XXXXX need to add "tar structFile" token */
            addKeyVal(&dataObjOprInp->condInput, DATA_TYPE_KW, TAR_DT_STR);
        }
        else if (strcmp(mountType, "f") == 0 || strcmp(mountType, "filesystem") == 0) {
            /* the collection is a filesystem mount point */
            addKeyVal(&dataObjOprInp->condInput, COLLECTION_TYPE_KW, MOUNT_POINT_STR);
        }
        else if (strcmp(mountType, "l") == 0 || strcmp(mountType, "link") == 0) {
            /* the collection is a softlink point */
            addKeyVal(&dataObjOprInp->condInput, COLLECTION_TYPE_KW, LINK_POINT_STR);
        }
    }
    ...
    if (rodsArgs->resource == True) {
        if (rodsArgs->resourceString == NULL) {
            rodsLog(LOG_ERROR, "initCondForReg: NULL resourceString error");
            return USER__NULL_INPUT_ERR;
        }
        else {
        ...
        addKeyVal(&dataObjOprInp->condInput, DEST_RESC_NAME_KW, myRodsEnv->rodsDefResource);
    }
