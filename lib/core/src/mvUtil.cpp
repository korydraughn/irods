...
int mvUtil(rcComm_t* conn, rodsArguments_t* myRodsArgs, rodsPathInp_t* rodsPathInp)
{
    ...
    if (savedStatus < 0) {
        rodsLogError(LOG_ERROR, savedStatus, "mvUtil: resolveRodsTarget error, status = %d", savedStatus);
        return savedStatus;
    }
    ...
    for (int i = 0; i < rodsPathInp->numSrc; i++) {
        ...
        int status = mvObjUtil(
            conn, rodsPathInp->srcPath[i].outPath, targPath->outPath, targPath->objType, myRodsArgs, &dataObjRenameInp);

        /* XXXX may need to return a global status */
        if (status < 0 && status != CAT_NO_ROWS_FOUND) {
            rodsLogError(LOG_ERROR, status, "mvUtil: mv error for %s, status = %d", targPath->outPath, status);
            savedStatus = status;
        }
    }
    return savedStatus;
              ...
              dataObjCopyInp_t* dataObjRenameInp)
{
    ...
    if (srcPath == NULL || targPath == NULL) {
        rodsLog(LOG_ERROR, "mvFileUtil: NULL srcPath or targPath input");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (objType == DATA_OBJ_T) {
        ...
        /* should not be here */
        rodsLog(LOG_ERROR, "mvObjUtil: invalid cp dest objType %d for %s", objType, targPath);
    ...
    if (status >= 0 && rodsArgs->verbose == True) {
        ...
        printTiming(conn, dataObjRenameInp->destDataObjInp.objPath, 0, NULL, &startTime, &endTime);
    }
...
int initCondForMv(dataObjCopyInp_t* dataObjRenameInp)
{
    if (dataObjRenameInp == NULL) {
        rodsLog(LOG_ERROR, "initCondForMv: NULL dataObjRenameInp input");
        return USER__NULL_INPUT_ERR;
    }
