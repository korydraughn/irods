...
int replUtil(rcComm_t* conn, rodsEnv* myRodsEnv, rodsArguments_t* myRodsArgs, rodsPathInp_t* rodsPathInp)
{
    ...
    for (int i = 0; i < rodsPathInp->numSrc; i++) {
        if (rodsPathInp->srcPath[i].objType == UNKNOWN_OBJ_T) {
            ...
            if (rodsPathInp->srcPath[i].objState == NOT_EXIST_ST) {
                rodsLog(LOG_ERROR, "replUtil: srcPath %s does not exist", rodsPathInp->srcPath[i].outPath);
    ...
    for (int i = 0; i < rodsPathInp->numSrc; i++) {
        ...
        if (rodsPathInp->srcPath[i].objType == DATA_OBJ_T) {
            ...
            status = replDataObjUtil(
                conn, rodsPathInp->srcPath[i].outPath, rodsPathInp->srcPath[i].size, myRodsArgs, &dataObjInp);
        }
        else if (rodsPathInp->srcPath[i].objType == COLL_OBJ_T) {
            setStateForRestart(&rodsRestart, &rodsPathInp->srcPath[i], myRodsArgs);
            addKeyVal(&dataObjInp.condInput, TRANSLATED_PATH_KW, "");
            status =
                replCollUtil(conn, rodsPathInp->srcPath[i].outPath, myRodsEnv, myRodsArgs, &dataObjInp, &rodsRestart);
        ...
        if (status < 0 && status != CAT_NO_ROWS_FOUND) {
            rodsLogError(
                LOG_ERROR, status, "replUtil: repl error for %s, status = %d", rodsPathInp->srcPath[i].outPath, status);
            savedStatus = status;
        }
    }
    return savedStatus;
                    ...
                    dataObjInp_t* dataObjInp)
{
    ...
    if (srcPath == NULL) {
        rodsLog(LOG_ERROR, "replDataObjUtil: NULL srcPath input");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (status >= 0) {
        if (rodsArgs->verbose == True) {
            ...
            printTiming(conn, dataObjInp->objPath, conn->transStat.bytesWritten, NULL, &startTime, &endTime);
        }
        if (gGuiProgressCB != NULL) {
...
int initCondForRepl(rodsEnv* myRodsEnv, rodsArguments_t* rodsArgs, dataObjInp_t* dataObjInp, rodsRestart_t* rodsRestart)
{
    ...
    if (dataObjInp == NULL) {
        rodsLog(LOG_ERROR, "initCondForRepl: NULL dataObjInp input");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (rodsArgs->replNum == True) {
        addKeyVal(&dataObjInp->condInput, REPL_NUM_KW, rodsArgs->replNumValue);
    }
    ...
    if (rodsArgs->srcResc == True) {
        addKeyVal(&dataObjInp->condInput, RESC_NAME_KW, rodsArgs->srcRescString);
    }
    ...
    if (rodsArgs->resource == True) {
        ...
        addKeyVal(&dataObjInp->condInput, DEF_RESC_NAME_KW, myRodsEnv->rodsDefResource);
    }

    if (myResc != NULL && rodsArgs->backupMode == True) {
        addKeyVal(&dataObjInp->condInput, BACKUP_RESC_NAME_KW, myResc);
    }
    ...
    if (rodsArgs->restart == True) {
        ...
        if (status < 0) {
            rodsLogError(
                LOG_ERROR, status, "initCondForRepl: openRestartFile of %s errno", rodsArgs->restartFileString);
            return status;
        }
    }
                 ...
                 rodsRestart_t* rodsRestart)
{
    ...
    if (srcColl == NULL) {
        rodsLog(LOG_ERROR, "replCollUtil: NULL srcColl input");
        return USER__NULL_INPUT_ERR;
    }

    if (rodsArgs->recursive != True) {
        rodsLog(LOG_ERROR, "replCollUtil: -r option must be used for getting %s collection", srcColl);
    ...
    status = rclOpenCollection(conn, srcColl, INCLUDE_CONDINPUT_IN_QUERY, &collHandle);
    if (status < 0) {
        rodsLog(LOG_ERROR, "replCollUtil: rclOpenCollection of %s error. status = %d", srcColl, status);
        return status;
    }
    if (collHandle.rodsObjStat->specColl != NULL && collHandle.rodsObjStat->specColl->collClass != LINKED_COLL) {
    ...
    while ((status = rclReadCollection(conn, &collHandle, &collEnt)) >= 0) {
        if (collEnt.objType == DATA_OBJ_T) {
            ...
            int status =
                chkStateForResume(conn, rodsRestart, srcChildPath, rodsArgs, DATA_OBJ_T, &dataObjInp->condInput, 0);
            ...
            status = replDataObjUtil(conn, srcChildPath, collEnt.dataSize, rodsArgs, dataObjInp);
            ...
            int status = replCollUtil(conn, collEnt.collName, myRodsEnv, rodsArgs, &childDataObjInp, rodsRestart);
            if (status < 0 && status != CAT_NO_ROWS_FOUND) {
                savedStatus = status;
            }
        }
