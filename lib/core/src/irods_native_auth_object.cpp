...
namespace irods
{
    // =-=-=-=-=-=-=-
    // public - ctor
    native_auth_object::native_auth_object(rError_t* _r_error)
    ...
    // =-=-=-=-=-=-=-
    // public - cctor
    native_auth_object::native_auth_object(const native_auth_object& _rhs)
        : auth_object(_rhs)
    {
    ...
    native_auth_object& native_auth_object::operator=(const native_auth_object& _rhs)
    {
    ...
    bool native_auth_object::operator==(const native_auth_object&) const
    {
        return false;
    }
    ...
    error native_auth_object::resolve(const std::string& _interface, plugin_ptr& _ptr)
    {
        // =-=-=-=-=-=-=-
        ...
        auth_ptr a_ptr;
        error ret = auth_mgr.resolve(AUTH_NATIVE_SCHEME, a_ptr);
        if (!ret.ok()) {
            // =-=-=-=-=-=-=-
            ...
            ret = auth_mgr.init_from_type(
                ProcessType, AUTH_NATIVE_SCHEME, AUTH_NATIVE_SCHEME, AUTH_NATIVE_SCHEME, empty_context, a_ptr);
            if (!ret.ok()) {
                return PASS(ret);
            }
            else {
    ...
    error native_auth_object::get_re_vars(rule_engine_vars_t& _kvp)
    {
        // =-=-=-=-=-=-=-
