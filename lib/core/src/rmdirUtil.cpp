...
int rmdirCollUtil(rcComm_t* conn, rodsArguments_t* myRodsArgs, int treatAsPathname, rodsPath_t collPath)
{
    ...
    if (!checkCollExists(conn, myRodsArgs, collPath.outPath)) {
        std::cout << "Failed to remove [" << collPath.outPath << "]: Collection does not exist" << std::endl;
        return 0;
    }
    ...
    if (!checkCollIsEmpty(conn, myRodsArgs, collPath.outPath)) {
        std::cout << "Failed to remove [" << collPath.outPath << "]: Collection is not empty" << std::endl;
        return 0;
    }
    else {
        ...
        if (status < 0) {
            std::cout << "rmdirColl: rcRmColl failed with error " << status << std::endl;
            return status;
        }
