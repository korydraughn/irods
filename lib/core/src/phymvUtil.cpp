    ...
    for (int i = 0; i < rodsPathInp->numSrc; i++) {
        ...
        if (rodsPathInp->srcPath[i].objType == UNKNOWN_OBJ_T) {
            ...
            if (rodsPathInp->srcPath[i].objState == NOT_EXIST_ST) {
                rodsLog(LOG_ERROR, "phymvUtil: srcPath %s does not exist", rodsPathInp->srcPath[i].outPath);
                savedStatus = USER_INPUT_PATH_ERR;
                continue;
            }
        }
        ...
        if (rodsPathInp->srcPath[i].objType == DATA_OBJ_T) {
            ...
            status = phymvDataObjUtil(conn, rodsPathInp->srcPath[i].outPath, myRodsArgs, &dataObjInp);
        }
        else if (rodsPathInp->srcPath[i].objType == COLL_OBJ_T) {
            addKeyVal(&dataObjInp.condInput, TRANSLATED_PATH_KW, "");
            status = phymvCollUtil(conn, rodsPathInp->srcPath[i].outPath, myRodsEnv, myRodsArgs, &dataObjInp);
        }
        else {
            /* should not be here */
            rodsLog(LOG_ERROR,
...
int phymvDataObjUtil(rcComm_t* conn, char* srcPath, rodsArguments_t* rodsArgs, dataObjInp_t* dataObjInp)
{
    ...
    if (srcPath == NULL) {
        rodsLog(LOG_ERROR, "phymvDataObjUtil: NULL srcPath input");
        return USER__NULL_INPUT_ERR;
    }
...
{
    if (dataObjInp == NULL) {
        rodsLog(LOG_ERROR, "initCondForPhymv: NULL dataObjInp input");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (rodsArgs->resource == True) {
        if (rodsArgs->resourceString == NULL) {
            rodsLog(LOG_ERROR, "initCondForPhymv: NULL resourceString error");
            return USER__NULL_INPUT_ERR;
        }
        else {
                  ...
                  dataObjInp_t* dataObjInp)
{
    ...
    if (srcColl == NULL) {
        rodsLog(LOG_ERROR, "phymvCollUtil: NULL srcColl input");
        return USER__NULL_INPUT_ERR;
    }

    if (rodsArgs->recursive != True) {
        rodsLog(LOG_ERROR, "phymvCollUtil: -r option must be used for getting %s collection", srcColl);
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "phymvCollUtil: rclOpenCollection of %s error. status = %d", srcColl, status);
        return status;
    }
    ...
    while ((status = rclReadCollection(conn, &collHandle, &collEnt)) >= 0) {
        if (collEnt.objType == DATA_OBJ_T) {
            ...
            int status = phymvDataObjUtil(conn, srcChildPath, rodsArgs, dataObjInp);
            if (status < 0) {
                rodsLogError(LOG_ERROR,
                             status,
            ...
            int status = phymvCollUtil(conn, collEnt.collName, myRodsEnv, rodsArgs, &childDataObjInp);
            if (status < 0 && status != CAT_NO_ROWS_FOUND) {
                savedStatus = status;
            }
        }
