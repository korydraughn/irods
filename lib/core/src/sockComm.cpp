...
irods::error readMsgHeader(irods::network_object_ptr _ptr, msgHeader_t* _header, struct timeval* _time_val)
{
    ...
    irods::network_ptr net = boost::dynamic_pointer_cast<irods::network>(p_ptr);
    ret_err = net->call<void*, struct timeval*>(nullptr, irods::NETWORK_OP_READ_HEADER, ptr, tmp_buf, _time_val);
    ...
    int status = unpackStruct(static_cast<void*>(tmp_buf),
                         ...
                         struct timeval* _time_val)
{
    ...
    irods::first_class_object_ptr ptr = boost::dynamic_pointer_cast<irods::first_class_object>(_ptr);
    irods::network_ptr net = boost::dynamic_pointer_cast<irods::network>(p_ptr);
    ret_err = net->call<msgHeader_t*, bytesBuf_t*, bytesBuf_t*, bytesBuf_t*, irodsProt_t, struct timeval*>(
        nullptr,
...
int sockOpenForInConn(rsComm_t* rsComm, int* portNum, char** addr, int proto)
{
    ...
    if (sock < 0) {
        ...
        rodsLogError(LOG_NOTICE, status, "sockOpenForInConn: open socket error.");
        return status;
    }
    ...
    try {
        svr_port_range_start_wrapper.reset(
    ...
    if (*portNum <= 0 && svr_port_range_start_wrapper) {
        ...
        try {
            ...
            if (svr_port_range_end > 65535) {
                rodsLog(
                    LOG_ERROR, "sockOpenForInConn: PortRangeEnd %d > 65535", svr_port_range_start, svr_port_range_end);
        ...
        while (bindCnt < portRangeCount) {
            ...
            if ((status = bind(sock, (struct sockaddr*) &mySockAddr, sizeof mySockAddr)) >= 0) {
                *portNum = myPortNum;
        ...
        if (bindCnt >= portRangeCount) {
            ...
            rodsLog(LOG_ERROR, "port range exhausted, errno = %d", errno);
        }
    }
    else {
        mySockAddr.sin_port = htons(*portNum);
        status = bind(sock, (struct sockaddr*) &mySockAddr, sizeof mySockAddr);
    }

    if (status < 0) {
        status = SYS_SOCK_BIND_ERR - errno;
        rodsLog(LOG_NOTICE, "sockOpenForInConn: bind socket error. portNum = %d, errno = %d", *portNum, errno);
        close(sock);
        return status;
    }
...
int rsAcceptConn(rsComm_t* svrComm)
{
    ...
    if (newSock < 0) {
        ...
        rodsLogError(LOG_NOTICE, status, "rsAcceptConn: accept error for socket %d", svrComm->sock);
        return newSock;
    }
...
// =-=-=-=-=-=-=-
//
irods::error writeMsgHeader(irods::network_object_ptr _ptr, const msgHeader_t* _header)
{
    // =-=-=-=-=-=-=-
    // always use XML_PROT for the Header
    bytesBuf_t* header_buf = 0;
    int status = packStruct(static_cast<const void*>(_header), &header_buf, "MsgHeader_PI", RodsPackTable, 0, XML_PROT);
    ...
    irods::first_class_object_ptr ptr = boost::dynamic_pointer_cast<irods::first_class_object>(_ptr);
    irods::network_ptr net = boost::dynamic_pointer_cast<irods::network>(p_ptr);
    ret = net->call<const bytesBuf_t*>(nullptr, irods::NETWORK_OP_WRITE_HEADER, ptr, header_buf);
...
irods::error readVersion(irods::network_object_ptr _ptr, version_t** _version)
{
    ...
    ret = readMsgBody(_ptr, &myHeader, &inputStructBBuf, &bsBBuf, &errorBBuf, XML_PROT, NULL);
    if (!ret.ok()) {
        return PASS(ret);
    }
    ...
    if (strcmp(myHeader.type, RODS_VERSION_T) != 0) {
        ...
        msg << "wrong msg type [" << myHeader.type << " expected [" << RODS_VERSION_T << "]";
    ...
    if (myHeader.bsLen != 0) {
        free(bsBBuf.buf);
        rodsLog(LOG_NOTICE, "readVersion: myHeader.bsLen = %d is not 0", myHeader.bsLen);
    }
    ...
    if (myHeader.errorLen != 0) {
        free(errorBBuf.buf);
        rodsLog(LOG_NOTICE, "readVersion: myHeader.errorLen = %d is not 0", myHeader.errorLen);
    }
    ...
    if (myHeader.msgLen > (int) sizeof(version_t) * 2 || myHeader.msgLen <= 0) {
        free(inputStructBBuf.buf);
        std::stringstream msg;
        msg << "header length is not within bounds: " << myHeader.msgLen;
        return ERROR(SYS_HEADER_READ_LEN_ERR, msg.str());
    }

    // =-=-=-=-=-=-=-
    // unpack the message, always use XML for this message type
    int status = unpackStruct(inputStructBBuf.buf, (void**) (_version), "Version_PI", RodsPackTable, XML_PROT);
    free(inputStructBBuf.buf);
    if (status < 0) {
        rodsLogError(LOG_NOTICE, status, "readVersion:unpackStruct error.");
    }
...
int set_tcp_buffer_size(const int sock, const int tcp_buffer_size, const int socket_option_number)
{
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR,
                "set_tcp_buffer_size: failed to set socket option %d to %d, errno %d",
                socket_option_number,
                tcp_buffer_size,
                errno);
        return status;
    }
    ...
    status = getsockopt(
        ...
        SOL_SOCKET,
        socket_option_number,
        &getsockopt_return_value,
        &getsockopt_return_value_len); // getsockopt SO_{SND,RCV}BUF return is doubled, as it includes OS overhead
    if (status < 0) {
    ...
    if (getsockopt_return_value / 2 != tcp_buffer_size) {
        rodsLog(LOG_ERROR,
                "set_tcp_buffer_size: option %d was set to %d instead of requested %d",
                socket_option_number,
                getsockopt_return_value / 2,
                tcp_buffer_size);
...
#    error socket settings not implemented for windows
    ...
    if (tcp_buffer_size < 0) {
        rodsLog(
            LOG_ERROR, "rodsSetSockOpt: requested tcp buffer size is negative, changing to zero: %d", tcp_buffer_size);
        tcp_buffer_size = 0;
    }
...
int connectToRhostPortal(char* rodsHost, int rodsPort, int cookie, int windowSize)
{
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "connectToRhostPortal: setSockAddr error for %s, errno = %d", rodsHost, errno);
        return status;
    }
    ...
    if (sock < 0) {
        rodsLog(
            LOG_ERROR, "connectToRhostPortal: connectTo Rhost %s port %d error, status = %d", rodsHost, rodsPort, sock);
        return sock;
    }
...
{
    int status;
    conn->sock = connectToRhostWithRaddr(&conn->remoteAddr, conn->windowSize, 1);
    if (conn->sock < 0) {
        rodsLogError(
            LOG_NOTICE, conn->sock, "connectToRhost: connect to host %s on port %d failed", conn->host, conn->portNum);
        return conn->sock;
    }
    ...
    if (status < 0) {
        rodsLogError(LOG_ERROR, status, "connectToRhost: sendStartupPack to %s failed", conn->host);
        close(conn->sock);
        return status;
    }
    ...
    if (irods::do_client_server_negotiation_for_client()) {
        ...
        std::string results;
        ret = irods::client_server_negotiation_for_client(net_obj, conn->host, results);
        if (!ret.ok()) {
            return ret.code();
        }
    ...
    if (conn->svrVersion->status < 0) {
        rodsLogError(LOG_ERROR, conn->svrVersion->status, "connectToRhost: error returned from host %s", conn->host);
        if (conn->svrVersion->status == SYS_EXCEED_CONNECT_CNT)
            rodsLog(LOG_ERROR,
                    "It is likely %s is a localhost but not recognized by this server. A line can be added to the "
                    "server/config/irodsHost file to fix the problem",
                    conn->host);
        close(conn->sock);
...
int try_twice_to_create_socket(void)
{
    ...
    if (sock < 0) {
        rodsLog(LOG_ERROR, "try_twice_to_create_socket() - socket() failed: errno=%d", errno);
...
int connectToRhostWithRaddr(struct sockaddr_in* remoteAddr, int windowSize, int timeoutFlag)
{
    ...
    if (timeoutFlag > 0) {
        ...
        const int status = connect(sock, (struct sockaddr*) remoteAddr, sizeof(struct sockaddr));
        if (status < 0) {
#ifdef _WIN32
            closesocket(sock);
#else
...
int create_nonblocking_socket(void)
{
    ...
    if (flags < 0) {
        close(sock);
        rodsLog(LOG_ERROR, "create_nonblocking_socket: fcntl F_GETFL error, errno = %d", errno);
        return USER_SOCK_CONNECT_ERR - errno;
    }
    flags |= O_NONBLOCK;
    if (fcntl(sock, F_SETFL, flags) < 0) {
        close(sock);
        rodsLog(LOG_ERROR, "create_nonblocking_socket: fcntl F_SETFL error, errno = %d", errno);
        return USER_SOCK_CONNECT_ERR - errno;
    }
    return sock;
}
...
int connectToRhostWithTout(struct sockaddr* sin)
{
    ...
    while (timeoutCnt < MAX_CONN_RETRY_CNT) {
        ...
        if (errno == EINPROGRESS || errno == EINTR) {
            ...
            if (status < 0) {
                if (errno != EINTR) {
                    rodsLog(LOG_NOTICE, "connectToRhostWithTout: connect error, errno = %d", errno);
                    timeoutCnt++;
                }
                close(sock);
                continue;
...
#    endif
                if (getsockopt(sock, SOL_SOCKET, SO_ERROR, (void*) (&myval), &mylen) < 0) {
                    rodsLog(LOG_ERROR, "connectToRhostWithTout: getsockopt error, errno = %d", errno);
                    close(sock);
                    return USER_SOCK_CONNECT_ERR - errno;
                }
                /* Check the returned value */
                if (myval) {
                    rodsLog(LOG_DEBUG, "connectToRhostWithTout: myval error, errno = %d", myval);
                    timeoutCnt++;
    ...
    if (socket_flags < 0) {
        close(sock);
        rodsLog(LOG_ERROR, "connectToRhostWithTout: fcntl F_GETFL error, errno = %d", errno);
        return USER_SOCK_CONNECT_ERR - errno;
    }
    ...
    if (fcntl(sock, F_SETFL, socket_flags) < 0) {
        close(sock);
        rodsLog(LOG_ERROR, "connectToRhostWithTout: fcntl F_SETFL error, errno = %d", errno);
        return USER_SOCK_CONNECT_ERR - errno;
    }
    return sock;
}
...
int setRemoteAddr(int sock, struct sockaddr_in* remoteAddr)
{
     ...
     * of hosts */
    if (getpeername(sock, (struct sockaddr*) remoteAddr, &laddrlen) < 0) {
        rodsLog(LOG_NOTICE, "setLocalAddr() -- getpeername() failed: errno=%d, errstring=%s", errno, strerror(errno));
...
int setLocalAddr(int sock, struct sockaddr_in* localAddr)
{
    ...
    if (getsockname(sock, (struct sockaddr*) localAddr, &laddrlen) < 0) {
        rodsLog(LOG_NOTICE, "setLocalAddr() -- getsockname() failed: errno=%d, errstring=%s", errno, strerror(errno));
        return USER_RODS_HOSTNAME_ERR;
    }
...
int sendStartupPack(rcComm_t* conn, int connectCnt, int reconnFlag)
{
    ...
    if (0 == strcmp(REQ_SVR_NEG, rods_env.rodsClientServerNegotiation)) {
        ...
        if (status >= 0 && cs_neg_len > 0) {
            ...
            strncat(startupPack.option, rods_env.rodsClientServerNegotiation, opt_sz - opt_len);
        }
    }

    /* always use XML_PROT for the startupPack */
    status = packStruct((void*) &startupPack, &startupPackBBuf, "StartupPack_PI", RodsPackTable, 0, XML_PROT);
    if (status < 0) {
        rodsLogError(LOG_NOTICE, status, "sendStartupPack: packStruct error");
        return status;
    }
    ...
    ret = sendRodsMsg(net_obj, RODS_CONNECT_T, startupPackBBuf, NULL, NULL, 0, XML_PROT);
    freeBBuf(startupPackBBuf);
    if (!ret.ok()) {
        irods::log(PASS(ret));
        return ret.code();
                         ...
                         int cookie)
{
    ...
    status = packStruct((char*) &myVersion, &versionBBuf, "Version_PI", RodsPackTable, 0, XML_PROT);
    if (status < 0) {
        return ERROR(status, "packStruct error");
    }

    irods::error ret = sendRodsMsg(_ptr, RODS_VERSION_T, versionBBuf, NULL, NULL, 0, XML_PROT);
                         ...
                         irodsProt_t _protocol)
{
    ...
    ret_err = net->call<const char*, const bytesBuf_t*, const bytesBuf_t*, const bytesBuf_t*, int, irodsProt_t>(
        nullptr, irods::NETWORK_OP_WRITE_BODY, ptr, _msg_type, _msg_buf, _bs_buf, _error_buf, _int_info, _protocol);
...
irods::error readReconMsg(irods::network_object_ptr _ptr, reconnMsg_t** _msg)
{
    ...
    if (strcmp(myHeader.type, RODS_RECONNECT_T) != 0) {
        ...
        free(errorBBuf.buf);
        std::stringstream msg;
        msg << "wrong msg type [" << myHeader.type << "] expected [" << RODS_CONNECT_T << "]";
        return ERROR(SYS_HEADER_TYPE_LEN_ERR, msg.str());
    }
    ...
    if (myHeader.bsLen != 0) {
        ...
        rodsLog(LOG_NOTICE, "readReconMsg: myHeader.bsLen = %d is not 0", myHeader.bsLen);
    }
    ...
    if (myHeader.errorLen != 0) {
        ...
        rodsLog(LOG_NOTICE, "readReconMsg: myHeader.errorLen = %d is not 0", myHeader.errorLen);
    }
    ...
    if (myHeader.msgLen <= 0) {
        ...
        rodsLog(LOG_NOTICE, "readReconMsg: problem with myHeader.msgLen = %d", myHeader.msgLen);
        std::stringstream msg;
        msg << "message length is invalid: " << myHeader.msgLen;
        return ERROR(SYS_HEADER_READ_LEN_ERR, msg.str());
    }

    /* always use XML_PROT for the startup pack */
    status = unpackStruct(inputStructBBuf.buf, (void**) (_msg), "ReconnMsg_PI", RodsPackTable, XML_PROT);
    clearBBuf(&inputStructBBuf);
    if (status < 0) {
        rodsLogError(LOG_NOTICE, status, "readReconMsg:unpackStruct error.");
    }
...
irods::error sendReconnMsg(irods::network_object_ptr _ptr, reconnMsg_t* _msg)
{
    ...
    int status = packStruct(static_cast<void*>(_msg), &recon_buf, "ReconnMsg_PI", RodsPackTable, 0, XML_PROT);
    if (status < 0) {
    ...
    // =-=-=-=-=-=-=-
    // pack outgoing message - alway use XML for version
    irods::error ret = sendRodsMsg(_ptr, RODS_RECONNECT_T, recon_buf, NULL, NULL, 0, XML_PROT);
    freeBBuf(recon_buf);
    if (!ret.ok()) {
        rodsLogError(LOG_ERROR, status, "sendReconnMsg: sendRodsMsg of reconnect msg failed");
    }
...
int svrSwitchConnect(rsComm_t* rsComm)
{
    ...
    if (rsComm->reconnectedSock > 0) {
        ...
        rodsLog(LOG_NOTICE, "svrSwitchConnect: Switch connection");
        return 1;
    }
    else {
        return 0;
...
int cliSwitchConnect(rcComm_t* conn)
{
    ...
    if (conn->reconnectedSock > 0) {
        ...
        fprintf(stderr, "The client/server socket connection has been renewed\n");
        return 1;
    }
    else {
        return 0;
...
int isReadMsgError(int status)
{
    ...
    if (irodsErr == SYS_READ_MSG_BODY_LEN_ERR || irodsErr == SYS_HEADER_READ_LEN_ERR ||
...
int redirectConnToRescSvr(rcComm_t** conn, dataObjInp_t* dataObjInp, rodsEnv* myEnv, int reconnFlag)
{
    ...
    if (dataObjInp->oprType == PUT_OPR) {
        ...
        rodsLog(LOG_NOTICE, "redirectConnToRescSvr: Unknown oprType %d\n", dataObjInp->oprType);
        return 0;
    }
...
int rcReconnect(rcComm_t** conn, char* newHost, rodsEnv* myEnv, int reconnFlag)
{
    ...
    newConn = rcConnect(newHost, myEnv->rodsPort, myEnv->rodsUserName, myEnv->rodsZone, reconnFlag, &errMsg);
