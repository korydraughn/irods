...
int lsUtil(rcComm_t* conn, rodsEnv* myRodsEnv, rodsArguments_t* myRodsArgs, rodsPathInp_t* rodsPathInp)
{
    ...
    for (int i = 0; i < rodsPathInp->numSrc; i++) {
        ...
        if (rodsPathInp->srcPath[i].objType == UNKNOWN_OBJ_T || rodsPathInp->srcPath[i].objState == UNKNOWN_ST) {
        ...
        if (status < 0 && status != CAT_NO_ROWS_FOUND && status != SYS_SPEC_COLL_OBJ_NOT_EXIST) {
            rodsLogError(
                LOG_ERROR, status, "lsUtil: ls error for %s, status = %d", rodsPathInp->srcPath[i].outPath, status);
            savedStatus = status;
        }
    }
    return savedStatus;
...
int lsDataObjUtil(rcComm_t* conn, rodsPath_t* srcPath, rodsArguments_t* rodsArgs, genQueryInp_t* genQueryInp)
{
    ...
    if (rodsArgs->longOption == True) {
        if (srcPath->rodsObjStat != NULL && srcPath->rodsObjStat->specColl != NULL) {
            if (srcPath->rodsObjStat->specColl->collClass == LINKED_COLL) {
                lsDataObjUtilLong(conn, srcPath->rodsObjStat->specColl->objPath, rodsArgs, genQueryInp);
            }
            else {
...
int lsDataObjUtilLong(rcComm_t* conn, char* srcPath, rodsArguments_t* rodsArgs, genQueryInp_t* genQueryInp)
{
    ...
    if ((status = splitPathByKey(srcPath, myColl, MAX_NAME_LEN, myData, MAX_NAME_LEN, '/')) < 0) {
        rodsLogError(
            LOG_ERROR, status, "setQueryInpForLong: splitPathByKey for %s error, status = %d", srcPath, status);
        return status;
    }
    ...
    if (status < 0) {
        if (status == CAT_NO_ROWS_FOUND) {
            ...
            rodsLogError(LOG_ERROR, status, "lsDataObjUtilLong: rcGenQuery error for %s", srcPath);
        }
        return status;
    }
...
{
    int i = 0;
    sqlResult_t *dataName = 0, *replNum = 0, *dataSize = 0, *rescName = 0, *replStatus = 0, *dataModify = 0,
                *dataOwnerName = 0, *dataId = 0;
    ...
    if (rodsArgs->veryLongOption == True) {
        if ((chksumStr = getSqlResultByInx(genQueryOut, COL_D_DATA_CHECKSUM)) == NULL) {
            rodsLog(LOG_ERROR, "printLsLong: getSqlResultByInx for COL_D_DATA_CHECKSUM failed");
            return UNMATCHED_KEY_OR_INDEX;
        }

        if ((dataPath = getSqlResultByInx(genQueryOut, COL_D_DATA_PATH)) == NULL) {
            rodsLog(LOG_ERROR, "printLsLong: getSqlResultByInx for COL_D_DATA_PATH failed");
            return UNMATCHED_KEY_OR_INDEX;
        }

        if ((dataType = getSqlResultByInx(genQueryOut, COL_DATA_TYPE_NAME)) == NULL) {
            rodsLog(LOG_ERROR, "printLsLong: getSqlResultByInx for COL_DATA_TYPE_NAME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
    }

    if ((dataId = getSqlResultByInx(genQueryOut, COL_D_DATA_ID)) == NULL) {
        rodsLog(LOG_ERROR, "printLsLong: getSqlResultByInx for COL_D_DATA_ID failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataName = getSqlResultByInx(genQueryOut, COL_DATA_NAME)) == NULL) {
        rodsLog(LOG_ERROR, "printLsLong: getSqlResultByInx for COL_DATA_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((replNum = getSqlResultByInx(genQueryOut, COL_DATA_REPL_NUM)) == NULL) {
        rodsLog(LOG_ERROR, "printLsLong: getSqlResultByInx for COL_DATA_REPL_NUM failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataSize = getSqlResultByInx(genQueryOut, COL_DATA_SIZE)) == NULL) {
        rodsLog(LOG_ERROR, "printLsLong: getSqlResultByInx for COL_DATA_SIZE failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((rescName = getSqlResultByInx(genQueryOut, COL_D_RESC_NAME)) == NULL) {
        rodsLog(LOG_ERROR, "printLsLong: getSqlResultByInx for COL_D_RESC_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
    ...
    if ((replStatus = getSqlResultByInx(genQueryOut, COL_D_REPL_STATUS)) == NULL) {
        rodsLog(LOG_ERROR, "printLsLong: getSqlResultByInx for COL_D_REPL_STATUS failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataModify = getSqlResultByInx(genQueryOut, COL_D_MODIFY_TIME)) == NULL) {
        rodsLog(LOG_ERROR, "printLsLong: getSqlResultByInx for COL_D_MODIFY_TIME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataOwnerName = getSqlResultByInx(genQueryOut, COL_D_OWNER_NAME)) == NULL) {
        rodsLog(LOG_ERROR, "printLsLong: getSqlResultByInx for COL_D_OWNER_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
...
int lsSpecDataObjUtilLong(rodsPath_t* srcPath, rodsArguments_t* rodsArgs)
{
    ...
    status = printSpecLsLong(
        srcPath->outPath, rodsObjStat->ownerName, sizeStr, rodsObjStat->modifyTime, rodsObjStat->specColl, rodsArgs);
...
int printLsShort(rcComm_t* conn, rodsArguments_t* rodsArgs, genQueryOut_t* genQueryOut)
{
    ...
    if ((dataId = getSqlResultByInx(genQueryOut, COL_D_DATA_ID)) == NULL) {
        rodsLog(LOG_ERROR, "printLsShort: getSqlResultByInx for COL_D_DATA_ID failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataName = getSqlResultByInx(genQueryOut, COL_DATA_NAME)) == NULL) {
        rodsLog(LOG_ERROR, "printLsShort: getSqlResultByInx for COL_DATA_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
...
{
    if (genQueryInp == NULL) {
        rodsLog(LOG_ERROR, "initCondForLs: NULL genQueryInp input");
        return USER__NULL_INPUT_ERR;
    }
...
int lsCollUtil(rcComm_t* conn, rodsPath_t* srcPath, rodsEnv* myRodsEnv, rodsArguments_t* rodsArgs)
{
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "lsCollUtil: rclOpenCollection of %s error. status = %d", srcColl, status);
        return status;
    }
    while ((status = rclReadCollection(conn, &collHandle, &collEnt)) >= 0) {
        if (collEnt.objType == DATA_OBJ_T) {
            if (rodsArgs->bundle == True) {
                ...
                snprintf(myPath, MAX_NAME_LEN, "%s/%s", collEnt.collName, collEnt.dataName);
            ...
            /* need to drill down */
            if (rodsArgs->recursive == True && strcmp(collEnt.collName, "/") != 0) {
                rodsPath_t tmpPath;
...
int printDataCollEntLong(collEnt_t* collEnt, int flags)
{
    ...
    if (collEnt->specColl.collClass == NO_SPEC_COLL || collEnt->specColl.collClass == LINKED_COLL) {
...
int printCollCollEnt(collEnt_t* collEnt, int flags)
{
    ...
    if ((flags & LONG_METADATA_FG) != 0) {
        ...
        if (collEnt->specColl.collClass == NO_SPEC_COLL) {
            ...
            if (collEnt->specColl.collClass == MOUNTED_COLL || collEnt->specColl.collClass == LINKED_COLL) {
                printf("  C- %s  %6.6s  %s  %s   %s\n",
                       collEnt->collName,
                       typeStr,
                       collEnt->specColl.objPath,
                       collEnt->specColl.phyPath,
...
int printDataAcl(rcComm_t* conn, char* dataId)
{
    ...
    if ((userName = getSqlResultByInx(genQueryOut, COL_USER_NAME)) == NULL) {
        rodsLog(LOG_ERROR, "printDataAcl: getSqlResultByInx for COL_USER_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((userZone = getSqlResultByInx(genQueryOut, COL_USER_ZONE)) == NULL) {
        rodsLog(LOG_ERROR, "printDataAcl: getSqlResultByInx for COL_USER_ZONE failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataAccess = getSqlResultByInx(genQueryOut, COL_DATA_ACCESS_NAME)) == NULL) {
        rodsLog(LOG_ERROR, "printDataAcl: getSqlResultByInx for COL_DATA_ACCESS_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
...
int printCollAcl(rcComm_t* conn, char* collName)
{
    ...
    if ((userName = getSqlResultByInx(genQueryOut, COL_COLL_USER_NAME)) == NULL) {
        rodsLog(LOG_ERROR, "printCollAcl: getSqlResultByInx for COL_COLL_USER_NAME failed");
        freeGenQueryOut(&genQueryOut);
        return UNMATCHED_KEY_OR_INDEX;
    }
    if ((userZone = getSqlResultByInx(genQueryOut, COL_COLL_USER_ZONE)) == NULL) {
        rodsLog(LOG_ERROR, "printCollAcl: getSqlResultByInx for COL_COLL_USER_ZONE failed");
        freeGenQueryOut(&genQueryOut);
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataAccess = getSqlResultByInx(genQueryOut, COL_COLL_ACCESS_NAME)) == NULL) {
        rodsLog(LOG_ERROR, "printCollAcl: getSqlResultByInx for COL_COLL_ACCESS_NAME failed");
        freeGenQueryOut(&genQueryOut);
        return UNMATCHED_KEY_OR_INDEX;
    }
...
int printCollInheritance(rcComm_t* conn, char* collName)
{
    ...
    if ((inheritResult = getSqlResultByInx(genQueryOut, COL_COLL_INHERITANCE)) == NULL) {
        rodsLog(LOG_ERROR, "printCollInheritance: getSqlResultByInx for COL_COLL_INHERITANCE failed");
        freeGenQueryOut(&genQueryOut);
        return UNMATCHED_KEY_OR_INDEX;
    }
...
int lsSubfilesInBundle(rcComm_t* conn, char* srcPath)
{
    ...
    while (continueInx > 0) {
        ...
        if (status < 0) {
            if (status != CAT_NO_ROWS_FOUND) {
                rodsLogError(LOG_ERROR, status, "lsSubfilesInBundle: rsGenQuery error for %s", srcPath);
            }
            clearGenQueryInp(&genQueryInp);
            return status;
        }

        if ((collection = getSqlResultByInx(genQueryOut, COL_COLL_NAME)) == NULL) {
            rodsLog(LOG_ERROR, "lsSubfilesInBundle: getSqlResultByInx for COL_COLL_NAME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        if ((dataName = getSqlResultByInx(genQueryOut, COL_DATA_NAME)) == NULL) {
            rodsLog(LOG_ERROR, "lsSubfilesInBundle: getSqlResultByInx for COL_DATA_NAME failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        if ((dataSize = getSqlResultByInx(genQueryOut, COL_DATA_SIZE)) == NULL) {
            rodsLog(LOG_ERROR, "lsSubfilesInBundle: getSqlResultByInx for COL_DATA_SIZE failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
