    ...
    if (rodsRestart->restartState & MATCHED_RESTART_COLL) {
        ...
        if (rodsRestart->restartState & LAST_PATH_MATCHED) {
            if (objType == DATA_OBJ_T || objType == LOCAL_FILE_T) {
                ...
                setStateForResume(conn, rodsRestart, targPath, objType, condInput, deleteFlag);
            }
            status = 1;
        }
                      ...
                      int deleteFlag)
{
    ...
    if (restartPath != NULL && deleteFlag > 0) {
        if (objType == DATA_OBJ_T) {
            if ((condInput == NULL || getValByKey(condInput, FORCE_FLAG_KW) == NULL) &&
            ...
            rodsLog(LOG_ERROR, "setStateForResume: illegal objType %d for %s", objType, restartPath);
        }
    }
...
int putUtil(rcComm_t** myConn, rodsEnv* myRodsEnv, rodsArguments_t* myRodsArgs, rodsPathInp_t* rodsPathInp)
{
    ...
    status = initCondForPut(conn, myRodsEnv, myRodsArgs, &dataObjOprInp, &bulkOprInp, &rodsRestart);
    if (status < 0) {
        return status;
    }
    ...
    if (rodsPathInp->resolved == False) {
        ...
        if (status < 0) {
            rodsLogError(LOG_ERROR, status, "putUtil: resolveRodsTarget error, status = %d", status);
            return status;
        }
        ...
        for (i = 0; i < rodsPathInp->numSrc; i++) {
            ...
            if (targPath->objType == DATA_OBJ_T) {
                ...
                if (rodsPathInp->srcPath[i].size > 0) {
                    conn->operProgress.totalFileSize += rodsPathInp->srcPath[i].size;
                }
            }
            else {
    ...
    if (conn->fileRestart.flags == FILE_RESTART_ON) {
        ...
        if (status >= 0) {
            ...
            if (status >= 0) {
                ...
                rstrcpy(conn->fileRestart.info.objPath, info->objPath, MAX_NAME_LEN);
                conn->fileRestart.info.status = FILE_RESTARTED;
    ...
    for (i = 0; i < rodsPathInp->numSrc; i++) {
        ...
        if (targPath->objType == DATA_OBJ_T) {
            ...
            /* should not be here */
            rodsLog(LOG_ERROR, "putUtil: invalid put dest objType %d for %s", targPath->objType, targPath->outPath);
            return USER_INPUT_PATH_ERR;
        }
        /* XXXX may need to return a global status */
        if (status < 0) {
            rodsLogError(LOG_ERROR, status, "putUtil: put error for %s, status = %d", targPath->outPath, status);
            savedStatus = status;
            break;
        }
    }
    ...
    if (status < 0 && myRodsArgs->retries == True) {
        ...
        while (myRodsArgs->retriesValue > 0) {
            ...
            status = rcReconnect(myConn, myRodsEnv->rodsHost, myRodsEnv, reconnFlag);
            if (status < 0) {
                rodsLogError(LOG_ERROR, status, "putUtil: rcReconnect error for %s", targPath->outPath);
                return status;
            }
            ...
            if (status >= 0) {
                ...
                rodsLogError(LOG_ERROR, status, "putUtil: retry putUtil error");
            }
                ...
                dataObjInp_t* dataObjOprInp)
{
    ...
    if (srcPath == NULL || targPath == NULL) {
        rodsLog(LOG_ERROR, "putFileUtil: NULL srcPath or targPath input");
        return USER__NULL_INPUT_ERR;
    }

    if (conn->fileRestart.info.status == FILE_RESTARTED && strcmp(conn->fileRestart.info.objPath, targPath) == 0) {
        /* it was restarted */
    ...
    if (rodsArgs->checksum == True) {
        ...
        if (ret < 0) {
            rodsLog(LOG_ERROR, "putFileUtil - failed to capture rods env %d", ret);
            return ret;
        }

        status = rcChksumLocFile(srcPath, VERIFY_CHKSUM_KW, &dataObjOprInp->condInput, env.rodsDefaultHashScheme);
        if (status < 0) {
            rodsLogError(LOG_ERROR, status, "putFileUtil: rcChksumLocFile error for %s, status = %d", srcPath, status);
            return status;
        }
    }
    ...
    if (status >= 0) {
        if (rodsArgs->verbose == True) {
            ...
            printTiming(conn, dataObjOprInp->objPath, srcSize, srcPath, &startTime, &endTime);
        }
        if (gGuiProgressCB != NULL) {
                   ...
                   rodsRestart_t* rodsRestart)
{
    ...
    if (dataObjOprInp == NULL) {
        rodsLog(LOG_ERROR, "initCondForPut: NULL dataObjOprInp input");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (rodsArgs->kv_pass) {
        addKeyVal(&dataObjOprInp->condInput, KEY_VALUE_PASSTHROUGH_KW, rodsArgs->kv_pass_string);
    }

    if (rodsArgs->bulk == True) {
        if (bulkOprInp == NULL) {
            rodsLog(LOG_ERROR, "initCondForPut: NULL bulkOprInp input");
            return USER__NULL_INPUT_ERR;
        }
    ...
    if (rodsArgs->dataType == True) {
        if (rodsArgs->dataTypeString == NULL) {
            ...
            if (strcmp(rodsArgs->dataTypeString, "t") == 0 || strcmp(rodsArgs->dataTypeString, "tar") == 0) {
                addKeyVal(&dataObjOprInp->condInput, DATA_TYPE_KW, "tar file");
            }
            else {
                addKeyVal(&dataObjOprInp->condInput, DATA_TYPE_KW, rodsArgs->dataTypeString);
            }
        }
    }
    else {
    ...
    if (rodsArgs->physicalPath == True) {
        if (rodsArgs->physicalPathString == NULL) {
            rodsLog(LOG_ERROR, "initCondForPut: NULL physicalPathString error");
            return USER__NULL_INPUT_ERR;
        }
        else {
            fs::path slash("/");
            ...
            addKeyVal(&dataObjOprInp->condInput, FILE_PATH_KW, rodsArgs->physicalPathString);
        }
    }

    if (rodsArgs->resource == True) {
        if (rodsArgs->resourceString == NULL) {
            rodsLog(LOG_ERROR, "initCondForPut: NULL resourceString error");
            return USER__NULL_INPUT_ERR;
        }
        else {
            addKeyVal(&dataObjOprInp->condInput, DEST_RESC_NAME_KW, rodsArgs->resourceString);
            if (rodsArgs->bulk == True) {
                addKeyVal(&bulkOprInp->condInput, DEST_RESC_NAME_KW, rodsArgs->resourceString);
            }
        }
    }
        ...
        addKeyVal(&dataObjOprInp->condInput, DEF_RESC_NAME_KW, myRodsEnv->rodsDefResource);
        if (rodsArgs->bulk == True) {
            addKeyVal(&bulkOprInp->condInput, DEF_RESC_NAME_KW, myRodsEnv->rodsDefResource);
        }
    }

    if (rodsArgs->replNum == True) {
        addKeyVal(&dataObjOprInp->condInput, REPL_NUM_KW, rodsArgs->replNumValue);
    }
    ...
    if (rodsArgs->restart == True) {
        ...
        if (status < 0) {
            rodsLogError(LOG_ERROR, status, "initCondForPut: openRestartFile of %s error", rodsArgs->restartFileString);
            return status;
        }
        if (rodsRestart->doneCnt == 0) {
            ...
            if (rodsArgs->bulk == True) {
                if (strcmp(rodsRestart->oprType, BULK_OPR_KW) != 0) {
                    rodsLog(LOG_ERROR, "initCondForPut: -b cannot be used to restart this iput");
                    return BULK_OPR_MISMATCH_FOR_RESTART;
                }
            }
            else {
                if (strcmp(rodsRestart->oprType, NON_BULK_OPR_KW) != 0) {
                    rodsLog(LOG_ERROR, "initCondForPut: -b must be used to restart this iput");
                    return BULK_OPR_MISMATCH_FOR_RESTART;
                }
            }
        }
    }
    if (rodsArgs->retries == True && rodsArgs->restart == False && rodsArgs->lfrestart == False) {
        rodsLog(LOG_ERROR, "initCondForPut: --retries must be used with -X or --lfrestart option");
    ...
    if (rodsArgs->lfrestart == True) {
        if (rodsArgs->bulk == True) {
            rodsLog(LOG_NOTICE, "initCondForPut: --lfrestart cannot be used with -b option");
        }
        else if (rodsArgs->rbudp == True) {
            rodsLog(LOG_NOTICE, "initCondForPut: --lfrestart cannot be used with -Q option");
        }
        else {
               ...
               bulkOprInfo_t* bulkOprInfo)
{
    ...
    if (rodsArgs->recursive != True) {
        rodsLog(LOG_ERROR, "putDirUtil: -r option must be used for putting %s directory", srcDir);
        return USER_INPUT_OPTION_ERR;
    }
    ...
    if (!exists(srcDirPath) || !is_directory(srcDirPath)) {
        rodsLog(LOG_ERROR, "putDirUtil: opendir local dir error for %s, errno = %d\n", srcDir, errno);
        return USER_INPUT_PATH_ERR;
    }
    ...
    if (ec && it == end) {
        rodsLog(LOG_ERROR,
                "Error ecountered when processing directory %s for put: %s",
                srcDirPath.c_str(),
                ec.message().c_str());
        ...
        for (; it != end; it.increment(ec)) {
            ...
            status = chkStateForResume(
                conn, rodsRestart, targChildPath, rodsArgs, childObjType, &dataObjOprInp->condInput, 1);
            ...
            if (status == 0) {
                if (bulkFlag == BULK_OPR_SMALL_FILES && (rodsRestart->restartState & LAST_PATH_MATCHED) != 0) {
            ...
            if (childObjType == DATA_OBJ_T) { /* a file */
                if (bulkFlag == BULK_OPR_SMALL_FILES) {
                    status = bulkPutFileUtil(conn,
                                             srcChildPath,
                                             targChildPath,
                                             dataSize,
                                             dataObjOprInp->createMode,
                                             rodsArgs,
                                             bulkOprInp,
                                             bulkOprInfo);
                }
                    ...
                    /* normal put */
                    try {
                        status = putFileUtil(conn, srcChildPath, targChildPath, dataSize, rodsArgs, dataObjOprInp);
                    }
                   ...
                   rodsRestart_t* rodsRestart)
{
    ...
    status =
        putDirUtil(myConn, srcDir, targColl, myRodsEnv, rodsArgs, dataObjOprInp, bulkOprInp, rodsRestart, &bulkOprInfo);

    if (status < 0) {
        rodsLogError(LOG_ERROR, status, "bulkPutDirUtil: Large files bulkPut error for %s", srcDir);
        return status;
    }
    ...
    if (status < 0) {
        rodsLogError(LOG_ERROR, status, "bulkPutDirUtil: Small files bulkPut error for %s", srcDir);
        return status;
    }
    ...
    if (bulkOprInfo.count > 0) {
        ...
        if (status >= 0) {
            ...
            rodsLogError(LOG_ERROR, status, "bulkPutDirUtil: tarAndBulkPut error for %s", bulkOprInfo.phyBunDir);
        }
...
int getPhyBunDir(char* phyBunRootDir, char* userName, char* outPhyBunDir)
{
    ...
    while (1) {
        snprintf(
            outPhyBunDir, MAX_NAME_LEN, "%s/%s.phybun.%u", phyBunRootDir, userName, irods::getRandom<unsigned int>());
                    ...
                    bulkOprInfo_t* bulkOprInfo)
{
    ...
    if (in_fd < 0) { /* error */
        ...
        rodsLog(LOG_ERROR, "bulkPutFileUtilcannot open file %s, status = %d", srcPath, status);
        return status;
    }
    ...
    if (status != srcSize) {
        if (status >= 0) {
            ...
            rodsLogError(LOG_ERROR,
                         status,
                         "bulkPutFileUtil: bytesRead %ju does not match srcSize %ju for %s",
                         (uintmax_t) bytesRead,
                         (uintmax_t) srcSize,
                         srcPath);
        }
    ...
    if (getValByKey(&bulkOprInp->condInput, REG_CHKSUM_KW) != NULL ||
        ...
        int ret = getRodsEnv(&env);
        if (ret < 0) {
            rodsLog(LOG_ERROR, "bulkPutFileUtil: error getting rods env %d", ret);
            return ret;
        }
        status = chksumLocFile(srcPath, chksumStr, env.rodsDefaultHashScheme);
        if (status < 0) {
            rodsLog(LOG_ERROR, "bulkPutFileUtil: chksumLocFile error for %s ", srcPath);
            return status;
        }
        status = fillAttriArrayOfBulkOprInp(targPath, createMode, chksumStr, bulkOprInfo->size, bulkOprInp);
    }
    else {
        status = fillAttriArrayOfBulkOprInp(targPath, createMode, NULL, bulkOprInfo->size, bulkOprInp);
    }
    if (status < 0) {
        rodsLogError(LOG_ERROR, status, "bulkPutFileUtil: fillAttriArrayOfBulkOprInp error for %s", srcPath);
        return status;
    }
    if (bulkOprInfo->count >= MAX_NUM_BULK_OPR_FILES ||
        ...
        if (status >= 0) {
            ...
            rodsLogError(LOG_ERROR, status, "bulkPutFileUtil: tarAndBulkPut error for %s", srcPath);
        }
        clearBulkOprInfo(bulkOprInfo);
    }
    return status;
...
int sendBulkPut(rcComm_t* conn, bulkOprInp_t* bulkOprInp, bulkOprInfo_t* bulkOprInfo, rodsArguments_t* rodsArgs)
{
    ...
    if (status >= 0) {
        ...
        if (gGuiProgressCB != NULL) {
            rstrcpy(conn->operProgress.curFileName, bulkOprInfo->cachedTargPath, MAX_NAME_LEN);
