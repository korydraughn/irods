...
{
    snprintf(prevChallengeSignatureClient,
             200,
...
int clientLoginTTL(rcComm_t* Conn, int ttl)
{
    ...
    if (int status = rcGetLimitedPassword(Conn, &getLimitedPasswordInp, &getLimitedPasswordOut)) {
    ...
    obfMakeOneWayHash(HASH_TYPE_DEFAULT, (unsigned char*) hashBuf, 100, digest);
    memset(hashBuf, 0, sizeof(hashBuf));
...
/* To enable easier debugging in apache webservers, each error condition is returning a unique arbitrary negative
 * number. These return values override any error code passed back from functions where the error originates, but this
...
int clientLoginOpenID(rcComm_t* _comm, const char* _context, int reprompt)
{
    ...
    if (reprompt) {
        ...
        if (!ret.ok()) {
            printError(_comm, ret.code(), (char*) ret.result().c_str());
            return ret.code();
        }
    }
    else {
        ...
        if (out_map.find("port") == out_map.end() || out_map.find("nonce") == out_map.end()) {
            rodsLog(LOG_ERROR, "map missing values: %s", req_out->result_);
            return -8;
        }
        int portno = std::stoi(out_map["port"]);
        std::string nonce = out_map["nonce"];
        std::cout << "received comm info from server: port: [" + out_map["port"] + "], nonce: [" + out_map["nonce"] +
                         "]"
                  << std::endl;
        ...
        if (authorization_url_buf.compare("SUCCESS") == 0) {
            ...
            std::cout << "session information was invalid, received: [" << authorization_url_buf << "]" << std::endl;
            return -16;
        }
    }
    // =-=-=-=-=-=-=-
