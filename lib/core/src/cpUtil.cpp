...
int cpUtil(rcComm_t* conn, rodsEnv* myRodsEnv, rodsArguments_t* myRodsArgs, rodsPathInp_t* rodsPathInp)
{
    ...
    if (savedStatus < 0) {
        rodsLogError(LOG_ERROR, savedStatus, "cpUtil: resolveRodsTarget error, status = %d", savedStatus);
        return savedStatus;
    }
    ...
    if (gGuiProgressCB != NULL) {
        ...
        for (int i = 0; i < rodsPathInp->numSrc; i++) {
            ...
            if (targPath->objType == DATA_OBJ_T) {
                ...
                getCollSizeForProgStat(conn, rodsPathInp->srcPath[i].outPath, &conn->operProgress);
            }
        }
    }
    ...
    for (int i = 0; i < rodsPathInp->numSrc; i++) {
        ...
        if (rodsPathInp->srcPath[i].rodsObjStat != NULL && rodsPathInp->srcPath[i].rodsObjStat->specColl != NULL) {
            dataObjCopyInp.srcDataObjInp.specColl = rodsPathInp->srcPath[i].rodsObjStat->specColl;
        }

        int status = 0;
        if (targPath->objType == DATA_OBJ_T) {
            rmKeyVal(&dataObjCopyInp.srcDataObjInp.condInput, TRANSLATED_PATH_KW);
            status = cpFileUtil(conn,
            ...
            /* should not be here */
            rodsLog(LOG_ERROR, "cpUtil: invalid cp dest objType %d for %s", targPath->objType, targPath->outPath);
            return USER_INPUT_PATH_ERR;
        }
        /* XXXX may need to return a global status */
        if (status < 0 && status != CAT_NO_ROWS_FOUND && status != SYS_SPEC_COLL_OBJ_NOT_EXIST) {
            rodsLogError(LOG_ERROR, status, "cpUtil: cp error for %s, status = %d", targPath->outPath, status);
            savedStatus = status;
        }
    }
               ...
               dataObjCopyInp_t* dataObjCopyInp)
{
    ...
    if (srcPath == NULL || targPath == NULL) {
        rodsLog(LOG_ERROR, "cpFileUtil: NULL srcPath or targPath in cp");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (status >= 0) {
        if (rodsArgs->verbose == True) {
            ...
            printTiming(
                conn, dataObjCopyInp->destDataObjInp.objPath, conn->transStat.bytesWritten, NULL, &startTime, &endTime);
        }
        if (gGuiProgressCB != NULL) {
                  ...
                  rodsRestart_t* rodsRestart)
{
    ...
    if (dataObjCopyInp == NULL) {
        rodsLog(LOG_ERROR, "initCondForCp: NULL dataObjCopyInp in cp");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (rodsArgs->dataType == True) {
        if (rodsArgs->dataTypeString != NULL) {
            addKeyVal(&dataObjCopyInp->destDataObjInp.condInput, DATA_TYPE_KW, rodsArgs->dataTypeString);
        }
    }
    ...
    if (rodsArgs->physicalPath == True) {
        if (rodsArgs->physicalPathString == NULL) {
            rodsLog(LOG_ERROR, "initCondForCp: NULL physicalPathString error");
            return USER__NULL_INPUT_ERR;
        }
        else {
            addKeyVal(&dataObjCopyInp->destDataObjInp.condInput, FILE_PATH_KW, rodsArgs->physicalPathString);
        }
    }

    if (rodsArgs->resource == True) {
        if (rodsArgs->resourceString == NULL) {
            rodsLog(LOG_ERROR, "initCondForCp: NULL resourceString error");
            return USER__NULL_INPUT_ERR;
        }
        else {
    ...
    if (rodsArgs->rbudp == True) {
        ...
        addKeyVal(&dataObjCopyInp->destDataObjInp.condInput, RBUDP_TRANSFER_KW, "");
        addKeyVal(&dataObjCopyInp->srcDataObjInp.condInput, RBUDP_TRANSFER_KW, "");
    }
    ...
    if ((tmpStr = getenv(RBUDP_SEND_RATE_KW)) != NULL) {
        addKeyVal(&dataObjCopyInp->destDataObjInp.condInput, RBUDP_SEND_RATE_KW, tmpStr);
        addKeyVal(&dataObjCopyInp->srcDataObjInp.condInput, RBUDP_SEND_RATE_KW, tmpStr);
    }

    if ((tmpStr = getenv(RBUDP_PACK_SIZE_KW)) != NULL) {
        addKeyVal(&dataObjCopyInp->destDataObjInp.condInput, RBUDP_PACK_SIZE_KW, tmpStr);
        addKeyVal(&dataObjCopyInp->srcDataObjInp.condInput, RBUDP_PACK_SIZE_KW, tmpStr);
    }
    ...
    if (rodsArgs->restart == True) {
        ...
        if (status < 0) {
            rodsLogError(LOG_ERROR, status, "initCondForCp: openRestartFile of %s errno", rodsArgs->restartFileString);
            return status;
        }
    }

    // dataObjCopyInp->destDataObjInp.createMode = 0600;   // seems unused, and caused
    // https://github.com/irods/irods/issues/2085
               ...
               rodsRestart_t* rodsRestart)
{
    ...
    if (srcColl == NULL || targColl == NULL) {
        rodsLog(LOG_ERROR, "cpCollUtil: NULL srcColl or targColl in cp");
        return USER__NULL_INPUT_ERR;
    }

    if (rodsArgs->recursive != True) {
        rodsLog(LOG_ERROR, "cpCollUtil: -r option must be used for copying %s directory", srcColl);
    ...
    if (separator.size() > 1) {
        rodsLog(LOG_ERROR,
                "cpCollUtil: irods::get_virtual_path_separator() returned a string with more than one character.");
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "cpCollUtil: rclOpenCollection of %s error. status = %d", srcColl, status);
        return status;
    }
    while ((status = rclReadCollection(conn, &collHandle, &collEnt)) >= 0) {
        if (collEnt.objType == DATA_OBJ_T) {
            snprintf(srcChildPath, MAX_NAME_LEN, "%s/%s", collEnt.collName, collEnt.dataName);
            snprintf(targChildPath, MAX_NAME_LEN, "%s/%s", targColl, collEnt.dataName);

            int status = chkStateForResume(
                conn, rodsRestart, targChildPath, rodsArgs, DATA_OBJ_T, &dataObjCopyInp->destDataObjInp.condInput, 1);
            ...
            status = cpFileUtil(conn, srcChildPath, targChildPath, collEnt.dataSize, rodsArgs, dataObjCopyInp);

            if (status < 0) {
                rodsLogError(
                    LOG_ERROR, status, "getCollUtil: getDataObjUtil failed for %s. status = %d", srcChildPath, status);
                savedStatus = status;
                if (rodsRestart->fd > 0) {
                    break;
                }
            ...
            int status = cpCollUtil(
                conn, collEnt.collName, targChildPath, myRodsEnv, rodsArgs, &childDataObjCopyInp, rodsRestart);
