...
int rsyncUtil(rcComm_t* conn, rodsEnv* myRodsEnv, rodsArguments_t* myRodsArgs, rodsPathInp_t* rodsPathInp)
{
    ...
    if (rodsPathInp->srcPath[0].objType <= COLL_OBJ_T && rodsPathInp->targPath[0].objType <= COLL_OBJ_T) {
    ...
    for (int i = 0; i < rodsPathInp->numSrc; i++) {
        ...
        if (srcPath->objState != EXIST_ST) {
            rodsLog(LOG_ERROR, "rsyncUtil: Source path %s does not exist");
            return USER_INPUT_PATH_ERR;
        }

        if (srcPath->objType <= COLL_OBJ_T && srcPath->rodsObjStat != NULL &&
            rodsPathInp->srcPath[i].rodsObjStat->specColl != NULL) {
            dataObjOprInp.specColl = dataObjCopyInp.srcDataObjInp.specColl = srcPath->rodsObjStat->specColl;
        }
        else {
            dataObjOprInp.specColl = dataObjCopyInp.srcDataObjInp.specColl = NULL;
        }

        status = 0;
        if (srcType == DATA_OBJ_T && targType == LOCAL_FILE_T) {
            rmKeyVal(&dataObjOprInp.condInput, TRANSLATED_PATH_KW);
            status = rsyncDataToFileUtil(conn, srcPath, targPath, myRodsArgs, &dataObjOprInp);
        }
            ...
            status = rsyncFileToDataUtil(conn, srcPath, targPath, myRodsArgs, &dataObjOprInp);
        }
        else if (srcType == DATA_OBJ_T && targType == DATA_OBJ_T) {
            rmKeyVal(&dataObjCopyInp.srcDataObjInp.condInput, TRANSLATED_PATH_KW);
            addKeyVal(&dataObjCopyInp.destDataObjInp.condInput, REG_CHKSUM_KW, "");
            status = rsyncDataToDataUtil(conn, srcPath, targPath, myRodsArgs, &dataObjCopyInp);
        }
        else if (srcType == COLL_OBJ_T && targType == LOCAL_DIR_T) {
            addKeyVal(&dataObjOprInp.condInput, TRANSLATED_PATH_KW, "");
            status = rsyncCollToDirUtil(conn, srcPath, targPath, myRodsEnv, myRodsArgs, &dataObjOprInp);
            ...
            status = rsyncDirToCollUtil(conn, srcPath, targPath, myRodsEnv, myRodsArgs, &dataObjOprInp);
        }
        else if (srcType == COLL_OBJ_T && targType == COLL_OBJ_T) {
            addKeyVal(&dataObjCopyInp.srcDataObjInp.condInput, TRANSLATED_PATH_KW, "");
            addKeyVal(&dataObjCopyInp.destDataObjInp.condInput, REG_CHKSUM_KW, "");
            status = rsyncCollToCollUtil(conn, srcPath, targPath, myRodsEnv, myRodsArgs, &dataObjCopyInp);
            if (status >= 0 && dataObjOprInp.specColl != NULL &&
        ...
        if (status < 0 && status != CAT_NO_ROWS_FOUND && status != SYS_SPEC_COLL_OBJ_NOT_EXIST) {
            rodsLogError(LOG_ERROR, status, "rsyncUtil: rsync error for %s", targPath->outPath);
            savedStatus = status;
        }
    }
    return savedStatus;
                        ...
                        dataObjInp_t* dataObjOprInp)
{
    ...
    if (srcPath == NULL || targPath == NULL) {
        rodsLog(LOG_ERROR, "rsyncDataToFileUtil: NULL srcPath or targPath input");
        return USER__NULL_INPUT_ERR;
    }
    /* check the age */
    if (myRodsArgs->age == True) {
        if (srcPath->rodsObjStat != NULL) {
            if (ageExceeded(
                    myRodsArgs->agevalue, atoi(srcPath->rodsObjStat->modifyTime), srcPath->outPath, srcPath->size)) {
                return 0;
            }
        }
    }
    ...
    if (ret < 0) {
        rodsLogError(LOG_ERROR, ret, "rsyncDataToFileUtil: getRodsEnv failed");
        return ret;
    }
    ...
    if (targPath->objState == NOT_EXIST_ST) {
        ...
        std::string scheme;
        irods::error ret = irods::get_hash_scheme_from_checksum(srcPath->chksum, scheme);
        if (!ret.ok()) {
            printf("%s", ret.result().c_str());
        }

        /* src has a checksum value */
        status = rcChksumLocFile(targPath->outPath, RSYNC_CHKSUM_KW, &dataObjOprInp->condInput, scheme.c_str());
        if (status < 0) {
        ...
        /* exist but no chksum */
        status =
            rcChksumLocFile(targPath->outPath, RSYNC_CHKSUM_KW, &dataObjOprInp->condInput, env.rodsDefaultHashScheme);
        if (status < 0) {
            rodsLogError(LOG_ERROR,
                         status,
                         "rsyncDataToFileUtil: rcChksumLocFile error for %s, status = %d",
    ...
    if (getFlag == 1) {
        ...
        addKeyVal(&dataObjOprInp->condInput, RSYNC_DEST_PATH_KW, targPath->outPath);
    ...
    if (status >= 0 && myRodsArgs->verbose == True) {
        if (getFlag > 0 || (syncFlag > 0 && status == SYS_RSYNC_TARGET_MODIFIED)) {
            (void) gettimeofday(&endTime, (struct timezone*) 0);
            printTiming(conn, srcPath->outPath, srcPath->size, targPath->outPath, &startTime, &endTime);
        }
        else {
                        ...
                        dataObjInp_t* dataObjOprInp)
{
    ...
    if (srcPath == NULL || targPath == NULL) {
        rodsLog(LOG_ERROR, "rsyncFileToDataUtil: NULL srcPath or targPath input");
        return USER__NULL_INPUT_ERR;
    }
    /* check the age */
    if (myRodsArgs->age == True) {
...
#endif
        if (status < 0) {
            rodsLog(LOG_ERROR, "rsyncFileToDataUtil: stat error for %s, errno = %d\n", srcPath->outPath, errno);
            return USER_INPUT_PATH_ERR;
        }
        if (ageExceeded(myRodsArgs->agevalue, statbuf.st_mtime, srcPath->outPath, srcPath->size)) {
            return 0;
        }
    }
    ...
    int ret = getRodsEnv(&env);
    if (ret < 0) {
        rodsLogError(LOG_ERROR, ret, "rsyncFileToDataUtil: getRodsEnv failed");
        return ret;
    }
    ...
    if (targPath->objState == NOT_EXIST_ST) {
        ...
        if (True == myRodsArgs->verifyChecksum) {
            status = rcChksumLocFile(
                srcPath->outPath, RSYNC_CHKSUM_KW, &dataObjOprInp->condInput, env.rodsDefaultHashScheme);
            if (status < 0) {
                rodsLogError(LOG_ERROR,
                             status,
        ...
        // extract scheme from checksum string
        std::string scheme;
        irods::error ret = irods::get_hash_scheme_from_checksum(targPath->chksum, scheme);
        if (!ret.ok()) {
            printf("%s", ret.result().c_str());
        }

        /* src has a checksum value */
        status = rcChksumLocFile(srcPath->outPath, RSYNC_CHKSUM_KW, &dataObjOprInp->condInput, scheme.c_str());
        if (status < 0) {
            rodsLogError(LOG_ERROR,
                         status,
                         "rsyncFileToDataUtil: rcChksumLocFile error for %s, status = %d",
        ...
        /* exist but no chksum */
        status =
            rcChksumLocFile(srcPath->outPath, RSYNC_CHKSUM_KW, &dataObjOprInp->condInput, env.rodsDefaultHashScheme);
        if (status < 0) {
            rodsLogError(LOG_ERROR,
                         status,
                         "rsyncFileToDataUtil: rcChksumLocFile error for %s, status = %d",
    ...
    if (putFlag == 1) {
        ...
        addKeyVal(&dataObjOprInp->condInput, RSYNC_DEST_PATH_KW, srcPath->outPath);
    ...
    if (status >= 0 && myRodsArgs->verbose == True) {
        if (putFlag > 0 || (syncFlag > 0 && status == SYS_RSYNC_TARGET_MODIFIED)) {
            (void) gettimeofday(&endTime, (struct timezone*) 0);
            printTiming(conn, srcPath->outPath, srcPath->size, targPath->outPath, &startTime, &endTime);
        }
        else {
                        ...
                        dataObjCopyInp_t* dataObjCopyInp)
{
    ...
    if (srcPath == NULL || targPath == NULL) {
        rodsLog(LOG_ERROR, "rsyncDataToDataUtil: NULL srcPath or targPath input");
        return USER__NULL_INPUT_ERR;
    }
    /* check the age */
    if (myRodsArgs->age == True) {
    ...
    if (cpFlag == 1) {
        rstrcpy(dataObjCopyInp->srcDataObjInp.objPath, srcPath->outPath, MAX_NAME_LEN);
        dataObjCopyInp->srcDataObjInp.dataSize = srcPath->size;
        rstrcpy(dataObjCopyInp->destDataObjInp.objPath, targPath->outPath, MAX_NAME_LEN);
        /* only do the sync if no -l option specified */
        if (myRodsArgs->longOption != True) {
    ...
    if (status >= 0 && myRodsArgs->verbose == True) {
        if (cpFlag > 0 || (syncFlag > 0 && status == SYS_RSYNC_TARGET_MODIFIED)) {
            (void) gettimeofday(&endTime, (struct timezone*) 0);
            printTiming(conn, srcPath->outPath, srcPath->size, targPath->outPath, &startTime, &endTime);
        }
        else {
                       ...
                       dataObjInp_t* dataObjOprInp)
{
    ...
    if (srcPath == NULL || targPath == NULL) {
        rodsLog(LOG_ERROR, "rsyncCollToDirUtil: NULL srcPath or targPath input");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (rodsArgs->recursive != True) {
        rodsLog(LOG_ERROR, "rsyncCollToDirUtil: -r option must be used for collection %s", targDir);
        return USER_INPUT_OPTION_ERR;
    }

    status = rclOpenCollection(conn, srcColl, VERY_LONG_METADATA_FG, &collHandle);

    if (status < 0) {
        rodsLog(LOG_ERROR, "rsyncCollToDirUtil: rclOpenCollection of %s error. status = %d", srcColl, status);
        return status;
    }
    ...
    while ((status = rclReadCollection(conn, &collHandle, &collEnt)) >= 0) {
        if (collEnt.objType == DATA_OBJ_T) {
            if (rodsArgs->age == True) {
                if (ageExceeded(rodsArgs->agevalue, atoi(collEnt.modifyTime), collEnt.dataName, collEnt.dataSize)) {
                    continue;
                }
            }
            snprintf(myTargPath.outPath, MAX_NAME_LEN, "%s/%s", targDir, collEnt.dataName);
            snprintf(mySrcPath.outPath, MAX_NAME_LEN, "%s/%s", collEnt.collName, collEnt.dataName);
            ...
            int status = rsyncDataToFileUtil(conn, &mySrcPath, &myTargPath, rodsArgs, dataObjOprInp);
            if (status < 0) {
                rodsLogError(LOG_ERROR,
                             status,
            ...
            snprintf(targChildPath, MAX_NAME_LEN, "%s/%s", targDir, childPath);
            ...
            int status = rsyncCollToDirUtil(conn, &mySrcPath, &myTargPath, myRodsEnv, rodsArgs, &childDataObjInp);
    ...
    else if (status == CAT_NO_ROWS_FOUND || status == SYS_SPEC_COLL_OBJ_NOT_EXIST) {
        return 0;
    }
    else {
        return status;
                       ...
                       dataObjInp_t* dataObjOprInp)
{
    ...
    if (srcPath == NULL || targPath == NULL) {
        rodsLog(LOG_ERROR, "rsyncDirToCollUtil: NULL srcPath or targPath input");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (rodsArgs->recursive != True) {
        rodsLog(LOG_ERROR, "rsyncDirToCollUtil: -r option must be used for putting %s directory", srcDir);
        return USER_INPUT_OPTION_ERR;
    }

    path srcDirPath(srcDir);
    if (!exists(srcDirPath) || !is_directory(srcDirPath)) {
        rodsLog(LOG_ERROR, "rsyncDirToCollUtil: opendir local dir error for %s, errno = %d\n", srcDir, errno);
        return USER_INPUT_PATH_ERR;
    }
    ...
    for (directory_iterator itr(srcDirPath); itr != end_itr; ++itr) {
        ...
        if ((is_regular_file(p) && rodsArgs->age == True) &&
            ageExceeded(rodsArgs->agevalue, last_write_time(p), mySrcPath.outPath, file_size(p))) {
            continue;
        }

        bzero(&myTargPath, sizeof(myTargPath));
        path childPath = p.filename();
        snprintf(myTargPath.outPath, MAX_NAME_LEN, "%s/%s", targColl, childPath.c_str());
        if (is_symlink(p)) {
        ...
        if (is_regular_file(p)) {
            ...
            status = rsyncFileToDataUtil(conn, &mySrcPath, &myTargPath, rodsArgs, dataObjOprInp);
            /* fix a big mem leak */
            ...
            if (status < 0) {
                rodsLogError(LOG_ERROR, status, "rsyncDirToCollUtil: mkColl error for %s", myTargPath.outPath);
            }
            else {
                ...
                getRodsObjType(conn, &myTargPath);
                status = rsyncDirToCollUtil(conn, &mySrcPath, &myTargPath, myRodsEnv, rodsArgs, dataObjOprInp);
                /* fix a big mem leak */
                if (myTargPath.rodsObjStat != NULL) {
                    freeRodsObjStat(myTargPath.rodsObjStat);
                    myTargPath.rodsObjStat = NULL;
            ...
            rodsLog(LOG_ERROR, "rsyncDirToCollUtil: unknown local path %s", mySrcPath.outPath);
            status = USER_INPUT_PATH_ERR;
        }
        ...
        if (status < 0 && status != CAT_NO_ROWS_FOUND && status != SYS_SPEC_COLL_OBJ_NOT_EXIST) {
            savedStatus = status;
            rodsLogError(
                LOG_ERROR, status, "rsyncDirToCollUtil: put %s failed. status = %d", mySrcPath.outPath, status);
        }
    }
                        ...
                        dataObjCopyInp_t* dataObjCopyInp)
{
    ...
    if (srcPath == NULL || targPath == NULL) {
        rodsLog(LOG_ERROR, "rsyncCollToCollUtil: NULL srcPath or targPath input");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (rodsArgs->recursive != True) {
        rodsLog(LOG_ERROR, "rsyncCollToCollUtil: -r option must be used for collection %s", targColl);
        return USER_INPUT_OPTION_ERR;
    }
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "getCollUtil: rclOpenCollection of %s error. status = %d", srcColl, status);
        return status;
    }
    ...
    while ((status = rclReadCollection(conn, &collHandle, &collEnt)) >= 0) {
        if (collEnt.objType == DATA_OBJ_T) {
            ...
            snprintf(myTargPath.outPath, MAX_NAME_LEN, "%s/%s", targColl, collEnt.dataName);
            ...
            int status = rsyncDataToDataUtil(conn, &mySrcPath, &myTargPath, rodsArgs, dataObjCopyInp);
            if (myTargPath.rodsObjStat != NULL) {
                freeRodsObjStat(myTargPath.rodsObjStat);
                myTargPath.rodsObjStat = NULL;
            }
            ...
            snprintf(targChildPath, MAX_NAME_LEN, "%s/%s", targColl, childPath);
            ...
            if (collEnt.specColl.collClass != NO_SPEC_COLL)
                childDataObjCopyInp.srcDataObjInp.specColl = &collEnt.specColl;
            rstrcpy(myTargPath.outPath, targChildPath, MAX_NAME_LEN);
            rstrcpy(mySrcPath.outPath, collEnt.collName, MAX_NAME_LEN);
            int status = rsyncCollToCollUtil(conn, &mySrcPath, &myTargPath, myRodsEnv, rodsArgs, &childDataObjCopyInp);
...
int initCondForRsync(rodsEnv* myRodsEnv, rodsArguments_t* rodsArgs, dataObjInp_t* dataObjInp)
{
    ...
    if (dataObjInp == NULL) {
        rodsLog(LOG_ERROR, "initCondForRsync: NULL dataObjOprInp input");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (rodsArgs->resource == True) {
        if (rodsArgs->resourceString == NULL) {
            rodsLog(LOG_ERROR, "initCondForRepl: NULL resourceString error");
            return USER__NULL_INPUT_ERR;
        }
        else {
            addKeyVal(&dataObjInp->condInput, DEST_RESC_NAME_KW, rodsArgs->resourceString);
        }
    }
    else if (myRodsEnv != NULL && strlen(myRodsEnv->rodsDefResource) > 0) {
        addKeyVal(&dataObjInp->condInput, DEST_RESC_NAME_KW, myRodsEnv->rodsDefResource);
    }
    if (rodsArgs->age == True) {
...
int initCondForIrodsToIrodsRsync(rodsEnv* myRodsEnv, rodsArguments_t* rodsArgs, dataObjCopyInp_t* dataObjCopyInp)
{
    ...
    if (dataObjCopyInp == NULL) {
        rodsLog(LOG_ERROR, "initCondForCp: NULL dataObjCopyInp incp");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (rodsArgs->sizeFlag == True) {
        addKeyVal(&dataObjCopyInp->destDataObjInp.condInput, VERIFY_BY_SIZE_KW, "");
    }
    ...
    if (rodsArgs->resource == True) {
        if (rodsArgs->resourceString == NULL) {
            ...
            addKeyVal(&dataObjCopyInp->destDataObjInp.condInput, DEST_RESC_NAME_KW, rodsArgs->resourceString);
        }
    }
    else if (myRodsEnv != NULL && strlen(myRodsEnv->rodsDefResource) > 0) {
        addKeyVal(&dataObjCopyInp->destDataObjInp.condInput, DEST_RESC_NAME_KW, myRodsEnv->rodsDefResource);
    }
// =-=-=-=-=-=-=-
// JMC - backport 4865
#ifdef windows_platform
