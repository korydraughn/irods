...
namespace irods
{
                                            ...
                                            const char escape_char)
    {
        ...
        if (escape_char) {
            ...
            for (std::vector<std::string>::const_iterator iter = strs.begin(); iter != strs.end(); ++iter) {
                escaped_strs.push_back(boost::regex_replace(
                    *iter, special_character_set_regex, get_format_string_for_escape(escape_char)));
            }
        }
        else {
            for (std::vector<std::string>::const_iterator iter = strs.begin(); iter != strs.end(); ++iter) {
                                              ...
                                              const char escape_char)
    {
        ...
        for (std::string::const_iterator iter = list.begin(); iter != list.end(); ++iter) {
            if (*iter == escape_char) {
                ...
                if (remaining_delimiters.size() &&
                    (*iter == escape_char || remaining_delimiters.find(*iter) != std::string::npos)) {
    ...
    std::vector<std::vector<std::string>> deserialize_acl(const std::string& acl)
    {
        ...
        for (std::vector<std::string>::const_iterator iter = shallow_deserialized_acl.begin();
             iter != shallow_deserialized_acl.end();
             ++iter) {
