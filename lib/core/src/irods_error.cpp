...
namespace irods
{
    ...
    std::string error::build_result_string(std::string _file, int _line, std::string _fcn)
    {
        // =-=-=-=-=-=-=-
        ...
        result += line_info + error::colon_token_ + error::status_token_ + irods_err + "]  errno [" + errno_str + "]" +
                  " -- message [" + message_ + "]";
    ...
    error::error(bool _status, long long _code, std::string _msg, std::string _file, int _line, std::string _fcn)
        : status_(_status)
        , code_(_code)
        , message_(_msg)
    {
    ...
    error::error(bool _status, long long _code, boost::format _msg, std::string _file, int _line, std::string _fcn)
    ...
    error::error(bool, long long, std::string _msg, std::string _file, int _line, std::string _fcn, const error& _rhs)
        : status_(_rhs.status())
        , code_(_rhs.code())
        , message_(_msg)
    {
    ...
    // =-=-=-=-=-=-=-
    // public - useful constructor
    error::error(std::string _msg, std::string _file, int _line, std::string _fcn, const error& _rhs)
        : error(_rhs)
    {
        if (exception_) {
    ...
    error assert_pass(const error& _prev_error,
