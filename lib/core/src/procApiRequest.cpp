                   ...
                   bytesBuf_t* outBsBBuf)
{
    ...
    if (status < 0) {
        rodsLogError(LOG_DEBUG, status, "procApiRequest: sendApiRequest failed. status = %d", status);
        return status;
    }
    ...
    if (status < 0) {
        rodsLogError(LOG_DEBUG, status, "procApiRequest: readAndProcApiReply failed. status = %d", status);
    }
...
int branchReadAndProcApiReply(rcComm_t* conn, int apiNumber, void** outStruct, bytesBuf_t* outBsBBuf)
{
    ...
    apiInx = apiTableLookup(apiNumber);
    if (apiInx < 0) {
        rodsLog(LOG_ERROR, "branchReadAndProcApiReply: apiTableLookup of apiNum %d failed", apiNumber);
        return apiInx;
    }
    ...
    status = readAndProcApiReply(conn, apiInx, outStruct, outBsBBuf);
    if (status < 0) {
        rodsLogError(LOG_DEBUG, status, "branchReadAndProcApiReply: readAndProcApiReply failed. status = %d", status);
    }
    return status;
}
...
int sendApiRequest(rcComm_t* conn, int apiInx, const void* inputStruct, const bytesBuf_t* inputBsBBuf)
{
    ...
    if (RcApiTable[apiInx]->inPackInstruct != NULL) {
        ...
        status = packStruct(inputStruct,
                            &inputStructBBuf,
                            (char*) RcApiTable[apiInx]->inPackInstruct,
                            RodsPackTable,
                            0,
                            conn->irodsProt);
        if (status < 0) {
            rodsLogError(LOG_ERROR, status, "sendApiRequest: packStruct error, status = %d", status);
            cliChkReconnAtSendEnd(conn);
            return status;
        }
    ...
    ret = sendRodsMsg(
        net_obj, RODS_API_REQ_T, myInputStructBBuf, inputBsBBuf, NULL, RcApiTable[apiInx]->apiNumber, conn->irodsProt);
    if (!ret.ok()) {
        irods::log(PASS(ret));
        if (conn->svrVersion != NULL && conn->svrVersion->reconnPort > 0) {
            ...
            if (status1 > 0) {
                /* should not be here */
                rodsLog(LOG_NOTICE, "sendApiRequest: Switch connection and retry sendRodsMsg");
...
int readAndProcApiReply(rcComm_t* conn, int apiInx, void** outStruct, bytesBuf_t* outBsBBuf)
{
    ...
    if (RcApiTable[apiInx]->outPackInstruct != NULL && outStruct == NULL) {
        rodsLog(LOG_ERROR, "readAndProcApiReply: outStruct error for A apiNumber %d", RcApiTable[apiInx]->apiNumber);
        cliChkReconnAtReadEnd(conn);
        return USER_API_INPUT_ERR;
    }

    if (RcApiTable[apiInx]->outBsFlag > 0 && outBsBBuf == NULL) {
        rodsLog(LOG_ERROR, "readAndProcApiReply: outBsBBuf error for B apiNumber %d", RcApiTable[apiInx]->apiNumber);
        cliChkReconnAtReadEnd(conn);
        return USER_API_INPUT_ERR;
    }
    ...
    ret = readMsgBody(net_obj, &myHeader, &outStructBBuf, outBsBBuf, &errorBBuf, conn->irodsProt, NULL);
    if (!ret.ok()) {
        irods::log(PASS(ret));
        cliChkReconnAtReadEnd(conn);
        return status;
    ...
    if (strcmp(myHeader.type, RODS_API_REPLY_T) == 0) {
        status = procApiReply(conn, apiInx, outStruct, outBsBBuf, &myHeader, &outStructBBuf, NULL, &errorBBuf);
    }
                 ...
                 bytesBuf_t* errorBBuf)
{
    ...
    if (errorBBuf->len > 0) {
        status = unpackStruct(
            errorBBuf->buf, (void**) (static_cast<void*>(&conn->rError)), "RError_PI", RodsPackTable, conn->irodsProt);
        if (status < 0) {
            rodsLogError(LOG_ERROR, status, "readAndProcApiReply:unpackStruct error. status = %d", status);
        }
    }
    ...
    irods::api_entry_table& RcApiTable = irods::get_client_api_table();
    if (RcApiTable[apiInx]->outPackInstruct != NULL && outStruct == NULL) {
        rodsLog(LOG_ERROR, "readAndProcApiReply: outStruct error for C apiNumber %d", RcApiTable[apiInx]->apiNumber);
        if (retVal < 0) {
            return retVal;
        }
        else {
    ...
    if (RcApiTable[apiInx]->outBsFlag > 0 && outBsBBuf == NULL) {
        rodsLog(LOG_ERROR, "readAndProcApiReply: outBsBBuf error for D apiNumber %d", RcApiTable[apiInx]->apiNumber);
        if (retVal < 0) {
            return retVal;
        }
        else {
    ...
    if (outStructBBuf->len > 0) {
        if (outStruct != NULL) {
            status = unpackStruct(outStructBBuf->buf,
                                  (void**) outStruct,
                                  (char*) RcApiTable[apiInx]->outPackInstruct,
                                  RodsPackTable,
                                  conn->irodsProt);
            if (status < 0) {
                rodsLogError(LOG_ERROR, status, "readAndProcApiReply:unpackStruct error. status = %d", status);
    ...
    if (irods::CS_NEG_USE_SSL == conn->negotiation_results) {
        if (int bytes_left = 4 - sslWrite(static_cast<void*>(&myBuf), 4, NULL, conn->ssl)) {
            rodsLogError(LOG_ERROR,
                         SYS_SOCK_WRITE_ERR,
                         "sslWrite exited with %d bytes still left to write in %s.",
                         bytes_left,
                         __PRETTY_FUNCTION__);
        ...
        if (int bytes_left = 4 - myWrite(conn->sock, (void*) &myBuf, 4, NULL)) {
            rodsLogError(LOG_ERROR,
                         SYS_SOCK_WRITE_ERR,
                         "myWrite exited with %d bytes still left to write in %s.",
                         bytes_left,
                         __PRETTY_FUNCTION__);
            return SYS_SOCK_WRITE_ERR;
        }
