...
int regUtil(rcComm_t* conn, rodsEnv* myRodsEnv, rodsArguments_t* myRodsArgs, rodsPathInp_t* rodsPathInp)
{
    ...
    for (int i = 0; i < rodsPathInp->numSrc; i++) {
        ...
        if (destPath->objState == EXIST_ST && myRodsArgs->force == False && myRodsArgs->mountCollection == False &&
            myRodsArgs->regRepl != True) {
            rodsLog(LOG_ERROR, "regUtil: iRODSPath %s already exist", destPath->outPath);
        ...
        if (status < 0 && status != CAT_NO_ROWS_FOUND) {
            rodsLogError(LOG_ERROR, status, "regUtil: reg error for %s, status = %d", destPath->outPath, status);
            savedStatus = status;
        }
...
int initCondForReg(rodsEnv* myRodsEnv, rodsArguments_t* rodsArgs, dataObjInp_t* dataObjOprInp)
{
    ...
    if (rodsArgs->rescGroup == True) {
        if (rodsArgs->resource != True) {
            rodsLog(LOG_ERROR, "initCondForReg: rescGroup must be input together with -Rresource");
            return USER__NULL_INPUT_ERR;
        }
        else if (rodsArgs->rescGroupString == NULL) {
            rodsLog(LOG_ERROR, "initCondForReg: NULL rescGroupString error");
            return USER__NULL_INPUT_ERR;
        }
    }
    ...
    if (rodsArgs->excludeFile == True) {
        if (rodsArgs->excludeFileString == NULL) {
            ...
            addKeyVal(&dataObjOprInp->condInput, EXCLUDE_FILE_KW, rodsArgs->excludeFileString);
        }
    }
