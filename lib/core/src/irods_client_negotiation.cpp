...
namespace irods
{
        ...
        try {
            for (const auto& el : irods::get_server_property<const std::vector<boost::any>>(irods::CFG_FEDERATION_KW)) {
                try {
                    ...
                    try {
                        if (_host_name ==
                            boost::any_cast<const std::string&>(boost::any_cast<const std::vector<boost::any>&>(
                                federation.at(irods::CFG_CATALOG_PROVIDER_HOSTS_KW))[0])) {
                            return boost::any_cast<const std::string&>(federation.at(irods::CFG_NEGOTIATION_KEY_KW));
                        }
                    }
                    catch (const boost::bad_any_cast&) {
                        rodsLog(LOG_ERROR, "%s - failed to cast federation entry to string", __PRETTY_FUNCTION__);
                        continue;
                    }
    ...
    error sign_server_sid(const std::string _svr_sid, const std::string _enc_key, std::string& _signed_sid)
    {
        // =-=-=-=-=-=-=-
        ...
        irods::error err = crypt.encrypt(key,
                                         key, // reuse key as iv
                                         in_buf,
                                         out_buf);
        if (!err.ok()) {
    ...
    public:
        ...
        error operator()(const std::string& _cli_policy, const std::string& _svr_policy, std::string& _result)
        {
            ...
            if (cli_idx > 2 || cli_idx < 0) {
                return ERROR(SYS_INVALID_INPUT_PARAM, "client policy index is out of bounds");
            }
            ...
            if (svr_idx > 2 || svr_idx < 0) {
                return ERROR(SYS_INVALID_INPUT_PARAM, "server policy index is out of bounds");
            }
    ...
    error client_server_negotiation_for_client(irods::network_object_ptr _ptr,
                                               ...
                                               std::string& _result)
    {
        ...
        if (svr_policy.empty() || cs_neg->status_ != CS_NEG_STATUS_SUCCESS) {
            ...
            return ERROR(CLIENT_NEGOTIATION_ERROR, msg.str());
        }
        ...
        if (!err.ok() || CS_NEG_FAILURE == result) {
            ...
            snprintf(send_cs_neg.result_, sizeof(send_cs_neg.result_), "%s", CS_NEG_FAILURE.c_str());
            error send_err = send_client_server_negotiation_message(_ptr, send_cs_neg);
            if (!send_err.ok()) {
            ...
            ret = ERROR(CLIENT_NEGOTIATION_ERROR, msg.str());
            return ret;
        }
        ...
        if (!client_side) {
            ...
            try {
                ...
                try {
                    ...
                    err = sign_server_sid(*sid, neg_key, signed_sid);
                    if (err.ok()) {
                        // =-=-=-=-=-=-=-
                        // add the SID to the returning client message
                        cli_msg += CS_NEG_SID_KW + irods::kvp_association() + signed_sid + irods::kvp_delimiter();
                    }
                    else {
        ...
        cli_msg += CS_NEG_RESULT_KW + irods::kvp_association() + result + irods::kvp_delimiter();
        ...
        err = send_client_server_negotiation_message(_ptr, send_cs_neg);
        if (!err.ok()) {
    ...
    error send_client_server_negotiation_message(irods::network_object_ptr _ptr, cs_neg_t& _cs_neg_msg)
    {
        // =-=-=-=-=-=-=-
        // pack the negotiation message
        bytesBuf_t* cs_neg_buf = 0;
        int status = packStruct(&_cs_neg_msg, &cs_neg_buf, "CS_NEG_PI", RodsPackTable, 0, XML_PROT);
        if (status < 0) {
        ...
        // =-=-=-=-=-=-=-
        // pack the negotiation message
        irods::error ret = sendRodsMsg(_ptr, RODS_CS_NEG_T, cs_neg_buf, 0, 0, 0, XML_PROT);
        freeBBuf(cs_neg_buf);
        if (!ret.ok()) {
    ...
    /// =-=-=-=-=-=-=-
    /// @brief function which sends the negotiation message
    error read_client_server_negotiation_message(irods::network_object_ptr _ptr,
                                                 boost::shared_ptr<cs_neg_t>& _cs_neg_msg)
    {
        ...
        ret = readMsgBody(_ptr, &msg_header, &struct_buf, &data_buf, &error_buf, XML_PROT, 0);
        if (!ret.ok()) {
            return PASS(ret);
        }
        ...
        if (strcmp(msg_header.type, RODS_CS_NEG_T) != 0) {
            ...
            if (strcmp(msg_header.type, RODS_VERSION_T) == 0) {
                ...
                int status = unpackStruct(
                    struct_buf.buf, (void**) (static_cast<void*>(&version)), "Version_PI", RodsPackTable, XML_PROT);
                ...
                if (version->status < 0) {
                    rodsLog(LOG_ERROR,
                            "read_client_server_negotiation_message :: received error message %d",
                            version->status);
        ...
        if (msg_header.bsLen != 0) {
            ...
            rodsLog(
                LOG_NOTICE, "read_client_server_negotiation_message: msg_header.bsLen = %d is not 0", msg_header.bsLen);
        }
        ...
        if (msg_header.msgLen > (int) sizeof(irods::cs_neg_t) * 2 || msg_header.msgLen <= 0) {
        ...
        int status = unpackStruct(
            struct_buf.buf, (void**) (static_cast<void*>(&tmp_cs_neg)), "CS_NEG_PI", RodsPackTable, XML_PROT);
        free(struct_buf.buf);
        if (status < 0) {
            rodsLog(LOG_ERROR, "read_client_server_negotiation_message :: unpackStruct FAILED");
            return ERROR(status, "unpackStruct failed");
