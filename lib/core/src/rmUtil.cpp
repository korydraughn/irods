    ...
    for (int i = 0; i < rodsPathInp->numSrc; i++) {
        if (rodsPathInp->srcPath[i].objType == UNKNOWN_OBJ_T) {
            ...
            if (rodsPathInp->srcPath[i].objState == NOT_EXIST_ST) {
                rodsLog(LOG_ERROR, "rmUtil: srcPath %s does not exist", rodsPathInp->srcPath[i].outPath);
        ...
        if (rodsPathInp->srcPath[i].objType == DATA_OBJ_T) {
            status = rmDataObjUtil(conn, rodsPathInp->srcPath[i].outPath, myRodsArgs, &dataObjInp);
        }
        else if (rodsPathInp->srcPath[i].objType == COLL_OBJ_T) {
            status = rmCollUtil(conn, rodsPathInp->srcPath[i].outPath, myRodsArgs, &collInp);
        }
        else {
            /* should not be here */
            rodsLog(LOG_ERROR,
        ...
        if (status < 0 && status != CAT_NO_ROWS_FOUND) {
            rodsLogError(
                LOG_ERROR, status, "rmUtil: rm error for %s, status = %d", rodsPathInp->srcPath[i].outPath, status);
            savedStatus = status;
        }
    }
    return savedStatus;
...
int rmDataObjUtil(rcComm_t* conn, char* srcPath, rodsArguments_t* rodsArgs, dataObjInp_t* dataObjInp)
{
    ...
    if (srcPath == NULL) {
        rodsLog(LOG_ERROR, "rmDataObjUtil: NULL srcPath input");
        return USER__NULL_INPUT_ERR;
    }
...
int initCondForRm(rodsArguments_t* rodsArgs, dataObjInp_t* dataObjInp, collInp_t* collInp)
{
    ...
    if (dataObjInp == NULL) {
        rodsLog(LOG_ERROR, "initCondForRm: NULL dataObjInp input");
        return USER__NULL_INPUT_ERR;
    }
...
int rmCollUtil(rcComm_t* conn, char* srcColl, rodsArguments_t* rodsArgs, collInp_t* collInp)
{
    ...
    if (srcColl == NULL) {
        rodsLog(LOG_ERROR, "rmCollUtil: NULL srcColl input");
        return USER__NULL_INPUT_ERR;
    }

    if (rodsArgs->recursive != True) {
        rodsLog(LOG_ERROR, "rmCollUtil: -r option must be used for collection %s", srcColl);
