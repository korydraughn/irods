...
namespace irods
{
    ...
    void environment_properties::capture()
    {
        ...
        if (!boost::filesystem::exists(json_file)) {
            rodsLog(LOG_ERROR,
                    "environment_properties::capture: missing environment file. should be at [%s]",
                    json_file.c_str());
            return;
        }
    ...
    void environment_properties::capture_json(const std::string& _fn)
    {
