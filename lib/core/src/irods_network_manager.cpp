...
namespace irods
{
    ...
    error network_manager::resolve(std::string _key, network_ptr& _value)
    {
        if (_key.empty()) {
        ...
        if (plugins_.has_entry(_key)) {
            ...
            msg << "no network plugin found for name [" << _key << "]";
            return ERROR(SYS_INVALID_INPUT_PARAM, msg.str());
        }

    } // resolve

    static error load_network_plugin(network_ptr& _plugin,
                                     ...
                                     const std::string& _context)
    {
        ...
        network* net = 0;
        error ret = load_plugin<network>(net, _plugin_name, PLUGIN_TYPE_NETWORK, _inst_name, _context);
        if (ret.ok() && net) {
            _plugin.reset(net);
            return SUCCESS();
        }
    ...
    error network_manager::init_from_type(const int& _proc_type,
