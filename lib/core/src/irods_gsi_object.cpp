...
namespace irods
{
    gsi_auth_object::gsi_auth_object(rError_t* _r_error)
        ...
        if ((result = ASSERT_ERROR(_interface == AUTH_INTERFACE,
            ...
            if (!(result = ASSERT_PASS(ret, "Failed to resolve the GSI auth plugin.")).ok()) {
                ...
                ret = auth_mgr.init_from_type(
                    ProcessType, AUTH_GSI_SCHEME, AUTH_GSI_SCHEME, AUTH_GSI_SCHEME, empty_context, ath);
    ...
    bool gsi_auth_object::operator==(const gsi_auth_object& _rhs) const
    {
    ...
    error gsi_auth_object::get_re_vars(rule_engine_vars_t& _kvp)
    {
