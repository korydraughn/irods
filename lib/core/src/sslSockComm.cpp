...
int sslStart(rcComm_t* rcComm)
{
    ...
    snprintf(rcComm->negotiation_results, sizeof(rcComm->negotiation_results), "%s", irods::CS_NEG_USE_SSL.c_str());
    return 0;
}
...
int sslEnd(rcComm_t* rcComm)
{
    ...
    snprintf(rcComm->negotiation_results, sizeof(rcComm->negotiation_results), "%s", irods::CS_NEG_USE_TCP.c_str());
...
int sslAccept(rsComm_t* rsComm)
{
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "sslAccept - failed in getRodsEnv : %d", status);
        return status;
    }

    /* set up the context using a certificate file and separate
       keyfile passed through environment variables */
    rsComm->ssl_ctx = sslInit(env.irodsSSLCertificateChainFile, env.irodsSSLCertificateKeyFile);
    ...
    rsComm->ssl_on = 1;
    snprintf(rsComm->negotiation_results, sizeof(rsComm->negotiation_results), "%s", irods::CS_NEG_USE_SSL.c_str());
...
int sslShutdown(rsComm_t* rsComm)
{
    ...
    snprintf(rsComm->negotiation_results, sizeof(rsComm->negotiation_results), "%s", irods::CS_NEG_USE_TCP.c_str());
    rodsLog(LOG_DEBUG, "sslShutdown: shut down SSL connection");
...
int sslReadMsgHeader(int sock, msgHeader_t* myHeader, struct timeval* tv, SSL* ssl)
{
    ...
    nbytes = sslRead(sock, (void*) &myLen, sizeof(myLen), NULL, tv, ssl);
    ...
    if (myLen > MAX_NAME_LEN || myLen <= 0) {
        rodsLog(LOG_ERROR, "sslReadMsgHeader: header length %d out of range", myLen);
    ...
    if (nbytes != myLen) {
        ...
        rodsLog(
            LOG_ERROR, "sslReadMsgHeader:header read- read %d bytes, expect %d, status = %d", nbytes, myLen, status);
        return status;
    }
    ...
    status = unpackStruct(
        (void*) tmpBuf, (void**) (static_cast<void*>(&outHeader)), "MsgHeader_PI", RodsPackTable, XML_PROT);

    if (status < 0) {
        rodsLogError(LOG_ERROR, status, "sslReadMsgHeader:unpackStruct error. status = %d", status);
        return status;
    }
                   ...
                   SSL* ssl)
{
    ...
    if (myHeader->msgLen > 0) {
        ...
        nbytes = sslRead(sock, inputStructBBuf->buf, myHeader->msgLen, NULL, tv, ssl);
    ...
    if (myHeader->errorLen > 0) {
        ...
        nbytes = sslRead(sock, errorBBuf->buf, myHeader->errorLen, NULL, tv, ssl);
    ...
    if (myHeader->bsLen > 0) {
        ...
        nbytes = sslRead(sock, bsBBuf->buf, myHeader->bsLen, &bytesRead, tv, ssl);
...
int sslWriteMsgHeader(msgHeader_t* myHeader, SSL* ssl)
{
    ...
    status = packStruct((void*) myHeader, &headerBBuf, "MsgHeader_PI", RodsPackTable, 0, XML_PROT);

    if (status < 0) {
        rodsLogError(LOG_ERROR, status, "sslWriteMsgHeader: packStruct error, status = %d", status);
        return status;
    }

    if (getRodsLogLevel() >= LOG_DEBUG8) {
        printf("sending header: len = %d\n%s\n", headerBBuf->len, (char*) headerBBuf->buf);
    }
    ...
    if (nbytes != sizeof(myLen)) {
        rodsLog(LOG_ERROR,
                ...
                nbytes,
                sizeof(myLen),
                SYS_HEADER_WRITE_LEN_ERR - errno);
        freeBBuf(headerBBuf);
                   ...
                   SSL* ssl)
{
    ...
    if (errorBBuf && errorBBuf->len > 0) {
        ...
        status = sslWrite(errorBBuf->buf, errorBBuf->len, NULL, ssl);
        if (status < 0) {
            return status;
        }
    }
    if (byteStreamBBuf && byteStreamBBuf->len > 0) {
        status = sslWrite(byteStreamBBuf->buf, byteStreamBBuf->len, &bytesWritten, ssl);
        if (status < 0) {
            return status;
        }
    }
...
static SSL_CTX* sslInit(char* certfile, char* keyfile)
{
    ...
    int status = getRodsEnv(&env);
    if (status < 0) {
        rodsLog(LOG_ERROR, "sslInit - failed in getRodsEnv : %d", status);
        return NULL;
    }
...
static int sslVerifyCallback(int ok, X509_STORE_CTX* store)
{
    ...
    if (!ok) {
        ...
        rodsLog(LOG_NOTICE, "sslVerifyCallback:   err %i:%s", err, X509_verify_cert_error_string(err));
    }
...
static int sslPostConnectionCheck(SSL* ssl, char* peer)
{
    ...
    int status = getRodsEnv(&env);
    if (status < 0) {
        rodsLog(LOG_ERROR, "sslPostConnectionCheck - failed in getRodsEnv : %d", status);
        return status;
    }
    ...
    if (!match && (X509_NAME_get_text_by_NID(X509_get_subject_name(cert), NID_commonName, cn, 256) != -1)) {
        cn[255] = 0;
