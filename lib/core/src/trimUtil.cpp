    ...
    for (int i = 0; i < rodsPathInp->numSrc; i++) {
        if (rodsPathInp->srcPath[i].objType == UNKNOWN_OBJ_T) {
            ...
            if (rodsPathInp->srcPath[i].objState == NOT_EXIST_ST) {
                rodsLog(LOG_ERROR, "trimUtil: srcPath %s does not exist", rodsPathInp->srcPath[i].outPath);
        ...
        if (rodsPathInp->srcPath[i].objType == DATA_OBJ_T) {
            ...
            status = trimDataObjUtil(conn, rodsPathInp->srcPath[i].outPath, myRodsArgs, &dataObjInp);
        }
        else if (rodsPathInp->srcPath[i].objType == COLL_OBJ_T) {
            addKeyVal(&dataObjInp.condInput, TRANSLATED_PATH_KW, "");
            status = trimCollUtil(conn, rodsPathInp->srcPath[i].outPath, myRodsEnv, myRodsArgs, &dataObjInp);
        }
        else {
            /* should not be here */
            rodsLog(LOG_ERROR,
        ...
        if (status < 0 && status != CAT_NO_ROWS_FOUND) {
            rodsLogError(LOG_ERROR, status, "trimUtil: trim error for %s. ", rodsPathInp->srcPath[i].outPath);
            savedStatus = status;
        }
    }
    printf("Total size trimmed = %-.3f MB. Number of files trimmed = %d.\n",
...
int trimDataObjUtil(rcComm_t* conn, char* srcPath, rodsArguments_t* rodsArgs, dataObjInp_t* dataObjInp)
{
    ...
    if (srcPath == NULL) {
        rodsLog(LOG_ERROR, "trimDataObjUtil: NULL srcPath input");
        return USER__NULL_INPUT_ERR;
    }
...
int initCondForTrim(rodsArguments_t* rodsArgs, dataObjInp_t* dataObjInp)
{
    ...
    if (dataObjInp == NULL) {
        rodsLog(LOG_ERROR, "initCondForTrim: NULL dataObjInp input");
        return USER__NULL_INPUT_ERR;
    }
...
int trimCollUtil(rcComm_t* conn, char* srcColl, rodsEnv* myRodsEnv, rodsArguments_t* rodsArgs, dataObjInp_t* dataObjInp)
{
    ...
    if (srcColl == NULL) {
        rodsLog(LOG_ERROR, "trimCollUtil: NULL srcColl input");
        return USER__NULL_INPUT_ERR;
    }

    if (rodsArgs->recursive != True) {
        rodsLog(LOG_ERROR, "trimCollUtil: -r option must be used for getting %s collection", srcColl);
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "trimCollUtil: rclOpenCollection of %s error. status = %d", srcColl, status);
        return status;
    }
    ...
    while ((status = rclReadCollection(conn, &collHandle, &collEnt)) >= 0) {
        if (collEnt.objType == DATA_OBJ_T) {
            ...
            int status = trimDataObjUtil(conn, srcChildPath, rodsArgs, dataObjInp);
            if (status < 0) {
            ...
            int status = trimCollUtil(conn, collEnt.collName, myRodsEnv, rodsArgs, &childDataObjInp);
            if (status < 0 && status != CAT_NO_ROWS_FOUND) {
                return status;
            }
        }
