...
int parseUserName(const char* fullUserNameIn, char* userName, char* userZone)
{
    ...
    const std::string zoneNameString = octothorpePointer ? std::string(octothorpePointer + 1) : std::string();
    if (zoneNameString.find('#') != std::string::npos || userNameString.size() >= NAME_LEN ||
        zoneNameString.size() >= NAME_LEN) {
        if (userName != NULL) {
            userName[0] = '\0';
        }
        if (userZone != NULL) {
    ...
    while ((c = *inPtr) != '\0' && *inbufLen > 0) {
        ...
        if (isspace(c)) {
            ...
            if (bytesCopied >= outbufLen - 1) {
                rodsLog(LOG_ERROR, "getStrInBuf: outbuf overflow buf len %d", bytesCopied);
                break;
            }
            *outPtr = c;
            bytesCopied++;
    ...
    while ((c = *inPtr) != '\0' && *inbufLen > 0) {
        ...
        if (isspace(c) && hasQuote == False) {
            ...
            if (bytesCopied >= maxOutLen - 1) {
                rodsLog(LOG_ERROR, "getNextEleInStr: outbuf overflow buf len %d", bytesCopied);
                break;
            }
            *outPtr = c;
            bytesCopied++;
...
int getLine(FILE* fp, char* buf, int bufSize)
{
    ...
    while ((c = getc(fp)) != EOF) {
        ...
        *cptr++ = c;
        ...
        /* overflow buf ? */
        if (len >= bufSize - 1) {
            rodsLog(LOG_ERROR, "getLine: buffer overflow bufSize %d", bufSize);
            break;
        }
    }
    if (c == EOF && len == 0) {
...
int rmKeyVal(keyValPair_t* condInput, char* keyWord)
{
    int i, j;
    ...
    for (i = 0; i < condInput->len; i++) {
        if (condInput->keyWord[i] != NULL && strcmp(condInput->keyWord[i], keyWord) == 0) {
...
int replKeyVal(const keyValPair_t* srcCondInput, keyValPair_t* destCondInput)
{
    ...
    for (i = 0; i < srcCondInput->len; i++) {
        addKeyVal(destCondInput, srcCondInput->keyWord[i], srcCondInput->value[i]);
    }
    return 0;
}
...
int addKeyVal(keyValPair_t* condInput, const char* keyWord, const char* value)
{
    ...
    if ((condInput->len % PTR_ARRAY_MALLOC_LEN) == 0) {
        condInput->keyWord =
            (char**) realloc(condInput->keyWord, (condInput->len + PTR_ARRAY_MALLOC_LEN) * sizeof(*condInput->keyWord));
        condInput->value =
            (char**) realloc(condInput->value, (condInput->len + PTR_ARRAY_MALLOC_LEN) * sizeof(*condInput->value));
...
int addStrArray(strArray_t* strArray, char* value)
{
    ...
    if (strArray->size <= 0) {
        if (strArray->len == 0) {
            ...
            rodsLog(LOG_ERROR, "addStrArray: invalid size %d, len %d", strArray->size, strArray->len);
            return SYS_INTERNAL_NULL_INPUT_ERR;
        }
    }
    ...
    if (size != strArray->size || (strArray->len % PTR_ARRAY_MALLOC_LEN) == 0) {
...
int resizeStrArray(strArray_t* strArray, int newSize)
{
    ...
    if (newSize > strArray->size || (strArray->len % PTR_ARRAY_MALLOC_LEN) == 0) {
        int oldSize = strArray->size;
        /* have to redo it */
        ...
        for (i = 0; i < strArray->len; i++) {
            rstrcpy(&newValue[i * newSize], &strArray->value[i * oldSize], newSize);
        }
        if (strArray->value != NULL) {
            free(strArray->value);
        }
...
int catGenQueryOut(genQueryOut_t* targGenQueryOut, genQueryOut_t* genQueryOut, int maxRowCnt)
{
    ...
    if ((totalRowCnt = targGenQueryOut->rowCnt + genQueryOut->rowCnt) > maxRowCnt) {
        rodsLog(LOG_ERROR,
                ...
                targGenQueryOut->rowCnt + genQueryOut->rowCnt,
                maxRowCnt);
    ...
    if (targGenQueryOut->attriCnt != genQueryOut->attriCnt) {
        rodsLog(
            LOG_ERROR, "catGenQueryOut: attriCnt mismatch %d != %d", targGenQueryOut->attriCnt, genQueryOut->attriCnt);
        return SYS_STRUCT_ELEMENT_MISMATCH;
    }

    for (i = 0; i < genQueryOut->attriCnt; i++) {
        if (targGenQueryOut->sqlResult[i].attriInx != genQueryOut->sqlResult[i].attriInx) {
            rodsLog(LOG_ERROR,
        ...
        if (targGenQueryOut->sqlResult[i].len != genQueryOut->sqlResult[i].len) {
            rodsLog(LOG_ERROR,
                    "catGenQueryOut: len mismatch %d != %d",
                    targGenQueryOut->sqlResult[i].len,
    ...
    for (i = 0; i < genQueryOut->attriCnt; i++) {
        ...
        if (targGenQueryOut->sqlResult[i].value != NULL) {
            memcpy(tmpValue, targGenQueryOut->sqlResult[i].value, len * targGenQueryOut->rowCnt);
            free(targGenQueryOut->sqlResult[i].value);
        }
        targGenQueryOut->sqlResult[i].value = tmpValue;
        tmpValue += len * targGenQueryOut->rowCnt;
        memcpy(tmpValue, genQueryOut->sqlResult[i].value, len * genQueryOut->rowCnt);
    }
...
int getUnixUsername(int uid, char* username, int username_len)
{
    ...
    if (pwent == NULL) {
        if (errno) {
            rodsLog(LOG_ERROR, "getUnixUsername: error calling getpwuid for uid %d. errno = %d", uid, errno);
        }
        else {
    ...
    if ((unsigned int) username_len <= strlen(pwent->pw_name)) {
        rodsLog(LOG_ERROR,
                "getUnixUsername: username input buffer too small (%d <= %d)",
                username_len,
...
int getUnixGroupname(int gid, char* groupname, int groupname_len)
{
    ...
    if (grent == NULL) {
        if (errno) {
            rodsLog(LOG_ERROR, "getUnixGroupname: error calling getgrgid for gid %d. errno = %d", gid, errno);
        }
        else {
    ...
    if ((unsigned int) groupname_len <= strlen(grent->gr_name)) {
        rodsLog(LOG_ERROR,
                "getUnixGroupname: groupname input buffer too small (%d <= %d)",
                groupname_len,
                strlen(grent->gr_name));
        return USER_STRLEN_TOOLONG;
    }
...
int checkDateFormat(char* s)
{
    ...
    if (isdigit(s[0]) && isdigit(s[1]) && isdigit(s[2]) && isdigit(s[3]) && isdigit(s[5]) && isdigit(s[6]) &&
        isdigit(s[8]) && isdigit(s[9]) && isdigit(s[11]) && isdigit(s[12]) && isdigit(s[14]) && isdigit(s[15]) &&
        isdigit(s[17]) && isdigit(s[18]) && s[4] == '-' && s[7] == '-' && s[10] == '.' && s[13] == ':' &&
        s[16] == ':') {
...
int openRestartFile(char* restartFile, rodsRestart_t* rodsRestart)
{
    ...
    if (!exists(p) || file_size(p) == 0) {
        ...
        if (rodsRestart->fd < 0) {
            ...
            rodsLogError(LOG_ERROR, status, "openRestartFile: open error for %s", restartFile);
            return status;
        }
        ...
        rodsLogError(LOG_ERROR, status, "openRestartFile: %s is not a file", restartFile);
        ...
        if (status <= 0) {
            ...
            rodsLogError(LOG_ERROR, status, "openRestartFile: read error for %s", restartFile);
            return status;
        }

        inptr = buf;
        if (getLineInBuf(&inptr, rodsRestart->collection, MAX_NAME_LEN) < 0) {
            rodsLog(LOG_ERROR, "openRestartFile: restartFile %s is empty", restartFile);
            return USER_RESTART_FILE_INPUT_ERR;
        }
        if (getLineInBuf(&inptr, tmpStr, MAX_NAME_LEN) < 0) {
            rodsLog(LOG_ERROR, "openRestartFile: restartFile %s has 1 only line", restartFile);
            return USER_RESTART_FILE_INPUT_ERR;
        }
        rodsRestart->doneCnt = atoi(tmpStr);

        if (getLineInBuf(&inptr, rodsRestart->lastDonePath, MAX_NAME_LEN) < 0) {
            rodsLog(LOG_ERROR, "openRestartFile: restartFile %s has only 2 lines", restartFile);
            return USER_RESTART_FILE_INPUT_ERR;
        }

        if (getLineInBuf(&inptr, rodsRestart->oprType, NAME_LEN) < 0) {
            rodsLog(LOG_ERROR, "openRestartFile: restartFile %s has only 3 lines", restartFile);
            return USER_RESTART_FILE_INPUT_ERR;
        }
        ...
        printf(
            "Restarting collection/directory = %s     File count %d\n", rodsRestart->collection, rodsRestart->doneCnt);
...
int writeRestartFile(rodsRestart_t* rodsRestart, char* lastDonePath)
{
    ...
    if (status != MAX_NAME_LEN * 3) {
        rodsLog(LOG_ERROR, "writeRestartFile: write error, errno = %d", errno);
...
int setStateForRestart(rodsRestart_t* rodsRestart, rodsPath_t* targPath, rodsArguments_t* rodsArgs)
{
    if (rodsRestart->restartState & PATH_MATCHING) {
        ...
        if (strstr(targPath->outPath, rodsRestart->collection) != NULL) {
            ...
            rodsRestart->restartState = rodsRestart->restartState & (~MATCHED_RESTART_COLL);
        }
    }
    else if (rodsRestart->fd > 0) {
        /* just writing restart file */
        rstrcpy(rodsRestart->collection, targPath->outPath, MAX_NAME_LEN);
...
int getMountedSubPhyPath(char* logMountPoint, char* phyMountPoint, char* logSubPath, char* phySubPathOut)
{
    ...
    if (strncmp(logSubPath, logMountPoint, len) != 0) {
        rodsLog(LOG_ERROR, "getMountedSubPhyPath: sub path %s not in mount point %s", logSubPath, logMountPoint);
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    /* skip over the mount point */
    tmpPtr = logSubPath + len;
    /* compose the physical path */
    snprintf(phySubPathOut, MAX_NAME_LEN, "%s%s", phyMountPoint, tmpPtr);
    return 0;
}
...
int getSpecCollTypeStr(specColl_t* specColl, char* outStr)
{
    ...
    if (specColl->collClass == NO_SPEC_COLL) {
        ...
        rodsLog(LOG_ERROR, "getSpecCollTypeStr: unmatch specColl type %d", specColl->type);
        return SYS_UNMATCHED_SPEC_COLL_TYPE;
    }
}
    ...
    if (strcmp(type, MOUNT_POINT_STR) == 0) {
        ...
        for (i = 0; i < NumStructFileType; i++) {
            if (strcmp(type, StructFileTypeDef[i].typeName) == 0) {
                ...
                rstrcpy(specColl->objPath, collInfo1, MAX_NAME_LEN);
        ...
        specColl->collClass = NO_SPEC_COLL;
        rodsLog(LOG_ERROR, "resolveSpecCollType: unmatch specColl type %s", type);
        return SYS_UNMATCHED_SPEC_COLL_TYPE;
    }
}
...
int parseCachedStructFileStr(char* collInfo2, specColl_t* specColl)
{
    ...
    if (collInfo2 == NULL || specColl == NULL) {
        rodsLog(LOG_ERROR, "parseCachedStructFileStr: NULL input");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (tmpPtr1 == NULL) {
        rodsLog(LOG_NOTICE, "parseCachedStructFileStr: collInfo2 %s format error 1", collInfo2);
    ...
    if (tmpPtr2 == NULL) {
        rodsLog(LOG_NOTICE, "parseCachedStructFileStr: collInfo2 %s format error 2", collInfo2);
        return SYS_COLLINFO_2_FORMAT_ERR;
    }
    ...
    *tmpPtr2 = '\0';
    ...
    snprintf(specColl->resource, sizeof(specColl->resource), "%s", first_resc.c_str());
    snprintf(specColl->rescHier, sizeof(specColl->rescHier), "%s", tmpPtr1);
    tmpPtr2 += 3;
...
{
    if (collInfo2 == NULL || specColl == NULL) {
        rodsLog(LOG_ERROR, "makeCachedStructFileStr: NULL input");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
...
int clearSendXmsgInfo(sendXmsgInfo_t* sendXmsgInfo)
{
    ...
    if (sendXmsgInfo->deliAddress != NULL && *sendXmsgInfo->deliAddress != NULL) {
        int i;
...
int initBulkDataObjRegInp(genQueryOut_t* bulkDataObjRegInp)
{
    ...
    bulkDataObjRegInp->sqlResult[0].value = (char*) malloc(MAX_NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bzero(bulkDataObjRegInp->sqlResult[0].value, MAX_NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bulkDataObjRegInp->sqlResult[1].attriInx = COL_DATA_TYPE_NAME;
    bulkDataObjRegInp->sqlResult[1].len = NAME_LEN;
    bulkDataObjRegInp->sqlResult[1].value = (char*) malloc(NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bzero(bulkDataObjRegInp->sqlResult[1].value, NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bulkDataObjRegInp->sqlResult[2].attriInx = COL_DATA_SIZE;
    bulkDataObjRegInp->sqlResult[2].len = NAME_LEN;
    bulkDataObjRegInp->sqlResult[2].value = (char*) malloc(NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bzero(bulkDataObjRegInp->sqlResult[2].value, NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bulkDataObjRegInp->sqlResult[3].attriInx = COL_D_RESC_NAME;
    bulkDataObjRegInp->sqlResult[3].len = NAME_LEN;
    bulkDataObjRegInp->sqlResult[3].value = (char*) malloc(NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bzero(bulkDataObjRegInp->sqlResult[3].value, NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bulkDataObjRegInp->sqlResult[4].attriInx = COL_D_DATA_PATH;
    bulkDataObjRegInp->sqlResult[4].len = MAX_NAME_LEN;
    bulkDataObjRegInp->sqlResult[4].value = (char*) malloc(MAX_NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bzero(bulkDataObjRegInp->sqlResult[4].value, MAX_NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bulkDataObjRegInp->sqlResult[5].attriInx = COL_DATA_MODE;
    bulkDataObjRegInp->sqlResult[5].len = NAME_LEN;
    bulkDataObjRegInp->sqlResult[5].value = (char*) malloc(NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bzero(bulkDataObjRegInp->sqlResult[5].value, NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bulkDataObjRegInp->sqlResult[6].attriInx = OPR_TYPE_INX;
    bulkDataObjRegInp->sqlResult[6].len = NAME_LEN;
    bulkDataObjRegInp->sqlResult[6].value = (char*) malloc(NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bzero(bulkDataObjRegInp->sqlResult[6].value, NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bulkDataObjRegInp->sqlResult[7].attriInx = COL_DATA_REPL_NUM;
    bulkDataObjRegInp->sqlResult[7].len = NAME_LEN;
    bulkDataObjRegInp->sqlResult[7].value = (char*) malloc(NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bzero(bulkDataObjRegInp->sqlResult[7].value, NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bulkDataObjRegInp->sqlResult[8].attriInx = COL_D_DATA_CHECKSUM;
    bulkDataObjRegInp->sqlResult[8].len = NAME_LEN;
    bulkDataObjRegInp->sqlResult[8].value = (char*) malloc(NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bzero(bulkDataObjRegInp->sqlResult[8].value, NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bulkDataObjRegInp->sqlResult[9].attriInx = COL_D_RESC_ID;
    bulkDataObjRegInp->sqlResult[9].len = MAX_NAME_LEN;
    bulkDataObjRegInp->sqlResult[9].value = (char*) malloc(MAX_NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bzero(bulkDataObjRegInp->sqlResult[9].value, MAX_NAME_LEN * MAX_NUM_BULK_OPR_FILES);
...
int initBulkDataObjRegOut(genQueryOut_t** bulkDataObjRegOut)
{
    ...
    myBulkDataObjRegOut->sqlResult[0].value = (char*) malloc(NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bzero(myBulkDataObjRegOut->sqlResult[0].value, NAME_LEN * MAX_NUM_BULK_OPR_FILES);
...
int initAttriArrayOfBulkOprInp(bulkOprInp_t* bulkOprInp)
{
    ...
    attriArray->sqlResult[0].value = (char*) malloc(MAX_NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bzero(attriArray->sqlResult[0].value, MAX_NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    attriArray->sqlResult[1].attriInx = COL_DATA_MODE;
    attriArray->sqlResult[1].len = NAME_LEN;
    attriArray->sqlResult[1].value = (char*) malloc(NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bzero(attriArray->sqlResult[1].value, NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    attriArray->sqlResult[2].attriInx = OFFSET_INX;
    attriArray->sqlResult[2].len = NAME_LEN;
    attriArray->sqlResult[2].value = (char*) malloc(NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    bzero(attriArray->sqlResult[2].value, NAME_LEN * MAX_NUM_BULK_OPR_FILES);
    ...
    if (getValByKey(&bulkOprInp->condInput, REG_CHKSUM_KW) != NULL ||
        ...
        attriArray->sqlResult[i].value = (char*) malloc(NAME_LEN * MAX_NUM_BULK_OPR_FILES);
        bzero(attriArray->sqlResult[i].value, NAME_LEN * MAX_NUM_BULK_OPR_FILES);
        attriArray->attriCnt++;
    }
...
int fillAttriArrayOfBulkOprInp(char* objPath, int dataMode, char* inpChksum, int offset, bulkOprInp_t* bulkOprInp)
{
    ...
    if (inpChksum != NULL && strlen(inpChksum) > 0) {
        if (chksum == NULL) {
            rodsLog(LOG_ERROR, "initAttriArrayOfBulkOprInp: getSqlResultByInx for COL_D_DATA_CHECKSUM failed");
            return UNMATCHED_KEY_OR_INDEX;
        }
        else {
    ...
    rstrcpy(&attriArray->sqlResult[0].value[MAX_NAME_LEN * rowCnt], objPath, MAX_NAME_LEN);
    snprintf(&attriArray->sqlResult[1].value[NAME_LEN * rowCnt], NAME_LEN, "%d", dataMode);
    snprintf(&attriArray->sqlResult[2].value[NAME_LEN * rowCnt], NAME_LEN, "%d", offset);
...
int getAttriInAttriArray(const char* inpObjPath, genQueryOut_t* attriArray, int* outDataMode, char** outChksum)
{
    ...
    if (inpObjPath == NULL || attriArray == NULL || outDataMode == NULL || outChksum == NULL) {
        return USER__NULL_INPUT_ERR;
    }

    if ((objPath = getSqlResultByInx(attriArray, COL_DATA_NAME)) == NULL) {
        rodsLog(LOG_NOTICE, "getAttriInAttriArray: getSqlResultByInx for COL_DATA_NAME failed");
        return UNMATCHED_KEY_OR_INDEX;
    }

    if ((dataMode = getSqlResultByInx(attriArray, COL_DATA_MODE)) == NULL) {
        rodsLog(LOG_NOTICE, "getAttriInAttriArray: getSqlResultByInx for COL_DATA_MODE failed");
        return UNMATCHED_KEY_OR_INDEX;
    }
...
int getPathStMode(const char* p)
{
    ...
    if (stat(p, &statbuf) == 0 && (statbuf.st_mode & S_IFREG)) {
...
int hasSymlinkInDir(const char* mydir)
{
    ...
    while ((myDirent = readdir(dirPtr)) != NULL) {
        ...
        snprintf(subfilePath, MAX_NAME_LEN, "%s/%s", mydir, myDirent->d_name);
        status = lstat(subfilePath, &statbuf);
        if (status != 0) {
            rodsLog(LOG_ERROR, "hasSymlinkIndir: stat error for %s, errno = %d", subfilePath, errno);
            continue;
        }
        if ((statbuf.st_mode & S_IFLNK) == S_IFLNK) {
            rodsLog(LOG_ERROR, "hasSymlinkIndir: %s is a symlink", subfilePath);
            closedir(dirPtr);
            return 1;
        }
...
int hasSymlinkInPartialPath(const char* myPath, int pos)
{
    ...
    if (status != 0) {
        rodsLog(LOG_ERROR, "hasSymlinkInPartialPath: stat error for %s, errno = %d", myPath, errno);
        return 0;
    }
    if ((statbuf.st_mode & S_IFLNK) == S_IFLNK) {
        rodsLog(LOG_ERROR, "hasSymlinkInPartialPath: %s is a symlink", myPath);
        return 1;
    }
    ...
    while ((curPtr = strchr(curPtr, '/')) != NULL) {
        ...
        if (status != 0) {
            rodsLog(LOG_ERROR, "hasSymlinkInPartialPath: stat error for %s, errno = %d", sub_path.c_str(), errno);
            return 0;
        }
        if ((statbuf.st_mode & S_IFLNK) == S_IFLNK) {
            rodsLog(LOG_ERROR, "hasSymlinkInPartialPath: %s is a symlink", sub_path.c_str());
            return 1;
        }
        curPtr++;
    }
...
static std::string stringify_addrinfo_hints(const struct addrinfo* _hints)
{
    ...
    if (!_hints) {
        ...
        stream << "ai_flags: [" << _hints->ai_flags << "] ai_family: [" << _hints->ai_family << "] ai_socktype: ["
               << _hints->ai_socktype << "] ai_protocol: [" << _hints->ai_protocol << "]";
        ret = stream.str();
    }
    return ret;
}
                           ...
                           struct addrinfo** _res)
{
    *_res = 0;
    ...
    for (int i = 0; i < max_retry; ++i) {
        ...
        if (ret_getaddrinfo == EAI_AGAIN || ret_getaddrinfo == EAI_NONAME ||
            ...
            if (ret_getaddrinfo == EAI_SYSTEM) {
                ...
                rodsLog(LOG_ERROR,
                        "getaddrinfo_with_retry: getaddrinfo non-recoverable system error [%d] [%s] [%d] [%s] [%s]",
                        ret_getaddrinfo,
                ...
                std::string hint_str = stringify_addrinfo_hints(_hints);
                rodsLog(LOG_ERROR,
                        "getaddrinfo_with_retry: getaddrinfo non-recoverable error [%d] [%s] [%s] [%s]",
                        ret_getaddrinfo,
                        gai_strerror(ret_getaddrinfo),
                        _node,
                        hint_str.c_str());
