...
int chksumUtil(rcComm_t* conn, rodsEnv* myRodsEnv, rodsArguments_t* myRodsArgs, rodsPathInp_t* rodsPathInp)
{
    ...
    for (int i = 0; i < rodsPathInp->numSrc; i++) {
        ...
        if (rodsPathInp->srcPath[i].objType == DATA_OBJ_T) {
            ...
            status =
                chksumCollUtil(conn, rodsPathInp->srcPath[i].outPath, myRodsEnv, myRodsArgs, &dataObjInp, &collInp);
        }
...
int initCondForChksum(rodsArguments_t* rodsArgs, dataObjInp_t* dataObjInp, collInp_t* collInp)
{
    ...
    if (rodsArgs->resource == True) {
        addKeyVal(&dataObjInp->condInput, RESC_NAME_KW, rodsArgs->resourceString);
    }
                   ...
                   collInp_t* collInp)
{
    ...
    while ((status = rclReadCollection(conn, &collHandle, &collEnt)) >= 0) {
        if (collEnt.objType == DATA_OBJ_T) {
            ...
            if (status < 0) {
                rodsLogError(
                    LOG_ERROR, status, "chksumCollUtil:chksumDataObjU failed for %s.stat = %d", srcChildPath, status);
