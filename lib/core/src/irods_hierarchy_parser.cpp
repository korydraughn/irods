...
namespace irods
{
    ...
    hierarchy_parser::hierarchy_parser(const hierarchy_parser& parser)
    {
    ...
    error hierarchy_parser::set_string(const std::string& _resc_hier)
    {
        if (_resc_hier.empty()) {
    ...
    error hierarchy_parser::str(std::string& _ret_string, const std::string& _term_resc) const
    {
        error result = SUCCESS();
        _ret_string.clear();
        bool first = true;
        bool done = false;
        for (resc_list_t::const_iterator itr = resc_list_.begin(); !done && itr != resc_list_.end(); ++itr) {
            if (first) {
                first = false;
            }
            else {
    ...
    error hierarchy_parser::add_child(const std::string& _resc)
    {
        error result = SUCCESS();
    ...
    error hierarchy_parser::first_resc(std::string& _ret_resc) const
    {
        error result = SUCCESS();
        if (resc_list_.size()) {
    ...
    error hierarchy_parser::last_resc(std::string& _ret_resc) const
    {
        error result = SUCCESS();
        if (resc_list_.size()) {
    ...
    error hierarchy_parser::next(const std::string& _current, std::string& _ret_resc) const
    {
        error result = SUCCESS();
        _ret_resc.clear();
        bool found = false;
        for (resc_list_t::const_iterator itr = resc_list_.begin(); !found && itr != resc_list_.end(); ++itr) {
            if (*itr == _current) {
                found = true;
    ...
    error hierarchy_parser::num_levels(int& levels) const
    {
        error result = SUCCESS();
    ...
    hierarchy_parser& hierarchy_parser::operator=(const hierarchy_parser& rhs)
    {
