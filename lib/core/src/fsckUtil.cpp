            ...
            const char* argument_for_SetGenQueryInpFromPhysicalPath)
{
    if (rodsPathInp->numSrc != 1) {
        rodsLog(LOG_ERROR,
                "fsckObj: gave %i input source path, "
                "should give one and only one",
                rodsPathInp->numSrc);
    ...
    if (lenInpPath >= MAX_PATH_ALLOWED) {
        rodsLog(LOG_ERROR, "Path %s is longer than %ju characters in fsckObj", inpPathO, (intmax_t) MAX_PATH_ALLOWED);
               ...
               const char* argument_for_SetGenQueryInpFromPhysicalPath)
{
    ...
    for (directory_iterator itr(srcDirPath); itr != end_itr; ++itr) {
        path cp = itr->path();
        snprintf(fullPath, MAX_PATH_ALLOWED, "%s", cp.c_str());
        if (is_symlink(cp)) {
            ...
            if (myRodsArgs->verifyChecksum == True) {
                if (strcmp(objChksum, "") != 0) {
                    ...
                    if (status == USER_CHKSUM_MISMATCH) {
                        ...
                        printf("ERROR chkObjConsistency: verifyChksumLocFile failed: status [%d] file [%s] objPath "
                               "[%s] objName [%s] objChksum [%s]\n",
                               status,
                               inpPath,
                               objPath,
                               objName,
                    ...
                    printf("WARNING: checksum not available for iRODS object [%s/%s], no checksum comparison possible "
                           "with local file [%s] .\n",
                           objPath,
                           objName,
                           inpPath);
