...
int phybunUtil(rcComm_t* conn, rodsArguments_t* myRodsArgs, rodsPathInp_t* rodsPathInp)
{
    ...
    for (int i = 0; i < rodsPathInp->numSrc; i++) {
        ...
        if (collPath->objType != COLL_OBJ_T) {
            rodsLogError(LOG_ERROR, status, "phybunUtil: input path %s is not a collection", collPath->outPath);
        ...
        if (status < 0 && status != CAT_NO_ROWS_FOUND) {
            rodsLogError(LOG_ERROR, status, "phybunUtil: opr error for %s, status = %d", collPath->outPath, status);
            savedStatus = status;
        }
    }
    return savedStatus;
...
int initCondForPhybunOpr(rodsArguments_t* rodsArgs, structFileExtAndRegInp_t* phyBundleCollInp)
{
    ...
    if (phyBundleCollInp == NULL) {
        rodsLog(LOG_ERROR, "initCondForPhybunOpr: NULL phyBundleCollInp input");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (rodsArgs->resource == True) {
        if (rodsArgs->resourceString == NULL) {
            rodsLog(LOG_ERROR, "initCondForPhybunOpr: NULL resourceString error");
            return USER__NULL_INPUT_ERR;
        }
        else {
            addKeyVal(&phyBundleCollInp->condInput, DEST_RESC_NAME_KW, rodsArgs->resourceString);
        }
    }
    else {
        rodsLog(LOG_ERROR, "initCondForPhybunOpr: A -Rresource must be input");
        return USER__NULL_INPUT_ERR;
    }

    if (rodsArgs->srcResc == True) {
        addKeyVal(&phyBundleCollInp->condInput, RESC_NAME_KW, rodsArgs->srcRescString);
    }
    ...
    if (rodsArgs->dataTypeString != NULL) {
        if (strcmp(rodsArgs->dataTypeString, "t") == 0 || strcmp(rodsArgs->dataTypeString, TAR_DT_STR) == 0 ||
            strcmp(rodsArgs->dataTypeString, "tar") == 0) {
            addKeyVal(&phyBundleCollInp->condInput, DATA_TYPE_KW, TAR_BUNDLE_DT_STR);
        }
        else if (strcmp(rodsArgs->dataTypeString, "g") == 0 || strcmp(rodsArgs->dataTypeString, GZIP_TAR_DT_STR) == 0 ||
                 strcmp(rodsArgs->dataTypeString, "gzip") == 0) {
            addKeyVal(&phyBundleCollInp->condInput, DATA_TYPE_KW, GZIP_TAR_BUNDLE_DT_STR);
        }
            ...
            addKeyVal(&phyBundleCollInp->condInput, DATA_TYPE_KW, BZIP2_TAR_BUNDLE_DT_STR);
        }
        else if (strcmp(rodsArgs->dataTypeString, "z") == 0 || strcmp(rodsArgs->dataTypeString, ZIP_DT_STR) == 0 ||
                 strcmp(rodsArgs->dataTypeString, "zip") == 0) {
            addKeyVal(&phyBundleCollInp->condInput, DATA_TYPE_KW, ZIP_BUNDLE_DT_STR);
        }
        else {
            addKeyVal(&phyBundleCollInp->condInput, DATA_TYPE_KW, rodsArgs->dataTypeString);
        }
    }
    // =-=-=-=-=-=-=
