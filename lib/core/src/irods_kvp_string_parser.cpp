...
namespace irods
{
    ...
    static error parse_token_into_kvp(const std::string& _token, kvp_map_t& _kvp, const std::string& _assoc)
    {
        ...
        if (token_vec.size() != 2) {
            ...
            msg << "token vector size != 2 during parsing of [" << _token << "]";
    ...
    error parse_kvp_string(const std::string& _string,
                           kvp_map_t& _kvp,
    ...
    /// @brief given a string, break the string along the delimiter and then
    ///        break the tokens along the assignment for key-value pairs
    error parse_escaped_kvp_string(const std::string& _string,
                                   kvp_map_t& _kvp,
                                   const std::string& _assoc,
                                   const std::string& _delim,
                                   const std::string& _escape)
    {
        ...
        if (_delim == _escape || _delim == _assoc || _assoc == _escape || boost::starts_with(_assoc, _delim) ||
            boost::starts_with(_delim, _assoc)) {
            return ERROR(SYS_BAD_INPUT,
                         "_delim, _assoc, and _escape must all be distinct in calls to parse_kvp_string");
        }
        if (_assoc.find(_delim) != std::string::npos || _assoc.find(_escape) != std::string::npos ||
            ...
            return ERROR(SYS_BAD_INPUT,
                         "_delim, _assoc, and _escape may not contain each other in calls to parse_kvp_string");
        }
        ...
        if (!key.str().empty() && !assoc_encountered) {
            return ERROR(SYS_BAD_INPUT, "no unescaped _assoc token encountered before end of string when parsing kvp");
        }
