...
extern "C"
{
    int osauthVerifyResponse(char* challenge, char* username, char* response)
    {
        ...
        if ((status = osauthGetKey(&keybuf, &key_len))) {
            rodsLogError(LOG_ERROR, status, "%s: error retrieving key.", fname);
            return status;
        }

        /* generate the authenticator */
        status = osauthGenerateAuthenticator(username, uid, challenge, keybuf, key_len, authenticator, 16);
        if (status) {
            rodsLog(LOG_ERROR, "%s: could not generate the authenticator", fname);
            free(keybuf);
            return status;
        }
        free(keybuf);
        ...
        for (i = 0; i < RESPONSE_LEN; i++) {
            if (response[i] != md5digest[i]) {
                rodsLog(LOG_ERROR, "%s: calculated digest doesn't match client response", fname);
                                    ...
                                    int authenticator_len)
    {
        ...
        if (authenticator == NULL || authenticator_len < 16) {
        ...
        if (buffer == NULL) {
            rodsLog(LOG_ERROR, "%s: could not allocate memory buffer. errno = %d", fname, errno);
    ...
    int osauthGetKey(char** key, int* key_len)
    {
        ...
        if (key_fd < 0) {
            rodsLog(LOG_ERROR, "%s: couldn't open %s for reading. errno = %d", fname, keyfile, errno);
        ...
        if (keybuf == NULL) {
            rodsLog(LOG_ERROR, "%s: could not allocate memory for key buffer. errno = %d", fname, errno);
            close(key_fd);
            return SYS_MALLOC_ERR;
        }
        ...
        close(key_fd);
        if (nb < 0) {
            rodsLog(LOG_ERROR, "%s: couldn't read key from %s. errno = %d", fname, keyfile, errno);
            free(keybuf);
    ...
    int osauthGetAuth(char* challenge, char* username, char* authenticator, int authenticator_buflen)
    {
        ...
        if (pipe(pipe1) < 0) {
            rodsLog(LOG_ERROR, "%s: pipe1 create failed. errno = %d", fname, errno);
            return SYS_PIPE_ERROR - errno;
        }
        if (pipe(pipe2) < 0) {
            rodsLog(LOG_ERROR, "%s: pipe2 create failed. errno = %d", fname, errno);
            close(pipe1[0]);
            close(pipe1[1]);
            return SYS_PIPE_ERROR - errno;
        }
        ...
        int status =
            posix_spawn(&childPid, os_auth_command_path_string.c_str(), &file_actions, &attr, &argv[0], &envp[0]);
        posix_spawnattr_destroy(&attr);
        posix_spawn_file_actions_destroy(&file_actions);
        if (status != 0) {
            rodsLog(LOG_ERROR, "%s: posix_spawn failed. errno = %d", fname, errno);
            close(pipe1[0]);
            close(pipe1[1]);
            close(pipe2[0]);
            close(pipe2[1]);
        ...
        if (nb < 0) {
            rodsLog(LOG_ERROR,
                    ...
                    fname,
                    os_auth_command_path.string().c_str(),
                    errno);
            close(child_stdin);
            close(child_stdout);
            return SYS_PIPE_ERROR - errno;
        ...
        if (buflen < 0) {
            rodsLog(LOG_ERROR,
                    "%s: error reading from %s. errno = %d",
                    fname,
                    os_auth_command_path.string().c_str(),
                    errno);
            close(child_stdin);
        ...
        if (waitpid(childPid, &childStatus, 0) < 0) {
            rodsLog(LOG_ERROR, "%s: waitpid error. errno = %d", fname, errno);
            return EXEC_CMD_ERROR;
        }

        if (WIFEXITED(childStatus)) {
            if (WEXITSTATUS(childStatus)) {
                rodsLog(LOG_ERROR, "%s: command failed: %s", fname, buffer);
                return EXEC_CMD_ERROR;
            }
        }
        else {
            rodsLog(LOG_ERROR, "%s: some error running %s", fname, os_auth_command_path.string().c_str());
        }

        /* authenticator is in buffer now */
        if (buflen > authenticator_buflen) {
            rodsLog(LOG_ERROR, "%s: not enough space in return buffer for authenticator", fname);
    ...
    int osauthGetUid(char* username)
    {
        ...
        if (pwent == NULL) {
            if (errno) {
                rodsLog(LOG_ERROR, "%s: error calling getpwnam for %s. errno = %d", fname, username, errno);
                return -1;
            }
            else {
                rodsLog(LOG_ERROR, "%s: no such user %s", fname, username);
                return -1;
            }
        }
        return pwent->pw_uid;
    ...
    int osauthGetUsername(char* username, int username_len)
    {
        ...
        if (pwent == NULL) {
            if (errno) {
                rodsLog(LOG_ERROR, "%s: error calling getpwuid for uid %d. errno = %d", fname, uid, errno);
                return -1;
            }
            else {
                rodsLog(LOG_ERROR, "%s: no user with uid %d", fname, uid);
                return -1;
            }
        }
        if ((unsigned int) username_len <= strlen(pwent->pw_name)) {
            rodsLog(LOG_ERROR,
                    "%s: username input buffer too small (%d <= %d)",
                    fname,
                    username_len,
