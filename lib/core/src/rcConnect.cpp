                    ...
                    rErrMsg_t* errMsg)
{
    ...
    conn = _rcConnect(rodsHost, rodsPort, userName, rodsZone, NULL, NULL, errMsg, 0, reconnFlag);
                     ...
                     int reconnFlag)
{
    ...
    status =
        setUserInfo(proxyUserName, proxyRodsZone, clientUserName, clientRodsZone, &conn->clientUser, &conn->proxyUser);

    if (status < 0) {
        if (errMsg != NULL) {
            errMsg->status = status;
            snprintf(errMsg->msg, ERR_MSG_LEN - 1, "_rcConnect: setUserInfo failed\n");
        }
    ...
    if (status < 0) {
        if (errMsg != NULL) {
            rodsLogError(LOG_ERROR, status, "_rcConnect: setRhostInfo error, IRODS_HOST is probably not set correctly");
            errMsg->status = status;
            snprintf(errMsg->msg, ERR_MSG_LEN - 1, "_rcConnect: setRhostInfo failed\n");
        }
        if (conn->thread_ctx != NULL) {
            delete conn->thread_ctx->reconnThr;
            delete conn->thread_ctx->lock;
    ...
    if (status < 0) {
        if (getIrodsErrno(status) == SYS_SOCK_READ_TIMEDOUT) {
            /* timed out. try again */
            rodsLog(LOG_ERROR, "_rcConnect: connectToRhost timed out, retrying");
            status = connectToRhost(conn, connectCnt, reconnFlag);
        }
    }
    ...
    if (status < 0) {
        ...
        if (errMsg != NULL) {
            errMsg->status = status;
            snprintf(errMsg->msg, ERR_MSG_LEN - 1, "_rcConnect: connectToRhost failed\n");
        }
        if (conn->thread_ctx != NULL) {
            delete conn->thread_ctx->reconnThr;
            delete conn->thread_ctx->lock;
...
#ifndef windows_platform
    if (reconnFlag == RECONN_TIMEOUT && conn->svrVersion != NULL && conn->svrVersion->reconnPort > 0) {
        if (isLoopbackAddress(conn->svrVersion->reconnAddr) || strcmp(conn->svrVersion->reconnAddr, "0.0.0.0") == 0 ||
...
int rcDisconnect(rcComm_t* _conn)
{
    ...
    ret = sendRodsMsg(net_obj, RODS_DISCONNECT_T, NULL, NULL, NULL, 0, _conn->irodsProt);
#ifdef RODS_CLERVER
    if (!ret.ok()) {
        irods::log(PASS(ret));
    }
    ...
    if (_conn->thread_ctx->reconnThr) {
        try {
            ...
            _conn->exit_flg = true;
            bool val = _conn->thread_ctx->reconnThr->try_join_for(boost::chrono::seconds(2));
            if (true != val) {
...
rcComm_t* rcConnectXmsg(rodsEnv* myRodsEnv, rErrMsg_t* errMsg)
{
    ...
    conn = rcConnect(myRodsEnv->xmsgHost, myRodsEnv->xmsgPort, myRodsEnv->rodsUserName, myRodsEnv->rodsZone, 0, errMsg);
...
void cliReconnManager(rcComm_t* conn)
{
    ...
    if (conn == NULL || conn->svrVersion == NULL || conn->svrVersion->reconnPort <= 0) {
        return;
    }
    ...
    while (!conn->exit_flg) { /* JMC */
        ...
        while (conn->clientState != PROCESSING_STATE) {
            ...
            rodsLog(LOG_DEBUG, "cliReconnManager: clientState = %d", conn->clientState);
            printf("cliReconnManager: clientState = %d\n", conn->clientState);
            fflush(stdout);
            conn->thread_ctx->cond->wait(boost_lock);
        }
        rodsLog(LOG_DEBUG, "cliReconnManager: Reconnecting clientState = %d", conn->clientState);
        ...
        if (status != 0) {
            rodsLog(LOG_ERROR, "cliReconnManager: unknown hostname: %s", conn->svrVersion->reconnAddr);
            return;
        }
        ...
        conn->reconnectedSock = connectToRhostWithRaddr(&remoteAddr, conn->windowSize, 0);
        ...
        if (conn->clientState == PROCESSING_STATE) {
            ...
            rodsLog(LOG_DEBUG, "cliReconnManager: Not calling svrSwitchConnect,  clientState = %d", conn->clientState);
        }
        conn->thread_ctx->cond->notify_all();
        boost_lock.unlock();
    }
...
int cliChkReconnAtSendStart(rcComm_t* conn)
{
    ...
    if (conn->svrVersion != NULL && conn->svrVersion->reconnPort > 0) {
        ...
        if (conn->reconnThrState == CONN_WAIT_STATE) {
            rodsLog(LOG_DEBUG, "cliChkReconnAtSendStart:ThrState=CONN_WAIT_STATE,clientState=%d", conn->clientState);
...
int cliChkReconnAtReadEnd(rcComm_t* conn)
{
    if (conn->svrVersion != NULL && conn->svrVersion->reconnPort > 0) {
        ...
        conn->clientState = PROCESSING_STATE;
        if (conn->reconnThrState == CONN_WAIT_STATE) {
            rodsLog(LOG_DEBUG, "cliChkReconnAtReadEnd:ThrState=CONN_WAIT_STATE, clientState=%d", conn->clientState);
