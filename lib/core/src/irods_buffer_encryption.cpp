...
namespace irods
{
    ...
    std::string buffer_crypt::gen_hash(unsigned char* _buf, int _sz)
    {
        MD5_CTX ctx;
        MD5_Init(&ctx);
    ...
    buffer_crypt::buffer_crypt(int _key_sz, int _salt_sz, int _num_rnds, const char* _algo)
        : key_size_(_key_sz)
        , salt_size_(_salt_sz)
    ...
    irods::error buffer_crypt::generate_key(array_t& _out_key, int _key_size)
    {
        // =-=-=-=-=-=-=-
    ...
    irods::error buffer_crypt::hex_encode(const array_t& _in_buf, std::string& _out_str)
    {
        std::stringstream ss;
    ...
    irods::error buffer_crypt::initialization_vector(array_t& _out_iv)
    {
        // =-=-=-=-=-=-=-
        // generate a random initialization vector
        unsigned char* iv = new unsigned char[key_size_];
        int rnd_err = RAND_bytes(iv, key_size_);
        if (1 != rnd_err) {
            delete[] iv;
            char err[256];
        ...
        _out_iv.assign(&iv[0], &iv[key_size_]);
    ...
    // =-=-=-=-=-=-=-
    // public - encryptor
    irods::error buffer_crypt::encrypt(const array_t& _key,
                                       const array_t& _iv,
                                       ...
                                       array_t& _out_buf)
    {
        ...
        if (!algo) {
            rodsLog(LOG_NOTICE, "buffer_crypt::encrypt - algorithm not supported [%s]", algorithm_.c_str());
            // default to aes 256 cbc
            algo = EVP_aes_256_cbc();
        }

        int ret = EVP_EncryptInit_ex(context, algo, NULL, &_key[0], &_iv[0]);
        if (0 == ret) {
            char err[256];
            ERR_error_string_n(ERR_get_error(), err, 256);
        ...
        ret = EVP_EncryptUpdate(context, cipher_text, &cipher_len, &_in_buf[0], _in_buf.size());
        if (0 == ret) {
            delete[] cipher_text;
            char err[256];
            ERR_error_string_n(ERR_get_error(), err, 256);
        ...
        int final_len = 0;
        ret = EVP_EncryptFinal_ex(context, cipher_text + cipher_len, &final_len);
        if (0 == ret) {
            delete[] cipher_text;
            char err[256];
            ERR_error_string_n(ERR_get_error(), err, 256);
        ...
        // =-=-=-=-=-=-=-
        // copy the iv to the out variable
        _out_buf.assign(&cipher_text[0], &cipher_text[cipher_len + final_len]);
    ...
    // =-=-=-=-=-=-=-
    // public - decryptor
    irods::error buffer_crypt::decrypt(const array_t& _key,
                                       const array_t& _iv,
                                       const array_t& _in_buf,
                                       array_t& _out_buf)
    {
        ...
        auto* algo = EVP_get_cipherbyname(algorithm_.c_str());
        if (!algo) {
            rodsLog(LOG_NOTICE, "buffer_crypt::decrypt - algorithm not supported [%s]", algorithm_.c_str());
            // default to aes 256 cbc
            algo = EVP_aes_256_cbc();
        }

        int ret = EVP_DecryptInit_ex(context, algo, NULL, &_key[0], &_iv[0]);
        if (0 == ret) {
            char err[256];
            ERR_error_string_n(ERR_get_error(), err, 256);
        ...
        ret = EVP_DecryptUpdate(context, plain_text.get(), &plain_len, &_in_buf[0], _in_buf.size());
        if (0 == ret) {
            char err[256];
            ERR_error_string_n(ERR_get_error(), err, 256);
        ...
        int final_len = 0;
        ret = EVP_DecryptFinal_ex(context, plain_text.get() + plain_len, &final_len);
        if (0 == ret) {
            char err[256];
            ERR_error_string_n(ERR_get_error(), err, 256);
        ...
        // =-=-=-=-=-=-=-
        // copy the iv to the out variable
        _out_buf.assign(plain_text.get(), plain_text.get() + plain_len + final_len);
