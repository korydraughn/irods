...
namespace irods
{
    ...
    api_entry_table::api_entry_table(apidef_t _defs[], size_t _num)
    {
        // =-=-=-=-=-=-=-
    ...
    error init_api_table(api_entry_table& _api_tbl, pack_entry_table& _pack_tbl, bool _cli_flg)
    {
        // =-=-=-=-=-=-=-
        ...
        if (boost::filesystem::exists(so_dir)) {
            for (boost::filesystem::directory_iterator it(so_dir); it != boost::filesystem::directory_iterator();
                 ++it) {
                // =-=-=-=-=-=-=-
                ...
                api_entry* entry = 0;
                error ret = load_plugin<api_entry>(entry, name, PLUGIN_TYPE_API, "api_instance", "api_context");
                if (ret.ok() && entry) {
                    // =-=-=-=-=-=-=-
                    //
                    rodsLog(LOG_DEBUG,
