...
namespace
{
    std::string to_string(const struct in_addr& address)
    {
    ...
    std::string to_string(const struct in6_addr& address)
    {
    ...
    bool is_loopback(const struct in_addr& v4_addr)
    {
    ...
    bool is_loopback(const struct in6_addr& v6_addr)
    {
        ...
        const bool is_v4_mapped_loopback = addr_bytes[0] == 0 && addr_bytes[1] == 0 && addr_bytes[2] == 0 &&
                                           addr_bytes[3] == 0 && addr_bytes[4] == 0 && addr_bytes[5] == 0 &&
                                           addr_bytes[6] == 0 && addr_bytes[7] == 0 && addr_bytes[8] == 0 &&
                                           addr_bytes[9] == 0 && addr_bytes[10] == 255 && addr_bytes[11] == 255 &&
                                           addr_bytes[12] == 127;
        return is_v6_loopback || is_v4_mapped_loopback;
    }

    std::pair<std::vector<struct in_addr>, std::vector<struct in6_addr>> get_local_ipv4_and_ipv6_addresses()
    {
    ...
    void print_local_ipv4_and_ipv6_addresses(void)
    {
    ...
    int getaddrinfo_with_retry(const char* node,
                               const char* service,
    ...
    std::pair<std::vector<struct in_addr>, std::vector<struct in6_addr>> get_remote_ipv4_and_ipv6_addresses(
        const char* hostname)
    {
        std::pair<std::vector<struct in_addr>, std::vector<struct in6_addr>> ret;
    ...
    void print_remote_ipv4_and_ipv6_addresses(const char* hostname)
    {
