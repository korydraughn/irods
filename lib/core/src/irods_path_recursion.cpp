...
// Checks for existence of path as a symlink or a directory.
// Will throw irods::exception if boost file system errors occur in the process.
bool irods::is_path_valid_for_recursion(rodsArguments_t const* const rodsArgs, const char* myPath)
{
    namespace fs = boost::filesystem;
...
int irods::scan_all_source_directories_for_loops(irods::recursion_map_t& pathmap,
                                    ...
                                    rodsPathInp_t const* const rodsPathInp)
{
    ...
    for (int i = 0; i < rodsPathInp->numSrc; i++) {
        if (rodsPathInp->srcPath[i].objType == LOCAL_DIR_T && rodsPathInp->targPath[i].objType == COLL_OBJ_T) {
...
#define DISPLAY_PREFLIGHT true
    ...
    if (dirvec.size() > 0) {
        ...
        if (DISPLAY_PREFLIGHT) {
            std::cout << "Running recursive pre-scan... " << std::flush;
        }

        if ((status = irods::scan_all_source_directories_for_loops(
                 pathmap, dirvec, (rodsArgs->link == True ? true : false))) < 0) {
            if (DISPLAY_PREFLIGHT) {
...
std::string irods::scantime::get_duration_string() const
{
