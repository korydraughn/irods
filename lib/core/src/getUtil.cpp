...
int getUtil(rcComm_t** myConn, rodsEnv* myRodsEnv, rodsArguments_t* myRodsArgs, rodsPathInp_t* rodsPathInp)
{
    ...
    if (rodsPathInp->resolved == False) {
        ...
        if (status < 0) {
            rodsLogError(LOG_ERROR, status, "getUtil: resolveRodsTarget");
            return status;
        }
    ...
    if (conn->fileRestart.flags == FILE_RESTART_ON) {
        ...
        if (status >= 0) {
            ...
            if (status >= 0) {
                ...
                rstrcpy(conn->fileRestart.info.fileName, info->fileName, MAX_NAME_LEN);
    ...
    for (i = 0; i < rodsPathInp->numSrc; i++) {
        ...
        if (rodsPathInp->srcPath[i].rodsObjStat != NULL && rodsPathInp->srcPath[i].rodsObjStat->specColl != NULL) {
            dataObjOprInp.specColl = rodsPathInp->srcPath[i].rodsObjStat->specColl;
        }
        else {
        ...
        if (targPath->objType == LOCAL_FILE_T) {
            ...
            /* should not be here */
            rodsLog(LOG_ERROR, "getUtil: invalid get dest objType %d for %s", targPath->objType, targPath->outPath);
            return USER_INPUT_PATH_ERR;
        }
        /* XXXX may need to return a global status */
        if (status < 0) {
            rodsLogError(LOG_ERROR, status, "getUtil: get error for %s", targPath->outPath);
            savedStatus = status;
            break;
        }
    }
    ...
    if (status < 0 && myRodsArgs->retries == True) {
        ...
        while (myRodsArgs->retriesValue > 0) {
            ...
            if (status < 0) {
                rodsLogError(LOG_ERROR, status, "getUtil: rcReconnect error for %s", targPath->outPath);
                return status;
            }
            ...
            if (status >= 0) {
                ...
                rodsLogError(LOG_ERROR, status, "getUtil: retry getUtil error");
            }
                   ...
                   dataObjInp_t* dataObjOprInp)
{
    ...
    if (srcPath == NULL || targPath == NULL) {
        rodsLog(LOG_ERROR, "getDataObjUtil: NULL srcPath or targPath input");
        return USER__NULL_INPUT_ERR;
    }

    if (conn->fileRestart.info.status == FILE_RESTARTED && strcmp(conn->fileRestart.info.objPath, srcPath) == 0) {
        /* it was restarted */
...
int initCondForGet(rcComm_t* conn, rodsArguments_t* rodsArgs, dataObjInp_t* dataObjOprInp, rodsRestart_t* rodsRestart)
{
    ...
    if (dataObjOprInp == NULL) {
        rodsLog(LOG_ERROR, "initCondForGet: NULL dataObjOprInp input");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (rodsArgs->resource == True) {
        if (rodsArgs->resourceString == NULL) {
            ...
            addKeyVal(&dataObjOprInp->condInput, RESC_NAME_KW, rodsArgs->resourceString);
        }
    }
    ...
    if (rodsArgs->ticket == True) {
        if (rodsArgs->ticketString == NULL) {
            ...
            addKeyVal(&dataObjOprInp->condInput, TICKET_KW, rodsArgs->ticketString);
        }
    }
    ...
    if (rodsArgs->restart == True) {
        ...
        if (status < 0) {
            rodsLogError(LOG_ERROR, status, "initCondForPut: openRestartFile of %s errno", rodsArgs->restartFileString);
            return status;
        }
    }

    if (rodsArgs->retries == True && rodsArgs->restart == False && rodsArgs->lfrestart == False) {
        rodsLog(LOG_ERROR, "initCondForGet: --retries must be used with -X option");
...
int getCollUtil(rcComm_t** myConn,
                char* srcColl,
                char* targDir,
                rodsEnv* myRodsEnv,
                rodsArguments_t* rodsArgs,
                ...
                rodsRestart_t* rodsRestart)
{
    ...
    if (srcColl == NULL || targDir == NULL) {
        rodsLog(LOG_ERROR, "getCollUtil: NULL srcColl or targDir input");
        return USER__NULL_INPUT_ERR;
    }

    if (rodsArgs->recursive != True) {
        rodsLog(LOG_ERROR, "getCollUtil: -r option must be used for getting %s collection", targDir);
        return USER_INPUT_OPTION_ERR;
    }
    ...
    while ((status = rclReadCollection(conn, &collHandle, &collEnt)) >= 0) {
        if (collEnt.objType == DATA_OBJ_T) {
            ...
            snprintf(targChildPath, MAX_NAME_LEN, "%s/%s", targDir, collEnt.dataName);
            snprintf(srcChildPath, MAX_NAME_LEN, "%s/%s", collEnt.collName, collEnt.dataName);

            int status = chkStateForResume(
                conn, rodsRestart, targChildPath, rodsArgs, LOCAL_FILE_T, &dataObjOprInp->condInput, 1);
            ...
            status =
                getDataObjUtil(conn, srcChildPath, targChildPath, mySize, collEnt.dataMode, rodsArgs, dataObjOprInp);
            if (status < 0) {
                rodsLogError(
            ...
            int status = getCollUtil(
                myConn, collEnt.collName, targChildPath, myRodsEnv, rodsArgs, &childDataObjInp, rodsRestart);
            if (status < 0 && status != CAT_NO_ROWS_FOUND) {
                rodsLogError(
                    LOG_ERROR, status, "getCollUtil: getCollUtil failed for %s. status = %d", collEnt.collName, status);
                savedStatus = status;
                if (rodsRestart->fd > 0) {
                    break;
                }
