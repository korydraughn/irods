...
int scanObj(rcComm_t* conn, rodsArguments_t* myRodsArgs, rodsPathInp_t* rodsPathInp, const char* hostname)
{
    ...
    if (rodsPathInp->srcPath[0].objType == LOCAL_FILE_T || rodsPathInp->srcPath[0].objType == LOCAL_DIR_T) {
        path p(inpPathO);
        if (!exists(p)) {
    ...
    else if (rodsPathInp->srcPath[0].objType == UNKNOWN_OBJ_T || rodsPathInp->srcPath[0].objType == COLL_OBJ_T) {
...
int scanObjDir(rcComm_t* conn, rodsArguments_t* myRodsArgs, const char* inpPath, const char* hostname)
{
    ...
    for (directory_iterator itr(srcDirPath); itr != end_itr; ++itr) {
        path cp = itr->path();
        snprintf(fullPath, LONG_NAME_LEN, "%s", cp.c_str());
        if (is_symlink(cp)) {
            /* don't do anything if it is symlink */
            continue;
        }
...
int scanObjCol(rcComm_t* conn, rodsArguments_t* myRodsArgs, const char* inpPath)
{
    ...
    while ((0 == status || getIrodsErrno(status) == UNIX_FILE_STAT_ERR) && genQueryOut2->continueInx > 0) {
...
int statPhysFile(rcComm_t* conn, genQueryOut_t* genQueryOut2)
{
    ...
    for (int i = 0; i < genQueryOut2->rowCnt; i++) {
        ...
        if (dataPathStruct == NULL || dataSizeStruct == NULL || locStruct == NULL || zoneStruct == NULL ||
            dataNameStruct == NULL || collNameStruct == NULL || rescIDStruct == NULL) {
