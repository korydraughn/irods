...
namespace
{
    void init_from_server_properties(rodsEnv& _env)
    {
        ...
        std::strcpy(_env.rodsClientServerNegotiation, REQ_SVR_NEG);
        ...
        const auto copy_int = [&config](const char* const _k, int& _v) {
            _v = config.at(_k).get<int>();
        };
        ...
        _env.irodsDefaultNumberTransferThreads = advanced_settings.at(irods::KW_CFG_DEF_NUMBER_TRANSFER_THREADS).get<int>();
