...
extern "C"
{
    ...
    int getRodsEnvFromFile(rodsEnv* _env)
    {
        ...
        // default auth scheme
        snprintf(_env->rodsAuthScheme, sizeof(_env->rodsAuthScheme), "native");

        capture_string_property(irods::CFG_IRODS_SESSION_ENVIRONMENT_FILE_KW, configFileName);

        capture_string_property(irods::CFG_IRODS_USER_NAME_KW, _env->rodsUserName);

        capture_string_property(irods::CFG_IRODS_HOST_KW, _env->rodsHost);

        capture_string_property(irods::CFG_IRODS_XMSG_HOST_KW, _env->xmsgHost);

        capture_string_property(irods::CFG_IRODS_HOME_KW, _env->rodsHome);

        capture_string_property(irods::CFG_IRODS_CWD_KW, _env->rodsCwd);

        capture_string_property(irods::CFG_IRODS_AUTHENTICATION_SCHEME_KW, _env->rodsAuthScheme);

        capture_integer_property(irods::CFG_IRODS_PORT_KW, _env->rodsPort);

        capture_integer_property(irods::CFG_IRODS_XMSG_PORT_KW, _env->xmsgPort);

        capture_string_property(irods::CFG_IRODS_DEFAULT_RESOURCE_KW, _env->rodsDefResource);

        capture_string_property(irods::CFG_IRODS_ZONE_KW, _env->rodsZone);

        capture_string_property(irods::CFG_IRODS_CLIENT_SERVER_POLICY_KW, _env->rodsClientServerPolicy);

        capture_string_property(irods::CFG_IRODS_CLIENT_SERVER_NEGOTIATION_KW, _env->rodsClientServerNegotiation);

        capture_integer_property(irods::CFG_IRODS_ENCRYPTION_KEY_SIZE_KW, _env->rodsEncryptionKeySize);

        capture_integer_property(irods::CFG_IRODS_ENCRYPTION_SALT_SIZE_KW, _env->rodsEncryptionSaltSize);

        capture_integer_property(irods::CFG_IRODS_ENCRYPTION_NUM_HASH_ROUNDS_KW, _env->rodsEncryptionNumHashRounds);

        capture_string_property(irods::CFG_IRODS_ENCRYPTION_ALGORITHM_KW, _env->rodsEncryptionAlgorithm);

        capture_string_property(irods::CFG_IRODS_DEFAULT_HASH_SCHEME_KW, _env->rodsDefaultHashScheme);

        capture_string_property(irods::CFG_IRODS_MATCH_HASH_POLICY_KW, _env->rodsMatchHashPolicy);

        capture_string_property(irods::CFG_IRODS_DEBUG_KW, _env->rodsDebug);

        _env->rodsLogLevel = 0;
        int status = capture_integer_property(irods::CFG_IRODS_LOG_LEVEL_KW, _env->rodsLogLevel);
        ...
        status = capture_string_property(irods::CFG_IRODS_AUTHENTICATION_FILE_KW, _env->rodsAuthFile);
        if (status == 0) {
            rstrcpy(authFileName, _env->rodsAuthFile, LONG_NAME_LEN - 1);
        }

        // legacy ssl environment variables
        capture_string_property(irods::CFG_IRODS_SSL_CA_CERTIFICATE_PATH, _env->irodsSSLCACertificatePath);

        capture_string_property(irods::CFG_IRODS_SSL_CA_CERTIFICATE_FILE, _env->irodsSSLCACertificateFile);

        capture_string_property(irods::CFG_IRODS_SSL_VERIFY_SERVER, _env->irodsSSLVerifyServer);

        capture_string_property(irods::CFG_IRODS_SSL_CERTIFICATE_CHAIN_FILE, _env->irodsSSLCertificateChainFile);

        capture_string_property(irods::CFG_IRODS_SSL_CERTIFICATE_KEY_FILE, _env->irodsSSLCertificateKeyFile);

        capture_string_property(irods::CFG_IRODS_SSL_DH_PARAMS_FILE, _env->irodsSSLDHParamsFile);

        // control plane variables
        capture_string_property(irods::CFG_IRODS_SERVER_CONTROL_PLANE_KEY, _env->irodsCtrlPlaneKey);

        capture_integer_property(irods::CFG_IRODS_SERVER_CONTROL_PLANE_ENCRYPTION_NUM_HASH_ROUNDS_KW,
                                 _env->irodsCtrlPlaneEncryptionNumHashRounds);

        capture_string_property(irods::CFG_IRODS_SERVER_CONTROL_PLANE_ENCRYPTION_ALGORITHM_KW,
                                _env->irodsCtrlPlaneEncryptionAlgorithm);

        capture_integer_property(irods::CFG_IRODS_SERVER_CONTROL_PLANE_PORT, _env->irodsCtrlPlanePort);

        capture_integer_property(irods::CFG_IRODS_MAX_SIZE_FOR_SINGLE_BUFFER, _env->irodsMaxSizeForSingleBuffer);

        capture_integer_property(irods::CFG_IRODS_DEF_NUMBER_TRANSFER_THREADS, _env->irodsDefaultNumberTransferThreads);

        capture_integer_property(irods::CFG_IRODS_TRANS_BUFFER_SIZE_FOR_PARA_TRANS,
                                 _env->irodsTransBufferSizeForParaTrans);

        capture_string_property(irods::CFG_IRODS_PLUGINS_HOME_KW, _env->irodsPluginHome);
    ...
    {
        char* env = getenv(irods::to_env(_key).c_str());
        if (env) {
            strncpy(_val, env, strlen(env) + 1);
            rodsLog(LOG_DEBUG, "captured env [%s]-[%s]", _key.c_str(), _val);
        }
    ...
    static void capture_integer_env_var(const std::string& _key, int& _val)
    {
        ...
        if (env) {
            _val = atoi(env);
            rodsLog(LOG_DEBUG, "captured env [%s]-[%d]", _key.c_str(), _val);
        }
    ...
    {
        if (!_env) {
            rodsLog(LOG_ERROR, "get_legacy_ssl_variables - null env pointer");
            return SYS_INVALID_INPUT_PARAM;
        }
        ...
        if (val) {
            snprintf(_env->irodsSSLCACertificatePath, sizeof(_env->irodsSSLCACertificatePath), "%s", val);
        }

        val = getenv("irodsSSLCACertificateFile");
        if (val) {
            snprintf(_env->irodsSSLCACertificateFile, sizeof(_env->irodsSSLCACertificateFile), "%s", val);
        }

        val = getenv("irodsSSLVerifyServer");
        if (val) {
            snprintf(_env->irodsSSLVerifyServer, sizeof(_env->irodsSSLVerifyServer), "%s", val);
        }

        val = getenv("irodsSSLCertificateChainFile");
        if (val) {
            snprintf(_env->irodsSSLCertificateChainFile, sizeof(_env->irodsSSLCertificateChainFile), "%s", val);
        }

        val = getenv("irodsSSLCertificateKeyFile");
        if (val) {
            snprintf(_env->irodsSSLCertificateKeyFile, sizeof(_env->irodsSSLCertificateKeyFile), "%s", val);
        }

        val = getenv("irodsSSLDHParamsFile");
        if (val) {
            snprintf(_env->irodsSSLDHParamsFile, sizeof(_env->irodsSSLDHParamsFile), "%s", val);
        }
    ...
    int getRodsEnvFromEnv(rodsEnv* _env)
    {
        ...
        capture_string_env_var(env_var, _env->rodsUserName);

        env_var = irods::CFG_IRODS_HOST_KW;
        capture_string_env_var(env_var, _env->rodsHost);

        env_var = irods::CFG_IRODS_XMSG_HOST_KW;
        capture_string_env_var(env_var, _env->xmsgHost);

        env_var = irods::CFG_IRODS_PORT_KW;
        capture_integer_env_var(env_var, _env->rodsPort);

        env_var = irods::CFG_IRODS_XMSG_PORT_KW;
        capture_integer_env_var(env_var, _env->xmsgPort);

        env_var = irods::CFG_IRODS_HOME_KW;
        capture_string_env_var(env_var, _env->rodsHome);

        env_var = irods::CFG_IRODS_CWD_KW;
        capture_string_env_var(env_var, _env->rodsCwd);

        env_var = irods::CFG_IRODS_AUTHENTICATION_SCHEME_KW;
        capture_string_env_var(env_var, _env->rodsAuthScheme);

        env_var = irods::CFG_IRODS_DEFAULT_RESOURCE_KW;
        capture_string_env_var(env_var, _env->rodsDefResource);

        env_var = irods::CFG_IRODS_ZONE_KW;
        capture_string_env_var(env_var, _env->rodsZone);

        env_var = irods::CFG_IRODS_CLIENT_SERVER_POLICY_KW;
        capture_string_env_var(env_var, _env->rodsClientServerPolicy);

        env_var = irods::CFG_IRODS_CLIENT_SERVER_NEGOTIATION_KW;
        capture_string_env_var(env_var, _env->rodsClientServerNegotiation);

        env_var = irods::CFG_IRODS_ENCRYPTION_KEY_SIZE_KW;
        capture_integer_env_var(env_var, _env->rodsEncryptionKeySize);

        env_var = irods::CFG_IRODS_ENCRYPTION_SALT_SIZE_KW;
        capture_integer_env_var(env_var, _env->rodsEncryptionSaltSize);

        env_var = irods::CFG_IRODS_ENCRYPTION_NUM_HASH_ROUNDS_KW;
        capture_integer_env_var(env_var, _env->rodsEncryptionNumHashRounds);

        env_var = irods::CFG_IRODS_ENCRYPTION_ALGORITHM_KW;
        capture_string_env_var(env_var, _env->rodsEncryptionAlgorithm);

        env_var = irods::CFG_IRODS_DEFAULT_HASH_SCHEME_KW;
        capture_string_env_var(env_var, _env->rodsDefaultHashScheme);

        env_var = irods::CFG_IRODS_MATCH_HASH_POLICY_KW;
        capture_string_env_var(env_var, _env->rodsMatchHashPolicy);

        _env->rodsLogLevel = 0;
        env_var = irods::CFG_IRODS_LOG_LEVEL_KW;
        capture_integer_env_var(env_var, _env->rodsLogLevel);
        ...
        capture_string_env_var(env_var, _env->rodsAuthFile);
        if (strlen(_env->rodsAuthFile) > 0) {
            rstrcpy(authFileName, _env->rodsAuthFile, LONG_NAME_LEN);
        }

        env_var = irods::CFG_IRODS_DEBUG_KW;
        capture_string_env_var(env_var, _env->rodsDebug);

        // legacy ssl environment variables
        env_var = irods::CFG_IRODS_SSL_CA_CERTIFICATE_PATH;
        capture_string_env_var(env_var, _env->irodsSSLCACertificatePath);
        env_var = irods::CFG_IRODS_SSL_CA_CERTIFICATE_FILE;
        capture_string_env_var(env_var, _env->irodsSSLCACertificateFile);
        env_var = irods::CFG_IRODS_SSL_VERIFY_SERVER;
        capture_string_env_var(env_var, _env->irodsSSLVerifyServer);
        env_var = irods::CFG_IRODS_SSL_VERIFY_SERVER;
        capture_string_env_var(env_var, _env->irodsSSLVerifyServer);
        env_var = irods::CFG_IRODS_SSL_CERTIFICATE_CHAIN_FILE;
        capture_string_env_var(env_var, _env->irodsSSLCertificateChainFile);
        env_var = irods::CFG_IRODS_SSL_CERTIFICATE_KEY_FILE;
        capture_string_env_var(env_var, _env->irodsSSLCertificateKeyFile);
        env_var = irods::CFG_IRODS_SSL_DH_PARAMS_FILE;
        capture_string_env_var(env_var, _env->irodsSSLDHParamsFile);

        env_var = irods::CFG_IRODS_MAX_SIZE_FOR_SINGLE_BUFFER;
        capture_integer_env_var(env_var, _env->irodsMaxSizeForSingleBuffer);

        env_var = irods::CFG_IRODS_DEF_NUMBER_TRANSFER_THREADS;
        capture_integer_env_var(env_var, _env->irodsDefaultNumberTransferThreads);

        env_var = irods::CFG_IRODS_TRANS_BUFFER_SIZE_FOR_PARA_TRANS;
        capture_integer_env_var(env_var, _env->irodsTransBufferSizeForParaTrans);
    ...
    {
        if (!_fout) {
            fprintf(stderr, "printRodsEnv :: null input param(s)\n");
        ...
        try {
            ...
            for (auto itr = prop_map.cbegin(); itr != prop_map.cend(); ++itr) {
                ...
                try {
                    ...
                    fprintf(_fout, "%s - %s\n", itr->first.c_str(), val.c_str());
                    continue;
                }
                catch (const boost::bad_any_cast&) {
                    fprintf(stderr, "failed to cast %s", itr->first.c_str());
                }
    ...
    int createRodsEnvDefaults(rodsEnv* rodsEnvArg)
    {
        if (strlen(rodsEnvArg->rodsHome) == 0 && strlen(rodsEnvArg->rodsUserName) > 0 &&
            strlen(rodsEnvArg->rodsZone) > 0) {
            snprintf(rodsEnvArg->rodsHome, MAX_NAME_LEN, "/%s/home/%s", rodsEnvArg->rodsZone, rodsEnvArg->rodsUserName);
            rodsLog(LOG_NOTICE, "created irodsHome=%s", rodsEnvArg->rodsHome);
        }
        if (strlen(rodsEnvArg->rodsCwd) == 0 && strlen(rodsEnvArg->rodsHome) > 0) {
