...
{
    return addMsParamToArray(msParamArray, label, type, inOutStruct, inpOutBuf, 0);
}
                      ...
                      int replFlag)
{
    ...
    if (msParamArray == NULL || label == NULL) {
        rodsLog(LOG_ERROR, "addMsParam: NULL msParamArray or label input");
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (strcmp(msParamArray->msParam[i]->label, label) == 0) {
            ...
            if (!strcmp(msParamArray->msParam[i]->type, STR_MS_T) && !strcmp(type, STR_MS_T) &&
                !strcmp((char*) inOutStruct, (char*) msParamArray->msParam[i]->inOutStruct)) {
                return 0;
            }
            /***  Jan 28 2010 to make it not given an error ***/
            rodsLog(LOG_ERROR, "addMsParam: Two params have the same label %s", label);
            if (!strcmp(msParamArray->msParam[i]->type, STR_MS_T))
                rodsLog(
                    LOG_ERROR, "addMsParam: old string value = %s\n", (char*) msParamArray->msParam[i]->inOutStruct);
            else
                rodsLog(LOG_ERROR, "addMsParam: old param is of type: %s\n", msParamArray->msParam[i]->type);
            if (!strcmp(type, STR_MS_T))
                rodsLog(LOG_ERROR, "addMsParam: new string value = %s\n", (char*) inOutStruct);
            else
                rodsLog(LOG_ERROR, "addMsParam: new param is of type: %s\n", type);
    ...
    if (replFlag == 0) {
        fillMsParam(msParamArray->msParam[len], label, type, inOutStruct, inpOutBuf);
    }
    else {
...
int replMsParamArray(msParamArray_t* in, msParamArray_t* out)
{
    ...
    out->msParam = (msParam_t**) malloc(newLen * sizeof(*out->msParam));
    memset(out->msParam, 0, newLen * sizeof(*out->msParam));
...
int replInOutStruct(void* inStruct, void** outStruct, const char* type)
{
    if (outStruct == NULL) {
        rodsLogError(
            LOG_ERROR, SYS_INTERNAL_NULL_INPUT_ERR, "replInOutStruct was called with a null pointer in outStruct");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    if (inStruct == NULL) {
        *outStruct = NULL;
    ...
    int status = packStruct(inStruct, &packedResult, type, NULL, 0, NATIVE_PROT);
    if (status < 0) {
        rodsLogError(LOG_ERROR, status, "replInOutStruct: packStruct error for type %s", type);
        return status;
    }
    status = unpackStruct(packedResult->buf, outStruct, type, NULL, NATIVE_PROT);
...
int writeMsParam(char* buf, int len, msParam_t* msParam)
{
    ...
    if (msParam->label != NULL && msParam->type != NULL && msParam->inOutStruct != NULL) {
        if (strcmp(msParam->type, STR_MS_T) == 0) {
            ...
            for (j = 0; j < kVPairs->len; j++) {
                snprintf(
                    &buf[strlen(buf)], len - strlen(buf), "       %s = %s\n", kVPairs->keyWord[j], kVPairs->value[j]);
            }
        }
...
int clearMsParam(msParam_t* msParam, int freeStruct)
{
    ...
    if (msParam->inOutStruct != NULL &&
        (freeStruct > 0 || (msParam->type != NULL && strcmp(msParam->type, STR_MS_T) == 0))) {
...
int trimMsParamArray(msParamArray_t* msParamArray, char* outParamDesc)
{
    ...
    if (outParamDesc != NULL && strlen(outParamDesc) > 0) {
        ...
        if (status < 0) {
            rodsLog(LOG_ERROR, "trimMsParamArray: parseMultiStr error, status = %d", status);
        }
    }
    ...
    while (i < msParamArray->len) {
        ...
        if (msParam[i]->type == NULL || strlen(msParam[i]->type) == 0) {
            ...
            for (k = 0; k < strArray.len; k++) {
                if (strcmp(&value[k * strArray.size], msParam[i]->label) == 0) {
                    match = 1;
                    break;
                }
            }
                          ...
                          int outputToCache)
{
    *outDataObjInp = NULL;
    ...
    if (inpParam == NULL) {
        rodsLog(LOG_ERROR, "parseMspForDataObjInp: input inpParam is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (strcmp(inpParam->type, STR_MS_T) == 0) {
        ...
        if (dataObjInpCache == NULL) {
            rodsLog(LOG_ERROR, "parseMspForDataObjInp: input dataObjInpCache is NULL");
            return SYS_INTERNAL_NULL_INPUT_ERR;
        }
        memset(dataObjInpCache, 0, sizeof(dataObjInp_t));
        *outDataObjInp = dataObjInpCache;
        if (strcmp((char*) inpParam->inOutStruct, "null") != 0) {
            rstrcpy(dataObjInpCache->objPath, (char*) inpParam->inOutStruct, MAX_NAME_LEN);
        }
        return 0;
    }
        ...
        *outDataObjInp = dataObjInpCache;
        ...
        rodsLog(LOG_ERROR, "parseMspForDataObjInp: Unsupported input Param1 type %s", inpParam->type);
        return USER_PARAM_TYPE_ERR;
    }
}
    ...
    *outCollInp = NULL;
    ...
    if (strcmp(inpParam->type, STR_MS_T) == 0) {
        ...
        *outCollInp = collInpCache;
        if (strcmp((char*) inpParam->inOutStruct, "null") != 0) {
            rstrcpy(collInpCache->collName, (char*) inpParam->inOutStruct, MAX_NAME_LEN);
        }
        return 0;
    }
        ...
        rodsLog(LOG_ERROR, "parseMspForCollInp: Unsupported input Param1 type %s", inpParam->type);
        return USER_PARAM_TYPE_ERR;
    }
}
                              ...
                              dataObjCopyInp_t** outDataObjCopyInp)
{
    *outDataObjCopyInp = NULL;
    if (inpParam == NULL) {
        rodsLog(LOG_ERROR, "parseMspForDataObjCopyInp: input inpParam is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (strcmp(inpParam->type, STR_MS_T) == 0) {
        ...
        rstrcpy(dataObjCopyInpCache->srcDataObjInp.objPath, (char*) inpParam->inOutStruct, MAX_NAME_LEN);
        ...
        *outDataObjCopyInp = (dataObjCopyInp_t*) inpParam->inOutStruct;
        ...
        memset(dataObjCopyInpCache, 0, sizeof(dataObjCopyInp_t));
        dataObjCopyInpCache->srcDataObjInp = *(dataObjInp_t*) inpParam->inOutStruct;
        *outDataObjCopyInp = dataObjCopyInpCache;
    }
    else if (strcmp(inpParam->type, KeyValPair_MS_T) == 0) {
        /* key-val pair input needs ketwords "DATA_NAME" and  "COLL_NAME" */
        ...
        *outDataObjCopyInp = dataObjCopyInpCache;
        ...
        rodsLog(LOG_ERROR, "parseMspForDataObjCopyInp: Unsupported input Param1 type %s", inpParam->type);
        return USER_PARAM_TYPE_ERR;
    }
    return 0;
}
...
int parseMspForCondInp(msParam_t* inpParam, keyValPair_t* condInput, char* condKw)
{
    if (inpParam != NULL) {
        if (strcmp(inpParam->type, STR_MS_T) == 0) {
            /* str input */
            if (strcmp((char*) inpParam->inOutStruct, "null") != 0) {
                addKeyVal(condInput, condKw, (char*) inpParam->inOutStruct);
            }
        }
        else {
            rodsLog(LOG_ERROR, "parseMspForCondInp: Unsupported input Param type %s", inpParam->type);
            return USER_PARAM_TYPE_ERR;
        }
    }
    return 0;
...
int parseMspForCondKw(msParam_t* inpParam, keyValPair_t* condInput)
{
    if (inpParam != NULL) {
        if (strcmp(inpParam->type, STR_MS_T) == 0) {
            /* str input */
            if (strcmp((char*) inpParam->inOutStruct, "null") != 0 && strlen((const char*) inpParam->inOutStruct) > 0) {
                addKeyVal(condInput, (char*) inpParam->inOutStruct, "");
            }
        }
        else {
            rodsLog(LOG_ERROR, "parseMspForCondKw: Unsupported input Param type %s", inpParam->type);
            return USER_PARAM_TYPE_ERR;
        }
    }
    return 0;
...
int parseMspForPhyPathReg(msParam_t* inpParam, keyValPair_t* condInput)
{
    ...
    if (inpParam != NULL) {
        if (strcmp(inpParam->type, STR_MS_T) == 0) {
            ...
            /* str input */
            if (tmpStr != NULL && strlen(tmpStr) > 0 && strcmp(tmpStr, "null") != 0) {
...
int parseMspForPosInt(msParam_t* inpParam)
{
    ...
    if (strcmp(inpParam->type, STR_MS_T) == 0) {
    ...
    else if (strcmp(inpParam->type, INT_MS_T) == 0 || strcmp(inpParam->type, BUF_LEN_MS_T) == 0) {
        ...
        rodsLog(LOG_ERROR, "parseMspForPosInt: Unsupported input Param type %s", inpParam->type);
        return USER_PARAM_TYPE_ERR;
    }
    if (myInt < 0) {
        rodsLog(LOG_DEBUG, "parseMspForPosInt: parsed int %d is negative", myInt);
    }
    return myInt;
}
...
char* parseMspForStr(msParam_t* inpParam)
{
    ...
    if (strcmp(inpParam->type, STR_MS_T) != 0) {
        rodsLog(LOG_ERROR, "parseMspForStr: inpParam type %s is not STR_MS_T", inpParam->type);
        return NULL;
    }
...
int parseMspForFloat(msParam_t* inpParam, float* floatout)
{
    ...
    else if (strcmp(inpParam->type, INT_MS_T) == 0 || strcmp(inpParam->type, FLOAT_MS_T) == 0) {
        *floatout = *(float*) inpParam->inOutStruct;
    }
    else {
        rodsLog(LOG_ERROR, "parseMspForPosFloat: Unsupported input Param type %s", inpParam->type);
        return USER_PARAM_TYPE_ERR;
    }
    return 0;
}
...
int parseMspForDouble(msParam_t* inpParam, double* doubleout)
{
    ...
    if (strcmp(inpParam->type, STR_MS_T) == 0) {
        ...
        *doubleout = *(double*) inpParam->inOutStruct;
        ...
        rodsLog(LOG_ERROR, "%s: Unsupported input Param type %s", __FUNCTION__, inpParam->type);
        return USER_PARAM_TYPE_ERR;
    }
    return 0;
}
int parseMspForExecCmdInp(msParam_t* inpParam, execCmd_t* execCmdInpCache, execCmd_t** ouExecCmdInp)
{
    *ouExecCmdInp = NULL;
    if (inpParam == NULL) {
        rodsLog(LOG_ERROR, "parseMspForExecCmdInp: input inpParam is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (strcmp(inpParam->type, STR_MS_T) == 0) {
        ...
        rstrcpy(execCmdInpCache->cmd, (char*) inpParam->inOutStruct, LONG_NAME_LEN);
        ...
        *ouExecCmdInp = (execCmd_t*) inpParam->inOutStruct;
        ...
        rodsLog(LOG_ERROR, "parseMspForExecCmdInp: Unsupported input Param1 type %s", inpParam->type);
        return USER_PARAM_TYPE_ERR;
    }
    return 0;
}
int getStdoutInExecCmdOut(msParam_t* inpExecCmdOut, char** outStr)
{
    ...
    if (inpExecCmdOut == NULL) {
        rodsLog(LOG_ERROR, "getStdoutInExecCmdOut input inpParam is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (strcmp(inpExecCmdOut->type, ExecCmdOut_MS_T) == 0) {
        ...
        *outStr = (char*) malloc(execCmdOut->stdoutBuf.len + 1);
        ...
        rodsLog(LOG_ERROR, "getStdoutInExecCmdOut: Unsupported input Param type %s", inpExecCmdOut->type);
        return USER_PARAM_TYPE_ERR;
    }
}
...
int getStderrInExecCmdOut(msParam_t* inpExecCmdOut, char** outStr)
{
    ...
    if (inpExecCmdOut == NULL) {
        rodsLog(LOG_ERROR, "getStderrInExecCmdOut input inpParam is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if (strcmp(inpExecCmdOut->type, ExecCmdOut_MS_T) == 0) {
        ...
        *outStr = (char*) malloc(execCmdOut->stderrBuf.len + 1);
        ...
        rodsLog(LOG_ERROR, "getStderrInExecCmdOut: Unsupported input Param type %s", inpExecCmdOut->type);
        return USER_PARAM_TYPE_ERR;
    }
}
...
int clearParsedMsKeyValStr(parsedMsKeyValStr_t* parsedMsKeyValStr)
{
    if (parsedMsKeyValStr == NULL) {
        rodsLog(LOG_ERROR, "clearParsedMsKeyValStr: input parsedMsKeyValStr is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
...
int getNextKeyValFromMsKeyValStr(parsedMsKeyValStr_t* parsedMsKeyValStr)
{
    ...
    if (parsedMsKeyValStr == NULL) {
        rodsLog(LOG_ERROR, "getNextKeyValFromMsKeyValStr: input parsedMsKeyValStr is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    if ((tmpEndPtr = strstr(parsedMsKeyValStr->curPtr, MS_INP_SEP_STR)) != NULL) {
                                  ...
                                  char** outBadKeyWd)
{
    ...
    if (inpParam == NULL || dataObjInp == NULL) {
        rodsLog(LOG_ERROR, "parseMsKeyValStrForDataObjInp: input inpParam or dataObjInp is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    while (getNextKeyValFromMsKeyValStr(&parsedMsKeyValStr) >= 0) {
        if (parsedMsKeyValStr.kwPtr == NULL) {
            if (hintForMissingKw == NULL) {
                ...
                rodsLogError(
                    LOG_ERROR, status, "parseMsKeyValStrForDataObjInp: no keyWd for %s", parsedMsKeyValStr.valPtr);
        ...
        if ((status = chkDataObjInpKw(parsedMsKeyValStr.kwPtr, validKwFlags)) < 0) {
            if (outBadKeyWd != NULL) {
        ...
        if (status == CREATE_MODE_FLAG) {
            ...
            rstrcpy(dataObjInp->objPath, parsedMsKeyValStr.valPtr, MAX_NAME_LEN);
            continue;
        }
                               ...
                               char** outBadKeyWd)
{
    ...
    if (inpParam == NULL || collInp == NULL) {
        rodsLog(LOG_ERROR, "parseMsKeyValStrForCollInp: input inpParam or collInp is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    while (getNextKeyValFromMsKeyValStr(&parsedMsKeyValStr) >= 0) {
        if (parsedMsKeyValStr.kwPtr == NULL) {
            if (hintForMissingKw == NULL) {
                status = NO_KEY_WD_IN_MS_INP_STR;
                rodsLogError(
                    LOG_ERROR, status, "parseMsKeyValStrForCollInp: no keyWd for %s", parsedMsKeyValStr.valPtr);
                clearParsedMsKeyValStr(&parsedMsKeyValStr);
                return status;
            }
            else if (strcmp(hintForMissingKw, KEY_WORD_KW) == 0) {
        ...
        if ((status = chkCollInpKw(parsedMsKeyValStr.kwPtr, validKwFlags)) < 0) {
            if (outBadKeyWd != NULL) {
                *outBadKeyWd = strdup(parsedMsKeyValStr.kwPtr);
            }
            return status;
        ...
        if (status == COLL_FLAGS_FLAG) {
            ...
            rstrcpy(collInp->collName, parsedMsKeyValStr.valPtr, MAX_NAME_LEN);
            continue;
        }
        addKeyVal(condInput, parsedMsKeyValStr.kwPtr, parsedMsKeyValStr.valPtr);
    }
                                              ...
                                              char** outBadKeyWd)
{
    if (inpParam == NULL || structFileExtAndRegInp == NULL) {
        rodsLog(LOG_ERROR, "parseMsKeyValStrForStructFile:inpParam or structFileInp is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }
    ...
    while (getNextKeyValFromMsKeyValStr(&parsedMsKeyValStr) >= 0) {
        ...
        if ((status = chkStructFileExtAndRegInpKw(parsedMsKeyValStr.kwPtr, validKwFlags)) < 0) {
            if (outBadKeyWd != NULL) {
                *outBadKeyWd = strdup(parsedMsKeyValStr.kwPtr);
            }
            clearParsedMsKeyValStr(&parsedMsKeyValStr);
        ...
        if (status == COLL_FLAGS_FLAG) {
            ...
            rstrcpy(structFileExtAndRegInp->objPath, parsedMsKeyValStr.valPtr, MAX_NAME_LEN);
            continue;
        }
        else if (status == COLL_NAME_FLAG) {
            rstrcpy(structFileExtAndRegInp->collection, parsedMsKeyValStr.valPtr, MAX_NAME_LEN);
            continue;
        }
        addKeyVal(condInput, parsedMsKeyValStr.kwPtr, parsedMsKeyValStr.valPtr);
    }
...
int chkStructFileExtAndRegInpKw(char* keyWd, int validKwFlags)
{
    ...
    for (i = 0; i < NumStructFileExtAndRegInpKeyWd; i++) {
        if (strcmp(StructFileExtAndRegInpKeyWd[i].keyWd, keyWd) == 0) {
            if ((StructFileExtAndRegInpKeyWd[i].flag & validKwFlags) == 0) {
                /* not valid */
                break;
            }
            else {
...
int parseMsParamFromIRFile(msParamArray_t* inpParamArray, char* inBuf)
{
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "parseMsParamFromIRFile: parseMultiStr error, status = %d", status);
        inpParamArray = NULL;
        return status;
    }
    value = strArray.value;
    ...
    for (i = 0; i < strArray.len; i++) {
        ...
        if ((tmpPtr = strstr(valPtr, "=")) != NULL) {
            *tmpPtr = '\0';
            ...
            addMsParam(inpParamArray, valPtr, STR_MS_T, param, NULL);
        }
        else {
            rodsLog(LOG_ERROR, "parseMsParamFromIRFile: inpParam %s format error", valPtr);
        }
    }
