...
namespace irods
{
    ...
    std::string normalize_resource_type(const std::string& resource_type)
    {
        ...
        normalized_resource_type.erase(
            std::remove_if(normalized_resource_type.begin(), normalized_resource_type.end(), not_allowed_char),
    ...
    error plugin_name_generator::operator()(const std::string& _base_name,
                                            ...
                                            std::string& _rtn_soname)
    {
        ...
        clean_plugin_name.erase(std::remove_if(clean_plugin_name.begin(), clean_plugin_name.end(), not_allowed_char),
    ...
    bool plugin_name_generator::exists(const std::string& _base_name, const std::string& _dir_name)
    {
        bool result = true;
        std::string so_name;
    ...
    error plugin_name_generator::generate_plugin_name(const std::string& filename, std::string& _rtn_name)
    {
        error result = SUCCESS();
        _rtn_name.clear();
        int sub_length = filename.length() - 6;
        if (sub_length > 0 && filename.find("lib") == 0 && filename.find(".so") == (filename.length() - 3)) {
