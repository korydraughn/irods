...
int sendTranHeader(int sock, int oprType, int flags, rodsLong_t offset, rodsLong_t length)
{
    ...
    if (retVal != sizeof(myHeader)) {
        rodsLog(LOG_ERROR, "sendTranHeader: toWrite = %d, written = %d", sizeof(myHeader), retVal);
        if (retVal < 0) {
            return retVal;
        }
        else {
...
int rcvTranHeader(int sock, transferHeader_t* myHeader)
{
    ...
    retVal = myRead(sock, (void*) &tmpHeader, sizeof(tmpHeader), NULL, NULL);

    if (retVal != sizeof(tmpHeader)) {
        rodsLog(LOG_ERROR, "rcvTranHeader: toread = %d, read = %d", sizeof(tmpHeader), retVal);
        if (retVal < 0) {
            return retVal;
        }
        else {
...
int fillBBufWithFile(rcComm_t* conn, bytesBuf_t* myBBuf, char* locFilePath, rodsLong_t dataSize)
{
    ...
    if (dataSize > single_buff_sz) {
        rodsLog(LOG_ERROR, "fillBBufWithFile: dataSize %lld too large", dataSize);
    ...
    if (in_fd < 0) { /* error */
        ...
        rodsLogError(LOG_ERROR, status, "cannot open file %s", locFilePath, status);
        return status;
    }
    ...
    status = myRead(in_fd, myBBuf->buf, (int) dataSize, NULL, NULL);
...
int putFileToPortal(rcComm_t* conn, portalOprOut_t* portalOprOut, char* locFilePath, char* objPath, rodsLong_t dataSize)
{
    ...
    if (portalOprOut == NULL || portalOprOut->numThreads <= 0) {
        rodsLog(LOG_ERROR, "putFileToPortal: invalid portalOprOut");
    ...
    if (portalOprOut->numThreads > MAX_NUM_CONFIG_TRAN_THR) {
        for (int i = 0; i < portalOprOut->numThreads; i++) {
            const int sock = connectToRhostPortal(
                myPortList->hostAddr, myPortList->portNum, myPortList->cookie, myPortList->windowSize);
            if (sock >= 0) {
                close(sock);
            }
        }
        rodsLog(LOG_ERROR, "putFileToPortal: numThreads %d too large", portalOprOut->numThreads);
        return SYS_INVALID_PORTAL_OPR;
    }

    initFileRestart(conn, locFilePath, objPath, dataSize, portalOprOut->numThreads);
    memset(myInput, 0, sizeof(myInput));

    if (numThreads == 1) {
        const int sock =
            connectToRhostPortal(myPortList->hostAddr, myPortList->portNum, myPortList->cookie, myPortList->windowSize);
        if (sock < 0) {
            return sock;
        }
#ifdef windows_platform
        ...
        if (in_fd < 0) { /* error */
            ...
            rodsLogError(LOG_ERROR, retVal, "cannot open file %s", locFilePath, retVal);
            close(sock);
            return retVal;
        }
        ...
        if (retVal < 0) {
            ...
            if (dataSize <= 0 || totalWritten == dataSize) {
                ...
                rodsLog(LOG_ERROR, "putFileToPortal: totalWritten %lld dataSize %lld mismatch", totalWritten, dataSize);
                return SYS_COPY_LEN_ERR;
            }
        }
    }
...
void rcPartialDataPut(rcPortalTransferInp_t* myInput)
{
    ...
    if (myInput == NULL) {
        rodsLog(LOG_ERROR, "rcPartialDataPut: NULL input");
        return;
    }
    ...
    bool use_encryption_flg = (myInput->conn->negotiation_results == irods::CS_NEG_USE_SSL);
    ...
    irods::buffer_crypt crypt(rods_env.rodsEncryptionKeySize,
    ...
    while (myInput->status >= 0) {
        ...
        while (toPut > 0) {
            ...
            bytesRead = myRead(srcFd, buf, toRead, &bytesRead, NULL);
            if (bytesRead != toRead) {
                myInput->status = SYS_COPY_LEN_ERR - errno;
                rodsLogError(
                    LOG_ERROR, myInput->status, "rcPartialDataPut: toPut %lld, bytesRead %d", toPut, bytesRead);
                break;
            }
            ...
            if (info->numSeg > 0) { /* file restart */
                ...
                if (threadNum == 0 && conn->fileRestart.writtenSinceUpdated >= RESTART_FILE_UPDATE_SIZE) {
                    int status;
                    /* time to write to the restart file */
                    status = writeLfRestartFile(conn->fileRestart.infoFile, &conn->fileRestart.info);
                    if (status < 0) {
                        rodsLog(LOG_ERROR,
                                "rcPartialDataPut: writeLfRestartFile for %s, status = %d",
                                conn->fileRestart.info.fileName,
                                status);
                    }
...
int putFile(rcComm_t* conn, int l1descInx, char* locFilePath, char* objPath, rodsLong_t dataSize)
{
    ...
    while ((dataObjWriteInpBBuf.len = myRead(in_fd, dataObjWriteInpBBuf.buf, trans_buff_sz, &bytesRead, NULL)) > 0) {
        ...
        if (bytesWritten < dataObjWriteInp.len) {
            rodsLog(LOG_ERROR, "putFile: Read %d bytes, Wrote %d bytes.\n ", dataObjWriteInp.len, bytesWritten);
            ...
            if (info->numSeg > 0) { /* file restart */
                ...
                if (totalWritten - lastUpdateSize >= RESTART_FILE_UPDATE_SIZE) {
                    ...
                    status = writeLfRestartFile(conn->fileRestart.infoFile, &conn->fileRestart.info);
                    if (status < 0) {
                        rodsLog(LOG_ERROR, "putFile: writeLfRestartFile for %s, status = %d", locFilePath, status);
                        free(dataObjWriteInpBBuf.buf);
                        close(in_fd);
                        return status;
                    }
            ...
            if (gGuiProgressCB != NULL) {
                if (progressCnt >= (MAX_PROGRESS_CNT - 1)) {
                    conn->operProgress.curFileSizeDone += ((MAX_PROGRESS_CNT - 1) * trans_buff_sz + bytesWritten);
                    gGuiProgressCB(&conn->operProgress);
                    progressCnt = 0;
                }
                else {
    ...
    if (dataSize <= 0 || totalWritten == dataSize) {
        ...
        rodsLog(LOG_ERROR, "putFile: totalWritten %lld dataSize %lld mismatch", totalWritten, dataSize);
        return SYS_COPY_LEN_ERR;
    }
}
...
int getIncludeFile(rcComm_t* conn, bytesBuf_t* dataObjOutBBuf, char* locFilePath)
{
    ...
    if (strcmp(locFilePath, STDOUT_FILE_NAME) == 0) {
        ...
        bytesWritten = fwrite(dataObjOutBBuf->buf, dataObjOutBBuf->len, 1, stdout);
        if (bytesWritten == 1) {
        ...
        bytesWritten = myWrite(out_fd, dataObjOutBBuf->buf, dataObjOutBBuf->len, NULL);
...
int getFile(rcComm_t* conn, int l1descInx, char* locFilePath, char* objPath, rodsLong_t dataSize)
{
    ...
    while ((bytesRead = rcDataObjRead(conn, &dataObjReadInp, &dataObjReadInpBBuf)) > 0) {
        if (out_fd == 1) {
            bytesWritten = fwrite(dataObjReadInpBBuf.buf, bytesRead, 1, stdout);
            if (bytesWritten == 1) {
                bytesWritten = bytesRead;
            }
        }
        else {
            bytesWritten = myWrite(out_fd, dataObjReadInpBBuf.buf, bytesRead, NULL);
        }

        if (bytesWritten != bytesRead) {
            rodsLog(LOG_ERROR, "getFile: Read %d bytes, Wrote %d bytes.\n ", bytesRead, bytesWritten);
            ...
            if (info->numSeg > 0) { /* file restart */
                ...
                if (totalWritten - lastUpdateSize >= RESTART_FILE_UPDATE_SIZE) {
                    ...
                    status = writeLfRestartFile(conn->fileRestart.infoFile, &conn->fileRestart.info);
                    if (status < 0) {
                        rodsLog(LOG_ERROR, "getFile: writeLfRestartFile for %s, status = %d", locFilePath, status);
                        free(dataObjReadInpBBuf.buf);
                        if (out_fd != 1) {
                            close(out_fd);
                        }
    ...
    if (bytesRead >= 0) {
        ...
        rodsLog(LOG_ERROR, "getFile: totalWritten %lld dataSize %lld mismatch", totalWritten, dataSize);
        return bytesRead;
    }
}
                      ...
                      rodsLong_t dataSize)
{
    ...
    if (portalOprOut == NULL || portalOprOut->numThreads <= 0) {
        rodsLog(LOG_ERROR, "getFileFromPortal: invalid portalOprOut");
        return SYS_INVALID_PORTAL_OPR;
    }
    ...
    if (portalOprOut->numThreads > MAX_NUM_CONFIG_TRAN_THR) {
        ...
        rodsLog(LOG_ERROR, "getFileFromPortal: numThreads %d too large", numThreads);
        return SYS_INVALID_PORTAL_OPR;
    }
    ...
    if (numThreads == 1) {
        ...
        if (retVal < 0) {
            ...
            if (dataSize <= 0 || totalWritten == dataSize) {
                ...
                rodsLog(
                    LOG_ERROR, "getFileFromPortal: totalWritten %lld dataSize %lld mismatch", totalWritten, dataSize);
                return SYS_COPY_LEN_ERR;
            }
        }
    }
...
void rcPartialDataGet(rcPortalTransferInp_t* myInput)
{
    ...
    int threadNum;
    if (myInput == NULL) {
        rodsLog(LOG_ERROR, "rcPartialDataGet: NULL input");
        return;
    }
        ...
        while (toGet > 0) {
            ...
            if (use_encryption_flg) {
                ...
                if (bytesRead != sizeof(int)) {
                    rodsLog(LOG_ERROR, "_partialDataGet:Bytes Read != %d", sizeof(int));
                    break;
                }
            }
            ...
            // the incoming size
            bytesRead = myRead(srcFd, buf, new_size, &bytesRead, NULL);
            if (bytesRead != new_size) {
                myInput->status = SYS_COPY_LEN_ERR - errno;
                rodsLogError(
                    LOG_ERROR, myInput->status, "rcPartialDataGet: toGet %lld, bytesRead %d", toGet, bytesRead);
                break;
            }
            ...
            bytesWritten = myWrite(destFd, buf, plain_size, &bytesWritten);
                         ...
                         int packetSize)
{
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "putFileToPortalRbudp: initRbudpClient error for %s", myPortList->hostAddr);
        return status;
    }
    rbudpSender.rbudpBase.verbose = veryVerbose;
    if ((mysendRate = sendRate) < 1 &&
        ((tmpStr = getenv(RBUDP_SEND_RATE_KW)) == NULL || (mysendRate = atoi(tmpStr)) < 1)) {
        mysendRate = DEF_UDP_SEND_RATE;
    }
    if ((mypacketSize = packetSize) < 1 &&
        ((tmpStr = getenv(RBUDP_PACK_SIZE_KW)) == NULL || (mypacketSize = atoi(tmpStr)) < 1)) {
        mypacketSize = DEF_UDP_PACKET_SIZE;
    }

    if (locFilePath == NULL) {
        status = sendfileByFd(&rbudpSender, mysendRate, mypacketSize, locFd);
    }
    else {
        status = rbSendfile(&rbudpSender, mysendRate, mypacketSize, locFilePath);
    }

    sendClose(&rbudpSender);
    if (status < 0) {
        rodsLog(LOG_ERROR,
                "putFileToPortalRbudp: sendfile error for %s:%d", // JMC - backport 4590
                myPortList->hostAddr,
                myPortList->portNum & 0xffff0000);
        return status;
    }
    return status;
}
...
int getFileToPortalRbudp(portalOprOut_t* portalOprOut, char* locFilePath, int locFd, int veryVerbose, int packetSize)
{
    ...
    if (portalOprOut == NULL || portalOprOut->numThreads != 1) {
        rodsLog(LOG_ERROR, "getFileToPortalRbudp: invalid portalOprOut");
        return SYS_INVALID_PORTAL_OPR;
    }
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "getFileToPortalRbudp: initRbudpClient error for %s", myPortList->hostAddr);
        return status;
    }
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "getFileToPortalRbudp: getfile error for %s", myPortList->hostAddr);
        return status;
    }
    return status;
}
...
int initRbudpClient(rbudpBase_t* rbudpBase, portList_t* myPortList)
{
    ...
    if ((udpPort = getUdpPortFromPortList(myPortList)) == 0) {
        rodsLog(LOG_ERROR, "putFileToPortalRbudp: udpPort == 0");
        return SYS_INVALID_PORTAL_OPR;
    }

    tcpPort = getTcpPortFromPortList(myPortList);

    tcpSock = connectToRhostPortal(myPortList->hostAddr, tcpPort, myPortList->cookie, myPortList->windowSize);
    if (tcpSock < 0) {
        return tcpSock;
    }
    ...
    if (status < 0) {
        rodsLog(
            LOG_ERROR, "initRbudpClient: passiveUDP connect to %s error. status = %d", myPortList->hostAddr, status);
        return SYS_UDP_CONNECT_ERR + status;
    }

    /* inform the server of the UDP port */
    rbudpBase->udpLocalPort = setLocalAddr(rbudpBase->udpSockfd, &localUdpAddr);
    if (rbudpBase->udpLocalPort < 0) {
        return rbudpBase->udpLocalPort;
    }
    udpLocalPort = htonl(rbudpBase->udpLocalPort);
    status = writen(rbudpBase->tcpSockfd, (char*) &udpLocalPort, sizeof(udpLocalPort));
    if (status != sizeof(udpLocalPort)) {
        rodsLog(
            LOG_ERROR, "initRbudpClient: writen error. towrite %d, bytes written %d ", sizeof(udpLocalPort), status);
...
int initFileRestart(rcComm_t* conn, char* fileName, char* objPath, rodsLong_t fileSize, int numThr)
{
    ...
    if (fileRestart->flags != FILE_RESTART_ON || fileSize < MIN_RESTART_SIZE || numThr <= 0) {
        info->numSeg = 0; /* indicate no restart */
        return 0;
    }
    if (numThr > MAX_NUM_CONFIG_TRAN_THR) {
        rodsLog(LOG_NOTICE, "initFileRestart: input numThr %d larger than max %d ", numThr, MAX_NUM_CONFIG_TRAN_THR);
        info->numSeg = 0; /* indicate no restart */
        return 0;
    }
    info->numSeg = numThr;
...
int writeLfRestartFile(char* infoFile, fileRestartInfo_t* info)
{
    ...
    status = packStruct((void*) info, &packedBBuf, "FileRestartInfo_PI", RodsPackTable, 0, XML_PROT);
    if (status < 0 || packedBBuf == NULL) {
        rodsLog(LOG_ERROR, "writeLfRestartFile: packStruct error for %s, status = %d", info->fileName, status);
        freeBBuf(packedBBuf);
        return status;
    }
    ...
    if (fd < 0) {
        status = UNIX_FILE_OPEN_ERR - errno;
        rodsLog(LOG_ERROR, "writeLfRestartFile: open failed for %s, status = %d", infoFile, status);
        freeBBuf(packedBBuf);
        return status;
    }
    ...
    if (status < 0) {
        ...
        rodsLog(LOG_ERROR, "writeLfRestartFile: write failed for %s, status = %d", infoFile, status);
        return status;
    }
    return status;
}
...
int readLfRestartFile(char* infoFile, fileRestartInfo_t** info)
{
    ...
    *info = NULL;
    ...
    if (!exists(p) || !is_regular_file(p)) {
        ...
        status = UNIX_FILE_STAT_ERR - errno;
        rodsLog(LOG_ERROR, "readLfRestartFile restart infoFile size is 0 for %s", infoFile);
        return status;
    }
    ...
    if (fd < 0) {
        status = UNIX_FILE_OPEN_ERR - errno;
        rodsLog(LOG_ERROR, "readLfRestartFile open failed for %s, status = %d", infoFile, status);
        return status;
    }
    ...
    if (status != mySize) {
        rodsLog(LOG_ERROR, "readLfRestartFile error failed for %s, toread %d, read %d", infoFile, mySize, status);
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "readLfRestartFile: unpackStruct error for %s, status = %d", infoFile, status);
    }
    free(buf);
    return status;
}
...
int lfRestartPutWithInfo(rcComm_t* conn, fileRestartInfo_t* info)
{
    ...
    if (localFd < 0) { /* error */
        status = UNIX_FILE_OPEN_ERR - errno;
        rodsLogError(LOG_ERROR, status, "cannot open file %s", info->fileName, status);
        return status;
    }
    ...
    if (irodsFd < 0) { /* error */
        rodsLogError(LOG_ERROR, irodsFd, "cannot open target file %s, status = %d", info->objPath, irodsFd);
        close(localFd);
        return irodsFd;
    }
    ...
    for (i = 0; i < info->numSeg; i++) {
        ...
        if (info->dataSeg[i].len > 0) {
            ...
            if (lseek(localFd, curOffset, SEEK_SET) < 0) {
                ...
                rodsLogError(
                    LOG_ERROR, status, "lfRestartWithInfo: lseek to %lld error for %s", curOffset, info->fileName);
                break;
            }
           ...
           rodsLong_t* dataSegLen)
{
    ...
    while (gap > 0) {
        ...
        dataObjWriteInpBBuf->len = myRead(localFd, dataObjWriteInpBBuf->buf, toRead, NULL, NULL);
        /* Write to the data object */
        dataObjWriteInp->len = dataObjWriteInpBBuf->len;
        bytesWritten = rcDataObjWrite(conn, dataObjWriteInp, dataObjWriteInpBBuf);
        if (bytesWritten < dataObjWriteInp->len) {
            ...
            *dataSegLen += toRead;
            if (*writtenSinceUpdated >= RESTART_FILE_UPDATE_SIZE) {
                ...
                if (status < 0) {
                    rodsLog(LOG_ERROR, "putSeg: writeLfRestartFile for %s, status = %d", info->fileName, status);
                    return status;
                }
...
int lfRestartGetWithInfo(rcComm_t* conn, fileRestartInfo_t* info)
{
    ...
    if (localFd < 0) { /* error */
        status = UNIX_FILE_OPEN_ERR - errno;
        rodsLogError(LOG_ERROR, status, "cannot open local file %s, status = %d", info->fileName, status);
        return status;
    }
    ...
    irodsFd = rcDataObjOpen(conn, &dataObjOpenInp);
    if (irodsFd < 0) { /* error */
        rodsLogError(LOG_ERROR, irodsFd, "cannot open iRODS src file %s, status = %d", info->objPath, irodsFd);
        close(localFd);
        return irodsFd;
    }
           ...
           rodsLong_t* dataSegLen)
{
    ...
    while (gap > 0) {
        ...
        if (bytesRead < 0) {
            rodsLog(LOG_ERROR, "getSeg: rcDataObjRead error. status = %d", bytesRead);
            return bytesRead;
        }
        else if (bytesRead == 0) {
            /* EOF */
            rodsLog(LOG_ERROR, "getSeg: rcDataObjRead error. EOF reached. toRead = %d", toRead);
            return SYS_COPY_LEN_ERR;
        }
        bytesWritten = myWrite(localFd, dataObjReadInpBBuf->buf, bytesRead, NULL);

        if (bytesWritten != bytesRead) {
            rodsLog(LOG_ERROR, "getSeg: Read %d bytes, Wrote %d bytes.\n ", bytesRead, bytesWritten);
            return SYS_COPY_LEN_ERR;
        }
        else {
            gap -= bytesWritten;
            ...
            *dataSegLen += bytesWritten;
            if (*writtenSinceUpdated >= RESTART_FILE_UPDATE_SIZE) {
                status = writeLfRestartFile(conn->fileRestart.infoFile, info);
                if (status < 0) {
                    rodsLog(LOG_ERROR, "getSeg: writeLfRestartFile for %s, status = %d", info->fileName, status);
                    return status;
                }
                *writtenSinceUpdated = 0;
            }
...
int catDataObj(rcComm_t* conn, char* objPath)
{
    ...
    if (l1descInx < 0) {
        rodsLogError(LOG_ERROR, l1descInx, "catDataObj: rcDataObjOpen error for %s", objPath);
        return l1descInx;
    }
    ...
    while ((bytesRead = rcDataObjRead(conn, &dataObjReadInp, &dataObjReadOutBBuf)) > 0) {
