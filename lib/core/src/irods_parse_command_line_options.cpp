...
static int parse_program_options(int _argc, char** _argv, rodsArguments_t& _rods_args, path_list_t& _paths)
{
    ...
    opt_desc.add_options()("help,h", "show command usage")("all,a", "all - update all existing copies")(
        "bulk,b", "bulk upload to reduce overhead")("force,f",
                                                    "force - write data-object even it exists already; overwrite it")(
        ...
        "verify_checksum,K",
        "verify checksum - calculate and verify the checksum on the data, both client-side and server-side, without "
        "storing in the catalog.")(
        "repl_num,n", po::value<std::string>(), "replNum  - the replica to be replaced, typically not needed")(
        "num_threads,N",
        po::value<int>(),
        "numThreads - the number of threads to use for the transfer. A value of 0 means no threading. By default (-N "
        "option not used) the server decides the number of threads to use.")(
        "physical_path,p",
        po::value<std::string>(),
        "physicalPath - the absolute physical path of the uploaded file on the server")(
        "progress,P", "output the progress of the upload.")(
        "rbudp,Q", "use RBUDP (datagram) protocol for the data transfer")("recursive,r",
                                                                          "recursive - store the whole subdirectory")(
        "dest_resc,R",
        po::value<std::string>(),
        "resource - specifies the resource to store to. This can also be specified in your environment or via a rule "
        "set up by the administrator")(
        "ticket,t", po::value<std::string>(), "ticket - ticket (string) to use for ticket-based access")(
        "renew_socket,T", "renew socket connection after 10 minutes")("verbose,v", "verbose")(
        "very_verbose,V", "very verbose")("data_type,D", po::value<std::string>(), "dataType - the data type string")(
        "restart_file,X",
        po::value<std::string>(),
        "restartFile - specifies that the restart option is on and the restartFile input specifies a local file that "
        "contains the restart information.")("link", "ignore symlink.")(
        "lfrestart",
        po::value<std::string>(),
        "lfRestartFile - specifies that the large file restart option is on and the lfRestartFile input specifies a "
        "local file that contains the restart information.")(
        "retries",
        po::value<int>(),
        "count - Retry the iput in case of error. The 'count' input specifies the number of times to retry. It must be "
        "used with the -X option")("wlock", "use advisory write (exclusive) lock for the upload")(
    ...
    try {
        po::store(po::command_line_parser(_argc, _argv).options(opt_desc).positional(pos_desc).run(),
                  global_prog_ops_var_map);
        po::notify(global_prog_ops_var_map);
    }
    catch (po::error& _e) {
        std::cout << std::endl << "Error: " << _e.what() << std::endl << std::endl;
        return SYS_INVALID_INPUT_PARAM;
    }

    bool have_verify = global_prog_ops_var_map.count("verify_checksum") > 0;
    bool have_stdout = _paths.end() != std::find(_paths.begin(), _paths.end(), std::string(STDOUT_FILE_NAME));
                                      ...
                                      rodsPathInp_t* _rods_paths)
{
    ...
    if (_dst_type != NO_INPUT_T) {
        ...
        if (_path_list.size() > 1) {
            rstrcpy(_rods_paths->destPath->inPath, _path_list.rbegin()->c_str(), MAX_NAME_LEN);
        }
        else {
                         ...
                         rodsPathInp_t* _rods_paths)
{
    ...
    path_list_t paths;
    int p_err = parse_program_options(_argc, _argv, _rods_args, paths);
    if (p_err < 0) {
        return p_err;
    }
    ...
    p_err = build_irods_path_structure(paths, _rods_env, _src_type, _dst_type, _flag, _rods_paths);
    if (p_err < 0) {
        return p_err;
    }
