...
int bunUtil(rcComm_t* conn, rodsEnv* myRodsEnv, rodsArguments_t* myRodsArgs, rodsPathInp_t* rodsPathInp)
{
    ...
    for (int i = 0; i < rodsPathInp->numSrc; i++) {
        ...
        rstrcpy(structFileExtAndRegInp.collection, collPath->outPath, MAX_NAME_LEN);
        rstrcpy(structFileExtAndRegInp.objPath, structFilePath->outPath, MAX_NAME_LEN);
        ...
        if (myRodsArgs->extract == True) { /* -x */
            ...
            rodsLog(LOG_ERROR, "bunUtil: -x or -c must be specified");
            return -1;
        }

        /* XXXX may need to return a global status */
        if (status < 0 && status != CAT_NO_ROWS_FOUND) {
            if (status == SYS_CACHE_STRUCT_FILE_RESC_ERR && myRodsArgs->condition == True) {
                rodsLogError(LOG_ERROR, status, "bunUtil: A resc must be entered for non-existing structFile");
            }
            else {
                rodsLogError(LOG_ERROR, status, "bunUtil: opr error for %s, status = %d", collPath->outPath, status);
                savedStatus = status;
            }
        }
    }
...
int initCondForBunOpr(rodsEnv* myRodsEnv, rodsArguments_t* rodsArgs, structFileExtAndRegInp_t* structFileExtAndRegInp)
{
    if (structFileExtAndRegInp == NULL) {
        rodsLog(LOG_ERROR, "initCondForBunOpr: NULL structFileExtAndRegInp input");
        return USER__NULL_INPUT_ERR;
    }
    ...
    if (rodsArgs->dataType == True) {
        if (rodsArgs->dataTypeString != NULL) {
            if (strcmp(rodsArgs->dataTypeString, "t") == 0 ||
                ...
                rodsLog(LOG_ERROR,
                        "bunUtil: Unknown dataType %s for ibun", // JMC - backport 4648
        ...
        addKeyVal(&structFileExtAndRegInp->condInput, DATA_TYPE_KW, "tar file");
    }

    if (rodsArgs->resource == True) {
        if (rodsArgs->resourceString == NULL) {
            rodsLog(LOG_ERROR, "initCondForBunOpr: NULL resourceString error");
            return USER__NULL_INPUT_ERR;
        }
        else {
            addKeyVal(&structFileExtAndRegInp->condInput, DEST_RESC_NAME_KW, rodsArgs->resourceString);
            /* RESC_NAME_KW is need for unbundle. DEST_RESC_NAME_KW or
             * DEF_RESC_NAME_KW are neede for bundle */
            addKeyVal(&structFileExtAndRegInp->condInput, RESC_NAME_KW, rodsArgs->resourceString);
        }
    }
        ...
        addKeyVal(&structFileExtAndRegInp->condInput, DEF_RESC_NAME_KW, myRodsEnv->rodsDefResource);
    }
