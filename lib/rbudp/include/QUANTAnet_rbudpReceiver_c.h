...
/**
RBUDP Receiver class.  This class implements the receiver part of RBUDP protocol. First, instantiate the
QUANTAnet_rbudpReceiver_c class. Then call this object's init() method with the host name or IP address of the sender as
parameter.  After the connection is established, you should immediately call this object's receive(buffer, sizeofBuffer,
...
/** Receive a block of memory from the sender using RBUDP protocol.
        ...
        @param packetSize payload size of each UDP packet, suggest 1460 considering the Ethernet MTU restriction and the
   header size.  It has to be consistent with the sender.
*/
