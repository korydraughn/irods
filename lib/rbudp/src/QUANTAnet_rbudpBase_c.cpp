...
{
    rbudpBase->udpSockBufSize = nbytes > 0 ? rbudpBase->udpSockBufSize : UDPSOCKBUF;
}
...
int passiveUDP(rbudpBase_t* rbudpBase, char* host)
{
    ...
    bzero((char*) &rbudpBase->udpServerAddr, sizeof(rbudpBase->udpServerAddr));
    ...
    if ((bind(rbudpBase->udpSockfd, (struct sockaddr*) &rbudpBase->udpServerAddr, sizeof(rbudpBase->udpServerAddr))) <
        0) {
    ...
    if (connect(rbudpBase->udpSockfd, (struct sockaddr*) &cliaddr, sizeof(cliaddr)) < 0) {
        perror("connect() error");
        return errno ? (-1 * errno) : -1;
    }

    checkbuf(rbudpBase->udpSockfd, rbudpBase->udpSockBufSize, rbudpBase->verbose);
    return 0;
}
...
int connectUDP(rbudpBase_t* rbudpBase, char* host)
{
    ...
    if ((bind(rbudpBase->udpSockfd, (struct sockaddr*) &udpClientAddr, sizeof(udpClientAddr))) < 0) {
...
long long rb_ntohll(long long nll)
{
    ...
    return ((long long) cp[0] << 56) | ((long long) cp[1] << 48) | ((long long) cp[2] << 40) |
           ((long long) cp[3] << 32) | ((long long) cp[4] << 24) | ((long long) cp[5] << 16) |
           ((long long) cp[6] << 8) | ((long long) cp[7] << 0);
}
...
int setUdpSockOpt(int udpSockfd)
{
    ...
    int error_code;
    error_code = setsockopt(udpSockfd, SOL_SOCKET, SO_SNDBUF, &sockbufsize, sizeof(sockbufsize));
    if (error_code != 0) {
        rodsLog(LOG_ERROR, "setsockopt failed on the send buffer in setUdpSockOpt with %d", error_code);
    }
    error_code = setsockopt(udpSockfd, SOL_SOCKET, SO_RCVBUF, &sockbufsize, sizeof(sockbufsize));
    if (error_code != 0) {
