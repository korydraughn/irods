...
void QUANTAnet_rbudpSender_c(rbudpSender_t* rbudpSender, int port)
{
...
void QUANTAnet_rbudpSender_reuse(rbudpSender_t* rbudpSender, int tcpsock, int port)
{
...
int sendBuf(rbudpSender_t* rbudpSender, void* buffer, int bufSize, int sendRate, int packetSize)
{
    int done = 0;
    int status = 0;
    ...
    while (!done) {
        ...
        srate = (double) rbudpSender->rbudpBase.remainNumberOfPackets * rbudpSender->rbudpBase.payloadSize * 8 /
        ...
        if (lastRemainNumberOfPackets == 0) {
            lastRemainNumberOfPackets = rbudpSender->rbudpBase.remainNumberOfPackets;
        }
        // send end of UDP signal
        if (rbudpSender->rbudpBase.verbose > 1)
            TRACE_DEBUG("send to socket %d an end signal.", rbudpSender->rbudpBase.tcpSockfd);
        if (rbudpSender->rbudpBase.verbose > 1) {
            fprintf(stderr, "write %d bytes.\n", (int) sizeof(rbudpSender->rbudpBase.endOfUdp));
        }
        int error_code = writen(rbudpSender->rbudpBase.tcpSockfd,
                                (char*) &rbudpSender->rbudpBase.endOfUdp,
        ...
        if ((unsigned char) rbudpSender->rbudpBase.errorBitmap[0] == 1) {
            ...
            rbudpSender->rbudpBase.remainNumberOfPackets = updateHashTable(&rbudpSender->rbudpBase);
            if (rbudpSender->rbudpBase.remainNumberOfPackets >= lastRemainNumberOfPackets) {
                noProgressCnt++;
        ...
        if (rbudpSender->rbudpBase.isFirstBlast) {
            ...
            double lossRate = (double) rbudpSender->rbudpBase.remainNumberOfPackets /
            ...
            if (rbudpSender->rbudpBase.verbose > 0) {
                float dt = (curTime.tv_sec - startTime.tv_sec) + 1e-6 * (curTime.tv_usec - startTime.tv_usec);
                float mbps = 1e-6 * 8 * bufSize / (dt == 0 ? .01 : dt);
                TRACE_DEBUG(
                    "loss rate: %f  on %dK in %.3f seconds (%.2f Mbits/s)", lossRate, (int) bufSize >> 10, dt, mbps);
                if (rbudpSender->rbudpBase.verbose > 1)
                    TRACE_DEBUG("usecsPerPacket updated to %d", rbudpSender->rbudpBase.usecsPerPacket);
            }
        }
    }
...
int udpSend(rbudpSender_t* rbudpSender)
{
    ...
    while (!done) {
        ...
        if (USEC(&start, &now) < rbudpSender->rbudpBase.usecsPerPacket * i) {
            ...
            // as regular packets
            if (rbudpSender->rbudpBase.hashTable[i] < rbudpSender->rbudpBase.totalNumberOfPackets - 1) {
                actualPayloadSize = rbudpSender->rbudpBase.payloadSize;
            }
            else {
                actualPayloadSize = rbudpSender->rbudpBase.lastPayloadSize;
            }
            rbudpSender->sendHeader.seq = rbudpSender->rbudpBase.hashTable[i];
            bcopy(&rbudpSender->sendHeader, msg, rbudpSender->rbudpBase.headerSize);
            bcopy((char*) ((char*) rbudpSender->rbudpBase.mainBuffer +
                           (rbudpSender->sendHeader.seq * rbudpSender->rbudpBase.payloadSize)),
            ...
            if (rbudpSender->rbudpBase.udpServerAddr.sin_addr.s_addr == htonl(INADDR_ANY)) {
                if (send(rbudpSender->rbudpBase.udpSockfd,
                         msg,
                         actualPayloadSize + rbudpSender->rbudpBase.headerSize,
                         0) < 0) {
                    perror("send");
                    sendErrCnt++;
                ...
                if (sendto(rbudpSender->rbudpBase.udpSockfd,
                           msg,
                           actualPayloadSize + rbudpSender->rbudpBase.headerSize,
                           0,
                           (const struct sockaddr*) &rbudpSender->rbudpBase.udpServerAddr,
                           sizeof(rbudpSender->rbudpBase.udpServerAddr)) < 0) {
                    perror("sendto");
                    sendErrCnt++;
                    if (sendErrCnt > MAX_SEND_ERR_CNT) {
                        return SYS_UDP_TRANSFER_ERR - errno;
...
int sendstream(rbudpSender_t* rbudpSender, int fromfd, int sendRate, int packetSize, int bufSize)
{
...
int rbSendfile(rbudpSender_t* rbudpSender, int sendRate, int packetSize, char* fname)
{
    int tcpSockfd = rbudpSender->rbudpBase.tcpSockfd;
    int verbose = rbudpSender->rbudpBase.verbose;
...
int sendfileByFd(rbudpSender_t* rbudpSender, int sendRate, int packetSize, int fd)
{
    int tcpSockfd = rbudpSender->rbudpBase.tcpSockfd;
    int verbose = rbudpSender->rbudpBase.verbose;
    int status = 0;
    ...
    while (remaining > 0) {
        ...
        if (verbose > 0)
            TRACE_DEBUG("Sending %d bytes chunk. %lld bytes remaining", toSend, remaining - toSend);
...
/* sendRate: Kbps */
int initSendRudp(rbudpSender_t* rbudpSender, void* buffer, int bufSize, int sRate, int pSize)
{
    int i;
    ...
    rbudpSender->rbudpBase.packetSize = rbudpSender->rbudpBase.payloadSize + rbudpSender->rbudpBase.headerSize;
    rbudpSender->rbudpBase.usecsPerPacket =
        8 * rbudpSender->rbudpBase.payloadSize * 1000 / rbudpSender->rbudpBase.sendRate;
    rbudpSender->rbudpBase.isFirstBlast = 1;

    if (rbudpSender->rbudpBase.dataSize % rbudpSender->rbudpBase.payloadSize == 0) {
        rbudpSender->rbudpBase.totalNumberOfPackets =
            rbudpSender->rbudpBase.dataSize / rbudpSender->rbudpBase.payloadSize;
        rbudpSender->rbudpBase.lastPayloadSize = rbudpSender->rbudpBase.payloadSize;
    }
    else {
        rbudpSender->rbudpBase.totalNumberOfPackets =
            rbudpSender->rbudpBase.dataSize / rbudpSender->rbudpBase.payloadSize + 1; /* the last packet is not full */
        rbudpSender->rbudpBase.lastPayloadSize =
            rbudpSender->rbudpBase.dataSize -
            rbudpSender->rbudpBase.payloadSize * (rbudpSender->rbudpBase.totalNumberOfPackets - 1);
    }

    rbudpSender->rbudpBase.remainNumberOfPackets = rbudpSender->rbudpBase.totalNumberOfPackets;
    rbudpSender->rbudpBase.sizeofErrorBitmap = rbudpSender->rbudpBase.totalNumberOfPackets / 8 + 2;
    rbudpSender->rbudpBase.errorBitmap = (char*) malloc(rbudpSender->rbudpBase.sizeofErrorBitmap);
    rbudpSender->rbudpBase.hashTable =
        (long long*) malloc(rbudpSender->rbudpBase.totalNumberOfPackets * sizeof(long long));
    ...
    if (rbudpSender->rbudpBase.verbose > 1)
        TRACE_DEBUG("totalNumberOfPackets: %d", rbudpSender->rbudpBase.totalNumberOfPackets);
    if (rbudpSender->rbudpBase.verbose > 1)
        TRACE_DEBUG("usecsPerPacket: %d", rbudpSender->rbudpBase.usecsPerPacket);
