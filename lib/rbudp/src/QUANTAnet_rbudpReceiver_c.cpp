...
void QUANTAnet_rbudpReceiver_c(rbudpReceiver_t* rbudpReceiver, int port)
{
...
void QUANTAnet_rbudpReceiver_c_reuse(rbudpReceiver_t* rbudpReceiver, int tcpsock, int port)
{
...
int receiveBuf(rbudpReceiver_t* rbudpReceiver, void* buffer, int bufSize, int packetSize)
{
    int done = 0;
    int status = 0;
    ...
    while (!done) {
        ...
        if (verbose) {
            ...
            int got = packetSize * (rbudpReceiver->rbudpBase.totalNumberOfPackets - nerrors);
            float mbps = 1e-6 * 8 * got / (dt == 0 ? .01 : dt);
            TRACE_DEBUG("Error: %d, Loss rate: %.5f Got %d/%dK in %.2fs (%g Mbit/s)",
                        nerrors,
                        (double) nerrors / (double) rbudpReceiver->rbudpBase.totalNumberOfPackets,
                        (int) (got >> 10),
                        (int) (bufSize >> 10),
                        dt,
                        mbps);
        ...
        if (writen(rbudpReceiver->rbudpBase.tcpSockfd,
                   ...
                   rbudpReceiver->rbudpBase.sizeofErrorBitmap) != rbudpReceiver->rbudpBase.sizeofErrorBitmap) {
            perror("tcp send");
...
#define QMAX(x, y) ((x) > (y) ? (x) : (y))
    const int maxfdpl = QMAX(rbudpReceiver->rbudpBase.udpSockfd, rbudpReceiver->rbudpBase.tcpSockfd) + 1;
    FD_ZERO(&rset);
    while (!done) {
        ...
        if (FD_ISSET(rbudpReceiver->rbudpBase.udpSockfd, &rset)) {
            if (rbudpReceiver->rbudpBase.udpServerAddr.sin_addr.s_addr == htonl(INADDR_ANY)) {
                ...
                if (recv(rbudpReceiver->rbudpBase.udpSockfd, &msg[0], rbudpReceiver->rbudpBase.packetSize, 0) < 0) {
                    perror("recv");
                    return errno ? (-1 * errno) : -1;
                }
            }
            ...
            bcopy(&msg[0] + rbudpReceiver->rbudpBase.headerSize,
                  (char*) rbudpReceiver->rbudpBase.mainBuffer + (seqno * rbudpReceiver->rbudpBase.payloadSize),
                  actualPayloadSize);

            updateErrorBitmap(&rbudpReceiver->rbudpBase, seqno);

            rbudpReceiver->rbudpBase.receivedNumberOfPackets++;
            const float prog = (float) rbudpReceiver->rbudpBase.receivedNumberOfPackets /
                               (float) rbudpReceiver->rbudpBase.totalNumberOfPackets * 100;
            if ((int) prog > oldprog) {
                oldprog = (int) prog;
                if (oldprog > 100) {
                    oldprog = 100;
                ...
                if (rbudpReceiver->rbudpBase.progress != 0) {
                    fseek(rbudpReceiver->rbudpBase.progress, 0, SEEK_SET);
                    fprintf(rbudpReceiver->rbudpBase.progress, "%d\n", oldprog);
                }
            }
        }
...
int getstream(rbudpReceiver_t* rbudpReceiver, int tofd, int packetSize)
{
    ...
    while (true) {
        ...
        int n = readn(rbudpReceiver->rbudpBase.tcpSockfd, (char*) &nbufSize, sizeof(nbufSize));
        if (n < 0) {
        ...
        if (buf == NULL || bufSize != curSize) {
            ...
            if (bufSize < 1 || bufSize > std::numeric_limits<long long>::max()) {
                fprintf(stderr,
                        "bufSize %ji must be greater than zero and no more than %ji\n",
                        (intmax_t) bufSize,
                        (intmax_t) std::numeric_limits<long long>::max());
                ok = FAILED;
                break;
            }
...
int getfile(rbudpReceiver_t* rbudpReceiver, char* origFName, char* destFName, int packetSize)
{
    int status;

    // Send getfile message.
    if (origFName != NULL) {
        if (writen(rbudpReceiver->rbudpBase.tcpSockfd, origFName, SIZEOFFILENAME) != SIZEOFFILENAME) {
            perror("tcp send");
            return FAILED;
        }
    }
...
int getfileByFd(rbudpReceiver_t* rbudpReceiver, int fd, int packetSize)
{
    long long filesize;
    int verbose = rbudpReceiver->rbudpBase.verbose;
    long long offset = 0;

    int n = readn(rbudpReceiver->rbudpBase.tcpSockfd, (char*) &filesize, sizeof(filesize));
    if (n < 0) {
        fprintf(stderr, "read error.\n");
        return errno ? (-1 * errno) : -1;
    }
    ...
    if (filesize < 0 || filesize > std::numeric_limits<long long>::max()) {
        fprintf(stderr,
                "Invalid file size %ji. File size must be no less than zero and no greater than %ji.",
                (intmax_t) filesize,
                (intmax_t) std::numeric_limits<long long>::max());
        return -1;
    }
    ...
    while (remaining > 0) {
        ...
        if (verbose > 0)
            TRACE_DEBUG("Receiving %d bytes chunk. %lld bytes remaining", toRead, remaining - toRead);

        buf = (char*) mmap(NULL, toRead, PROT_READ | PROT_WRITE, MAP_SHARED, fd, offset);
        if (buf == MAP_FAILED) {
...
int getfilelist(rbudpReceiver_t* rbudpReceiver, char* fileList, int packetSize)
{
...
/* sendRate: Mbps */
int initReceiveRudp(rbudpReceiver_t* rbudpReceiver, void* buffer, int bufSize, int pSize)
{
    int i;
    ...
    rbudpReceiver->rbudpBase.packetSize = rbudpReceiver->rbudpBase.payloadSize + rbudpReceiver->rbudpBase.headerSize;
    rbudpReceiver->rbudpBase.isFirstBlast = 1;

    if (rbudpReceiver->rbudpBase.dataSize % rbudpReceiver->rbudpBase.payloadSize == 0) {
        rbudpReceiver->rbudpBase.totalNumberOfPackets =
            rbudpReceiver->rbudpBase.dataSize / rbudpReceiver->rbudpBase.payloadSize;
        rbudpReceiver->rbudpBase.lastPayloadSize = rbudpReceiver->rbudpBase.payloadSize;
    }
    else {
        rbudpReceiver->rbudpBase.totalNumberOfPackets =
            rbudpReceiver->rbudpBase.dataSize / rbudpReceiver->rbudpBase.payloadSize +
            1; /* the last packet is not full */
        rbudpReceiver->rbudpBase.lastPayloadSize =
            rbudpReceiver->rbudpBase.dataSize -
            rbudpReceiver->rbudpBase.payloadSize * (rbudpReceiver->rbudpBase.totalNumberOfPackets - 1);
    }
    rbudpReceiver->rbudpBase.remainNumberOfPackets = rbudpReceiver->rbudpBase.totalNumberOfPackets;
    rbudpReceiver->rbudpBase.receivedNumberOfPackets = 0;
    rbudpReceiver->rbudpBase.sizeofErrorBitmap = rbudpReceiver->rbudpBase.totalNumberOfPackets / 8 + 2;
    rbudpReceiver->rbudpBase.errorBitmap = (char*) malloc(rbudpReceiver->rbudpBase.sizeofErrorBitmap);
    rbudpReceiver->rbudpBase.hashTable =
        (long long*) malloc(rbudpReceiver->rbudpBase.totalNumberOfPackets * sizeof(long long));

    if (rbudpReceiver->rbudpBase.verbose > 1)
        TRACE_DEBUG("totalNumberOfPackets: %d", rbudpReceiver->rbudpBase.totalNumberOfPackets);
...
void initReceiver(rbudpReceiver_t* rbudpReceiver, char* remoteHost)
{
    ...
    rbudpReceiver->iovRecv[0].iov_base = (char*) (&rbudpReceiver->recvHeader);
