...
namespace irods
{
    error MD5Strategy::init(boost::any& _context) const
    {
        _context = MD5_CTX();
    ...
    error MD5Strategy::update(const std::string& data, boost::any& _context) const
    {
        MD5_Update(boost::any_cast<MD5_CTX>(&_context), (const unsigned char*) data.c_str(), data.size());
        return SUCCESS();
    }

    error MD5Strategy::digest(std::string& messageDigest, boost::any& _context) const
    {
