...
namespace irods
{
    namespace
    {
        ...
        boost::unordered_map<const std::string, const HashStrategy*> make_map()
        {
    ...
    error getHasher(const std::string& _name, Hasher& _hasher)
    {
    ...
    error get_hash_scheme_from_checksum(const std::string& _chksum, std::string& _scheme)
    {
        if (_chksum.empty()) {
            return ERROR(SYS_INVALID_INPUT_PARAM, "empty chksum string");
        }
        ...
        return ERROR(SYS_INVALID_INPUT_PARAM, "hash scheme not found");
