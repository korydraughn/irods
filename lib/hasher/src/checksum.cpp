...
int chksumLocFile(const char* _file_name, char* _checksum, const char* _hash_scheme)
{
    if (!_file_name || !_checksum || !_hash_scheme) {
        rodsLog(LOG_ERROR, "chksumLocFile :: null input param - %p %p %p", _file_name, _checksum, _hash_scheme);
    ...
    if (strlen(env.rodsMatchHashPolicy) > 0) {
        ...
        std::transform(env_scheme.begin(), env_scheme.end(), env_scheme.begin(), ::tolower);
    }

    // =-=-=-=-=-=-=-
    // capture the incoming scheme if it is valid
    std::string hash_scheme;
    if (_hash_scheme && strlen(_hash_scheme) > 0 && strlen(_hash_scheme) < NAME_LEN) {
    ...
    // =-=-=-=-=-=-=-
    // open the local file
    std::ifstream in_file(_file_name, std::ios::in | std::ios::binary);
    if (!in_file.is_open()) {
        status = UNIX_FILE_OPEN_ERR - errno;
        rodsLogError(LOG_ERROR, status, "chksumLocFile - fopen failed for %s, status = %d", _file_name, status);
        return status;
    }
    ...
    if (in_file.eof()) {
        ...
        rodsLogError(LOG_ERROR, status, "chksumLocFile - read() failed for %s, status = %d", _file_name, status);
        return status;
    }
    ...
    strncpy(_checksum, digest.c_str(), digest.size() + 1);
...
int rcChksumLocFile(char* fileName, char* chksumFlag, keyValPair_t* condInput, const char* _scheme)
{
    ...
    if (condInput == NULL || chksumFlag == NULL || fileName == NULL) {
        rodsLog(LOG_NOTICE, "rcChksumLocFile: NULL input");
        return USER__NULL_INPUT_ERR;
    }

    if (strcmp(chksumFlag, VERIFY_CHKSUM_KW) != 0 && strcmp(chksumFlag, REG_CHKSUM_KW) != 0 &&
        strcmp(chksumFlag, RSYNC_CHKSUM_KW) != 0) {
        rodsLog(LOG_NOTICE, "rcChksumLocFile: bad input chksumFlag %s", chksumFlag);
