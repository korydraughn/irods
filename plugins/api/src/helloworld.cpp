...
{
    return _api->call_handler<helloInp_t*, helloOut_t**>(_comm, _inp, _out);
}

#ifdef RODS_SERVER
static irods::error serialize_helloInp_ptr(boost::any _p, irods::re_serialization::serialized_parameter_t& _out)
{
    try {
        ...
        return ERROR(INVALID_ANY_CAST, "failed to cast helloInp ptr");
    }

    return SUCCESS();
} // serialize_helloInp_ptr

static irods::error serialize_helloOut_ptr_ptr(boost::any _p, irods::re_serialization::serialized_parameter_t& _out)
{
    try {
        ...
        return ERROR(INVALID_ANY_CAST, "failed to cast helloOut ptr ptr");
    }
