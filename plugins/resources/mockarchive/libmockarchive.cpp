...
irods::error mock_archive_generate_full_path(irods::plugin_property_map& _prop_map,
...
irods::error unix_check_path(irods::plugin_context& _ctx)
{
    ...
    try {
        ...
        irods::error ret = mock_archive_generate_full_path(_ctx.prop_map(), data_obj->physical_path(), full_path);
...
irods::error mock_archive_file_rename(irods::plugin_context& _ctx, const char* _new_file_name)
{
    ...
    if ((result = ASSERT_PASS(ret, "Invalid parameters or physical path.")).ok()) {
        ...
        if ((result = ASSERT_PASS(ret, "Unable to generate full path for destination file: \"%s\".", _new_file_name))
            ...
            std::string new_hash;
            ret = make_hashed_path(_ctx.prop_map(), _new_file_name, new_hash);
...
int mockArchiveCopyPlugin(int mode, const char* srcFileName, const char* destFileName)
{
    ...
    if (inFd < 0) {
        ...
        rodsLog(LOG_ERROR, "mockArchiveCopyPlugin: open error for srcFileName %s, status = %d", srcFileName, status);
        return status;
    }
    else if (status < 0) {
        status = UNIX_FILE_STAT_ERR - errno;
        rodsLog(LOG_ERROR, "mockArchiveCopyPlugin: stat of %s error, status = %d", srcFileName, status);
    ...
    if (outFd < 0) {
        status = UNIX_FILE_OPEN_ERR - errno;
        rodsLog(LOG_ERROR, "mockArchiveCopyPlugin: open error for destFileName %s, status = %d", destFileName, status);
        close(inFd);
        return status;
    }
    ...
    while ((bytesRead = read(inFd, (void*) myBuf.data(), trans_buff_size)) > 0) {
        ...
        if (bytesWritten <= 0) {
            ...
            rodsLog(
                LOG_ERROR, "mockArchiveCopyPlugin: write error for srcFileName %s, status = %d", destFileName, status);
            close(inFd);
            close(outFd);
            return status;
        }
...
irods::error mock_archive_file_stage_to_cache(irods::plugin_context& _ctx, const char* _cache_file_name)
{
    ...
    if ((result = ASSERT_PASS(ret, "Invalid plugin context.")).ok()) {
        ...
        if ((result = ASSERT_PASS(ret, "Failed to retrieve vault path for resource.")).ok()) {
            ...
            result = ASSERT_ERROR(status >= 0,
                                  status,
                                  "Failed copying archive file: \"%s\" to cache file: \"%s\".",
                                  fco->physical_path().c_str(),
                                  _cache_file_name);
        }
    }
...
irods::error mock_archive_file_sync_to_arch(irods::plugin_context& _ctx, const char* _cache_file_name)
{
    ...
    if ((result = ASSERT_PASS(ret, "Invalid plugin context.")).ok()) {
        ...
        // get the vault path for the resource
        std::string path;
        ret = make_hashed_path(_ctx.prop_map(), fco->physical_path(), path);
        if ((result = ASSERT_PASS(ret, "Failed to gen hashed path")).ok()) {
            // =-=-=-=-=-=-=-
            // append the hash to the path as the new 'cache file name'
            ...
            rodsLog(LOG_NOTICE,
                    "mock archive :: new hashed file name for [%s] is [%s]",
                    fco->physical_path().c_str(),
                    path.c_str());
...
irods::error mock_archive_redirect_open(irods::plugin_property_map& _prop_map,
...
irods::error mock_archive_file_resolve_hierarchy(irods::plugin_context& _ctx,
...
class mockarchive_resource : public irods::resource
{
    ...
    public:
        ...
        {
            rodsLog(LOG_NOTICE, "mockarchive_resource::post_disconnect_maintenance_operation - [%s]", name_.c_str());
            return SUCCESS();
        }
...
extern "C" irods::resource* plugin_factory(const std::string& _inst_name, const std::string& _context)
{
    ...
    resc->add_operation(irods::RESOURCE_OP_UNLINK, function<error(plugin_context&)>(mock_archive_file_unlink));

    resc->add_operation<const char*>(irods::RESOURCE_OP_STAGETOCACHE,
                                     function<error(plugin_context&, const char*)>(mock_archive_file_stage_to_cache));

    resc->add_operation<const char*>(irods::RESOURCE_OP_SYNCTOARCH,
                                     function<error(plugin_context&, const char*)>(mock_archive_file_sync_to_arch));

    resc->add_operation(irods::RESOURCE_OP_MKDIR, function<error(plugin_context&)>(mock_archive_file_mkdir));

    resc->add_operation<const char*>(irods::RESOURCE_OP_RENAME,
                                     function<error(plugin_context&, const char*)>(mock_archive_file_rename));

    resc->add_operation<struct stat*>(irods::RESOURCE_OP_STAT,
                                      function<error(plugin_context&, struct stat*)>(mock_archive_file_stat));

    resc->add_operation(irods::RESOURCE_OP_TRUNCATE, function<error(plugin_context&)>(mock_archive_file_truncate));
    ...
    resc->add_operation(irods::RESOURCE_OP_REBALANCE, function<error(plugin_context&)>(mock_archive_file_rebalance));
