    ...
    if ((result = ASSERT_PASS(err, "Failed in set_string")).ok()) {
        ...
        if ((result = ASSERT_PASS(err, "Failed in next.")).ok()) {
            ...
            if ((result = ASSERT_ERROR(
                     cmap_ref->has_entry(next), CHILD_NOT_FOUND, "Child map missing entry: \"%s\"", next.c_str()))
                    .ok()) {
                // =-=-=-=-=-=-=-
                // assign resource
...
irods::error load_balanced_file_read(irods::plugin_context& _ctx, void* _buf, const int _len)
{
    irods::error result = SUCCESS();
...
irods::error load_balanced_file_write(irods::plugin_context& _ctx, const void* _buf, const int _len)
{
    irods::error result = SUCCESS();
...
irods::error load_balanced_file_lseek(irods::plugin_context& _ctx, const long long _offset, const int _whence)
{
    irods::error result = SUCCESS();
    ...
    if ((result = ASSERT_PASS(err, "Failed to select load_balanced child.")).ok()) {
        ...
        err =
            resc->call<const long long, const int>(_ctx.comm(), irods::RESOURCE_OP_LSEEK, _ctx.fco(), _offset, _whence);
...
irods::error get_load_lists(irods::plugin_context& _ctx,
...
/// =-=-=-=-=-=-=-
/// @brief
irods::error load_balanced_redirect_for_create_operation(irods::plugin_context& _ctx,
                                                         ...
                                                         float* _out_vote)
{
    ...
    irods::error ret = get_load_lists(_ctx, names, loads, times);
    if (!ret.ok()) {
        return PASS(ret);
    }
    ...
    for (; itr != cmap_ref->end(); ++itr) {
        ...
        for (size_t i = 0; i < names.size(); ++i) {
            if (resc_name == names[i]) {
                if (loads[i] >= 0 && min_load > loads[i] && (time_now - times[i]) < MAX_ELAPSE_TIME) {
                    resc_found = true;
                    min_load = loads[i];
    ...
    if (!resc_found) {
        return ERROR(CHILD_NOT_FOUND, "failed to find child resc in load list");
    }
    ...
    irods::error err =
        selected_resource->call<const std::string*, const std::string*, irods::hierarchy_parser*, float*>(
            _ctx.comm(), irods::RESOURCE_OP_RESOLVE_RESC_HIER, _ctx.fco(), _opr, _curr_host, &parser, &vote);
    std::string hier;
    parser.str(hier);
...
/// =-=-=-=-=-=-=-
/// @brief
irods::error load_balanced_redirect_for_open_operation(irods::plugin_context& _ctx,
                                                       const std::string* _opr,
                                                       const std::string* _curr_host,
                                                       irods::hierarchy_parser* _out_parser,
                                                       float* _out_vote)
{
    *_out_vote = 0.0;
    ...
    for (; itr != cmap_ref->end(); ++itr) {
        ...
        // forward the redirect call to the child for assertion of the whole operation,
        // there may be more than a leaf beneath us
        irods::error err = resc->call<const std::string*, const std::string*, irods::hierarchy_parser*, float*>(
            _ctx.comm(), irods::RESOURCE_OP_RESOLVE_RESC_HIER, _ctx.fco(), _opr, _curr_host, &parser, &vote);
        std::string hier;
        parser.str(hier);
        rodsLog(LOG_DEBUG10, "load_balanced node - hier : [%s], vote %f", hier.c_str(), vote);
...
irods::error load_balanced_file_resolve_hierarchy(irods::plugin_context& _ctx,
                                                  const std::string* _opr,
                                                  const std::string* _curr_host,
                                                  irods::hierarchy_parser* _out_parser,
                                                  float* _out_vote)
...
public:
        ...
        : irods::resource(_inst_name, _context)
    {
        ...
        if (!ret.ok()) {
            rodsLog(LOG_ERROR, "libload_balanced: invalid context [%s]", _context.c_str());
            return;
        }

        if (kvp.end() != kvp.find(DEFER_POLICY_KEY)) {
            properties_.set<std::string>(DEFER_POLICY_KEY, kvp[DEFER_POLICY_KEY]);
        }
        else {
            properties_.set<std::string>(DEFER_POLICY_KEY, DEFER_POLICY_LOCALHOST);
            rodsLog(LOG_DEBUG, "load_balanced_resource :: using localhost policy, none specificed");
        }
    }
...
extern "C" irods::resource* plugin_factory(const std::string& _inst_name, const std::string& _context)
{
    ...
    using namespace std;
    resc->add_operation(RESOURCE_OP_CREATE, function<error(plugin_context&)>(load_balanced_file_create));

    resc->add_operation(irods::RESOURCE_OP_OPEN, function<error(plugin_context&)>(load_balanced_file_open));

    resc->add_operation<void*, const int>(
        irods::RESOURCE_OP_READ,
        std::function<error(irods::plugin_context&, void*, const int)>(load_balanced_file_read));

    resc->add_operation<const void*, const int>(
        irods::RESOURCE_OP_WRITE, function<error(plugin_context&, const void*, const int)>(load_balanced_file_write));

    resc->add_operation(RESOURCE_OP_CLOSE, function<error(plugin_context&)>(load_balanced_file_close));

    resc->add_operation(irods::RESOURCE_OP_UNLINK, function<error(plugin_context&)>(load_balanced_file_unlink));

    resc->add_operation<struct stat*>(irods::RESOURCE_OP_STAT,
                                      function<error(plugin_context&, struct stat*)>(load_balanced_file_stat));

    resc->add_operation(irods::RESOURCE_OP_MKDIR, function<error(plugin_context&)>(load_balanced_file_mkdir));

    resc->add_operation(irods::RESOURCE_OP_OPENDIR, function<error(plugin_context&)>(load_balanced_file_opendir));

    resc->add_operation<struct rodsDirent**>(
        irods::RESOURCE_OP_READDIR, function<error(plugin_context&, struct rodsDirent**)>(load_balanced_file_readdir));

    resc->add_operation<const char*>(irods::RESOURCE_OP_RENAME,
                                     function<error(plugin_context&, const char*)>(load_balanced_file_rename));

    resc->add_operation(irods::RESOURCE_OP_FREESPACE,
                        function<error(plugin_context&)>(load_balanced_file_getfs_freespace));

    resc->add_operation<const long long, const int>(
        irods::RESOURCE_OP_LSEEK,
        function<error(plugin_context&, const long long, const int)>(load_balanced_file_lseek));

    resc->add_operation(irods::RESOURCE_OP_RMDIR, function<error(plugin_context&)>(load_balanced_file_rmdir));

    resc->add_operation(irods::RESOURCE_OP_CLOSEDIR, function<error(plugin_context&)>(load_balanced_file_closedir));

    resc->add_operation<const char*>(irods::RESOURCE_OP_STAGETOCACHE,
                                     function<error(plugin_context&, const char*)>(load_balanced_file_stage_to_cache));

    resc->add_operation<const char*>(irods::RESOURCE_OP_SYNCTOARCH,
                                     function<error(plugin_context&, const char*)>(load_balanced_file_sync_to_arch));

    resc->add_operation(irods::RESOURCE_OP_REGISTERED, function<error(plugin_context&)>(load_balanced_file_registered));

    resc->add_operation(irods::RESOURCE_OP_UNREGISTERED,
                        function<error(plugin_context&)>(load_balanced_file_unregistered));

    resc->add_operation(irods::RESOURCE_OP_MODIFIED, function<error(plugin_context&)>(load_balanced_file_modified));

    resc->add_operation<const std::string*>(
        irods::RESOURCE_OP_NOTIFY, function<error(plugin_context&, const std::string*)>(load_balanced_file_notify));

    resc->add_operation(irods::RESOURCE_OP_TRUNCATE, function<error(plugin_context&)>(load_balanced_file_truncate));
    ...
    resc->add_operation(irods::RESOURCE_OP_REBALANCE, function<error(plugin_context&)>(load_balanced_file_rebalance));
