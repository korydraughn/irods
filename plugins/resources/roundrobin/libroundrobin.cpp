...
inline irods::error round_robin_check_params(irods::plugin_context& _ctx)
{
    // =-=-=-=-=-=-=-
                                    ...
                                    irods::resource_ptr& _resc)
{
    ...
    _props.get<irods::resource_child_map*>(irods::RESC_CHILD_MAP_PROP, cmap_ref);
...
irods::error get_next_child_for_open_or_write(const std::string& _name,
                                              ...
                                              irods::resource_ptr& _resc)
{
    ...
    for (; itr != objs.end(); ++itr) {
        ...
        return get_next_child_in_hier(_name, itr->resc_hier(), _props, _resc);

    } // for itr

    std::string msg("no hier found for resc [");
    msg += _name + "]";
    return ERROR(CHILD_NOT_FOUND, msg);
...
irods::error build_sorted_child_vector(irods::plugin_property_map& _props, std::vector<std::string>& _child_vector)
{
    ...
    for (itr = cmap_ref->begin(); itr != cmap_ref->end(); ++itr) {
    ...
    for (itr = cmap_ref->begin(); itr != cmap_ref->end(); ++itr) {
        ...
        for (vitr = _child_vector.begin(); vitr != _child_vector.end(); ++vitr) {
            if (vitr->empty()) {
                // =-=-=-=-=-=-=-
                // snag child resource name
                std::string name;
...
{
    irods::resource_child_map* cmap_ref;
    _prop_map.get<irods::resource_child_map*>(irods::RESC_CHILD_MAP_PROP, cmap_ref);
    ...
    for (size_t i = 0; i < child_vector.size(); ++i) {
        rodsLog(LOG_DEBUG, "round_robin_start_operation :: RR Child [%s] at [%d]", child_vector[i].c_str(), i);
    }
...
irods::error round_robin_file_read(irods::plugin_context& _ctx, void* _buf, const int _len)
{
    // =-=-=-=-=-=-=-
    // get the child resc to call
    irods::resource_ptr resc;
...
irods::error round_robin_file_write(irods::plugin_context& _ctx, const void* _buf, const int _len)
{
    // =-=-=-=-=-=-=-
    // get the child resc to call
    irods::resource_ptr resc;
...
irods::error round_robin_file_lseek(irods::plugin_context& _ctx, const long long _offset, const int _whence)
{
    // =-=-=-=-=-=-=-
    // get the child resc to call
    irods::resource_ptr resc;
...
irods::error round_robin_file_modified(irods::plugin_context& _ctx)
{
    ...
    err = _ctx.prop_map().get<std::string>(OPERATION_PROP, operation);
    if (err.ok() && irods::CREATE_OPERATION == operation) {
        ...
        int status = rsSetRoundRobinContext(_ctx.comm(), &inp);
        if (status < 0) {
            std::stringstream msg;
...
irods::error get_next_valid_child_resource(irods::plugin_context& _ctx,
                                           ...
                                           float* _out_vote)
{
    ...
    while (!child_found && child_ctr < cmap_ref->size()) {
        // =-=-=-=-=-=-=-
        ...
        std::string next_child;
        irods::error err = _ctx.prop_map().get<std::string>(NEXT_CHILD_PROP, next_child);
        if (!err.ok()) {
        ...
        if (!err.ok()) {
            rodsLog(LOG_ERROR, "forward of put redirect failed");
            continue;
        }
...
irods::error round_robin_file_resolve_hierarchy(irods::plugin_context& _ctx,
                                                const std::string* _opr,
                                                const std::string* _curr_host,
                                                irods::hierarchy_parser* _out_parser,
                                                float* _out_vote)
{
    ...
    if (!err.ok()) {
        return PASSMSG("round_robin_resolve_hierarchy - bad resource context", err);
    }
    if (!_opr) {
        return ERROR(SYS_INVALID_INPUT_PARAM, "round_robin_resolve_hierarchy - null operation");
    }
    if (!_curr_host) {
        return ERROR(SYS_INVALID_INPUT_PARAM, "round_robin_resolve_hierarchy - null host");
    }
    if (!_out_parser) {
        return ERROR(SYS_INVALID_INPUT_PARAM, "round_robin_resolve_hierarchy - null outgoing hier parser");
    }
    if (!_out_vote) {
        return ERROR(SYS_INVALID_INPUT_PARAM, "round_robin_resolve_hierarchy - null outgoing vote");
    }

    // =-=-=-=-=-=-=-
    // set operation property for use in file_modified for
    // determining and storing the next child when appropriate
    err = _ctx.prop_map().set<std::string>(OPERATION_PROP, *_opr);
    if (!err.ok()) {
        return PASS(err);
    }
    ...
    if (irods::OPEN_OPERATION == (*_opr) || irods::WRITE_OPERATION == (*_opr) || irods::UNLINK_OPERATION == (*_opr)) {
        ...
        irods::error err = get_next_valid_child_resource(_ctx, _opr, _curr_host, _out_parser, _out_vote);
        if (!err.ok()) {
            (*_out_vote) = 0.0;
            return PASS(err);
        }
        ...
        rodsLog(LOG_DEBUG, "round robin - create :: resc hier [%s] vote [%f]", hier.c_str(), *_out_vote);
...
irods::error round_robin_file_notify(irods::plugin_context& _ctx, const std::string* _opr)
{
    ...
    // =-=-=-=-=-=-=-
    // call open operation on the child
    return resc->call<const std::string*>(_ctx.comm(), irods::RESOURCE_OP_NOTIFY, _ctx.fco(), _opr);
...
extern "C" irods::resource* plugin_factory(const std::string& _inst_name, const std::string& _context)
{
    ...
    using namespace std;
    resc->add_operation(RESOURCE_OP_CREATE, function<error(plugin_context&)>(round_robin_file_create));

    resc->add_operation(irods::RESOURCE_OP_OPEN, function<error(plugin_context&)>(round_robin_file_open));

    resc->add_operation<void*, const int>(
        irods::RESOURCE_OP_READ, std::function<error(irods::plugin_context&, void*, const int)>(round_robin_file_read));

    resc->add_operation<const void*, const int>(
        irods::RESOURCE_OP_WRITE, function<error(plugin_context&, const void*, const int)>(round_robin_file_write));

    resc->add_operation(RESOURCE_OP_CLOSE, function<error(plugin_context&)>(round_robin_file_close));

    resc->add_operation(irods::RESOURCE_OP_UNLINK, function<error(plugin_context&)>(round_robin_file_unlink));

    resc->add_operation<struct stat*>(irods::RESOURCE_OP_STAT,
                                      function<error(plugin_context&, struct stat*)>(round_robin_file_stat));

    resc->add_operation(irods::RESOURCE_OP_MKDIR, function<error(plugin_context&)>(round_robin_file_mkdir));

    resc->add_operation(irods::RESOURCE_OP_OPENDIR, function<error(plugin_context&)>(round_robin_file_opendir));

    resc->add_operation<struct rodsDirent**>(
        irods::RESOURCE_OP_READDIR, function<error(plugin_context&, struct rodsDirent**)>(round_robin_file_readdir));

    resc->add_operation<const char*>(irods::RESOURCE_OP_RENAME,
                                     function<error(plugin_context&, const char*)>(round_robin_file_rename));

    resc->add_operation(irods::RESOURCE_OP_FREESPACE,
                        function<error(plugin_context&)>(round_robin_file_getfs_freespace));

    resc->add_operation<const long long, const int>(
        irods::RESOURCE_OP_LSEEK, function<error(plugin_context&, const long long, const int)>(round_robin_file_lseek));

    resc->add_operation(irods::RESOURCE_OP_RMDIR, function<error(plugin_context&)>(round_robin_file_rmdir));

    resc->add_operation(irods::RESOURCE_OP_CLOSEDIR, function<error(plugin_context&)>(round_robin_file_closedir));

    resc->add_operation<const char*>(irods::RESOURCE_OP_STAGETOCACHE,
                                     function<error(plugin_context&, const char*)>(round_robin_file_stage_to_cache));

    resc->add_operation<const char*>(irods::RESOURCE_OP_SYNCTOARCH,
                                     function<error(plugin_context&, const char*)>(round_robin_file_sync_to_arch));

    resc->add_operation(irods::RESOURCE_OP_REGISTERED, function<error(plugin_context&)>(round_robin_file_registered));

    resc->add_operation(irods::RESOURCE_OP_UNREGISTERED,
                        function<error(plugin_context&)>(round_robin_file_unregistered));

    resc->add_operation(irods::RESOURCE_OP_MODIFIED, function<error(plugin_context&)>(round_robin_file_modified));

    resc->add_operation<const std::string*>(
        irods::RESOURCE_OP_NOTIFY, function<error(plugin_context&, const std::string*)>(round_robin_file_notify));

    resc->add_operation(irods::RESOURCE_OP_TRUNCATE, function<error(plugin_context&)>(round_robin_file_truncate));
    ...
    resc->add_operation(irods::RESOURCE_OP_REBALANCE, function<error(plugin_context&)>(round_robin_file_rebalance));
