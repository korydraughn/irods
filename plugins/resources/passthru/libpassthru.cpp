...
irods::error passthru_get_first_child_resc(irods::plugin_property_map& _props, irods::resource_ptr& _resc)
{
    ...
    if (cmap_ref->size() != 1) {
        ...
        msg << "passthru_get_first_child_resc - Passthru resource can have 1 and only 1 child. This resource has "
            << cmap_ref->size();
...
irods::error passthru_file_read(irods::plugin_context& _ctx, void* _buf, const int _len)
{
    irods::error result = SUCCESS();
    irods::error ret;
...
irods::error passthru_file_write(irods::plugin_context& _ctx, const void* _buf, const int _len)
{
    irods::error result = SUCCESS();
    irods::error ret;
...
irods::error passthru_file_lseek(irods::plugin_context& _ctx, const long long _offset, const int _whence)
{
    irods::error result = SUCCESS();
    irods::error ret;
    ...
    if (!ret.ok()) {
        ...
        if (!ret.ok()) {
            ...
            ret = resc->call<const long long, const int>(
...
irods::error passthru_file_getfs_freespace(irods::plugin_context& _ctx)
{
    ...
    if (!ret.ok()) {
        ...
        if (!ret.ok()) {
            result =
                PASSMSG("passthru_file_getfs_freespace_plugin - failed getting the first child resource pointer.", ret);
        }
        else {
...
irods::error passthru_file_sync_to_arch(irods::plugin_context& _ctx, const char* _cache_file_name)
{
    ...
    if (!ret.ok()) {
        ...
        if (!ret.ok()) {
            result =
                PASSMSG("passthru_file_sync_to_arch_plugin - failed getting the first child resource pointer.", ret);
        }
        else {
...
static irods::error capture_weight(irods::plugin_context& _ctx, const std::string& _type, double& _weight)
{
    ...
    if (!results.empty()) {
        ...
        irods::kvp_map_t kvp;
        irods::parse_kvp_string(results, kvp);
    ...
    if (_ctx.prop_map().has_entry(_type)) {
        irods::error ret = _ctx.prop_map().get<double>(_type, _weight);
        if (!ret.ok()) {
            return PASS(ret);
        }
    }
...
irods::error passthru_file_resolve_hierarchy(irods::plugin_context& _ctx,
                                             ...
                                             float* _out_vote)
{
    ...
    irods::error final_ret = resc->call<const std::string*, const std::string*, irods::hierarchy_parser*, float*>(
    ...
    rodsLog(LOG_DEBUG, "passthru_file_resolve_hierarchy - [%s] : %f - %f", _opr->c_str(), orig_vote, *_out_vote);
...
irods::error passthru_file_notify(irods::plugin_context& _ctx, const std::string* _opr)
{
    ...
    irods::resource_child_map::iterator itr = cmap_ref->begin();
    for (; itr != cmap_ref->end(); ++itr) {
        irods::error ret = itr->second.second->call(_ctx.comm(), irods::RESOURCE_OP_NOTIFY, _ctx.fco(), _opr);
        if (!ret.ok()) {
            irods::log(PASS(ret));
            result = ret;
        }
...
public:
        ...
        : irods::resource(_inst_name, _context)
    {
        ...
        if (!_context.empty()) {
            ...
            if (!ret.ok()) {
                rodsLog(LOG_ERROR, "invalid context [%s] : %d", _context.c_str(), _context.size());
                return;
            }
            ...
            if (kvp_map.find(WRITE_WEIGHT_KW) != kvp_map.end()) {
                try {
                    ...
                    std::stringstream msg;
                    msg << "failed to cast weight for write [" << kvp_map[WRITE_WEIGHT_KW] << "]";
            ...
            if (kvp_map.find(READ_WEIGHT_KW) != kvp_map.end()) {
                try {
                    ...
                    std::stringstream msg;
                    msg << "failed to cast weight for read [" << kvp_map[READ_WEIGHT_KW] << "]";
                    irods::log(ERROR(SYS_INVALID_INPUT_PARAM, msg.str()));
                }
            }
...
extern "C" irods::resource* plugin_factory(const std::string& _inst_name, const std::string& _context)
{
    ...
    using namespace std;
    resc->add_operation(RESOURCE_OP_CREATE, function<error(plugin_context&)>(passthru_file_create));

    resc->add_operation(irods::RESOURCE_OP_OPEN, function<error(plugin_context&)>(passthru_file_open));

    resc->add_operation<void*, const int>(
        irods::RESOURCE_OP_READ, std::function<error(irods::plugin_context&, void*, const int)>(passthru_file_read));

    resc->add_operation<const void*, const int>(
        irods::RESOURCE_OP_WRITE, function<error(plugin_context&, const void*, const int)>(passthru_file_write));

    resc->add_operation(RESOURCE_OP_CLOSE, function<error(plugin_context&)>(passthru_file_close));

    resc->add_operation(irods::RESOURCE_OP_UNLINK, function<error(plugin_context&)>(passthru_file_unlink));

    resc->add_operation<struct stat*>(irods::RESOURCE_OP_STAT,
                                      function<error(plugin_context&, struct stat*)>(passthru_file_stat));

    resc->add_operation(irods::RESOURCE_OP_MKDIR, function<error(plugin_context&)>(passthru_file_mkdir));

    resc->add_operation(irods::RESOURCE_OP_OPENDIR, function<error(plugin_context&)>(passthru_file_opendir));

    resc->add_operation<struct rodsDirent**>(
        irods::RESOURCE_OP_READDIR, function<error(plugin_context&, struct rodsDirent**)>(passthru_file_readdir));

    resc->add_operation<const char*>(irods::RESOURCE_OP_RENAME,
                                     function<error(plugin_context&, const char*)>(passthru_file_rename));

    resc->add_operation(irods::RESOURCE_OP_FREESPACE, function<error(plugin_context&)>(passthru_file_getfs_freespace));

    resc->add_operation<const long long, const int>(
        irods::RESOURCE_OP_LSEEK, function<error(plugin_context&, const long long, const int)>(passthru_file_lseek));

    resc->add_operation(irods::RESOURCE_OP_RMDIR, function<error(plugin_context&)>(passthru_file_rmdir));

    resc->add_operation(irods::RESOURCE_OP_CLOSEDIR, function<error(plugin_context&)>(passthru_file_closedir));

    resc->add_operation<const char*>(irods::RESOURCE_OP_STAGETOCACHE,
                                     function<error(plugin_context&, const char*)>(passthru_file_stage_to_cache));

    resc->add_operation<const char*>(irods::RESOURCE_OP_SYNCTOARCH,
                                     function<error(plugin_context&, const char*)>(passthru_file_sync_to_arch));

    resc->add_operation(irods::RESOURCE_OP_REGISTERED, function<error(plugin_context&)>(passthru_file_registered));

    resc->add_operation(irods::RESOURCE_OP_UNREGISTERED, function<error(plugin_context&)>(passthru_file_unregistered));

    resc->add_operation(irods::RESOURCE_OP_MODIFIED, function<error(plugin_context&)>(passthru_file_modified));

    resc->add_operation<const std::string*>(irods::RESOURCE_OP_NOTIFY,
                                            function<error(plugin_context&, const std::string*)>(passthru_file_notify));

    resc->add_operation(irods::RESOURCE_OP_TRUNCATE, function<error(plugin_context&)>(passthru_file_truncate));
    ...
    resc->add_operation(irods::RESOURCE_OP_REBALANCE, function<error(plugin_context&)>(passthru_file_rebalance));
