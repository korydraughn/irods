...
inline irods::error compound_check_param(irods::plugin_context& _ctx)
{
    // =-=-=-=-=-=-=-
...
irods::error get_stage_policy(const std::string& _results, std::string& _policy)
{
    ...
    irods::kvp_map_t kvp;
    irods::error kvp_err = irods::parse_kvp_string(_results, kvp);
    if (!kvp_err.ok()) {
        return PASS(kvp_err);
    }
    ...
    if (value.empty()) {
        return ERROR(SYS_INVALID_INPUT_PARAM, "stage policy value not found");
    }
    _policy = value;
...
irods::error repl_object(irods::plugin_context& _ctx, const char* _stage_sync_kw)
{
    ...
    if ((result = ASSERT_ERROR(
             _stage_sync_kw || strlen(_stage_sync_kw) != 0, SYS_INVALID_INPUT_PARAM, "Null or empty _stage_sync_kw."))
            .ok()) {
        // =-=-=-=-=-=-=-
                ...
                if ((result = ASSERT_PASS(ret, "Failed to get the archive name.")).ok()) {
                    ...
                    if ((result = ASSERT_PASS(ret, "Failed to get the resource name.")).ok()) {
                        ...
                        if (std::string::npos == pos) {
                            std::stringstream msg;
                            msg << "parent resc [" << parent_name << "] not in fco resc hier [" << inp_hier << "]";
                            return ERROR(SYS_INVALID_INPUT_PARAM, msg.str());
                        }
                        ...
                        dst_hier += current_name + irods::hierarchy_parser::delimiter() + tgt_name;
...
irods::error compound_file_read(irods::plugin_context& _ctx, void* _buf, const int _len)
{
    // =-=-=-=-=-=-=-
    // check the context for validity
    irods::error ret = compound_check_param<irods::file_object>(_ctx);
...
irods::error compound_file_write(irods::plugin_context& _ctx, const void* _buf, const int _len)
{
    // =-=-=-=-=-=-=-
    // check the context for validity
    irods::error ret = compound_check_param<irods::file_object>(_ctx);
...
irods::error compound_file_lseek(irods::plugin_context& _ctx, const long long _offset, const int _whence)
{
    // =-=-=-=-=-=-=-
    // check the context for validity
    irods::error ret = compound_check_param<irods::file_object>(_ctx);
...
static bool auto_replication_is_enabled(irods::plugin_context& _ctx)
{
    ...
    irods::error ret = _ctx.prop_map().get<std::string>(AUTO_REPL_POLICY, auto_repl);
    if (ret.ok()) {
...
irods::error compound_file_notify(irods::plugin_context& _ctx, const std::string* _opr)
{
    ...
    if ((result = ASSERT_PASS(ret, "Invalid resource context.")).ok()) {
        ...
        ret = _ctx.prop_map().get<std::string>(OPERATION_TYPE, operation);
        if (ret.ok()) {
            rodsLog(LOG_DEBUG, "compound_file_notify - oper [%s] changed to [%s]", _opr->c_str(), operation.c_str());
        } // if ret ok
        if (irods::WRITE_OPERATION == (*_opr) || irods::CREATE_OPERATION == (*_opr)) {
            _ctx.prop_map().set<std::string>(OPERATION_TYPE, (*_opr));
        }
        else {
            rodsLog(LOG_DEBUG, "compound_file_notify - skipping [%s]", _opr->c_str());
        }
...
irods::error compound_file_redirect_create(irods::plugin_context& _ctx,
...
irods::error compound_file_redirect_unlink(irods::plugin_context& _ctx,
                                           const std::string& _operation,
                                           const std::string* _curr_host,
                                           irods::hierarchy_parser* _out_parser,
                                           float* _out_vote)
...
irods::error open_for_prefer_archive_policy(irods::plugin_context& _ctx,
                                            const std::string* _curr_host,
                                            irods::hierarchy_parser* _out_parser,
                                            float* _out_vote)
{
    ...
    if (!ret.ok() || 0.0 == arch_check_vote) {
        rodsLog(LOG_NOTICE, "replica not found in archive for [%s]", f_ptr->logical_path().c_str());
        // =-=-=-=-=-=-=-
...
irods::error open_for_prefer_cache_policy(irods::plugin_context& _ctx,
                                          ...
                                          float* _out_vote)
{
    ...
    ret = cache_resc->get_property<std::string>(irods::RESOURCE_NAME, cache_resc_name);
    if (!ret.ok()) {
        return PASS(ret);
    }
...
irods::error compound_file_redirect_open(irods::plugin_context& _ctx,
                                         const std::string* _opr,
                                         const std::string* _curr_host,
                                         irods::hierarchy_parser* _out_parser,
                                         float* _out_vote)
...
irods::error compound_file_resolve_hierarchy(irods::plugin_context& _ctx,
                                             const std::string* _opr,
                                             const std::string* _curr_host,
                                             irods::hierarchy_parser* _out_parser,
                                             float* _out_vote)
...
extern "C" irods::resource* plugin_factory(const std::string& _inst_name, const std::string& _context)
{
    ...
    using namespace std;
    resc->add_operation(RESOURCE_OP_CREATE, function<error(plugin_context&)>(compound_file_create));

    resc->add_operation(irods::RESOURCE_OP_OPEN, function<error(plugin_context&)>(compound_file_open));

    resc->add_operation<void*, const int>(
        irods::RESOURCE_OP_READ, std::function<error(irods::plugin_context&, void*, const int)>(compound_file_read));

    resc->add_operation<const void*, const int>(
        irods::RESOURCE_OP_WRITE, function<error(plugin_context&, const void*, const int)>(compound_file_write));

    resc->add_operation(RESOURCE_OP_CLOSE, function<error(plugin_context&)>(compound_file_close));

    resc->add_operation(irods::RESOURCE_OP_UNLINK, function<error(plugin_context&)>(compound_file_unlink));

    resc->add_operation<struct stat*>(irods::RESOURCE_OP_STAT,
                                      function<error(plugin_context&, struct stat*)>(compound_file_stat));

    resc->add_operation(irods::RESOURCE_OP_MKDIR, function<error(plugin_context&)>(compound_file_mkdir));

    resc->add_operation(irods::RESOURCE_OP_OPENDIR, function<error(plugin_context&)>(compound_file_opendir));

    resc->add_operation<struct rodsDirent**>(
        irods::RESOURCE_OP_READDIR, function<error(plugin_context&, struct rodsDirent**)>(compound_file_readdir));

    resc->add_operation<const char*>(irods::RESOURCE_OP_RENAME,
                                     function<error(plugin_context&, const char*)>(compound_file_rename));

    resc->add_operation(irods::RESOURCE_OP_FREESPACE, function<error(plugin_context&)>(compound_file_getfs_freespace));

    resc->add_operation<const long long, const int>(
        irods::RESOURCE_OP_LSEEK, function<error(plugin_context&, const long long, const int)>(compound_file_lseek));

    resc->add_operation(irods::RESOURCE_OP_RMDIR, function<error(plugin_context&)>(compound_file_rmdir));

    resc->add_operation(irods::RESOURCE_OP_CLOSEDIR, function<error(plugin_context&)>(compound_file_closedir));

    resc->add_operation<const char*>(irods::RESOURCE_OP_STAGETOCACHE,
                                     function<error(plugin_context&, const char*)>(compound_file_stage_to_cache));

    resc->add_operation<const char*>(irods::RESOURCE_OP_SYNCTOARCH,
                                     function<error(plugin_context&, const char*)>(compound_file_sync_to_arch));

    resc->add_operation(irods::RESOURCE_OP_REGISTERED, function<error(plugin_context&)>(compound_file_registered));

    resc->add_operation(irods::RESOURCE_OP_UNREGISTERED, function<error(plugin_context&)>(compound_file_unregistered));

    resc->add_operation(irods::RESOURCE_OP_MODIFIED, function<error(plugin_context&)>(compound_file_modified));

    resc->add_operation<const std::string*>(irods::RESOURCE_OP_NOTIFY,
                                            function<error(plugin_context&, const std::string*)>(compound_file_notify));

    resc->add_operation(irods::RESOURCE_OP_TRUNCATE, function<error(plugin_context&)>(compound_file_truncate));
    ...
    resc->add_operation(irods::RESOURCE_OP_REBALANCE, function<error(plugin_context&)>(compound_file_rebalance));
