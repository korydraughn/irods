...
irods::error non_blocking_check_path(irods::plugin_context& _ctx)
{
    ...
    if ((result = ASSERT_ERROR(data_obj.get(), SYS_INVALID_INPUT_PARAM, "Failed to cast fco to data_object.")).ok()) {
        ...
        irods::error ret = non_blocking_generate_full_path(_ctx.prop_map(), data_obj->physical_path(), full_path);
...
#if defined(solaris_platform) || defined(sgi_platform) || defined(aix_platform) || defined(linux_platform) ||          \
...
irods::error non_blocking_file_read(irods::plugin_context& _ctx, void* _buf, const int _len)
{
    irods::error result = SUCCESS();
...
irods::error non_blocking_file_write(irods::plugin_context& _ctx, const void* _buf, const int _len)
{
    irods::error result = SUCCESS();
...
irods::error non_blocking_file_lseek(irods::plugin_context& _ctx, const long long _offset, const int _whence)
{
    irods::error result = SUCCESS();
...
irods::error non_blocking_file_redirect_create(irods::plugin_property_map& _prop_map,
...
irods::error non_blocking_file_redirect_open(irods::plugin_property_map& _prop_map,
...
irods::error non_blocking_file_resolve_hierarchy(irods::plugin_context& _ctx,
...
extern "C" irods::resource* plugin_factory(const std::string& _inst_name, const std::string& _context)
{
    ...
    using namespace std;
    resc->add_operation(RESOURCE_OP_CREATE, function<error(plugin_context&)>(non_blocking_file_create));

    resc->add_operation(irods::RESOURCE_OP_OPEN, function<error(plugin_context&)>(non_blocking_file_open));

    resc->add_operation<void*, const int>(
        irods::RESOURCE_OP_READ,
        std::function<error(irods::plugin_context&, void*, const int)>(non_blocking_file_read));

    resc->add_operation<const void*, const int>(
        irods::RESOURCE_OP_WRITE, function<error(plugin_context&, const void*, const int)>(non_blocking_file_write));

    resc->add_operation(RESOURCE_OP_CLOSE, function<error(plugin_context&)>(non_blocking_file_close));

    resc->add_operation(irods::RESOURCE_OP_UNLINK, function<error(plugin_context&)>(non_blocking_file_unlink));

    resc->add_operation<struct stat*>(irods::RESOURCE_OP_STAT,
                                      function<error(plugin_context&, struct stat*)>(non_blocking_file_stat));

    resc->add_operation(irods::RESOURCE_OP_MKDIR, function<error(plugin_context&)>(non_blocking_file_mkdir));

    resc->add_operation(irods::RESOURCE_OP_OPENDIR, function<error(plugin_context&)>(non_blocking_file_opendir));

    resc->add_operation<struct rodsDirent**>(
        irods::RESOURCE_OP_READDIR, function<error(plugin_context&, struct rodsDirent**)>(non_blocking_file_readdir));

    resc->add_operation<const char*>(irods::RESOURCE_OP_RENAME,
                                     function<error(plugin_context&, const char*)>(non_blocking_file_rename));

    resc->add_operation(irods::RESOURCE_OP_FREESPACE,
                        function<error(plugin_context&)>(non_blocking_file_getfs_freespace));

    resc->add_operation<const long long, const int>(
        irods::RESOURCE_OP_LSEEK,
        function<error(plugin_context&, const long long, const int)>(non_blocking_file_lseek));

    resc->add_operation(irods::RESOURCE_OP_RMDIR, function<error(plugin_context&)>(non_blocking_file_rmdir));

    resc->add_operation(irods::RESOURCE_OP_CLOSEDIR, function<error(plugin_context&)>(non_blocking_file_closedir));

    resc->add_operation<const char*>(irods::RESOURCE_OP_STAGETOCACHE,
                                     function<error(plugin_context&, const char*)>(non_blocking_file_stage_to_cache));

    resc->add_operation<const char*>(irods::RESOURCE_OP_SYNCTOARCH,
                                     function<error(plugin_context&, const char*)>(non_blocking_file_sync_to_arch));

    resc->add_operation(irods::RESOURCE_OP_REGISTERED, function<error(plugin_context&)>(non_blocking_file_registered));

    resc->add_operation(irods::RESOURCE_OP_UNREGISTERED,
                        function<error(plugin_context&)>(non_blocking_file_unregistered));

    resc->add_operation(irods::RESOURCE_OP_MODIFIED, function<error(plugin_context&)>(non_blocking_file_modified));

    resc->add_operation<const std::string*>(
        irods::RESOURCE_OP_NOTIFY, function<error(plugin_context&, const std::string*)>(non_blocking_file_notify));

    resc->add_operation(irods::RESOURCE_OP_TRUNCATE, function<error(plugin_context&)>(non_blocking_file_truncate));
    ...
    resc->add_operation(irods::RESOURCE_OP_REBALANCE, function<error(plugin_context&)>(non_blocking_file_rebalance));
