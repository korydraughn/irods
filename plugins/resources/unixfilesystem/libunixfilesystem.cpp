...
static irods::error unix_file_copy(const int mode, const char* srcFileName, const char* destFileName)
{
    ...
    if (inFd < 0) {
        ...
        if (status < 0) {
            ...
            if (outFd < 0) {
                ...
                try {
                    trans_buff_size =
                        irods::get_advanced_setting<const int>(irods::CFG_TRANS_BUFFER_SIZE_FOR_PARA_TRANS) * 1024 *
                        1024;
                }
                ...
                while (result.ok() && (bytesRead = read(inFd, (void*) myBuf.data(), trans_buff_size)) > 0) {
                    ...
                    if ((result = ASSERT_ERROR(bytesWritten > 0,
                                               err_status,
                ...
                if (result.ok()) {
                    result = ASSERT_ERROR(bytesCopied == statbuf.st_size,
                                          SYS_COPY_LEN_ERR,
...
irods::error unix_generate_full_path(irods::plugin_property_map& _prop_map,
                                     ...
                                     std::string& _ret_string)
{
    ...
    if ((result = ASSERT_ERROR(ret.ok(), SYS_INVALID_INPUT_PARAM, "resource has no vault path.")).ok()) {
        if (_phy_path.compare(0, 1, "/") != 0 && _phy_path.compare(0, vault_path.size(), vault_path) != 0) {
...
irods::error unix_check_path(irods::plugin_context& _ctx)
{
    ...
    if ((result = ASSERT_ERROR(data_obj.get(), SYS_INVALID_INPUT_PARAM, "Failed to cast fco to data_object.")).ok()) {
        ...
        irods::error ret = unix_generate_full_path(_ctx.prop_map(), data_obj->physical_path(), full_path);
...
irods::error unix_file_mkdir_r(const std::string& path, mode_t mode)
{
    ...
    while (!done && result.ok()) {
        ...
        if (pos > 0) {
            ...
            // =-=-=-=-=-=-=-
            // handle error cases
            result = ASSERT_ERROR(status >= 0 || errno == EEXIST,
                                  UNIX_FILE_RENAME_ERR - errno,
...
#    endif
        ...
        if ((result = ASSERT_ERROR(
                 status >= 0, err_status, "Statfs error for \"%s\", status = %d.", path.c_str(), err_status))
                .ok()) {
...
#    if defined(aix_platform) || defined(osx_platform) || defined(linux_platform)
...
irods::error stat_vault_path(const std::string& _path, struct statfs& _sb)
{
    ...
    if (err < 0) {
        ...
        msg << "statfs failed for [" << _path << "] errno " << errno;
...
void warn_if_deprecated_context_string_set(irods::plugin_context& _ctx)
{
    ...
    if (ret.code() != KEY_NOT_FOUND) {
        rodsLog(LOG_NOTICE,
                "warn_if_deprecated_context_string_set: resource [%s] is using deprecated context string [%s]",
                resource_name.c_str(),
    ...
    if (ret.code() != KEY_NOT_FOUND) {
        rodsLog(LOG_NOTICE,
                "warn_if_deprecated_context_string_set: resource [%s] is using deprecated context string [%s]",
                resource_name.c_str(),
                REQUIRED_FREE_INODES_FOR_CREATE.c_str());
    }
}
...
static bool replica_exceeds_resource_free_space(irods::plugin_context& _ctx, rodsLong_t _file_size)
{
    ...
    irods::error err =
        _ctx.prop_map().get<std::string>(MINIMUM_FREE_SPACE_FOR_CREATE_IN_BYTES, minimum_free_space_string);
    if (err.code() == KEY_NOT_FOUND) {
        return false;
    }
    else if (!err.ok()) {
        rodsLog(LOG_ERROR,
                "replica_exceeds_resource_free_space: failed to get MINIMUM_FREE_SPACE_FOR_CREATE_IN_BYTES property "
                "for resource [%s]",
                resource_name.c_str());
        irods::log(err);
        return true;
    ...
    if (minimum_free_space_string.size() > 0 &&
        minimum_free_space_string[0] == '-') { // do sign check on string because boost::lexical_cast will wrap negative
                                               // numbers around instead of throwing when casting string to unsigned
        rodsLog(LOG_ERROR,
                "replica_exceeds_resource_free_space: minimum free space < 0 [%s] for resource [%s]",
    ...
    try {
        ...
        rodsLog(LOG_ERROR,
                "replica_exceeds_resource_free_space: invalid MINIMUM_FREE_SPACE_FOR_CREATE_IN_BYTES [%s] for resource "
                "[%s]",
                minimum_free_space_string.c_str(),
                resource_name.c_str());
        return true;
    ...
    if (!err.ok()) {
        rodsLog(LOG_ERROR,
                "replica_exceeds_resource_free_space: minimum free space constraint was requested, and failed to get "
                "resource free space for resource [%s]",
                resource_name.c_str());
        irods::log(err);
        return true;
    }

    if (resource_free_space_string.size() > 0 &&
        resource_free_space_string[0] ==
            '-') { // do sign check on string because boost::lexical_cast will wrap negative numbers around instead of
                   // throwing when casting string to unsigned
        rodsLog(LOG_ERROR,
                "replica_exceeds_resource_free_space: resource free space < 0 [%s] for resource [%s]",
    ...
    try {
        ...
        rodsLog(LOG_ERROR,
                "replica_exceeds_resource_free_space: invalid free space [%s] for resource [%s]",
                resource_free_space_string.c_str(),
                resource_name.c_str());
        return true;
    }
...
irods::error unix_file_create(irods::plugin_context& _ctx)
{
    ...
    if ((result = ASSERT_PASS(ret, "Invalid parameters or physical path.")).ok()) {
        ...
        char* kvp_str = getValByKey(&fco->cond_input(), KEY_VALUE_PASSTHROUGH_KW);
        if (kvp_str) {
            irods::kvp_map_t kvp;
            ret = irods::parse_kvp_string(kvp_str, kvp);
            if (!ret.ok()) {
                irods::log(PASS(ret));
            }
            else {
        ...
        if ((result = ASSERT_PASS(ret, "Error determining freespace on system.")).ok()) {
            ...
            if ((result = ASSERT_ERROR(file_size < 0 || ret.code() >= file_size,
                                       USER_FILE_TOO_LARGE,
                                       "File size: %ld is greater than space left on device: %ld",
                                       file_size,
                                       ret.code()))
                    .ok()) {
                // =-=-=-=-=-=-=-
...
irods::error unix_file_read(irods::plugin_context& _ctx, void* _buf, const int _len)
{
    irods::error result = SUCCESS();
    ...
    if ((result = ASSERT_PASS(ret, "Invalid parameters or physical path.")).ok()) {
        ...
        if (!(result = ASSERT_ERROR(status >= 0,
                                    err_status,
...
irods::error unix_file_write(irods::plugin_context& _ctx, const void* _buf, const int _len)
{
    irods::error result = SUCCESS();
...
irods::error unix_file_lseek(irods::plugin_context& _ctx, const long long _offset, const int _whence)
{
    irods::error result = SUCCESS();
...
irods::error unix_file_readdir(irods::plugin_context& _ctx, struct rodsDirent** _dirent_ptr)
{
    ...
    if ((result = ASSERT_PASS(ret, "Invalid parameters or physical path.")).ok()) {
        ...
        if ((result = ASSERT_ERROR(tmp_dirent != NULL, -1, "End of directory list reached.")).ok()) {
            ...
            if ((result = ASSERT_ERROR(
                     errno == 0, status, "Readdir error, status = %d, errno= \"%s\".", status, strerror(errno)))
                    .ok()) {
                result.code(-1);
            }
        }
...
irods::error unix_file_rename(irods::plugin_context& _ctx, const char* _new_file_name)
{
    ...
    if ((result = ASSERT_PASS(ret, "Invalid parameters or physical path.")).ok()) {
        ...
        if ((result = ASSERT_PASS(ret, "Unable to generate full path for destination file: \"%s\".", _new_file_name))
                .ok()) {
            // =-=-=-=-=-=-=-
            // cast down the hierarchy to the desired object
            irods::file_object_ptr fco = boost::dynamic_pointer_cast<irods::file_object>(_ctx.fco());
            ...
            mode_t mode = 0750;
            ret = _ctx.prop_map().get<mode_t>(DEFAULT_VAULT_DIR_MODE, mode);
            if (!ret.ok()) {
                return PASS(ret);
            }
...
irods::error unix_resolve_hierarchy_create(irods::plugin_context& _ctx,
...
irods::error unix_resolve_hierarchy_open(irods::plugin_property_map& _prop_map,
...
irods::error unix_file_resolve_hierarchy(irods::plugin_context& _ctx,
                                         ...
                                         float* _out_vote)
{
    ...
    rodsLog(LOG_DEBUG,
            ...
            __FUNCTION__,
            RECURSIVE_OPR__KW,
            getValByKey(&file_obj->cond_input(), RECURSIVE_OPR__KW));
    ...
    if (irods::OPEN_OPERATION == (*_opr) || irods::WRITE_OPERATION == (*_opr) || irods::UNLINK_OPERATION == (*_opr)) {
        // =-=-=-=-=-=-=-
...
extern "C" irods::resource* plugin_factory(const std::string& _inst_name, const std::string& _context)
{
    ...
    using namespace std;
    resc->add_operation(RESOURCE_OP_CREATE, function<error(plugin_context&)>(unix_file_create));

    resc->add_operation(irods::RESOURCE_OP_OPEN, function<error(plugin_context&)>(unix_file_open));

    resc->add_operation<void*, const int>(
        irods::RESOURCE_OP_READ, std::function<error(irods::plugin_context&, void*, const int)>(unix_file_read));

    resc->add_operation<const void*, const int>(
        irods::RESOURCE_OP_WRITE, function<error(plugin_context&, const void*, const int)>(unix_file_write));

    resc->add_operation(RESOURCE_OP_CLOSE, function<error(plugin_context&)>(unix_file_close));

    resc->add_operation(irods::RESOURCE_OP_UNLINK, function<error(plugin_context&)>(unix_file_unlink));

    resc->add_operation<struct stat*>(irods::RESOURCE_OP_STAT,
                                      function<error(plugin_context&, struct stat*)>(unix_file_stat));

    resc->add_operation(irods::RESOURCE_OP_MKDIR, function<error(plugin_context&)>(unix_file_mkdir));

    resc->add_operation(irods::RESOURCE_OP_OPENDIR, function<error(plugin_context&)>(unix_file_opendir));

    resc->add_operation<struct rodsDirent**>(irods::RESOURCE_OP_READDIR,
                                             function<error(plugin_context&, struct rodsDirent**)>(unix_file_readdir));

    resc->add_operation<const char*>(irods::RESOURCE_OP_RENAME,
                                     function<error(plugin_context&, const char*)>(unix_file_rename));

    resc->add_operation(irods::RESOURCE_OP_FREESPACE, function<error(plugin_context&)>(unix_file_getfs_freespace));

    resc->add_operation<const long long, const int>(
        irods::RESOURCE_OP_LSEEK, function<error(plugin_context&, const long long, const int)>(unix_file_lseek));

    resc->add_operation(irods::RESOURCE_OP_RMDIR, function<error(plugin_context&)>(unix_file_rmdir));

    resc->add_operation(irods::RESOURCE_OP_CLOSEDIR, function<error(plugin_context&)>(unix_file_closedir));

    resc->add_operation<const char*>(irods::RESOURCE_OP_STAGETOCACHE,
                                     function<error(plugin_context&, const char*)>(unix_file_stage_to_cache));

    resc->add_operation<const char*>(irods::RESOURCE_OP_SYNCTOARCH,
                                     function<error(plugin_context&, const char*)>(unix_file_sync_to_arch));

    resc->add_operation(irods::RESOURCE_OP_REGISTERED, function<error(plugin_context&)>(unix_file_registered));

    resc->add_operation(irods::RESOURCE_OP_UNREGISTERED, function<error(plugin_context&)>(unix_file_unregistered));

    resc->add_operation(irods::RESOURCE_OP_MODIFIED, function<error(plugin_context&)>(unix_file_modified));

    resc->add_operation<const std::string*>(irods::RESOURCE_OP_NOTIFY,
                                            function<error(plugin_context&, const std::string*)>(unix_file_notify));

    resc->add_operation(irods::RESOURCE_OP_TRUNCATE, function<error(plugin_context&)>(unix_file_truncate));
    ...
    resc->add_operation(irods::RESOURCE_OP_REBALANCE, function<error(plugin_context&)>(unix_file_rebalance));
