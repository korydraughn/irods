...
inline irods::error tar_check_params(irods::plugin_context& _ctx)
{
    // =-=-=-=-=-=-=-
...
// irods rsFile API for file access
ssize_t irods_file_read(struct archive* _arch, void* _data, const void** _buff)
{
    if (!_arch || !_data || !_buff) {
    ...
    if (status != 0) {
        if (status != UNIX_FILE_STAT_ERR - ENOENT) {
            rodsLog(LOG_DEBUG, "irods_file_read: can't stat %s. status = %d", f_inp.fileName, status);
        }
        return -1;
    }
...
ssize_t irods_file_write(struct archive* _arch, void* _data, const void* _buff, size_t _len)
{
    ...
    write_buf.len = _len;
    int sz = rsFileWrite(cb_ctx->desc_->rsComm, &fileWriteInp, &write_buf);
    if (sz < 0) {
        return -1;
    }
    else {
...
irods::error extract_file(int _index)
{
    ...
    if (spec_coll == NULL || strlen(spec_coll->cacheDir) == 0 || strlen(spec_coll->phyPath) == 0) {
        std::stringstream msg;
    ...
    if (archive_read_open(arch, &cb_ctx, irods_file_open_for_read, irods_file_read, irods_file_close) != ARCHIVE_OK) {
        std::stringstream msg;
...
irods::error make_tar_cache_dir(int _index, std::string _host)
{
    ...
    while (true) {
        ...
        snprintf(fileMkdirInp.dirName, MAX_NAME_LEN, "%s.%s%d", spec_coll->phyPath, CACHE_DIR_STR, dir_index);
...
irods::error stage_tar_struct_file(int _index, std::string _host)
{
    ...
    if (strlen(spec_coll->cacheDir) == 0) {
        ...
        if (hasSymlinkInDir(spec_coll->cacheDir)) {
            rodsLog(LOG_ERROR, "extractTarFile: cacheDir %s has symlink in it", spec_coll->cacheDir);
...
int free_struct_file_desc(int _idx)
{
    if (_idx < 0 || _idx >= NUM_STRUCT_FILE_DESC) {
        rodsLog(LOG_NOTICE, "free_struct_file_desc: index %d out of range", _idx);
...
{
    for (int i = 1; i < NUM_STRUCT_FILE_DESC; i++) {
        if (PluginStructFileDesc[i].inuseFlag == FD_INUSE && PluginStructFileDesc[i].specColl != NULL &&
...
irods::error tar_struct_file_open(rsComm_t* _comm,
...
int alloc_tar_sub_file_desc()
{
    ...
    rodsLog(LOG_NOTICE, "alloc_tar_sub_file_desc: out of PluginTarSubFileDesc");
...
int free_tar_sub_file_desc(int _idx)
{
    if (_idx < 0 || _idx >= NUM_TAR_SUB_FILE_DESC) {
        rodsLog(LOG_NOTICE, "free_tar_sub_file_desc: index %d out of range", _idx);
        return SYS_FILE_DESC_OUT_OF_RANGE;
    }
...
irods::error tar_file_create(irods::plugin_context& _ctx)
{
    ...
    irods::error open_err = tar_struct_file_open(comm, spec_coll, struct_file_index, fco->resc_hier(), resc_host);
    if (!open_err.ok()) {
        std::stringstream msg;
    ...
    irods::error comp_err =
        compose_cache_dir_physical_path(fileCreateInp.fileName, spec_coll, fco->sub_file_path().c_str());
    if (!comp_err.ok()) {
...
irods::error tar_file_open(irods::plugin_context& _ctx)
{
    ...
    // build a physical path name to the cache dir
    irods::error comp_err =
        compose_cache_dir_physical_path(fileOpenInp.fileName, spec_coll, fco->sub_file_path().c_str());
    if (!comp_err.ok()) {
        return PASSMSG("compose_cache_dir_physical_path failed.", comp_err);
    }
...
irods::error tar_file_read(irods::plugin_context& _ctx, void* _buf, const int _len)
{
    // =-=-=-=-=-=-=-
    // check incoming parameters
    irods::error chk_err = tar_check_params(_ctx);
    ...
    if (fco->file_descriptor() < 1 || fco->file_descriptor() >= NUM_TAR_SUB_FILE_DESC ||
...
irods::error tar_file_write(irods::plugin_context& _ctx, const void* _buf, const int _len)
{
    // =-=-=-=-=-=-=-
    // check incoming parameters
    irods::error chk_err = tar_check_params(_ctx);
    ...
    const fileWriteInp_t fileWriteInp{.len = _len, .fileInx = PluginTarSubFileDesc[fco->file_descriptor()].fd};
    const bytesBuf_t fileWriteInpBBuf{.len = _len, .buf = const_cast<void*>(_buf)};
...
irods::error tar_file_unlink(irods::plugin_context& _ctx)
{
    ...
    // build a physical path name to the cache dir
    irods::error comp_err =
        compose_cache_dir_physical_path(fileUnlinkInp.fileName, spec_coll, fco->sub_file_path().c_str());
    if (!comp_err.ok()) {
        return PASSMSG("tar_file_unlink_plugin - compose_cache_dir_physical_path failed.", comp_err);
    }
...
irods::error tar_file_stat(irods::plugin_context& _ctx, struct stat* _statbuf)
{
    ...
    // build a physical path name to the cache dir
    irods::error comp_err =
        compose_cache_dir_physical_path(fileStatInp.fileName, spec_coll, fco->sub_file_path().c_str());
    if (!comp_err.ok()) {
        return PASSMSG("tar_file_stat_plugin - compose_cache_dir_physical_path failed.", comp_err);
    }
...
irods::error tar_file_lseek(irods::plugin_context& _ctx, const long long _offset, const int _whence)
{
    // =-=-=-=-=-=-=-
    // check incoming parameters
    irods::error chk_err = tar_check_params(_ctx);
...
irods::error tar_file_mkdir(irods::plugin_context& _ctx)
{
    ...
    // build a physical path name to the cache dir
    irods::error comp_err =
        compose_cache_dir_physical_path(fileMkdirInp.dirName, spec_coll, fco->sub_file_path().c_str());
    if (!comp_err.ok()) {
        return PASSMSG("tar_file_mkdir_plugin - compose_cache_dir_physical_path failed.", comp_err);
    }
...
irods::error tar_file_rmdir(irods::plugin_context& _ctx)
{
    ...
    // build a physical path name to the cache dir
    irods::error comp_err =
        compose_cache_dir_physical_path(fileRmdirInp.dirName, spec_coll, fco->sub_file_path().c_str());
    if (!comp_err.ok()) {
        return PASSMSG("tar_file_rmdir_plugin - compose_cache_dir_physical_path failed.", comp_err);
    }
...
irods::error tar_file_opendir(irods::plugin_context& _ctx)
{
    ...
    // build a physical path name to the cache dir
    irods::error comp_err =
        compose_cache_dir_physical_path(fileOpendirInp.dirName, spec_coll, fco->sub_file_path().c_str());
    if (!comp_err.ok()) {
        return PASSMSG("tarFileRmdirPlugin - compose_cache_dir_physical_path failed.", comp_err);
    }
...
irods::error tar_file_rename(irods::plugin_context& _ctx, const char* _new_file_name)
{
    ...
    // =-=-=-=-=-=-=-
    // build a physical path name to the cache dir
    irods::error comp_err_old =
        compose_cache_dir_physical_path(fileRenameInp.oldFileName, spec_coll, fco->sub_file_path().c_str());
    if (!comp_err_old.ok()) {
        return PASSMSG("tar_file_rename_plugin - compose_cache_dir_physical_path failed for old file name.",
                       comp_err_old);
    }

    irods::error comp_err_new = compose_cache_dir_physical_path(fileRenameInp.newFileName, spec_coll, _new_file_name);
    if (!comp_err_new.ok()) {
        return PASSMSG("tar_file_rename_plugin - compose_cache_dir_physical_path failed for new file name.",
                       comp_err_new);
    }
...
irods::error tar_file_truncate(irods::plugin_context& _ctx)
{
    ...
    int struct_file_index = 0;
    std::string resc_host;
    irods::error open_err =
        tar_struct_file_open(comm, spec_coll, struct_file_index, struct_obj->resc_hier(), resc_host);
    if (!open_err.ok()) {
        std::stringstream msg;
    ...
    // build a physical path name to the cache dir
    irods::error comp_err =
        compose_cache_dir_physical_path(fileTruncateInp.fileName, spec_coll, struct_obj->sub_file_path().c_str());
    if (!comp_err.ok()) {
        return PASSMSG("tar_file_truncate_plugin - compose_cache_dir_physical_path failed.", comp_err);
    }
...
irods::error bundle_cache_dir(int _index, std::string _data_type)
{
    ...
    // check struct desc table in use flag
    specColl_t* spec_coll = PluginStructFileDesc[_index].specColl;
    if (spec_coll == NULL || spec_coll->cacheDirty <= 0 || strlen(spec_coll->cacheDir) == 0 ||
    ...
    if (archive_write_open(arch, &cb_ctx, irods_file_open_for_write, irods_file_write, irods_file_close) < ARCHIVE_OK) {
        std::stringstream msg;
...
irods::error tar_file_sync(irods::plugin_context& _ctx)
{
    ...
    if (strlen(spec_coll->cacheDir) > 0) {
        if (spec_coll->cacheDirty > 0) {
            ...
            irods::error sync_err = sync_cache_dir_to_tar_file(struct_file_index, fco->opr_type(), resc_host);
            if (!sync_err.ok()) {
                std::stringstream msg;
...
extern "C" irods::resource* plugin_factory(const std::string& _inst_name, const std::string& _context)
{
    ...
    using namespace std;
    resc->add_operation(RESOURCE_OP_CREATE, function<error(plugin_context&)>(tar_file_create));

    resc->add_operation(irods::RESOURCE_OP_OPEN, function<error(plugin_context&)>(tar_file_open));

    resc->add_operation<void*, const int>(
        irods::RESOURCE_OP_READ, std::function<error(irods::plugin_context&, void*, const int)>(tar_file_read));

    resc->add_operation<const void*, const int>(
        irods::RESOURCE_OP_WRITE, function<error(plugin_context&, const void*, const int)>(tar_file_write));

    resc->add_operation(RESOURCE_OP_CLOSE, function<error(plugin_context&)>(tar_file_close));

    resc->add_operation(irods::RESOURCE_OP_UNLINK, function<error(plugin_context&)>(tar_file_unlink));

    resc->add_operation<struct stat*>(irods::RESOURCE_OP_STAT,
                                      function<error(plugin_context&, struct stat*)>(tar_file_stat));

    resc->add_operation(irods::RESOURCE_OP_MKDIR, function<error(plugin_context&)>(tar_file_mkdir));

    resc->add_operation(irods::RESOURCE_OP_OPENDIR, function<error(plugin_context&)>(tar_file_opendir));

    resc->add_operation<struct rodsDirent**>(irods::RESOURCE_OP_READDIR,
                                             function<error(plugin_context&, struct rodsDirent**)>(tar_file_readdir));

    resc->add_operation<const char*>(irods::RESOURCE_OP_RENAME,
                                     function<error(plugin_context&, const char*)>(tar_file_rename));

    resc->add_operation(irods::RESOURCE_OP_FREESPACE, function<error(plugin_context&)>(tar_file_getfs_freespace));

    resc->add_operation<const long long, const int>(
        irods::RESOURCE_OP_LSEEK, function<error(plugin_context&, const long long, const int)>(tar_file_lseek));

    resc->add_operation(irods::RESOURCE_OP_RMDIR, function<error(plugin_context&)>(tar_file_rmdir));

    resc->add_operation(irods::RESOURCE_OP_CLOSEDIR, function<error(plugin_context&)>(tar_file_closedir));

    resc->add_operation(irods::RESOURCE_OP_REGISTERED, function<error(plugin_context&)>(tar_file_registered));

    resc->add_operation(irods::RESOURCE_OP_UNREGISTERED, function<error(plugin_context&)>(tar_file_unregistered));

    resc->add_operation(irods::RESOURCE_OP_MODIFIED, function<error(plugin_context&)>(tar_file_modified));

    resc->add_operation<const std::string*>(irods::RESOURCE_OP_NOTIFY,
                                            function<error(plugin_context&, const std::string*)>(tar_file_notify));

    resc->add_operation(irods::RESOURCE_OP_TRUNCATE, function<error(plugin_context&)>(tar_file_truncate));

    resc->add_operation(irods::RESOURCE_OP_REBALANCE, function<error(plugin_context&)>(tar_file_rebalance));

    // =-=-=-=-=-=-=-
    // struct file specific operations
    resc->add_operation("extract", function<error(plugin_context&)>(tar_file_extract));

    resc->add_operation("sync", function<error(plugin_context&)>(tar_file_sync));
