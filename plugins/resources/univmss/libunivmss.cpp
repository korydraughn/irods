...
inline irods::error univ_mss_check_param(irods::plugin_context& _ctx)
{
    // =-=-=-=-=-=-=-
...
irods::error univ_mss_file_rename(irods::plugin_context& _ctx, const char* _new_file_name)
{
    ...
    irods::collection_object_ptr coll_obj(new irods::collection_object(dirname, fco->resc_hier(), fco->mode(), 0));
    irods::plugin_context context(_ctx.prop_map(), coll_obj, "");
...
irods::error univ_mss_file_resolve_hierarchy_create(irods::plugin_property_map& _prop_map,
...
// =-=-=-=-=-=-=-
// redirect_get - code to determine redirection for get operation
irods::error univ_mss_file_resolve_hierarchy_open(irods::plugin_property_map& _prop_map,
...
irods::error univ_mss_file_resolve_hierarchy(irods::plugin_context& _ctx,
...
extern "C" irods::resource* plugin_factory(const std::string& _inst_name, const std::string& _context)
{
    ...
    resc->add_operation(irods::RESOURCE_OP_UNLINK, function<error(plugin_context&)>(univ_mss_file_unlink));

    resc->add_operation<const char*>(irods::RESOURCE_OP_STAGETOCACHE,
                                     function<error(plugin_context&, const char*)>(univ_mss_file_stage_to_cache));

    resc->add_operation<const char*>(irods::RESOURCE_OP_SYNCTOARCH,
                                     function<error(plugin_context&, const char*)>(univ_mss_file_sync_to_arch));

    resc->add_operation(irods::RESOURCE_OP_MKDIR, function<error(plugin_context&)>(univ_mss_file_mkdir));

    resc->add_operation<const char*>(irods::RESOURCE_OP_RENAME,
                                     function<error(plugin_context&, const char*)>(univ_mss_file_rename));

    resc->add_operation<struct stat*>(irods::RESOURCE_OP_STAT,
                                      function<error(plugin_context&, struct stat*)>(univ_mss_file_stat));

    resc->add_operation(irods::RESOURCE_OP_TRUNCATE, function<error(plugin_context&)>(univ_mss_file_truncate));
    ...
    resc->add_operation(irods::RESOURCE_OP_REBALANCE, function<error(plugin_context&)>(univ_mss_file_rebalance));
