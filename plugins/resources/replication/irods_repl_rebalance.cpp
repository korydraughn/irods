...
namespace
{
    irods::error repl_for_rebalance(irods::plugin_context& _ctx,
                                    ...
                                    const int _mode)
    {
        ...
        try {
            ...
            if (status < 0) {
                return ERROR(status, boost::format("Failed to replicate the data object [%s]") % _obj_path);
            }
        }
    ...
    sqlResult_t* extract_sql_result(const genQueryInp_t& genquery_inp,
                                    genQueryOut_t* genquery_out_ptr,
                                    const int column_number)
    {
        ...
        THROW(
            ...
            boost::format("getSqlResultByInx failed. column [%d] genquery_inp contents:\n%s\n\n possible iquest [%s]") %
                column_number % genquery_inp_to_diagnostic_string(&genquery_inp) %
    ...
    // throws irods::exception
    ReplicationSourceInfo get_source_data_object_attributes(rsComm_t* _comm,
                                                            ...
                                                            const std::vector<leaf_bundle_t>& _leaf_bundles)
    {
        ...
        addInxVal(&genquery_inp_wrapped.get().sqlCondInp,
                  COL_D_DATA_ID,
                  (boost::format("= '%lld'") % _data_id).str().c_str());
        ...
        sqlResult_t* data_name_result =
            extract_sql_result(genquery_inp_wrapped.get(), genquery_out_ptr_wrapped.get(), COL_DATA_NAME);
        char* data_name = &data_name_result->value[0];

        sqlResult_t* coll_name_result =
            extract_sql_result(genquery_inp_wrapped.get(), genquery_out_ptr_wrapped.get(), COL_COLL_NAME);
            ...
            try {
                ...
                THROW(INVALID_LEXICAL_CAST,
                      boost::format("boost::lexical_cast failed. tried to cast [%s]. genquery_inp contents:\n%s\n\n "
                                    "possible iquest [%s]") %
                          s % genquery_inp_to_diagnostic_string(&genquery_inp_wrapped.get()) %
                          genquery_inp_to_iquest_string(&genquery_inp_wrapped.get()));
            }
        };

        sqlResult_t* resc_id_result =
            extract_sql_result(genquery_inp_wrapped.get(), genquery_out_ptr_wrapped.get(), COL_D_RESC_ID);
        const rodsLong_t resc_id = cast_genquery_result(&resc_id_result->value[0]);

        sqlResult_t* data_mode_result =
            extract_sql_result(genquery_inp_wrapped.get(), genquery_out_ptr_wrapped.get(), COL_DATA_MODE);
                                                                 ...
                                                                 const int _batch_size)
    {
        ...
        sqlResult_t* data_id_results =
            extract_sql_result(genquery_inp_wrapped.get(), genquery_out_ptr_wrapped.get(), COL_D_DATA_ID);
        sqlResult_t* data_repl_num_results =
            extract_sql_result(genquery_inp_wrapped.get(), genquery_out_ptr_wrapped.get(), COL_DATA_REPL_NUM);
        sqlResult_t* data_resc_id_results =
            extract_sql_result(genquery_inp_wrapped.get(), genquery_out_ptr_wrapped.get(), COL_D_RESC_ID);
        ...
        for (int i = 0; i < genquery_out_ptr_wrapped.get()->rowCnt; ++i) {
            ...
            auto cast_genquery_result = [&genquery_inp_wrapped](int i, char* s) {
                try {
                    ...
                    THROW(INVALID_LEXICAL_CAST,
                          boost::format("boost::lexical_cast failed. index [%d]. tried to cast [%s]. genquery_inp "
                                        "contents:\n%s\npossible iquest [%s]") %
                              i % s % genquery_inp_to_diagnostic_string(&genquery_inp_wrapped.get()) %
                              genquery_inp_to_iquest_string(&genquery_inp_wrapped.get()));
                }
            };
            repl_and_resc.data_id = cast_genquery_result(i, &data_id_results->value[data_id_results->len * i]);
            repl_and_resc.replica_number =
                cast_genquery_result(i, &data_repl_num_results->value[data_repl_num_results->len * i]);
            repl_and_resc.resource_id =
                cast_genquery_result(i, &data_resc_id_results->value[data_resc_id_results->len * i]);
    ...
    // throws irods::exception
    std::string get_child_name_that_is_ancestor_of_bundle(const std::string& _resc_name, const leaf_bundle_t& _bundle)
    {
        std::string hier;
    ...
    std::string leaf_bundles_to_string(const std::vector<leaf_bundle_t>& _leaf_bundles)
    {
        std::stringstream ss;
                                    ...
                                    const dist_child_result_t& _data_ids_to_replicate)
    {
        if (!_ctx.comm()) {
            THROW(SYS_INVALID_INPUT_PARAM,
                  boost::format(
                      ...
                      _parent_resc_name % _child_resc_name % _bun_idx % leaf_bundles_to_string(_bundles));
        }
        ...
        for (auto data_id_to_replicate : _data_ids_to_replicate) {
            const ReplicationSourceInfo source_info =
                get_source_data_object_attributes(_ctx.comm(), data_id_to_replicate, _bundles);

            // create a file object so we can resolve a valid hierarchy to which to replicate
            irods::file_object_ptr f_ptr(
                new irods::file_object(_ctx.comm(), source_info.object_path, "", "", 0, source_info.data_mode, 0));
            ...
            // init the parser with the fragment of the upstream hierarchy not including the repl node as it should add
            // itself
            irods::hierarchy_parser parser;
            const size_t pos = source_info.resource_hierarchy.find(_parent_resc_name);
            if (std::string::npos == pos) {
                THROW(SYS_INVALID_INPUT_PARAM,
                      boost::format("missing repl name [%s] in source hier string [%s]") % _parent_resc_name %
            ...
            if (!err_resolve.ok()) {
                THROW(err_resolve.code(),
                      boost::format("failed to resolve resource plugin. child resc [%s] parent resc [%s] bundle index "
                                    "[%d] bundles [%s] data id [%lld]. resolve message [%s]") %
                          _child_resc_name % _parent_resc_name % _bun_idx % leaf_bundles_to_string(_bundles) %
                          data_id_to_replicate % err_resolve.result());
            }

            // then we need to query the target resource and ask it to determine a dest resc hier for the repl
            std::string host_name;
            float vote = 0.0;
            const irods::error err_vote =
                dst_resc->call<const std::string*, const std::string*, irods::hierarchy_parser*, float*>(
                    _ctx.comm(),
            ...
            if (!err_vote.ok()) {
                THROW(err_resolve.code(),
                      boost::format("failed to get dest hierarchy. child resc [%s] parent resc [%s] bundle index [%d] "
                                    "bundles [%s] data id [%lld]. vote message [%s]") %
                          _child_resc_name % _parent_resc_name % _bun_idx % leaf_bundles_to_string(_bundles) %
                          data_id_to_replicate % err_vote.result());
            }
            ...
            parser.str(dst_hier);
            rodsLog(LOG_NOTICE,
                    "proc_results_for_rebalance: creating new replica for data id [%lld] from [%s] on [%s]",
                    data_id_to_replicate,
                    source_info.resource_hierarchy.c_str(),
                    dst_hier.c_str());
            const irods::error err_rebalance = repl_for_rebalance(_ctx,
            ...
            if (!err_rebalance.ok()) {
                ...
                rodsLog(LOG_ERROR,
                        "proc_results_for_rebalance: repl_for_rebalance failed. object path [%s] parent resc [%s] "
                        "source hier [%s] dest hier [%s] root resc [%s] data mode [%d]",
        ...
        if (!first_rebalance_error.ok()) {
            THROW(first_rebalance_error.code(),
                  boost::format("proc_results_for_rebalance: repl_for_rebalance failed. child_resc [%s] parent resc "
                                "[%s]. rebalance message [%s]") %
                      _child_resc_name % _parent_resc_name % first_rebalance_error.result());
        }
    }
} // namespace
...
{
    // throws irods::exception
    void update_out_of_date_replicas(irods::plugin_context& _ctx,
                                     const std::vector<leaf_bundle_t>& _leaf_bundles,
                                     const int _batch_size,
                                     const std::string& _invocation_timestamp,
                                     const std::string& _resource_name)
    {
        while (true) {
            const std::vector<ReplicaAndRescId> replicas_to_update =
                get_out_of_date_replicas_batch(_ctx.comm(), _leaf_bundles, _invocation_timestamp, _batch_size);
            ...
            for (const auto& replica_to_update : replicas_to_update) {
                ...
                const error err_dst_hier =
                    resc_mgr.leaf_id_to_hier(replica_to_update.resource_id, destination_hierarchy);
                if (!err_dst_hier.ok()) {
                    THROW(err_dst_hier.code(),
                          boost::format(
                              "leaf_id_to_hier failed. data id [%lld]. replica number [%d] resource id [%lld]") %
                              replica_to_update.data_id % replica_to_update.replica_number %
                ...
                const error err_repl = repl_for_rebalance(_ctx,
                                                          source_info.object_path,
                                                          _resource_name,
                                                          source_info.resource_hierarchy,
                ...
                if (!err_repl.ok()) {
                    ...
                    rodsLog(LOG_ERROR,
                            "update_out_of_date_replicas: repl_for_rebalance failed with code [%ji] and message [%s]. "
                            "object [%s] source hierarchy [%s] data id [%ji] destination repl num [%ji] destination "
                            "hierarchy [%s]",
                            static_cast<intmax_t>(err_repl.code()),
                            err_repl.result().c_str(),
                            source_info.object_path.c_str(),
                            source_info.resource_hierarchy.c_str(),
                            static_cast<intmax_t>(replica_to_update.data_id),
                            static_cast<intmax_t>(replica_to_update.replica_number),
                            destination_hierarchy.c_str());
                }
            }
            if (!first_error.ok()) {
    ...
    // throws irods::exception
    void create_missing_replicas(irods::plugin_context& _ctx,
                                 const std::vector<leaf_bundle_t>& _leaf_bundles,
                                 const int _batch_size,
                                 const std::string& _invocation_timestamp,
                                 const std::string& _resource_name)
    {
        for (size_t i = 0; i < _leaf_bundles.size(); ++i) {
            ...
            while (true) {
                ...
                const int status_chlGetReplListForLeafBundles = chlGetReplListForLeafBundles(
                    _batch_size, i, &_leaf_bundles, &_invocation_timestamp, &data_ids_needing_new_replicas);
                if (status_chlGetReplListForLeafBundles != 0) {
                    THROW(status_chlGetReplListForLeafBundles,
                          boost::format("failed to get data objects needing new replicas for resource [%s] bundle "
                                        "index [%d] bundles [%s]") %
                              _resource_name % i % leaf_bundles_to_string(_leaf_bundles));
                }
                ...
                proc_results_for_rebalance(
                    _ctx, _resource_name, child_name, i, _leaf_bundles, data_ids_needing_new_replicas);
            }
        }
    }
} // namespace irods
