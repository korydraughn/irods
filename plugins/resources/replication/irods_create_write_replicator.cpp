...
namespace irods
{
    create_write_replicator::create_write_replicator(const std::string& _root_resource,
                                             ...
                                             const object_oper& _object_oper)
    {
        ...
        if ((result = ASSERT_ERROR(_object_oper.operation() == irods::CREATE_OPERATION ||
                                       _object_oper.operation() == irods::WRITE_OPERATION,
                                   INVALID_OPERATION,
                                   "Performing create/write replication but objects operation is: \"%s\".",
                                   _object_oper.operation().c_str()))
                .ok()) {
            ...
            for (it = _siblings.begin(); it != _siblings.end(); ++it) {
                ...
                if ((result = ASSERT_PASS(ret, "Failed to get the hierarchy string from the sibling hierarchy parser."))
                        .ok()) {
                    ...
                    try {
                        ...
                        result = ASSERT_ERROR(status >= 0,
                                              status,
                                              "Failed to replicate the data object: \"%s\" from resource: \"%s\" "
                                              "to sibling: \"%s\" - %s %s.",
                                              object.logical_path().c_str(),
                                              child_.c_str(),
                                              hierarchy_string.c_str(),
                                              rods_error,
                                              sys_error);
                        free(sys_error);
                        ...
                        if (!result.ok()) {
                            last_error = result;
                            irods::log(result);
                            addRErrorMsg(&_ctx.comm()->rError, result.code(), result.result().c_str());
                            result = SUCCESS();
                        }
                    }
