////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Plug-in defining a replicating resource. This resource makes sure that all of its data is replicated to all of its
// children
//
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 */
irods::error replGetNextRescInHier(const irods::hierarchy_parser& _parser,
...
bool replObjectInList(const object_list_t& _object_list,
...
irods::error replUpdateObjectAndOperProperties(irods::plugin_context& _ctx, const std::string& _oper)
{
    ...
    // The object list is now a queue of operations and their associated objects. Their corresponding replicating
    // operations will be performed one at a time in the order in which they were put into the queue.
...
irods::error get_selected_hierarchy(irods::plugin_context& _ctx, std::string& _hier_string, std::string& _root_resc)
{
    ...
    if (!ret.ok()) {
        return PASSMSG(
            (boost::format("[%s] - Failed to get the parser for the selected resource hierarchy.") % __FUNCTION__)
                .str(),
            ret);
    }
    ...
    if (!ret.ok()) {
        return PASSMSG(
            (boost::format("[%s] - Failed to get the hierarchy string from the parser.") % __FUNCTION__).str(), ret);
    }

    ret = selected_parser.first_resc(_root_resc);
    if (!ret.ok()) {
        return PASSMSG((boost::format("[%s] - Failed to get the root resource from the parser.") % __FUNCTION__).str(),
                       ret);
    }
    return SUCCESS();
}
...
irods::error replReplicateCreateWrite(irods::plugin_context& _ctx)
{
    ...
    if (!ret.ok() && KEY_NOT_FOUND != ret.code()) {
        return PASSMSG((boost::format("[%s] - Failed to get object list for replication.") % __FUNCTION__).str(), ret);
    }
    ...
    if (!ret.ok()) {
        return PASSMSG((boost::format("[%s] - Failed to get child list for replication.") % __FUNCTION__).str(), ret);
    }
    ...
    if (!ret.ok()) {
        return PASSMSG(
            (boost::format("[%s] - Failed to determine the root resource and selected hierarchy.") % __FUNCTION__)
                .str(),
            ret);
    }
    ...
    if (!ret.ok()) {
        return PASSMSG((boost::format("[%s] - Could not determine resource name.") % __FUNCTION__).str(), ret);
    }
    ...
    if (!ret.ok()) {
        return PASSMSG(
            (boost::format("[%s] - Failed to replicate the create/write operation to the siblings.") % __FUNCTION__)
                .str(),
            ret);
    }
    ...
    if (!ret.ok()) {
        return PASSMSG(
            (boost::format("[%s] - Failed to update the object list in the properties.") % __FUNCTION__).str(), ret);
    }
    return SUCCESS();
}
            ...
            if ((result = ASSERT_PASS(ret, "Failed while calling child operation.")).ok()) {
                ...
                if ((result = ASSERT_PASS(ret, "Failed to get the resource name.")).ok()) {
                    if (!sub_parser.resc_in_hier(name)) {
                        ...
                        result = ASSERT_PASS(ret,
                                             "Failed to replicate create/write operation for object: \"%s\".",
...
irods::error repl_file_read(irods::plugin_context& _ctx, void* _buf, const int _len)
{
    irods::error result = SUCCESS();
    irods::error ret;
...
irods::error repl_file_write(irods::plugin_context& _ctx, const void* _buf, const int _len)
{
    irods::error result = SUCCESS();
    irods::error ret;
    // get the list of objects that need to be replicated
...
irods::error repl_file_lseek(irods::plugin_context& _ctx, const long long _offset, const int _whence)
{
    irods::error result = SUCCESS();
    irods::error ret;
    ...
    if (!ret.ok()) {
        ...
        if (!ret.ok()) {
            ...
            ret = child->call<const long long, const int>(
                _ctx.comm(), irods::RESOURCE_OP_LSEEK, _ctx.fco(), _offset, _whence);
            if (!ret.ok()) {
                std::stringstream msg;
                msg << __FUNCTION__;
                msg << " - Failed while calling child operation.";
...
irods::error repl_file_stage_to_cache(irods::plugin_context& _ctx, const char* _cache_file_name)
{
    ...
    if (!ret.ok()) {
        ...
        if (!ret.ok()) {
            ...
            ret = child->call<const char*>(_ctx.comm(), irods::RESOURCE_OP_STAGETOCACHE, _ctx.fco(), _cache_file_name);
            if (!ret.ok()) {
                std::stringstream msg;
                msg << __FUNCTION__;
                msg << " - Failed while calling child operation.";
...
irods::error replAddSelfToHierarchy(irods::plugin_context& _ctx, irods::hierarchy_parser& _parser)
{
    irods::error result = SUCCESS();
    irods::error ret;
    std::string name;
...
irods::error replRedirectToChildren(irods::plugin_context& _ctx,
...
/// @brief Creates a list of hierarchies to which this operation must be replicated, all children except the one on
/// which we are operating.
irods::error replCreateChildReplList(irods::plugin_context& _ctx, const redirect_map_t& _redirect_map)
{
    ...
    if (!ret.ok()) {
        return PASSMSG(
            (boost::format("[%s] - Failed to store the repl child list as a property.") % __FUNCTION__).str(), ret);
    }
...
irods::error process_redirect_map_for_random_open(const redirect_map_t& _redirect_map,
...
irods::error replSelectChild(irods::plugin_context& _ctx,
                             const std::string& _operation,
                             ...
                             float* _out_vote)
{
    ...
    *_out_vote = vote;
    ...
    if (!ret.ok()) {
        return PASSMSG(
            (boost::format("[%s] - Failed to add unselected children to the replication list.") % __FUNCTION__).str(),
            ret);
    }

    ret = _ctx.prop_map().set<irods::hierarchy_parser>(HIERARCHY_PROP, parser);
    if (!ret.ok()) {
        return PASSMSG((boost::format("[%s] - Failed to add hierarchy property to resource.") % __FUNCTION__).str(),
                       ret);
    }
    return SUCCESS();
}
...
irods::error replValidOperation(irods::plugin_context& _ctx)
{
    irods::error result = SUCCESS();
    // cast the first class object to a file object
    try {
        irods::file_object_ptr file_obj = boost::dynamic_pointer_cast<irods::file_object>(_ctx.fco());
        // if the file object has a requested replica then fail since that circumvents the coordinating nodes
        // management.
        if (file_obj->repl_requested() >= 0 &&
            false) { // For migration we no longer have this restriction but will be added back later - harry
            std::stringstream msg;
            msg << __FUNCTION__;
            ...
            if (in_repl != NULL &&
                false) { // For migration we no longer have this restriction but might be added later. - harry
                std::stringstream msg;
                msg << __FUNCTION__;
...
irods::error proc_child_list_for_create_policy(irods::plugin_context& _ctx)
{
    ...
    if (num_repl < child_list.size()) {
        ...
        while (!pick_done) {
            ...
            if (std::find(picked_indicies.begin(), picked_indicies.end(), rand_index) == picked_indicies.end()) {
...
irods::error repl_file_resolve_hierarchy(irods::plugin_context& _ctx,
                                         const std::string* _operation,
                                         const std::string* _curr_host,
                                         ...
                                         float* _out_vote)
{
    if (NULL == _operation || NULL == _curr_host || NULL == _inout_parser || NULL == _out_vote) {
        return ERROR(SYS_INVALID_INPUT_PARAM,
                     (boost::format("[%s]: null parameters passed to redirect") % __FUNCTION__).str().c_str());
    }
    ...
    if (!(ret = replValidOperation(_ctx)).ok()) {
        return PASSMSG((boost::format("[%s] - Invalid operation on replicating resource.") % __FUNCTION__).str(), ret);
    }
    // add ourselves to the hierarchy parser
    else if (!(ret = replAddSelfToHierarchy(_ctx, parser)).ok()) {
        return PASSMSG(
            (boost::format("[%s] - Failed to add ourselves to the resource hierarchy.") % __FUNCTION__).str(), ret);
    }
    // call redirect on each child with the appropriate parser
        ...
        return PASSMSG((boost::format("[%s] - Failed to redirect to all children.") % __FUNCTION__).str(), ret);
    }
    // foreach child parser determine the best to access based on host
        ...
        return PASSMSG((boost::format("[%s] - Failed to select an appropriate child.") % __FUNCTION__).str(), ret);
    }
    else if (irods::WRITE_OPERATION == (*_operation) || irods::CREATE_OPERATION == (*_operation)) {
        ret = _ctx.prop_map().set<std::string>(OPERATION_TYPE_PROP, *_operation);
        if (!ret.ok()) {
            return PASSMSG((boost::format("[%s] - Failed to set operation_type property") % __FUNCTION__).str(), ret);
        }
...
irods::error repl_file_notify(irods::plugin_context& _ctx, const std::string* _opr)
{
    ...
    if (irods::CREATE_OPERATION == (*_opr) || irods::WRITE_OPERATION == (*_opr)) {
        result = ASSERT_PASS(_ctx.prop_map().set<std::string>(OPERATION_TYPE_PROP, *_opr),
                             "failed to set operation_type property");
    }
...
public:
        ...
        : irods::resource(_inst_name, _context)
    {
        if (_context.empty()) {
            ...
            properties_.set<decltype(irods::DEFAULT_RETRY_ATTEMPTS)>(irods::RETRY_ATTEMPTS_KW,
                                                                     irods::DEFAULT_RETRY_ATTEMPTS);
            properties_.set<decltype(irods::DEFAULT_RETRY_FIRST_DELAY_IN_SECONDS)>(
                irods::RETRY_FIRST_DELAY_IN_SECONDS_KW, irods::DEFAULT_RETRY_FIRST_DELAY_IN_SECONDS);
            properties_.set<decltype(irods::DEFAULT_RETRY_BACKOFF_MULTIPLIER)>(irods::RETRY_BACKOFF_MULTIPLIER_KW,
        ...
        if (kvp_map.find(NUM_REPL_KW) != kvp_map.end()) {
            try {
                ...
                if (num_repl <= 0) {
                    irods::log(
                        ERROR(SYS_INVALID_INPUT_PARAM,
                              boost::format("%s:%d - [%s] is 0") % __FUNCTION__ % __LINE__ % NUM_REPL_KW.c_str()));
                }
                else {
                ...
                irods::log(ERROR(SYS_INVALID_INPUT_PARAM,
                                 boost::format("failed to cast [%s] to value [%s]") % NUM_REPL_KW.c_str() %
                                     kvp_map[NUM_REPL_KW]));
            }
        }
        ...
        if (kvp_map.find(irods::RETRY_ATTEMPTS_KW) != kvp_map.end()) {
            try {
                ...
                if (int_retry_attempts < 0) {
                    irods::log(ERROR(SYS_INVALID_INPUT_PARAM,
                                     boost::format("%s:%d - [%s] is < 0; using default value [%d]") % __FUNCTION__ %
                                         __LINE__ % irods::RETRY_ATTEMPTS_KW.c_str() % irods::DEFAULT_RETRY_ATTEMPTS));
                }
                else {
                ...
                irods::log(ERROR(SYS_INVALID_INPUT_PARAM,
                                 boost::format("failed to cast [%s] to value [%s]; using default value [%d]") %
                                     irods::RETRY_ATTEMPTS_KW.c_str() % kvp_map[irods::RETRY_ATTEMPTS_KW] %
                                     irods::DEFAULT_RETRY_ATTEMPTS));
            }
        }
        ...
        if (kvp_map.find(irods::RETRY_FIRST_DELAY_IN_SECONDS_KW) != kvp_map.end()) {
            try {
                ...
                if (int_retry_delay <= 0) {
                    irods::log(ERROR(SYS_INVALID_INPUT_PARAM,
                                     boost::format("%s:%d - [%s] is <= 0; using default value [%d]") % __FUNCTION__ %
                                         __LINE__ % irods::RETRY_FIRST_DELAY_IN_SECONDS_KW.c_str() %
        ...
        properties_.set<decltype(retry_delay_in_seconds)>(irods::RETRY_FIRST_DELAY_IN_SECONDS_KW,
        ...
        if (kvp_map.find(irods::RETRY_BACKOFF_MULTIPLIER_KW) != kvp_map.end()) {
            try {
                backoff_multiplier =
                    boost::lexical_cast<decltype(backoff_multiplier)>(kvp_map[irods::RETRY_BACKOFF_MULTIPLIER_KW]);
                if (backoff_multiplier < 1) {
                    irods::log(ERROR(SYS_INVALID_INPUT_PARAM,
                                     boost::format("%s:%d - [%s] is < 1; using default value [%f]") % __FUNCTION__ %
                                         __LINE__ % irods::RETRY_BACKOFF_MULTIPLIER_KW.c_str() %
                                         irods::DEFAULT_RETRY_BACKOFF_MULTIPLIER));
...
extern "C" irods::resource* plugin_factory(const std::string& _inst_name, const std::string& _context)
{
    // =-=-=-=-=-=-=-
    ...
    using namespace std;
    resc->add_operation(RESOURCE_OP_CREATE, function<error(plugin_context&)>(repl_file_create));

    resc->add_operation(irods::RESOURCE_OP_OPEN, function<error(plugin_context&)>(repl_file_open));

    resc->add_operation<void*, const int>(
        irods::RESOURCE_OP_READ, std::function<error(irods::plugin_context&, void*, const int)>(repl_file_read));

    resc->add_operation<const void*, const int>(
        irods::RESOURCE_OP_WRITE, function<error(plugin_context&, const void*, const int)>(repl_file_write));

    resc->add_operation(RESOURCE_OP_CLOSE, function<error(plugin_context&)>(repl_file_close));

    resc->add_operation(irods::RESOURCE_OP_UNLINK, function<error(plugin_context&)>(repl_file_unlink));

    resc->add_operation<struct stat*>(irods::RESOURCE_OP_STAT,
                                      function<error(plugin_context&, struct stat*)>(repl_file_stat));

    resc->add_operation(irods::RESOURCE_OP_MKDIR, function<error(plugin_context&)>(repl_file_mkdir));

    resc->add_operation(irods::RESOURCE_OP_OPENDIR, function<error(plugin_context&)>(repl_file_opendir));

    resc->add_operation<struct rodsDirent**>(irods::RESOURCE_OP_READDIR,
                                             function<error(plugin_context&, struct rodsDirent**)>(repl_file_readdir));

    resc->add_operation<const char*>(irods::RESOURCE_OP_RENAME,
                                     function<error(plugin_context&, const char*)>(repl_file_rename));

    resc->add_operation(irods::RESOURCE_OP_FREESPACE, function<error(plugin_context&)>(repl_file_getfs_freespace));

    resc->add_operation<const long long, const int>(
        irods::RESOURCE_OP_LSEEK, function<error(plugin_context&, const long long, const int)>(repl_file_lseek));

    resc->add_operation(irods::RESOURCE_OP_RMDIR, function<error(plugin_context&)>(repl_file_rmdir));

    resc->add_operation(irods::RESOURCE_OP_CLOSEDIR, function<error(plugin_context&)>(repl_file_closedir));

    resc->add_operation<const char*>(irods::RESOURCE_OP_STAGETOCACHE,
                                     function<error(plugin_context&, const char*)>(repl_file_stage_to_cache));

    resc->add_operation<const char*>(irods::RESOURCE_OP_SYNCTOARCH,
                                     function<error(plugin_context&, const char*)>(repl_file_sync_to_arch));

    resc->add_operation(irods::RESOURCE_OP_REGISTERED, function<error(plugin_context&)>(repl_file_registered));

    resc->add_operation(irods::RESOURCE_OP_UNREGISTERED, function<error(plugin_context&)>(repl_file_unregistered));

    resc->add_operation(irods::RESOURCE_OP_MODIFIED, function<error(plugin_context&)>(repl_file_modified));

    resc->add_operation<const std::string*>(irods::RESOURCE_OP_NOTIFY,
                                            function<error(plugin_context&, const std::string*)>(repl_file_notify));

    resc->add_operation(irods::RESOURCE_OP_TRUNCATE, function<error(plugin_context&)>(repl_file_truncate));
    ...
    resc->add_operation(irods::RESOURCE_OP_REBALANCE, function<error(plugin_context&)>(repl_file_rebalance));
