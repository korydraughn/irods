...
int irods::data_obj_repl_with_retry(irods::plugin_context& _ctx, dataObjInp_t& dataObjInp)
{
    ...
    irods::error err{_ctx.prop_map().get<decltype(retry_attempts)>(irods::RETRY_ATTEMPTS_KW, retry_attempts)};
    if (!err.ok()) {
        THROW(err.code(), err.result());
    }
    err = _ctx.prop_map().get<decltype(delay_in_seconds)>(irods::RETRY_FIRST_DELAY_IN_SECONDS_KW, delay_in_seconds);
    if (!err.ok()) {
        THROW(err.code(), err.result());
    }
    err = _ctx.prop_map().get<decltype(backoff_multiplier)>(irods::RETRY_BACKOFF_MULTIPLIER_KW, backoff_multiplier);
    if (!err.ok()) {
        THROW(err.code(), err.result());
    }
    ...
    try {
        while (status < 0 && retry_attempts-- > 0) {
            ...
            if (status < 0 && retry_attempts > 0) {
                delay_in_seconds =
                    boost::numeric_cast<decltype(delay_in_seconds)>(delay_in_seconds * backoff_multiplier);
            }
        }
    }
