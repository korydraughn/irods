...
namespace
{
    ...
    auto escape_single_quotes(irods::plugin_context& _ctx, const std::string& _input) -> std::string
    {
        ...
        if (const auto err = _ctx.prop_map().get<std::string>(ESCAPE_SINGLE_QUOTES, value); !err.ok()) {
            ...
            log_resc::error(
                "{}: Missing context string property [{}] for univmss resource.", __func__, ESCAPE_SINGLE_QUOTES);
        }
  ...
  public:
        ...
        : irods::resource(_inst_name, _context)
    {
        ...
        if (context_.find('/') != std::string::npos) {
            log_resc::warn("univmss resource: Expected a single filename which should reside in msiExecCmd_bin. "
                           "Context string is set to [{}].",
                           context_);
        }
