...
namespace irods
{
    ...
    public:
        ...
        error get_property(const std::string& _key, T& _val)
        {
            if (!captured_) {
                return ERROR(
                    SYS_INTERNAL_ERR,
                    "'capture' must be called on the catalog_properties instance before properties may be accessed.");
            }
    ...
    {
        irods::error ret = irods::catalog_properties::instance().get_property<T>(_key, _val);
        if (!ret.ok()) {
            return PASS(ret);
        }
