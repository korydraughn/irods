...
#endif
    int status = cllExecSqlWithResultBV(icss, &stmtNum, updatedSql, bindVars);
    if (status != 0) {
...
// TODO: this should be a function, probably inside low-level icat
    ...
    i = cllExecSqlWithResultBV(icss, &stmtNum, updatedSql, emptyBindVars);
    if (i != 0) {
        if (i <= CAT_ENV_ERR) {
            return (i); /* already an iRODS error code */
        }
...
// TODO: this should be a function, probably inside low-level icat
    ...
    i = cllExecSqlWithResultBV(icss, &stmtNum, updatedSql, bindVars);
                             ...
                             icatSessionStruct* icss)
{
    ...
    status = cmlGetOneRowFromSqlBV(sql, cVals, iVals, 1, bindVars, icss);
    if (status == 1) {
        return 0;
    }
    else {
...
int cmlGetStringValuesFromSql(const char* sql,
                              ...
                              icatSessionStruct* icss)
{
    int i = cmlGetOneRowFromSqlBV(sql, cVal, cValSize, numberOfStringsToGet, bindVars, icss);
                                      ...
                                      icatSessionStruct* icss)
{
    ...
    for (;;) {
        ...
        for (j = 0; j < icss->stmtPtr[stmtNum]->numOfCols; j++) {
            rstrcpy(pString, icss->stmtPtr[stmtNum]->resultValue[j], maxStringLen);
            tsg++;
                              ...
                              icatSessionStruct* icss)
{
    ...
    i = cmlGetOneRowFromSqlBV(sql, cVal, &cValSize, 1, bindVars, icss);
    if (i == 1) {
        if (*cVal[0] == '\0') {
            return CAT_NO_ROWS_FOUND;
        }
...
int cmlCheckNameToken(const char* nameSpace, const char* tokenName, icatSessionStruct* icss)
{
    ...
    status = cmlGetIntegerValueFromSql(
        "select token_id from  R_TOKN_MAIN where token_namespace=? and token_name=?", &iVal, bindVars, icss);
    return status;
}
...
rodsLong_t cmlGetNextSeqVal(icatSessionStruct* icss)
{
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "cmlGetNextSeqVal cmlGetIntegerValueFromSql failure %d", status);
        return status;
    }
    return iVal;
}
...
int cmlGetNextSeqStr(char* seqStr, int maxSeqStrLen, icatSessionStruct* icss)
{
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "cmlGetNextSeqStr cmlGetStringValueFromSql failure %d", status);
    }
    return status;
}
...
int cmlTest(icatSessionStruct* icss)
{
    ...
    cVal[0] = cValStr;
    cValSize = MAX_INTEGER_SIZE;
    snprintf(sql, sizeof sql, "select coll_id from R_COLL_MAIN where coll_name='a'");
    ...
    snprintf(sql, sizeof sql, "select data_id from R_DATA_MAIN where coll_id='1' and data_name='a'");
    i = cmlGetOneRowFromSql(sql, cVal, &cValSize, 1, icss);
    if (i == 1) {
        printf("result = %s\n", cValStr);
        i = 0;
...
*/
rodsLong_t cmlCheckResc(const char* rescName,
                        const char* userName,
                        const char* userZone,
                        const char* accessLevel,
                        icatSessionStruct* icss)
{
    ...
    status = cmlGetIntegerValueFromSql(
        "select resc_id from R_RESC_MAIN RM, R_OBJT_ACCESS OA, R_USER_GROUP UG, R_USER_MAIN UM, R_TOKN_MAIN TM where "
        "RM.resc_name=? and UM.user_name=? and UM.zone_name=? and UM.user_type_name!='rodsgroup' and UM.user_id = "
        "UG.user_id and OA.object_id = RM.resc_id and UG.group_user_id = OA.user_id and OA.access_type_id >= "
        "TM.token_id and  TM.token_namespace ='access_type' and TM.token_name = ?",
        &iVal,
        bindVars,
        icss);
    if (status) {
        ...
        bindVars.clear();
        bindVars.push_back(rescName);
        status = cmlGetIntegerValueFromSql("select resc_id from R_RESC_MAIN where resc_name=?", &iVal, bindVars, icss);
        if (status) {
...
/*
  ...
  Return code is either an iRODS error code (< 0) or the collectionId.
*/
rodsLong_t cmlCheckDir(const char* dirName,
                       const char* userName,
                       const char* userZone,
                       const char* accessLevel,
                       icatSessionStruct* icss)
{
    ...
    bindVars.push_back(accessLevel);
    status = cmlGetIntegerValueFromSql(
        "select coll_id from R_COLL_MAIN CM, R_OBJT_ACCESS OA, R_USER_GROUP UG, R_USER_MAIN UM, R_TOKN_MAIN TM where "
        "CM.coll_name=? and UM.user_name=? and UM.zone_name=? and UM.user_type_name!='rodsgroup' and UM.user_id = "
        "UG.user_id and OA.object_id = CM.coll_id and UG.group_user_id = OA.user_id and OA.access_type_id >= "
        "TM.token_id and  TM.token_namespace ='access_type' and TM.token_name = ?",
        &iVal,
        bindVars,
        icss);
...
*/
rodsLong_t cmlCheckDirAndGetInheritFlag(const char* dirName,
                                        const char* userName,
                                        const char* userZone,
                                        const char* accessLevel,
                                        int* inheritFlag,
                                        ...
                                        icatSessionStruct* icss)
{
    ...
    *inheritFlag = 0;
    ...
    if (ticketStr != NULL && *ticketStr != '\0') {
        ...
        status = cmlGetOneRowFromSqlBV(
            "select coll_id, coll_inheritance from R_COLL_MAIN CM, R_TICKET_MAIN TM where CM.coll_name=? and "
            "TM.ticket_string=? and TM.ticket_type = 'write' and TM.object_id = CM.coll_id",
            cVal,
            cValSize,
            2,
            bindVars,
        ...
        bindVars.push_back(accessLevel);
        status = cmlGetOneRowFromSqlBV(
            "select coll_id, coll_inheritance from R_COLL_MAIN CM, R_OBJT_ACCESS OA, R_USER_GROUP UG, R_USER_MAIN UM, "
            "R_TOKN_MAIN TM where CM.coll_name=? and UM.user_name=? and UM.zone_name=? and "
     ...
     */
    if (ticketStr != NULL && *ticketStr != '\0') {
        status = checkObjIdByTicket(cValStr1, accessLevel, ticketStr, ticketHost, userName, userZone, icss);
        if (status != 0) {
            return status;
        }
    }
...
/*
  ...
  Return code is either an iRODS error code (< 0) or the collectionId.
*/
rodsLong_t cmlCheckDirId(const char* dirId,
                         const char* userName,
                         const char* userZone,
                         const char* accessLevel,
                         icatSessionStruct* icss)
{
    ...
    bindVars.push_back(accessLevel);
    status = cmlGetIntegerValueFromSql(
        "select object_id from R_OBJT_ACCESS OA, R_USER_GROUP UG, R_USER_MAIN UM, R_TOKN_MAIN TM where UM.user_name=? "
        "and UM.zone_name=? and UM.user_type_name!='rodsgroup' and UM.user_id = UG.user_id and OA.object_id = ? and "
        "UG.group_user_id = OA.user_id and OA.access_type_id >= TM.token_id and  TM.token_namespace ='access_type' and "
        "TM.token_name = ?",
        &iVal,
        bindVars,
        icss);
    if (status) {
        ...
        std::vector<std::string> bindVars;
        bindVars.push_back(dirId);
        status = cmlGetIntegerValueFromSql("select coll_id from R_COLL_MAIN where coll_id=?", &iVal, bindVars, icss);
        if (status) {
            return CAT_UNKNOWN_COLLECTION;
        }
        return CAT_NO_ACCESS_PERMISSION;
...
*/
rodsLong_t cmlCheckDataObjOnly(const char* dirName,
                               const char* dataName,
                               const char* userName,
                               const char* userZone,
                               const char* accessLevel,
                               icatSessionStruct* icss)
{
    ...
    bindVars.push_back(accessLevel);
    status = cmlGetIntegerValueFromSql(
        "select data_id from R_DATA_MAIN DM, R_OBJT_ACCESS OA, R_USER_GROUP UG, R_USER_MAIN UM, R_TOKN_MAIN TM, "
        "R_COLL_MAIN CM where DM.data_name=? and DM.coll_id=CM.coll_id and CM.coll_name=? and UM.user_name=? and "
        "UM.zone_name=? and UM.user_type_name!='rodsgroup' and UM.user_id = UG.user_id and OA.object_id = DM.data_id "
        "and UG.group_user_id = OA.user_id and OA.access_type_id >= TM.token_id and  TM.token_namespace ='access_type' "
        "and TM.token_name = ?",
        &iVal,
        bindVars,
        icss);
    ...
    if (status) {
        ...
        bindVars.push_back(dataName);
        bindVars.push_back(dirName);
        status = cmlGetIntegerValueFromSql("select data_id from R_DATA_MAIN DM, R_COLL_MAIN CM where DM.data_name=? "
                                           "and DM.coll_id=CM.coll_id and CM.coll_name=?",
                                           &iVal,
                                           bindVars,
                                           icss);
        if (status) {
...
*/
rodsLong_t cmlCheckDataObjOwn(const char* dirName,
                              const char* dataName,
                              const char* userName,
                              const char* userZone,
                              icatSessionStruct* icss)
...
int cmlCheckUserInGroup(const char* userName, const char* userZone, const char* groupName, icatSessionStruct* icss)
{
    ...
    status =
        cmlGetIntegerValueFromSql("select group_user_id from R_USER_GROUP where user_id=? and group_user_id = (select "
                                  "user_id from R_USER_MAIN where user_type_name='rodsgroup' and user_name=?)",
                                  &iVal,
                                  bindVars,
                                  icss);
    if (status) {
 ...
 * allowed */
int cmlCheckTicketRestrictions(const char* ticketId,
                               const char* ticketHost,
                               const char* userName,
                               const char* userZone,
                               icatSessionStruct* icss)
{
    ...
    status =
        cmlGetFirstRowFromSqlBV("select host from R_TICKET_ALLOWED_HOSTS where ticket_id=?", bindVars, &stmtNum, icss);
    if (status == CAT_NO_ROWS_FOUND) {
        hostOK = 1;
    }
    else {
    ...
    for (; status != CAT_NO_ROWS_FOUND;) {
        if (strncmp(ticketHost, icss->stmtPtr[stmtNum]->resultValue[0], NAME_LEN) == 0) {
            hostOK = 1;
        }
    ...
    status = cmlGetFirstRowFromSqlBV(
        "select user_name from R_TICKET_ALLOWED_USERS where ticket_id=?", bindVars, &stmtNum, icss);
    if (status == CAT_NO_ROWS_FOUND) {
        userOK = 1;
    }
    else {
    ...
    myUser += userZone;
    for (; status != CAT_NO_ROWS_FOUND;) {
        if (strncmp(userName, icss->stmtPtr[stmtNum]->resultValue[0], NAME_LEN) == 0) {
            userOK = 1;
        }
        else {
            /* try user#zone */
            if (strncmp(myUser.c_str(), icss->stmtPtr[stmtNum]->resultValue[0], NAME_LEN) == 0) {
                userOK = 1;
            }
        }
        status = cmlGetNextRowFromStatement(stmtNum, icss);
    ...
    bindVars.push_back(ticketId);
    status = cmlGetFirstRowFromSqlBV(
        "select group_name from R_TICKET_ALLOWED_GROUPS where ticket_id=?", bindVars, &stmtNum, icss);
    if (status == CAT_NO_ROWS_FOUND) {
        groupOK = 1;
    }
    else {
    ...
    for (; status != CAT_NO_ROWS_FOUND;) {
        int status2;
        status2 = cmlCheckUserInGroup(userName, userZone, icss->stmtPtr[stmtNum]->resultValue[0], icss);
        if (status2 == 0) {
            groupOK = 1;
        }
        status = cmlGetNextRowFromStatement(stmtNum, icss);
                       ...
                       icatSessionStruct* icss)
{
    ...
    if (strncmp(accessLevel, "modify", 6) == 0) {
        ...
        status = cmlGetStringValuesFromSql(
            ...
            "write_file_limit, write_byte_count, write_byte_limit from R_TICKET_MAIN where ticket_type = 'write' and "
            "ticket_string = ? and (object_id = ? or object_id in (select coll_id from R_DATA_MAIN where data_id = ?))",
            cVal,
            iVal,
            9,
            bindVars,
        ...
        bindVars.push_back(dataId);
        status = cmlGetStringValuesFromSql(
            "select ticket_id, uses_limit, uses_count, ticket_expiry_ts, restrictions from R_TICKET_MAIN where "
            "ticket_string = ? and (object_id = ? or object_id in (select coll_id from R_DATA_MAIN where data_id = ?))",
            cVal,
            iVal,
            5,
    ...
    if (strncmp(ticketId, prevTicketId, sizeof(prevTicketId)) != 0) {
        ...
        status = cmlAudit3(AU_USE_TICKET, ticketId, userName, userZone, ticketStr, icss);
        if (status != 0) {
            return status;
        }
    }
                              ...
                              icatSessionStruct* icss)
{
    ...
    bindVars.push_back(objectId);
    status = cmlGetStringValuesFromSql(
        "select ticket_id, write_byte_count, write_byte_limit from R_TICKET_MAIN where ticket_type = 'write' and "
        "ticket_string = ? and (object_id = ? or object_id in (select coll_id from R_DATA_MAIN where data_id = ?))",
        cVal,
        iVal,
        3,
...
*/
int cmlCheckDataObjId(const char* dataId,
                      const char* userName,
                      const char* zoneName,
                      const char* accessLevel,
                      const char* ticketStr,
                      ...
                      icatSessionStruct* icss)
{
    ...
    iVal = 0;
    if (ticketStr != NULL && *ticketStr != '\0') {
        status = checkObjIdByTicket(dataId, accessLevel, ticketStr, ticketHost, userName, zoneName, icss);
        if (status != 0) {
            return status;
        }
    }
        ...
        bindVars.push_back(accessLevel);
        status = cmlGetIntegerValueFromSql(
            "select object_id from R_OBJT_ACCESS OA, R_DATA_MAIN DM, R_USER_GROUP UG, R_USER_MAIN UM, R_TOKN_MAIN TM "
            "where OA.object_id=? and UM.user_name=? and UM.zone_name=? and UM.user_type_name!='rodsgroup' and "
...
int cmlGetGroupMemberCount(const char* groupName, icatSessionStruct* icss)
{
    ...
    bindVars.push_back(groupName);
    status = cmlGetIntegerValueFromSql(
        "select count(user_id) from R_USER_GROUP where group_user_id != user_id and group_user_id in (select user_id "
        "from R_USER_MAIN where user_name=? and user_type_name='rodsgroup')",
        &iVal,
        bindVars,
        icss);
    if (status == 0) {
              ...
              icatSessionStruct* icss)
{
    ...
    status =
        cmlExecuteNoAnswerSql("insert into R_OBJT_AUDIT (object_id, user_id, action_id, r_comment, create_ts, "
                              "modify_ts) values ((select user_id from R_USER_MAIN where user_name=? and zone_name=?), "
              ...
              icatSessionStruct* icss)
{
    ...
    status = cmlExecuteNoAnswerSql(
        "insert into R_OBJT_AUDIT (object_id, user_id, action_id, r_comment, create_ts, modify_ts) values (?, (select "
    ...
    if (zoneName[0] == '\0') {
        ...
        */
        ...
        status = cmlExecuteNoAnswerSql(
            ...
            "(select user_id from R_USER_MAIN where user_name=? and zone_name=(select zone_name from R_ZONE_MAIN where "
    ...
    if (zoneName[0] == '\0') {
        ...
        */
        snprintf(mySQL,
                 MAX_SQL_SIZE,
                 "insert into R_OBJT_AUDIT (object_id, user_id, action_id, r_comment, create_ts, modify_ts) values "
                 "((%s), (select user_id from R_USER_MAIN where user_name=? and zone_name=(select zone_name from "
                 "R_ZONE_MAIN where zone_type_name='local')), ?, ?, ?, ?)",
                 sql);
        i = 0;
        if (sqlParm[0] != '\0') {
        ...
        snprintf(mySQL,
                 MAX_SQL_SIZE,
                 "insert into R_OBJT_AUDIT (object_id, user_id, action_id, r_comment, create_ts, modify_ts) values "
                 "((%s), (select user_id from R_USER_MAIN where user_name=? and zone_name=?), ?, ?, ?, ?)",
                 sql);
        i = 0;
        if (sqlParm[0] != '\0') {
            cllBindVars[i++] = sqlParm;
...
int cmlAudit5(int actionId, const char* objId, const char* userId, const char* comment, icatSessionStruct* icss)
{
    ...
    status = cmlExecuteNoAnswerSql("insert into R_OBJT_AUDIT (object_id, user_id, action_id, r_comment, create_ts, "
                                   "modify_ts) values (?,?,?,?,?,?)",
                                   icss);
    if (status != 0) {
