...
int validateAndParseUserName(const char* fullUserNameIn, char* userName, char* userZone)
{
    ...
    try {
        ...
        if (!matched || matches.str(1).size() >= NAME_LEN || matches.str(1).size() < 1 ||
            matches.str(4).size() >= NAME_LEN || matches.str(1) == "." || matches.str(1) == "..") {
            if (userName != NULL) {
                userName[0] = '\0';
            }
            if (userZone != NULL) {
...
{
    if (!_fc.get()) {
        return ERROR(SYS_INVALID_INPUT_PARAM, "incoming fco is null");
    }

    _pg = boost::dynamic_pointer_cast<irods::postgres_object>(_fc);

    if (_pg.get()) {
        return SUCCESS();
    }
    else {
        return ERROR(INVALID_DYNAMIC_CAST, "failed to dynamic cast to postgres_object_ptr");
    }
...
int _rollback(const char* functionName)
{
    ...
    if (status == 0) {
        rodsLog(LOG_NOTICE, "%s cmlExecuteNoAnswerSql(rollback) succeeded", functionName);
    }
    else {
        rodsLog(LOG_NOTICE, "%s cmlExecuteNoAnswerSql(rollback) failure %d", functionName, status);
    }
...
irods::error getLocalZone(irods::plugin_property_map& _prop_map, icatSessionStruct* _icss, std::string& _zone)
{
    ...
    if (!ret.ok()) {
        ...
        int status;
        {
            ...
            status = cmlGetStringValueFromSql((char*) "select zone_name from R_ZONE_MAIN where zone_type_name=?",
                                              local_zone,
                                              MAX_NAME_LEN,
                                              bindVars,
                                              _icss);
...
int get_object_count_of_resource_by_name(icatSessionStruct* _icss, const std::string& _resc_name, rodsLong_t& _count)
{
    ...
    int status = cmlGetIntegerValueFromSql(
        (char*) "select count(data_id) from R_DATA_MAIN where resc_id=?", &_count, bindVars, _icss);
                                                       ...
                                                       const std::string& _zone)
{
    ...
    rodsLong_t num_data_objects = -1;
    {
        ...
        status =
            cmlGetIntegerValueFromSql("select count(DISTINCT DM.data_id) from R_DATA_MAIN DM, R_COLL_MAIN CM where "
                                      "DM.data_name like ? and DM.coll_id=CM.coll_id and CM.coll_name like ?",
                                      &num_data_objects,
                                      bind_vars,
                                      &icss);
        if (0 != status) {
    ...
    {
        std::vector<std::string> bind_vars;
        int status = cmlGetIntegerValueFromSql("select token_id  from R_TOKN_MAIN where token_name = 'modify metadata' "
                                               "and token_namespace = 'access_type'",
                                               &access_needed,
                                               bind_vars,
                                               &icss);
        if (status < 0) {
            return ERROR(status, "query for modify metadata token_id failed");
        }
    }
    ...
    rodsLong_t access_permission = -1;
    {
#if ORA_ICAT
        std::string query =
            "select min(max_access_type_id) from ( select max(access_type_id) max_access_type_id from ( select "
            "access_type_id, DM.data_id from R_DATA_MAIN DM, R_OBJT_ACCESS OA, R_USER_GROUP UG, R_USER_MAIN UM, "
            "R_COLL_MAIN CM where DM.data_name like ? and DM.coll_id=CM.coll_id and CM.coll_name like ? and "
            "UM.user_name=? and UM.zone_name = ? and UM.user_type_name!='rodsgroup' and UM.user_id = UG.user_id and "
            "OA.object_id = DM.data_id and UG.group_user_id = OA.user_id ) group by data_id )";
#else
        std::string query =
            "select min(max_access_type_id) from ( select max(access_type_id) max_access_type_id from ( select "
            "access_type_id, DM.data_id from R_DATA_MAIN DM, R_OBJT_ACCESS OA, R_USER_GROUP UG, R_USER_MAIN UM, "
            "R_COLL_MAIN CM where DM.data_name like ? and DM.coll_id=CM.coll_id and CM.coll_name like ? and "
            "UM.user_name=? and UM.zone_name = ? and UM.user_type_name!='rodsgroup' and UM.user_id = UG.user_id and "
            "OA.object_id = DM.data_id and UG.group_user_id = OA.user_id ) as foo group by data_id ) as bar";
#endif
        std::vector<std::string> bind_vars;
        ...
        status = cmlGetIntegerValueFromSql(query.c_str(), &access_permission, bind_vars, &icss);
    ...
    rodsLong_t access_permission_count = -1;
    {
#if ORA_ICAT
        std::string query =
            "select count( max ) from ( select max(access_type_id) max from ( select access_type_id, DM.data_id from "
            "R_DATA_MAIN DM, R_OBJT_ACCESS OA, R_USER_GROUP UG, R_USER_MAIN UM, R_COLL_MAIN CM where DM.data_name like "
            "? and DM.coll_id=CM.coll_id and CM.coll_name like ? and UM.user_name=? and UM.zone_name = ? and "
            "UM.user_type_name!='rodsgroup' and UM.user_id = UG.user_id and OA.object_id = DM.data_id and "
            "UG.group_user_id = OA.user_id ) group by data_id )";
#else
        std::string query =
            "select count( max ) from ( select max(access_type_id) max from ( select access_type_id, DM.data_id from "
            "R_DATA_MAIN DM, R_OBJT_ACCESS OA, R_USER_GROUP UG, R_USER_MAIN UM, R_COLL_MAIN CM where DM.data_name like "
            "? and DM.coll_id=CM.coll_id and CM.coll_name like ? and UM.user_name=? and UM.zone_name = ? and "
            "UM.user_type_name!='rodsgroup' and UM.user_id = UG.user_id and OA.object_id = DM.data_id and "
            "UG.group_user_id = OA.user_id ) as foo group by data_id ) as baz";
#endif
        std::vector<std::string> bind_vars;
        bind_vars.push_back(_data_name.c_str());
        bind_vars.push_back(_collection.c_str());
        bind_vars.push_back(_user_name.c_str());
        bind_vars.push_back(_zone.c_str());
        status = cmlGetIntegerValueFromSql(query.c_str(), &access_permission_count, bind_vars, &icss);
        if (0 != status) {
            _rollback("chlAddAVUMetadataWild");
        ...
        if (num_data_objects > access_permission_count) {
            ...
            msg << "access denined - num_data_objects " << num_data_objects << " > access_permission_count "
                << access_permission_count;
            _rollback("chlAddAVUMetadataWild");
            return ERROR(CAT_NO_ACCESS_PERMISSION, msg.str());
        }
    }
...
static int removeAVUs()
{
    ...
    snprintf(tSQL,
             MAX_SQL_SIZE,
             "delete from R_META_MAIN where meta_id in (select meta_id from R_META_MAIN minus select meta_id from "
             "R_OBJT_METAMAP)");
#elif MY_ICAT
    /* MYSQL does not have 'minus' or 'except' (to my knowledge) so
     * use previous version of the SQL, which is very slow */
    snprintf(tSQL, MAX_SQL_SIZE, "delete from R_META_MAIN where meta_id not in (select meta_id from R_OBJT_METAMAP)");
#else
    /* Postgres */
    snprintf(tSQL,
             MAX_SQL_SIZE,
             "delete from R_META_MAIN where meta_id in (select meta_id from R_META_MAIN except select meta_id from "
             "R_OBJT_METAMAP)");
#endif
...
int _resolveHostName(rsComm_t* _rsComm, const char* _hostAddress)
{
    ...
    if (strcmp(_hostAddress, "localhost") == 0) {
        addRErrorMsg(&_rsComm->rError,
                     0,
                     "Warning, resource host address 'localhost' will not work properly as it maps to the local host "
                     "from each client.");
    }
                                ...
                                const std::string& _parent_child_context)
{
    ...
    int status = cmlExecuteNoAnswerSql("update R_RESC_MAIN set resc_parent=?, resc_parent_context=?, modify_ts=? "
                                       "where resc_id=?",
                                       &icss);
    if (status != 0) {
...
int _rescHasData(icatSessionStruct* _icss, const std::string& _resc_name, bool& _has_data)
{
    ...
    int status = get_object_count_of_resource_by_name(_icss, _resc_name, obj_count);
    if (0 == status) {
        if (0 == obj_count) {
            _has_data = false;
        }
...
bool _rescHasParentOrChild(char* rescId)
{
    ...
    children[0] = '\0';
    {
        ...
        status = cmlGetStringValueFromSql(
            "select resc_parent from R_RESC_MAIN where resc_id=?", parent, MAX_NAME_LEN, bindVars, &icss);
    }
    if (status != 0) {
        if (status == CAT_NO_ROWS_FOUND) {
            std::stringstream ss;
    ...
    }
    {
        ...
        bindVars.push_back(rescId);
        status = cmlGetStringValueFromSql(
            "select resc_id from R_RESC_MAIN where resc_parent=?", children, MAX_NAME_LEN, bindVars, &icss);
    }
    if (status != 0) {
        if (status != CAT_NO_ROWS_FOUND) {
            _rollback("_rescHasParentOrChild");
...
bool _userInRUserAuth(const char* userName, const char* zoneName, const char* auth_name)
{
    ...
    logger.log();
    {
        ...
        status = cmlGetIntegerValueFromSql("select user_id from R_USER_AUTH where user_id=(select user_id from "
                                           "R_USER_MAIN where user_name=? and zone_name=?) and user_auth_name=?",
                                           &iVal,
                                           bindVars,
                                           &icss);
    }
...
{
    std::string::iterator itr = std::find_if(_zone_name.begin(), _zone_name.end(), allowed_zone_char);
...
static int _delColl(rsComm_t* rsComm, collInfo_t* collInfo)
{
    ...
    status = cmlCheckDir(
        logicalParentDirName, rsComm->clientUser.userName, rsComm->clientUser.rodsZone, ACCESS_MODIFY_OBJECT, &icss);
    if (status < 0) {
        char errMsg[105];
        if (status == CAT_UNKNOWN_COLLECTION) {
            snprintf(errMsg, 100, "collection '%s' is unknown", logicalParentDirName);
    ...
    status = cmlCheckDir(
        collInfo->collName, rsComm->clientUser.userName, rsComm->clientUser.rodsZone, ACCESS_DELETE_OBJECT, &icss);
    if (status < 0) {
        return status;
    }
    ...
    }
    {
        ...
        status = cmlGetIntegerValueFromSql(
            "select coll_id from R_COLL_MAIN where parent_coll_name=? union select coll_id from R_DATA_MAIN where "
            "coll_id=(select coll_id from R_COLL_MAIN where coll_name=?)",
            &iVal,
            bindVars,
            &icss);
    }
    ...
    status = cmlExecuteNoAnswerSql("delete from R_COLL_MAIN where coll_name=? and coll_id=?", &icss);
    if (status != 0) { /* error, odd one as everything checked above */
        rodsLog(LOG_NOTICE, "_delColl cmlExecuteNoAnswerSql delete failure %d", status);
        _rollback("_delColl");
        return status;
    }
    ...
    status = cmlExecuteNoAnswerSql("delete from R_OBJT_ACCESS where object_id=?", &icss);
    if (status != 0) { /* error, odd one as everything checked above */
        rodsLog(LOG_NOTICE, "_delColl cmlExecuteNoAnswerSql delete access failure %d", status);
        _rollback("_delColl");
    }
    ...
    /* Audit */
    status = cmlAudit3(
        AU_DELETE_COLL, collIdNum, rsComm->clientUser.userName, rsComm->clientUser.rodsZone, collInfo->collName, &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModColl cmlAudit3 failure %d", status);
        _rollback("_delColl");
        return status;
    }
                                         ...
                                         const char* _response)
{
    ...
    ret = auth_plugin->call<const char*, const char*, const char*>(
        0, irods::AUTH_AGENT_AUTH_VERIFY, auth_obj, _challenge, _user_name, _response);
    if (!ret.ok()) {
        irods::log(PASS(ret));
        return ret;
    }
...
int decodePw(rsComm_t* rsComm, const char* in, char* out)
{
    ...
    char rand[] = "1gCBizHWbwIYyWLo"; /* must match clients */
    int pwLen1, pwLen2;
    ...
    }
    {
        ...
        status =
            cmlGetStringValueFromSql("select rcat_password from R_USER_PASSWORD, R_USER_MAIN where user_name=? and "
    ...
    if (pwLen2 > MAX_PASSWORD_LEN - 5 && pwLen2 == pwLen1) {
        ...
        snprintf(errMsg,
                 250,
                 "Error with password encoding.  This can be caused by not connecting directly to the ICAT host, not "
                 "using password authentication (using GSI or Kerberos instead), or entering your password incorrectly "
                 "(if prompted).");
        addRErrorMsg(&rsComm->rError, 0, errMsg);
                               ...
                               const char* access)
{
    ...
    if (itype == 1) {
        ...
        status = cmlCheckDataObjOnly(logicalParentDirName,
                                     logicalEndName,
    ...
    if (itype == 2) {
        ...
        status = cmlCheckDir(name, rsComm->clientUser.userName, rsComm->clientUser.rodsZone, access, &icss);
        if (status < 0) {
            char errMsg[105];
            if (status == CAT_UNKNOWN_COLLECTION) {
                snprintf(errMsg, 100, "collection '%s' is unknown", name);
                addRErrorMsg(&rsComm->rError, 0, errMsg);
            }
            return status;
        }
    ...
    if (itype == 3) {
        ...
        }
        {
            ...
            bindVars.push_back(zone);
            status = cmlGetIntegerValueFromSql(
                "select resc_id from R_RESC_MAIN where resc_name=? and zone_name=?", &objId, bindVars, &icss);
        }
        if (status != 0) {
            if (status == CAT_NO_ROWS_FOUND) {
    ...
    if (itype == 4) {
        ...
        }
        {
            ...
            status = cmlGetIntegerValueFromSql(
                "select user_id from R_USER_MAIN where user_name=? and zone_name=?", &objId, bindVars, &icss);
        }
        if (status != 0) {
            if (status == CAT_NO_ROWS_FOUND) {
...
rodsLong_t findAVU(const char* attribute, const char* value, const char* units)
{
    ...
    if (*units != '\0') {
        ...
        }
        {
            ...
            status = cmlGetIntegerValueFromSql(
                "select meta_id from R_META_MAIN where meta_attr_name=? and meta_attr_value=? and (meta_attr_unit='' "
                "or meta_attr_unit IS NULL)", // JMC - backport 4827
                &iVal,
                bindVars,
                &icss);
        }
...
int findOrInsertAVU(const char* attribute, const char* value, const char* units)
{
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "findOrInsertAVU cmlGetNextSeqVal failure %d", status);
        return status;
    }
    ...
    status = cmlExecuteNoAnswerSql("insert into R_META_MAIN (meta_id, meta_attr_name, meta_attr_value, meta_attr_unit, "
                                   "create_ts, modify_ts) values (?, ?, ?, ?, ?, ?)",
                                   &icss);
    if (status < 0) {
...
int _modInheritance(int inheritFlag, int recursiveFlag, const char* collIdStr, const char* pathName)
{
    ...
    if (recursiveFlag == 0) {
        ...
        status = cmlExecuteNoAnswerSql("update R_COLL_MAIN set coll_inheritance=?, modify_ts=? where coll_id=?", &icss);
    }
    else {
        /* Recursive mode */
...
#ifdef ORA_ICAT
        status = cmlExecuteNoAnswerSql("update R_COLL_MAIN set coll_inheritance=?, modify_ts=? where coll_name = ? or "
                                       "coll_name like ? ESCAPE '\\'",
                                       &icss);
#else
        status = cmlExecuteNoAnswerSql(
            "update R_COLL_MAIN set coll_inheritance=?, modify_ts=? where coll_name = ? or coll_name like ?", &icss);
#endif
    }
    if (status != 0) {
    ...
    char auditStr[30];
    snprintf(auditStr, sizeof(auditStr), "inheritance %srecursive %s", recursiveFlag ? "" : "non-", newValue);

    /* Audit */
    status = cmlAudit5(AU_MOD_ACCESS_CONTROL_COLL, collIdStr, "0", auditStr, &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "_modInheritance cmlAudit5 failure %d", status);
        _rollback("_modInheritance");
        return status;
    }
...
int setOverQuota(rsComm_t* rsComm)
{
    ...
    char mySQL1[] = "select sum(quota_usage), UM1.user_id, R_QUOTA_USAGE.resc_id from R_QUOTA_USAGE, R_QUOTA_MAIN, "
                    "R_USER_MAIN UM1, R_USER_GROUP, R_USER_MAIN UM2 where R_QUOTA_MAIN.user_id = UM1.user_id and "
                    "UM1.user_type_name = 'rodsgroup' and R_USER_GROUP.group_user_id = UM1.user_id and UM2.user_id = "
                    "R_USER_GROUP.user_id and R_QUOTA_USAGE.user_id = UM2.user_id and R_QUOTA_MAIN.resc_id = "
                    "R_QUOTA_USAGE.resc_id group by UM1.user_id, R_QUOTA_USAGE.resc_id";

    /* For each defined group limit on total usage (if any), get a
     * total usage on any resource for all users in that group: */
    char mySQL2a[] = "select sum(quota_usage), R_QUOTA_MAIN.quota_limit, UM1.user_id from R_QUOTA_USAGE, R_QUOTA_MAIN, "
                     "R_USER_MAIN UM1, R_USER_GROUP, R_USER_MAIN UM2 where R_QUOTA_MAIN.user_id = UM1.user_id and "
                     "UM1.user_type_name = 'rodsgroup' and R_USER_GROUP.group_user_id = UM1.user_id and UM2.user_id = "
                     "R_USER_GROUP.user_id and R_QUOTA_USAGE.user_id = UM2.user_id and R_QUOTA_USAGE.resc_id != %s and "
                     "R_QUOTA_MAIN.resc_id = %s group by UM1.user_id,  R_QUOTA_MAIN.quota_limit";
    ...
    status = cmlExecuteNoAnswerSql("update R_QUOTA_MAIN set quota_over = -quota_limit", &icss);
    ...
    status = cmlExecuteNoAnswerSql(
#if ORA_ICAT
        "update R_QUOTA_MAIN set quota_over = (select distinct R_QUOTA_USAGE.quota_usage - R_QUOTA_MAIN.quota_limit "
        "from R_QUOTA_USAGE where R_QUOTA_MAIN.user_id = R_QUOTA_USAGE.user_id and R_QUOTA_MAIN.resc_id = "
        "R_QUOTA_USAGE.resc_id) where exists (select 1 from R_QUOTA_USAGE where R_QUOTA_MAIN.user_id = "
        "R_QUOTA_USAGE.user_id and R_QUOTA_MAIN.resc_id = R_QUOTA_USAGE.resc_id)",
#elif MY_ICAT
        "update R_QUOTA_MAIN, R_QUOTA_USAGE set R_QUOTA_MAIN.quota_over = R_QUOTA_USAGE.quota_usage - "
        "R_QUOTA_MAIN.quota_limit where R_QUOTA_MAIN.user_id = R_QUOTA_USAGE.user_id and R_QUOTA_MAIN.resc_id = "
        "R_QUOTA_USAGE.resc_id",
#else
        "update R_QUOTA_MAIN set quota_over = quota_usage - quota_limit from R_QUOTA_USAGE where R_QUOTA_MAIN.user_id "
        "= R_QUOTA_USAGE.user_id and R_QUOTA_MAIN.resc_id = R_QUOTA_USAGE.resc_id",
#endif
        &icss);
    if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
    ...
    */
    ...
    for (rowsFound = 0;; rowsFound++) {
        int status2;
        if (rowsFound == 0) {
            status = cmlGetFirstRowFromSql("select sum(quota_usage), R_QUOTA_MAIN.user_id from R_QUOTA_USAGE, "
                                           "R_QUOTA_MAIN where R_QUOTA_MAIN.user_id = R_QUOTA_USAGE.user_id and "
                                           "R_QUOTA_MAIN.resc_id = '0' group by R_QUOTA_MAIN.user_id",
                                           &statementNum,
                                           0,
                                           &icss);
        }
        ...
        status2 = cmlExecuteNoAnswerSql("update R_QUOTA_MAIN set quota_over=?-quota_limit, modify_ts=? where user_id=? "
                                        "and ?-quota_limit > quota_over and resc_id='0'",
                                        &icss);
    ...
    for (rowsFound = 0;; rowsFound++) {
        int status2;
        if (rowsFound == 0) {
            status = cmlGetFirstRowFromSql(mySQL1, &statementNum, 0, &icss);
        }
        else {
            status = cmlGetNextRowFromStatement(statementNum, &icss);
        }
        ...
        status2 = cmlExecuteNoAnswerSql("update R_QUOTA_MAIN set quota_over=?-quota_limit, modify_ts=? where user_id=? "
                                        "and ?-quota_limit > quota_over and R_QUOTA_MAIN.resc_id=?",
                                        &icss);
        if (status2 == CAT_SUCCESS_BUT_WITH_NO_INFO) {
            status2 = 0;
        }
    ...
    snprintf(mySQL2b, sizeof mySQL2b, mySQL2a, "cast('0' as integer)", "cast('0' as integer)");
    snprintf(mySQL3b, sizeof mySQL3b, mySQL3a, "cast(? as integer)", "cast(? as integer)");
#elif MY_ICAT
    snprintf(mySQL2b, sizeof mySQL2b, mySQL2a, "'0'", "'0'");
    snprintf(mySQL3b, sizeof mySQL3b, mySQL3a, "?", "?");
#else
    snprintf(mySQL2b, sizeof mySQL2b, mySQL2a, "cast('0' as bigint)", "cast('0' as bigint)");
    snprintf(mySQL3b, sizeof mySQL3b, mySQL3a, "cast(? as bigint)", "cast(? as bigint)");
#endif
    if (logSQL != 0) {
    ...
    for (rowsFound = 0;; rowsFound++) {
        int status2;
        if (rowsFound == 0) {
            status = cmlGetFirstRowFromSql(mySQL2b, &statementNum, 0, &icss);
        }
        else {
            status = cmlGetNextRowFromStatement(statementNum, &icss);
        }
        ...
        status2 = cmlExecuteNoAnswerSql(mySQL3b, &icss);
        if (status2 == CAT_SUCCESS_BUT_WITH_NO_INFO) {
            status2 = 0;
        }
        if (status2 != 0) {
...
int icatGetTicketUserId(irods::plugin_property_map& _prop_map, const char* userName, char* userIdStr)
{
    ...
    }
    {
        ...
        status = cmlGetStringValueFromSql("select user_id from R_USER_MAIN where user_name=? and "
...
irods::error db_debug_op(irods::plugin_context& _ctx, const char* _mode)
{
    ...
    if (!_mode) {
        return ERROR(CAT_INVALID_ARGUMENT, "mode is null");
    }
...
irods::error db_open_op(irods::plugin_context& _ctx)
{
    ...
    // cache db creds
    try {
        const auto& db_plugin_map = irods::get_server_property<const std::unordered_map<std::string, boost::any>>(
        ...
        snprintf(icss.databaseUsername,
                 ...
                 boost::any_cast<const std::string&>(
                     boost::any_cast<const std::unordered_map<std::string, boost::any>>(db_plugin).at(
    ...
    if (0 != status) {
        return ERROR(status, "failed to open db connection");
    }
    ...
    // set pam properties
    try {
        irods_pam_auth_no_extend = irods::get_server_property<const bool>(std::vector<std::string>{
            irods::PLUGIN_TYPE_AUTHENTICATION, irods::AUTH_PAM_SCHEME, irods::CFG_PAM_NO_EXTEND_KW});
        irods_pam_password_len = irods::get_server_property<const size_t>(std::vector<std::string>{
            irods::PLUGIN_TYPE_AUTHENTICATION, irods::AUTH_PAM_SCHEME, irods::CFG_PAM_PASSWORD_LENGTH_KW});
        snprintf(
            irods_pam_password_min_time,
            NAME_LEN,
            "%s",
            irods::get_server_property<const std::string>(std::vector<std::string>{irods::PLUGIN_TYPE_AUTHENTICATION,
    ...
    if (irods_pam_auth_no_extend) {
        snprintf(irods_pam_password_default_time, sizeof(irods_pam_password_default_time), "%s", "28800");
    }
...
irods::error db_close_op(irods::plugin_context& _ctx)
{
    ...
    if (0 != status) {
        return ERROR(status, "failed to close db connection");
    }
                                           ...
                                           const char* _access)
{
    ...
    //        icatSessionStruct icss;
    //        _ctx.prop_map().get< icatSessionStruct >( ICSS_PROP, icss );
    rodsLong_t status = checkAndGetObjectId(_ctx.comm(), _ctx.prop_map(), _type, _name, _access);
    if (status < 0) {
                                     ...
                                     keyValPair_t* _reg_param)
{
    ...
    const char* regParamNames[] = {REPL_NUM_KW,
                                   DATA_TYPE_KW,
                                   DATA_SIZE_KW,
                                   RESC_NAME_KW,
                                   FILE_PATH_KW,
    ...
    const char* colNames[] = {"data_repl_num",
                              "data_type_name",
                              "data_size",
                              "resc_name",
                              "data_path",
    ...
    for (i = 0, j = 0; strcmp(regParamNames[i], "END"); i++) {
        ...
        if (theVal != NULL) {
            ...
            if (i == dataTypeIndex) {
                status = cmlCheckNameToken("data_type", theVal, &icss);
                if (status != 0) {
                    std::stringstream msg;
                    msg << __FUNCTION__;
                    msg << " - Invalid data type specified.";
                    addRErrorMsg(&_ctx.comm()->rError, 0, msg.str().c_str());
                    return ERROR(CAT_INVALID_DATA_TYPE, msg.str());
                }
            }
        }
    }
    ...
    */
    ...
    if (_data_obj_info->dataId <= 0) {
        ...
        }
        {
            ...
            status =
                cmlGetIntegerValueFromSql("select coll_id from R_COLL_MAIN where coll_name=?", &iVal, bindVars, &icss);
        }

        if (status != 0) {
            char errMsg[105];
            snprintf(errMsg, 100, "collection '%s' is unknown", logicalDirName);
            addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);
            _rollback("chlModDataObjMeta");
            return ERROR(CAT_UNKNOWN_COLLECTION, "failed with unknown collection");
        }
        ...
        }
        {
            ...
            status = cmlGetIntegerValueFromSql(
                "select data_id from R_DATA_MAIN where coll_id=? and data_name=?", &iVal, bindVars, &icss);
        }
        if (status != 0) {
            std::stringstream msg;
            msg << __FUNCTION__;
            ...
            addRErrorMsg(&_ctx.comm()->rError, 0, msg.str().c_str());
            _rollback("chlModDataObjMeta");
            return ERROR(CAT_UNKNOWN_FILE, "failed with unknown file");
        }
    ...
    if (adminMode) {
        ...
        if (doingDataSize == 1 && strlen(mySessionTicket) > 0) {
            status = cmlTicketUpdateWriteBytes(mySessionTicket, dataSizeString, objIdString, &icss);
            if (status != 0) {
                return ERROR(status, "cmlTicketUpdateWriteBytes failed");
            }
        }

        status = cmlCheckDataObjId(objIdString,
        ...
        if (status != 0) {
            ...
            if (theVal != NULL && upCols == 1 && strcmp(updateCols[0], "data_path") == 0) {
                ...
                if (strncmp(theVal, _data_obj_info->objPath, len) == 0) {
                    iVal = cmlCheckDir(
                        theVal, _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, ACCESS_OWN, &icss);
                }
                if (iVal > 0) {
                    status = 0;
            ...
            if (status) {
                _rollback("chlModDataObjMeta");
                return ERROR(CAT_NO_ACCESS_PERMISSION, "failed with no permission");
            }
        }
    }
    ...
    // If we are moving the data object from one resource to another resource, update the object counts for those
    // resources appropriately - hcj
    ...
    if (new_resc_hier != NULL) {
        ...
        rodsLong_t resc_id = 0;
        {
            ...
            status = cmlGetIntegerValueFromSql(
                "select resc_id from R_DATA_MAIN where data_id=? and data_repl_num=?", &resc_id, bindVars, &icss);
        }
        if (status != 0) {
            std::stringstream msg;
            msg << __FUNCTION__;
    ...
    if (mode == 0) {
        ...
        status = cmlModifySingleTable("R_DATA_MAIN",
                                      &(updateCols[0]),
                                      &(updateVals[0]),
                                      whereColsAndConds,
                                      whereValues,
        ...
        if (status == 0) {
            ...
            status2 = cmlModifySingleTable("R_DATA_MAIN",
                                           &(updateCols[0]),
                                           &(updateVals[0]),
                                           whereColsAndConds,
                                           whereValues,
            ...
            if (status2 != 0 && status2 != CAT_SUCCESS_BUT_WITH_NO_INFO) {
                ...
                rodsLog(LOG_NOTICE, "chlModDataObjMeta cmlModifySingleTable failure for other replicas %d", status2);
                _rollback("chlModDataObjMeta");
                return ERROR(status2, "cmlModifySingleTable failure for other replicas");
            }
        }
    }
    if (status != 0) {
        _rollback("chlModDataObjMeta");
        rodsLog(LOG_NOTICE, "chlModDataObjMeta cmlModifySingleTable failure %d", status);
        return ERROR(status, "cmlModifySingleTable failure");
    }

    if (!(_data_obj_info->flags & NO_COMMIT_FLAG)) {
        status = cmlExecuteNoAnswerSql("commit", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModDataObjMeta cmlExecuteNoAnswerSql commit failure %d", status);
...
irods::error db_reg_data_obj_op(irods::plugin_context& _ctx, dataObjInfo_t* _data_obj_info)
{
    ...
    if (seqNum < 0) {
        rodsLog(LOG_NOTICE, "chlRegDataObj cmlGetNextSeqVal failure %d", seqNum);
    ...
    }
    {
        ...
        status = cmlGetIntegerValueFromSql("select coll_id from R_COLL_MAIN where coll_name=?", &iVal, bindVars, &icss);
    }
    if (status == 0) {
    ...
    status = cmlCheckNameToken("data_type", _data_obj_info->dataType, &icss);
    if (status != 0) {
    ...
    status = cmlExecuteNoAnswerSql(
        ...
        "modify_ts, data_expiry_ts, resc_name, resc_hier, resc_group_name) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
        "?, ?, ?, ?, ?, ?, ?, ?)",
        &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegDataObj cmlExecuteNoAnswerSql failure %d", status);
        _rollback("chlRegDataObj");
    ...
    if (inheritFlag) {
        ...
        status = cmlExecuteNoAnswerSql(
            "insert into R_OBJT_ACCESS (object_id, user_id, access_type_id, create_ts, modify_ts) (select ?, user_id, "
            "access_type_id, ?, ? from R_OBJT_ACCESS where object_id = ?)",
            &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlRegDataObj cmlExecuteNoAnswerSql insert access failure %d", status);
            _rollback("chlRegDataObj");
        ...
        status = cmlExecuteNoAnswerSql(
            "insert into R_OBJT_ACCESS values (?, (select user_id from R_USER_MAIN where user_name=? and zone_name=?), "
    ...
    status = cmlAudit3(
        AU_REGISTER_DATA_OBJ, dataIdNum, _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, "", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegDataObj cmlAudit3 failure %d", status);
        _rollback("chlRegDataObj");
    ...
    if (!(_data_obj_info->flags & NO_COMMIT_FLAG)) {
        status = cmlExecuteNoAnswerSql("commit", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlRegDataObj cmlExecuteNoAnswerSql commit failure %d", status);
                               ...
                               keyValPair_t* _cond_input)
{
    ...
    // =-=-=-=-=-=-=-
    // check the params
    if (!_src_data_obj_info || !_dst_data_obj_info || !_cond_input) {
        return ERROR(CAT_INVALID_ARGUMENT, "null parameter");
    }

    if (_dst_data_obj_info->rescId <= 0) {
        std::stringstream msg;
        msg << "invalid resource id " << _dst_data_obj_info->rescId << " for [" << _dst_data_obj_info->objPath << "]";
        return ERROR(SYS_INVALID_INPUT_PARAM, msg.str());
    }
    ...
    if (adminMode) {
        ...
        status = cmlCheckDataObjOnly(logicalDirName,
                                     logicalFileName,
                                     _ctx.comm()->clientUser.userName,
                                     _ctx.comm()->clientUser.rodsZone,
                                     ACCESS_READ_OBJECT,
    ...
    }
    {
        ...
        bindVars.push_back(objIdString);
        status = cmlGetIntegerValueFromSql(
            "select max(data_repl_num) from R_DATA_MAIN where data_id = ?", &iVal, bindVars, &icss);
    }
    ...
    snprintf(tSQL, MAX_SQL_SIZE, "select %s from R_DATA_MAIN where data_id = ? and data_repl_num = ?", theColls);
    if (logSQL != 0) {
    ...
    snprintf(tSQL,
             MAX_SQL_SIZE,
             "insert into R_DATA_MAIN ( %s ) select ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,? from DUAL where not "
             "exists (select data_id from R_DATA_MAIN where resc_id=? and data_path=? and data_id=?)",
             theColls);
#else
    /* Postgres */
    snprintf(tSQL,
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "chlRegReplica cmlExecuteNoAnswerSql(insert) failure %d", status);
        _rollback("chlRegReplica");
    ...
    status = cmlAudit3(AU_REGISTER_DATA_REPLICA,
                       objIdString,
                       _ctx.comm()->clientUser.userName,
                       _ctx.comm()->clientUser.rodsZone,
                       nextRepl,
                       &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegDataReplica cmlAudit3 failure %d", status);
        _rollback("chlRegReplica");
        return ERROR(status, "cmlAudit3 failure");
    }
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegReplica cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswerSql commit failure");
    }
...
irods::error db_unreg_replica_op(irods::plugin_context& _ctx, dataObjInfo_t* _data_obj_info, keyValPair_t* _cond_input)
{
    ...
    if (adminMode == 0) {
        ...
        if (trashMode) {
            ...
            if (_data_obj_info->dataId > 0) {
                snprintf(dataObjNumber, sizeof dataObjNumber, "%lld", _data_obj_info->dataId);
            }
        }
        else {
            if (_data_obj_info->replNum >= 0 && _data_obj_info->dataId >= 0) {
                ...
                if (status != 0) {
                    addRErrorMsg(&_ctx.comm()->rError, 0, "This is the last replica, removal by admin not allowed");
                    return ERROR(CAT_LAST_REPLICA, "This is the last replica, removal by admin not allowed");
                }
            }
            else {
                addRErrorMsg(&_ctx.comm()->rError, 0, "dataId and replNum required");
                _rollback("chlUnregDataObj");
    ...
    if (_data_obj_info->replNum >= 0) {
        ...
        snprintf(tSQL,
                 MAX_SQL_SIZE,
                 "delete from R_DATA_MAIN where coll_id=(select coll_id from R_COLL_MAIN where coll_name=?) and "
                 "data_name=? and data_repl_num=?");
    }
    else {
        cllBindVarCount = 2;
        if (logSQL != 0) {
    ...
    if (status != 0) {
        if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
            ...
            snprintf(errMsg, 100, "data object '%s' is unknown", logicalFileName);
            addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);
    ...
    if (dataObjNumber[0] != '\0') {
        ...
        status = cmlExecuteNoAnswerSql(
            "delete from R_OBJT_ACCESS where object_id=? and not exists (select * from R_DATA_MAIN where data_id=?)",
            &icss);
        if (status == 0) {
    ...
    /* Audit */
    if (dataObjNumber[0] != '\0') {
        status = cmlAudit3(AU_UNREGISTER_DATA_OBJ,
                           dataObjNumber,
                           _ctx.comm()->clientUser.userName,
                           _ctx.comm()->clientUser.rodsZone,
                           "",
    ...
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlUnregDataObj cmlAudit3 failure %d", status);
        _rollback("chlUnregDataObj");
        return ERROR(status, "cmlAudit3 failure");
    }
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlUnregDataObj cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswerSql commit failure");
    }
...
irods::error db_reg_rule_exec_op(irods::plugin_context& _ctx, ruleExecSubmitInp_t* _re_sub_inp)
{
    ...
    status = cmlExecuteNoAnswerSql("insert into R_RULE_EXEC (rule_exec_id, rule_name, rei_file_path, user_name, "
                                   "exe_address, exe_time, exe_frequency, priority, estimated_exe_time, "
                                   "notification_addr, create_ts, modify_ts) values (?,?,?,?,?,?,?,?,?,?,?,?)",
                                   &icss);
    if (status != 0) {
    ...
    /* Audit */
    status = cmlAudit3(AU_REGISTER_DELAYED_RULE,
                       ruleExecIdNum,
                       _ctx.comm()->clientUser.userName,
                       _ctx.comm()->clientUser.rodsZone,
                       ...
                       &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegRuleExec cmlAudit3 failure %d", status);
        _rollback("chlRegRuleExec");
        return ERROR(status, "cmlAudit3 failure");
    }
...
irods::error db_mod_rule_exec_op(irods::plugin_context& _ctx, const char* _re_id, keyValPair_t* _reg_param)
{
    ...
    if (status != 0) {
        ...
        rodsLog(LOG_NOTICE, "chlModRuleExec cmlExecuteNoAnswer(update) failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswer(update) failure");
    }

    /* Audit */
    status = cmlAudit3(
        AU_MODIFY_DELAYED_RULE, _re_id, _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, "", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModRuleExec cmlAudit3 failure %d", status);
        _rollback("chlModRuleExec");
        return ERROR(status, "cmlAudit3 failure");
    }
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModRuleExecMeta cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswerSql commit failure");
    }
...
irods::error db_del_rule_exec_op(irods::plugin_context& _ctx, const char* _re_id)
{
    ...
    if (_ctx.comm()->proxyUser.authInfo.authFlag < LOCAL_PRIV_USER_AUTH) {
        if (_ctx.comm()->proxyUser.authInfo.authFlag == LOCAL_USER_AUTH) {
            ...
            int status;
            {
                ...
                status = cmlGetStringValueFromSql(
                    "select user_name from R_RULE_EXEC where rule_exec_id=?", userName, MAX_NAME_LEN, bindVars, &icss);
            }
            if (status != 0 || strncmp(userName, _ctx.comm()->clientUser.userName, MAX_NAME_LEN) != 0) {
    ...
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlDelRuleExec delete failure %d", status);
    ...
    /* Audit */
    status = cmlAudit3(
        AU_DELETE_DELAYED_RULE, _re_id, _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, "", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlDelRuleExec cmlAudit3 failure %d", status);
        _rollback("chlDelRuleExec");
        return ERROR(status, "cmlAudit3 failure");
    }
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlDelRuleExec cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswerSql commit failure");
    }
                                                               ...
                                                               std::string& _resc_parent)
{
    ...
    irods::error ret = resc_mgr.resolve(_resc_name, resc);
    if (!ret.ok()) {
        return PASS(ret);
    }

    ret = resc->get_property<std::string>(irods::RESOURCE_PARENT, _resc_parent);
    if (!ret.ok()) {
        return PASS(ret);
    }

    rodsLong_t resc_id;
    ret = resc->get_property<rodsLong_t>(irods::RESOURCE_ID, resc_id);
    if (!ret.ok()) {
        return PASS(ret);
    }
    ...
    try {
        ...
        return ERROR(INVALID_LEXICAL_CAST, msg.str());
    }
...
irods::error db_add_child_resc_op(irods::plugin_context& _ctx, std::map<std::string, std::string>* _resc_input)
{
    ...
    ret = extract_resource_properties_for_operations(child_name, child_resource_id, child_parent_name);

    if (!ret.ok()) {
        if (SYS_RESC_DOES_NOT_EXIST == ret.code()) {
            return ERROR(CHILD_NOT_FOUND, child_parent_name.c_str());
        }
        return PASS(ret);
    }
    ...
    if (!child_parent_name.empty()) {
        std::stringstream msg;
        msg << "Encountered an error adding '" << child_name << "' as a child resource.";
        addRErrorMsg(&_ctx.comm()->rError, 0, msg.str().c_str());
        return ERROR(CHILD_HAS_PARENT, msg.str());
    }

    std::string& parent_name = resc_input[irods::RESOURCE_NAME];
    std::string parent_resource_id;
    std::string parent_parent_name;
    ret = extract_resource_properties_for_operations(parent_name, parent_resource_id, parent_parent_name);
    if (!ret.ok()) {
        if (SYS_RESC_DOES_NOT_EXIST == ret.code()) {
            return ERROR(CAT_INVALID_RESOURCE, child_parent_name.c_str());
        }
        return PASS(ret);
    }
    ...
    if (0 != status) {
        return ERROR(status, "_canConnectToCatalog failed");
    }
    ...
    if (resc_input[irods::RESOURCE_ZONE].length() > 0 && resc_input[irods::RESOURCE_ZONE] != zone) {
        addRErrorMsg(&_ctx.comm()->rError, 0, "Currently, resources must be in the local zone");
    ...
    ret = _updateChildParent(child_resource_id, parent_resource_id, child_context);
    if (!ret.ok()) {
        return PASS(ret);
    }
...
irods::error db_reg_resc_op(irods::plugin_context& _ctx, std::map<std::string, std::string>* _resc_input)
{
    ...
    seqNum = cmlGetNextSeqVal(&icss);
    if (seqNum < 0) {
        rodsLog(LOG_NOTICE, "chlRegResc cmlGetNextSeqVal failure %d", seqNum);
    ...
    status = cmlExecuteNoAnswerSql("insert into R_RESC_MAIN (resc_id, resc_name, zone_name, resc_type_name, "
                                   "resc_class_name, resc_net, resc_def_path, create_ts, modify_ts, resc_children, "
                                   "resc_context, resc_parent) values (?,?,?,?,?,?,?,?,?,?,?,?)",
                                   &icss);

    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegResc cmlExectuteNoAnswerSql(insert) failure %d", status);
        _rollback("chlRegResc");
        return ERROR(status, "cmlExectuteNoAnswerSql(insert) failure");
    }

    /* Audit */
    status = cmlAudit3(AU_REGISTER_RESOURCE,
                       idNum,
                       _ctx.comm()->clientUser.userName,
                       _ctx.comm()->clientUser.rodsZone,
                       ...
                       &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegResc cmlAudit3 failure %d", status);
        _rollback("chlRegResc");
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegResc cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswerSql commit failure");
    }
...
irods::error db_del_child_resc_op(irods::plugin_context& _ctx, std::map<std::string, std::string>* _resc_input)
{
    ...
    ret = extract_resource_properties_for_operations(parent_name, parent_resource_id, parent_parent_resource_id);
    if (!ret.ok()) {
        return PASS(ret);
    }
    ...
    ret = extract_resource_properties_for_operations(child_name, child_resource_id, child_parent_resource_id);
    if (!ret.ok()) {
        return PASS(ret);
    }
    ...
    ret = _updateChildParent(child_resource_id, std::string(""), std::string(""));
    if (!ret.ok()) {
        return PASS(ret);
    }
...
irods::error db_del_resc_op(irods::plugin_context& _ctx, const char* _resc_name, int _dry_run)
{
    ...
    if (strncmp(_resc_name, BUNDLE_RESC, strlen(BUNDLE_RESC)) == 0) {
        char errMsg[155];
        snprintf(errMsg, 150, "%s is a built-in resource needed for bundle operations.", BUNDLE_RESC);
        addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "%s - _rescHasData failed for [%s] %d", __FUNCTION__, _resc_name, status);
        return ERROR(status, "failed to get object count for resource");
    }

    if (has_data) {
        char errMsg[105];
        snprintf(errMsg, 100, "resource '%s' contains one or more dataObjects", _resc_name);
        addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);
    ...
    }
    {
        ...
        status = cmlGetStringValueFromSql(
            "select resc_id from R_RESC_MAIN where resc_name=?", rescId, MAX_NAME_LEN, bindVars, &icss);
    }
    if (status != 0) {
        if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
            char errMsg[105];
            snprintf(errMsg, 100, "resource '%s' does not exist", _resc_name);
            addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);
    ...
    if (_rescHasParentOrChild(rescId)) {
        char errMsg[105];
        snprintf(errMsg, 100, "resource '%s' has a parent or child", _resc_name);
        addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);
    ...
    status = cmlExecuteNoAnswerSql("delete from R_RESC_MAIN where resc_name=?", &icss);
    if (status != 0) {
        if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
            char errMsg[105];
            snprintf(errMsg, 100, "resource '%s' does not exist", _resc_name);
    ...
    status = cmlAudit3(AU_DELETE_RESOURCE,
                       ...
                       &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlDelResc cmlAudit3 failure %d", status);
        _rollback("chlDelResc");
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlDelResc cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswerSql commit failure");
    }
    return CODE(status);
...
irods::error db_rollback_op(irods::plugin_context& _ctx)
{
    ...
    int status = cmlExecuteNoAnswerSql("rollback", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRollback cmlExecuteNoAnswerSql failure %d", status);
...
irods::error db_commit_op(irods::plugin_context& _ctx)
{
    ...
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlCommit cmlExecuteNoAnswerSql failure %d", status);
...
irods::error db_del_user_re_op(irods::plugin_context& _ctx, userInfo_t* _user_info)
{
    ...
    }
    {
        ...
        bindVars.push_back(zoneToUse);
        status = cmlGetStringValueFromSql(
            "select user_id from R_USER_MAIN where user_name=? and zone_name=?", iValStr, 200, bindVars, &icss);
    }
    ...
    status = cmlExecuteNoAnswerSql("delete from R_USER_MAIN where user_name=? and zone_name=?", &icss);
    if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
        return ERROR(CAT_INVALID_USER, "invalid user");
    }
    if (status != 0) {
    ...
    if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
        ...
        rodsLog(LOG_NOTICE, "chlDelUserRE delete password failure %d", status);
    ...
    status = cmlExecuteNoAnswerSql("delete from R_USER_GROUP where user_id=? or group_user_id=?", &icss);
    if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
        char errMsg[MAX_NAME_LEN + 40];
        rodsLog(LOG_NOTICE, "chlDelUserRE delete user_group entry failure %d", status);
    ...
    status = cmlExecuteNoAnswerSql("delete from R_USER_AUTH where user_id=?", &icss);
    if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
        char errMsg[MAX_NAME_LEN + 40];
        rodsLog(LOG_NOTICE, "chlDelUserRE delete user_auth entries failure %d", status);
    ...
    /* Audit */
    snprintf(userStr, sizeof userStr, "%s#%s", userName2, zoneToUse);
    status = cmlAudit3(
        AU_DELETE_USER_RE, iValStr, _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, userStr, &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlDelUserRE cmlAudit3 failure %d", status);
        _rollback("chlDelUserRE");
...
irods::error db_reg_coll_by_admin_op(irods::plugin_context& _ctx, collInfo_t* _coll_info)
{
    ...
    if (_ctx.comm()->clientUser.authInfo.authFlag < LOCAL_PRIV_USER_AUTH ||
        ...
        int status2;
        status2 =
            cmlCheckGroupAdminAccess(_ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, "", &icss);
        if (status2 != 0) {
    ...
    snprintf(tSQL,
             MAX_SQL_SIZE,
             "insert into R_COLL_MAIN (coll_id, parent_coll_name, coll_name, coll_owner_name, coll_owner_zone, "
             "coll_type, coll_info1, coll_info2, create_ts, modify_ts) values (%s, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
             nextStr);
    ...
    if (status != 0) {
        ...
        if (status == CATALOG_ALREADY_HAS_ITEM_BY_THAT_NAME) {
            snprintf(errMsg, 100, "Error %d %s", status, "CATALOG_ALREADY_HAS_ITEM_BY_THAT_NAME");
            addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);
        }

        rodsLog(LOG_NOTICE, "chlRegCollByAdmin cmlExecuteNoAnswerSQL(insert) failure %d", status);
        _rollback("chlRegCollByAdmin");
        return ERROR(status, "cmlExecuteNoAnswerSQL(insert) failure");
    }
    ...
    snprintf(
        tSQL,
        MAX_SQL_SIZE,
        "insert into R_OBJT_ACCESS values (%s, (select user_id from R_USER_MAIN where user_name=? and zone_name=?), "
        "(select token_id from R_TOKN_MAIN where token_namespace = 'access_type' and token_name = ?), ?, ?)",
        currStr2);
    if (logSQL != 0) {
    ...
    status = cmlExecuteNoAnswerSql(tSQL, &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegCollByAdmin cmlExecuteNoAnswerSql(insert access) failure %d", status);
        _rollback("chlRegCollByAdmin");
        return ERROR(status, "cmlExecuteNoAnswerSql(insert access) failure");
    }
    ...
    /* Audit */
    status = cmlAudit4(
        AU_REGISTER_COLL_BY_ADMIN, currStr2, "", userName2, zoneName, _ctx.comm()->clientUser.userName, &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegCollByAdmin cmlAudit4 failure %d", status);
        _rollback("chlRegCollByAdmin");
...
irods::error db_reg_coll_op(irods::plugin_context& _ctx, collInfo_t* _coll_info)
{
    ...
    }
    {
        ...
        status = cmlGetIntegerValueFromSql(
            "select data_id from R_DATA_MAIN where data_name=? and coll_id=?", &iVal, bindVars, &icss);
    }
    ...
    status = cmlExecuteNoAnswerSql(tSQL, &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegColl cmlExecuteNoAnswerSql(insert) failure %d", status);
        _rollback("chlRegColl");
        return ERROR(status, "cmlExecuteNoAnswerSql(insert) failure");
    }
    ...
    if (inheritFlag) {
        ...
        snprintf(tSQL,
                 MAX_SQL_SIZE,
                 "insert into R_OBJT_ACCESS (object_id, user_id, access_type_id, create_ts, modify_ts) (select %s, "
                 "user_id, access_type_id, ?, ? from R_OBJT_ACCESS where object_id = ?)",
                 currStr2);
        status = cmlExecuteNoAnswerSql(tSQL, &icss);
        ...
        if (status == 0) {
            ...
            */
            ...
            if (status > 0) {
                ...
                status = cmlExecuteNoAnswerSql("update R_COLL_MAIN set coll_inheritance=?, modify_ts=? where coll_id=?",
                                               &icss);
            }
#else
            /*
        ...
        snprintf(tSQL,
                 MAX_SQL_SIZE,
                 "insert into R_OBJT_ACCESS values (%s, (select user_id from R_USER_MAIN where user_name=? and "
                 "zone_name=?), (select token_id from R_TOKN_MAIN where token_namespace = 'access_type' and token_name "
                 "= ?), ?, ?)",
                 currStr2);
        if (logSQL != 0) {
    ...
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegColl cmlExecuteNoAnswerSql(insert access) failure %d", status);
        _rollback("chlRegColl");
        return ERROR(status, "cmlExecuteNoAnswerSql(insert access) failure");
    }
    ...
    status = cmlAudit4(AU_REGISTER_COLL,
                       ...
                       &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegColl cmlAudit4 failure %d", status);
        _rollback("chlRegColl");
        return ERROR(status, "cmlAudit4 failure");
    }
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegColl cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswerSql commit failure");
    }
...
irods::error db_mod_coll_op(irods::plugin_context& _ctx, collInfo_t* _coll_info)
{
    ...
    status = cmlExecuteNoAnswerSql(tSQL.c_str(), &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModColl cmlExecuteNoAnswerSQL(update) failure %d", status);
                            ...
                            const char* _zone_comment)
{
    ...
    // =-=-=-=-=-=-=-
    // check the params
    if (!_zone_name || !_zone_type || !_zone_conn_info || !_zone_comment) {
        return ERROR(CAT_INVALID_ARGUMENT, "null parameter");
    }
    ...
    if (strncmp(_zone_type, "remote", 6) != 0) {
        addRErrorMsg(&_ctx.comm()->rError, 0, "Currently, only zones of type 'remote' are allowed");
    ...
    snprintf(tSQL,
             MAX_SQL_SIZE,
             "insert into R_ZONE_MAIN (zone_id, zone_name, zone_type_name, zone_conn_string, r_comment, create_ts, "
             "modify_ts) values (%s, ?, 'remote', ?, ?, ?, ?)",
             nextStr);
    status = cmlExecuteNoAnswerSql(tSQL, &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegZone cmlExecuteNoAnswerSql(insert) failure %d", status);
    ...
    /* Audit */
    status =
        cmlAudit3(AU_REGISTER_ZONE, "0", _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, "", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegResc cmlAudit3 failure %d", status);
        return ERROR(status, "cmlAudit3 failure");
    }
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegZone cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswerSql commit failure");
    }
                            ...
                            const char* _option_value)
{
    ...
    // =-=-=-=-=-=-=-
    // check the params
    if (!_zone_name || !_option || !_option_value) {
        return ERROR(CAT_INVALID_ARGUMENT, "null parameter");
    }
    ...
    }
    {
        ...
        status = cmlGetStringValueFromSql(
            "select zone_id from R_ZONE_MAIN where zone_name=?", zoneId, MAX_NAME_LEN, bindVars, &icss);
    }
    if (status != 0) {
        if (status == CAT_NO_ROWS_FOUND) {
    ...
    if (strcmp(_option, "comment") == 0) {
        ...
        status = cmlExecuteNoAnswerSql("update R_ZONE_MAIN set r_comment = ?, modify_ts=? where zone_id=?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModZone cmlExecuteNoAnswerSql update failure %d", status);
            return ERROR(status, "cmlExecuteNoAnswerSql update failure");
        }
        OK = 1;
    }
    if (strcmp(_option, "conn") == 0) {
        ...
        status =
            cmlExecuteNoAnswerSql("update R_ZONE_MAIN set zone_conn_string = ?, modify_ts=? where zone_id=?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModZone cmlExecuteNoAnswerSql update failure %d", status);
            return ERROR(status, "cmlExecuteNoAnswerSql update failure");
        }
    ...
    if (strcmp(_option, "name") == 0) {
        ...
        status = cmlExecuteNoAnswerSql("update R_ZONE_MAIN set zone_name = ?, modify_ts=? where zone_id=?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModZone cmlExecuteNoAnswerSql update failure %d", status);
            return ERROR(status, "cmlExecuteNoAnswerSql update failure");
        }
    ...
    status = cmlAudit3(
        AU_MOD_ZONE, zoneId, _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, commentStr, &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModZone cmlAudit3 failure %d", status);
        return ERROR(status, "cmlAudit3 failure");
    }

    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModZone cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswerSql commit failure");
    }
...
irods::error db_rename_coll_op(irods::plugin_context& _ctx, const char* _old_coll, const char* _new_coll)
{
    ...
    status1 =
        cmlCheckDir(_old_coll, _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, ACCESS_OWN, &icss);
                                     ...
                                     const char* _path_name)
{
    ...
    // =-=-=-=-=-=-=-
    // check the params
    if (!_access_level || !_user_name || !_path_name) {
        return ERROR(CAT_INVALID_ARGUMENT, "null parameter");
    }
    ...
    status =
        chlModAccessControl(_ctx.comm(), 0, _access_level, _user_name, _ctx.comm()->clientUser.rodsZone, _path_name);
    if (!status) {
...
irods::error db_rename_local_zone_op(irods::plugin_context& _ctx, const char* _old_zone, const char* _new_zone)
{
    ...
    snprintf(commentStr, sizeof commentStr, "renamed local zone %s to %s", _old_zone, _new_zone);
    status = cmlAudit3(
        AU_MOD_ZONE, "0", _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, commentStr, &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRenameLocalZone cmlAudit3 failure %d", status);
        return ERROR(status, "cmlAudit3 failure");
    }
    ...
    status =
        cmlExecuteNoAnswerSql("update R_COLL_MAIN set coll_owner_zone = ?, modify_ts=? where coll_owner_zone=?", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRenameLocalZone cmlExecuteNoAnswerSql update failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswerSql update failure");
    }
    ...
    status =
        cmlExecuteNoAnswerSql("update R_DATA_MAIN set data_owner_zone = ?, modify_ts=? where data_owner_zone=?", &icss);
    if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
        rodsLog(LOG_NOTICE, "chlRenameLocalZone cmlExecuteNoAnswerSql update failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswerSql update failure");
    }
    ...
    status = cmlExecuteNoAnswerSql("update R_RESC_MAIN set zone_name = ?, modify_ts=? where zone_name=?", &icss);
    if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
        rodsLog(LOG_NOTICE, "chlRenameLocalZone cmlExecuteNoAnswerSql update failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswerSql update failure");
    }
    ...
    status =
        cmlExecuteNoAnswerSql("update R_RULE_MAIN set rule_owner_zone=?, modify_ts=? where rule_owner_zone=?", &icss);
    if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
        rodsLog(LOG_NOTICE, "chlRenameLocalZone cmlExecuteNoAnswerSql update failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswerSql update failure");
    }
    ...
    status = cmlExecuteNoAnswerSql("update R_USER_MAIN set zone_name=?, modify_ts=? where zone_name=?", &icss);
    if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
        rodsLog(LOG_NOTICE, "chlRenameLocalZone cmlExecuteNoAnswerSql update failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswerSql update failure");
    }
    ...
    status = cmlExecuteNoAnswerSql("update R_ZONE_MAIN set zone_name=?, modify_ts=? where zone_name=?", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRenameLocalZone cmlExecuteNoAnswerSql update failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswerSql update failure");
    }
...
irods::error db_del_zone_op(irods::plugin_context& _ctx, const char* _zone_name)
{
...

    {
        ...
        bindVars.push_back(_zone_name);
        status = cmlGetStringValueFromSql(
            "select zone_type_name from R_ZONE_MAIN where zone_name=?", zoneType, MAX_NAME_LEN, bindVars, &icss);
    }
    if (status != 0) {
        if (status == CAT_NO_ROWS_FOUND) {
            return ERROR(CAT_INVALID_ZONE, "invalid zone name");
    ...
    if (strcmp(zoneType, "remote") != 0) {
        addRErrorMsg(&_ctx.comm()->rError, 0, "It is not permitted to remove the local zone");
    ...
    status = cmlExecuteNoAnswerSql("delete from R_ZONE_MAIN where zone_name = ?", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlDelZone cmlExecuteNoAnswerSql delete failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswerSql delete failure");
    }

    /* Audit */
    status = cmlAudit3(
        AU_DELETE_ZONE, "0", _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, _zone_name, &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlDelZone cmlAudit3 failure %d", status);
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlDelZone cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "cmlExecuteNoAnswerSql commit failure");
    }
                                       ...
                                       int _max_out_buf)
{
    ...
    char* allowedSQL[] = {
        ...
        "select user_name||'#'||zone_name from R_USER_MAIN, R_USER_GROUP where "
        "R_USER_GROUP.user_id=R_USER_MAIN.user_id and R_USER_GROUP.group_user_id=(select user_id from R_USER_MAIN "
        "where user_name=?)",
        "select * from R_DATA_MAIN where data_id=?",
        "select data_name, data_id, data_repl_num from R_DATA_MAIN where coll_id =(select coll_id from R_COLL_MAIN "
        "where coll_name=?)",
        ...
        "select R_RESC_GROUP.resc_group_name, R_RESC_GROUP.resc_id, resc_name, R_RESC_GROUP.create_ts, "
        "R_RESC_GROUP.modify_ts from R_RESC_MAIN, R_RESC_GROUP where R_RESC_MAIN.resc_id = R_RESC_GROUP.resc_id and "
        "resc_group_name=?",
        ...
        "select user_name, user_auth_name from R_USER_AUTH, R_USER_MAIN where R_USER_AUTH.user_id = "
        "R_USER_MAIN.user_id and R_USER_MAIN.user_name=?",
        "select user_name, user_auth_name from R_USER_AUTH, R_USER_MAIN where R_USER_AUTH.user_id = "
        "R_USER_MAIN.user_id and R_USER_MAIN.user_name=? and R_USER_MAIN.zone_name=?",
        "select user_name, user_auth_name from R_USER_AUTH, R_USER_MAIN where R_USER_AUTH.user_id = "
        "R_USER_MAIN.user_id",
        "select user_name, user_auth_name from R_USER_AUTH, R_USER_MAIN where R_USER_AUTH.user_id = "
        "R_USER_MAIN.user_id and R_USER_AUTH.user_auth_name=?",
        "select user_name, R_USER_MAIN.zone_name, resc_name, quota_limit, quota_over, R_QUOTA_MAIN.modify_ts from "
        "R_QUOTA_MAIN, R_USER_MAIN, R_RESC_MAIN where R_USER_MAIN.user_id = R_QUOTA_MAIN.user_id and "
        "R_RESC_MAIN.resc_id = R_QUOTA_MAIN.resc_id",
        "select user_name, R_USER_MAIN.zone_name, resc_name, quota_limit, quota_over, R_QUOTA_MAIN.modify_ts from "
        "R_QUOTA_MAIN, R_USER_MAIN, R_RESC_MAIN where R_USER_MAIN.user_id = R_QUOTA_MAIN.user_id and "
        "R_RESC_MAIN.resc_id = R_QUOTA_MAIN.resc_id and user_name=? and R_USER_MAIN.zone_name=?",
        "select user_name, R_USER_MAIN.zone_name, quota_limit, quota_over, R_QUOTA_MAIN.modify_ts from R_QUOTA_MAIN, "
        "R_USER_MAIN where R_USER_MAIN.user_id = R_QUOTA_MAIN.user_id and R_QUOTA_MAIN.resc_id = 0",
        "select user_name, R_USER_MAIN.zone_name, quota_limit, quota_over, R_QUOTA_MAIN.modify_ts from R_QUOTA_MAIN, "
        "R_USER_MAIN where R_USER_MAIN.user_id = R_QUOTA_MAIN.user_id and R_QUOTA_MAIN.resc_id = 0 and user_name=? and "
    ...
    rows = 0;
    if (*_control == 0) {
        status = cmlGetFirstRowFromSqlBV(_sql, _bindVars, &stmtNum, &icss);
        if (status < 0) {
            if (status != CAT_NO_ROWS_FOUND) {
                rodsLog(LOG_NOTICE, "chlSimpleQuery cmlGetFirstRowFromSqlBV failure %d", status);
            }
    ...
    for (;;) {
        if (needToGet) {
            ...
            if (status < 0) {
                rodsLog(LOG_NOTICE, "chlSimpleQuery cmlGetNextRowFromStatement failure %d", status);
...
irods::error db_del_coll_by_admin_op(irods::plugin_context& _ctx, collInfo_t* _coll_info)
{
    ...
    if (status != CAT_NO_ROWS_FOUND) {
        if (status == 0) {
            char errMsg[105];
            snprintf(errMsg, 100, "collection '%s' is not empty", _coll_info->collName);
            addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);
    ...
    status = cmlExecuteNoAnswerSql(
        "delete from R_OBJT_ACCESS where object_id=(select coll_id from R_COLL_MAIN where coll_name=?)", &icss);
    if (status != 0) {
        /* error, but let it fall thru to below, probably doesn't exist */
        rodsLog(LOG_NOTICE, "chlDelCollByAdmin delete access failure %d", status);
        _rollback("chlDelCollByAdmin");
    }
    ...
    if (status != 0) {
        ...
        msg << "db_del_coll_by_admin_op: should be exactly one collection id corresponding to collection name ["
            << _coll_info->collName << "]. status [" << status << "]";
    ...
    status = cmlAudit4(AU_DELETE_COLL_BY_ADMIN,
                       ...
                       &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlDelCollByAdmin cmlAudit4 failure %d", status);
        _rollback("chlDelCollByAdmin");
        return ERROR(status, "cmlAudit4 failure");
    }
    ...
    status = cmlExecuteNoAnswerSql("delete from R_COLL_MAIN where coll_name=?", &icss);

    if (status != 0) {
        char errMsg[105];
        snprintf(errMsg, 100, "collection '%s' is unknown", _coll_info->collName);
        addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);
        _rollback("chlDelCollByAdmin");
        return ERROR(CAT_UNKNOWN_COLLECTION, "unknown collection");
    }
...
irods::error db_del_coll_op(irods::plugin_context& _ctx, collInfo_t* _coll_info)
{
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlDelColl cmlExecuteNoAnswerSql commit failure %d", status);
    ...
    *_client_priv_level = NO_USER_AUTH;
...

    {
        ...
        status = cmlGetMultiRowStringValuesFromSql(
            "select rcat_password, pass_expiry_ts, R_USER_PASSWORD.create_ts, R_USER_PASSWORD.modify_ts from "
            "R_USER_PASSWORD, "
    ...
    if (nPasswords == MAX_PASSWORDS) {
        {
            ...
            status = cmlGetIntegerValueFromSql(
                "select count(UP.user_id) from R_USER_PASSWORD UP, R_USER_MAIN where user_name=?",
                &MAX_PASSWORDS,
                bindVars,
                &icss);
        }
    ...
    for (k = 0; OK == 0 && k < MAX_PASSWORDS && k < nPasswords; k++) {
        ...
        obfMakeOneWayHash(hashType, (unsigned char*) md5Buf, CHALLENGE_LEN + MAX_PASSWORD_LEN, (unsigned char*) digest);
    ...
    if ((strncmp(goodPwExpiry, "9999", 4) != 0) && expireTime >= pamMinTime && expireTime <= pamMaxTime) {
        time_t modTime;
        ...
        if (modTime + expireTime < nowTime) {
            ...
            status =
                cmlExecuteNoAnswerSql("delete from R_USER_PASSWORD where rcat_password=? and create_ts=? and user_id = "
                                      "(select user_id from R_USER_MAIN where user_name=? and zone_name=?)",
                                      &icss);
            memset(goodPw, 0, sizeof(goodPw));
            memset(lastPw, 0, sizeof(lastPw));
            if (status != 0) {
                rodsLog(LOG_NOTICE, "chlCheckAuth cmlExecuteNoAnswerSql delete expired password failure %d", status);
                return ERROR(status, "delete expired password failure");
            }
            status = cmlExecuteNoAnswerSql("commit", &icss);
            if (status != 0) {
                rodsLog(LOG_NOTICE, "chlCheckAuth cmlExecuteNoAnswerSql commit failure %d", status);
                return ERROR(status, "commit failure");
            }
    ...
    if (expireTime < temp_password_max_time) {
        ...
        status = cmlExecuteNoAnswerSql("delete from R_USER_PASSWORD where rcat_password=?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlCheckAuth cmlExecuteNoAnswerSql delete failure %d", status);
        ...
        snprintf(expireStrCreate, sizeof expireStrCreate, "%011d", (int) pwExpireMaxCreateTime);
        cllBindVars[cllBindVarCount++] = expireStrCreate;

        status = cmlExecuteNoAnswerSql("delete from R_USER_PASSWORD where pass_expiry_ts = ? and create_ts < ?", &icss);
        if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
            rodsLog(LOG_NOTICE, "chlCheckAuth cmlExecuteNoAnswerSql delete2 failure %d", status);
            _rollback("chlCheckAuth");
...
checkLevel:
    ...
    }
    {
        ...
        bindVars.push_back(userName2);
        bindVars.push_back(myUserZone);
        status = cmlGetStringValueFromSql("select user_type_name from R_USER_MAIN where user_name=? and zone_name=?",
                                          userType,
                                          MAX_NAME_LEN,
                                          bindVars,
                                          &icss);
...
irods::error db_make_temp_pw_op(irods::plugin_context& _ctx, char* _pw_value_to_hash, const char* _other_user)
{
    ...
    snprintf(tSQL,
             MAX_SQL_SIZE,
             "select rcat_password from R_USER_PASSWORD, R_USER_MAIN where user_name=? and R_USER_MAIN.zone_name=? and "
             "R_USER_MAIN.user_id = R_USER_PASSWORD.user_id and pass_expiry_ts != '%d'",
             temp_password_time);
    ...
    obfMakeOneWayHash(HASH_TYPE_DEFAULT, (unsigned char*) md5Buf, 100, (unsigned char*) digest);
    ...
    cllBindVars[cllBindVarCount++] = _ctx.comm()->clientUser.rodsZone, cllBindVars[cllBindVarCount++] = newPw;
    ...
    status = cmlExecuteNoAnswerSql(
        "insert into R_USER_PASSWORD (user_id, rcat_password, pass_expiry_ts,  create_ts, modify_ts) values ((select "
        "user_id from R_USER_MAIN where user_name=? and zone_name=?), ?, ?, ?, ?)",
        &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlMakeTempPw cmlExecuteNoAnswerSql insert failure %d", status);
        _rollback("chlMakeTempPw");
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlMakeTempPw cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "commit failed");
    }
...
irods::error db_make_limited_pw_op(irods::plugin_context& _ctx, int _ttl, char* _pw_value_to_hash)
{
    ...
    pamMaxTime = atoll(irods_pam_password_max_time);
    pamMinTime = atoll(irods_pam_password_min_time);
    if (timeToLive < pamMinTime || timeToLive > pamMaxTime) {
    ...
    status = cmlExecuteNoAnswerSql(
        ...
        &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlMakeLimitedPw cmlExecuteNoAnswerSql insert failure %d", status);
        _rollback("chlMakeLimitedPw");
    ...
    cllBindVars[cllBindVarCount++] = myTime;
#if MY_ICAT
    cmlExecuteNoAnswerSql("delete from R_USER_PASSWORD where pass_expiry_ts not like '9999%' and cast(pass_expiry_ts "
                          "as signed integer)>=? and cast(pass_expiry_ts as signed integer)<=? and "
                          "(cast(pass_expiry_ts as signed integer) + cast(modify_ts as signed integer) < ?)",
                          &icss);
#else
    cmlExecuteNoAnswerSql(
        "delete from R_USER_PASSWORD where pass_expiry_ts not like '9999%' and cast(pass_expiry_ts as integer)>=? and "
        "cast(pass_expiry_ts as integer)<=? and (cast(pass_expiry_ts as integer) + cast(modify_ts as integer) < ?)",
        &icss);
#endif

    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlMakeLimitedPw cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "commit failed");
    }
                                       ...
                                       char** _irods_password)
{
    ...
    if (_ttl == 0) {
        ...
        pamMaxTime = atoll(irods_pam_password_max_time);
        _ttl = _ttl * 3600;
        if (_ttl < pamMinTime || _ttl > pamMaxTime) {
    ...
    cmlExecuteNoAnswerSql(
        ...
        "cast(pass_expiry_ts as integer)<=? and (cast(pass_expiry_ts as integer) + cast(modify_ts as integer) < ?)",
                          ...
                          &icss);
    if (logSQL != 0)
        rodsLog(LOG_SQL, "chlUpdateIrodsPamPassword SQL 3");
...
#if MY_ICAT
            "select rcat_password, modify_ts from R_USER_PASSWORD where user_id=? and pass_expiry_ts not like '9999%' "
            "and cast(pass_expiry_ts as signed integer) >= ? and cast (pass_expiry_ts as signed integer) <= ?",
#else
            "select rcat_password, modify_ts from R_USER_PASSWORD where user_id=? and pass_expiry_ts not like '9999%' "
            "and cast(pass_expiry_ts as integer) >= ? and cast (pass_expiry_ts as integer) <= ?",
#endif
            cVal,
            iVal,
            2,
    ...
    if (status == 0) {
        if (!irods_pam_auth_no_extend) {
            ...
            status = cmlExecuteNoAnswerSql("commit", &icss);
            if (status != 0) {
                rodsLog(LOG_NOTICE, "chlUpdateIrodsPamPassword cmlExecuteNoAnswerSql commit failure %d", status);
                return ERROR(status, "commit failure");
            }
        } // if !irods_pam_auth_no_extend
    ...
    while (!pw_good) {
        ...
        if (!strstr(randomPwEncoded, "\'")) {
            ...
            rodsLog(LOG_DEBUG,
                    "chlUpdateIrodsPamPassword :: getting a new password [%s] has a single quote",
                    randomPwEncoded);
        }
    ...
    if (logSQL != 0)
        rodsLog(LOG_SQL, "chlUpdateIrodsPamPassword SQL 5");
    cllBindVars[cllBindVarCount++] = selUserId;
    cllBindVars[cllBindVarCount++] = randomPwEncoded;
    cllBindVars[cllBindVarCount++] = expTime;
    cllBindVars[cllBindVarCount++] = myTime;
    cllBindVars[cllBindVarCount++] = myTime;
    status = cmlExecuteNoAnswerSql("insert into R_USER_PASSWORD (user_id, rcat_password, pass_expiry_ts,  create_ts, "
                                   "modify_ts) values (?, ?, ?, ?, ?)",
                                   &icss);
    if (status)
        return ERROR(status, "insert failure");
                            ...
                            const char* _new_value)
{
    ...
    char form4[] = "insert into R_USER_PASSWORD (user_id, rcat_password, pass_expiry_ts,  create_ts, modify_ts) values "
                   "((select user_id from R_USER_MAIN where user_name=? and zone_name=?), ?, ?, ?, ?)";
    char form5[] = "insert into R_USER_AUTH (user_id, user_auth_name, create_ts) values ((select user_id from "
                   "R_USER_MAIN where user_name=? and zone_name=?), ?, ?)";
    char form6[] = "delete from R_USER_AUTH where user_id = (select user_id from R_USER_MAIN where user_name=? and "
                   "zone_name=?) and user_auth_name = ?";
#if MY_ICAT
    char form7[] = "delete from R_USER_PASSWORD where pass_expiry_ts not like '9999%' and cast(pass_expiry_ts as "
                   "signed integer)>=? and cast(pass_expiry_ts as signed integer)<=? and user_id = (select user_id "
                   "from R_USER_MAIN where user_name=? and zone_name=?)";
#else
    char form7[] = "delete from R_USER_PASSWORD where pass_expiry_ts not like '9999%' and cast(pass_expiry_ts as "
                   "integer)>=? and cast(pass_expiry_ts as integer)<=? and user_id = (select user_id from R_USER_MAIN "
    ...
    if (*_new_value == '\0' && (strcmp(_option, "type") == 0 || strcmp(_option, "zone") == 0 ||
                                strcmp(_option, "addAuth") == 0 || strcmp(_option, "rmAuth") == 0)) {
    ...
    if (strcmp(_option, "type") == 0 || strcmp(_option, "user_type_name") == 0) {
        ...
        snprintf(tSQL, MAX_SQL_SIZE, form2, "user_type_name", tsubSQL);
        cllBindVars[cllBindVarCount++] = myTime;
        cllBindVars[cllBindVarCount++] = userName2;
        cllBindVars[cllBindVarCount++] = zoneName;
        opType = 1;
    ...
    if (strcmp(_option, "zone") == 0 || strcmp(_option, "zone_name") == 0) {
    ...
    if (strcmp(_option, "info") == 0 || strcmp(_option, "user_info") == 0) {
        snprintf(tSQL, MAX_SQL_SIZE, form1, "user_info");
        cllBindVars[cllBindVarCount++] = _new_value;
        cllBindVars[cllBindVarCount++] = myTime;
        cllBindVars[cllBindVarCount++] = userName2;
        cllBindVars[cllBindVarCount++] = zoneName;
    ...
    if (strcmp(_option, "comment") == 0 || strcmp(_option, "r_comment") == 0) {
        snprintf(tSQL, MAX_SQL_SIZE, form1, "r_comment");
        cllBindVars[cllBindVarCount++] = _new_value;
        cllBindVars[cllBindVarCount++] = myTime;
        cllBindVars[cllBindVarCount++] = userName2;
        cllBindVars[cllBindVarCount++] = zoneName;
    ...
    if (status != 0) {     /* error */
        if (opType == 1) { /* doing a type change, check if user_type problem */
            ...
            if (status2 != 0) {
                char errMsg[105];
                snprintf(errMsg, 100, "user_type '%s' is not valid", _new_value);
                addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);

                rodsLog(LOG_NOTICE, "chlModUser invalid user_type");
        ...
        if (opType == 4) { /* trying to insert password or auth-name */
            ...
            if (status2 != 0) {
                rodsLog(LOG_NOTICE, "chlModUser invalid user %s zone %s", userName2, zoneName);
                return ERROR(CAT_INVALID_USER, "invalid user");
            }
        }
        rodsLog(LOG_NOTICE, "chlModUser cmlExecuteNoAnswerSql failure %d", status);
        return ERROR(status, "get user_id failed");
    }

    status =
        cmlAudit1(auditId, _ctx.comm()->clientUser.userName, (char*) zone.c_str(), auditUserName, auditComment, &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModUser cmlAudit1 failure %d", status);
        _rollback("chlModUser");
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModUser cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "commit failed");
    }
                             ...
                             const char* _user_zone)
{
    ...
    if (_ctx.comm()->clientUser.authInfo.authFlag < LOCAL_PRIV_USER_AUTH ||
        ...
        status2 = cmlCheckGroupAdminAccess(
            _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, _group_name, &icss);
        if (status2 != 0) {
            ...
            if (strcmp(_option, "add") == 0) {
                int status3 = cmlCheckGroupAdminAccess(
                    _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, "", &icss);
                if (status3 == 0) {
    ...
    if (strcmp(_option, "remove") == 0) {
        ...
        status = cmlExecuteNoAnswerSql("delete from R_USER_GROUP where group_user_id = ? and user_id = ?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModGroup cmlExecuteNoAnswerSql delete failure %d", status);
            _rollback("chlModGroup");
            return ERROR(status, "delete failure");
        }
        OK = 1;
    ...
    if (strcmp(_option, "add") == 0) {
        ...
        status = cmlExecuteNoAnswerSql(
            "insert into R_USER_GROUP (group_user_id, user_id , create_ts, modify_ts) values (?, ?, ?, ?)", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModGroup cmlExecuteNoAnswerSql add failure %d", status);
            _rollback("chlModGroup");
    ...
    status = cmlAudit3(
        AU_MOD_GROUP, groupId, _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, commentStr, &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModGroup cmlAudit3 failure %d", status);
        _rollback("chlModGroup");
        return ERROR(status, "cmlAudit3 failure");
    }
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModGroup cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "commit failure");
    }
                            ...
                            const char* _option_value)
{
    ...
    // =-=-=-=-=-=-=-
    // check the params
    if (!_resc_name || !_option || !_option_value) {
        return ERROR(CAT_INVALID_ARGUMENT, "null parameter");
    }
    ...
    }
    {
        ...
        bindVars.push_back(zone);
        status = cmlGetStringValueFromSql(
            "select resc_id from R_RESC_MAIN where resc_name=? and zone_name=?", rescId, MAX_NAME_LEN, bindVars, &icss);
    }
    if (status != 0) {
        if (status == CAT_NO_ROWS_FOUND) {
    ...
    if (strcmp(_option, "comment") == 0) {
        ...
        status = cmlExecuteNoAnswerSql("update R_RESC_MAIN set r_comment = ?, modify_ts=? where resc_id=?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModResc cmlExecuteNoAnswerSql update failure %d", status);
            _rollback("chlModResc");
    ...
    if (strcmp(_option, "info") == 0) {
        ...
        status = cmlExecuteNoAnswerSql("update R_RESC_MAIN set resc_info=?, modify_ts=? where resc_id=?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModResc cmlExecuteNoAnswerSql update failure %d", status);
            _rollback("chlModResc");
    ...
    if (strcmp(_option, "freespace") == 0 || strcmp(_option, "free_space") == 0) {
        ...
        if (inType == 0) {
            status = cmlExecuteNoAnswerSql(
                "update R_RESC_MAIN set free_space = ?, free_space_ts = ?, modify_ts=? where resc_id=?", &icss);
        }
        if (inType == 1) {
#if ORA_ICAT
            /* For Oracle cast is to integer, for Postgres to bigint,for MySQL no cast*/
            status = cmlExecuteNoAnswerSql("update R_RESC_MAIN set free_space = cast(free_space as integer) + cast(? "
                                           "as integer), free_space_ts = ?, modify_ts=? where resc_id=?",
                                           &icss);
#elif MY_ICAT
            status = cmlExecuteNoAnswerSql(
                ...
                &icss);
#else
            status = cmlExecuteNoAnswerSql("update R_RESC_MAIN set free_space = cast(free_space as bigint) + cast(? as "
                                           "bigint), free_space_ts = ?, modify_ts=? where resc_id=?",
                                           &icss);
#endif
        }
        if (inType == 2) {
    ...
    if (strcmp(_option, "host") == 0) {
        ...
        status = cmlExecuteNoAnswerSql("update R_RESC_MAIN set resc_net = ?, modify_ts=? where resc_id=?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModResc cmlExecuteNoAnswerSql update failure %d", status);
            _rollback("chlModResc");
    ...
    if (strcmp(_option, "type") == 0) {
        ...
        status = cmlExecuteNoAnswerSql("update R_RESC_MAIN set resc_type_name = ?, modify_ts=? where resc_id=?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModResc cmlExecuteNoAnswerSql update failure %d", status);
            _rollback("chlModResc");
    ...
    if (strcmp(_option, "path") == 0) {
        ...
        }
        {
            ...
            bindVars.push_back(rescId);
            status = cmlGetStringValueFromSql(
                "select resc_def_path from R_RESC_MAIN where resc_id=?", rescPath, MAX_NAME_LEN, bindVars, &icss);
        }
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModResc cmlGetStringValueFromSql query failure %d", status);
            _rollback("chlModResc");
        ...
        cllBindVars[cllBindVarCount++] = myTime;
        cllBindVars[cllBindVarCount++] = rescId;
        status = cmlExecuteNoAnswerSql("update R_RESC_MAIN set resc_def_path=?, modify_ts=? where resc_id=?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModResc cmlExecuteNoAnswerSql update failure %d", status);
            _rollback("chlModResc");
    ...
    if (strcmp(_option, "status") == 0) {
        ...
        cllBindVars[cllBindVarCount++] = myTime;
        cllBindVars[cllBindVarCount++] = rescId;
        status = cmlExecuteNoAnswerSql("update R_RESC_MAIN set resc_status=?, modify_ts=? where resc_id=?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModResc cmlExecuteNoAnswerSql update failure %d", status);
            _rollback("chlModResc");
    ...
    if (strcmp(_option, "name") == 0) {
        ...
        status = cmlExecuteNoAnswerSql("update R_RESC_MAIN set resc_name=?, modify_ts=? where resc_id=?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModResc cmlExecuteNoAnswerSql update failure %d", status);
            _rollback("chlModResc");
        ...
        cllBindVars[cllBindVarCount++] = _option_value;
        cllBindVars[cllBindVarCount++] = _resc_name;
        status = cmlExecuteNoAnswerSql("update R_SERVER_LOAD set resc_name=? where resc_name=?", &icss);
        if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
            status = 0;
        }
        if (status != 0) {
        ...
        cllBindVars[cllBindVarCount++] = _option_value;
        cllBindVars[cllBindVarCount++] = _resc_name;
        status = cmlExecuteNoAnswerSql("update R_SERVER_LOAD_DIGEST set resc_name=? where resc_name=?", &icss);
        if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
            status = 0;
        }
        if (status != 0) {
    ...
    if (strcmp(_option, "context") == 0) {
        ...
        cllBindVars[cllBindVarCount++] = myTime;
        cllBindVars[cllBindVarCount++] = rescId;
        status = cmlExecuteNoAnswerSql("update R_RESC_MAIN set resc_context=?, modify_ts=? where resc_id=?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModResc cmlExecuteNoAnswerSql update failure for resc context %d", status);
            _rollback("chlModResc");
    ...
    snprintf(commentStr, sizeof commentStr, "%s %s", _option, _option_value);
    status = cmlAudit3(
        AU_MOD_RESC, rescId, _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, commentStr, &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModResc cmlAudit3 failure %d", status);
        _rollback("chlModResc");
        return ERROR(status, "cmlAudit3 failure");
    }
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModResc cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "commit failure");
    }
    ...
    if (rescPath[0] != '\0') {
        ...
        snprintf(rescPathMsg, sizeof(rescPathMsg), "Previous resource path: %s", rescPath);
                                       ...
                                       const char* _user_name)
{
    ...
    // =-=-=-=-=-=-=-
    // check the params
    if (!_resc_name || !_old_path || !_new_path) {
        return ERROR(CAT_INVALID_ARGUMENT, "null parameter");
    }
    ...
    if (_user_name != NULL && *_user_name != '\0') {
        ...
        cllBindVars[cllBindVarCount++] = userName2;
        cllBindVars[cllBindVarCount++] = zoneToUse;
        status = cmlExecuteNoAnswerSql("update R_DATA_MAIN set data_path = replace (R_DATA_MAIN.data_path, ?, ?) where "
                                       "resc_id=? and data_path like ? and data_owner_name=? and data_owner_zone=?",
                                       &icss);
    }
    else {
        if (logSQL != 0) {
        ...
        cllBindVars[cllBindVarCount++] = oldPath2;
        status = cmlExecuteNoAnswerSql("update R_DATA_MAIN set data_path = replace (R_DATA_MAIN.data_path, ?, ?) where "
                                       "resc_id=? and data_path like ?",
                                       &icss);
    }
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModRescDataPaths cmlExecuteNoAnswerSql update failure %d", status);
        _rollback("chlModResc");
    ...
    if (rows > 0) {
        char rowsMsg[100];
        snprintf(rowsMsg, 100, "%d rows updated", rows);
...
irods::error db_mod_resc_freespace_op(irods::plugin_context& _ctx, const char* _resc_name, int _update_value)
{
    ...
    */
    ...
    status = cmlExecuteNoAnswerSql("update R_RESC_MAIN set free_space = ?, free_space_ts=? where resc_name=?", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModRescFreeSpace cmlExecuteNoAnswerSql update failure %d", status);
    ...
    status = cmlAudit4(AU_MOD_RESC_FREE_SPACE,
                       ...
                       &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModRescFreeSpace cmlAudit4 failure %d", status);
        _rollback("chlModRescFreeSpace");
        return ERROR(status, "cmlAudit4 failure");
    }
...
irods::error db_reg_user_re_op(irods::plugin_context& _ctx, userInfo_t* _user_info)
{
    ...
    */
    if (*_user_info->userType == '\0' || strcmp(_user_info->userType, lastValidUserType) != 0) {
        char errMsg[105];
        if (logSQL != 0) {
        ...
        if (status == 0) {
            ...
            snprintf(errMsg, 100, "user_type '%s' is not valid", _user_info->userType);
            addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);
            return ERROR(CAT_INVALID_USER_TYPE, "invalid user type");
        }
    }
    ...
    if (zoneForm) {
        ...
        if (status != 0) {
            if (status == CAT_NO_ROWS_FOUND) {
                char errMsg[105];
                snprintf(errMsg, 100, "zone '%s' does not exist", userZone);
                addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);
                return ERROR(CAT_INVALID_ZONE, "invalid zone name");
            }
    ...
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegUserRE cmlGetNextSeqStr failure %d", status);
    ...
    status = cmlExecuteNoAnswerSql("insert into R_USER_MAIN (user_id, user_name, user_type_name, zone_name, create_ts, "
                                   "modify_ts) values (?, ?, ?, ?, ?, ?)",
                                   &icss);
    ...
    if (status != 0) {
        ...
        rodsLog(LOG_NOTICE, "chlRegUserRE insert failure %d", status);
        return ERROR(status, "insert failure");
    }
    ...
    status = cmlExecuteNoAnswerSql(
        "insert into R_USER_GROUP (group_user_id, user_id, create_ts, modify_ts) values (?, ?, ?, ?)", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegUserRE insert into R_USER_GROUP failure %d", status);
        _rollback("chlRegUserRE");
    ...
    */
    if (strlen(_user_info->authInfo.authStr) > 0) {
        status = chlModUser(_ctx.comm(), _user_info->userName, "addAuth", _user_info->authInfo.authStr);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlRegUserRE chlModUser insert auth failure %d", status);
            _rollback("chlRegUserRE");
    ...
    /* Audit */
    snprintf(
        auditSQL, MAX_SQL_SIZE - 1, "select user_id from R_USER_MAIN where user_name=? and zone_name='%s'", userZone);
    status = cmlAudit4(AU_REGISTER_USER_RE,
                       ...
                       &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegUserRE cmlAudit4 failure %d", status);
        _rollback("chlRegUserRE");
        return ERROR(status, "cmlAudit4 failure");
    }
                                    ...
                                    const char* _new_unit)
{
...

    {
        ...
        status = cmlGetMultiRowStringValuesFromSql(
            "select meta_id from R_OBJT_METAMAP where meta_id in (select meta_id from R_META_MAIN where "
            "meta_attr_name=? AND meta_id in (select meta_id from R_OBJT_METAMAP where object_id=?))",
            metaIdStr,
            MAX_NAME_LEN,
            2,
            bindVars,
    ...
    if (status <= 0) {
        if (status == CAT_NO_ROWS_FOUND) {
            ...
            status = chlAddAVUMetadata(_ctx.comm(), 0, _type, _name, _attribute, _new_value, _new_unit);
        }
        else {
            rodsLog(LOG_NOTICE, "chlSetAVUMetadata cmlGetMultiRowStringValuesFromSql failure %d", status);
        }
    ...
    if (status > 1) {
        ...
        status = chlDeleteAVUMetadata(_ctx.comm(), 1, _type, _name, _attribute, "%", "%", 1);
        if (status != 0) {
    ...
    status = cmlExecuteNoAnswerSql(
        "update R_META_MAIN set meta_attr_value=?,meta_attr_unit=?,modify_ts=? where meta_id=?", &icss);

    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlSetAVUMetadata cmlExecuteNoAnswerSql update failure %d", status);
        _rollback("chlSetAVUMetadata");
    ...
    status = cmlAudit3(AU_ADD_AVU_METADATA,
                       ...
                       &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlSetAVUMetadata cmlAudit3 failure %d", status);
        _rollback("chlSetAVUMetadata");
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlSetAVUMetadata cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "commit failed");
    }
                                         ...
                                         const char* _units)
{
    ...
    ret = determine_user_has_modify_metadata_access(
        objectName, collection, _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone);
    if (!ret.ok()) {
        return PASS(ret);
    }
    ...
    if (status < 0) {
        rodsLog(LOG_NOTICE, "chlAddAVUMetadataWild findOrInsertAVU failure %d", status);
    ...
    status = cmlExecuteNoAnswerSql("insert into R_OBJT_METAMAP \
                   ...
                   group by DM.data_id",
                                   &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlAddAVUMetadataWild cmlExecuteNoAnswerSql insert failure %d", status);
        _rollback("chlAddAVUMetadataWild");
        return ERROR(status, "insert failure");
    }
    ...
    status = cmlAudit3(AU_ADD_AVU_WILD_METADATA,
                       ...
                       &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlAddAVUMetadataWild cmlAudit3 failure %d", status);
        _rollback("chlAddAVUMetadataWild");
        return ERROR(status, "cmlAudit3 failure");
    }
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlAddAVUMetadataWild cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "commit failure");
    }
                                    ...
                                    const char* _units)
{
    ...
    if (itype == 1) {
        ...
        if (_admin_mode == 1) {
            ...
            }
            {
                ...
                bindVars.push_back(logicalEndName);
                bindVars.push_back(logicalParentDirName);
                status = cmlGetIntegerValueFromSql("select data_id from R_DATA_MAIN DM, R_COLL_MAIN CM where "
    ...
    if (itype == 2) {
        if (_admin_mode == 1) {
            ...
            }
            {
                ...
                status = cmlGetIntegerValueFromSql(
                    "select coll_id from R_COLL_MAIN where coll_name=?", &iVal, bindVars, &icss);
            }
            if (status == 0) {
    ...
    if (itype == 5) {
        ...
        }
        {
            ...
            bindVars.push_back(_name);
            status = cmlGetIntegerValueFromSql(
                "select distinct resc_group_id from R_RESC_GROUP where resc_group_name=?", &objId, bindVars, &icss);
        }
        if (status != 0) {
            _rollback("chlAddAVUMetadata");
            if (status == CAT_NO_ROWS_FOUND) {
    ...
    status = findOrInsertAVU(_attribute, _value, _units);
    if (status < 0) {
        rodsLog(LOG_NOTICE, "chlAddAVUMetadata findOrInsertAVU failure %d", status);
        _rollback("chlAddAVUMetadata");
        return ERROR(status, "findOrInsertAVU failure");
    }
    seqNum = status;
    ...
    status = cmlExecuteNoAnswerSql(
        "insert into R_OBJT_METAMAP (object_id, meta_id, create_ts, modify_ts) values (?, ?, ?, ?)", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlAddAVUMetadata cmlExecuteNoAnswerSql insert failure %d", status);
        _rollback("chlAddAVUMetadata");
        return ERROR(status, "insert failure");
    }
    ...
    status = cmlAudit3(AU_ADD_AVU_METADATA,
                       ...
                       &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlAddAVUMetadata cmlAudit3 failure %d", status);
        _rollback("chlAddAVUMetadata");
        return ERROR(status, "cmlAudit3 failure");
    }
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlAddAVUMetadata cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "commit failure");
    }
                                    ...
                                    const char* _arg3)
{
    ...
    status = chlDeleteAVUMetadata(_ctx.comm(), 0, _type, _name, _attribute, _value, myUnits, 1);
    if (status != 0) {
    ...
    if (*addAttr == '\0' && *addValue == '\0' && addUnits == NULL) {
        _rollback("chlModAVUMetadata");
    ...
    status = chlAddAVUMetadata(_ctx.comm(), 0, _type, _name, addAttr, addValue, addUnits);
    return CODE(status);
                                    ...
                                    int _nocommit)
{
    ...
    if (itype == 2) {
        ...
        status = cmlCheckDir(
            _name, _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, ACCESS_DELETE_METADATA, &icss);
        if (status < 0) {
            char errMsg[105];
            if (status == CAT_UNKNOWN_COLLECTION) {
                snprintf(errMsg, 100, "collection '%s' is unknown", _name);
    ...
    if (itype == 5) {
        ...
        }
        {
            ...
            bindVars.push_back(_name);
            status = cmlGetIntegerValueFromSql(
                "select resc_group_id from R_RESC_GROUP where resc_group_name=?", &objId, bindVars, &icss);
        }
        if (status != 0) {
            if (status == CAT_NO_ROWS_FOUND) {
                return ERROR(CAT_INVALID_RESOURCE, "invalid resource");
    ...
    if (_option == 2) {
        ...
        status = cmlExecuteNoAnswerSql("delete from R_OBJT_METAMAP where object_id=? and meta_id =?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlDeleteAVUMetadata cmlExecuteNoAnswerSql delete failure %d", status);
            if (_nocommit != 1) {
                _rollback("chlDeleteAVUMetadata");
            }
        ...
        status = cmlAudit3(AU_DELETE_AVU_METADATA,
                           ...
                           &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlDeleteAVUMetadata cmlAudit3 failure %d", status);
            if (_nocommit != 1) {
                _rollback("chlDeleteAVUMetadata");
            }
        ...
        if (_nocommit != 1) {
            status = cmlExecuteNoAnswerSql("commit", &icss);
            if (status != 0) {
                rodsLog(LOG_NOTICE, "chlDeleteAVUMetadata cmlExecuteNoAnswerSql commit failure %d", status);
                return ERROR(status, "commit failure");
            }
        }
        return ERROR(status, "delete failure");
    ...
    if (allowNullUnits) {
        if (_option == 1) { /* use wildcards ('like') */
            ...
            status = cmlExecuteNoAnswerSql("delete from R_OBJT_METAMAP where object_id=? and meta_id IN (select "
                                           "meta_id from R_META_MAIN where meta_attr_name like ? and meta_attr_value "
                                           "like ? and (meta_attr_unit like ? or meta_attr_unit IS NULL) )",
                                           &icss);
        }
        else {
            if (logSQL != 0) {
            ...
            status = cmlExecuteNoAnswerSql(
                "delete from R_OBJT_METAMAP where object_id=? and meta_id IN (select meta_id from R_META_MAIN where "
                "meta_attr_name = ? and meta_attr_value = ? and (meta_attr_unit = ? or meta_attr_unit IS NULL) )",
                &icss);
        }
    }
    else {
        if (_option == 1) { /* use wildcards ('like') */
            ...
            status = cmlExecuteNoAnswerSql(
                "delete from R_OBJT_METAMAP where object_id=? and meta_id IN (select meta_id from R_META_MAIN where "
                "meta_attr_name like ? and meta_attr_value like ? and meta_attr_unit like ?)",
                &icss);
        }
        else {
            if (logSQL != 0) {
            ...
            status = cmlExecuteNoAnswerSql(
                "delete from R_OBJT_METAMAP where object_id=? and meta_id IN (select meta_id from R_META_MAIN where "
                "meta_attr_name = ? and meta_attr_value = ? and meta_attr_unit = ?)",
                &icss);
        }
    }
    if (status != 0) {
                                     ...
                                     const char* _name2)
{
    ...
    status = cmlExecuteNoAnswerSql("insert into R_OBJT_METAMAP (object_id, meta_id, create_ts, modify_ts) select ?, "
                                   "meta_id, ?, ? from R_OBJT_METAMAP where object_id=?",
                                   &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlCopyAVUMetadata cmlExecuteNoAnswerSql insert failure %d", status);
    ...
    status = cmlAudit3(AU_COPY_AVU_METADATA,
                       ...
                       &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlCopyAVUMetadata cmlAudit3 failure %d", status);
        _rollback("chlCopyAVUMetadata");
        return ERROR(status, "cmlAudit3 failure");
    }
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlCopyAVUMetadata cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "commit failure");
    }
                                           ...
                                           const char* _resc_name)
{
    ...
    if (strcmp(myAccessStr, AP_NULL) == 0) {
        ...
        char errMsg[105];
        snprintf(errMsg, 100, "access level '%s' is invalid for a resource", myAccessStr);
        addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);
        return ERROR(CAT_INVALID_ARGUMENT, "invalid argument");
    }
    ...
    if (_ctx.comm()->clientUser.authInfo.authFlag >= LOCAL_PRIV_USER_AUTH) {
        ...
        }
        {
            ...
            bindVars.push_back(_resc_name);
            status =
                cmlGetIntegerValueFromSql("select resc_id from R_RESC_MAIN where resc_name=?", &iVal, bindVars, &icss);
        }
        if (status == CAT_NO_ROWS_FOUND) {
        ...
        status = cmlCheckResc(
            _resc_name, _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, ACCESS_OWN, &icss);
        if (status < 0) {
    ...
    }
    {
        ...
        status = cmlGetIntegerValueFromSql(
            "select user_id from R_USER_MAIN where user_name=? and R_USER_MAIN.zone_name=?", &userId, bindVars, &icss);
    }
    if (status != 0) {
        if (status == CAT_NO_ROWS_FOUND) {
            return ERROR(CAT_INVALID_USER, "invalid user");
    ...
    status = cmlExecuteNoAnswerSql("delete from R_OBJT_ACCESS where user_id=? and object_id=?", &icss);
    if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
        return ERROR(status, "delete failure");
    }
    ...
    /* Audit */
    status = cmlAudit5(AU_MOD_ACCESS_CONTROL_RESOURCE, rescIdStr, userIdStr, myAccessLev, &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModAccessControlResc cmlAudit5 failure %d", status);
        _rollback("chlModAccessControlResc");
                                      ...
                                      const char* _path_name)
{
    ...
    if (strncmp(_access_level, MOD_RESC_PREFIX, strlen(MOD_RESC_PREFIX)) == 0) {
        ret = db_mod_access_control_resc_op(_ctx, _recursive_flag, _access_level, _user_name, _zone, _path_name);
        return PASS(ret);
    }

    int adminMode = 0;
    char myAccessStr[LONG_NAME_LEN];
    if (strncmp(_access_level, MOD_ADMIN_MODE_PREFIX, strlen(MOD_ADMIN_MODE_PREFIX)) == 0) {
        if (_ctx.comm()->clientUser.authInfo.authFlag < LOCAL_PRIV_USER_AUTH) {
            addRErrorMsg(&_ctx.comm()->rError, 0, "You must be the admin to use the -M admin mode");
    ...
    if (adminMode) {
        ...
        status1 = cmlCheckDir(
            _path_name, _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, ACCESS_OWN, &icss);
    }
    ...
    if (status1 < 0) {
        ...
        if (adminMode) {
            ...
            }
            {
                ...
                bindVars.push_back(logicalEndName);
                bindVars.push_back(logicalParentDirName);
                status2 = cmlGetIntegerValueFromSql("select data_id from R_DATA_MAIN DM, R_COLL_MAIN CM where "
                                                    "DM.data_name=? and DM.coll_id=CM.coll_id and CM.coll_name=?",
                                                    &iVal,
                                                    bindVars,
                                                    &icss);
            ...
            status2 = cmlCheckDataObjOnly(logicalParentDirName,
                                          logicalEndName,
                                          _ctx.comm()->clientUser.userName,
                                          _ctx.comm()->clientUser.rodsZone,
                                          ACCESS_OWN,
        ...
        /* If both failed, it doesn't exist or there's no permission */
            ...
            if (status1 == CAT_UNKNOWN_COLLECTION && status2 == CAT_UNKNOWN_FILE) {
                snprintf(errMsg, 200, "Input path is not a collection and not a dataObj: %s", _path_name);
                addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);
            ...
            if (status1 != CAT_UNKNOWN_COLLECTION) {
                ...
                int status = cmlCheckDirOwn(
                    _path_name, _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, &icss);
                if (status < 0) {
                ...
                if (status2 == CAT_NO_ACCESS_PERMISSION) {
                    ...
                    int status = cmlCheckDataObjOwn(logicalParentDirName,
                                                    logicalEndName,
                                                    _ctx.comm()->clientUser.userName,
                                                    _ctx.comm()->clientUser.rodsZone,
                                                    &icss);
    ...
    if (_recursive_flag == 0) {
        ...
        if (objId) {
            ...
            int status = cmlExecuteNoAnswerSql("delete from R_OBJT_ACCESS where user_id=? and object_id=?", &icss);
            if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
                return ERROR(status, "delete failure");
            }
            if (rmFlag == 0) { /* if not just removing: */
                ...
                int status =
                    cmlExecuteNoAnswerSql("insert into R_OBJT_ACCESS (object_id, user_id, access_type_id, create_ts, "
                                          "modify_ts)  values (?, ?, (select token_id from R_TOKN_MAIN where "
                                          "token_namespace = 'access_type' and token_name = ?), ?, ?)",
                                          &icss);
                if (status != 0) {
                    _rollback("chlModAccessControl");
                    return ERROR(status, "insert failure");
            ...
            /* Audit */
            status = cmlAudit5(AU_MOD_ACCESS_CONTROL_OBJ, objIdStr, userIdStr, myAccessLev, &icss);
            if (status != 0) {
                rodsLog(LOG_NOTICE, "chlModAccessControl cmlAudit5 failure %d", status);
                _rollback("chlModAccessControl");
                return ERROR(status, "cmlAudit5 failure");
            }
        ...
        if (rmFlag) { /* just removing */
            /* Audit */
            int status = cmlAudit5(AU_MOD_ACCESS_CONTROL_COLL, collIdStr, userIdStr, myAccessLev, &icss);
            if (status != 0) {
                rodsLog(LOG_NOTICE, "chlModAccessControl cmlAudit5 failure %d", status);
                _rollback("chlModAccessControl");
                return ERROR(status, "cmlAudit5 failure");
        ...
        /* Audit */
        status = cmlAudit5(AU_MOD_ACCESS_CONTROL_COLL, collIdStr, userIdStr, myAccessLev, &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModAccessControl cmlAudit5 failure %d", status);
            _rollback("chlModAccessControl");
            return ERROR(status, "cmlAudit5 failure");
    ...
    if (objId) {
        ...
        snprintf(errMsg, 200, "Input path is not a collection and recursion was requested: %s", _path_name);
        addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);
        return ERROR(CAT_INVALID_ARGUMENT, errMsg);
    }
    ...
    */
    status = cmlExecuteNoAnswerSql("create temporary table R_MOD_ACCESS_TEMP1 (coll_id bigint not null, coll_name "
                                   "varchar(2700) not null) on commit drop",
                                   &icss);
    if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
        status = 0;
    }
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModAccessControl cmlExecuteNoAnswerSql create temp table failure %d", status);
        _rollback("chlModAccessControl");
        return ERROR(status, "chlModAccessControl cmlExecuteNoAnswerSql create temp table failure");
    }

    status =
        cmlExecuteNoAnswerSql("create unique index idx_r_mod_access_temp1 on R_MOD_ACCESS_TEMP1 (coll_name)", &icss);
    if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
        status = 0;
    }
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModAccessControl cmlExecuteNoAnswerSql create index failure %d", status);
        _rollback("chlModAccessControl");
    ...
    cllBindVars[cllBindVarCount++] = pathStart.c_str();
    status = cmlExecuteNoAnswerSql("insert into R_MOD_ACCESS_TEMP1 (coll_id, coll_name) select  coll_id, coll_name "
                                   "from R_COLL_MAIN where coll_name = ? or coll_name like ?",
                                   &icss);
    if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
        status = 0;
    }
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlModAccessControl cmlExecuteNoAnswerSql insert failure %d", status);
        _rollback("chlModAccessControl");
    ...
    status = cmlExecuteNoAnswerSql(
#if defined ORA_ICAT
        "delete from R_OBJT_ACCESS where user_id=? and object_id = ANY (select data_id from R_DATA_MAIN where coll_id "
        "in (select coll_id from R_COLL_MAIN where coll_name = ? or coll_name like ? ESCAPE '\\'))",
#elif defined MY_ICAT
        "delete from R_OBJT_ACCESS where user_id=? and object_id = ANY (select data_id from R_DATA_MAIN where coll_id "
        "in (select coll_id from R_COLL_MAIN where coll_name = ? or coll_name like ?))",
#else
        ...
        constraints.
        */
        "delete from R_OBJT_ACCESS where user_id=? and object_id = ANY(ARRAY(select data_id from R_DATA_MAIN where "
        "coll_id in (select coll_id from R_MOD_ACCESS_TEMP1 where coll_name = ? or coll_name like ?)))",
#endif
        &icss);
    ...
    status = cmlExecuteNoAnswerSql(
#if defined ORA_ICAT
        "delete from R_OBJT_ACCESS where user_id=? and object_id = ANY (select coll_id from R_COLL_MAIN where "
        "coll_name = ? or coll_name like ? ESCAPE '\\')",
#elif defined MY_ICAT
        "delete from R_OBJT_ACCESS where user_id=? and object_id = ANY (select coll_id from R_COLL_MAIN where "
        "coll_name = ? or coll_name like ?)",
#else
        "delete from R_OBJT_ACCESS where user_id=? and object_id = ANY(ARRAY(select coll_id from R_MOD_ACCESS_TEMP1 "
        "where coll_name = ? or coll_name like ?))",
#endif
        &icss);
    if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
        _rollback("chlModAccessControl");
    ...
    if (rmFlag) { /* just removing */
        ...
        /* Audit */
        status = cmlAudit5(AU_MOD_ACCESS_CONTROL_COLL_RECURSIVE, collIdStr, userIdStr, myAccessLev, &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModAccessControl cmlAudit5 failure %d", status);
            _rollback("chlModAccessControl");
            return ERROR(status, "cmlAudit5 failure");
    ...
    /* For Oracle cast is to integer, for Postgres to bigint,for MySQL no cast*/
    status = cmlExecuteNoAnswerSql(
        "insert into R_OBJT_ACCESS (object_id, user_id, access_type_id, create_ts, modify_ts)  (select distinct "
        "data_id, cast(? as integer), (select token_id from R_TOKN_MAIN where token_namespace = 'access_type' and "
        "token_name = ?), ?, ? from R_DATA_MAIN where coll_id in (select coll_id from R_COLL_MAIN where coll_name = ? "
        "or coll_name like ? ESCAPE '\\'))",
        &icss);
#elif MY_ICAT
    status = cmlExecuteNoAnswerSql(
        "insert into R_OBJT_ACCESS (object_id, user_id, access_type_id, create_ts, modify_ts)  (select distinct "
        "data_id, ?, (select token_id from R_TOKN_MAIN where token_namespace = 'access_type' and token_name = ?), ?, ? "
        "from R_DATA_MAIN where coll_id in (select coll_id from R_COLL_MAIN where coll_name = ? or coll_name like ?))",
        &icss);
#else
    /* For Postgres, also use the temporary R_MOD_ACCESS_TEMP1 table */
    status = cmlExecuteNoAnswerSql(
        "insert into R_OBJT_ACCESS (object_id, user_id, access_type_id, create_ts, modify_ts)  (select distinct "
        "data_id, cast(? as bigint), (select token_id from R_TOKN_MAIN where token_namespace = 'access_type' and "
        "token_name = ?), ?, ? from R_DATA_MAIN where coll_id in (select coll_id from R_MOD_ACCESS_TEMP1 where "
        "coll_name = ? or coll_name like ?))",
        &icss);
#endif
    if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
    ...
    status = cmlExecuteNoAnswerSql(
        "insert into R_OBJT_ACCESS (object_id, user_id, access_type_id, create_ts, modify_ts)  (select distinct "
        "coll_id, cast(? as integer), (select token_id from R_TOKN_MAIN where token_namespace = 'access_type' and "
        "token_name = ?), ?, ? from R_COLL_MAIN where coll_name = ? or coll_name like ? ESCAPE '\\')",
        &icss);
#elif MY_ICAT
    status = cmlExecuteNoAnswerSql(
        "insert into R_OBJT_ACCESS (object_id, user_id, access_type_id, create_ts, modify_ts)  (select distinct "
        "coll_id, ?, (select token_id from R_TOKN_MAIN where token_namespace = 'access_type' and token_name = ?), ?, ? "
        "from R_COLL_MAIN where coll_name = ? or coll_name like ?)",
        &icss);
#else
    status = cmlExecuteNoAnswerSql(
        "insert into R_OBJT_ACCESS (object_id, user_id, access_type_id, create_ts, modify_ts)  (select distinct "
        "coll_id, cast(? as bigint), (select token_id from R_TOKN_MAIN where token_namespace = 'access_type' and "
        "token_name = ?), ?, ? from R_COLL_MAIN where coll_name = ? or coll_name like ?)",
        &icss);
#endif
    if (status != 0) {
        _rollback("chlModAccessControl");
...
irods::error db_rename_object_op(irods::plugin_context& _ctx, rodsLong_t _obj_id, const char* _new_name)
{
...

    {
        ...
        bindVars.push_back(_ctx.comm()->clientUser.rodsZone);
        status = cmlGetIntegerValueFromSql(
            "select coll_id from R_DATA_MAIN DM, R_OBJT_ACCESS OA, R_USER_GROUP UG, R_USER_MAIN UM, R_TOKN_MAIN TM "
            "where DM.data_id=? and UM.user_name=? and UM.zone_name=? and UM.user_type_name!='rodsgroup' and "
            "UM.user_id = UG.user_id and OA.object_id = DM.data_id and UG.group_user_id = OA.user_id and "
            "OA.access_type_id >= TM.token_id and TM.token_namespace ='access_type' and TM.token_name = 'own'",
            &collId,
            bindVars,
            &icss);
    }
    ...
    if (status == 0) { /* it is a dataObj and user has access to it */
        ...
        }
        {
            ...
            status = cmlGetIntegerValueFromSql(
                "select data_id from R_DATA_MAIN where data_name=? and coll_id=?", &otherDataId, bindVars, &icss);
        }
        if (status != CAT_NO_ROWS_FOUND) {
        ...
        }
        {
            ...
            status = cmlGetIntegerValueFromSql("select coll_id from R_COLL_MAIN where coll_name = ( select coll_name "
                                               "from R_COLL_MAIN where coll_id=? ) || ?",
                                               &otherCollId,
                                               bindVars,
                                               &icss);
        }
        ...
        status = cmlExecuteNoAnswerSql("update R_DATA_MAIN set data_name = ? where data_id=?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlRenameObject cmlExecuteNoAnswerSql update1 failure %d", status);
        ...
        status = cmlExecuteNoAnswerSql("update R_COLL_MAIN set modify_ts=? where coll_id=?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlRenameObject cmlExecuteNoAnswerSql update2 failure %d", status);
            _rollback("chlRenameObject");
        ...
        status = cmlAudit3(AU_RENAME_DATA_OBJ,
                           ...
                           &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlRenameObject cmlAudit3 failure %d", status);
            _rollback("chlRenameObject");
            return ERROR(status, "cmlAudit3 failure");
        }
    ...
    if (status == 0) {
        ...
        }
        {
            ...
            status = cmlGetIntegerValueFromSql("select data_id from R_DATA_MAIN where data_name=? and coll_id= (select "
                                               "coll_id from R_COLL_MAIN  where coll_name = ?)",
                                               &otherDataId,
                                               bindVars,
                                               &icss);
        }
        ...
        }
        {
            ...
            bindVars.push_back(collNameTmp);
            status = cmlGetIntegerValueFromSql(
                "select coll_id from R_COLL_MAIN where coll_name = ?", &otherCollId, bindVars, &icss);
        }
        if (status != CAT_NO_ROWS_FOUND) {
            return ERROR(CAT_NAME_EXISTS_AS_COLLECTION, "select coll_id failed");
        }
        ...
        if (pLen <= 0 || cLen <= 0) {
            return ERROR(CAT_INVALID_ARGUMENT, "coll name or parent is invalid");
                                                                  ...
                                                                  argument is not really right, but something is really
             wrong */
        ...
        status =
            cmlExecuteNoAnswerSql("update R_COLL_MAIN set coll_name = substr(coll_name,1,?) || ? || substr(coll_name, "
                                  "?) where substr(parent_coll_name,1,?) = ? or parent_coll_name  = ?",
                                  &icss);
        if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
            rodsLog(LOG_NOTICE, "chlRenameObject cmlExecuteNoAnswerSql update failure %d", status);
            _rollback("chlRenameObject");
            return ERROR(status, "cmlExecuteNoAnswerSql update failure");
        }
        ...
        status = cmlExecuteNoAnswerSql(
            "update R_COLL_MAIN set parent_coll_name = substr(parent_coll_name,1,?) || ? || substr(parent_coll_name, "
            "?) where substr(parent_coll_name,1,?) = ? or parent_coll_name  = ?",
            &icss);
        if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
            rodsLog(LOG_NOTICE, "chlRenameObject cmlExecuteNoAnswerSql update failure %d", status);
        ...
        status = cmlExecuteNoAnswerSql("update R_COLL_MAIN set coll_name=?, modify_ts=? where coll_id=?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlRenameObject cmlExecuteNoAnswerSql update failure %d", status);
            _rollback("chlRenameObject");
            return ERROR(status, "cmlExecuteNoAnswerSql update failure");
    ...
    }
    {
        ...
        bindVars.push_back(objIdString);
        status =
            cmlGetIntegerValueFromSql("select coll_id from R_DATA_MAIN where data_id=?", &otherDataId, bindVars, &icss);
    }
    if (status == 0) {
    ...
    }
    {
        ...
        bindVars.push_back(collIdString);
        status =
            cmlGetIntegerValueFromSql("select coll_id from R_COLL_MAIN where coll_id=?", &otherDataId, bindVars, &icss);
    }
    if (status == 0) {
...
irods::error db_move_object_op(irods::plugin_context& _ctx, rodsLong_t _obj_id, rodsLong_t _target_coll_id)
{
    ...
    }
    {
        ...
        status = cmlGetStringValuesFromSql(
            ...
            "UM.user_type_name!='rodsgroup' and UM.user_id = UG.user_id and OA.object_id = CM.coll_id and "
            "UG.group_user_id = OA.user_id and OA.access_type_id >= TM.token_id and TM.token_namespace ='access_type' "
            "and TM.token_name = 'modify object'",
            cVal,
            iVal,
            2,
            bindVars,
    ...
    if (status != 0) {
        ...
        }
        {
            ...
            bindVars.push_back(collIdString);
            status =
                cmlGetIntegerValueFromSql("select coll_id from R_COLL_MAIN where coll_id=?", &collId, bindVars, &icss);
        }
        if (status == 0) {
    ...
    }
    {
        ...
        bindVars.push_back(_ctx.comm()->clientUser.rodsZone);
        status = cmlGetStringValueFromSql(
            "select data_name from R_DATA_MAIN DM, R_OBJT_ACCESS OA, R_USER_GROUP UG, R_USER_MAIN UM, R_TOKN_MAIN TM "
            "where DM.data_id=? and UM.user_name=? and UM.zone_name=? and UM.user_type_name!='rodsgroup' and "
            "UM.user_id = UG.user_id and OA.object_id = DM.data_id and UG.group_user_id = OA.user_id and "
            "OA.access_type_id >= TM.token_id and TM.token_namespace ='access_type' and TM.token_name = 'own'",
            dataObjName,
    ...
    if (status == 0) { /* it is a dataObj and user has access to it */
        ...
        status = cmlExecuteNoAnswerSql("update R_DATA_MAIN set coll_id=?, modify_ts=? where data_id=?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlMoveObject cmlExecuteNoAnswerSql update1 failure %d", status);
        ...
        status = cmlExecuteNoAnswerSql("update R_COLL_MAIN set modify_ts=? where coll_id=?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlMoveObject cmlExecuteNoAnswerSql update2 failure %d", status);
            _rollback("chlMoveObject");
            return ERROR(status, "cmlExecuteNoAnswerSql update2 failure");
        }
        ...
        status = cmlAudit3(AU_MOVE_DATA_OBJ,
                           ...
                           &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlMoveObject cmlAudit3 failure %d", status);
            _rollback("chlMoveObject");
            return ERROR(status, "cmlAudit3 failure");
        }
    ...
    if (status == 0) {
        ...
        if (pLen <= 0 || ocLen <= 0) {
            return ERROR(CAT_INVALID_ARGUMENT, "parent or coll name null");
                                                                   ...
                                                                   argument is not really the right error code, but
        ...
        status = cmlCheckDir(parentCollName,
                             _ctx.comm()->clientUser.userName,
                             _ctx.comm()->clientUser.rodsZone,
                             ACCESS_MODIFY_OBJECT,
                             &icss);
        ...
        if (cp == targetCollName &&
            (targetCollName[strlen(oldCollName)] == '/' || targetCollName[strlen(oldCollName)] == '\0')) {
        ...
        status = cmlExecuteNoAnswerSql(
            "update R_COLL_MAIN set coll_name = ?, parent_coll_name=?, modify_ts=? where coll_id = ?", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlMoveObject cmlExecuteNoAnswerSql update failure %d", status);
            _rollback("chlMoveObject");
            return ERROR(status, "cmlExecuteNoAnswerSql update failure");
        }
        ...
        status = cmlExecuteNoAnswerSql(
            "update R_COLL_MAIN set parent_coll_name = ? || substr(parent_coll_name, ?), coll_name = ? || "
            "substr(coll_name, ?) where substr(parent_coll_name,1,?) = ? or parent_coll_name = ?",
            &icss);
        if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
            status = 0;
        }
                             ...
                             const char* _comment)
{
    ...
    }
    {
        ...
        status = cmlGetIntegerValueFromSql(
            "select token_id from R_TOKN_MAIN where token_namespace=? and token_name=?", &objId, bindVars, &icss);
    }
    if (status != 0) {
        snprintf(errMsg, 200, "Token namespace '%s' does not exist", _name_space);
        addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);
    ...
    if (status == 0) {
        snprintf(errMsg, 200, "Token '%s' already exists in namespace '%s'", _name, _name_space);
        addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);
    ...
    seqNum = cmlGetNextSeqVal(&icss);
    if (seqNum < 0) {
        rodsLog(LOG_NOTICE, "chlRegToken cmlGetNextSeqVal failure %d", seqNum);
    ...
    status = cmlExecuteNoAnswerSql("insert into R_TOKN_MAIN values (?, ?, ?, ?, ?, ?, ?, ?, ?)", &icss);
    if (status != 0) {
    ...
    /* Audit */
    status = cmlAudit3(
        AU_REG_TOKEN, seqNumStr, _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, _name, &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegToken cmlAudit3 failure %d", status);
        _rollback("chlRegToken");
        return ERROR(status, "cmlAudit3 failure");
    }
...
irods::error db_del_token_op(irods::plugin_context& _ctx, const char* _name_space, const char* _name)
{
    ...
    if (status != 0) {
        snprintf(errMsg, 200, "Token '%s' does not exist in namespace '%s'", _name, _name_space);
        addRErrorMsg(&_ctx.comm()->rError, 0, errMsg);
    ...
    cllBindVars[cllBindVarCount++] = _name_space;
    cllBindVars[cllBindVarCount++] = _name;
    status = cmlExecuteNoAnswerSql("delete from R_TOKN_MAIN where token_namespace=? and token_name=?", &icss);
    if (status != 0) {
    ...
    snprintf(objIdStr, sizeof objIdStr, "%lld", objId);
    status = cmlAudit3(
        AU_DEL_TOKEN, objIdStr, _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, _name, &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlDelToken cmlAudit3 failure %d", status);
        _rollback("chlDelToken");
        return ERROR(status, "cmlAudit3 failure");
    }
                                   ...
                                   const char* _net_output)
{
    ...
    status = cmlExecuteNoAnswerSql(
        "insert into R_SERVER_LOAD (host_name, resc_name, cpu_used, mem_used, swap_used, runq_load, disk_space, "
        "net_input, net_output, create_ts) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegServerLoad cmlExecuteNoAnswerSql failure %d", status);
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegServerLoad cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "commit failure");
    }
...
irods::error db_purge_server_load_op(irods::plugin_context& _ctx, const char* _seconds_ago)
{
    ...
    status = cmlExecuteNoAnswerSql("delete from R_SERVER_LOAD where create_ts <?", &icss);
    if (status != 0) {
...
irods::error db_reg_server_load_digest_op(irods::plugin_context& _ctx, const char* _resc_name, const char* _load_factor)
{
    ...
    status = cmlExecuteNoAnswerSql(
        "insert into R_SERVER_LOAD_DIGEST (resc_name, load_factor, create_ts) values (?, ?, ?)", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegServerLoadDigest cmlExecuteNoAnswerSql failure %d", status);
    ...
    status = cmlExecuteNoAnswerSql("commit", &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlRegServerLoadDigest cmlExecuteNoAnswerSql commit failure %d", status);
        return ERROR(status, "commit failure");
    }
...
irods::error db_purge_server_load_digest_op(irods::plugin_context& _ctx, const char* _seconds_ago)
{
    ...
    cllBindVars[cllBindVarCount++] = thenStr;
    status = cmlExecuteNoAnswerSql("delete from R_SERVER_LOAD_DIGEST where create_ts <?", &icss);
    if (status != 0) {
...
irods::error db_calc_usage_and_quota_op(irods::plugin_context& _ctx)
{
    ...
    rodsLog(LOG_NOTICE, "chlCalcUsageAndQuota called");
    ...
    cllBindVars[cllBindVarCount++] = myTime;
    status = cmlExecuteNoAnswerSql("delete from R_QUOTA_USAGE where modify_ts < ?", &icss);
    if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
    ...
    cllBindVars[cllBindVarCount++] = myTime;
    status = cmlExecuteNoAnswerSql(
        "insert into R_QUOTA_USAGE (quota_usage, resc_id, user_id, modify_ts) (select sum(R_DATA_MAIN.data_size), "
        "R_RESC_MAIN.resc_id, R_USER_MAIN.user_id, ? from R_DATA_MAIN, R_USER_MAIN, R_RESC_MAIN where "
        "R_USER_MAIN.user_name = R_DATA_MAIN.data_owner_name and R_USER_MAIN.zone_name = R_DATA_MAIN.data_owner_zone "
        "and R_RESC_MAIN.resc_id = R_DATA_MAIN.resc_id group by R_RESC_MAIN.resc_id, user_id)",
        &icss);
    if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
        status = 0; /* no files, OK */
    }
                             ...
                             const char* _limit)
{
    ...
    if (strncmp(_resc_name, "total", 5) != 0) {
        ...
        }
        {
            ...
            bindVars.push_back(zone);
            status = cmlGetIntegerValueFromSql(
                "select resc_id from R_RESC_MAIN where resc_name=? and zone_name=?", &rescId, bindVars, &icss);
        }
        if (status != 0) {
            if (status == CAT_NO_ROWS_FOUND) {
    ...
    status = cmlExecuteNoAnswerSql("delete from R_QUOTA_MAIN where user_id=? and resc_id=?", &icss);
    if (status != 0) {
        rodsLog(LOG_DEBUG, "chlSetQuota cmlExecuteNoAnswerSql delete failure %d", status);
    }
    if (atol(_limit) > 0) {
        getNowStr(myTime);
        cllBindVars[cllBindVarCount++] = userIdStr;
        ...
        status = cmlExecuteNoAnswerSql(
            "insert into R_QUOTA_MAIN (user_id, resc_id, quota_limit, modify_ts) values (?, ?, ?, ?)", &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlSetQuota cmlExecuteNoAnswerSql insert failure %d", status);
            _rollback("chlSetQuota");
            return ERROR(status, "cmlExecuteNoAnswerSql insert failure");
        }
    }
                               ...
                               int* _quota_status)
{
    ...
    */
    ...
    char mySQL[] =
        "select distinct QM.user_id, QM.resc_id, QM.quota_limit, QM.quota_over from R_QUOTA_MAIN QM, R_USER_MAIN UM, "
        "R_RESC_MAIN RM, R_USER_GROUP UG, R_USER_MAIN UM2 where ( (QM.user_id = UM.user_id and UM.user_name = ?) or "
        "(QM.user_id = UG.group_user_id and UM2.user_name = ? and UG.user_id = UM2.user_id) ) and ((QM.resc_id = "
        "RM.resc_id and RM.resc_name = ?) or QM.resc_id = '0') order by quota_over desc";
    ...
    *_user_quota = 0;
    ...
    status = cmlGetFirstRowFromSql(mySQL, &statementNum, 0, &icss);

    if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
        rodsLog(LOG_NOTICE, "chlCheckQuota - CAT_SUCCESS_BUT_WITH_NO_INFO");
        *_quota_status = QUOTA_UNRESTRICTED;
        return SUCCESS();
    }

    if (status == CAT_NO_ROWS_FOUND) {
        rodsLog(LOG_NOTICE, "chlCheckQuota - CAT_NO_ROWS_FOUND");
        *_quota_status = QUOTA_UNRESTRICTED;
        return SUCCESS();
    }
    ...
    /* For now, log it */
    rodsLog(LOG_NOTICE,
            "checkQuota: inUser:%s inResc:%s RescId:%s Quota:%s",
            _user_name,
            _resc_name,
                                  ...
                                  const char* _my_time)
{
    ...
    cllBindVarCount = i;
    status =
        cmlGetIntegerValueFromSqlV3("select rule_id from R_RULE_MAIN where  rule_base_name = ? and  rule_name = ? and "
                                    "rule_event = ? and rule_condition = ? and rule_body = ? and  rule_recovery = ?",
                                    &seqNum,
                                    &icss);
    if (status != 0 && status != CAT_NO_ROWS_FOUND) {
        rodsLog(LOG_NOTICE, "chlInsRuleTable cmlGetIntegerValueFromSqlV3 find rule if any failure %d", status);
        return ERROR(status, "cmlGetIntegerValueFromSqlV3 find rule if any failure");
    }
    if (seqNum < 0) {
        seqNum = cmlGetNextSeqVal(&icss);
        if (seqNum < 0) {
            rodsLog(LOG_NOTICE, "chlInsRuleTable cmlGetNextSeqVal failure %d", seqNum);
        ...
        status = cmlExecuteNoAnswerSql("insert into R_RULE_MAIN(rule_id, rule_base_name, rule_name, rule_event, "
                                       "rule_condition, rule_body, rule_recovery, rule_owner_name, rule_owner_zone, "
                                       "create_ts, modify_ts) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                       &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlInsRuleTable cmlExecuteNoAnswerSql Rule Main Insert failure %d", status);
    ...
    cllBindVars[i++] = _my_time;
    cllBindVarCount = i;
    status = cmlExecuteNoAnswerSql("insert into R_RULE_BASE_MAP  (map_base_name, map_priority, rule_id, "
                                   "map_owner_name,map_owner_zone, create_ts, modify_ts) values (?, ?, ?, ?, ?, ?, ?)",
                                   &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlInsRuleTable cmlExecuteNoAnswerSql Rule Map insert failure %d", status);
                                 ...
                                 const char* _my_time)
{
    ...
    cllBindVarCount = i;
    status = cmlGetIntegerValueFromSqlV3("select dvm_id from R_RULE_DVM where  dvm_base_name = ? and  dvm_ext_var_name "
                                         "= ? and  dvm_condition = ? and dvm_int_map_path = ? ",
                                         &seqNum,
                                         &icss);
    if (status != 0 && status != CAT_NO_ROWS_FOUND) {
        rodsLog(LOG_NOTICE, "chlInsDvmTable cmlGetIntegerValueFromSqlV3 find DVM if any failure %d", status);
        return ERROR(status, "find DVM if any failure");
    }
    if (seqNum < 0) {
        seqNum = cmlGetNextSeqVal(&icss);
        if (seqNum < 0) {
            rodsLog(LOG_NOTICE, "chlInsDvmTable cmlGetNextSeqVal failure %d", seqNum);
        ...
        status = cmlExecuteNoAnswerSql(
            "insert into R_RULE_DVM(dvm_id, dvm_base_name, dvm_ext_var_name, dvm_condition, dvm_int_map_path, "
            "dvm_owner_name, dvm_owner_zone, create_ts, modify_ts) values (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlInsDvmTable cmlExecuteNoAnswerSql DVM Main Insert failure %d", status);
    ...
    cllBindVars[i++] = _my_time;
    cllBindVarCount = i;
    status = cmlExecuteNoAnswerSql("insert into R_RULE_DVM_MAP  (map_dvm_base_name, dvm_id, "
                                   "map_owner_name,map_owner_zone, create_ts, modify_ts) values (?, ?, ?, ?, ?, ?)",
                                   &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlInsDvmTable cmlExecuteNoAnswerSql DVM Map insert failure %d", status);
                                 ...
                                 const char* _my_time)
{
    ...
    status = cmlGetIntegerValueFromSqlV3(
        ...
        &icss);
    if (status != 0 && status != CAT_NO_ROWS_FOUND) {
        rodsLog(LOG_NOTICE, "chlInsFnmTable cmlGetIntegerValueFromSqlV3 find FNM if any failure %d", status);
        return ERROR(status, "find FNM if any failure");
    }
    if (seqNum < 0) {
        seqNum = cmlGetNextSeqVal(&icss);
        if (seqNum < 0) {
            rodsLog(LOG_NOTICE, "chlInsFnmTable cmlGetNextSeqVal failure %d", seqNum);
        ...
        status = cmlExecuteNoAnswerSql(
            "insert into R_RULE_FNM(fnm_id, fnm_base_name, fnm_ext_func_name, fnm_int_func_name, fnm_owner_name, "
            "fnm_owner_zone, create_ts, modify_ts) values (?, ?, ?, ?, ?, ?, ?, ?)",
            &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlInsFnmTable cmlExecuteNoAnswerSql FNM Main Insert failure %d", status);
    ...
    cllBindVars[i++] = _my_time;
    cllBindVarCount = i;
    status = cmlExecuteNoAnswerSql("insert into R_RULE_FNM_MAP  (map_fnm_base_name, fnm_id, "
                                   "map_owner_name,map_owner_zone, create_ts, modify_ts) values (?, ?, ?, ?, ?, ?)",
                                   &icss);
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlInsFnmTable cmlExecuteNoAnswerSql FNM Map insert failure %d", status);
                                   ...
                                   const char* _my_time)
{
    ...
    cllBindVarCount = i;
    status = cmlGetIntegerValueFromSqlV3(
        "select msrvc_id from R_MICROSRVC_MAIN where  msrvc_module_name = ? and  msrvc_name = ? ", &seqNum, &icss);
    if (status != 0 && status != CAT_NO_ROWS_FOUND) {
        rodsLog(LOG_NOTICE, "chlInsMsrvcTable cmlGetIntegerValueFromSqlV3 find MSRVC if any failure %d", status);
        return ERROR(status, "cmlGetIntegerValueFromSqlV3 find MSRVC if any failure");
    }
    if (seqNum < 0) { /* No micro-service found */
        seqNum = cmlGetNextSeqVal(&icss);
        if (seqNum < 0) {
            rodsLog(LOG_NOTICE, "chlInsMsrvcTable cmlGetNextSeqVal failure %d", seqNum);
        ...
        status = cmlExecuteNoAnswerSql(
            "insert into R_MICROSRVC_MAIN(msrvc_id, msrvc_name, msrvc_module_name, msrvc_signature, msrvc_doxygen, "
            "msrvc_variations, msrvc_owner_name, msrvc_owner_zone, create_ts, modify_ts) values (?, ?, ?, ?,   'NONE', "
            "'NONE',  ?, ?, ?, ?)",
            &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlInsMsrvcTable cmlExecuteNoAnswerSql R_MICROSRVC_MAIN Insert failure %d", status);
        ...
        status =
            cmlExecuteNoAnswerSql("insert into R_MICROSRVC_VER(msrvc_id, msrvc_version, msrvc_host, msrvc_location, "
                                  "msrvc_language, msrvc_type_name, msrvc_status, msrvc_owner_name, msrvc_owner_zone, "
                                  "create_ts, modify_ts) values (?, ?, ?, ?, ?, ?,  ?, ?, ?, ?, ?)",
                                  &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlInsMsrvcTable cmlExecuteNoAnswerSql R_MICROSRVC_VER Insert failure %d", status);
        ...
        if (status != 0 && status != CAT_NO_ROWS_FOUND) {
            rodsLog(
                LOG_NOTICE, "chlInsMsrvcTable cmlGetIntegerValueFromSqlV4 find MSRVC_HOST if any failure %d", status);
        ...
        status = cmlExecuteNoAnswerSql("insert into R_MICROSRVC_VER(msrvc_id, msrvc_version, msrvc_host, "
                                       "msrvc_location, msrvc_language, msrvc_type_name, msrvc_owner_name, "
                                       "msrvc_owner_zone, create_ts, modify_ts) values (?, ?, ?, ?, ?, ?,  ?, ?, ?, ?)",
                                       &icss);
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlInsMsrvcTable cmlExecuteNoAnswerSql R_MICROSRVC_VER Insert failure %d", status);
...
irods::error db_version_rule_base_op(irods::plugin_context& _ctx, const char* _base_name, const char* _my_time)
{
    ...
    status = cmlExecuteNoAnswerSql(
        ...
        &icss);
    if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
        rodsLog(LOG_NOTICE, "chlVersionRuleBase cmlExecuteNoAnswerSql Rule Map version update  failure %d", status);
...
irods::error db_version_dvm_base_op(irods::plugin_context& _ctx, const char* _base_name, const char* _my_time)
{
    ...
    status = cmlExecuteNoAnswerSql("update R_RULE_DVM_MAP set map_dvm_version = ?, modify_ts = ? where "
                                   "map_dvm_base_name = ? and map_dvm_version = '0'",
                                   &icss);
    if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
        rodsLog(LOG_NOTICE, "chlVersionDvmBase cmlExecuteNoAnswerSql DVM Map version update  failure %d", status);
...
irods::error db_version_fnm_base_op(irods::plugin_context& _ctx, const char* _base_name, const char* _my_time)
{
    ...
    status = cmlExecuteNoAnswerSql("update R_RULE_FNM_MAP set map_fnm_version = ?, modify_ts = ? where "
                                   "map_fnm_base_name = ? and map_fnm_version = '0'",
                                   &icss);
    if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
        rodsLog(LOG_NOTICE, "chlVersionFnmBase cmlExecuteNoAnswerSql FNM Map version update  failure %d", status);
...
irods::error db_add_specific_query_op(irods::plugin_context& _ctx, const char* _sql, const char* _alias)
{
    ...
    if (_alias != NULL && strlen(_alias) > 0) {
        ...
        }
        {
            ...
            status = cmlGetStringValueFromSql(
                "select create_ts from R_SPECIFIC_QUERY where alias=?", tsCreateTime, 50, bindVars, &icss);
        }
        if (status == 0) {
        ...
        status =
            cmlExecuteNoAnswerSql("insert into R_SPECIFIC_QUERY  (sqlStr, alias, create_ts) values (?, ?, ?)", &icss);
    }
    else {
        i = 0;
        cllBindVars[i++] = _sql;
        ...
        status = cmlExecuteNoAnswerSql("insert into R_SPECIFIC_QUERY  (sqlStr, create_ts) values (?, ?)", &icss);
    }

    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlAddSpecificQuery cmlExecuteNoAnswerSql insert failure %d", status);
        return ERROR(status, "insert failure");
    }
...
irods::error db_del_specific_query_op(irods::plugin_context& _ctx, const char* _sql_or_alias)
{
    ...
    status = cmlExecuteNoAnswerSql("delete from R_SPECIFIC_QUERY where sqlStr = ?", &icss);
    ...
    if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
        ...
        cllBindVars[i++] = _sql_or_alias;
        cllBindVarCount = i;
        status = cmlExecuteNoAnswerSql("delete from R_SPECIFIC_QUERY where alias = ?", &icss);
    }

    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlDelSpecificQuery cmlExecuteNoAnswerSql delete failure %d", status);
        return ERROR(status, "delete failure");
    }
    ...
    if (_spec_query_inp->continueInx == 0) {
        ...
        */
        ...
        }
        {
            ...
            status = cmlGetStringValueFromSql(
                "select create_ts from R_SPECIFIC_QUERY where sqlStr=?", tsCreateTime, 50, bindVars, &icss);
        }
        if (status == CAT_NO_ROWS_FOUND) {
            int status2;
            if (logSQL != 0) {
            ...
            }
            {
                std::vector<std::string> bindVars;
                bindVars.push_back(_spec_query_inp->sql);
                status2 = cmlGetStringValueFromSql("select sqlStr from R_SPECIFIC_QUERY where alias=?",
                                                   combinedSQL,
                                                   sizeof(combinedSQL),
                                                   bindVars,
                                                   &icss);
        ...
        status = cmlGetFirstRowFromSql(combinedSQL, &statementNum, _spec_query_inp->rowOffset, &icss);
        if (status < 0) {
            if (status != CAT_NO_ROWS_FOUND) {
                rodsLog(LOG_NOTICE, "chlSpecificQuery cmlGetFirstRowFromSql failure %d", status);
            }
    ...
    for (i = 0; i < _spec_query_inp->maxRows; i++) {
        ...
        for (j = 0; j < numOfCols; j++) {
            ...
            tResult2 += currentMaxColSize * (_result->rowCnt - 1); /* skip forward
                                                               for this row */
            strncpy(
                tResult2, icss.stmtPtr[statementNum]->resultValue[j], currentMaxColSize); /* copy in the value text */
        }
    }
                                                           ...
                                                           long long* _count)
{
    ...
    if (!_resc_name || !_count) {
        return ERROR(SYS_INVALID_INPUT_PARAM, "null input param");
    }
    ...
    std::string base_query = "select count(distinct data_id) from R_DATA_MAIN where resc_hier like '%s;%s' or "
                             "resc_hier like '%s;%s;%s' or resc_hier like '%s;%s'";
    sprintf(query,
            base_query.c_str(),
            _resc_name,
            "%", // root node
    ...
    // invoke the query
    int statement_num = 0;
    int status = cmlGetFirstRowFromSql(query, &statement_num, 0, &icss);
    if (status != 0) {
                                                                          ...
                                                                          dist_child_result_t* _results)
{
    ...
    // =-=-=-=-=-=-=-
    // check incoming pointers
    if (!_parent || !_child || _limit <= 0 || !_results) {
        return ERROR(SYS_INVALID_INPUT_PARAM, "null or invalid input param");
    }
    ...
    std::string base_query =
        "select distinct data_id from R_DATA_MAIN where ( resc_hier like '%s;%s' or resc_hier like '%s;%s;%s' or "
        "resc_hier like '%s;%s' ) and data_id not in ( select data_id from R_DATA_MAIN where resc_hier like '%s;%s' or "
        "resc_hier like '%s;%s;%s' or resc_hier like '%s;%s' ) and rownum < %d";

#elif MY_ICAT
    std::string base_query =
        "select distinct data_id from R_DATA_MAIN where ( resc_hier like '%s;%s' or resc_hier like '%s;%s;%s' or "
        "resc_hier like '%s;%s' ) and data_id not in ( select data_id from R_DATA_MAIN where resc_hier like '%s;%s' or "
        "resc_hier like '%s;%s;%s' or resc_hier like '%s;%s' ) limit %d;";

#else
    std::string base_query = "select distinct data_id from R_DATA_MAIN where resc_hier like '%s;%s' or resc_hier like "
                             "'%s;%s;%s' or resc_hier like '%s;%s' except ( select data_id from R_DATA_MAIN where "
                             "resc_hier like '%s;%s' or resc_hier like '%s;%s;%s' or resc_hier like '%s;%s' ) limit %d";
    ...
    for (int i = 0;; i++) {
        ...
        int status = 0;
        if (0 == i) {
            status = cmlGetFirstRowFromSql(query, &statement_num, 0, &icss);
        }
        else {
                                                  ...
                                                  dist_child_result_t* _results)
{
    ...
    const std::string query =
        (boost::format("select data_id from (select distinct data_id from R_DATA_MAIN where data_id in (select data_id "
                       ...
                       "where resc_id in (%s)) and modify_ts <= '%s') where rownum <= %d") %
         not_child_array % child_array % _invocation_timestamp->c_str() % _count)
            .str();
#else
    const std::string query =
        (boost::format("select distinct data_id from R_DATA_MAIN where data_id in (select data_id from R_DATA_MAIN "
                       "where resc_id in (%s)) and data_id not in (select data_id from R_DATA_MAIN where resc_id in "
                       "(%s)) and modify_ts <= '%s' limit %d") %
         not_child_array % child_array % _invocation_timestamp->c_str() % _count)
            .str();
#endif
    ...
    for (rodsLong_t i = 1; i < _count; ++i) {
        ...
        if (status_cmlGetNextRowFromStatement != 0) {
            cmlFreeStatement(statement_num, &icss);
            return ERROR(status_cmlGetNextRowFromStatement,
                         boost::format("failed to get row [%d] from query [%s]") % i % query);
        }
        _results->push_back(atoll(icss.stmtPtr[statement_num]->resultValue[0]));
    }
    cmlFreeStatement(statement_num, &icss);
                                          ...
                                          std::string* _hierarchy)
{
    ...
    // =-=-=-=-=-=-=-
    // check incoming pointers
    if (!_resc_name || !_zone_name || !_hierarchy) {
        return ERROR(SYS_INVALID_INPUT_PARAM, "null or invalid input param");
    }
    ...
    while (current_node) {
        ...
        if (status == CAT_NO_ROWS_FOUND) { // Resource doesn't exist
            ...
            char type_name[250] = "";
            {
                ...
                bindVars.push_back(*_zone_name);
                status =
                    cmlGetStringValueFromSql("select resc_type_name from R_RESC_MAIN where resc_name=? and zone_name=?",
                                             type_name,
                                             250,
                                             bindVars,
                                             &icss);
        ...
        if (strlen(parent)) {
            (*_hierarchy) =
                parent + irods::hierarchy_parser::delimiter() + (*_hierarchy); // Add parent to hierarchy string
            current_node = parent;
        }
        else {
            current_node = NULL;
                              ...
                              const char* _arg5)
{
    ...
    if (strcmp(_op_name, "session") == 0) {
        ...
        status = cmlAudit3(AU_USE_TICKET,
                           "0",
                           _ctx.comm()->clientUser.userName,
                           _ctx.comm()->clientUser.rodsZone,
                           _ticket_string,
    ...
    if (strcmp(_op_name, "create") == 0) {
        ...
        if (status2 > 0) {
            ...
            status3 = cmlCheckDir(
                _arg4, _ctx.comm()->clientUser.userName, _ctx.comm()->clientUser.rodsZone, ACCESS_OWN, &icss);
        ...
        }
        {
            ...
            bindVars.push_back(_ctx.comm()->clientUser.rodsZone);
            status = cmlGetIntegerValueFromSql(
                "select user_id from R_USER_MAIN where user_name=? and zone_name=?", &userId, bindVars, &icss);
        }
        if (status != 0) {
        ...
        seqNum = cmlGetNextSeqVal(&icss);
        if (seqNum < 0) {
            rodsLog(LOG_NOTICE, "chlModTicket failure %ld", seqNum);
        ...
        status = cmlExecuteNoAnswerSql("insert into R_TICKET_MAIN (ticket_id, ticket_string, ticket_type, user_id, "
                                       "object_id, object_type, modify_ts, create_ts) values (?, ?, ?, ?, ?, ?, ?, ?)",
                                       &icss);

        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModTicket cmlExecuteNoAnswerSql insert failure %d", status);
            return ERROR(status, "insert failure");
        }
        status = cmlAudit3(AU_CREATE_TICKET,
                           seqNumStr,
                           _ctx.comm()->clientUser.userName,
                           _ctx.comm()->clientUser.rodsZone,
                           _ticket_string,
    ...
    }
    {
        ...
        status = cmlGetIntegerValueFromSql(
            "select ticket_id from R_TICKET_MAIN where user_id=? and ticket_string=?", &ticketId, bindVars, &icss);
    }
    if (status != 0) {
        if (logSQL != 0) {
        ...
        }
        {
            ...
            bindVars.push_back(_ticket_string);
            status = cmlGetIntegerValueFromSql(
                "select ticket_id from R_TICKET_MAIN where user_id=? and ticket_id=?", &ticketId, bindVars, &icss);
        }
        if (status != 0) {
    ...
    if (strcmp(_op_name, "delete") == 0) {
        ...
        status = cmlExecuteNoAnswerSql("delete from R_TICKET_MAIN where ticket_id = ? and user_id = ?", &icss);
        if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
            return CODE(status);
        }
        if (status != 0) {
            rodsLog(LOG_NOTICE, "chlModTicket cmlExecuteNoAnswerSql delete failure %d", status);
            return ERROR(status, "delete failure");
        }
        ...
        cllBindVars[i++] = ticketIdStr;
        cllBindVarCount = i;
        status = cmlExecuteNoAnswerSql("delete from R_TICKET_ALLOWED_HOSTS where ticket_id = ?", &icss);
        if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
            rodsLog(LOG_NOTICE, "chlModTicket cmlExecuteNoAnswerSql delete 2 failure %d", status);
        }

        i = 0;
        cllBindVars[i++] = ticketIdStr;
        cllBindVarCount = i;
        status = cmlExecuteNoAnswerSql("delete from R_TICKET_ALLOWED_USERS where ticket_id = ?", &icss);
        if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
            rodsLog(LOG_NOTICE, "chlModTicket cmlExecuteNoAnswerSql delete 3 failure %d", status);
        }

        i = 0;
        cllBindVars[i++] = ticketIdStr;
        cllBindVarCount = i;
        status = cmlExecuteNoAnswerSql("delete from R_TICKET_ALLOWED_GROUPS where ticket_id = ?", &icss);
        if (status != 0 && status != CAT_SUCCESS_BUT_WITH_NO_INFO) {
            rodsLog(LOG_NOTICE, "chlModTicket cmlExecuteNoAnswerSql delete 4 failure %d", status);
        }
        status = cmlAudit3(AU_DELETE_TICKET,
                           ticketIdStr,
                           _ctx.comm()->clientUser.userName,
                           _ctx.comm()->clientUser.rodsZone,
                           _ticket_string,
    ...
    if (strcmp(_op_name, "mod") == 0) {
        if (strcmp(_arg3, "uses") == 0) {
            ...
            status = cmlExecuteNoAnswerSql("update R_TICKET_MAIN set uses_limit=? where ticket_id = ? and user_id = ?",
                                           &icss);
            if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
                return CODE(status);
            }
            if (status != 0) {
                rodsLog(LOG_NOTICE, "chlModTicket cmlExecuteNoAnswerSql update failure %d", status);
                return ERROR(status, "update failure");
            }
            status = cmlAudit3(AU_MOD_TICKET,
                               ticketIdStr,
                               _ctx.comm()->clientUser.userName,
                               _ctx.comm()->clientUser.rodsZone,
                               "uses",
        ...
        if (strncmp(_arg3, "write", 5) == 0) {
            if (strstr(_arg3, "file") != NULL) {
                ...
                status = cmlExecuteNoAnswerSql(
                    "update R_TICKET_MAIN set write_file_limit=? where ticket_id = ? and user_id = ?", &icss);
                if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
                    return CODE(status);
                }
                if (status != 0) {
            ...
            if (strstr(_arg3, "byte") != NULL) {
                ...
                status = cmlExecuteNoAnswerSql(
                    "update R_TICKET_MAIN set write_byte_limit=? where ticket_id = ? and user_id = ?", &icss);
                if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
                    return CODE(status);
                }
                if (status != 0) {
        ...
        if (strncmp(_arg3, "expir", 5) == 0) {
            ...
            status = cmlExecuteNoAnswerSql(
                "update R_TICKET_MAIN set ticket_expiry_ts=? where ticket_id = ? and user_id = ?", &icss);
            if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
                return CODE(status);
            }
            if (status != 0) {
        ...
        if (strcmp(_arg3, "add") == 0) {
            if (strcmp(_arg4, "host") == 0) {
                ...
                status =
                    cmlExecuteNoAnswerSql("insert into R_TICKET_ALLOWED_HOSTS (ticket_id, host) values (? , ?)", &icss);
                if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
                    return CODE(status);
                }
                if (status != 0) {
                    rodsLog(LOG_NOTICE, "chlModTicket cmlExecuteNoAnswerSql insert host failure %d", status);
            ...
            if (strcmp(_arg4, "user") == 0) {
                ...
                if (status != 0) {
                    rodsLog(LOG_NOTICE, "chlModTicket cmlExecuteNoAnswerSql insert user failure %d", status);
        ...
        if (strcmp(_arg3, "remove") == 0) {
            if (strcmp(_arg4, "host") == 0) {
                ...
                status =
                    cmlExecuteNoAnswerSql("delete from R_TICKET_ALLOWED_HOSTS where ticket_id=? and host=?", &icss);
                if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
                    return CODE(status);
                }
                if (status != 0) {
                    rodsLog(LOG_NOTICE, "chlModTicket cmlExecuteNoAnswerSql delete host failure %d", status);
                    return ERROR(status, "delete failed");
                }
                status = cmlAudit3(AU_MOD_TICKET,
                                   ticketIdStr,
            ...
            if (strcmp(_arg4, "user") == 0) {
                ...
                status = cmlExecuteNoAnswerSql("delete from R_TICKET_ALLOWED_USERS where ticket_id=? and user_name=?",
                                               &icss);
                if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
                    return CODE(status);
                }
                if (status != 0) {
                    rodsLog(LOG_NOTICE, "chlModTicket cmlExecuteNoAnswerSql delete user failure %d", status);
                    return ERROR(status, "delete failed");
                }
                status = cmlAudit3(AU_MOD_TICKET,
                                   ticketIdStr,
            ...
            if (strcmp(_arg4, "group") == 0) {
                ...
                status = cmlExecuteNoAnswerSql("delete from R_TICKET_ALLOWED_GROUPS where ticket_id=? and group_name=?",
                                               &icss);
                if (status == CAT_SUCCESS_BUT_WITH_NO_INFO) {
                    return CODE(status);
                }
                if (status != 0) {
                    rodsLog(LOG_NOTICE, "chlModTicket cmlExecuteNoAnswerSql delete group failure %d", status);
...
irods::error db_gen_query_op(irods::plugin_context& _ctx, genQueryInp_t* _gen_query_inp, genQueryOut_t* _result)
{
    ...
    //        icatSessionStruct icss;
    //        _ctx.prop_map().get< icatSessionStruct >( ICSS_PROP, icss );
    int status = chl_gen_query_impl(*_gen_query_inp, _result);
    //         if( status < 0 ) {
                                                  ...
                                                  int _control_flag)
{
    ...
    //        icatSessionStruct icss;
    //        _ctx.prop_map().get< icatSessionStruct >( ICSS_PROP, icss );
    int status = chl_gen_query_access_control_setup_impl(_user, _zone, _host, _priv, _control_flag);
    if (status < 0) {
...
irods::error db_gen_query_ticket_setup_op(irods::plugin_context& _ctx, const char* _ticket, const char* _client_addr)
{
    ...
    //        icatSessionStruct icss;
    //        _ctx.prop_map().get< icatSessionStruct >( ICSS_PROP, icss );
    int status = chl_gen_query_ticket_setup_impl(_ticket, _client_addr);
    if (status < 0) {
...
irods::error db_general_update_op(irods::plugin_context& _ctx, generalUpdateInp_t* _update_inp)
{
    ...
    //        icatSessionStruct icss;
    //        _ctx.prop_map().get< icatSessionStruct >( ICSS_PROP, icss );
    int status = chl_general_update_impl(*_update_inp);
    if (status < 0) {
...
extern "C" irods::database* plugin_factory(const std::string& _inst_name, const std::string& _context)
{
    ...
    postgres_database_plugin* pg = new postgres_database_plugin(_inst_name, _context);
    ...
    using namespace std;
    pg->add_operation(DATABASE_OP_START, function<error(plugin_context&)>(db_start_op));
    pg->add_operation<const char*>(DATABASE_OP_DEBUG, function<error(plugin_context&, const char*)>(db_debug_op));
    pg->add_operation(DATABASE_OP_OPEN, function<error(plugin_context&)>(db_open_op));
    pg->add_operation(DATABASE_OP_CLOSE, function<error(plugin_context&)>(db_close_op));
    pg->add_operation<std::string*>(DATABASE_OP_GET_LOCAL_ZONE,
                                    function<error(plugin_context&, std::string*)>(db_get_local_zone_op));
    pg->add_operation<const std::string*, int>(
        DATABASE_OP_UPDATE_RESC_OBJ_COUNT,
        function<error(plugin_context&, const std::string*, int)>(db_update_resc_obj_count_op));
    pg->add_operation<dataObjInfo_t*, keyValPair_t*>(
        DATABASE_OP_MOD_DATA_OBJ_META,
        function<error(plugin_context&, dataObjInfo_t*, keyValPair_t*)>(db_mod_data_obj_meta_op));
    pg->add_operation<dataObjInfo_t*>(DATABASE_OP_REG_DATA_OBJ,
                                      function<error(plugin_context&, dataObjInfo_t*)>(db_reg_data_obj_op));
    pg->add_operation<dataObjInfo_t*, dataObjInfo_t*, keyValPair_t*>(
        DATABASE_OP_REG_REPLICA,
        function<error(plugin_context&, dataObjInfo_t*, dataObjInfo_t*, keyValPair_t*)>(db_reg_replica_op));
    pg->add_operation<dataObjInfo_t*, keyValPair_t*>(
        DATABASE_OP_UNREG_REPLICA,
        function<error(plugin_context&, dataObjInfo_t*, keyValPair_t*)>(db_unreg_replica_op));
    pg->add_operation<ruleExecSubmitInp_t*>(
        DATABASE_OP_REG_RULE_EXEC, function<error(plugin_context&, ruleExecSubmitInp_t*)>(db_reg_rule_exec_op));
    pg->add_operation<const char*, keyValPair_t*>(
        DATABASE_OP_MOD_RULE_EXEC, function<error(plugin_context&, const char*, keyValPair_t*)>(db_mod_rule_exec_op));
    pg->add_operation<const char*>(DATABASE_OP_DEL_RULE_EXEC,
                                   function<error(plugin_context&, const char*)>(db_del_rule_exec_op));
    pg->add_operation<map<string, string>*>(
        DATABASE_OP_ADD_CHILD_RESC, function<error(plugin_context&, map<string, string>*)>(db_add_child_resc_op));
    pg->add_operation<map<string, string>*>(DATABASE_OP_REG_RESC,
                                            function<error(plugin_context&, map<string, string>*)>(db_reg_resc_op));
    pg->add_operation<map<string, string>*>(
        DATABASE_OP_DEL_CHILD_RESC, function<error(plugin_context&, map<string, string>*)>(db_del_child_resc_op));
    pg->add_operation<const char*, int>(DATABASE_OP_DEL_RESC,
                                        function<error(plugin_context&, const char*, int)>(db_del_resc_op));
    pg->add_operation(DATABASE_OP_ROLLBACK, function<error(plugin_context&)>(db_rollback_op));
    pg->add_operation(DATABASE_OP_COMMIT, function<error(plugin_context&)>(db_commit_op));
    pg->add_operation<userInfo_t*>(DATABASE_OP_DEL_USER_RE,
                                   function<error(plugin_context&, userInfo_t*)>(db_del_user_re_op));
    pg->add_operation<collInfo_t*>(DATABASE_OP_REG_COLL_BY_ADMIN,
                                   function<error(plugin_context&, collInfo_t*)>(db_reg_coll_by_admin_op));
    pg->add_operation<collInfo_t*>(DATABASE_OP_REG_COLL, function<error(plugin_context&, collInfo_t*)>(db_reg_coll_op));
    pg->add_operation<collInfo_t*>(DATABASE_OP_MOD_COLL, function<error(plugin_context&, collInfo_t*)>(db_mod_coll_op));
    pg->add_operation<const char*, const char*, const char*, const char*>(
        DATABASE_OP_REG_ZONE,
        function<error(plugin_context&, const char*, const char*, const char*, const char*)>(db_reg_zone_op));
    pg->add_operation<const char*, const char*, const char*>(
        DATABASE_OP_MOD_ZONE, function<error(plugin_context&, const char*, const char*, const char*)>(db_mod_zone_op));
    pg->add_operation<const char*, const char*>(
        DATABASE_OP_RENAME_COLL, function<error(plugin_context&, const char*, const char*)>(db_rename_coll_op));
    pg->add_operation<const char*, const char*, const char*>(
        DATABASE_OP_MOD_ZONE_COLL_ACL,
        function<error(plugin_context&, const char*, const char*, const char*)>(db_mod_zone_coll_acl_op));
    pg->add_operation<const char*, const char*>(
        DATABASE_OP_RENAME_LOCAL_ZONE,
        function<error(plugin_context&, const char*, const char*)>(db_rename_local_zone_op));
    pg->add_operation<const char*>(DATABASE_OP_DEL_ZONE, function<error(plugin_context&, const char*)>(db_del_zone_op));
    pg->add_operation<const char*, const char*, const char*, const char*, const char*, int, int*, char*, int>(
        DATABASE_OP_SIMPLE_QUERY,
        function<error(
            plugin_context&, const char*, const char*, const char*, const char*, const char*, int, int*, char*, int)>(
            db_simple_query_op));
    pg->add_operation<collInfo_t*>(DATABASE_OP_DEL_COLL_BY_ADMIN,
                                   function<error(plugin_context&, collInfo_t*)>(db_del_coll_by_admin_op));
    pg->add_operation<collInfo_t*>(DATABASE_OP_DEL_COLL, function<error(plugin_context&, collInfo_t*)>(db_del_coll_op));
    pg->add_operation<const char*, const char*, const char*, const char*, int*, int*>(
        ...
        function<error(plugin_context&, const char*, const char*, const char*, const char*, int*, int*)>(
            db_check_auth_op));
    pg->add_operation<char*, const char*>(DATABASE_OP_MAKE_TEMP_PW,
                                          function<error(plugin_context&, char*, const char*)>(db_make_temp_pw_op));
    pg->add_operation<const char*, int, const char*, char**>(
        DATABASE_OP_UPDATE_PAM_PASSWORD,
        function<error(plugin_context&, const char*, int, const char*, char**)>(db_update_pam_password_op));
    pg->add_operation<const char*, const char*, const char*>(
        DATABASE_OP_MOD_USER, function<error(plugin_context&, const char*, const char*, const char*)>(db_mod_user_op));
    pg->add_operation<int, char*>(DATABASE_OP_MAKE_LIMITED_PW,
                                  function<error(plugin_context&, int, char*)>(db_make_limited_pw_op));
    pg->add_operation<const char*, const char*, const char*, const char*>(
        DATABASE_OP_MOD_GROUP,
        function<error(plugin_context&, const char*, const char*, const char*, const char*)>(db_mod_group_op));
    pg->add_operation<const char*, const char*, const char*>(
        DATABASE_OP_MOD_RESC, function<error(plugin_context&, const char*, const char*, const char*)>(db_mod_resc_op));
    pg->add_operation<const char*, const char*, const char*, const char*>(
        DATABASE_OP_MOD_RESC_DATA_PATHS,
        function<error(plugin_context&, const char*, const char*, const char*, const char*)>(
            db_mod_resc_data_paths_op));
    pg->add_operation<const char*, int>(DATABASE_OP_MOD_RESC_FREESPACE,
                                        function<error(plugin_context&, const char*, int)>(db_mod_resc_freespace_op));
    pg->add_operation<userInfo_t*>(DATABASE_OP_REG_USER_RE,
                                   function<error(plugin_context&, userInfo_t*)>(db_reg_user_re_op));
    ...
    pg->add_operation<const char*,
                      ...
                      const char*,
                      const char*,
                      const char*>(DATABASE_OP_MOD_AVU_METADATA,
                                   function<error(plugin_context&,
                                                  ...
                                                  const char*,
                                                  const char*,
                                                  const char*)>(db_mod_avu_metadata_op));
    ...
    pg->add_operation<const char*, const char*, const char*, const char*>(
        ...
        function<error(plugin_context&, const char*, const char*, const char*, const char*)>(db_copy_avu_metadata_op));
    ...
    pg->add_operation<rodsLong_t, const char*>(
        DATABASE_OP_RENAME_OBJECT, function<error(plugin_context&, rodsLong_t, const char*)>(db_rename_object_op));
    pg->add_operation<rodsLong_t, rodsLong_t>(
        DATABASE_OP_MOVE_OBJECT, function<error(plugin_context&, rodsLong_t, rodsLong_t)>(db_move_object_op));
    pg->add_operation<const char*, const char*, const char*, const char*, const char*, const char*>(
        ...
        function<error(plugin_context&, const char*, const char*, const char*, const char*, const char*, const char*)>(
            db_reg_token_op));
    pg->add_operation<const char*, const char*>(
        DATABASE_OP_DEL_TOKEN, function<error(plugin_context&, const char*, const char*)>(db_del_token_op));
    pg->add_operation<const char*,
                      const char*,
                      const char*,
                      const char*,
                      ...
                      const char*,
                      const char*,
                      const char*>(DATABASE_OP_REG_SERVER_LOAD,
                                   function<error(plugin_context&,
                                                  const char*,
                                                  const char*,
                                                  const char*,
                                                  ...
                                                  const char*,
                                                  const char*,
                                                  const char*)>(db_reg_server_load_op));
    pg->add_operation<const char*>(DATABASE_OP_PURGE_SERVER_LOAD,
                                   function<error(plugin_context&, const char*)>(db_purge_server_load_op));
    pg->add_operation<const char*, const char*>(
        DATABASE_OP_REG_SERVER_LOAD_DIGEST,
        function<error(plugin_context&, const char*, const char*)>(db_reg_server_load_digest_op));
    pg->add_operation<const char*>(DATABASE_OP_PURGE_SERVER_LOAD_DIGEST,
                                   function<error(plugin_context&, const char*)>(db_purge_server_load_digest_op));
    pg->add_operation(DATABASE_OP_CALC_USAGE_AND_QUOTA, function<error(plugin_context&)>(db_calc_usage_and_quota_op));
    pg->add_operation<const char*, const char*, const char*, const char*>(
        DATABASE_OP_SET_QUOTA,
        function<error(plugin_context&, const char*, const char*, const char*, const char*)>(db_set_quota_op));
    pg->add_operation<const char*, const char*, rodsLong_t*, int*>(
        DATABASE_OP_CHECK_QUOTA,
        function<error(plugin_context&, const char*, const char*, rodsLong_t*, int*)>(db_check_quota_op));
    pg->add_operation(DATABASE_OP_DEL_UNUSED_AVUS, function<error(plugin_context&)>(db_del_unused_avus_op));
    pg->add_operation<const char*,
                      const char*,
                      const char*,
                      const char*,
                      ...
                      const char*,
                      const char*,
                      const char*>(DATABASE_OP_INS_RULE_TABLE,
                                   function<error(plugin_context&,
                                                  const char*,
                                                  const char*,
                                                  const char*,
                                                  ...
                                                  const char*,
                                                  const char*,
                                                  const char*)>(db_ins_rule_table_op));
    pg->add_operation<const char*, const char*, const char*, const char*, const char*>(
    ...
    pg->add_operation<const char*, const char*, const char*, const char*>(
        ...
        function<error(plugin_context&, const char*, const char*, const char*, const char*)>(db_ins_fnm_table_op));
    pg->add_operation<const char*,
                      const char*,
                      const char*,
                      const char*,
                      ...
                      const char*,
                      const char*,
                      const char*>(DATABASE_OP_INS_MSRVC_TABLE,
                                   function<error(plugin_context&,
                                                  const char*,
                                                  const char*,
                                                  const char*,
                                                  ...
                                                  const char*,
                                                  const char*,
                                                  const char*)>(db_ins_msrvc_table_op));
    pg->add_operation<const char*, const char*>(
        DATABASE_OP_VERSION_RULE_BASE,
        function<error(plugin_context&, const char*, const char*)>(db_version_rule_base_op));
    pg->add_operation<const char*, const char*>(
        DATABASE_OP_VERSION_DVM_BASE,
        function<error(plugin_context&, const char*, const char*)>(db_version_dvm_base_op));
    pg->add_operation<const char*, const char*>(
        DATABASE_OP_VERSION_FNM_BASE,
        function<error(plugin_context&, const char*, const char*)>(db_version_fnm_base_op));
    pg->add_operation<const char*, const char*>(
        DATABASE_OP_ADD_SPECIFIC_QUERY,
        function<error(plugin_context&, const char*, const char*)>(db_add_specific_query_op));
    pg->add_operation<const char*>(DATABASE_OP_DEL_SPECIFIC_QUERY,
                                   function<error(plugin_context&, const char*)>(db_del_specific_query_op));
    pg->add_operation<specificQueryInp_t*, genQueryOut_t*>(
        DATABASE_OP_SPECIFIC_QUERY,
        function<error(plugin_context&, specificQueryInp_t*, genQueryOut_t*)>(db_specific_query_op));
    pg->add_operation<const string*, const string*, std::string*>(
        DATABASE_OP_GET_HIERARCHY_FOR_RESC,
        function<error(plugin_context&, const string*, const string*, std::string*)>(db_get_hierarchy_for_resc_op));
    pg->add_operation<const char*, const char*, const char*, const char*, const char*>(
    ...
    pg->add_operation<const char*, const char*, const char*>(
        ...
        function<error(plugin_context&, const char*, const char*, const char*)>(db_check_and_get_object_id_op));
    pg->add_operation<icatSessionStruct**>(DATABASE_OP_GET_RCS,
                                           function<error(plugin_context&, icatSessionStruct**)>(db_get_icss_op));
    pg->add_operation<genQueryInp_t*, genQueryOut_t*>(
        DATABASE_OP_GEN_QUERY, function<error(plugin_context&, genQueryInp_t*, genQueryOut_t*)>(db_gen_query_op));
    pg->add_operation<generalUpdateInp_t*>(DATABASE_OP_GENERAL_UPDATE,
                                           function<error(plugin_context&, generalUpdateInp_t*)>(db_general_update_op));
    ...
    pg->add_operation<const char*, const char*>(
        ...
        function<error(plugin_context&, const char*, const char*)>(db_gen_query_ticket_setup_op));
    pg->add_operation<const char*, long long*>(
        DATABASE_OP_GET_DISTINCT_DATA_OBJ_COUNT_ON_RESOURCE,
        function<error(plugin_context&, const char*, long long*)>(db_get_distinct_data_obj_count_on_resource_op));
    ...
    pg->add_operation<rodsLong_t, size_t, const std::vector<leaf_bundle_t>*, const std::string*, dist_child_result_t*>(
        ...
        function<error(plugin_context&,
                       ...
                       const std::string*,
                       dist_child_result_t*)>(db_get_repl_list_for_leaf_bundles_op));
    return pg;
