...
int sFklink(const char* table1, const char* table2, const char* connectingSQL)
{
    ...
    if (debug > 1)
        printf("link %d is from %d to %d\n", nLinks, Links[nLinks].table1, Links[nLinks].table2);
    if (debug2)
        printf("T%2.2d L%2.2d T%2.2d\n", Links[nLinks].table1, nLinks, Links[nLinks].table2);
    nLinks++;
    return 0;
}
...
int tScan(int table, int link)
{
    ...
    for (i = 0; i < nLinks; i++) {
        if (Links[i].table1 == table && link != i) {
            ...
            if (debug > 1)
                printf("subKeep %d, this table %d, link %d, table2 %d\n", subKeep, table, i, Links[i].table2);
            if (subKeep) {
                thisKeep = 1;
                if (debug > 1) {
                    printf("%d use link %d\n", table, i);
                ...
                if (strlen(whereSQL) > 6) {
                    if (!rstrcat(whereSQL, " AND ", MAX_SQL_SIZE_GQ)) {
                        return USER_STRLEN_TOOLONG;
                    }
                }
                if (!rstrcat(whereSQL, Links[i].connectingSQL, MAX_SQL_SIZE_GQ)) {
                    return USER_STRLEN_TOOLONG;
                }
                if (!rstrcat(whereSQL, " ", MAX_SQL_SIZE_GQ)) {
                    return USER_STRLEN_TOOLONG;
                }
                if (tablePresent(Tables[Links[i].table2].tableAlias, fromSQL) == 0) {
                    if (!rstrcat(fromSQL, ", ", MAX_SQL_SIZE_GQ)) {
                        return USER_STRLEN_TOOLONG;
                    }
                    if (!rstrcat(fromSQL, Tables[Links[i].table2].tableAlias, MAX_SQL_SIZE_GQ)) {
                        return USER_STRLEN_TOOLONG;
                    }
                    if (!rstrcat(fromSQL, " ", MAX_SQL_SIZE_GQ)) {
                        return USER_STRLEN_TOOLONG;
                ...
                if (tablePresent(Tables[Links[i].table1].tableAlias, fromSQL) == 0) {
                    ...
                    if (!rstrcat(fromSQL, Tables[Links[i].table1].tableAlias, MAX_SQL_SIZE_GQ)) {
                        return USER_STRLEN_TOOLONG;
                    }
                    if (!rstrcat(fromSQL, " ", MAX_SQL_SIZE_GQ)) {
                        return USER_STRLEN_TOOLONG;
    ...
    for (i = 0; i < nLinks; i++) {
        if (Links[i].table2 == table && link != i) {
            ...
            if (debug > 1)
                printf("subKeep %d, this table %d, link %d, table1 %d\n", subKeep, table, i, Links[i].table1);
            if (subKeep) {
                thisKeep = 1;
                if (debug > 1) {
                    printf("%d use link %d\n", table, i);
        ...
        if (strcmp(Tables[i].tableName, Columns[colIx].tableName) == 0) {
            ...
            if (sel) {
                if (selectSQLInitFlag == 0) {
                    if (!rstrcat(selectSQL, ",", MAX_SQL_SIZE_GQ)) {
                        return USER_STRLEN_TOOLONG;
                    }
                }
                ...
                if (selectOption != 0) {
                    if (selectOption == SELECT_MIN) {
                        if (!rstrcat(selectSQL, "min(", MAX_SQL_SIZE_GQ)) {
                            return USER_STRLEN_TOOLONG;
                        }
                        selectOptFlag = 1;
                    }
                    if (selectOption == SELECT_MAX) {
                        if (!rstrcat(selectSQL, "max(", MAX_SQL_SIZE_GQ)) {
                            return USER_STRLEN_TOOLONG;
                        }
                        selectOptFlag = 1;
                    }
                    if (selectOption == SELECT_SUM) {
                        if (!rstrcat(selectSQL, "sum(", MAX_SQL_SIZE_GQ)) {
                            return USER_STRLEN_TOOLONG;
                        }
                        selectOptFlag = 1;
                    }
                    if (selectOption == SELECT_AVG) {
                        if (!rstrcat(selectSQL, "avg(", MAX_SQL_SIZE_GQ)) {
                            return USER_STRLEN_TOOLONG;
                        }
                        selectOptFlag = 1;
                    }
                    if (selectOption == SELECT_COUNT) {
                        if (!rstrcat(selectSQL, "count(", MAX_SQL_SIZE_GQ)) {
                            return USER_STRLEN_TOOLONG;
                        }
                        selectOptFlag = 1;
                    }
                }
                if (!rstrcat(selectSQL, Tables[i].tableName, MAX_SQL_SIZE_GQ)) {
                    return USER_STRLEN_TOOLONG;
                }
                if (!rstrcat(selectSQL, ".", MAX_SQL_SIZE_GQ)) {
                    return USER_STRLEN_TOOLONG;
                }
                if (!rstrcat(selectSQL, Columns[colIx].columnName, MAX_SQL_SIZE_GQ)) {
                    return USER_STRLEN_TOOLONG;
                }
                if (!rstrcat(selectSQL, " ", MAX_SQL_SIZE_GQ)) {
                    return USER_STRLEN_TOOLONG;
                ...
                if (selectOptFlag) {
                    ...
                    if (strlen(groupBySQL) > 10) {
                        if (!rstrcat(groupBySQL, ",", MAX_SQL_SIZE_GQ)) {
                            return USER_STRLEN_TOOLONG;
                        }
                    }
                    if (!rstrcat(groupBySQL, Tables[i].tableName, MAX_SQL_SIZE_GQ)) {
                        return USER_STRLEN_TOOLONG;
                    }
                    if (!rstrcat(groupBySQL, ".", MAX_SQL_SIZE_GQ)) {
                        return USER_STRLEN_TOOLONG;
                    }
                    if (!rstrcat(groupBySQL, Columns[colIx].columnName, MAX_SQL_SIZE_GQ)) {
                        return USER_STRLEN_TOOLONG;
                    }
                    if (!rstrcat(groupBySQL, " ", MAX_SQL_SIZE_GQ)) {
                        return USER_STRLEN_TOOLONG;
                ...
                if (tablePresent(Tables[i].tableAlias, fromSQL) == 0) {
                    ...
                    fromCount++;
                    if (!rstrcat(fromSQL, Tables[i].tableAlias, MAX_SQL_SIZE_GQ)) {
                        return USER_STRLEN_TOOLONG;
                    }
                    if (!rstrcat(fromSQL, " ", MAX_SQL_SIZE_GQ)) {
                        return USER_STRLEN_TOOLONG;
                ...
                if (castOption == 1) {
                    if (!rstrcat(whereSQL, "cast (", MAX_SQL_SIZE_GQ)) {
                        return USER_STRLEN_TOOLONG;
                    }
                }

                if (doUpperCase == 1 && castOption == 0) {
                    if (!rstrcat(whereSQL, "upper (", MAX_SQL_SIZE_GQ)) {
                        return USER_STRLEN_TOOLONG;
                    }
                }

                if (!rstrcat(whereSQL, Tables[i].tableName, MAX_SQL_SIZE_GQ)) {
                    return USER_STRLEN_TOOLONG;
                }
                if (!rstrcat(whereSQL, ".", MAX_SQL_SIZE_GQ)) {
                    return USER_STRLEN_TOOLONG;
                }
                if (!rstrcat(whereSQL, Columns[colIx].columnName, MAX_SQL_SIZE_GQ)) {
                    return USER_STRLEN_TOOLONG;
                }
...
#if ORA_ICAT
                    if (!rstrcat(whereSQL, " as number)", MAX_SQL_SIZE_GQ)) {
                        return USER_STRLEN_TOOLONG;
                    }
#else
                    if (!rstrcat(whereSQL, " as decimal)", MAX_SQL_SIZE_GQ)) {
                        return USER_STRLEN_TOOLONG;
                    }
#endif
                }
...
void handleMultiDataAVUConditions(int nConditions)
{
    ...
    for (i = 2; i <= nConditions; i++) {
        char newStr[100];
        snprintf(newStr, sizeof newStr, ", R_OBJT_METAMAP r_data_metamap%d, R_META_MAIN r_data_meta_mn%2.2d ", i, i);
        if (!rstrcat(fromSQL, newStr, MAX_SQL_SIZE_GQ)) {
            return;
        }
    }
    ...
    */
    for (i = 2; i <= nConditions; i++) {
        char newStr[100];
        snprintf(newStr, sizeof newStr, " AND r_data_metamap%d.meta_id = r_data_meta_mn%2.2d.meta_id", i, i);
        if (!rstrcat(whereSQL, newStr, MAX_SQL_SIZE_GQ)) {
            return;
        }
        snprintf(newStr, sizeof newStr, " AND R_DATA_MAIN.data_id = r_data_metamap%d.object_id ", i);
        if (!rstrcat(whereSQL, newStr, MAX_SQL_SIZE_GQ)) {
            return;
        }
    }
...
void handleMultiCollAVUConditions(int nConditions)
{
    ...
    for (i = 2; i <= nConditions; i++) {
        char newStr[100];
        snprintf(newStr, sizeof newStr, ", R_OBJT_METAMAP r_coll_metamap%d, R_META_MAIN r_coll_meta_mn%2.2d ", i, i);
        if (!rstrcat(fromSQL, newStr, MAX_SQL_SIZE_GQ)) {
            return;
        }
    }
    ...
    */
    for (i = 2; i <= nConditions; i++) {
        char newStr[100];
        snprintf(newStr, sizeof newStr, " AND r_coll_metamap%d.meta_id = r_coll_meta_mn%2.2d.meta_id", i, i);
        if (!rstrcat(whereSQL, newStr, MAX_SQL_SIZE_GQ)) {
            return;
        }
        snprintf(newStr, sizeof newStr, " AND R_COLL_MAIN.coll_id = r_coll_metamap%d.object_id ", i);
        if (!rstrcat(whereSQL, newStr, MAX_SQL_SIZE_GQ)) {
            return;
        }
    }
...
int compoundConditionSpecified(char* condition)
{
    ...
    if (strstr(condition, "||") == NULL && strstr(condition, "&&") == NULL) {
        return 0;
    }
    ...
    /* And now test again */
    if (strstr(myCondition, "||") == NULL && strstr(myCondition, "&&") == NULL) {
        return 0;
    }
    return 1;
}
...
int handleCompoundCondition(char* condition, int prevWhereLen)
{
    ...
    while (true) {
        ...
        *cptr = '\0';
        ...
        if (!rstrcat(whereSQL, tabAndColumn, MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
        if (!rstrcpy((char*) &conditionsForBind[conditionsForBindIx],
                     condPart1,
        ...
        if (type == 1) {
            if (!rstrcat(whereSQL, " OR ", MAX_SQL_SIZE_GQ)) {
                return USER_STRLEN_TOOLONG;
            }
        }
        else if (type == 2) {
                ...
                for (j = 0; j < nColumns; j++) {
                    if (Columns[j].defineValue == column) {
                        if (strlen(orderBySQL) > 10) {
                            if (!rstrcat(orderBySQL, ", ", MAX_SQL_SIZE_GQ)) {
                                return;
                            }
                        }
                        if (!rstrcat(orderBySQL, Columns[j].tableName, MAX_SQL_SIZE_GQ)) {
                            return;
                        }
                        if (!rstrcat(orderBySQL, ".", MAX_SQL_SIZE_GQ)) {
                            return;
                        }
                        if (!rstrcat(orderBySQL, Columns[j].columnName, MAX_SQL_SIZE_GQ)) {
                            return;
                        }
                        break;
                    }
...
void setOrderByUser(genQueryInp_t genQueryInp)
{
    int i, j, done;
    for (i = 0; i < genQueryInp.selectInp.len; i++) {
        if ((genQueryInp.selectInp.value[i] & ORDER_BY) || (genQueryInp.selectInp.value[i] & ORDER_BY_DESC)) {
            done = 0;
            for (j = 0; j < nColumns && done == 0; j++) {
                if (Columns[j].defineValue == genQueryInp.selectInp.inx[i]) {
                    ...
                    if (genQueryInp.selectInp.value[i] & ORDER_BY_DESC) {
                        if (!rstrcat(orderBySQL, " DESC ", MAX_SQL_SIZE_GQ)) {
                            return;
                        }
                    }
                    done = 1;
...
int addInClauseToWhereForParentOf(char* inArg)
{
    ...
    if (!rstrcat(whereSQL, whereString.c_str(), MAX_SQL_SIZE_GQ)) {
        return USER_STRLEN_TOOLONG;
    }
...
int addInClauseToWhereForIn(char* inArg, int option)
{
    ...
    if (!rstrcat(whereSQL, " IN (", MAX_SQL_SIZE_GQ)) {
        return USER_STRLEN_TOOLONG;
    }
    len = strlen(inArg);
    for (i = 0; i < len + 1; i++) {
        if (inArg[i] == '\'') {
            ...
            if (quoteState == 2) {
                ...
                if (!rstrcpy((char*) &inStrings[inStrIx], tmpStr, (MAX_SQL_SIZE_GQ * 2) - inStrIx)) {
                    return USER_STRLEN_TOOLONG;
                }
...
int addBetweenClauseToWhere(char* inArg)
{
    ...
    int inStrIx = 0;
    int ncopy;
    if (!rstrcat(whereSQL, " BETWEEN ", MAX_SQL_SIZE_GQ)) {
        return USER_STRLEN_TOOLONG;
    }
    len = strlen(inArg);
    for (i = 0; i < len + 1; i++) {
        if (inArg[i] == '\'') {
            ...
            if (quoteState == 2) {
                ...
                ncopy = endIx - startIx + 1;
                rstrncat(tmpStr, (char*) &inArg[startIx], ncopy, MAX_SQL_SIZE_GQ);
                if (!rstrcpy((char*) &inStrings[inStrIx], tmpStr, MAX_SQL_SIZE_GQ - inStrIx)) {
                    return USER_STRLEN_TOOLONG;
                }
                inStrings[inStrIx + ncopy] = '\0';
                if (cllBindVarCount + 1 >= MAX_BIND_VARS) { // JMC - backport 4848
...
int insertWhere(char* condition, int option)
{
    ...
    if (strcmp(condition, "IS NULL") == 0) {
        ...
        if (!rstrcat(whereSQL, condition, MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
        if (!rstrcat(whereSQL, " ", MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
    ...
    if (cp != NULL) {
...
#if ORA_ICAT
        if (!rstrcat(whereSQL, "=substr(?,1,length(", MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
        if (!rstrcat(whereSQL, tmpStr2, MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
        if (!rstrcat(whereSQL, "))", MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
        if (!rstrcat(whereSQL, " AND length(", MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
#else
        if (!rstrcat(whereSQL, "=substr(?,1,char_length(", MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
        if (!rstrcat(whereSQL, tmpStr2, MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        ...
        if (!rstrcat(whereSQL, " AND char_length(", MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
#endif
        if (!rstrcat(whereSQL, tmpStr2, MAX_SQL_SIZE_GQ)) {
        ...
        if (!rstrcat(whereSQL, ")>0", MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
    }
    else {
        ...
        if (cp != NULL) {
            ...
            tmpStr[i++] = ' ';
            tmpStr[i++] = '\0';
            if (!rstrcat(whereSQL, tmpStr, MAX_SQL_SIZE_GQ)) {
                return USER_STRLEN_TOOLONG;
            }
        }
    }
...
int genqAppendAccessCheck()
{
    ...
    if (strstr(selectSQL, "ticket_string") != NULL && strstr(selectSQL, "R_TICKET_MAIN") != NULL) {
        if (strlen(whereSQL) > 6) {
            if (!rstrcat(whereSQL, " AND ", MAX_SQL_SIZE_GQ)) {
                return USER_STRLEN_TOOLONG;
            }
        ...
        if (!rstrcat(whereSQL,
                     "R_TICKET_MAIN.user_id in (select user_id from R_USER_MAIN UM where UM.user_name = ? AND "
                     "UM.zone_name=?)",
                     MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
    ...
    if (sessionTicket[0] == '\0') {
        ...
        if (strstr(selectSQL, "R_DATA_MAIN") != NULL || strstr(whereSQL, "R_DATA_MAIN") != NULL) {
            if (strlen(whereSQL) > 6) {
                if (!rstrcat(whereSQL, " AND ", MAX_SQL_SIZE_GQ)) {
                    return USER_STRLEN_TOOLONG;
                }
            ...
            if (!rstrcat(whereSQL,
                         ...
                         "TM.token_namespace ='access_type' and TM.token_name = 'read object')",
                         MAX_SQL_SIZE_GQ)) {
                return USER_STRLEN_TOOLONG;
            }
        }

        if (strstr(selectSQL, "R_COLL_MAIN") != NULL || strstr(whereSQL, "R_COLL_MAIN") != NULL) {
            if (strlen(whereSQL) > 6) {
                if (!rstrcat(whereSQL, " AND ", MAX_SQL_SIZE_GQ)) {
                    return USER_STRLEN_TOOLONG;
                }
        ...
        if (strstr(selectSQL, "R_DATA_MAIN") != NULL || strstr(whereSQL, "R_DATA_MAIN") != NULL) {
            ...
            if (!rstrcat(
                    whereSQL,
                    "( R_DATA_MAIN.data_id in (select object_id from R_TICKET_MAIN TICK where TICK.ticket_string=?) OR "
                    "R_COLL_MAIN.coll_id in (select object_id from R_TICKET_MAIN TICK where TICK.ticket_string=?) OR "
                    "R_COLL_MAIN.coll_name LIKE (select (coll_name || '%') from R_COLL_MAIN where coll_id in (select "
                    "object_id from R_TICKET_MAIN TICK where TICK.ticket_string=?)))",
                    MAX_SQL_SIZE_GQ)) {
                return USER_STRLEN_TOOLONG;
            }
        ...
        if (!ticketAlreadyChecked) {
            if (strstr(selectSQL, "R_COLL_MAIN") != NULL || strstr(whereSQL, "R_COLL_MAIN") != NULL) {
                ...
                cllBindVars[cllBindVarCount++] = sessionTicket;
                if (!rstrcat(whereSQL,
                             "( R_COLL_MAIN.coll_id in (select object_id from R_TICKET_MAIN TICK where "
                             "TICK.ticket_string=?) OR R_COLL_MAIN.coll_name LIKE (select (coll_name || '%') from "
                             "R_COLL_MAIN where coll_id in (select object_id from R_TICKET_MAIN TICK where "
                             "TICK.ticket_string=?)))",
                             MAX_SQL_SIZE_GQ)) {
                    return USER_STRLEN_TOOLONG;
                }
...
int generateSpecialQuery(genQueryInp_t genQueryInp, char* resultingSQL)
{
    ...
    char quotaQuery1[] =
        "( select distinct QM.user_id, RM.resc_name, QM.quota_limit, QM.quota_over, QM.resc_id from R_QUOTA_MAIN QM, "
        "R_RESC_MAIN RM, R_USER_GROUP UG, R_USER_MAIN UM2 where QM.resc_id = RM.resc_id AND (QM.user_id = "
        "UG.group_user_id and UM2.user_name = ? and UM2.zone_name = ? and UG.user_id = UM2.user_id )) UNION ( select "
        "distinct QM.user_id, RM.resc_name, QM.quota_limit, QM.quota_over, QM.resc_id from R_QUOTA_MAIN QM, "
        "R_USER_GROUP UG, R_USER_MAIN UM2, R_RESC_MAIN RM where QM.resc_id = '0' AND (QM.user_id = UG.group_user_id "
        "and UM2.user_name = ? and UM2.zone_name = ? and UG.user_id = UM2.user_id)) UNION ( select distinct "
        "QM.user_id, RM.resc_name, QM.quota_limit, QM.quota_over, QM.resc_id from R_QUOTA_MAIN QM, R_USER_MAIN UM, "
        "R_RESC_MAIN RM WHERE (QM.resc_id = RM.resc_id or QM.resc_id = '0') AND (QM.user_id = UM.user_id and "
        "UM.user_name = ? and UM.zone_name = ? )) order by quota_over DESC";

    char quotaQuery2[] =
        "( select distinct QM.user_id, RM.resc_name, QM.quota_limit, QM.quota_over, QM.resc_id from R_QUOTA_MAIN QM, "
        "R_RESC_MAIN RM, R_USER_GROUP UG, R_USER_MAIN UM2 where QM.resc_id = RM.resc_id AND RM.resc_name = ? AND "
        "(QM.user_id = UG.group_user_id and UM2.user_name = ? and UM2.zone_name = ? and UG.user_id = UM2.user_id )) "
        "UNION ( select distinct QM.user_id, RM.resc_name, QM.quota_limit, QM.quota_over, QM.resc_id from R_QUOTA_MAIN "
        "QM, R_USER_GROUP UG, R_USER_MAIN UM2, R_RESC_MAIN RM where QM.resc_id = '0' AND RM.resc_name = ? AND "
        "(QM.user_id = UG.group_user_id and UM2.user_name = ? and UM2.zone_name = ? and UG.user_id = UM2.user_id)) "
        "UNION ( select distinct QM.user_id, RM.resc_name, QM.quota_limit, QM.quota_over, QM.resc_id from R_QUOTA_MAIN "
        "QM, R_USER_MAIN UM, R_RESC_MAIN RM WHERE (QM.resc_id = RM.resc_id or QM.resc_id = '0') AND RM.resc_name = ? "
        "AND (QM.user_id = UM.user_id and UM.user_name = ? and UM.zone_name = ? )) order by quota_over DESC";
    ...
    for (i = 0; i < genQueryInp.sqlCondInp.len; i++) {
        if (genQueryInp.sqlCondInp.inx[i] == COL_USER_NAME) {
            int status = parseUserName(genQueryInp.sqlCondInp.value[i], userName, userZone);
            if (status) {
                rodsLog(LOG_ERROR,
                        "parseUserName failed in generateSpecialQuery on %s with status %d.",
...
int generateSQL(genQueryInp_t genQueryInp, char* resultingSQL, char* resultingCountSQL)
{
    ...
    for (i = 0; i < genQueryInp.selectInp.len; i++) {
        table = setTable(genQueryInp.selectInp.inx[i], 1, genQueryInp.selectInp.value[i] & 0xf, 0);
        if (table < 0) {
    ...
    for (i = 0; i < genQueryInp.sqlCondInp.len; i++) {
        ...
        if ((*cptr == 'n' && *(cptr + 1) == '<') || (*cptr == 'n' && *(cptr + 1) == '>') ||
            (*cptr == 'n' && *(cptr + 1) == '=')) {
            castOption = 1;
            *cptr = ' '; /* clear the 'n' that was just checked so what
                       remains is proper SQL */
        }
        table = setTable(genQueryInp.sqlCondInp.inx[i], 0, 0, castOption);
        if (table < 0) {
            std::cerr << irods::stacktrace().dump(); // XXXX - JMC
    ...
    combinedSQL[0] = '\0';
    if (!rstrcat(combinedSQL, selectSQL, MAX_SQL_SIZE_GQ)) {
        return USER_STRLEN_TOOLONG;
    }
    if (!rstrcat(combinedSQL, " ", MAX_SQL_SIZE_GQ)) {
        return USER_STRLEN_TOOLONG;
    }
    if (!rstrcat(combinedSQL, fromSQL, MAX_SQL_SIZE_GQ)) {
        return USER_STRLEN_TOOLONG;
    }
    ...
    if (strlen(whereSQL) > 6) {
        ...
        if (!rstrcat(combinedSQL, whereSQL, MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
    }
    if (useGroupBy) {
        ...
        if (!rstrcat(combinedSQL, groupBySQL, MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
    }
    ...
    if (strlen(orderBySQL) > 10) {
        if (!rstrcat(combinedSQL, orderBySQL, MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
    }
    ...
    if (genQueryInp.rowOffset > 0) {
        ...
        if (!rstrcat(combinedSQL, " limit ", MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
        if (!rstrcat(combinedSQL, offsetStr, MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
        if (!rstrcat(combinedSQL, ",18446744073709551615", MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
#else
        ...
        if (!rstrcat(combinedSQL, " offset ?", MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
#endif
    }
...
#if ORA_ICAT
    countSQL[0] = '\0';
    if (!rstrcat(countSQL, "select distinct count(*) ", MAX_SQL_SIZE_GQ)) {
        return USER_STRLEN_TOOLONG;
    }
    if (!rstrcat(countSQL, fromSQL, MAX_SQL_SIZE_GQ)) {
        return USER_STRLEN_TOOLONG;
    }

    if (strlen(whereSQL) > 6) {
        if (!rstrcat(countSQL, " ", MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
        if (!rstrcat(countSQL, whereSQL, MAX_SQL_SIZE_GQ)) {
            return USER_STRLEN_TOOLONG;
        }
    }
...
int checkCondInputAccess(genQueryInp_t genQueryInp, int statementNum, icatSessionStruct* icss, int continueFlag)
{
    ...
    for (i = 0; i < genQueryInp.condInput.len; i++) {
        if (strcmp(genQueryInp.condInput.keyWord[i], USER_NAME_CLIENT_KW) == 0) {
            userIx = i;
        }
        if (strcmp(genQueryInp.condInput.keyWord[i], RODS_ZONE_CLIENT_KW) == 0) {
            zoneIx = i;
        }
        if (strcmp(genQueryInp.condInput.keyWord[i], ACCESS_PERMISSION_KW) == 0) {
            accessIx = i;
        }
        if (strcmp(genQueryInp.condInput.keyWord[i], TICKET_KW) == 0) {
            /* for now, log it but the one used is the session ticket */
            rodsLog(LOG_NOTICE, "ticket input, value: %s", genQueryInp.condInput.value[i]);
        }
    }
    if (genQueryInp.condInput.len == 1 && strcmp(genQueryInp.condInput.keyWord[0], ZONE_KW) == 0) {
        return 0;
    }
    ...
    if (dataIx >= 0) {
        if (continueFlag == 1) {
            if (strcmp(prevDataId, icss->stmtPtr[statementNum]->resultValue[dataIx]) == 0) {
                if (strcmp(prevUser, genQueryInp.condInput.value[userIx]) == 0) {
                    if (strcmp(prevAccess, genQueryInp.condInput.value[accessIx]) == 0) {
                        return prevStatus;
                    }
                }
            }
        ...
        status = cmlCheckDataObjId(icss->stmtPtr[statementNum]->resultValue[dataIx],
    ...
    if (collIx >= 0) {
        ...
        status = cmlCheckDirId(icss->stmtPtr[statementNum]->resultValue[collIx],
                               genQueryInp.condInput.value[userIx],
                               (char*) zoneName.c_str(),
                               genQueryInp.condInput.value[accessIx],
                               icss);
...
int chl_gen_query_impl(genQueryInp_t genQueryInp, genQueryOut_t* result)
{
    ...
    if (genQueryInp.continueInx == 0) {
        ...
        if (genQueryInp.options & RETURN_TOTAL_ROW_COUNT) {
            ...
            status = cmlGetIntegerValueFromSqlV3(countSQL, &iVal, icss);
            if (status < 0) {
                if (status != CAT_NO_ROWS_FOUND) {
                    rodsLog(LOG_NOTICE, "chlGenQuery cmlGetIntegerValueFromSqlV3 failure %d", status);
                }
                return status;
            }
            if (iVal >= 0) {
        ...
        status = cmlGetFirstRowFromSql(combinedSQL, &statementNum, genQueryInp.rowOffset, icss);
        if (status < 0) {
            if (status != CAT_NO_ROWS_FOUND) {
                rodsLog(LOG_NOTICE, "chlGenQuery cmlGetFirstRowFromSql failure %d", status);
            }
#if ORA_ICAT
#else
            else {
                int saveStatus;
                if (genQueryInp.options & RETURN_TOTAL_ROW_COUNT && genQueryInp.rowOffset > 0) {
