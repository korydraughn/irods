...
int generalInsert(generalUpdateInp_t generalUpdateInp)
{
    ...
    for (i = 0; i < generalUpdateInp.values.len; i++) {
        j = sGetColumnInfo(generalUpdateInp.values.inx[i], &tableName, &columnName);
        ...
        doBind = 1;
        if (strncmp(generalUpdateInp.values.value[i], UPDATE_NEXT_SEQ_VALUE, MAX_NAME_LEN) == 0) {
        ...
        if (i == 0) {
            ...
            if (doBind) {
                if (strncmp(generalUpdateInp.values.value[i], UPDATE_NOW_TIME, MAX_NAME_LEN) == 0) {
                    getNowStr(myTime);
...
int chl_general_update_impl(generalUpdateInp_t generalUpdateInp)
{
    ...
    if (status != 0) {
        rodsLog(LOG_NOTICE, "chlGeneralUpdate cmlExecuteNoAnswerSql commit failure %d", status);
        return status;
    }
