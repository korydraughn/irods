...
irods::error osauth_auth_client_start(irods::plugin_context& _ctx, rcComm_t* _comm, const char*)
{
    ...
    if (!_ctx.valid<irods::osauth_auth_object>().ok()) {
        ...
        return ERROR(SYS_INVALID_INPUT_PARAM, "null rcConn_t ptr");
    }

    // =-=-=-=-=-=-=-
    // get the native auth object
    irods::osauth_auth_object_ptr ptr = boost::dynamic_pointer_cast<irods::osauth_auth_object>(_ctx.fco());
    // =-=-=-=-=-=-=-
...
irods::error osauth_auth_establish_context(irods::plugin_context& _ctx)
{
    ...
    if (strncmp(ANONYMOUS_USER, ptr->user_name().c_str(), NAME_LEN) == 0) {
        ...
        need_password = osauthGetAuth(const_cast<char*>(ptr->request_result().c_str()),
...
irods::error osauth_auth_client_request(irods::plugin_context& _ctx, rcComm_t* _comm)
{
    ...
    strncpy(req_in.auth_scheme_, irods::AUTH_OSAUTH_SCHEME.c_str(), irods::AUTH_OSAUTH_SCHEME.size() + 1);
...
irods::error osauth_auth_agent_request(irods::plugin_context& _ctx)
{
    // =-=-=-=-=-=-=-
    // validate incoming parameters
    if (!_ctx.valid<irods::osauth_auth_object>().ok()) {
...
irods::error osauth_auth_client_response(irods::plugin_context& _ctx, rcComm_t* _comm)
{
    ...
    if (!_ctx.valid<irods::osauth_auth_object>().ok()) {
        ...
        return ERROR(SYS_INVALID_INPUT_PARAM, "null rcComm_t ptr");
    }
    ...
    if (status < 0) {
        return ERROR(status, "call to rcAuthResponse failed.");
    }
    else {
        return SUCCESS();
    }
...
// =-=-=-=-=-=-=-
// handle an agent-side auth request call
irods::error osauth_auth_agent_response(irods::plugin_context& _ctx, authResponseInp_t* _resp)
{
    // =-=-=-=-=-=-=-
    // validate incoming parameters
    if (!_ctx.valid().ok()) {
        ...
        return ERROR(SYS_INVALID_INPUT_PARAM, "null authResponseInp_t ptr");
    }
    ...
    // zone auth
    status = getAndConnRcatHostNoLogin(_ctx.comm(), MASTER_RCAT, _ctx.comm()->proxyUser.rodsZone, &rodsServerHost);
    if (status < 0) {
        return ERROR(status, "getAndConnRcatHostNoLogin failed");
    }
    ...
    std::string resp_str = irods::AUTH_SCHEME_KEY + irods::kvp_association() + irods::AUTH_OSAUTH_SCHEME +
                           irods::kvp_delimiter() + irods::AUTH_RESPONSE_KEY + irods::kvp_association() +
                           _resp->response;
    ...
    if (status < 0 || authCheckOut == NULL) { // JMC cppcheck
        ...
        free(authCheckOut);
        return ERROR(status, "rxAuthCheck failed");
    }
    ...
    if (rodsServerHost->localFlag != LOCAL_HOST) {
        if (authCheckOut->serverResponse == NULL) {
            ...
            if (requireServerAuth) {
                free(authCheckOut);
                return ERROR(REMOTE_SERVER_AUTH_NOT_PROVIDED, "Authentication disallowed, no serverResponse field");
            }
        }
        else {
            char* cp;
            ...
            if (*authCheckOut->serverResponse == '\0') {
                ...
                if (requireServerAuth) {
                    free(authCheckOut->serverResponse);
                    free(authCheckOut);
                    return ERROR(REMOTE_SERVER_AUTH_EMPTY, "Authentication disallowed, empty serverResponse");
                }
            }
            else {
                ...
                if (len <= 0) {
                    rodsLog(LOG_NOTICE,
                            "rsAuthResponse: Warning, cannot authenticate the remote server, no RemoteZoneSID defined "
                            "in server_config.json",
                            status);
                    if (requireServerAuth) {
                        free(authCheckOut->serverResponse);
                        free(authCheckOut);
                        return ERROR(REMOTE_SERVER_SID_NOT_DEFINED,
                    ...
                    if (OK == 0) {
                        free(authCheckOut->serverResponse);
                        free(authCheckOut);
                        return ERROR(REMOTE_SERVER_AUTHENTICATION_FAILURE,
    ...
    if (status < 0) {
        free(authCheckOut->serverResponse);
        free(authCheckOut);
        return ERROR(status, "check_proxy_user_privileges failed");
    }
    ...
    if (strcmp(_ctx.comm()->proxyUser.userName, _ctx.comm()->clientUser.userName) != 0) {
        ...
        _ctx.comm()->proxyUser.authInfo.authFlag = _ctx.comm()->clientUser.authInfo.authFlag = authCheckOut->privLevel;
    }
...
irods::error osauth_auth_agent_auth_verify(irods::plugin_context&,
                                           ...
                                           const char* _response)
{
    ...
    int status = osauthVerifyResponse(
        const_cast<char*>(_challenge), const_cast<char*>(_user_name), const_cast<char*>(_response));
    if (status) {
        return ERROR(status, "osauthVerifyResponse failed");
    }
    else {
        return SUCCESS();
    }
...
irods::error osauth_auth_agent_verify(irods::plugin_context&, const char*, const char*, const char*)
{
    return SUCCESS();
...
extern "C" irods::auth* plugin_factory(const std::string& _inst_name, const std::string& _context)
{
    // =-=-=-=-=-=-=-
    // create an auth object
    osauth_auth_plugin* nat = new osauth_auth_plugin(_inst_name, _context);
    ...
    nat->add_operation<rcComm_t*, const char*>(
        AUTH_CLIENT_START, function<error(plugin_context&, rcComm_t*, const char*)>(osauth_auth_client_start));
    nat->add_operation(AUTH_ESTABLISH_CONTEXT, function<error(plugin_context&)>(osauth_auth_establish_context));
    nat->add_operation<rcComm_t*>(AUTH_CLIENT_AUTH_REQUEST,
                                  function<error(plugin_context&, rcComm_t*)>(osauth_auth_client_request));
    nat->add_operation<rcComm_t*>(AUTH_CLIENT_AUTH_RESPONSE,
                                  function<error(plugin_context&, rcComm_t*)>(osauth_auth_client_response));
#ifdef RODS_SERVER
    nat->add_operation<const char*>(AUTH_AGENT_START,
                                    function<error(plugin_context&, const char*)>(osauth_auth_agent_start));
    nat->add_operation(AUTH_AGENT_AUTH_REQUEST, function<error(plugin_context&)>(osauth_auth_agent_request));
    nat->add_operation<authResponseInp_t*>(
        AUTH_AGENT_AUTH_RESPONSE, function<error(plugin_context&, authResponseInp_t*)>(osauth_auth_agent_response));
    nat->add_operation<const char*, const char*, const char*>(
        AUTH_AGENT_AUTH_VERIFY,
        function<error(plugin_context&, const char*, const char*, const char*)>(osauth_auth_agent_verify));
#endif
