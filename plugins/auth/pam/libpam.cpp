...
irods::error pam_auth_client_request(irods::plugin_context& _ctx, rcComm_t* _comm)
{
    ...
    irods::kvp_map_t ctx_map;
    irods::error ret = irods::parse_escaped_kvp_string(context, ctx_map);
    if (!ret.ok()) {
        return PASS(ret);
    }
    ...
    strncpy(req_in.auth_scheme_, irods::AUTH_PAM_SCHEME.c_str(), irods::AUTH_PAM_SCHEME.size() + 1);
...
int run_pam_auth_check(const std::string& _username, const std::string& _password)
{
    ...
    if (pid) {
        ...
        */
        ...
        close(p2cp[1]);
        i = execl(PAM_AUTH_CHECK_PROG, PAM_AUTH_CHECK_PROG, _username.c_str(), (char*) NULL);
        perror("execl");
...
irods::error pam_auth_agent_request(irods::plugin_context& _ctx)
{
    ...
    int status =
        getAndConnRcatHost(_ctx.comm(), MASTER_RCAT, (const char*) _ctx.comm()->clientUser.rodsZone, &server_host);
    if (status < 0) {
    ...
    if (kvp.find(irods::AUTH_USER_KEY) == kvp.end() || kvp.find(irods::AUTH_TTL_KEY) == kvp.end() ||
...
irods::error pam_auth_agent_verify(irods::plugin_context&, const char*, const char*, const char*)
{
    return SUCCESS();
}
#endif
...
extern "C" irods::auth* plugin_factory(const std::string& _inst_name, const std::string& _context)
{
    ...
    pam_auth_plugin* pam = new pam_auth_plugin(_inst_name, _context);
    ...
    using namespace std;
    pam->add_operation(AUTH_ESTABLISH_CONTEXT, function<error(plugin_context&)>(pam_auth_establish_context));
    pam->add_operation<rcComm_t*, const char*>(
        AUTH_CLIENT_START, function<error(plugin_context&, rcComm_t*, const char*)>(pam_auth_client_start));
    pam->add_operation<rcComm_t*>(AUTH_CLIENT_AUTH_REQUEST,
                                  function<error(plugin_context&, rcComm_t*)>(pam_auth_client_request));
    pam->add_operation<rcComm_t*>(AUTH_CLIENT_AUTH_RESPONSE,
                                  function<error(plugin_context&, rcComm_t*)>(pam_auth_client_response));
#ifdef RODS_SERVER
    pam->add_operation<const char*>(AUTH_AGENT_START,
                                    function<error(plugin_context&, const char*)>(pam_auth_agent_start));
    pam->add_operation(AUTH_AGENT_AUTH_REQUEST, function<error(plugin_context&)>(pam_auth_agent_request));
    pam->add_operation<authResponseInp_t*>(
        AUTH_AGENT_AUTH_RESPONSE, function<error(plugin_context&, authResponseInp_t*)>(pam_auth_agent_response));
    pam->add_operation<const char*, const char*, const char*>(
        AUTH_AGENT_AUTH_VERIFY,
        function<error(plugin_context&, const char*, const char*, const char*)>(pam_auth_agent_verify));
#endif
