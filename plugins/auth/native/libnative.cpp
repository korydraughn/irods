...
// =-=-=-=-=-=-=-
// handle an agent-side auth request call
irods::error native_auth_agent_response(irods::plugin_context& _ctx, authResponseInp_t* _resp)
{
        ...
        if (rodsServerHost->localFlag != LOCAL_HOST) {
            if (authCheckOut->serverResponse == NULL) {
                ...
                if (*authCheckOut->serverResponse == '\0') {
                    ...
                    if (len <= 0) {
                        rodsLog(LOG_NOTICE,
                                "rsAuthResponse: Warning, cannot authenticate the remote server, no RemoteZoneSID "
                                "defined in server_config.json",
                                status);
                        if (requireServerAuth) {
                            ret = ERROR(REMOTE_SERVER_SID_NOT_DEFINED,
                        ...
                        if (0 == OK) {
                            ret = ERROR(REMOTE_SERVER_AUTHENTICATION_FAILURE,
                                        "Authentication disallowed, server response incorrect.");
                        }
                    }
                }
            }
        ...
        if (ret.ok()) {
            ...
            if (!ret.ok()) {
                ...
                if (strcmp(_ctx.comm()->proxyUser.userName, _ctx.comm()->clientUser.userName) != 0) {
                    ...
                    _ctx.comm()->proxyUser.authInfo.authFlag = _ctx.comm()->clientUser.authInfo.authFlag =
                        authCheckOut->privLevel;
                }
            }
        }
...
// not need to support
irods::error native_auth_agent_verify(irods::plugin_context&, const char*, const char*, const char*)
{
    return SUCCESS();
...
// stub for ops that the native plug does
// not need to support
irods::error native_auth_agent_start(irods::plugin_context&, const char*)
{
    return SUCCESS();
...
extern "C" irods::auth* plugin_factory(const std::string& _inst_name, const std::string& _context)
{
    ...
    native_auth_plugin* nat = new native_auth_plugin(_inst_name, _context);
    ...
    using namespace std;
    nat->add_operation(AUTH_ESTABLISH_CONTEXT, function<error(plugin_context&)>(native_auth_establish_context));
    nat->add_operation<rcComm_t*, const char*>(
        AUTH_CLIENT_START, function<error(plugin_context&, rcComm_t*, const char*)>(native_auth_client_start));
    nat->add_operation<rcComm_t*>(AUTH_CLIENT_AUTH_REQUEST,
                                  function<error(plugin_context&, rcComm_t*)>(native_auth_client_request));
    nat->add_operation<rcComm_t*>(AUTH_CLIENT_AUTH_RESPONSE,
                                  function<error(plugin_context&, rcComm_t*)>(native_auth_client_response));
#ifdef RODS_SERVER
    nat->add_operation<const char*>(AUTH_AGENT_START,
                                    function<error(plugin_context&, const char*)>(native_auth_agent_start));
    nat->add_operation(AUTH_AGENT_AUTH_REQUEST, function<error(plugin_context&)>(native_auth_agent_request));
    nat->add_operation<authResponseInp_t*>(
        AUTH_AGENT_AUTH_RESPONSE, function<error(plugin_context&, authResponseInp_t*)>(native_auth_agent_response));
    nat->add_operation<const char*, const char*, const char*>(
        AUTH_AGENT_AUTH_VERIFY,
        function<error(plugin_context&, const char*, const char*, const char*)>(native_auth_agent_verify));
#endif
