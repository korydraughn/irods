...
irods::error find_compound_resource_in_hierarchy(const std::string& _hier, irods::resource_ptr& _resc)
{
...
/**
 * \fn msisync_to_archive (msParam_t* _resource_hierarchy, msParam_t* _physical_path, msParam_t* _logical_path,
 *ruleExecInfo_t *rei)
 *
 ...
 * \sa none
 **/
int msisync_to_archive(msParam_t* _resource_hierarchy,
                       ...
                       ruleExecInfo_t* _rei)
{
    ...
    irods::file_object_ptr file_obj(new irods::file_object(_rei->rsComm,
                                                           logical_path,
                                                           physical_path,
                                                           resource_hierarchy,
                                                           0,   // fd
    ...
    irods::error ret = find_compound_resource_in_hierarchy(resource_hierarchy, resc);
    if (!ret.ok()) {
        return ret.code();
    }

    std::string auto_repl("off");
    ret = resc->get_property<std::string>("auto_repl", auto_repl);
    if (!ret.ok()) {
        irods::log(PASS(ret));
    }
    ...
    if ("off" == auto_repl) {
        reset_to_off = true;
        resc->set_property<std::string>("auto_repl", "on");
    }

    // inform the resource that a write operation happened
    // to put it in a state where it will need to replicate
    ret = fileNotify(_rei->rsComm, file_obj, irods::WRITE_OPERATION);
    if (!ret.ok()) {
        cout << "msisync_to_archive - fileNotify failed [" << ret.result().c_str() << "] - [" << ret.code() << "]"
             << endl;
        if (reset_to_off) {
            resc->set_property<std::string>("auto_repl", "off");
        }
        return ret.code();
    }

    const keyValPair_t& kvp = file_obj->cond_input();
    addKeyVal((keyValPair_t*) &kvp, ADMIN_KW, "true");
    ...
    ret = fileModified(_rei->rsComm, file_obj);
    ...
    if (!ret.ok()) {
        cout << "msisync_to_archive - fileModified failed [" << ret.result().c_str() << "] - [" << ret.code() << "]"
             << endl;
        return ret.code();
    }
...
extern "C" irods::ms_table_entry* plugin_factory()
{
    ...
    msvc->add_operation<msParam_t*, msParam_t*, msParam_t*, ruleExecInfo_t*>(
        "msisync_to_archive",
        std::function<int(msParam_t*, msParam_t*, msParam_t*, ruleExecInfo_t*)>(msisync_to_archive));
    return msvc;
}
