...
int msi_update_unixfilesystem_resource_free_space(msParam_t* resource_name_msparam, ruleExecInfo_t* rei)
{
    ...
    if (strcmp(resource_name_msparam->type, STR_MS_T)) {
        rodsLog(
            LOG_ERROR, "[%s]: first argument should be STR_MS_T, was [%s]", __FUNCTION__, resource_name_msparam->type);
    ...
    const irods::error get_resource_type_error =
        resource_ptr->get_property<std::string>(irods::RESOURCE_TYPE, resource_type_unnormalized);
    ...
    const irods::error get_resource_vault_path_error =
        resource_ptr->get_property<std::string>(irods::RESOURCE_PATH, resource_vault_path);
    ...
    while (!boost::filesystem::exists(path_to_stat)) {
        rodsLog(LOG_NOTICE,
                "[%s]: path to stat [%s] for resource [%s] doesn't exist, moving to parent",
                __FUNCTION__,
                path_to_stat.string().c_str(),
                resource_name_cstring);
        path_to_stat = path_to_stat.parent_path();
        if (path_to_stat.empty()) {
            rodsLog(LOG_ERROR,
                    "[%s]: could not find existing path from vault path [%s] for resource [%s]",
                    __FUNCTION__,
                    resource_vault_path.c_str(),
                    resource_name_cstring);
    ...
    if (absolute_vault_path.root_path() == path_to_stat) {
        rodsLog(LOG_ERROR,
                "[%s]: could not find existing non-root path from vault path [%s] for resource [%s]",
                __FUNCTION__,
                resource_vault_path.c_str(),
                resource_name_cstring);
        return SYS_INVALID_RESC_INPUT;
    ...
    if (statvfs_ret != 0) {
        rodsLog(LOG_ERROR,
                "[%s]: statvfs() of [%s] for resource [%s] failed with return %d and errno %d",
                __FUNCTION__,
                path_to_stat.string().c_str(),
                resource_name_cstring,
                statvfs_ret,
    ...
    if (statvfs_buf.f_bavail > ULONG_MAX / statvfs_buf.f_frsize) {
        rodsLog(LOG_NOTICE,
                "[%s]: statvfs() of resource [%s] reports free space in excess of ULONG_MAX f_bavail %ju f_frsize %ju "
                "failed with return %d and errno %d",
                __FUNCTION__,
                resource_name_cstring,
                static_cast<uintmax_t>(statvfs_buf.f_bavail),
                static_cast<uintmax_t>(statvfs_buf.f_frsize));
    ...
    rcComm_t* admin_connection = rcConnect(service_account_environment.rodsHost,
                                           service_account_environment.rodsPort,
                                           service_account_environment.rodsUserName,
                                           service_account_environment.rodsZone,
                                           0,
                                           &errMsg);
...
extern "C" irods::ms_table_entry* plugin_factory()
{
    ...
    msvc->add_operation<msParam_t*, ruleExecInfo_t*>(
        "msi_update_unixfilesystem_resource_free_space",
        std::function<int(msParam_t*, ruleExecInfo_t*)>(msi_update_unixfilesystem_resource_free_space));
    return msvc;
}
