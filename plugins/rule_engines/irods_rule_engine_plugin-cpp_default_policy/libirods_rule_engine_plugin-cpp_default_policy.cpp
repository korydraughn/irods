        ...
        if (itr.type() == typeid(std::string*)) {
            try {
                *boost::any_cast<std::string*>(itr) = "CS_NEG_DONT_CARE";
                ...
                return ERROR(INVALID_ANY_CAST, "Bad any_cast in acPreConnect()");
            }
        }
    }
...
irods::error acSetRescSchemeForCreate(irods::callback _cb, std::list<boost::any>&)
{
    ...
    */
    ...
    try {
        ...
        rodsLog(LOG_NOTICE, "No default_resource_name in server properties - using 'demoResc'");
    }
...
{
    _ret = (static_policy_enforcement_points.find(_rn) != static_policy_enforcement_points.end());
    return SUCCESS();
}
                       ...
                       irods::callback _eff_hdlr)
{
    ...
    if (static_policy_enforcement_points.find(_rn) != static_policy_enforcement_points.end()) {
        return static_policy_enforcement_points[_rn](_eff_hdlr, _ps);
    }
    else {
        rodsLog(LOG_ERROR, "[%s] not defined in default rule engine", _rn.c_str());
        return SUCCESS();
    }
}
