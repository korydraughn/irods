...
int initializeReDebug(rsComm_t* svrComm)
{
    ...
    sprintf(condRead,
            "(*XUSER  == \"%s@%s\") && (*XHDR == \"STARTDEBUG\")",
            svrComm->clientUser.userName,
            svrComm->clientUser.rodsZone);
    ...
    if (status >= 0) {
        ...
        snprintf(waitMsg,
                 MAX_NAME_LEN,
                 "PROCESS BEGIN at %s:%i. Client connected from %s at port %i\n",
                 myHostName,
                 myPID,
                 svrComm->clientAddr,
                 ntohs(svrComm->localAddr.sin_port));
...
int processXMsg(int streamId, char* readmsg, RuleEngineEventParam* param, Node* node, Env* env, ruleExecInfo_t* rei)
{
        ...
        if (node->secondaryIndex) {
            ...
            for (i = 0; i < n; i++) {
                ...
                while (b != NULL) {
                    ...
                    snprintf(mymsg + strlen(mymsg),
                             MAX_NAME_LEN - strlen(mymsg),
                             "%i: %s\n%s\n",
                             node->ruleIndex,
                             rd->node->base[0] == 's' ? "<source>" : rd->node->base + 1,
                             buf);
                    b = b->next;
                }
            }
    ...
    for (i = 0; i < breakPointsInx; i++) {
        if (breakPoints[i].actionName != NULL) {
            if (breakPoints[i].base != NULL) {
                snprintf(mymsg + strlen(mymsg),
                         MAX_NAME_LEN - strlen(mymsg),
                         "Breaking at BreakPoint %i:%s %s:%d\n",
                         i,
                         breakPoints[i].actionName,
                         breakPoints[i].base[0] == 's' ? "<source>" : breakPoints[i].base + 1,
                         breakPoints[i].line);
            }
            else {
                snprintf(mymsg + strlen(mymsg),
                         MAX_NAME_LEN - strlen(mymsg),
                         "Breaking at BreakPoint %i:%s\n",
                         i,
                         breakPoints[i].actionName);
            }
        }
        ...
        for (i = 0; i < n; i++) {
            ...
            while (b != NULL) {
                if (b->key[0] == '*') { /* skip none * variables */
                    ...
                    snprintf(
                        mymsg + strlen(mymsg), MAX_NAME_LEN - strlen(mymsg), "%s of type %s\n", b->key, typeString);
                }
                b = b->next;
            }
        }
    ...
    if (getNodeType(n) == N_ERROR) {
        ...
        if (env != NULL) {
            ...
            res = computeNode(
                n, NULL, regionRegionCpEnv(env, r, (RegionRegionCopyFuncType*) regionRegionCpNode), rei, 0, &errmsg, r);
...
int reDebug(RuleEngineEvent label, int flag, RuleEngineEventParam* param, Node* node, Env* env, ruleExecInfo_t* rei)
{
    ...
    if (GlobalREAuditFlag > 0) {
        if (flag == -4) {
            if (rei->uoic != NULL && strlen(rei->uoic->userName) > 0 && strlen(rei->uoic->rodsZone) > 0) {
                snprintf(
                    myActionStr[aNum], MAX_NAME_LEN + 10, "  USER:%s@%s", rei->uoic->userName, rei->uoic->rodsZone);
                aNum++;
            }
    ...
    if (GlobalREDebugFlag > 5) {
        ...
        if (curStat == REDEBUG_CONTINUE && reDebugStackCurrPtr <= reDebugStackPtr &&
            (reDebugPCType(label) & reDebugStopAt) != 0) {
        ...
        while (GlobalREDebugFlag > 5) {
            ...
            sprintf(condRead,
                    "(*XSEQNUM >= %d) && (*XADDR != \"%s:%i\") && (*XUSER  == \"%s@%s\") && ((*XHDR == \"CMSG:ALL\") "
                    "%%%% (*XHDR == \"CMSG:%s:%i\"))",
                    s,
                    myHostName,
                    myPID,
            ...
            */
            ...
            if (status >= 0) {
                ...
                curStat = processXMsg(GlobalREDebugFlag, readmsg, param, node, env, rei);
                if (readhdr != NULL) {
                    free(readhdr);
                }
                if (readmsg != NULL) {
            ...
            if (curStat == REDEBUG_CONTINUE || curStat == REDEBUG_CONTINUE_VERBOSE) {
                ...
                curStat = processBreakPoint(GlobalREDebugFlag, param, node, curStat);
