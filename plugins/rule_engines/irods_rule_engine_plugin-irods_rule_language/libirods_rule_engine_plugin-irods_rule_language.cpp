    ...
    try {
        ...
        for (const auto& elem : arr) {
            try {
                ...
                rodsLog(LOG_ERROR, "%s - failed to cast regex to string", __FUNCTION__);
                continue;
            }
        }
    }
...
void initialize_microservice_table()
{
    ...
    table["msiGetStdoutInExecCmdOut"] = new irods::ms_table_entry(
        "msiGetStdoutInExecCmdOut",
        2,
        std::function<int(msParam_t*, msParam_t*, ruleExecInfo_t*)>(msiGetStdoutInExecCmdOut));
    table["msiGetStderrInExecCmdOut"] = new irods::ms_table_entry(
        "msiGetStderrInExecCmdOut",
        2,
        std::function<int(msParam_t*, msParam_t*, ruleExecInfo_t*)>(msiGetStderrInExecCmdOut));
    table["msiAddKeyValToMspStr"] = new irods::ms_table_entry(
        "msiAddKeyValToMspStr",
        3,
        std::function<int(msParam_t*, msParam_t*, msParam_t*, ruleExecInfo_t*)>(msiAddKeyValToMspStr));
    table["msiWriteRodsLog"] = new irods::ms_table_entry(
        "msiWriteRodsLog", 2, std::function<int(msParam_t*, msParam_t*, ruleExecInfo_t*)>(msiWriteRodsLog));
    table["msiSplitPath"] = new irods::ms_table_entry(
        "msiSplitPath", 3, std::function<int(msParam_t*, msParam_t*, msParam_t*, ruleExecInfo_t*)>(msiSplitPath));
    table["msiSplitPathByKey"] = new irods::ms_table_entry(
        "msiSplitPathByKey",
        4,
        std::function<int(msParam_t*, msParam_t*, msParam_t*, msParam_t*, ruleExecInfo_t*)>(msiSplitPathByKey));
    table["msiGetSessionVarValue"] = new irods::ms_table_entry(
        "msiGetSessionVarValue", 2, std::function<int(msParam_t*, msParam_t*, ruleExecInfo_t*)>(msiGetSessionVarValue));
    table["msiStrlen"] = new irods::ms_table_entry(
        "msiStrlen", 2, std::function<int(msParam_t*, msParam_t*, ruleExecInfo_t*)>(msiStrlen));
    table["msiStrCat"] = new irods::ms_table_entry(
        "msiStrCat", 2, std::function<int(msParam_t*, msParam_t*, ruleExecInfo_t*)>(msiStrCat));
    table["msiStrchop"] = new irods::ms_table_entry(
        "msiStrchop", 2, std::function<int(msParam_t*, msParam_t*, ruleExecInfo_t*)>(msiStrchop));
    table["msiSubstr"] = new irods::ms_table_entry(
        "msiSubstr", 4, std::function<int(msParam_t*, msParam_t*, msParam_t*, msParam_t*, ruleExecInfo_t*)>(msiSubstr));
    table["msiExit"] =
        new irods::ms_table_entry("msiExit", 2, std::function<int(msParam_t*, msParam_t*, ruleExecInfo_t*)>(msiExit));
}
...
irods::error start(irods::default_re_ctx&, const std::string& _instance_name)
{
    ...
    try {
        const auto& re_plugin_arr = boost::any_cast<const std::vector<boost::any>&>(
        ...
        for (const auto& el : re_plugin_arr) {
            ...
            if (inst_name == _instance_name) {
                const auto& shmem_value =
                    boost::any_cast<const std::string&>(plugin_config.at(irods::CFG_SHARED_MEMORY_INSTANCE_KW));
                ...
                std::string core_fnm =
                    get_string_array_from_array(plugin_spec_cfg.at(irods::CFG_RE_FUNCTION_NAME_MAPPING_SET_KW));
                std::string core_dvm =
                    get_string_array_from_array(plugin_spec_cfg.at(irods::CFG_RE_DATA_VARIABLE_MAPPING_SET_KW));
                int status =
                    initRuleEngine(shmem_value.c_str(), nullptr, core_re.c_str(), core_dvm.c_str(), core_fnm.c_str());
                if (status < 0) {
                    return ERROR(status, "failed to initialize native rule engine");
                }
    ...
    std::stringstream msg;
    msg << "failed to find configuration for re-irods plugin [" << _instance_name << "]";
...
irods::error rule_exists(irods::default_re_ctx&, const std::string& _rn, bool& _ret)
{
    if (ruleEngineConfig.ruleEngineStatus == UNINITIALIZED) {
        rodsLog(LOG_DEBUG, "rule engine xre has not initialized, skip rule [%s]", _rn.c_str());
        _ret = false;
        return SUCCESS();
    }
    _ret = lookupFromEnv(ruleEngineConfig.extFuncDescIndex, const_cast<char*>(_rn.c_str())) != NULL;
    rodsLog(LOG_DEBUG, "looking up rule name %s, found = %d", _rn.c_str(), _ret);
                       ...
                       irods::callback _eff_hdlr)
{
    ...
    rodsLog(LOG_DEBUG, "applying rule %s, params %ld", _rn.c_str(), _ps.size());
    ...
    rodsLog(LOG_DEBUG, "rule engine return %d", ret);
...
{
    if (ruleEngineConfig.ruleEngineStatus == UNINITIALIZED) {
        rodsLog(LOG_DEBUG, "rule engine xre has not initialized, skip rule [%s]", _rt.c_str());
        return SUCCESS();
    }
    ...
    rodsLog(LOG_DEBUG, "applying rule %s, params %ld", _rt.c_str(), _ms_params->len);

    const int status = execMyRule((char*) _rt.c_str(), _ms_params, _out_desc.c_str(), rei);
                                  ...
                                  irods::callback _eff_hdlr)
{
    ...
    int status = computeExpression((char*) rule_text.c_str(), _ms_params, rei, NO_SAVE_REI, res);
