...
char* findBoundary(char* inStr, char* bound, int length)
{
    ...
    while (length > 0) {
        ...
        /**printf("  BB:%i:%c %i:%c\n", (char *) tmpPtr, (int) *((char *) tmpPtr), (char *) tmpPtr1,(int) *((char *)
         * tmpPtr1)); **/
        if (tmpPtr1 == NULL) {
            return NULL;
        }
...
int getmultipartword(entry* iEntry, char** stquery, char* boundary, int length)
{
    ...
    *tmp1 = '\0';
    ...
    if ((int) (strstr(tmp1, "filename") != NULL && strstr(tmp1, "filename") - tmp1) < 5) {
...
int getEntries(inStruct Sentries)
{
    ...
    if (strstr(Sentries->cookieStr, "*") != NULL || strstr(Sentries->cookieStr, "..") != NULL ||
        strstr(Sentries->cookieStr, "?") != NULL || strstr(Sentries->cookieStr, "/") != NULL ||
...
int showRules(inStruct Sentries)
{
    ...
    */
    ...
    for (j = 0; j < coreRuleStrct.MaxNumOfRules; j++) {
        ...
        fprintf(stdout,
                "<TR><TD><BR>&nbsp;%i</TD><TD><BR>&nbsp;<FONT COLOR=#0000FF>ON</FONT></TD><TD "
                "COLSPAN=3><BR>&nbsp;<FONT COLOR=#FF0000>%s.</FONT><FONT COLOR=#0000FF>%s</FONT></TD></TR>\n",
                j,
                ruleBase,
                ruleHead);
        if (strlen(ruleCondition) != 0) {
            fprintf(stdout,
                    "<TR><TD></TD><TD></TD><TD VALIGN=TOP>IF &nbsp;</TD><TD  COLSPAN=3 VALIGN=TOP><FONT "
                    "COLOR=#FF0000>%s</FONT></TD></TR>\n",
                    ruleCondition);
        }
        for (i = 0; i < n; i++) {
            if (i == 0)
                fprintf(stdout,
                        "<TR><TD></TD><TD></TD><TD VALIGN=TOP>DO &nbsp;</TD><TD VALIGN=TOP>%s</TD><TD "
                        "NOWRAP>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</TD><TD VALIGN=TOP>[%s]</TD></TR>\n",
                        actionArray[i],
                        recoveryArray[i]);
            else
                fprintf(stdout,
                        "<TR><TD></TD><TD></TD><TD VALIGN=TOP>AND &nbsp;</TD><TD VALIGN=TOP>%s</TD><TD "
                        "NOWRAP>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</TD><TD VALIGN=TOP>[%s]</TD></TR>\n",
                        actionArray[i],
                        recoveryArray[i]);
        }
