        ...
        while (resumingBucket != NULL) {
            ...
            while (currIndexNode != NULL) {
                ...
                while (currIndexNode != NULL) {
                    ...
                    if (!(getNodeType(ruleCond) == N_APPLICATION && getNodeType(ruleCond->subtrees[0]) == TK_TEXT &&
                          strcmp(ruleCond->subtrees[0]->text, "==") == 0 && /* comparison */
                          getNodeType(ruleCond->subtrees[1]) == N_TUPLE && ruleCond->subtrees[1]->degree == 2 &&
                    ...
                    if (condExp == NULL) {
                        ...
                        for (i = 0; i < params->degree; i++) {
                            updateInHashTable(varMapping,
                                              params->subtrees[i]->text,
                                              ruleNode->subtrees[0]->subtrees[0]->subtrees[i]->text);
                        }
...
#ifdef DEBUG_INDEX
                printf(
                    "inserting rule group %s(%d) into ruleEngineConfig.condIndex\n", resumingBucket->key, groupCount);
#endif
        ...
        if (ruleType == RK_REL || ruleType == RK_FUNC) {
            ...
            if (fd != NULL) {
                ...
                if (getNodeType(fd) == N_FD_RULE_INDEX_LIST) {
                    ...
                    if (updateInHashTable(ruleIndex,
                                          key,
                                          newRuleIndexListFD(newRuleIndexList(key, i + offset, r), fd->exprType, r)) ==
                        0) {
                        return 0;
                    }
                }
                ...
                if (insertIntoHashTable(
                        ruleIndex, key, newRuleIndexListFD(newRuleIndexList(key, i + offset, r), NULL, r)) == 0) {
                    return 0;
                }
            }
        }
...
int mapExternalFuncToInternalProc2(char* funcName)
{
    ...
    if (coreRuleFuncMapDefIndex != NULL &&
        (i = (int*) lookupFromHashTable(coreRuleFuncMapDefIndex, funcName)) != NULL) {
