...
int applyRuleArg331(const char* action,
...
/* This function computes the expression in inAction using inMsParamArray as the env and updates inMsParamArray with the
 * new env. */
...
/**
 * \fn applyAllRules( char *inAction, msParamArray_t *inMsParamArray, ruleExecInfo_t *rei, int reiSaveFlag, int
 *allRuleExecFlag )
 *
 ...
 *
 **/
int applyAllRules(char* inAction,
                  msParamArray_t* inMsParamArray,
                  ruleExecInfo_t* rei,
                  int reiSaveFlag,
                  int allRuleExecFlag)
...
void appendOutputToInput(msParamArray_t* inpParamArray, char** outParamNames, int outParamN)
{
    ...
    for (i = 0; i < outParamN; i++) {
        ...
        for (k = 0; k < inpParamArray->len; k++) {
            if (inpParamArray->msParam[k]->label != NULL &&
                strcmp(outParamNames[i], inpParamArray->msParam[k]->label) == 0) {
                repeat = 1;
                break;
            }
        }
...

{
    ...
    Region* r = make_region(0, NULL);
    status = parseAndComputeRuleAdapter(ruleDef, inMsParamArray, rei, reiSaveFlag, r);
    region_free(r);
    if (status < 0) {
...
int readDVarStructFromFile(char* dvarBaseName, rulevardef_t* inRuleVarDef)
{
    ...
    if (dvarBaseName[0] == '/' || dvarBaseName[0] == '\\' || dvarBaseName[1] == ':') {
    ...
    if (file == NULL) {
        rodsLog(LOG_NOTICE, "readDvarStructFromFile() could not open dvm file %s\n", dvarsFileName);
...
int readFuncMapStructFromFile(char* fmapBaseName, rulefmapdef_t* inRuleFuncMapDef)
{
    ...
    if (fmapBaseName[0] == '/' || fmapBaseName[0] == '\\' || fmapBaseName[1] == ':') {
    ...
    if (file == NULL) {
        rodsLog(LOG_NOTICE, "readFmapStructFromFile() could not open fnm file %s\n", fmapsFileName);
...
int readMsrvcStructFromFile(char* msrvcFileName, msrvcStruct_t* inMsrvcStruct)
{
    ...
    if (msrvcFileName[0] == '/' || msrvcFileName[0] == '\\' || msrvcFileName[1] == ':') {
        snprintf(mymsrvcFileName, MAX_NAME_LEN, "%s", msrvcFileName);
    }
    else {
        std::string re_dir;
        irods::error ret = irods::get_full_path_for_unmoved_configs("reConfigs", re_dir);
        if (!ret.ok()) {
            irods::log(PASS(ret));
            return ret.code();
        }
    ...
    if (file == NULL) {
        rodsLog(LOG_NOTICE, "readMservcStructFromFile() could not open msrvc file %s\n", mymsrvcFileName);
...
int writeRulesIntoFile(char* inFileName, ruleStruct_t* myRuleStruct, ruleExecInfo_t*)
{
    ...
    if (inFileName[0] == '/' || inFileName[0] == '\\' || inFileName[1] == ':') {
    ...
    if (file == NULL) {
        rodsLog(LOG_NOTICE, "writeRulesIntoFile() could not open rules file %s for writing\n", fileName);
...
int writeDVMapsIntoFile(char* inFileName, dvmStruct_t* myDVMapStruct, ruleExecInfo_t*)
{
    ...
    file = fopen(fileName, "w");
    if (file == NULL) {
        rodsLog(LOG_NOTICE, "writeDVMapsIntoFile() could not open rules file %s for writing\n", fileName);
        return FILE_OPEN_ERR;
    }
...
int writeFNMapsIntoFile(char* inFileName, fnmapStruct_t* myFNMapStruct, ruleExecInfo_t*)
{
    ...
    file = fopen(fileName, "w");
    if (file == NULL) {
        rodsLog(LOG_NOTICE, "writeFNMapsIntoFile() could not open rules file %s for writing\n", fileName);
        return FILE_OPEN_ERR;
    }
    for (i = 0; i < myFNMapStruct->MaxNumOfFMaps; i++) {
        fprintf(file, "%s|%s|%ld\n", myFNMapStruct->funcName[i], myFNMapStruct->func2CMap[i], myFNMapStruct->fmapId[i]);
    }
    fclose(file);
    return 0;
}
...
int writeMSrvcsIntoFile(char* inFileName, msrvcStruct_t* myMsrvcStruct, ruleExecInfo_t*)
{
    ...
    file = fopen(fileName, "w");
    if (file == NULL) {
        rodsLog(LOG_NOTICE, "writeMsrvcsIntoFile() could not open microservics file %s for writing\n", fileName);
        return FILE_OPEN_ERR;
    }
