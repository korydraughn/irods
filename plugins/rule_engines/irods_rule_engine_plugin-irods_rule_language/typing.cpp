...
Satisfiability createSimpleConstraint(ExprType* a,
                                      ExprType* b,
                                      int flex,
                                      Node* node,
                                      Hashtable* typingEnv,
                                      Hashtable* equivalence,
                                      List* simpleTypingConstraints,
                                      Region* r);
 ...
 *        -1 success
 */
int typeParameters(ExprType** paramTypes,
                   int len,
                   Node** subtrees,
                   int dynamictyping,
                   Env* funcDesc,
                   ...
                   Region* r)
{
    int i;
    for (i = 0; i < len; i++) {
        paramTypes[i] = dereference(
            typeExpression3(
                subtrees[i], dynamictyping, funcDesc, symbol_type_table, typingConstraints, errmsg, errnode, r),
            symbol_type_table,
            r);
...
int tautologyLt(ExprType* type, ExprType* expected)
{
    ...
    else if ((getNodeType(type) == T_CONS && getNodeType(expected) == T_CONS) ||
             (getNodeType(type) == T_TUPLE && getNodeType(expected) == T_TUPLE)) {
...
int tautologyLtBase(ExprType* a, ExprType* b)
{
    ...
    if (getNodeType(a) == T_IRODS && getNodeType(b) == T_IRODS &&
...
Satisfiability splitVarR(ExprType* consTuple,
                         ExprType* var,
                         int flex,
                         Node* node,
                         Hashtable* typingEnv,
...
Satisfiability splitVarL(ExprType* var,
                         ExprType* consTuple,
                         int flex,
                         Node* node,
                         Hashtable* typingEnv,
 ...
 *         -1 absurdity
 */
Satisfiability simplifyR(ExprType* a,
                         ExprType* b,
                         int flex,
                         Node* node,
                         Hashtable* typingEnv,
...
Satisfiability simplifyL(ExprType* a,
                         ExprType* b,
                         int flex,
                         Node* node,
                         Hashtable* typingEnv,
...
Satisfiability narrow(ExprType* type,
                      ExprType* expected,
                      int flex,
                      Node* node,
                      Hashtable* typingEnv,
                      ...
                      Region* r)
{
    if (T_VAR_ID(type) == T_VAR_ID(expected)) {
        ...
        int nln, nrn;
        Node *cl[100], *cr[100];
        doNarrow(T_VAR_DISJUNCTS(type),
                 T_VAR_DISJUNCTS(expected),
                 T_VAR_NUM_DISJUNCTS(type),
                 T_VAR_NUM_DISJUNCTS(expected),
                 flex,
                 cl,
                 cr,
                 &nln,
...
Satisfiability splitConsOrTuple(ExprType* a,
                                ExprType* b,
                                int flex,
                                Node* node,
                                Hashtable* typingEnv,
                                ...
                                Region* r)
{
    ...
    if ((getNodeType(a) == T_CONS && strcmp(T_CONS_TYPE_NAME(a), T_CONS_TYPE_NAME(b)) != 0) ||
        ...
        for (i = 0; i < T_CONS_ARITY(a); i++) {
            ...
            Satisfiability sat =
                simplifyLocally(simpa, simpb, flex, node, typingEnv, equivalence, simpleTypingConstraints, r);
            switch (sat) {
                case ABSURDITY:
                    return ABSURDITY;
                case TAUTOLOGY:
...
int applyBaseTypeRule(ExprType* tca, ExprType* tcb, int flex)
{
    return (flex && tautologyLtBase(tca, tcb)) ||
           (!flex &&
            ((getNodeType(tca) == T_IRODS && getNodeType(tcb) == T_IRODS && (tca->text == NULL || tcb->text == NULL)) ||
...
Satisfiability simplifyLocally(ExprType* tca,
                               ExprType* tcb,
                               int flex,
                               Node* node,
                               Hashtable* typingEnv,
                               ...
                               Region* r)
{
    ...
    */
    ...
    if (getNodeType(tca) == T_UNSPECED || getNodeType(tca) == T_DYNAMIC ||
        getNodeType(tcb) == T_DYNAMIC) { /* is an undefined variable argument or a parameter with dynamic type */
        return TAUTOLOGY;
    }
        ...
        logicalAnd(simplifyLocally(tca, templa, flex, node, typingEnv, equivalence, simpleTypingConstraints, r),
                   simplifyLocally(templb, tcb, flex, node, typingEnv, equivalence, simpleTypingConstraints, r),
                   c);
        return c;
    }
 ...
 * 0 not solved
 */
Satisfiability solveConstraints(List* typingConstraints,
                                Hashtable* typingEnv,
                                rError_t* errmsg,
                                Node** errnode,
                                Region* r)
    ...
    do {
        ...
        while (nextNode != NULL && !changed) {
            ...
            */
            ...
            /*
                        printf("warning: collasping %s with %s.\n", typeToString(a, typingEnv, buf2, 1024),
               typeToString(b, typingEnv, buf3, 1024));
            */
...
Satisfiability simplify(List* typingConstraints, Hashtable* typingEnv, rError_t* errmsg, Node** errnode, Region* r)
{
    ...
    do {
        ...
        while (ln != NULL) {
            ...
            switch (simplifyLocally(
                TC_A(tc), TC_B(tc), 0, TC_NODE(tc), typingEnv, equivalence, simpleTypingConstraints, r)) {
                case TAUTOLOGY:
                    break;
                case CONTINGENCY:
                    /* printf("contingency\n"); */
                    ...
                    while(tcp!=NULL) {
                            printf("simplified %s<%s to %s<%s.\n",
                                            ...
                                            typeToString(tcp->a, NULL, buf1, 1024), typeToString(tcp->b, NULL, buf2,
                    1024)); tcp = tcp->next;
                    }*/
                    ret = CONTINGENCY;
                    break;
    ...
    switch (getNodeType(derefedType)) {
        case T_CONS:
            if (strcmp(type->text, LIST) == 0) { /* list */
                /* dereference element type as only top level vars are dereferenced by the dereference function and we
                 * are accessing a subtree of the type */
        ...
        case T_TUPLE:
            ...
            if (getNodeType(comp0) != T_IRODS || strcmp(T_CONS_TYPE_NAME(comp0), GenQueryInp_MS_T) != 0 ||
                getNodeType(comp1) != T_IRODS || strcmp(T_CONS_TYPE_NAME(comp1), GenQueryOut_MS_T) != 0) {
                return NULL;
            }
            return newIRODSType(KeyValPair_MS_T, r);
...
ExprType* typeFunction3(Node* node,
                        int dynamictyping,
                        Env* funcDesc,
                        Hashtable* var_type_table,
                        List* typingConstraints,
                        rError_t* errmsg,
                        Node** errnode,
                        Region* r)
{
    ...
    if (getNodeType(fn) == TK_TEXT && strcmp(fn->text, "foreach") == 0) {
        ...
        res3 = typeExpression3(
            arg->subtrees[1], dynamictyping, funcDesc, var_type_table, typingConstraints, errmsg, errnode, r);
        RE_ERROR2(getNodeType(res3) == T_ERROR, "foreach loop type error");
        res3 = typeExpression3(
            arg->subtrees[2], dynamictyping, funcDesc, var_type_table, typingConstraints, errmsg, errnode, r);
        ...
        ExprType* fType = getNodeType(fnType) == T_CONS && strcmp(fnType->text, FUNC) == 0
                              ? fnType
                              : unifyWith(fnType, newFuncType(newTVar(r), newTVar(r), r), var_type_table, r);

        RE_ERROR2(getNodeType(fType) == T_ERROR,
                  "the first component of a function application does not have a function type");
        ...
        RE_ERROR2(getNodeType(fn) == TK_TEXT && strcmp(fn->text, "assign") == 0 && arg->degree > 0 &&
                      !isPattern(arg->subtrees[0]),
                  "the first argument of microservice assign is not a variable or a pattern");
        RE_ERROR2(getNodeType(fn) == TK_TEXT && strcmp(fn->text, "let") == 0 && arg->degree > 0 &&
                      !isPattern(arg->subtrees[0]),
        ...
        */
        ...
        if (getVararg(fType) != OPTION_VARARG_ONCE) {
            ...
            if (copyN < (getVararg(fType) == OPTION_VARARG_PLUS ? 1 : 0) ||
                (getVararg(fType) == OPTION_VARARG_OPTIONAL && copyN > 1)) {
                snprintf(buf,
                         1024,
                         ...
                         getVararg(fType) == OPTION_VARARG_PLUS
                             ? "*"
                             : getVararg(fType) == OPTION_VARARG_OPTIONAL ? "?" : "+");
                RE_ERROR2(1, buf);
            }
...
int typeFuncParam(Node* param,
                  Node* paramType,
                  Node* formalParamType,
                  Hashtable* var_type_table,
                  List* typingConstraints,
                  rError_t* errmsg,
                  Region* r)
...
ExprType* typeTypeAscription(Node* expr,
                             int dynamictyping,
                             Env* funcDesc,
                             Hashtable* varTypes,
                             List* typingConstraints,
    ...
    if (t == NULL) {
        *errnode = expr;
        snprintf(errmsgbuf,
                 ...
                 typeToString(exprType, varTypes, typebuf, ERR_MSG_LEN),
                 typeToString(ascType, varTypes, typebuf2, ERR_MSG_LEN));
        generateErrMsg(errmsgbuf, NODE_EXPR_POS((*errnode)), (*errnode)->base, errbuf);
        addRErrorMsg(errmsg, RE_TYPE_ERROR, errbuf);
        return newSimpType(T_ERROR, r);
    }
...
ExprType* typeExpression3(Node* expr,
                          int dynamictyping,
                          Env* funcDesc,
                          Hashtable* varTypes,
                          List* typingConstraints,
    ...
    switch (getNodeType(expr)) {
        ...
        case TK_TEXT:
            if (strcmp(expr->text, "nop") == 0) {
                ...
                FunctionDesc* fDesc;
                if (funcDesc != NULL && (fDesc = (FunctionDesc*) lookupFromEnv(funcDesc, expr->text)) != NULL &&
                    fDesc->exprType != NULL) {
        ...
        case N_TUPLE:
            ...
            for (i = 0; i < expr->degree; i++) {
                components[i] = typeExpression3(
                    expr->subtrees[i], dynamictyping, funcDesc, varTypes, typingConstraints, errmsg, errnode, r);
        ...
        case N_ACTIONS_RECOVERY:
            res = typeExpression3(
                expr->subtrees[0], dynamictyping, funcDesc, varTypes, typingConstraints, errmsg, errnode, r);
            if (getNodeType(res) == T_ERROR) {
                return expr->exprType = res;
            }
            res = typeExpression3(
                expr->subtrees[1], dynamictyping, funcDesc, varTypes, typingConstraints, errmsg, errnode, r);
            return expr->exprType = res;
        case N_ATTR:
            /* todo type */
            for (i = 0; i < expr->degree; i++) {
        ...
        case N_EXTERN_DEF:
            return expr->exprType = typeTypeAscription(
                       expr, dynamictyping, funcDesc, varTypes, typingConstraints, errmsg, errnode, r);
        default:
            break;
    }
    *errnode = expr;
 ...
 * If the type coerced to cannot be inferred, i.e., an expression is to be coerced to some tvar, bounded to a union type
 * or free, then we leave further type checking to runtime, which can be done locally under the assumption of
 * indistinguishable inclusion.
 *
 */
