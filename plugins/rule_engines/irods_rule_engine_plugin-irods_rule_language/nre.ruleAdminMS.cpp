...
int _admShowDVM(ruleExecInfo_t* rei, rulevardef_t* inRuleVarDef, int inx)
{
    ...
    for (j = 0; j < inRuleVarDef->MaxNumOfDVars; j++) {
        sprintf(outStr,
                " %-5i %-15.15s %s ===> %s\n",
                j + inx,
                inRuleVarDef->action[j],
                inRuleVarDef->varName[j],
                inRuleVarDef->var2CMap[j]);
...
/**
 * \fn msiAdmReadDVMapsFromFileIntoStruct(msParam_t *inDvmFileNameParam, msParam_t *outCoreDVMapStruct, ruleExecInfo_t
 **rei)
 *
 ...
 **/
int msiAdmReadDVMapsFromFileIntoStruct(msParam_t* inDvmFileNameParam,
                                       msParam_t* outCoreDVMapStruct,
                                       ruleExecInfo_t* rei)
{
    int i;
    ...
    if (inDvmFileNameParam == NULL || strcmp(inDvmFileNameParam->type, STR_MS_T) != 0 ||
        inDvmFileNameParam->inOutStruct == NULL || strlen((char*) inDvmFileNameParam->inOutStruct) == 0) {
        return PARAOPR_EMPTY_IN_STRUCT_ERR;
    }
    if (outCoreDVMapStruct->type != NULL && strcmp(outCoreDVMapStruct->type, DVMapStruct_MS_T) == 0 &&
...
/**
 * \fn msiAdmInsertDVMapsFromStructIntoDB(msParam_t *inDvmBaseNameParam, msParam_t *inCoreDVMapStruct, ruleExecInfo_t
 **rei)
 *
...
int msiAdmInsertDVMapsFromStructIntoDB(msParam_t* inDvmBaseNameParam, msParam_t* inCoreDVMapStruct, ruleExecInfo_t* rei)
{
    ...
    if (inDvmBaseNameParam == NULL || inCoreDVMapStruct == NULL || strcmp(inDvmBaseNameParam->type, STR_MS_T) != 0 ||
        strcmp(inCoreDVMapStruct->type, DVMapStruct_MS_T) != 0 || inDvmBaseNameParam->inOutStruct == NULL ||
        inCoreDVMapStruct->inOutStruct == NULL || strlen((char*) inDvmBaseNameParam->inOutStruct) == 0) {
        return PARAOPR_EMPTY_IN_STRUCT_ERR;
    }
...
/**
 * \fn msiGetDVMapsFromDBIntoStruct(msParam_t *inDvmBaseNameParam, msParam_t *inVersionParam, msParam_t
 **outCoreDVMapStruct, ruleExecInfo_t *rei)
 *
 ...
 * \note This microservice requires iRODS administration privileges.
 *
 * \note  This is a microservice that queries the iCAT for DVM with a given base name and version number and populates a
 *DVM rule structure.
 *
 * \usage See clients/icommands/test/rules/
 *
 * \param[in] inDvmBaseNameParam - a msParam of type STR_MS_T, the name of the base being queried
 * \param[in] inVersionParam - a msParam of type STR_MS_T, which is the version string of the base being queried (use 0
 *for current version) \param[in] outCoreDVMapStruct - a msParam of type RuleStruct_MS_T \param[in,out] rei - The
 *RuleExecInfo structure that is automatically handled by the rule engine. The user does not include rei as a parameter
 *in the rule invocation.
 *
 * \DolVarDependence none
 * \DolVarModified none
 * \iCatAttrDependence none
 ...
 **/
int msiGetDVMapsFromDBIntoStruct(msParam_t* inDvmBaseNameParam,
                                 msParam_t* inVersionParam,
                                 msParam_t* outCoreDVMapStruct,
                                 ruleExecInfo_t* rei)
{
    int i;
    ...
    if (inDvmBaseNameParam == NULL || strcmp(inDvmBaseNameParam->type, STR_MS_T) != 0 ||
        inDvmBaseNameParam->inOutStruct == NULL || strlen((char*) inDvmBaseNameParam->inOutStruct) == 0) {
        return PARAOPR_EMPTY_IN_STRUCT_ERR;
    }
    if (inVersionParam == NULL || strcmp(inVersionParam->type, STR_MS_T) != 0 || inVersionParam->inOutStruct == NULL ||
    ...
    i = readDVMapStructFromDB(
        (char*) inDvmBaseNameParam->inOutStruct, (char*) inVersionParam->inOutStruct, coreDVMapStruct, rei);
    if (i != 0) {
...
/**
 * \fn msiAdmWriteDVMapsFromStructIntoFile(msParam_t *inDvmFileNameParam, msParam_t *inCoreDVMapStruct, ruleExecInfo_t
 **rei)
 *
 ...
 **/
int msiAdmWriteDVMapsFromStructIntoFile(msParam_t* inDvmFileNameParam,
                                        ...
                                        ruleExecInfo_t* rei)
{
    ...
    if (inDvmFileNameParam == NULL || inCoreDVMapStruct == NULL || strcmp(inDvmFileNameParam->type, STR_MS_T) != 0 ||
        strcmp(inCoreDVMapStruct->type, DVMapStruct_MS_T) != 0 || inDvmFileNameParam->inOutStruct == NULL ||
...
/**
 * \fn msiAdmReadFNMapsFromFileIntoStruct(msParam_t *inFnmFileNameParam, msParam_t *outCoreFNMapStruct, ruleExecInfo_t
 **rei)
 *
 ...
 **/
int msiAdmReadFNMapsFromFileIntoStruct(msParam_t* inFnmFileNameParam,
                                       msParam_t* outCoreFNMapStruct,
                                       ruleExecInfo_t* rei)
{
    int i;
    ...
    if (inFnmFileNameParam == NULL || strcmp(inFnmFileNameParam->type, STR_MS_T) != 0 ||
        inFnmFileNameParam->inOutStruct == NULL || strlen((char*) inFnmFileNameParam->inOutStruct) == 0) {
        return PARAOPR_EMPTY_IN_STRUCT_ERR;
    }
    if (outCoreFNMapStruct->type != NULL && strcmp(outCoreFNMapStruct->type, FNMapStruct_MS_T) == 0 &&
...
/**
 * \fn msiAdmInsertFNMapsFromStructIntoDB(msParam_t *inFnmBaseNameParam, msParam_t *inCoreFNMapStruct, ruleExecInfo_t
 **rei)
 *
...
int msiAdmInsertFNMapsFromStructIntoDB(msParam_t* inFnmBaseNameParam, msParam_t* inCoreFNMapStruct, ruleExecInfo_t* rei)
{
    ...
    if (inFnmBaseNameParam == NULL || inCoreFNMapStruct == NULL || strcmp(inFnmBaseNameParam->type, STR_MS_T) != 0 ||
        strcmp(inCoreFNMapStruct->type, FNMapStruct_MS_T) != 0 || inFnmBaseNameParam->inOutStruct == NULL ||
        inCoreFNMapStruct->inOutStruct == NULL || strlen((char*) inFnmBaseNameParam->inOutStruct) == 0) {
        return PARAOPR_EMPTY_IN_STRUCT_ERR;
    }
...
/**
 * \fn msiGetFNMapsFromDBIntoStruct(msParam_t *inFnmBaseNameParam, msParam_t *inVersionParam, msParam_t
 **outCoreFNMapStruct, ruleExecInfo_t *rei)
 *
 ...
 * \note This microservice requires iRODS administration privileges.
 *
 * \note  This is a microservice that queries the iCAT for FNM with a given base name and version number and populates a
 *FNM rule structure.
 *
 * \usage See clients/icommands/test/rules/
 *
 ...
 * \param[in] inVersionParam - a msParam of type STR_MS_T, which is the version string of the base being queried (use 0
 *for current version) \param[in] outCoreFNMapStruct - a msParam of type RuleStruct_MS_T \param[in,out] rei - The
 *RuleExecInfo structure that is automatically handled by the rule engine. The user does not include rei as a parameter
 *in the rule invocation.
 *
 * \DolVarDependence none
 ...
 **/
int msiGetFNMapsFromDBIntoStruct(msParam_t* inFnmBaseNameParam,
                                 msParam_t* inVersionParam,
                                 msParam_t* outCoreFNMapStruct,
                                 ruleExecInfo_t* rei)
{
    ...
    if (inFnmBaseNameParam == NULL || strcmp(inFnmBaseNameParam->type, STR_MS_T) != 0 ||
        inFnmBaseNameParam->inOutStruct == NULL || strlen((char*) inFnmBaseNameParam->inOutStruct) == 0) {
        return PARAOPR_EMPTY_IN_STRUCT_ERR;
    }
    if (inVersionParam == NULL || strcmp(inVersionParam->type, STR_MS_T) != 0 || inVersionParam->inOutStruct == NULL ||
        strlen((char*) inVersionParam->inOutStruct) == 0) {
    ...
    i = readFNMapStructFromDB(
        (char*) inFnmBaseNameParam->inOutStruct, (char*) inVersionParam->inOutStruct, coreFNMapStrct, rei);
    if (i != 0) {
...
/**
 * \fn msiAdmWriteFNMapsFromStructIntoFile(msParam_t *inFnmFileNameParam, msParam_t *inCoreFNMapStruct, ruleExecInfo_t
 **rei)
 *
 ...
 **/
int msiAdmWriteFNMapsFromStructIntoFile(msParam_t* inFnmFileNameParam,
                                        ...
                                        ruleExecInfo_t* rei)
{
    ...
    if (inFnmFileNameParam == NULL || inCoreFNMapStruct == NULL || strcmp(inFnmFileNameParam->type, STR_MS_T) != 0 ||
        strcmp(inCoreFNMapStruct->type, FNMapStruct_MS_T) != 0 || inFnmFileNameParam->inOutStruct == NULL ||
...
/**
 * \fn msiAdmReadMSrvcsFromFileIntoStruct(msParam_t *inMsrvcFileNameParam, msParam_t *outCoreMsrvcStruct, ruleExecInfo_t
 **rei)
 *
 ...
 **/
int msiAdmReadMSrvcsFromFileIntoStruct(msParam_t* inMsrvcFileNameParam,
                                       msParam_t* outCoreMsrvcStruct,
                                       ruleExecInfo_t* rei)
{
    int i;
    ...
    if (inMsrvcFileNameParam == NULL || strcmp(inMsrvcFileNameParam->type, STR_MS_T) != 0 ||
        inMsrvcFileNameParam->inOutStruct == NULL || strlen((char*) inMsrvcFileNameParam->inOutStruct) == 0) {
        return PARAOPR_EMPTY_IN_STRUCT_ERR;
    }
    if (outCoreMsrvcStruct->type != NULL && strcmp(outCoreMsrvcStruct->type, MsrvcStruct_MS_T) == 0 &&
...
/**
 * \fn msiAdmInsertMSrvcsFromStructIntoDB(msParam_t *inMsrvcBaseNameParam, msParam_t *inCoreMsrvcStruct, ruleExecInfo_t
 **rei)
 *
...
int msiAdmInsertMSrvcsFromStructIntoDB(msParam_t*, msParam_t* inCoreMsrvcStruct, ruleExecInfo_t* rei)
{
    ...
    if (inCoreMsrvcStruct == NULL || strcmp(inCoreMsrvcStruct->type, MsrvcStruct_MS_T) != 0 ||
 ...
 * \note This microservice requires iRODS administration privileges.
 *
 * \note  This is a microservice that queries the iCAT for microservices with a given base name and version number and
 *populates a microservice rule structure.
 *
 * \usage See clients/icommands/test/rules/
 *
 * \param[in] inStatus - a msParam of type INT_MS_T or an integer stored in STR_MS_T, normally 1 if microservice is
 *available, 0 otherwise. \param[out] outCoreMsrvcStruct - a msParam of type MsrvcStruct_MS_T \param[in,out] rei - The
 *RuleExecInfo structure that is automatically handled by the rule engine. The user does not include rei as a parameter
 *in the rule invocation.
 *
 * \DolVarDependence none
...
/**
 * \fn msiAdmWriteMSrvcsFromStructIntoFile(msParam_t *inMsrvcFileNameParam, msParam_t *inCoreMsrvcStruct, ruleExecInfo_t
 **rei)
 *
 ...
 **/
int msiAdmWriteMSrvcsFromStructIntoFile(msParam_t* inMsrvcFileNameParam,
                                        msParam_t* inCoreMsrvcStruct,
                                        ruleExecInfo_t* rei)
{
    int i;
    ...
    if (inMsrvcFileNameParam == NULL || inCoreMsrvcStruct == NULL ||
        strcmp(inMsrvcFileNameParam->type, STR_MS_T) != 0 || strcmp(inCoreMsrvcStruct->type, MsrvcStruct_MS_T) != 0 ||
        inMsrvcFileNameParam->inOutStruct == NULL || strlen((char*) inMsrvcFileNameParam->inOutStruct) == 0) {
        return PARAOPR_EMPTY_IN_STRUCT_ERR;
    }
