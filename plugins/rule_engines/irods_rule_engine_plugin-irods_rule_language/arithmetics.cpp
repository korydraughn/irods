...
#define OUTOFMEMORY(x, res)                                                                                            \
    if (x) {                                                                                                           \
        (res)->value.e = OUT_OF_MEMORY;                                                                                \
        TYPE(res) = RE_ERROR;                                                                                          \
        return;                                                                                                        \
    }
...
Res* evaluateExpression3(Node* expr,
                         int applyAll,
                         int force,
                         ruleExecInfo_t* rei,
                         int reiSaveFlag,
    ...
    if (discardResult) {
        switch (getNodeType(expr)) {
            ...
            case N_APPLICATION:
                ...
                */
                funcRes = evaluateExpression3(expr->subtrees[0],
                                              applyAll > 1 ? applyAll : 0,
                                              0,
                                              rei,
                                              reiSaveFlag & ~DISCARD_EXPRESSION_RESULT,
                                              env,
                                              errmsg,
                                              r);
                ...
                argRes = evaluateExpression3(expr->subtrees[1],
                                             applyAll > 1 ? applyAll : 0,
                                             0,
                                             rei,
                                             reiSaveFlag & ~DISCARD_EXPRESSION_RESULT,
            ...
            case N_TUPLE:
                for (i = 0; i < expr->degree; i++) {
                    res = evaluateExpression3(
                        expr->subtrees[i], applyAll > 1 ? applyAll : 0, 0, rei, reiSaveFlag, env, errmsg, r);
            ...
            case N_EXTERN_DEF:
                res = evaluateExpression3(
                    expr->subtrees[0], applyAll > 1 ? applyAll : 0, 0, rei, reiSaveFlag, env, errmsg, r);
                break;
            default:
        ...
        switch (getNodeType(expr)) {
            ...
            case N_APPLICATION:
                ...
                */
                ...
                /* printTree(expr->subtrees[1], 0); */
                argRes = evaluateExpression3(
                    expr->subtrees[1], applyAll > 1 ? applyAll : 0, 0, rei, reiSaveFlag, env, errmsg, r);
                if (getNodeType(argRes) == N_ERROR) {
                    res = argRes;
                    break;
                }
            ...
            case N_TUPLE:
                ...
                for (i = 0; i < expr->degree; i++) {
                    res = tupleComps[i] = evaluateExpression3(
                        expr->subtrees[i], applyAll > 1 ? applyAll : 0, 0, rei, reiSaveFlag, env, errmsg, r);
                    if (getNodeType(res) == N_ERROR) {
                        break;
                    }
...
Res* processCoercion(Node* node, Res* res, ExprType* type, Hashtable* tvarEnv, rError_t* errmsg, Region* r)
{
    ...
    if (getNodeType(coercion) == T_VAR) {
        if (T_VAR_NUM_DISJUNCTS(coercion) == 0) {
            /* generateErrMsg("error: cannot instantiate coercion type for node.", NODE_EXPR_POS(node), node->base,
            buf); addRErrorMsg(errmsg, -1, buf); return newErrorRes(r, -1); */
            return res;
        }
                ...
                switch (TYPE(res)) {
                    case T_DOUBLE:
                        if ((int) RES_DOUBLE_VAL(res) != RES_DOUBLE_VAL(res)) {
                            generateErrMsg(
                                "error: dynamic type conversion DOUBLE -> INTEGER: the double is not an integer",
                                NODE_EXPR_POS(node),
                                node->base,
                                buf);
                            addRErrorMsg(errmsg, RE_DYNAMIC_COERCION_ERROR, buf);
                ...
                switch (TYPE(res)) {
                    ...
                    case T_STRING:
                        if (strcmp(res->text, "true") == 0) {
                            ...
                            generateErrMsg(
                                "error: dynamic type conversion  string -> bool: the string is not in {true, false}",
                                NODE_EXPR_POS(node),
                                node->base,
                                buf);
                            addRErrorMsg(errmsg, RE_DYNAMIC_COERCION_ERROR, buf);
    ...
    if (nres != NULL) {
        if (futureCoercion != NULL) {
            if (getNodeType(futureCoercion) != T_VAR || futureCoercion->degree == 0) {
                snprintf(buf,
                         ERR_MSG_LEN,
                         "error: flexible coercion target type supported only for union types, but is applied to %s",
            ...
            switch (TYPE(nres)) {
                ...
                default:
                    snprintf(buf,
                             ERR_MSG_LEN,
                             "error: flexible coercion source type supported only for integer or double, but is "
                             "applied to %s",
    ...
    snprintf(buf,
             ERR_MSG_LEN,
             "error: coerce from type %s to type %s",
    ...
    switch (getNodeType(expr)) {
        case N_ACTIONS:
            for (i = 0; i < expr->degree; i++) {
                ...
                if (getNodeType(nodei) == N_APPLICATION && getNodeType(nodei->subtrees[0]) == TK_TEXT &&
                    strcmp(nodei->subtrees[0]->text, "cut") == 0) {
                    cutFlag = 1;
                    continue;
                }
                res = evaluateExpression3(nodei,
                                          applyAll,
                                          0,
                                          rei,
                                          reiSaveFlag | (i == expr->degree - 1 ? 0 : DISCARD_EXPRESSION_RESULT),
                                          env,
                                          errmsg,
                                          r);
                if (getNodeType(res) == N_ERROR) {
                    ...
                    rodsLog(LOG_NOTICE,
                            "executeRuleBody: Microservice or Action %s Failed with status %i",
                            errAction,
                            RES_ERR_CODE(res));
#endif
                    /* run recovery chain */
...
#ifndef DEBUG
                                char* errAction = getNodeType(reco->subtrees[i2]) == N_APPLICATION
                                                      ? N_APP_FUNC(reco->subtrees[i2])->text
                                                      : reco->subtrees[i2]->text;
...
Res* evaluateFunctionApplication(Node* func,
                                 Node* arg,
                                 int applyAll,
                                 Node* node,
                                 ruleExecInfo_t* rei,
 ...
 */
Res* evaluateFunction3(Node* appRes,
                       int applyAll,
                       Node* node,
                       Env* env,
                       ruleExecInfo_t* rei,
                       ...
                       Region* r)
{
            ...
            case IO_TYPE_INPUT | IO_TYPE_OUTPUT: /* input/output */
                ...
                if (!isVariableNode(appArgs[i])) {
                    ...
                    generateAndAddErrMsg(
                        "unsupported output parameter type", appArgs[i], RE_UNSUPPORTED_AST_NODE_TYPE, errmsg);
                    RETURN;
                }
                args[i] = evaluateExpression3(appArgs[i],
                                              applyAll > 1 ? applyAll : 0,
                                              1,
                                              rei,
                                              reiSaveFlag & ~DISCARD_EXPRESSION_RESULT,
    ...
    if (coercionType != NULL) {
        ...
        if (typeFuncParam(
                node->subtrees[1], argType, coercionType, env->current, localTypingConstraints, errmsg, newRegion) !=
            0) {
        ...
        if (discardResult)
            switch (getNodeType(fd)) {
                ...
                case N_FD_CONSTRUCTOR:
                    ...
                    break;
                case N_FD_FUNCTION:
                    res = (Res*) FD_SMSI_FUNC_PTR(fd)(
                        &argsProcessed[0], n, node, rei, reiSaveFlag, env, errmsg, newRegion);
                    if (getNodeType(res) != N_ERROR) {
                        switch (TYPE(res)) {
                            case T_BREAK:
                            case T_SUCCESS:
                ...
                case N_FD_EXTERNAL:
                    res = execAction3(
                        fn, &argsProcessed[0], n, applyAll, node, nEnv, rei, reiSaveFlag, errmsg, newRegion);
                    break;
    ...
    for (i = 0; i < n; i++) {
        ...
        if ((ioParam[i] & IO_TYPE_OUTPUT) == IO_TYPE_OUTPUT) {
            if ((appArgs[i]->option & OPTION_COERCE) != 0) {
                argsProcessed[i] = processCoercion(
                    nodeArgs[i], argsProcessed[i], appArgs[i]->exprType, env->current, errmsg, newRegion);
            }
            if (getNodeType(argsProcessed[i]) == N_ERROR) {
                res = (Res*) argsProcessed[i];
                RETURN;
 ...
 */
Res* execAction3(char* actionName,
                 Res** args,
                 unsigned int nargs,
                 int applyAllRule,
                 Node* node,
                 ...
                 Region* r)
{
    ...
    if (getNodeType(actionRet) == N_ERROR && (RES_ERR_CODE(actionRet) == NO_RULE_FOUND_ERR)) {
        ...
        // if redirecting to framework with something besides strings and expecting the call to eventually be handled by
        // a microservice, the code that converts REPF arguments back to msParams will need to be updated to handle the
        // new types
        ...
        if (safe_to_redirect_to_re_framework) {
            ...
            if (actionInx >= 0) { /* rule */
                ...
                snprintf(buf,
                         ERR_MSG_LEN,
                         "error: cannot find rule for action \"%s\" available: %d.",
                         action,
                         availableRules());
                generateErrMsg(buf, NODE_EXPR_POS(node), node->base, buf2);
 ...
 */
Res* execMicroService3(char* msName,
                       Res** args,
                       unsigned int nargs,
                       Node* node,
                       Env* env,
                       ...
                       Region* r)
{
    ...
    for (unsigned int i = 0; i < numOfStrArgs; i++) {
        ...
        Res* res = args[i];
        if (res != NULL) {
            int ret = convertResToMsParam(myArgv[i], res, errmsg);
            if (ret != 0) {
                generateErrMsg("execMicroService3: error converting arguments to MsParam",
                               NODE_EXPR_POS(node->subtrees[1]->subtrees[i]),
                               node->subtrees[1]->subtrees[i]->base,
                               errbuf);
                addRErrorMsg(errmsg, ret, errbuf);
                for (int j = i - 1; j >= 0; j--) {
                    int freeStruct = 1;
            ...
            myArgv[i]->label = fillInParamLabel && isVariableNode(node->subtrees[1]->subtrees[i])
                                   ? strdup(node->subtrees[1]->subtrees[i]->text)
                                   : NULL;
        }
        else {
    ...
    for (unsigned int i = 0; i < numOfStrArgs; i++) {
        if (myArgv[i] != NULL) {
            ...
            if (res != NULL && getNodeType(res) == N_ERROR) {
                generateErrMsg("execMicroService3: error converting arguments from MsParam",
                               NODE_EXPR_POS(node),
                               node->base,
                               errbuf);
...
Res* execRuleFromCondIndex(char* ruleName,
                           Res** args,
                           int argc,
                           CondIndexVal* civ,
                           int applyAll,
                           ...
                           Region* r)
{
    ...
    if (civ->params->degree != argc) {
        ...
        snprintf(buf,
                 ERR_MSG_LEN,
                 "error: cannot apply rule %s from rule conditional index because of wrong number of arguments, "
                 "declared %d, supplied %d.",
                 ruleName,
                 civ->params->degree,
                 argc);
    ...
    if (TYPE(res) != T_STRING) {
        /* todo try coercion */
        addRErrorMsg(
            errmsg, RE_DYNAMIC_TYPE_ERROR, "error: the lhs of indexed rule condition does not evaluate to a string");
    ...
    rd = getRuleDesc(
        indexNode
            ->ruleIndex); /* increase the index to move it to core rules index below MAX_NUM_APP_RULES are app rules */
    ...
    if (getNodeType(status) == N_ERROR) {
        rodsLog(
            LOG_NOTICE, "execRuleFromCondIndex: applyRule Failed: %s with status %i", ruleName, RES_ERR_CODE(status));
    }
              ...
              Region* r)
{
    ...
    if (strlen(ruleNameInp) < sizeof(ruleName)) {
        ...
        rodsLog(
            LOG_ERROR, "ruleName: [%s] must be fewer than %ju characters in length.", ruleNameInp, sizeof(ruleName));
        ruleName[0] = '\0';
    }

    mapExternalFuncToInternalProc2(ruleName);

    int systemSpaceRuleFlag = (reiSaveFlag & SYSTEM_SPACE_RULE) != 0 ||
                                      lookupFromHashTable(ruleEngineConfig.coreFuncDescIndex->current, ruleName) != NULL
                                  ? SYSTEM_SPACE_RULE
                                  : 0;
...
//                rodsLog (LOG_NOTICE,"execRule: no more rules: %s with status %i",ruleName, statusI);
        ...
        if (ruleIndexListNode->secondaryIndex) {
            statusRes = execRuleFromCondIndex(ruleName,
                                              args,
                                              argc,
                                              ...
                                              env,
                                              rei,
                                              reiSaveFlag | systemSpaceRuleFlag,
                                              errmsg,
                                              r);
        }
        else {
            ...
            if (rd->ruleType == RK_REL || rd->ruleType == RK_FUNC) {
                ...
                if (reTestFlag > 0) {
                    if (reTestFlag == COMMAND_TEST_1) {
                        ...
                        fprintf(stdout,
                                "+Testing Rule Number:<FONT COLOR=#FF0000>%i</FONT> for Action:<FONT "
                                "COLOR=#0000FF>%s</FONT><BR>\n",
                                ruleInx,
                                ruleName);
                    }
                ...
                statusRes = execRuleNodeRes(rule,
                                            args,
                                            argc,
                                            applyAllRule > 1 ? applyAllRule : 0,
                                            env,
                                            rei,
                                            reiSaveFlag | systemSpaceRuleFlag,
                                            errmsg,
                     ...
                     Region* r)
{
    ...
    /* printEnvToStdOut(envNew->current); */
    Res* res =
        evaluateExpression3(ruleCondition, 0, 0, rei, reiSaveFlag & ~DISCARD_EXPRESSION_RESULT, envNew, errmsg, rNew);
    ...
    if (getNodeType(res) != N_ERROR && TYPE(res) == T_BOOL && RES_BOOL_VAL(res) != 0) {
...
#ifndef DEBUG
            rodsLog(LOG_NOTICE,
                    "execRuleNodeRes: applyRule Failed: %s with status %i",
                    ruleHead->text,
                    RES_ERR_CODE(statusRes));
#endif
        }
    }
    else {
        if (getNodeType(res) != N_ERROR && TYPE(res) != T_BOOL) {
            char buf[ERR_MSG_LEN];
            generateErrMsg("error: the rule condition does not evaluate to a boolean value",
                           NODE_EXPR_POS(ruleCondition),
                           ruleCondition->base,
                           buf);
...
Res* matchPattern(Node* pattern, Node* val, Env* env, ruleExecInfo_t* rei, int reiSaveFlag, rError_t* errmsg, Region* r)
{
    ...
    if (getNodeType(pattern) == N_APPLICATION && pattern->subtrees[1]->degree == 0) {
        ...
        if (findNextRule2(fn, 0, &node) == 0 && node->secondaryIndex == 0) {
            ...
            if (rd->ruleType == RK_FUNC &&
                (getNodeType(rd->node->subtrees[2]) == TK_BOOL || getNodeType(rd->node->subtrees[2]) == TK_STRING ||
                 getNodeType(rd->node->subtrees[2]) == TK_INT || getNodeType(rd->node->subtrees[2]) == TK_DOUBLE)) {
    ...
    switch (getNodeType(pattern)) {
        case N_APPLICATION:
            if (strcmp(N_APP_FUNC(pattern)->text, ".") == 0) {
                ...
                if (getNodeType(N_APP_ARG(pattern, 1)) == N_APPLICATION && N_APP_ARITY(N_APP_ARG(pattern, 1)) == 0) {
                    ...
                    Res* res = evaluateExpression3(
                        N_APP_ARG(pattern, 1), 0, 1, rei, reiSaveFlag & ~DISCARD_EXPRESSION_RESULT, env, errmsg, r);
                ...
                if (getNodeType(N_APP_ARG(pattern, 0)) == TK_VAR && varName[0] == '*' &&
                    ((res = (Res*) lookupFromEnv(env, varName)) == NULL ||
                     TYPE(res) == T_UNSPECED)) { /* if local var is empty then create new kvp */
                    ...
                    if (res != NULL) {
                        ...
                        if (insertIntoHashTable(env->current, varName, res2) == 0) {
                            ...
                            snprintf(localErrorMsg,
                                     ERR_MSG_LEN,
                                     "error: unable to write to local variable \"%s\".",
                                     varName);
                            generateErrMsg(localErrorMsg,
                                           NODE_EXPR_POS(N_APP_ARG(pattern, 0)),
                                           N_APP_ARG(pattern, 0)->base,
                                           errbuf);
                    ...
                    res = evaluateExpression3(N_APP_ARG(pattern, 0),
                                              0,
                                              0,
                                              rei,
                                              reiSaveFlag & ~DISCARD_EXPRESSION_RESULT,
                                              ...
                                              r); /* kvp */
                    CASCADE_N_ERROR(res);
                    RE_ERROR2(TYPE(res) != T_IRODS || strcmp(res->exprType->text, KeyValPair_MS_T) != 0,
                              "not a KeyValPair.");
                }
                ...
                if (findNextRule2(matcherName, 0, &node) == 0) {
                    ...
                    RE_ERROR2(v->text == NULL || strcmp(pattern->subtrees[0]->text, v->text) != 0,
                              "pattern mismatch constructor");
            ...
            if (varName[0] == '*') {
                if (lookupFromEnv(env, varName) == NULL) {
                    ...
                    if (insertIntoHashTable(env->current, varName, val) == 0) {
                        char localErrorMsg[ERR_MSG_LEN];
                        snprintf(
                            localErrorMsg, ERR_MSG_LEN, "error: unable to write to local variable \"%s\".", varName);
        ...
        case TK_BOOL:
            ...
            return newIntRes(r, 0);
        case TK_INT:
            RE_ERROR2(getNodeType(v) != N_VAL || (TYPE(v) != T_INT && TYPE(v) != T_DOUBLE),
                      "pattern mismatch value is not an integer.");
            res = evaluateExpression3(pattern, 0, 1, rei, reiSaveFlag & ~DISCARD_EXPRESSION_RESULT, env, errmsg, r);
            CASCADE_N_ERROR(res);
            RE_ERROR2(RES_INT_VAL(res) != (TYPE(v) == T_INT ? RES_INT_VAL(v) : RES_DOUBLE_VAL(v)),
                      "pattern mismatch integer.");
            return newIntRes(r, 0);
        case TK_DOUBLE:
            RE_ERROR2(getNodeType(v) != N_VAL || (TYPE(v) != T_DOUBLE && TYPE(v) != T_INT),
                      "pattern mismatch value is not a double.");
            res = evaluateExpression3(pattern, 0, 1, rei, reiSaveFlag & ~DISCARD_EXPRESSION_RESULT, env, errmsg, r);
            CASCADE_N_ERROR(res);
            RE_ERROR2(RES_DOUBLE_VAL(res) != (TYPE(v) == T_DOUBLE ? RES_DOUBLE_VAL(v) : RES_INT_VAL(v)),
                      "pattern mismatch integer.");
            return newIntRes(r, 0);
        default:
