...
int getVarMap(char* action, char* inVarName, char** varMap, int index)
{
    ...
    if (index < 1000) {
        for (i = index; i < appRuleVarDef.MaxNumOfDVars; i++) {
            if (!strcmp(appRuleVarDef.varName[i], varName)) {
                if (strlen(appRuleVarDef.action[i]) == 0 || strstr(appRuleVarDef.action[i], action) != NULL) {
    ...
    for (; i < coreRuleVarDef.MaxNumOfDVars; i++) {
        if (!strcmp(coreRuleVarDef.varName[i], varName)) {
            if (strlen(coreRuleVarDef.action[i]) == 0 || strstr(coreRuleVarDef.action[i], action) != NULL) {
...
int getAllSessionVarValue(ruleExecInfo_t* rei, keyValPair_t* varKeyVal)
{
    ...
    if (varKeyVal == NULL || rei == NULL) {
        rodsLog(LOG_ERROR, "getAllSessionVarValue: input rei or varKeyVal is NULL");
        return SYS_INTERNAL_NULL_INPUT_ERR;
    }

    for (i = 0; i < coreRuleVarDef.MaxNumOfDVars; i++) {
        if (lastVar == NULL || strcmp(lastVar, coreRuleVarDef.varName[i]) != 0) {
            status = getSessionVarValue("", coreRuleVarDef.varName[i], rei, &varValue);
