...
int coercible(ExprType* a, ExprType* b)
{
    return (getNodeType(a) != T_CONS && getNodeType(a) == getNodeType(b)) ||
           ...
           (getNodeType(b) == T_DATETIME && getNodeType(a) == T_DOUBLE) || (getNodeType(b) == T_DYNAMIC) ||
           (getNodeType(a) == T_DYNAMIC) ||
           (getNodeType(a) == T_CONS && getNodeType(b) == T_CONS &&
            coercible(T_CONS_TYPE_ARG(a, 0), T_CONS_TYPE_ARG(b, 0)));
}
/*
...
ExprType* unifyTVarL(ExprType* type, ExprType* expected, Hashtable* varTypes, Region* r)
{
    ...
    if (T_VAR_NUM_DISJUNCTS(type) == 0) { /* free */
        ...
        for (i = 0; i < T_VAR_NUM_DISJUNCTS(type); i++) {
            if (getNodeType(T_VAR_DISJUNCT(type, i)) ==
                getNodeType(expected)) { /* union types can only include primitive types */
                ty = expected;
                break;
            }
        }
...
ExprType* unifyTVarR(ExprType* type, ExprType* expected, Hashtable* varTypes, Region* r)
{
    ...
    if (T_VAR_NUM_DISJUNCTS(expected) == 0) { /* free */
        ...
        for (i = 0; i < T_VAR_NUM_DISJUNCTS(expected); i++) {
            if (getNodeType(type) ==
                getNodeType(T_VAR_DISJUNCT(expected, i))) { /* union types can only include primitive types */
                ty = type;
            }
        }
        if (ty != NULL) {
    ...
    if (getNodeType(type) == T_CONS && getNodeType(expected) == T_CONS) {
        if (strcmp(T_CONS_TYPE_NAME(type), T_CONS_TYPE_NAME(expected)) == 0 &&
            ...
            for (i = 0; i < T_CONS_ARITY(type); i++) {
                ExprType* elemType = unifyWith(T_CONS_TYPE_ARG(type, i),
                                               T_CONS_TYPE_ARG(expected, i),
                                               varTypes,
                                               r); /* unifyWithCoercion performs dereference */
                if (elemType == NULL) {
                    return NULL;
                }
                subtrees[i] = elemType;
            ...
            return dereference(
                newConsType(T_CONS_ARITY(expected), T_CONS_TYPE_NAME(expected), subtrees, r), varTypes, r);
        }
        else {
            return NULL;
        }
...
/* Res *cpRes2(Res *res0, Region *oldr, Region *r) {
    ...
    if(res->param!= NULL && !(IN_REGION(res->param->type, r) && IN_REGION(res->param, r))) {
        res->param = newMsParam(cpString2(res->param->type, oldr, r), res->param->inOutStruct, res->param->inpOutBuf,
r);
    }
