...
#define TRAVERSE_END(T) return ptr;

#define TRAVERSE_PTR(fn, f) CASCADE_NULL(ptr->f = fn(ptr->f, oldr, r));
...
#define TRAVERSE_PTR_TAPP(fn, f, cpfn) CASCADE_NULL(ptr->f = fn(ptr->f, oldr, r, cpfn));
...
#define COPY_ONLY_FROM_REGION2(f) if (IN_REGION(ptr->f, oldr))
