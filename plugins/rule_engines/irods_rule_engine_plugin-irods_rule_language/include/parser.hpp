...
#define NEXT_TOKEN(ext)                                                                                                \
    {                                                                                                                  \
        ...
        if (token->type == N_ERROR) {                                                                                  \
            context->error = 1;                                                                                        \
            if (pos.exprloc > context->errloc.exprloc)                                                                 \
                context->errloc = pos;                                                                                 \
            break;                                                                                                     \
        }                                                                                                              \
    }
...
#define UPDATE_ERR_LOC                                                                                                 \
    if (FPOS->exprloc > context->errloc.exprloc) {                                                                     \
        context->errloc = *FPOS;                                                                                       \
    }
#define CASCADE(x)                                                                                                     \
    {                                                                                                                  \
...
#define BUILD_NODE(type, cons, loc, deg, consume)                                                                      \
    if (context->error == 0) {                                                                                         \
        Node* var = newNode((type), (cons), (loc), context->region);                                                   \
        if (deg != 0) {                                                                                                \
...
#define UNZIP(n)                                                                                                       \
    {                                                                                                                  \
        ...
        for (i = 0; i < n; i++) {                                                                                      \
            ...
            context->nodeStack[context->nodeStackTop - 2 * n + i] =                                                    \
                context->nodeStack[context->nodeStackTop - 2 * n + 2 * i];                                             \
        }                                                                                                              \
        for (i = 0; i < n; i++) {                                                                                      \
...
#define TTEXT3(x, y, z)                                                                                                \
    NEXT_TOKEN(0);                                                                                                     \
    if (!((TOKEN_TYPE(TK_TEXT) || TOKEN_TYPE(TK_OP) || TOKEN_TYPE(TK_MISC_OP)) &&                                      \
          (TOKEN_TEXT(x) || TOKEN_TEXT(y) || TOKEN_TEXT(z)))) {                                                        \
        context->error = 1;                                                                                            \
        if (pos.exprloc > context->errloc.exprloc)                                                                     \
            context->errloc = pos;                                                                                     \
        break;                                                                                                         \
...
/**
 * skip a token of type TK_TEXT, TK_OP, or TK_MISC_OP and text text, token will has type N_ERROR if the token does not
 * match
 */
