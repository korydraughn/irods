...
#define T_FUNC_PARAM_TYPE_VARARG(x, n)                                                                                 \
    (n < T_FUNC_ARITY(x) ? T_FUNC_PARAM_TYPE(x, n) : T_FUNC_PARAM_TYPE(x, T_FUNC_ARITY(x) - 1))
...
struct node
{
    ...
    int ival;
    /* when this node represents a type or a pattern, this field indicates whether the trailing subtree represents
     * varargs */
