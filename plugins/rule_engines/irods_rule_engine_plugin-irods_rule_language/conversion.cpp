...
{
    if (var->type != NULL &&
        (strcmp(var->type, INT_MS_T) == 0 || strcmp(var->type, DOUBLE_MS_T) == 0 || strcmp(var->type, STR_MS_T) == 0)) {
        ...
        while (b != NULL && !IS_TVAR_NAME(b->key)) {
            ...
            if (v == NULL) {
                ...
                if (var->msParam == NULL) {
                    ...
                    var->msParam =
                        (msParam_t**) realloc(var->msParam, sizeof(msParam_t*) * (PTR_ARRAY_MALLOC_LEN + var->len));
                }
                ...
                case T_IRODS:
                    ...
                    if (strcmp(type, KeyValPair_MS_T) == 0) {
                        ...
                        if (j < 0) {
                            ...
                            snprintf(res + strlen(res),
                                     1024 - strlen(res),
                                     "COPY(%s,%s)",
                                     myDataObjCopyInp->srcDataObjInp.objPath,
                                     myDataObjCopyInp->destDataObjInp.objPath);
                    }
                    else if (strcmp(type, DataObjReadInp_MS_T) == 0 || strcmp(type, DataObjCloseInp_MS_T) == 0 ||
    ...
    for (i = 0; i < env->size; i++) {
        ...
        while (b != NULL) {
            ...
            if (res == NULL) {
                ...
                strncat(buf2, ":", MAX_NAME_LEN - strlen(buf2));
                strncat(buf2,
                        res->exprType == NULL ? "<null>" : typeToString(res->exprType, NULL, typeNameBuf, 128),
                        MAX_NAME_LEN - strlen(buf2));
                free(buf4);
            }
            k++;
