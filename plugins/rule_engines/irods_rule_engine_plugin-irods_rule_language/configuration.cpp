...
Cache::Cache()
    ...
    extRuleSet(NULL)
    , /* RuleSet *extRuleSet */
    sysFuncDescIndex(NULL)
    , /* Env *sysFuncDescIndex */
    coreFuncDescIndex(NULL)
    , /* Env *coreFuncDescIndex */
    appFuncDescIndex(NULL)
    , /* Env *appFuncDescIndex */
    extFuncDescIndex(NULL)
    ...
    clearDelayed(0)
    , /* int clearDelayed */
    timestamp(time_type_initializer)
    , /* time_type timestamp */
    logging(0)
    , /* int logging */
    ruleBase("")
...
{
    FunctionDesc* fd =
        (FunctionDesc*) lookupFromHashTable(ruleEngineConfig.extFuncDescIndex->current, RULE_NAME(rule->node));
    RuleIndexList* rd;
    if (fd == NULL) {
        ...
        if (getNodeType(fd) == N_FD_RULE_INDEX_LIST) {
            ...
            updateInHashTable(ruleEngineConfig.extFuncDescIndex->current,
                              RULE_NAME(rule->node),
                              newRuleIndexListFD(newRuleIndexList(RULE_NAME(rule->node), i, r), fd->exprType, r));
        }
        else {
            /* todo error handling */
        }
...
void prependRuleIntoAppIndex(RuleDesc* rule, int i, Region* r)
{
    RuleIndexList* rd;
    FunctionDesc* fd =
        (FunctionDesc*) lookupFromHashTable(ruleEngineConfig.appFuncDescIndex->current, RULE_NAME(rule->node));
    if (fd == NULL) {
        rd = newRuleIndexList(RULE_NAME(rule->node), i, r);
        fd = newRuleIndexListFD(rd, NULL, r);
...
{
    createRuleNodeIndex(ruleEngineConfig.coreRuleSet,
                        ruleEngineConfig.coreFuncDescIndex->current,
                        CORE_RULE_INDEX_OFF,
...
{
    createRuleNodeIndex(ruleEngineConfig.appRuleSet,
                        ruleEngineConfig.appFuncDescIndex->current,
                        APP_RULE_INDEX_OFF,
                        ruleEngineConfig.appRegion);
    return 0;
}
...
int hash_rules(const std::vector<std::string>& irbs, const int pid, std::string& digest)
{
    ...
    if (!ret.ok()) {
        status = ret.code();
        rodsLogError(LOG_ERROR, status, "hash_rules: cannot get hasher, status = %d", status);
        return status;
    }

    for (auto const& irb : irbs) {
        auto fn = get_rule_base_path_copy(irb, pid);
        std::ifstream in_file(fn, std::ios::in | std::ios::binary);
        if (!in_file.is_open()) {
            status = UNIX_FILE_OPEN_ERR - errno;
            rodsLogError(LOG_ERROR, status, "hash_rules: fopen failed for %s, status = %d", fn.c_str(), status);
            return status;
        }
        ...
        if (in_file.eof()) {
            ...
            rodsLogError(LOG_ERROR, status, "hash_rules: read failed for %s, status = %d", fn.c_str(), status);
            return status;
        }
    }
...
int readRuleStructAndRuleSetFromFile(const char* ruleBaseName, const char* rulesBaseFile)
{
    ...
    int res = 0;
    if ((res = readRuleSetFromLocalFile(ruleBaseName,
                                        rulesBaseFile,
...
int availableRules()
{
    return (isComponentInitialized(ruleEngineConfig.appRuleSetStatus) ? ruleEngineConfig.coreRuleSet->len : 0) +
           (isComponentInitialized(ruleEngineConfig.appRuleSetStatus) == INITIALIZED ? ruleEngineConfig.appRuleSet->len
                                                                                     : 0);
}
