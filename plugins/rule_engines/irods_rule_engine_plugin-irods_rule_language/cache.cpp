 ...
 * All sub structure status are either uninitialized or compressed, which meaning that they are not allocated separately
 * and therefore should not be deallocated. Only coreRuleSet and coreFuncDescIndex are copied. Starting from *p+size
 * backwards, a list of pointers to pointers in the copied data structures are stored. There pointers are used to
 * quickly access all pointers in the stored data structures so that they can be offset when they are copied to a new
 * memory address. This function returns NULL if it runs out of memory allocated between in *p and *p+size. The rule
 * engine status is also set to uninitialized. */
Cache* copyCache(unsigned char** p, size_t size, Cache* ptr)
{
    ...
    // With an average compiled object size of 75 bytes (taken over sample of 9000 lines of rule code), SHMMAX / 75 =
    // 400000 objects. The maximum size of the hash table should be twice the objects that can fit in shared memory
    // (400000 * 2 = 800000). The ratio of the shared memory region size to the maximum hash table size is 30000000 /
    // 800000 = 37.5 Since the calculations are based on an average size, the ratio is rounded to 40 to avoid imbuing
    // too much significance. The size of the hash table for a cache of the passed-in size, then, should be the size of
    // the cache divided by the ratio. hashtable size upper limit == cache size upper limit / 40 == 30000000 / 40 <
    // 800000
    ...
    ecopy->coreFuncDescIndexStatus =
        COMPRESSED; /* The coreFuncDescIndex is stored in a continuously allocated memory block in *buf */
 ...
 * 		   else return.
 * 	   except when the processType is RULE_ENGINE_INIT_CACHE, which means that the share caches has not been
 * initialized. or when the processType is RULE_ENGINE_REFRESH_CACHE, which means that we want to refresh the cache with
 * the new cache. It prepares the data in the new cache for copying into the shared cache. It checks the timestamp again
 * and does the actually copying.
 */
int updateCache(const char* _inst_name, size_t size, Cache* cache)
{
    ...
    if (buf != NULL) {
        ...
        rodsLog(LOG_ERROR,
                "Cannot update cache because of out of memory error, let some other process update it later when "
                "memory is available.");
        return -1;
    }
}
