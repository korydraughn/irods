...
{
    return getNodeType(node) == TK_VAR && node->text[0] == '*';
}
...
{
    return isLocalVariableNode(node) || isSessionVariableNode(node);
}
#define nKeywords 19
char* keywords[nKeywords] = {"in",
                             "let",
                             "match",
                             "with",
                             "for",
...
/**
 * skip a token of type TK_TEXT, TK_OP, or TK_MISC_OP and text text, token will have type N_ERROR if the token does not
 * match return 0 failure non 0 success
 */
int skip(Pointer* e, char* text, Token** token, ParserContext* pc, int rulegen)
{
    *token = nextTokenRuleGen(e, pc, rulegen, 0);
    if (((*token)->type != TK_TEXT && (*token)->type != TK_OP && (*token)->type != TK_MISC_OP) ||
    ...
    while (1) {
        ...
        if (ch == -1) { /* reach the end of stream */
            ...
            if (ch == '{' || ch == '}' || ch == '[' || ch == ']' || ch == '(' || ch == ')' || ch == ',' || ch == '@' ||
                (ch == '|' && (!rulegen || lookAhead(e, 1) != '|')) || ch == ';' || ch == '?') {
                *(token->text) = ch;
                (token->text)[1] = '\0';
                ...
                if (rulegen && lookAhead(e, 1) == ':' && lookAhead(e, 2) == ':') {
                    token->text[0] = ch;
                    token->text[1] = lookAhead(e, 1);
                ...
                nextStringBase(
                    ...
                    '\\',
                    0,
                    token->vars); /* path can be used in a foreach loop or assignment, or as a function argument */
                token->type = TK_PATH;
                break;
            }
            ...
            for (i = 0; i < num_ops; i++) {
                ...
                if (strncmp(op, new_ops[i].string, len) == 0 && (!isalpha(new_ops[i].string[0]) || !isalnum(op[len]))) {
...
else if (TOKEN_TEXT("+") || TOKEN_TEXT("-") || TOKEN_TEXT("*") || TOKEN_TEXT("/") || TOKEN_TEXT("<") ||
        ...
        *value = ch;
        switch (mode) {
            case 1:
                ...
                else if ((ch == '*' || ch == '$') && isalpha(lookAhead(e, 1))) {
...
void actionsToString(char** p, int* s, int indent, Node* na, Node* nr)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (nr != NULL && i < nr->degree &&
            (getNodeType(nr->subtrees[i]) != N_APPLICATION || strcmp(nr->subtrees[i]->subtrees[0]->text, "nop") != 0)) {
            PRINT(p, s, "%s", ":::");
...
void ruleToString(char* buf, int size, RuleDesc* rd)
{
    ...
    switch (rd->ruleType) {
        case RK_REL:
            ...
            PRINT(p, s, "%s", " ");
            if (getNodeType(subt) != TK_BOOL || strcmp(subt->text, "true") != 0) {
                PRINT(p, s, "%s", "{\n");
...
{
    if (getNodeType(a) == getNodeType(b) && strcmp(a->text, b->text) == 0 && a->degree == b->degree) {
        int i;
...
int eqExprNodeSyntacticVarMapping(Node* a, Node* b, Hashtable* varMapping /* from a to b */)
{
    ...
    if (getNodeType(a) == TK_VAR && getNodeType(b) == TK_VAR &&
        (val = (char*) lookupFromHashTable(varMapping, a->text)) != NULL && strcmp(val, b->text) == 0) {
        return 1;
    }
    if (getNodeType(a) == getNodeType(b) && strcmp(a->text, b->text) == 0 && a->degree == b->degree) {
        int i;
            ...
            if (strcmp(node->text, "INDUCT") == 0) {
                ...
                for (k = 1; k < n; k++) {
                    if (lookupFromEnv(funcDescIndex, nodes[k]->subtrees[0]->text) != NULL) {
                        generateErrMsg("parseRuleSet: redefinition of constructor.",
                                       NODE_EXPR_POS(nodes[k]->subtrees[0]),
                                       nodes[k]->subtrees[0]->base,
                                       errbuf);
                        addRErrorMsg(errmsg, RE_TYPE_ERROR, errbuf);
                        deleteParserContext(pc);
                        return RE_TYPE_ERROR;
                    }
                    insertIntoHashTable(funcDescIndex->current,
                                        nodes[k]->subtrees[0]->text,
                                        newConstructorFD2(nodes[k]->subtrees[1], r));
                ...
                if (lookupFromEnv(funcDescIndex, nodes[0]->subtrees[0]->text) != NULL) {
                    generateErrMsg("parseRuleSet: redefinition of constructor.",
                                   NODE_EXPR_POS(nodes[0]->subtrees[0]),
                                   nodes[0]->subtrees[0]->base,
                                   errbuf);
                    addRErrorMsg(errmsg, RE_TYPE_ERROR, errbuf);
                    deleteParserContext(pc);
                    return RE_TYPE_ERROR;
                ...
                insertIntoHashTable(
                    funcDescIndex->current, nodes[0]->subtrees[0]->text, newConstructorFD2(nodes[0]->subtrees[1], r));
                ...
                if ((fd = (FunctionDesc*) lookupFromEnv(funcDescIndex, nodes[0]->subtrees[0]->text)) != NULL) {
                    generateErrMsg("parseRuleSet: redefinition of function.",
                                   NODE_EXPR_POS(nodes[0]->subtrees[0]),
                                   nodes[0]->subtrees[0]->base,
                                   errbuf);
                    addRErrorMsg(errmsg, RE_TYPE_ERROR, errbuf);
                ...
                insertIntoHashTable(
                    funcDescIndex->current, nodes[0]->subtrees[0]->text, newExternalFD(nodes[0]->subtrees[1], r));
...
char* generateErrMsg(char* msg, long errloc, char* ruleBaseName, char errmsg[ERR_MSG_LEN])
{
    ...
    switch (ruleBaseName[0]) {
        ...
        default:
            rodsLog(LOG_ERROR,
                    "generateErrMsg: ruleBaseName of unknown type: [%s] [%ji] [%s]",
                    msg,
                    static_cast<intmax_t>(errloc),
                    ruleBaseName);
