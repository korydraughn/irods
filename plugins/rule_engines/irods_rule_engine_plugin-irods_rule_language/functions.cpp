...
int msiGetMoreRows(msParam_t* genQueryInpParam,
...
ReIterableTableRow reIterableTable[NUM_RE_ITERABLE] = {
    {RE_ITERABLE_GEN_QUERY,
     {reIterable_genQuery_init, reIterable_genQuery_hasNext, reIterable_genQuery_next, reIterable_genQuery_finalize}},
    ...
    {RE_ITERABLE_GEN_QUERY_OUT,
     {reIterable_irods_init, reIterable_irods_hasNext, reIterable_irods_next, reIterable_irods_finalize}},
...
{
    Res* res =
        evaluateExpression3((Node*) params[0], 0, 1, rei, reiSaveFlag & ~DISCARD_EXPRESSION_RESULT, env, errmsg, r);
    if (getNodeType(res) == N_ERROR) {
        return res;
    }
    if (RES_BOOL_VAL(res) == 0) {
        return evaluateActions(
            wrapToActions(params[2], r), wrapToActions(params[4], r), 0, rei, reiSaveFlag, env, errmsg, r);
    }
    else {
        return evaluateActions(
            wrapToActions(params[1], r), wrapToActions(params[3], r), 0, rei, reiSaveFlag, env, errmsg, r);
    }
}
                    ...
                    Region* r)
{
    ...
    GC_BEGIN
    while (1) {
        cond = evaluateExpression3(
            (Node*) params[0], 0, 1, rei, reiSaveFlag & ~DISCARD_EXPRESSION_RESULT, env, errmsg, GC_REGION);
                  ...
                  Region* r)
{
    ...
    init =
        evaluateExpression3((Node*) params[0], 0, 1, rei, reiSaveFlag | DISCARD_EXPRESSION_RESULT, env, errmsg, rnew);
    ...
    while (1) {
        cond = evaluateExpression3(
            (Node*) params[1], 0, 1, rei, reiSaveFlag & ~DISCARD_EXPRESSION_RESULT, env, errmsg, GC_REGION);
        if (getNodeType(cond) == N_ERROR) {
            res = cond;
            break;
        }
        ...
        step = evaluateExpression3(
            (Node*) params[2], 0, 1, rei, reiSaveFlag & DISCARD_EXPRESSION_RESULT, env, errmsg, GC_REGION);
...
ReIterableType collType(Res* coll)
{
    ...
    else if (TYPE(coll) == T_TUPLE && coll->degree == 2 && TYPE(coll->subtrees[0]) == T_IRODS &&
             strcmp(coll->subtrees[0]->exprType->text, GenQueryInp_MS_T) == 0 && TYPE(coll->subtrees[1]) == T_IRODS &&
             strcmp(coll->subtrees[1]->exprType->text, GenQueryOut_MS_T) == 0) {
...
Res* reIterable_genQuery_next(ReIterableData* itrData, Region* r)
{
    ...
    Res* res = evaluateActions(itrData->subtrees[2],
                               itrData->subtrees[3],
                               0,
                               itrData->rei,
                               itrData->reiSaveFlag,
...
void reIterable_collection_init(ReIterableData* itrData, Region* r)
{
    ...
    if (itrData->rei == NULL || itrData->rei->rsComm == NULL) {
        generateAndAddErrMsg("msiCollectionSpider: input rei or rsComm is NULL.",
                             itrData->node,
    ...
    if (TYPE(itrData->subtrees[1]) == T_PATH) {
        collRes = smsi_collection(&itrData->subtrees[1],
                                  1,
                                  itrData->node->subtrees[1],
                                  itrData->rei,
                                  itrData->reiSaveFlag,
                                  itrData->env,
                                  itrData->errmsg,
    ...
    if (data->handleInx < 0) {
        ...
        snprintf(buf,
                 ERR_MSG_LEN,
                 "collectionSpider: rsOpenCollection of %s error. status = %d",
                 data->collInp->collName,
                 data->handleInx);
...
Res* reIterable_collection_next(ReIterableData* itrData, Region* r)
{
    ...
    updateInEnv(
        itrData->env, itrData->varName, newUninterpretedRes(r, DataObjInp_MS_T, (void*) data->dataObjInp, NULL));

    /* Run actionStr on our object */
    Res* ret = evaluateActions(itrData->subtrees[2],
                               itrData->subtrees[3],
                               0,
                               itrData->rei,
                               itrData->reiSaveFlag,
                       ...
                       Region* r)
{
    ...
    switch (ctype) {
        ...
        default:
            snprintf(errbuf,
                     ERR_MSG_LEN,
                     "Error occurred when trying to determine if type %s is iterable.",
                     typeName_Res(subtrees[1]));
        ...
        switch (getNodeType(subQueNode)) {
            case N_ATTR:
                ...
                if (column_inx < 0) {
                    snprintf(errmsgBuf,
                             ERR_MSG_LEN,
                             "Unable to get valid ICAT column index for %s.",
                             subQueNode->subtrees[0]->text);
            ...
            case N_QUERY_COND_JUNCTION:
                ...
                if (lookupFromHashTable(queCondHashTable, attr) != NULL) {
                    ...
                    snprintf(errmsgBuf,
                             ERR_MSG_LEN,
                             "Unsupported gen query format: multiple query conditions on one attribute %s.",
                             attr);
                ...
                for (k = 1; k < subQueNode->degree; k++) {
                    ...
                    /* Make the condition */
                    res0 = evaluateExpression3(
                        node->subtrees[0], 0, 0, rei, reiSaveFlag & ~DISCARD_EXPRESSION_RESULT, env, errmsg, r);
                    ...
                    if (strcmp(node->text, "between") == 0) {
                        res1 = evaluateExpression3(
                            node->subtrees[1], 0, 0, rei, reiSaveFlag & ~DISCARD_EXPRESSION_RESULT, env, errmsg, r);
                        ...
                        if (((nodeType0 == T_DOUBLE || nodeType0 == T_INT) && nodeType1 != T_DOUBLE &&
                             nodeType1 != T_INT) ||
                            (nodeType0 == T_STRING && nodeType1 != T_STRING)) {
                            generateAndAddErrMsg(
                                "dynamic type error", node->subtrees[1], RE_DYNAMIC_TYPE_ERROR, errmsg);
                            deleteHashTable(queCondHashTable, nop);
                            freeGenQueryInp(&genQueryInp);
                            clearMsParam(&genQInpParam, 0);
                            return newErrorRes(r, RE_DYNAMIC_TYPE_ERROR);
                 ...
                 Region* r)
{
    ...
    Res* val =
        evaluateExpression3((Node*) subtrees[1], 0, 1, rei, reiSaveFlag & ~DISCARD_EXPRESSION_RESULT, env, errmsg, r);
...
{
    Res* res = newRes(r);
    ExprType* elemType = n == 0 ? newSimpType(T_UNSPECED, r) : params[0]->exprType;
    /* allocate memory for elements */
    res->exprType = newCollType(elemType, r);
    res->degree = n;
...
Res* smsi_elem(Node** params, int, Node* node, ruleExecInfo_t*, int, Env*, rError_t* errmsg, Region* r)
{
    ...
    if (TYPE(params[0]) == T_CONS) {
        ...
        if (index < 0 || index >= getCollectionSize(params[0]->exprType->text, RES_UNINTER_STRUCT(params[0]))) {
            snprintf(
                errbuf, ERR_MSG_LEN, "error: index out of range %d. %s", index, ((Res*) params[0])->exprType->text);
            generateAndAddErrMsg(errbuf, node, RE_RUNTIME_ERROR, errmsg);
            return newErrorRes(r, RE_RUNTIME_ERROR);
        }
        Res* res2 = getValueFromCollection(params[0]->exprType->text, RES_UNINTER_STRUCT(params[0]), index, r);
...
Res* smsi_size(Node** params, int, Node*, ruleExecInfo_t*, int, Env*, rError_t*, Region* r)
{
    ...
    if (TYPE(params[0]) == T_CONS) {
        ...
        RES_INT_VAL_LVAL(res) = getCollectionSize(params[0]->exprType->text, RES_UNINTER_STRUCT(params[0]));
    }
    return res;
}
Res* smsi_datetime(Node** params, int n, Node*, ruleExecInfo_t*, int, Env*, rError_t* errmsg, Region* r)
{
    ...
    char* format;
    if (TYPE(params[0]) == T_STRING && (n == 1 || (n == 2 && TYPE(params[1]) == T_STRING))) {
        if (n == 2) {
        ...
        snprintf(errbuf,
                 ERR_MSG_LEN,
                 "error: unsupported operator or type. can not apply datetime to type (%s[,%s]).",
                 typeName_Res((Res*) params[0]),
                 n == 2 ? typeName_Res((Res*) params[1]) : "null");
...
Res* smsi_timestr(Node** params, int n, Node*, ruleExecInfo_t*, int, Env*, rError_t* errmsg, Region* r)
{
    ...
    char* format;
    newRes(r);
    if (TYPE(params[0]) != T_DATETIME || (n == 2 && TYPE(params[1]) != T_STRING)) {
        res = newErrorRes(r, RE_UNSUPPORTED_OP_OR_TYPE);
        snprintf(errbuf,
                 ERR_MSG_LEN,
                 "error: unsupported operator or type. can not apply datetime to type (%s[,%s]).",
...
{
    Res *val = params[0], *res;
    if (TYPE(val) == T_INT || TYPE(val) == T_DOUBLE || TYPE(val) == T_BOOL || TYPE(val) == T_CONS ||
        TYPE(val) == T_STRING || TYPE(val) == T_PATH || TYPE(val) == T_DATETIME) {
...
Res* smsi_double(Node** params, int, Node* node, ruleExecInfo_t*, int, Env*, rError_t* errmsg, Region* r)
{
    ...
    if (TYPE(val) == T_STRING) {
        ...
        snprintf(errbuf,
                 ERR_MSG_LEN,
                 "error: unsupported operator or type. can not convert %s to double.",
                 typeName_Res(val));
        generateAndAddErrMsg(errbuf, node, RE_UNSUPPORTED_OP_OR_TYPE, errmsg);
    }
    return res;
...
Res* smsi_int(Node** params, int, Node* node, ruleExecInfo_t*, int, Env*, rError_t* errmsg, Region* r)
{
    ...
    if (TYPE(val) == T_STRING) {
        ...
        snprintf(errbuf,
                 ERR_MSG_LEN,
                 "error: unsupported operator or type. can not convert %s to integer.",
                 typeName_Res(val));
        generateAndAddErrMsg(errbuf, node, RE_UNSUPPORTED_OP_OR_TYPE, errmsg);
    }
    return res;
...
Res* smsi_bool(Node** params, int, Node* node, ruleExecInfo_t*, int, Env*, rError_t* errmsg, Region* r)
{
    ...
    if (TYPE(val) == T_BOOL) {
        ...
        snprintf(errbuf,
                 ERR_MSG_LEN,
                 "error: unsupported operator or type. can not convert %s to boolean.",
                 typeName_Res(val));
        generateAndAddErrMsg(errbuf, node, RE_UNSUPPORTED_OP_OR_TYPE, errmsg);
    }
    return res;
...
Res* smsi_lt(Node** params, int, Node* node, ruleExecInfo_t*, int, Env*, rError_t* errmsg, Region* r)
{
    ...
    snprintf(errbuf,
             ERR_MSG_LEN,
             "type error: comparing between %s and %s",
             typeToString(params[0]->exprType, NULL, type0, 128),
             typeToString(params[1]->exprType, NULL, type1, 128));
    generateAndAddErrMsg(errbuf, node, RE_DYNAMIC_TYPE_ERROR, errmsg);
    return newErrorRes(r, RE_DYNAMIC_TYPE_ERROR);
}
                    ...
                    Region* r)
{
    ...
    execCmdOut_t* out;
    if (((mP = getMsParamByLabel(inMsParamArray, "ruleExecOut")) != NULL) && (mP->inOutStruct != NULL) &&
...
int parseResForCollInp(Node* inpParam, collInp_t* collInpCache, collInp_t** outCollInp, int outputToCache)
{
    *outCollInp = NULL;
    ...
    if (inpParam == NULL) {
        rodsLog(LOG_ERROR, "parseMspForCollInp: input inpParam is NULL");
    ...
    if (TYPE(inpParam) == T_STRING) {
        ...
        *outCollInp = collInpCache;
        ...
        rodsLog(LOG_ERROR, "parseMspForCollInp: Unsupported input Param1 type %s", buf);
                          ...
                          Region* r)
{
    ...
    if (rei == NULL || rei->rsComm == NULL) {
        generateAndAddErrMsg(
            "msiCollectionSpider: input rei or rsComm is NULL.", node, SYS_INTERNAL_NULL_INPUT_ERR, errmsg);
    ...
    if (handleInx < 0) {
        ...
        snprintf(buf,
                 ERR_MSG_LEN,
                 "msiCollectionSpider: rsOpenCollection of %s error. status = %d",
                 collInp->collName,
                 handleInx);
...
void getSystemFunctions(Hashtable* ft, Region* r)
{
    ...
    insertIntoHashTable(ft,
                        "applyAllRules",
                        newFunctionFD("e ? * f 0{integer string } => integer * f 1{integer string} => integer->?",
                                      smsi_applyAllRules,
                                      r));
    ...
    /*insertIntoHashTable(ft, "tuple",
            ...
            newFunctionDescChain(newConstructorDesc("A * B * C * D * E * F * G * H-> <A * B * C * D * E * F * G * H>",
       r), newFunctionDescChain(newConstructorDesc("A * B * C * D * E * F * G * H * I-> <A * B * C * D * E * F * G * H *
       I>", r), newConstructorDesc("A * B * C * D * E * F * G * H * I * J-> <A * B * C * D * E * F * G * H * I * J>", r)
            ))))))))));*/
    ...
    insertIntoHashTable(
        ft, ".", newFunctionFD("forall X, `KeyValPair_PI` * expression X->string", smsi_getValByKey, r));
    ...
    insertIntoHashTable(
        ft,
        "==",
        newFunctionFD("forall X in {integer double boolean string time path}, f X * f X->boolean", smsi_eq, r));
    insertIntoHashTable(
        ft,
        "!=",
        newFunctionFD("forall X in {integer double boolean string time path}, f X * f X->boolean", smsi_neq, r));
    insertIntoHashTable(
        ft, ">", newFunctionFD("forall X in {integer double string time}, f X * f X->boolean", smsi_gt, r));
    insertIntoHashTable(
        ft, "<", newFunctionFD("forall X in {integer double string time}, f X * f X->boolean", smsi_lt, r));
    insertIntoHashTable(
        ft, ">=", newFunctionFD("forall X in {integer double string time}, f X * f X->boolean", smsi_ge, r));
    insertIntoHashTable(
        ft, "<=", newFunctionFD("forall X in {integer double string time}, f X * f X->boolean", smsi_le, r));
    ...
    insertIntoHashTable(
        ft, "remoteExec", newFunctionFD("string * string * string * string->integer", smsi_remoteExec, r));
    ...
    insertIntoHashTable(
        ft, "query", newFunctionFD("expression ? -> `GenQueryInp_PI` * `GenQueryOut_PI`", smsi_query, r));
    ...
    insertIntoHashTable(
        ft,
        "collectionSpider",
        newFunctionFD("forall X in {string `CollInpNew_PI`}, expression ? * X * actions ? * actions ? -> integer",
    ...
    /*    insertIntoHashTable(ft, "msiDataObjInfo", newFunctionFD("input `DataObjInp_PI` * output `DataObjInfo_PI` ->
     * integer", smsi_msiDataObjInfo, r));*/
