                             ...
                             Region* r)
{
    ...
    if (file == NULL) {
        snprintf(errbuf, ERR_MSG_LEN, "readRuleSetFromFile() could not open rules file %s\n", rulesFileName);
...
int parseAndComputeMsParamArrayToEnv(msParamArray_t* var,
                                     Env* env,
                                     ruleExecInfo_t* rei,
                                     int reiSaveFlag,
                                     rError_t* errmsg,
...
int parseAndComputeRuleNewEnv(char* rule,
                              ruleExecInfo_t* rei,
                              int reiSaveFlag,
                              msParamArray_t* msParamArray,
                              rError_t* errmsg,
...
int parseAndComputeRule(char* rule, Env* env, ruleExecInfo_t* rei, int reiSaveFlag, rError_t* errmsg, Region* r)
{
    ...
    for (i = tempLen; i < ruleEngineConfig.extRuleSet->len; i++) {
        if (ruleEngineConfig.extRuleSet->rules[i]->ruleType == RK_FUNC ||
            ruleEngineConfig.extRuleSet->rules[i]->ruleType == RK_REL) {
    ...
    for (i = tempLen; i < ruleEngineConfig.extRuleSet->len; i++) {
        if (ruleEngineConfig.extRuleSet->rules[i]->ruleType == RK_FUNC ||
            ...
            Node* errnode;
            ExprType* type = typeRule(ruleEngineConfig.extRuleSet->rules[i],
...
ExprType* typeRule(RuleDesc* rule,
                   Env* funcDesc,
                   Hashtable* varTypes,
                   ...
                   Region* r)
{
    ...
    ExprType* resType =
        typeExpression3(node->subtrees[1], dynamictyping, funcDesc, varTypes, typingConstraints, errmsg, errnode, r);
    /*printf("Type %d\n",resType->t); */
    ...
    resType =
        typeExpression3(node->subtrees[2], dynamictyping, funcDesc, varTypes, typingConstraints, errmsg, errnode, r);
    RE_ERROR(getNodeType(resType) == T_ERROR);
    resType =
        typeExpression3(node->subtrees[3], dynamictyping, funcDesc, varTypes, typingConstraints, errmsg, errnode, r);
    RE_ERROR(getNodeType(resType) == T_ERROR);
...
int typeNode(Node* node, Hashtable* varTypes, rError_t* errmsg, Node** errnode, Region* r)
{
    if ((node->option & OPTION_TYPED) == 0) {
        /*printTree(node, 0); */
        List* typingConstraints = newList(r);
        Res* resType = typeExpression3(
            node, 0, ruleEngineConfig.extFuncDescIndex, varTypes, typingConstraints, errmsg, errnode, r);
        /*printf("Type %d\n",resType->t); */
        if (getNodeType(resType) == T_ERROR) {
