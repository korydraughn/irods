    ...
    while (len_to_read > 0) {
        if (nullptr != _time_value) {
            ...
            if (status == 0) { // the select has timed out
                return ERROR(SYS_SOCK_READ_TIMEDOUT,
                             boost::format("socket timeout with [%d] bytes read") % _bytes_read);
            }
            else if (status < 0) {
                if (errno == EINTR) {
                    continue;
                    ...
                    return ERROR(SYS_SOCK_READ_ERR - errno,
                                 boost::format("error on select after [%d] bytes read") % _bytes_read);
                }
            } // else
        }     // if tv
        ...
        if (num_bytes < 0) {
            if (EINTR == errno) {
                ...
                return ERROR(SYS_SOCK_READ_ERR - errno,
                             boost::format("error reading from socket after [%d] bytes read") % _bytes_read);
            }
        }
        else if (num_bytes == 0) {
            break;
...
irods::error tcp_socket_write(int _socket, const void* _buffer, int _length, int& _bytes_written)
{
    // =-=-=-=-=-=-=-
    ...
    while (len_to_write > 0) {
        int num_bytes = write(_socket, static_cast<const void*>(write_ptr), len_to_write);
        // =-=-=-=-=-=-=-
...
irods::error tcp_read_msg_header(irods::plugin_context& _ctx, void* _buffer, struct timeval* _time_val)
{
    ...
    int bytes_read = 0;
    ret = tcp_socket_read(
        socket_handle, static_cast<void*>(&header_length), sizeof(header_length), bytes_read, _time_val);
    if (!ret.ok()) {
        return PASS(ret);
    }

    if (bytes_read != sizeof(header_length)) {
        return ERROR(SYS_HEADER_READ_LEN_ERR,
                     boost::format("only read [%d] of [%d]") % bytes_read % sizeof(header_length));
    }
    ...
    if (header_length > MAX_NAME_LEN || header_length <= 0) {
        ...
        msg << "header length is out of range: " << header_length << " expected >= 0 and < " << MAX_NAME_LEN;
        return ERROR(SYS_HEADER_READ_LEN_ERR, msg.str());
    }

    // now read the actual header
    ret = tcp_socket_read(socket_handle, _buffer, header_length, bytes_read, _time_val);
...
// =-=-=-=-=-=-=-
//
irods::error tcp_write_msg_header(irods::plugin_context& _ctx, const bytesBuf_t* _header)
{
    // =-=-=-=-=-=-=-
    // check the context
    irods::error ret = _ctx.valid<irods::tcp_object>();
    ...
    if (getRodsLogLevel() >= LOG_DEBUG8) {
        printf("sending header: len = %d\n%.*s\n", _header->len, _header->len, (const char*) _header->buf);
    }
    ...
    int bytes_written = 0;
    ret = tcp_socket_write(socket_handle, &header_length, sizeof(header_length), bytes_written);
    if (!ret.ok() || bytes_written != sizeof(header_length)) {
        std::stringstream msg;
        msg << "wrote " << bytes_written << " expected " << header_length;
        return ERROR(SYS_HEADER_WRITE_LEN_ERR - errno, msg.str());
    }

    // =-=-=-=-=-=-=-
    // now send the actual header
    ret = tcp_socket_write(socket_handle, _header->buf, _header->len, bytes_written);
    if (!ret.ok() || bytes_written != _header->len) {
        std::stringstream msg;
        msg << "wrote " << bytes_written << " expected " << _header->len;
        return ERROR(SYS_HEADER_WRITE_LEN_ERR - errno, msg.str());
    }
...
// =-=-=-=-=-=-=-
//
irods::error tcp_send_rods_msg(irods::plugin_context& _ctx,
                               ...
                               irodsProt_t _protocol)
{
    ...
    // =-=-=-=-=-=-=-
    // send the header
    ret = writeMsgHeader(tcp, &msg_header);
    if (!ret.ok()) {
    ...
    if (_msg_buf && _msg_buf->len > 0) {
        ...
        ret = tcp_socket_write(socket_handle, _msg_buf->buf, _msg_buf->len, bytes_written);
        if (!ret.ok()) {
            return PASS(ret);
        }
    ...
    if (_error_buf && _error_buf->len > 0) {
        ...
        ret = tcp_socket_write(socket_handle, _error_buf->buf, _error_buf->len, bytes_written);
        if (!ret.ok()) {
            return PASS(ret);
        }
    ...
    if (_stream_bbuf && _stream_bbuf->len > 0) {
        ...
        ret = tcp_socket_write(socket_handle, _stream_bbuf->buf, _stream_bbuf->len, bytes_written);
        if (!ret.ok()) {
            return PASS(ret);
        }
                            ...
                            struct timeval* _time_val)
{
    ...
    if (!_buffer || !_buffer->buf) {
        return ERROR(SYS_READ_MSG_BODY_INPUT_ERR, "null buffer ptr");
    }

    int bytes_read = 0;

    // read buffer
    irods::error ret = tcp_socket_read(_socket_handle, _buffer->buf, _length, bytes_read, _time_val);
...
irods::error tcp_read_msg_body(irods::plugin_context& _ctx,
                               msgHeader_t* _header,
                               ...
                               struct timeval* _time_val)
{
    ...
    // trap header ptr
    if (!_header) {
        return ERROR(SYS_READ_MSG_BODY_INPUT_ERR, "null header ptr");
    }
    ...
    if (0 != _input_struct_buf) {
        if (_header->msgLen > 0) {
            ...
            ret = read_bytes_buf(socket_handle, _header->msgLen, _input_struct_buf, _protocol, _time_val);
            if (!ret.ok()) {
                return PASS(ret);
            }
        }
    ...
    if (0 != _error_buf) {
        if (_header->errorLen > 0) {
            ...
            ret = read_bytes_buf(socket_handle, _header->errorLen, _error_buf, _protocol, _time_val);
            if (!ret.ok()) {
                return PASS(ret);
            }
        }
    ...
    if (0 != _bs_buf) {
        if (_header->bsLen > 0) {
            ...
            ret = read_bytes_buf(socket_handle, _header->bsLen, _bs_buf, _protocol, _time_val);
            if (!ret.ok()) {
                return PASS(ret);
            }
        }
...
extern "C" irods::network* plugin_factory(const std::string& _inst_name, const std::string& _context)
{
    ...
    tcp_network_plugin* tcp = new tcp_network_plugin(_inst_name, _context);
    ...
    using namespace std;
    tcp->add_operation<rodsEnv*>(NETWORK_OP_CLIENT_START, function<error(plugin_context&, rodsEnv*)>(tcp_client_start));
    tcp->add_operation<rodsEnv*>(NETWORK_OP_CLIENT_STOP, function<error(plugin_context&, rodsEnv*)>(tcp_client_stop));
    tcp->add_operation(NETWORK_OP_AGENT_START, function<error(plugin_context&)>(tcp_agent_start));
    tcp->add_operation(NETWORK_OP_AGENT_STOP, function<error(plugin_context&)>(tcp_agent_stop));
    tcp->add_operation<void*, struct timeval*>(
        NETWORK_OP_READ_HEADER, function<error(plugin_context&, void*, struct timeval*)>(tcp_read_msg_header));
    tcp->add_operation<msgHeader_t*, bytesBuf_t*, bytesBuf_t*, bytesBuf_t*, irodsProt_t, struct timeval*>(
        ...
        function<error(
            ...
            tcp_read_msg_body));
    tcp->add_operation<const bytesBuf_t*>(NETWORK_OP_WRITE_HEADER,
                                          function<error(plugin_context&, const bytesBuf_t*)>(tcp_write_msg_header));
