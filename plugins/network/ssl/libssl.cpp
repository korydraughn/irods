...
static DH* ssl_get_dh2048()
{
    static unsigned char dh2048_p[] = {
        ,
    };
...
static int ssl_verify_callback(int ok, X509_STORE_CTX* store)
{
    ...
    if (!ok) {
        ...
        rodsLog(LOG_NOTICE, "ssl_verify_callback:   err %i:%s", err, X509_verify_cert_error_string(err));
    }
...
static SSL_CTX* ssl_init_context(char* certfile, char* keyfile)
{
    ...
    if (status < 0) {
        rodsLog(LOG_ERROR, "ssl_init_context - failed in getRodsEnv : %d", status);
        return NULL;
    }
...
{
    BIO* bio = BIO_new_socket(_socket_handle, BIO_NOCLOSE);
    if (bio == NULL) {
...
irods::error ssl_socket_read(int _socket,
                             void* _buffer,
                             int _length,
                             int& _bytes_read,
...
irods::error ssl_socket_write(const void* _buffer, int _length, int& _bytes_written, SSL* _ssl)
{
    irods::error result = SUCCESS();
...
irods::error ssl_read_msg_header(irods::plugin_context& _ctx, void* _buffer, struct timeval* _time_val)
{
    ...
    if ((result = ASSERT_PASS(ret, "Invalid SSL plugin context.")).ok()) {
        ...
        int bytes_read = 0;
        ret = ssl_socket_read(
            socket_handle, static_cast<void*>(&header_length), sizeof(int), bytes_read, _time_val, ssl_obj->ssl());
        if (!ret.ok() || bytes_read != sizeof(header_length)) {
            ...
            msg << "read " << bytes_read << " expected " << sizeof(header_length);
        ...
        if ((result = ASSERT_ERROR(header_length <= MAX_NAME_LEN && header_length > 0,
                                   SYS_HEADER_READ_LEN_ERR,
                                   "Header length is out of range: %d expected >= 0 and < %d.",
                                   header_length,
                                   MAX_NAME_LEN))
                .ok()) {
            // =-=-=-=-=-=-=-
            ...
            if (!ret.ok() || bytes_read != header_length) {
                int status = 0;
                if (bytes_read < 0) {
                    status = bytes_read - errno;
                }
                ...
                std::stringstream msg;
                msg << "read " << bytes_read << " expected " << header_length;
                return ERROR(status, msg.str());
            }
            ...
            if (getRodsLogLevel() >= LOG_DEBUG8) {
                printf("received header: len = %d\n%s\n", header_length, static_cast<char*>(_buffer));
            }
        }
    }
                ...
                if (!(result = ASSERT_ERROR(status >= 1, SSL_HANDSHAKE_ERROR, err_str.c_str())).ok()) {
                    ...
                    if (!(result = ASSERT_ERROR(status, SSL_CERT_ERROR, err_str.c_str())).ok()) {
                        ...
                        if ((result = ASSERT_ERROR(_ctx.prop_map().has_entry(SHARED_KEY),
                                                   -1,
                                                   "irodsEncryption error. Failed to generate key."))
                                .ok()) {
                            // =-=-=-=-=-=-=-
                            ...
                            if ((result = ASSERT_ERROR(
                                     0 != msg_header.msgLen && 0 != msg_header.errorLen && 0 != msg_header.bsLen,
                                     -1,
                                     "irodsEncryption error. Key size, salt size or num hash rounds is 0."))
                                    .ok()) {
                                if ((result = ASSERT_ERROR(EVP_get_cipherbyname(msg_header.type),
                                                           -1,
                                                           "irods_encryption_algorithm \"%s\" is invalid.",
                                                           msg_header.type))
                                        .ok()) {
                                    // =-=-=-=-=-=-=-
...
irods::error ssl_agent_start(irods::plugin_context& _ctx)
{
    ...
    if ((result = ASSERT_PASS(ret, "Invalid SSL plugin context.")).ok()) {
        ...
        SSL_CTX* ctx = ssl_init_context(env.irodsSSLCertificateChainFile, env.irodsSSLCertificateKeyFile);
...
// =-=-=-=-=-=-=-
//
irods::error ssl_write_msg_header(irods::plugin_context& _ctx, const bytesBuf_t* _header)
{
    irods::error result = SUCCESS();
    ...
    if ((result = ASSERT_PASS(ret, "Invalid SSL plugin context.")).ok()) {
        ...
        if ((result = ASSERT_ERROR(ret.ok() && bytes_written == sizeof(header_length),
                                   status,
                                   "Wrote %d expected %d.",
                                   bytes_written,
                                   header_length))
            ...
            result = ASSERT_ERROR(ret.ok() && bytes_written == _header->len,
                                  status,
                                  "Wrote %d expected %d.",
                                  bytes_written,
                                  _header->len);
...
// =-=-=-=-=-=-=-
//
irods::error ssl_send_rods_msg(irods::plugin_context& _ctx,
...
irods::error read_bytes_buf(int _socket_handle,
                            int _length,
                            bytesBuf_t* _buffer,
                            irodsProt_t _protocol,
                            ...
                            SSL* _ssl)
{
    ...
    if ((result = ASSERT_ERROR(_buffer, SYS_READ_MSG_BODY_INPUT_ERR, "Null buffer pointer.")).ok()) {
        ...
        // =-=-=-=-=-=-=-
        // trap failed read
        if (!(result = ASSERT_ERROR(ret.ok() && bytes_read == _length,
...
irods::error ssl_read_msg_body(irods::plugin_context& _ctx,
                               msgHeader_t* _header,
        ...
        if ((result = ASSERT_ERROR(_header, SYS_READ_MSG_BODY_INPUT_ERR, "Null header pointer.")).ok()) {
            ...
            if (0 != _input_struct_buf) {
                if (_header->msgLen > 0) {
                    ...
                    ret = read_bytes_buf(
                        socket_handle, _header->msgLen, _input_struct_buf, _protocol, _time_val, ssl_obj->ssl());
            ...
            if (result.ok()) {
                ...
                if (0 != _error_buf) {
                    if (_header->errorLen > 0) {
                        ...
                        ret = read_bytes_buf(
                            socket_handle, _header->errorLen, _error_buf, _protocol, _time_val, ssl_obj->ssl());
                        result = ASSERT_PASS(ret, "Failed reading from SSL buffer.");
                    }
                    else {
                        _error_buf->len = 0;
                ...
                if (result.ok()) {
                    ...
                    if (0 != _bs_buf) {
                        if (_header->bsLen > 0) {
                            ...
                            ret = read_bytes_buf(
                                socket_handle, _header->bsLen, _bs_buf, _protocol, _time_val, ssl_obj->ssl());
                            result = ASSERT_PASS(ret, "Failed reading from SSL buffer.");
                        }
                        else {
                            _bs_buf->len = 0;
...
extern "C" irods::network* plugin_factory(const std::string& _inst_name, const std::string& _context)
{
    // =-=-=-=-=-=-=-
    // create a ssl network object
    ssl_network_plugin* ssl = new ssl_network_plugin(_inst_name, _context);
    ...
    using namespace std;
    ssl->add_operation<rodsEnv*>(NETWORK_OP_CLIENT_START, function<error(plugin_context&, rodsEnv*)>(ssl_client_start));
    ssl->add_operation<rodsEnv*>(NETWORK_OP_CLIENT_STOP, function<error(plugin_context&, rodsEnv*)>(ssl_client_stop));
    ssl->add_operation(NETWORK_OP_AGENT_START, function<error(plugin_context&)>(ssl_agent_start));
    ssl->add_operation(NETWORK_OP_AGENT_STOP, function<error(plugin_context&)>(ssl_agent_stop));
    ssl->add_operation<void*, struct timeval*>(
        NETWORK_OP_READ_HEADER, function<error(plugin_context&, void*, struct timeval*)>(ssl_read_msg_header));
    ssl->add_operation<msgHeader_t*, bytesBuf_t*, bytesBuf_t*, bytesBuf_t*, irodsProt_t, struct timeval*>(
        NETWORK_OP_READ_BODY,
        function<error(
            plugin_context&, msgHeader_t*, bytesBuf_t*, bytesBuf_t*, bytesBuf_t*, irodsProt_t, struct timeval*)>(
            ssl_read_msg_body));
    ssl->add_operation<const bytesBuf_t*>(NETWORK_OP_WRITE_HEADER,
                                          function<error(plugin_context&, const bytesBuf_t*)>(ssl_write_msg_header));
    ssl->add_operation<const char*, const bytesBuf_t*, const bytesBuf_t*, const bytesBuf_t*, int, irodsProt_t>(
        NETWORK_OP_WRITE_BODY,
        function<error(
            plugin_context&, const char*, const bytesBuf_t*, const bytesBuf_t*, const bytesBuf_t*, int, irodsProt_t)>(
            ssl_send_rods_msg));
