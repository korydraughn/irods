...
TEST_CASE("#????: filesystem does not expand group membership when listing permissions on data objects")
{
    ...
    // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
    const auto sandbox = fs::path{env.rodsHome} / "unit_testing_sandbox__issue_NNNN";
    ...
    irods::at_scope_exit remove_users{[&conn, &alice, &bob] {
        adm::client::remove_user(conn, alice);
        adm::client::remove_user(conn, bob);
    }};
    ...
    irods::at_scope_exit remove_groups{[&conn, &alice_group, &bob_group] {
        adm::client::remove_group(conn, alice_group);
        adm::client::remove_group(conn, bob_group);
    }};
    ...
    adm::client::add_user_to_group(conn, alice_group, bob);
    adm::client::add_user_to_group(conn, bob_group, alice);

    // Create a data object and adjust the permissions such that only the
    ...
    fs::client::permissions(conn, data_object, alice_group.name, fs::perms::read);
    fs::client::permissions(conn, data_object, bob_group.name, fs::perms::read);
    fs::client::permissions(conn, data_object, env.rodsUserName, fs::perms::null);
    ...
    // Show that retrieval of permissions for groups are not expanded.
    const auto status = fs::client::status(conn, data_object);
    CHECK(status.permissions().size() == 2);
